...
A deep dive on `src/parse/sexp.cpp`, the embedded mission-scripting virtual
...
The reader was carved out of `sexp.cpp` into **`src/parse/sexp_reader.cpp`**, a
...
in `src/parse/sexp_reader.cpp`, consumed by the game; verified node-for-node
