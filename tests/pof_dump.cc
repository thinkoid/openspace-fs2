...
static void
dump_vec(const char *tag, vector *v)
{
    printf("  %s %.6g %.6g %.6g\n", tag, v->x, v->y, v->z);
}

static void
dump_model(char *name)
{
    ...
    printf("  submodels %d details %d debris %d\n", pm->n_models,
           pm->n_detail_levels, pm->num_debris_objects);
    printf("  rad %.6g core_rad %.6g mass %.6g\n", pm->rad, pm->core_radius,
           pm->mass);
    dump_vec("mins", &pm->mins);
    dump_vec("maxs", &pm->maxs);
    dump_vec("center_of_mass", &pm->center_of_mass);
    printf(
        "  guns %d missiles %d docks %d thrusters %d paths %d eyes %d textures %d shield_tris %d\n",
        pm->n_guns, pm->n_missiles, pm->n_docks, pm->n_thrusters, pm->n_paths,
        pm->n_view_positions, pm->n_textures, pm->shield.ntris);

    for (int i = 0; i < pm->n_models; i++) {
        bsp_info *sm = &pm->submodel[i];
        printf(
            "  sub %2d %-24s parent %2d children %2d rad %.6g bsp %d off %.6g %.6g %.6g\n",
            i, sm->name, sm->parent, sm->num_children, sm->rad, sm->bsp_data_size,
...
bsp_defpoints(const ubyte *p)
{
    ...
    for (int n = 0; n < nverts; n++) {
        ...
        Bsp_points[n] = pos;
        printf("        v %d %.6g %.6g %.6g norms %d", n, pos.x, pos.y, pos.z,
               nnorm);
...
bsp_flatpoly(const ubyte *p)
{
    ...
    printf(
        "      flat nv %d normal %.6g %.6g %.6g center %.6g %.6g %.6g rad %.6g rgb %d %d %d\n",
...
bsp_tmappoly(const ubyte *p)
{
    ...
    printf(
        "      tmap nv %d normal %.6g %.6g %.6g center %.6g %.6g %.6g rad %.6g tmap %d\n",
        nv, norm.x, norm.y, norm.z, cen.x, cen.y, cen.z, rad, tmap);
    const ubyte *verts =
        p + 44; // nv * model_tmap_vert{short vertnum,normnum; float u,v}
    printf("        verts");
    for (int i = 0; i < nv; i++) {
        const ubyte *v = verts + i * 12;
        printf(" %d/%d(%.6g,%.6g)", rd_short(v), rd_short(v + 2), rd_float(v + 4),
               rd_float(v + 8));
    }
    printf("\n");
}
...
static void
bsp_walk(const ubyte *p, int version, int depth)
{
    if (depth > 256) {
        printf("      !! bsp recursion too deep\n");
        return;
    }
    ...
    for (;;) {
        ...
        case BOP_SORTNORM: {
            ...
            if (pre)
                bsp_walk(p + pre, version, depth + 1);
            if (back)
                bsp_walk(p + back, version, depth + 1);
            if (on)
                bsp_walk(p + on, version, depth + 1);
            if (front)
                bsp_walk(p + front, version, depth + 1);
            if (post)
                bsp_walk(p + post, version, depth + 1);
            break;
        }
        default:
...
dump_model_full(char *name)
{
    ...
    for (int r = 0; r < 3; r++)
        printf("    %.6g %.6g %.6g\n", pm->moment_of_inertia.a2d[r][0],
               pm->moment_of_inertia.a2d[r][1], pm->moment_of_inertia.a2d[r][2]);
    ...
    for (int i = 0; i < pm->n_models; i++) {
        ...
        printf("    offset %.6g %.6g %.6g\n", sm->offset.x, sm->offset.y,
               sm->offset.z);
        printf("    geometric_center %.6g %.6g %.6g rad %.6g\n",
               sm->geometric_center.x, sm->geometric_center.y,
               sm->geometric_center.z, sm->rad);
        printf("    min %.6g %.6g %.6g max %.6g %.6g %.6g\n", sm->min.x,
               sm->min.y, sm->min.z, sm->max.x, sm->max.y, sm->max.z);
    ...
    for (int i = 0; i < pm->n_guns; i++) {
        ...
        for (int s = 0; s < b->num_slots; s++)
            printf("    pnt %.6g %.6g %.6g norm %.6g %.6g %.6g\n", b->pnt[s].x,
                   b->pnt[s].y, b->pnt[s].z, b->norm[s].x, b->norm[s].y,
                   b->norm[s].z);
    }
    printf("missiles %d\n", pm->n_missiles);
    ...
    for (int i = 0; i < pm->n_docks; i++) {
        ...
        printf("  bay %d \"%s\" type 0x%x slots %d spline_paths %d\n", i, d->name,
               d->type_flags, d->num_slots, d->num_spline_paths);
        for (int s = 0; s < d->num_slots; s++)
            printf("    pnt %.6g %.6g %.6g norm %.6g %.6g %.6g\n", d->pnt[s].x,
                   d->pnt[s].y, d->pnt[s].z, d->norm[s].x, d->norm[s].y,
                   d->norm[s].z);
    }
    printf("thrusters %d\n", pm->n_thrusters);
    ...
    for (int i = 0; i < pm->n_paths; i++) {
        ...
        printf(
            "  path %d \"%s\" parent \"%s\"(%d) type %d value %d goal %d nverts %d\n",
            i, pa->name, pa->parent_name, pa->parent_submodel, pa->type,
            pa->value, pa->goal, pa->nverts);
        for (int v = 0; v < pa->nverts; v++)
            printf("    vert %d %.6g %.6g %.6g radius %.6g\n", v,
                   pa->verts[v].pos.x, pa->verts[v].pos.y, pa->verts[v].pos.z,
                   pa->verts[v].radius);
    }
    ...
    for (int i = 0; i < pm->num_ins; i++) {
        ...
        printf("  ins %d detail_level %d num_faces %d offset %.6g %.6g %.6g\n", i,
               in->detail_level, in->num_faces, in->offset.x, in->offset.y,
               in->offset.z);
        for (int f = 0; f < in->num_faces; f++)
            printf(
                "    face %d verts %d %d %d u %.6g %.6g %.6g v %.6g %.6g %.6g\n",
                f, in->faces[f][0], in->faces[f][1], in->faces[f][2], in->u[f][0],
                in->u[f][1], in->u[f][2], in->v[f][0], in->v[f][1], in->v[f][2]);
    }
...
static void
app(std::string &line, const char *fmt, ...)
{
    char buf[512];
    va_list args;
    va_start(args, fmt);
...
static void
app_f(std::string &line, float f)
{
    app(line, "%.9g", canon(f));
}

static void
app_vec(std::string &line, const vector &v)
{
    app(line, "(%.9g %.9g %.9g)", canon(v.x), canon(v.y), canon(v.z));
}

static void
put(const std::string &line)
{
    printf("%s\n", line.c_str());
}
...
static void
model_append(std::vector< canon_poly > &out, const ubyte *p, bool textured)
{
    vector norm = rd_vec(p + 8);
    vector cen = rd_vec(p + 20);
    float rad = rd_float(p + 32);
...
static void
model_walk(const ubyte *p, std::vector< canon_poly > &out, int depth)
{
    if (depth > 256)
        return;
    ...
    for (;;) {
        ...
        case BOP_DEFPOINTS:
            ...
            break;
        case BOP_FLATPOLY:
            model_append(out, p, false);
            break;
        case BOP_TMAPPOLY:
            model_append(out, p, true);
            break;
        case BOP_BOUNDBOX:
            break;
        case BOP_SORTNORM: {
            ...
            int front = rd_int(p + 36), post = rd_int(p + 48);
            if (pre)
                model_walk(p + pre, out, depth + 1);
            if (back)
                model_walk(p + back, out, depth + 1);
            if (on)
                model_walk(p + on, out, depth + 1);
            if (front)
                model_walk(p + front, out, depth + 1);
            if (post)
                model_walk(p + post, out, depth + 1);
            break;
        }
        default:
            return;
...
dump_model_model(char *name)
{
    ...
    for (size_t i = 0; i < subs.size(); i++)
        if (subs[i].subobj_num == -1) {
            fprintf(stderr,
                    "pof_dump: SPCL name collides with submodel "
                    "\"%s\" in %s\n",
    ...
    line = "min ";
    app_vec(line, pm->mins);
    put(line);
    line.clear();
    line = "max ";
    app_vec(line, pm->maxs);
    put(line);
    line.clear();
    line = "mass ";
    app_f(line, pm->mass);
    ...
    line.clear();
    line = "mass_center ";
    app_vec(line, pm->center_of_mass);
    put(line);
    line.clear();

    line = "moi ";
    app_vec(line, pm->moment_of_inertia.rvec);
    line += ' ';
    app_vec(line, pm->moment_of_inertia.uvec);
    line += ' ';
    ...
    for (int i = 0; i < pm->n_models; i++) {
        ...
        app_f(line, sm->rad);
        line += " offset ";
        app_vec(line, sm->offset);
        line += " center ";
        app_vec(line, sm->geometric_center);
        put(line);
        line.clear();
    ...
    for (size_t i = 0; i < subs.size(); i++) {
        ...
        if (ss->turret_num_firing_points <= 0)
            continue;
        app(line, "  turret sub %d arm %d normal ", ss->subobj_num,
            ss->turret_gun_sobj);
    ...
    for (int i = 0; i < pm->shield.ntris; i++) {
        ...
        app_vec(line, t->norm);
        app(line, " verts %d %d %d neighbors %d %d %d", t->verts[0], t->verts[1],
            t->verts[2], t->neighbors[0], t->neighbors[1], t->neighbors[2]);
        put(line);
        line.clear();
    }
    ...
    for (int i = 0; i < pm->n_paths; i++) {
        ...
        printf("  path %d \"%s\" parent \"%s\" sub %d verts %d\n", i, pa->name,
               pa->parent_name, pa->parent_submodel, pa->nverts);
        for (int v = 0; v < pa->nverts; v++) {
            app(line, "    vert %d ", v);
    ...
    for (int i = 0; i < pm->num_ins; i++) {
        ...
        for (int f = 0; f < in->num_faces; f++) {
            ...
            for (int k = 0; k < 3; k++) {
                ...
                vector pos = (idx >= 0 && idx < MAX_INS_VECS) ? in->vecs[idx]
                                                              : vector();
                app(line, " (%.9g %.9g %.9g %.9g %.9g)", canon(pos.x),
                    canon(pos.y), canon(pos.z), canon(in->u[f][k]),
                    canon(in->v[f][k]));
            }
            put(line);
            line.clear();
...
static bool
wanted(const std::string &name, const std::vector< std::string > &filters)
{
    if (filters.empty())
        return true;
...
main(int argc, char *argv[])
{
    ...
    if (game_root == NULL) {
        fprintf(stderr,
                "usage: pof_dump [--full|--model] <game-root> [model.pof ...]\n");
        return 2;
    }
    ...
    cf_get_file_list_preallocated(1024, arr, list, CF_TYPE_MODELS, (char *)"*",
                                  CF_SORT_NONE);
    ...
    for (const std::string &name : full_names)
        if (name.size() > 4 &&
            !strcasecmp(name.c_str() + name.size() - 4, ".pof") &&
            wanted(name, filters)) {
            if (model)
