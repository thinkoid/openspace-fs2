...
static int
fb_count_rect(int x0, int y0, int x1, int y1, int *common)
{
    int hist[256];
    int n = 0;
...
main(int argc, char *argv[])
{
    ...
    gr_set_bitmap(tex);
    int rc2 = g3_draw_poly(4, vlist2,
                           TMAP_FLAG_TEXTURED | TMAP_FLAG_CORRECT |
                               TMAP_FLAG_RAMP | TMAP_FLAG_GOURAUD);
    ...
    gr_set_bitmap(tex);
    g3_draw_laser(&lhead, 1.5f, &ltail, 1.5f,
    ...
    if (out) {
        ...
        for (int y = 0; y < gr_screen.max_h; y++)
            fwrite((ubyte *)gr_screen.offscreen_buffer + y * gr_screen.rowsize, 1,
                   gr_screen.max_w, out);
        fclose(out);
    }
    ...
    if (mn >= 0) {
        ...
        angles
            rots[7] = {
                ...
                { 0.0f, 1.57f, 0.0f }, { 1.57f, 0.0f, 0.3f },
        ...
        for (int k = 0; k < 7; k++) {
            ...
            if (mout) {
                fprintf(mout, "P5\n%d %d\n255\n", gr_screen.max_w,
                        gr_screen.max_h);
                for (int y = 0; y < gr_screen.max_h; y++)
                    fwrite((ubyte *)gr_screen.offscreen_buffer +
                               y * gr_screen.rowsize,
                           1, gr_screen.max_w, mout);
                fclose(mout);
            }
    ...
    // --- 7: laser storm: bolts at many angles, some near the camera ---
    {
        ...
        for (int k = 0; k < 60; k++) {
            ...
            float z0 = 0.02f + (float)(rand() % 100) /
                                   20.0f; // some heads nearly at the eye
            vector head = { cosf(ang) * 2.0f, sinf(ang) * 2.0f, z0 };
            vector tail = { cosf(ang) * 6.0f, sinf(ang) * 6.0f, z0 + 30.0f };
            g3_draw_laser(&head, 0.6f, &tail, 0.4f,
                          TMAP_FLAG_TEXTURED | TMAP_FLAG_XPARENT);
        }
