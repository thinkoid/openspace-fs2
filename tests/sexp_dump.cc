...
static void
emit_tree(int n, int depth)
{
    if (n < 0)
        return;
    printf("%*s[%d,%d,%08x] %s\n", depth * 2, "", SEXP_NODE_TYPE(n),
           Sexp_nodes[n].subtype, (unsigned)(Sexp_nodes[n].type & 0xffff0000),
           Sexp_nodes[n].text);
    ...
    for (;;) {
        ...
        if (*Mp == '(') {
            ...
            if (root >= 0) {
                ...
                free_sexp2(
                    root); // one tree at a time keeps the 2200-node pool ample
                trees++;
            }
        }
...
main(int argc, char *argv[])
{
    ...
    for (auto pattern : { "*.fs2", "*.fc2" }) {
        int n = cf_get_file_list_preallocated(MAX_ORACLE_MISSIONS, mission_arr,
                                              mission_list, CF_TYPE_MISSIONS,
                                              (char *)pattern, CF_SORT_NAME);
        for (int i = 0; i < n; i++) {
