...
parse_rank_tbl()
{
    ...
    while (required_string_either("#End", "$Name:")) {
        ...
        stuff_string(Ranks[idx].promotion_voice_base, F_NAME, NULL,
                     MAX_FILENAME_LEN - 2);
...
void
init_scoring_element(scoring_struct *s)
{
    int i;
...
//XSTR:OFF
void
scoring_eval_harbison(ship *shipp)
{
    FILE *fp;

    if (!stricmp(shipp->ship_name, "alpha 2") &&
        (!stricmp(Game_current_mission_filename, "demo01") ||
...
void
scoring_level_init(scoring_struct *scp)
{
    int i;
...
void
scoring_eval_rank(scoring_struct *sc)
{
...
void
scoring_eval_badges(scoring_struct *sc)
{
    ...
    total_kills = 0;
    for (i = 0; i < MAX_SHIP_TYPES; i++) {
        if ((Ship_info[i].flags & SIF_FIGHTER) ||
            (Ship_info[i].flags & SIF_BOMBER)) {
...
void
scoring_do_accept(scoring_struct *score)
{
    int idx;
    ...
    for (idx = 0; idx < MAX_SHIP_TYPES; idx++) {
        score->kills[idx] = (unsigned short)(score->kills[idx] +
                                             score->m_okKills[idx]);
    }
...
// after debriefing
void
scoring_backout_accept(scoring_struct *score)
{
    int idx;
...
void
scoring_level_close(int accepted)
{
    // want to calculate any other statistics.
    if (The_mission.game_type == MISSION_TYPE_TRAINING) {
    ...
    if (accepted) {
        ...
        if ((The_mission.flags & MISSION_FLAG_NO_PROMOTION) ||
            ...
            if (Player->stats.m_badge_earned != -1) {
                Player->stats.medals[Badge_index[Player->stats.m_badge_earned]] =
                    -1;
...
void
scoring_add_damage(object *ship_obj, object *other_obj, float damage)
{
...
void
scoring_eval_kill(object *ship_obj)
{
    ...
    if ((max_damage_pct >= KILL_PERCENTAGE) &&
        ...
        if (plr != NULL) {
            ...
            if (Ships[Objects[plr->objnum].instance].team == dead_ship->team) {
                ...
                plr->stats.m_score -= (int)(dead_ship->score *
...
void
scoring_eval_assists(ship *sp, int killer_sig)
{
    int idx;
    player *plr;
    ...
    for (idx = 0; idx < MAX_DAMAGE_SLOTS; idx++) {
        ...
        if ((sp->damage_ship[idx] / sp->total_damage_received) >=
            ...
            if ((plr != NULL) &&
                (sp->team != Ships[Objects[plr->objnum].instance].team) &&
...
void
scoring_eval_hit(object *hit_obj, object *other_obj, int from_blast)
{
    ...
    if ((other_obj->type == OBJ_WEAPON) &&
        ...
        int is_bonehead = 0;
        int sub_type =
            Weapon_info[Weapons[other_obj->instance].weapon_info_index].subtype;

        // determine if this was a bonehead hit or not
        if (hit_obj->type == OBJ_SHIP) {
            is_bonehead = Ships[hit_obj->instance].team ==
...
void
scoring_bash_rank(player *pl, int rank)
{
