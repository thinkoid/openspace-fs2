...
#include <ship/ship.hh> // for targeting subobjects
#include <mission/missioncampaign.hh> // for mission/campaign stuff
...
typedef struct player
{
    ...
    char short_callsign[CALLSIGN_LEN +
                        1]; // callsign truncated to SHORT_CALLSIGN_PIXEL_W pixels
    ...
    char squad_filename
        [MAX_FILENAME_LEN]; // filename of the squad image for this pilot
    char squad_name[NAME_LENGTH + 1]; // pilot's squadron name
    int num_campaigns; // tells how many array entries in the campaigns field
    char current_campaign
        [MAX_FILENAME_LEN]; // Name of the currently active campaign, or zero-length string if none
    campaign_info *
        campaigns; // holds information regarding all active campaigns the player is playing
    ...
    ship_subsys
        *locking_subsys; // Subsystem pointer that missile lock is trying to seek
    ...
    char killer_parent_name
        [NAME_LENGTH]; // name of parent object that killed the player
    ...
    multi_local_options
        m_local_options; // inert multiplayer options block - kept only for pilot file compatibility
    multi_server_options
        m_server_options; // inert multiplayer options block - kept only for pilot file compatibility
...
extern void
player_controls_init(); // initialize Descent style controls for use in various places
