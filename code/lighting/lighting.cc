...
typedef struct light
{
    ...
    vector
        vec; // location in world space of a point light or the direction of a directional light or the first point on the tube for a tube light
    ...
    float rad1,
        rad1_squared; // How big of an area a point light affect.  Is equal to l->intensity / MIN_LIGHT;
    float rad2,
        rad2_squared; // How big of an area a point light affect.  Is equal to l->intensity / MIN_LIGHT;
...
// Ignored for now.
void
light_set_ambient(float ambient_light)
{ }

void
light_add_directional(vector *dir, float intensity, float r, float g, float b)
{
    light *l;
...
void
light_add_point(vector *pos, float rad1, float rad2, float intensity, float r,
                float g, float b, int ignore_objnum)
{
    light *l;
...
void
light_add_point_unique(vector *pos, float rad1, float rad2, float intensity,
                       float r, float g, float b, int affected_objnum)
{
    light *l;
...
void
light_add_tube(vector *p0, vector *p1, float r1, float r2, float intensity,
               float r, float g, float b, int affected_objnum)
{
    light *l;
...
int
light_filter_push(int objnum, vector *pos, float rad)
{
    int i;
    light *l;
...
int
is_inside(vector *min, vector *max, vector *p0, float rad)
{
...
int
light_filter_push_box(vector *min, vector *max)
{
    int i;
    light *l;
    ...
    for (i = 0; i < Num_relevent_lights[n1]; i++) {
        ...
        case LT_TUBE:
            if (is_inside(min, max, &l->local_vec, l->rad2) ||
                is_inside(min, max, &l->local_vec2, l->rad2)) {
                Relevent_lights[Num_relevent_lights[n2]++][n2] = l;
            }
            break;
...
int
light_get_global_count()
{
    return Static_light_count;
}

int
light_get_global_dir(vector *pos, int n)
{
...
void
light_set_shadow(int state)
{
    Light_in_shadow = state;
}

ubyte
light_apply(vector *pos, vector *norm, float static_light_level)
{
    int i, idx;
    float lval;
    light *l;
...
void
light_apply_rgb(ubyte *param_r, ubyte *param_g, ubyte *param_b, vector *pos,
                vector *norm, float static_light_level)
{
    int i, idx;
    ...
    for (i = 0; i < Num_relevent_lights[n]; i++) {
        ...
        // tube lights
        case LT_TUBE:
            if (vm_vec_dist_to_line(pos, &l->local_vec, &l->local_vec2, &temp,
                                    &dist) != 0) {
                continue;
            }
