...
void ai_big_chase_attack(ai_info *aip, ship_info *sip, vector *enemy_pos,
                         float dist_to_enemy);
void ai_big_avoid_ship();
int ai_big_maybe_follow_subsys_path(int do_dot_check = 1);

extern int model_which_octant_distant_many(vector *pnt, int model_num,
                                           matrix *model_orient,
                                           vector *model_pos, polymodel **pm,
                                           int *octs);
extern void compute_desired_rvec(vector *rvec, vector *goal_pos, vector *cur_pos);
extern void big_ship_collide_recover_start(object *objp, object *big_objp,
                                           vector *collide_pos,
...
void
ai_bpap(object *objp, vector *attacker_objp_pos, vector *attacker_objp_fvec,
        vector *attack_point, vector *local_attack_point, float fov,
        float weapon_travel_dist, vector *surface_normal)
{
    float nearest_dist;
    ...
    model_which_octant_distant_many(attacker_objp_pos,
                                    Ships[objp->instance].modelnum, &objp->orient,
                                    &objp->pos, &pm, octs);
    ...
    for (q = 0; q < 4; q++) {
        ...
        if (octp->nverts > 0) {
            ...
            for (i = 0; i < num_tries; i++) {
                ...
                dist = vm_vec_normalized_dir(&v2p, &result_point,
                                             attacker_objp_pos);
                ...
                if (dot > fov) {
                    if (dist < nearest_dist) {
                        ...
                        *local_attack_point = rel_point;
                        ...
                        if (dot >
                            (1.0f + fov) /
                                2.0f) //        If this point is quite good, quit searching for a better one.
                            goto done_1;
                    }
                }
            }
...
void
ai_big_pick_attack_point_turret(object *objp, ship_subsys *ssp, vector *gpos,
                                vector *gvec, vector *attack_point, float fov,
                                float weapon_travel_dist)
{
    if (!timestamp_elapsed(ssp->turret_pick_big_attack_point_timestamp)) {
        vector result_point;
        vm_vec_unrotate(&result_point, &ssp->turret_big_attack_point,
                        &objp->orient);
        vm_vec_add(attack_point, &result_point, &objp->pos);
    }
    else {
        vector local_attack_point;
        ssp->turret_pick_big_attack_point_timestamp = timestamp(
            2000 + (int)(frand() * 500.0f));
        ai_bpap(objp, gpos, gvec, attack_point, &local_attack_point, fov,
                weapon_travel_dist, NULL);
...
void
ai_big_pick_attack_point(object *objp, object *attacker_objp,
                         vector *attack_point, float fov)
{
    ...
    case OBJ_SHIP: {
        ...
        if (!timestamp_elapsed(attacker_aip->pick_big_attack_point_timestamp)) {
            ...
            vm_vec_unrotate(&result_point, &attacker_aip->big_attack_point,
                            &objp->orient);
            vm_vec_add(attack_point, &result_point, &objp->pos);
        ...
        attacker_aip->pick_big_attack_point_timestamp = timestamp(
            2000 + (int)(frand() * 500.0f));
        break;
    }
    case OBJ_WEAPON: {
        ...
        wp->pick_big_attack_point_timestamp = timestamp(2000 +
                                                        (int)(frand() * 500.0f));
    ...
    ai_bpap(objp, &attacker_objp->pos, &attacker_objp->orient.fvec, attack_point,
            &local_attack_point, fov, 99999.9f, &surface_normal);
...
ai_big_evade_ship()
{
    ...
    else if ((box_dist > EVADE_BOX_MIN_DISTANCE) &&
...
void
ai_big_subsys_path_cleanup(ai_info *aip)
{
...
int
ai_big_maybe_follow_subsys_path(int do_dot_check)
{
    ai_info *aip;
    ...
    if ((aip->targeted_subsys != NULL) && (aip->target_objnum >= 0) &&
        ...
        if (timestamp_elapsed(aip->path_subsystem_next_check)) {
            ...
            model_find_world_point(&geye, &ep->pnt,
                                   Ships[Pl_objp->instance].modelnum, 0,
                                   &Pl_objp->orient, &Pl_objp->pos);

            // get world pos of subsystem
            vm_vec_unrotate(&gsubpos, &aip->targeted_subsys->system_info->pnt,
                            &En_objp->orient);
            ...
            if (ship_subsystem_in_sight(En_objp, aip->targeted_subsys, &geye,
                                        &gsubpos, 1)) {
                subsys_in_sight = 1;
            }
        }
        ...
        min_dot = (target_objp->phys_info.fspeed > 5.0f
        ...
        if ((checked_sight && ((!subsys_in_sight) || (dot < min_dot)))) {
            ...
            if ((aip->path_start) == -1 || (aip->mp_index != subsys_path_num)) {
                ...
                if (subsys_path_num >= 0) {
                    ...
                    ai_find_path(Pl_objp, aip->target_objnum, subsys_path_num, 0,
                                 1);
                    if (aip->path_start >= 0) {
                        aip->ai_flags |= AIF_ON_SUBSYS_PATH;
                    }
...
int
ai_big_maybe_start_strafe(ai_info *aip, ship_info *sip)
{
        ...
        if (timestamp_elapsed(aip->scan_for_enemy_timestamp)) {
            ...
            while (so != END_OF_LIST(&Ship_obj_list)) {
                ...
                if (test_sp->team != Ships[Pl_objp->instance].team) {
                    if (Ship_info[test_sp->ship_info_index].flags &
                        SIF_SMALL_SHIP) {
                        dist_squared = vm_vec_dist_squared(&Pl_objp->pos,
                                                           &test_objp->pos);
...
void
ai_big_chase_attack(ai_info *aip, ship_info *sip, vector *enemy_pos,
                    float dist_to_enemy)
{
    int start_bank;
    ...
    if (((time_to_hit = ai_endangered_by_weapon(aip)) < 4.0f) &&
        ...
        if (Pl_objp->phys_info.speed < 3.0f) {
            ...
            for (objp = GET_FIRST(&obj_used_list);
                ...
                if ((objp->type == OBJ_WEAPON) && (Weapons[objp->instance].team !=
                    ...
                    if (Weapon_info[objp->instance].subtype == WP_LASER) {
                        ...
                        if (vm_vec_dot(&in_vec, &objp->orient.fvec) > 0.0f) {
                            ...
                            if ((dist < 200.0f) &&
                                (vm_vec_dot(&in_vec, &objp->orient.fvec) >
                                 0.95f)) {
                                if ((Objects[objp->parent].signature ==
                                     objp->parent_sig) &&
                                    (vm_vec_dist_quick(
                                         &objp->pos, &Objects[objp->parent].pos) <
                                     300.0f)) {
            ...
            if (ai_big_maybe_start_strafe(aip, sip)) {
                ...
                aip->mode = AIM_STRAFE;
                aip->submode_parm0 =
                    Missiontime; // use parm0 as time strafe mode entered
        ...
        dist_to_enemy = vm_vec_normalized_dir(&vec_to_enemy, enemy_pos,
                                              &Pl_objp->pos);
        ...
        ai_turn_towards_vector(enemy_pos, Pl_objp, flFrametime,
                               sip->srotation_time, NULL, rel_pos, 0.0f, 0, rvec);

        // calc range of primary weapon
        weapon_travel_dist = ai_get_weapon_dist(
            &Ships[Pl_objp->instance].weapons);
...
extern void ai_select_secondary_weapon(object *objp, ship_weapon *swp,
                                       int priority1 = -1, int priority2 = -1);
extern float set_secondary_fire_delay(ai_info *aip, ship *shipp,
                                      weapon_info *swip);
extern void ai_choose_secondary_weapon(object *objp, ai_info *aip,
                                       object *en_objp);
extern int maybe_avoid_big_ship(object *objp, object *ignore_objp, ai_info *aip,
...
ai_big_maybe_fire_weapons(float dist_to_enemy, float dot_to_enemy,
                          vector *firing_pos, vector *enemy_pos,
                          vector *enemy_vel)
{
    ai_info *aip;
    ship_weapon *swp;
    ...
    if (dot_to_enemy > 0.95f - 0.5f * En_objp->radius /
        ...
        if (dot_to_enemy >
            ...
            if (tswp->num_primary_banks > 0) {
                ...
                weapon_info *wip =
                    &Weapon_info
                        [tswp->primary_bank_weapons[tswp->current_primary_bank]];
                ...
                if (!(En_objp->flags & OF_PROTECTED)) {
                    ...
                    weapon_info *swip =
                        &Weapon_info[tswp->secondary_bank_weapons[current_bank]];

                    //  If ship is protected and very low on hits, don't fire missiles.
                    if ((current_bank > -1) &&
                        (!(En_objp->flags & OF_PROTECTED) ||
                        ...
                        if (aip->ai_flags & AIF_UNLOAD_SECONDARIES) {
                            if (timestamp_until(
                                ...
                                swp->next_secondary_fire_stamp[current_bank] =
                                    timestamp((int)(swip->fire_wait * 1000.0f));
                            }
                        }
                        ...
                        if (timestamp_elapsed(
                            ...
                            float t =
                                0.25f; //       default delay in seconds until next fire.
...
void
ai_big_switch_to_chase_mode(ai_info *aip)
{
    aip->previous_mode = aip->mode;
    aip->mode = AIM_CHASE;
    aip->submode = SM_ATTACK;
...
ai_big_chase()
{
    ...
    if ((dot_to_enemy < 0.25f) || (aip->target_time < 1.0f) ||
        ...
        update_aspect_lock_information(
            aip, &vec_to_enemy, dist_to_enemy - En_objp->radius, En_objp->radius);
    }
        ...
        dist_to_enemy = vm_vec_normalized_dir(&vec_to_enemy, &enemy_pos,
                                              &player_pos); // - En_objp->radius;
        dot_to_enemy = vm_vec_dot(&vec_to_enemy, &Pl_objp->orient.fvec);
        update_aspect_lock_information(aip, &vec_to_enemy, dist_to_enemy,
                                       En_objp->radius);
    }
    else if (En_objp->flags &
             OF_PROTECTED) { // If protected and we're not attacking a subsystem, stop attacking!
        update_aspect_lock_information(
            aip, &vec_to_enemy, dist_to_enemy - En_objp->radius, En_objp->radius);
        aip->target_objnum = -1;
        if (find_enemy(Pl_objp - Objects, MAX_ENEMY_DISTANCE,
                       Skill_level_max_attackers[Game_skill_level]) == -1) {
        ...
        if (aip->target_time < 2.0f)
            if ((dot_to_enemy < 0.9f) || (dist_to_enemy > 300.0f)) {
                ...
                aip->submode_start_time =
                    Missiontime -
                    fl2f(
                        2.75f); //      This backdated start time allows immediate switchout.
                if (dot_to_enemy > 0.0f)
    ...
    if (aip->submode != SM_AVOID && aip->submode != SM_EVADE) {
        ...
        if (dist_to_enemy < ATTACK_COLLIDE_BASE_DIST) {
            ...
            if ((dist_normal_to_enemy < ATTACK_COLLIDE_AVOID_DIST + speed_dist) ||
                (time_to_enemy < ATTACK_COLLIDE_AVOID_TIME)) {
                // get away, simulate crsh recovery (don't use avoid)
                //                              accelerate_ship(aip, -1.0f);
                big_ship_collide_recover_start(Pl_objp, En_objp, &Pl_objp->pos,
                                               NULL);
    ...
    if (aip->mode != AIM_EVADE && aip->path_start == -1) {
        ai_big_maybe_fire_weapons(dist_to_enemy, dot_to_enemy, &player_pos,
                                  &predicted_enemy_pos, &En_objp->phys_info.vel);
    }
    else
        aip->time_enemy_in_range *= (1.0f - flFrametime);
}
...
void
ai_big_attack_get_data(vector *enemy_pos, float *dist_to_enemy,
                       float *dot_to_enemy)
{
    ...
    *dot_to_enemy = vm_vec_dot(&vec_to_enemy, &Pl_objp->orient.fvec);
    if ((*dot_to_enemy < 0.25f) || (aip->target_time < 1.0f) ||
        ...
        set_predicted_enemy_pos_turret(&predicted_enemy_pos, &gun_pos, Pl_objp,
                                       enemy_pos, &En_objp->phys_info.vel,
                                       weapon_speed, aip->time_enemy_in_range);
    }

    *dist_to_enemy = vm_vec_normalized_dir(&vec_to_enemy, &predicted_enemy_pos,
                                           &player_pos);
    *dot_to_enemy = vm_vec_dot(&vec_to_enemy, &Pl_objp->orient.fvec);
    update_aspect_lock_information(aip, &vec_to_enemy, *dist_to_enemy,
                                   En_objp->radius);
...
// AIS_STRAFE_RETREAT
int
ai_big_strafe_maybe_retreat(float dist, vector *target_pos)
{
    ai_info *aip;
    aip = &Ai_info[Ships[Pl_objp->instance].ai_index];
    ...
    if (vm_vec_mag_quick(&aip->big_attack_surface_normal) > 0.9) {
        dist_normal_to_target = -vm_vec_dotprod(&vec_to_target,
                                                &aip->big_attack_surface_normal);
    }
    else {
...
ai_big_strafe_attack()
{
    ...
    if (aip->ai_flags & AIF_KAMIKAZE) {
        if (target_dist < 1200.0f) {
            ...
            ai_turn_towards_vector(
                &target_pos, Pl_objp, flFrametime,
    ...
    if (!(aip->ai_flags & AIF_SEEK_LOCK) || (aip->aspect_locked_time < 2.0f)) {
        ...
        if (t > 0.0f && t < 1.5f) {
            ...
            switch (rand() % 4) {
            case 0:
                vm_vec_scale_add(&rand_vec, &Pl_objp->orient.fvec,
                                 &Pl_objp->orient.uvec, 2.0f);
                break;
            case 1:
                vm_vec_scale_add(&rand_vec, &Pl_objp->orient.fvec,
                                 &Pl_objp->orient.uvec, -2.0f);
    ...
    ai_big_maybe_fire_weapons(target_dist, target_dot, &Pl_objp->pos,
                              &En_objp->pos, &En_objp->phys_info.vel);
...
ai_big_strafe_retreat1()
{
    ...
    if (Missiontime - aip->submode_start_time > fl2f(1.50f)) {
        ...
        case 3:
            vm_vec_scale_add(&rand_vec, &Pl_objp->orient.fvec,
                             &Pl_objp->orient.rvec, -1.0f);
            break;
        } // end switch

        //vm_vec_scale(&rand_vec, 200.0f);
        //vm_vec_add2(&aip->prev_goal_point, &rand_vec);
        vm_vec_scale_add(&aip->prev_goal_point, &aip->goal_point, &rand_vec,
                         200.0f);
...
// so, enter AIM_STRAFE
int
ai_big_maybe_enter_strafe_mode(object *pl_objp, int weapon_objnum,
...
//                              weapon_objp     =>
void
ai_big_strafe_maybe_attack_turret(object *ship_objp, object *weapon_objp)
{
    ai_info *aip;
    object *parent_objp;
    ...
    // attack the turret
    set_targeted_subsys(aip, Weapons[weapon_objp->instance].turret_subsys,
