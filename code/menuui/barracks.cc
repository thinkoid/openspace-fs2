...
// stats defines
// fixed lines plus one per ship class with kills
#define NUM_STAT_LINES (21 + MAX_SHIP_TYPES)
...
// pilot stats field
static int Barracks_stats_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                              32, 212, 240, 250 },
                                                            { // GR_1024
                                                              42, 351, 240,
                                                              400 } };
...
// pilot picture field
static int Barracks_image_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                              461, 23, 160, 120 },
                                                            { // GR_1024
                                                              782, 58, 160,
                                                              120 } };
...
static int Barracks_image_number_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                                     461, 145 },
                                                                   { // GR_1024
                                                                     732, 239 } };
...
// B_PILOT_CREATE_BOTTON
#define B_PILOT_CREATE_BOTTON 0
// B_PILOT_SCROLL_UP_BUTTON
#define B_PILOT_SCROLL_UP_BUTTON 1
// B_PILOT_SCROLL_DOWN_BUTTON
#define B_PILOT_SCROLL_DOWN_BUTTON 2
// B_PILOT_B_PILOT_DELETE_BUTTON
#define B_PILOT_DELETE_BUTTON 11
// B_PILOT_B_PILOT_SET_ACTIVE_BUTTON
#define B_PILOT_SET_ACTIVE_BUTTON 12
// B_PILOT_B_PILOT_CLONE_BUTTON
#define B_PILOT_CLONE_BUTTON 13
// B_PILOT_SINGLE_MODE_BUTTON
#define B_PILOT_SINGLE_MODE_BUTTON 14
// B_PILOT_MULTI_MODE_BUTTON
#define B_PILOT_MULTI_MODE_BUTTON 15
// B_PILOT_B_PILOT_CONVERT_BUTTON
...
// B_PILOT_B_PIC_PREV_PILOT_BUTTON
#define B_PIC_PREV_PILOT_BUTTON 3
// B_PILOT_B_PIC_NEXT_PILOT_BUTTON
#define B_PIC_NEXT_PILOT_BUTTON 4

// pilot stat buttons
// B_STATS_MEDAL_BUTTON
#define B_STATS_MEDAL_BUTTON 8
// B_STATS_SCROLL_DOWN_BUTTON
#define B_STATS_SCROLL_DOWN_BUTTON 9
// B_STATS_SCROLL_UP_BUTTON
...
void
barracks_init_stats(scoring_struct *stats)
{
    int i;
    float f;
...
void
barracks_set_hotkeys(bool pilot_text_enter_mode)
{
    ...
    Buttons[gr_screen.res][B_PILOT_CREATE_BOTTON].button.set_hotkey(
...
void
barracks_set_callsign_enter_mode(bool set_callsign_enter_mode)
{
...
barracks_scroll_stats_down()
{
    ...
    if (Stats_scroll_offset +
            Barracks_stats_coords[gr_screen.res][BARRACKS_H_COORD] / font_height <
        Num_stat_lines) {
        Stats_scroll_offset++;
        gamesnd_play_iface(SND_SCROLL);
    }
...
barracks_prev_squad_pic()
{
    ...
    if ((Pic_squad_number >= 0) && (Pic_squad_number < Num_pilot_squad_images)) {
        strcpy(Cur_pilot->squad_filename,
...
int
barracks_pilot_filter(char *filename)
{
    int r, rank;

    r = verify_pilot_file(filename, Player_sel_mode == PLAYER_SELECT_MODE_SINGLE,
                          &rank);
...
void
barracks_init_player_stuff(int mode)
{
    ...
    Num_pilots = cf_get_file_list_preallocated(MAX_PILOTS, Pilots_arr, Pilots,
                                               CF_TYPE_SINGLE_PLAYERS,
    ...
    Buttons[gr_screen.res][B_SQUAD_PREV_BUTTON].button.set_disabled_action(
        barracks_squad_change_popup);
    Buttons[gr_screen.res][B_SQUAD_NEXT_BUTTON].button.hide();
    Buttons[gr_screen.res][B_SQUAD_NEXT_BUTTON].button.disable();
    Buttons[gr_screen.res][B_SQUAD_NEXT_BUTTON].button.set_disabled_action(
        barracks_squad_change_popup);
...
barracks_button_pressed(int n)
{
    ...
    case B_PILOT_SET_ACTIVE_BUTTON:
        if (barracks_new_pilot_selected()) {
            ...
            popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                  XSTR("You must create a single player pilot.", 66));
        }
        else {
            gamesnd_play_iface(SND_SCROLL);
        }
...
void
barracks_display_pilot_callsigns(int prospective_pilot)
{
    int y = 0;
    ...
    while (y + font_height <=
        ...
        if (!stricmp(Cur_pilot->callsign, Pilots[cur_pilot_idx])) {
            if ((cur_pilot_idx == Selected_line) ||
                (cur_pilot_idx == prospective_pilot)) {
        ...
        gr_bitmap(Barracks_list_coords[gr_screen.res][BARRACKS_X_COORD] - 34,
                  Barracks_list_coords[gr_screen.res][BARRACKS_Y_COORD] + y);
...
barracks_display_pilot_stats()
{
    ...
    while (y + font_height <=
        ...
        if (*str == '*') {
            ...
            gr_line(Barracks_stats_coords[gr_screen.res][BARRACKS_X_COORD], i,
                    Barracks_stats_coords[gr_screen.res][BARRACKS_X_COORD] +
                        ...
                        w - 2,
                    i);
            gr_line(Barracks_stats_coords[gr_screen.res][BARRACKS_X_COORD] +
                        Barracks_stats_coords[gr_screen.res][BARRACKS_W_COORD] +
                        1,
                    i,
        ...
        gr_printf(Barracks_stats_coords[gr_screen.res][BARRACKS_X_COORD] +
    ...
    for (i = 1; i < Num_pilots; i++) {
        if (!stricmp(buf, Pilots[i])) {
            ...
            if (pilot_verify_overwrite() == 1) {
                ...
                Num_pilots--;
                delete_pilot_file(
                    name, Player_sel_mode == PLAYER_SELECT_MODE_SINGLE ? 1 : 0);
                z = 0;
            }
            return;
        }
...
barracks_init()
{
    ...
    // add all strings
    w->add_XSTR("Create", 1034, Buttons[gr_screen.res][0].text_x,
                Buttons[gr_screen.res][0].text_y,
                &Buttons[gr_screen.res][0].button, UI_XSTR_COLOR_GREEN);
    w->add_XSTR("Accept", 1035, Buttons[gr_screen.res][5].text_x,
                Buttons[gr_screen.res][5].text_y,
                &Buttons[gr_screen.res][5].button, UI_XSTR_COLOR_PINK);
    w->add_XSTR("Help", 928, Buttons[gr_screen.res][6].text_x,
                Buttons[gr_screen.res][6].text_y,
                &Buttons[gr_screen.res][6].button, UI_XSTR_COLOR_GREEN);
    w->add_XSTR("Options", 1036, Buttons[gr_screen.res][7].text_x,
                Buttons[gr_screen.res][7].text_y,
                &Buttons[gr_screen.res][7].button, UI_XSTR_COLOR_GREEN);
    w->add_XSTR("Medals", 1037, Buttons[gr_screen.res][8].text_x,
                Buttons[gr_screen.res][8].text_y,
                &Buttons[gr_screen.res][8].button, UI_XSTR_COLOR_GREEN);
    w->add_XSTR("Remove", 1038, Buttons[gr_screen.res][11].text_x,
                Buttons[gr_screen.res][11].text_y,
                &Buttons[gr_screen.res][11].button, UI_XSTR_COLOR_GREEN);
    w->add_XSTR("Select", 1552, Buttons[gr_screen.res][12].text_x,
                Buttons[gr_screen.res][12].text_y,
                &Buttons[gr_screen.res][12].button, UI_XSTR_COLOR_GREEN);
    w->add_XSTR("Clone", 1040, Buttons[gr_screen.res][13].text_x,
                Buttons[gr_screen.res][13].text_y,
                &Buttons[gr_screen.res][13].button, UI_XSTR_COLOR_GREEN);
    w->add_XSTR("Single", 1041, Buttons[gr_screen.res][14].text_x,
                Buttons[gr_screen.res][14].text_y,
                &Buttons[gr_screen.res][14].button, UI_XSTR_COLOR_GREEN);
    w->add_XSTR("Multi", 1042, Buttons[gr_screen.res][15].text_x,
                Buttons[gr_screen.res][15].text_y,
                &Buttons[gr_screen.res][15].button, UI_XSTR_COLOR_GREEN);
    w->add_XSTR("Convert", 1043, Buttons[gr_screen.res][16].text_x,
                Buttons[gr_screen.res][16].text_y,
                &Buttons[gr_screen.res][16].button, UI_XSTR_COLOR_GREEN);
    for (int i = 0; i < BARRACKS_NUM_TEXT; i++) {
        w->add_XSTR(&Barracks_text[gr_screen.res][i]);
    }

    // button for selecting pilot
    List_region.create(&Ui_window, "",
                       Barracks_list_coords[gr_screen.res][BARRACKS_X_COORD],
                       Barracks_list_coords[gr_screen.res][BARRACKS_Y_COORD],
                       Barracks_list_coords[gr_screen.res][BARRACKS_W_COORD],
                       Barracks_list_coords[gr_screen.res][BARRACKS_H_COORD], 0,
                       1);
    List_region.hide();

    // create input box (for new pilot)
    Inputbox.create(&Ui_window,
                    Barracks_list_coords[gr_screen.res][BARRACKS_X_COORD],
                    Barracks_list_coords[gr_screen.res][BARRACKS_Y_COORD],
                    Barracks_list_coords[gr_screen.res][BARRACKS_W_COORD],
                    CALLSIGN_LEN - 1, "",
...
// -----------------------------------------------------------------------------
void
barracks_do_frame(float frametime)
{
    ...
    if ((Pic_squad_number >= 0) && (Pic_squad_number < Num_pilot_squad_images)) {
        if (Pilot_squad_images[Pic_squad_number] ==
            BARRACKS_IMAGE_NOT_LOADED) { // haven't tried loading it yet
            Pilot_squad_images[Pic_squad_number] = bm_load_duplicate(
                Pilot_squad_image_names[Pic_squad_number]);
            if (Pilot_squad_images[Pic_squad_number] >= 0) {
                int w, h;
