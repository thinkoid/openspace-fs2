...
int Main_hall_door_sound_handles[MAX_DOOR_SOUNDS] = { -1, -1, -1, -1, -1, -1 };
...
int Mh_ship_table_status[GR_NUM_RESOLUTIONS][2] = { { 1, 479 }, { 1, 767 } };
int Mh_weapon_table_status[GR_NUM_RESOLUTIONS][2] = { { 3, 479 }, { 3, 767 } };
void
main_hall_blit_table_status()
{
    // blit ship table status
    gr_set_color_fast(Game_ships_tbl_valid ? &Color_bright_green
                                           : &Color_bright_red);
    gr_line(Mh_ship_table_status[gr_screen.res][0],
            Mh_ship_table_status[gr_screen.res][1],
            Mh_ship_table_status[gr_screen.res][0],
            Mh_ship_table_status[gr_screen.res][1]);

    // blit weapon table status
    gr_set_color_fast(Game_weapons_tbl_valid ? &Color_bright_green
                                             : &Color_bright_red);
    gr_line(Mh_weapon_table_status[gr_screen.res][0],
            Mh_weapon_table_status[gr_screen.res][1],
            Mh_weapon_table_status[gr_screen.res][0],
            Mh_ship_table_status[gr_screen.res][1]);
}
...
void
main_hall_init(int main_hall_num)
{
    if (Main_hall_inited) {
        return;
    }
    ...
    snazzy_menu_init();
    read_menu_tbl(NOX("MAIN HALL"), temp, whee, Main_hall_region,
                  &Main_hall_num_options, 0);
    ...
    Main_hall->region_descript[1] = XSTR(
        "Barracks - Manage your FreeSpace 2 pilots", 354);
    Main_hall->region_descript[2] = XSTR(
        "Ready room - Start or continue a campaign", 355);
    Main_hall->region_descript[3] = XSTR(
        "Tech room - View specifications of FreeSpace 2 ships and weaponry", 356);
    Main_hall->region_descript[4] = XSTR(
        "Options - Change your FreeSpace 2 options", 357);
    Main_hall->region_descript[5] = XSTR(
        "Campaign Room - View all available campaigns", 358);
    Main_hall->region_descript[6] = XSTR(
        "Multiplayer - Start or join a multiplayer game", 359);

    // init tooltip shader
    float gray_intensity = 0.02f; // nearly black
    float c = 0.07f;
    gr_create_shader(&Main_hall_tooltip_shader, gray_intensity, gray_intensity,
                     gray_intensity, c);
...
*/
    ...
    mouse_set_pos(Main_hall->door_anim_coords[READY_ROOM_REGION][2],
                  Main_hall->door_anim_coords[READY_ROOM_REGION][3]);
...
main_hall_do(float frametime)
{
    ...
    snazzy_action = -1;
    code = snazzy_menu_do(Main_hall_mask_data, Main_hall_mask_w, Main_hall_mask_h,
                          Main_hall_num_options, Main_hall_region, &snazzy_action,
                          1, &key);
...
//#endif
        ...
        case QUICK_START_REGION:
            ...
            if (Num_recent_missions > 0) {
                ...
                strncpy(Game_current_mission_filename, Campaign.missions[0].name,
                        MAX_FILENAME_LEN);
            }
        ...
        case SKILL_LEVEL_REGION:
            ...
            sprintf(temp, XSTR("Skill level set to %s.", 370),
...
main_hall_close()
{
    ...
    if (Main_hall_mask >= 0) {
        ...
        if (!bm_unload(Main_hall_mask)) {
            nprintf(
                ("General", "WARNING! Couldn't unload main hall mask bitmap!\n"));
        }
    }
    ...
    for (idx = 0; idx < Main_hall->num_misc_animations; idx++) {
        if ((Main_hall_misc_anim_instance[idx] != NULL) &&
            (anim_playing(Main_hall_misc_anim_instance[idx]))) {
            Main_hall_misc_anim_instance[idx] = NULL;
        }
        if (Main_hall_misc_anim[idx] != NULL) {
            if (anim_free(Main_hall_misc_anim[idx]) == -1) {
                nprintf(
                    ("General",
                     "WARNING!, Could not free up misc anim %s in main hall\n",
    ...
    for (idx = 0; idx < Main_hall->num_door_animations; idx++) {
        if ((Main_hall_door_anim_instance[idx] != NULL) &&
            (anim_playing(Main_hall_door_anim_instance[idx]))) {
            Main_hall_door_anim_instance[idx] = NULL;
        }
        if (Main_hall_door_anim[idx] != NULL) {
            if (anim_free(Main_hall_door_anim[idx]) == -1) {
                nprintf(
                    ("General",
                     "WARNING!, Could not free up door anim %s in main hall\n",
                     Main_hall->door_anim_name[idx]));
            }
        }
    }
    ...
    for (idx = 0; idx < Main_hall->num_door_sounds - 2;
         idx++) { // don't cut off the glow sounds (requested by Dan)
        if ((Main_hall_door_sound_handles[idx] != -1) &&
            snd_is_playing(Main_hall_door_sound_handles[idx])) {
...
main_hall_start_music()
{
    ...
    int main_hall_spooled_music_index = event_music_get_spooled_music_index(
        Main_hall->music);

    if ((Main_hall_music_handle == -1) && (main_hall_spooled_music_index != -1)) {
        char *music_wavfile_name =
            Spooled_music[main_hall_spooled_music_index].filename;
        if (music_wavfile_name != NULL) {
            Main_hall_music_handle = audiostream_open(music_wavfile_name,
                                                      ASF_EVENTMUSIC);
            if (Main_hall_music_handle != -1)
                audiostream_play(Main_hall_music_handle,
                                 Master_event_music_volume);
        }
        else {
            nprintf(("Warning",
    ...
    for (idx = 0; idx < Main_hall->num_misc_animations; idx++) {
        ...
        if (Main_hall_misc_anim_instance[idx] == NULL) {
            ...
            if (Main_hall->misc_anim_delay[idx][0] == -1) {
                Main_hall->misc_anim_delay[idx][0] = timestamp(
                ...
                // if the timestamp is not -1 and has popped, play the anim and make the timestap -1
                     ...
                     Main_hall_misc_anim[idx]) {
                anim_play_struct aps;

                anim_play_init(&aps, Main_hall_misc_anim[idx],
                               Main_hall->misc_anim_coords[idx][0],
        ...
        // if the anim is playing
            ...
            for (s_idx = Main_hall->misc_anim_special_sounds[idx][0]; s_idx > 0;
                 s_idx--) {
                // if we've passed the trigger point, then play the sound and break out of the loop
                if ((Main_hall_misc_anim_instance[idx]->frame_num >=
                     Main_hall->misc_anim_special_trigger[idx][s_idx]) &&
                    ...
                    // play the sound
                    Main_hall->misc_anim_sound_handles[idx][s_idx] = snd_play(
                        &Snds_iface[Main_hall
                                        ->misc_anim_special_sounds[idx][s_idx]],
            ...
            if ((Main_hall->misc_anim_modes[idx] == MISC_ANIM_MODE_HOLD) &&
                (Main_hall_misc_anim_instance[idx]->frame_num ==
                 Main_hall_misc_anim_instance[idx]->stop_at)) {
            ...
            if ((Main_hall->misc_anim_modes[idx] == MISC_ANIM_MODE_LOOP) &&
                (Main_hall_misc_anim_instance[idx]->frame_num ==
                 Main_hall_misc_anim_instance[idx]->stop_at)) {
...
// render all playing misc animations
void
main_hall_render_misc_anims(float frametime)
{
    int idx;

    // HACKETY HACK HACK - always render misc anim 3 first, if it is playing
    if (Main_hall_misc_anim_instance[2] != NULL) {
        anim_render_one(GS_STATE_MAIN_MENU, Main_hall_misc_anim_instance[2],
                        frametime);
    }
    ...
    for (idx = 0; idx < MAX_MISC_ANIMATIONS; idx++) {
        ...
        // render it
        if (Main_hall_misc_anim_instance[idx] != NULL) {
            anim_render_one(GS_STATE_MAIN_MENU, Main_hall_misc_anim_instance[idx],
                            frametime);
        }
    }
}
...
// render all playing door animations
void
main_hall_render_door_anims(float frametime)
{
    int idx;
    ...
    for (idx = 0; idx < MAX_DOOR_ANIMATIONS; idx++) {
        // render it
        if (Main_hall_door_anim_instance[idx] != NULL) {
            anim_render_one(GS_STATE_MAIN_MENU, Main_hall_door_anim_instance[idx],
                            frametime);
        }
    }
}
...
main_hall_handle_region_anims()
{
    ...
    for (idx = 0; idx < Main_hall->num_door_animations; idx++) {
        ...
        if ((Main_hall_door_anim_instance[idx] != NULL) &&
            ...
            if ((Main_hall_mouse_region == idx) &&
                (Main_hall_door_anim_instance[idx]->frame_num ==
                 Main_hall_door_anim_instance[idx]->stop_at)) {
            ...
            if ((Main_hall_mouse_region != idx) &&
                (Main_hall_door_anim_instance[idx]->paused)) {
...
// do any necessary processing based upon the mouse location
void
main_hall_handle_mouse_location(int cur_region)
{
    if (Main_hall_frame_skip)
        return;
    ...
    if (cur_region != -1) {
        ...
        if (cur_region == Main_hall_mouse_region) {
            ...
            if (Main_hall_region_linger_stamp == -1) {
                ...
                Main_hall_region_linger_stamp = timestamp(
                    MAIN_HALL_REGION_LINGER);
            }
        }
    }
...
main_hall_mouse_release_region(int region)
{
    ...
    if ((Main_hall_door_anim_instance[region] != NULL) &&
        anim_playing(Main_hall_door_anim_instance[region]) &&
        (Main_hall_door_anim_instance[region]->direction ==
         ANIM_DIRECT_FORWARD)) {
    ...
    if ((Main_hall_door_anim_instance[region] != NULL)) {
        ...
        Main_hall_door_sound_handles[region] = snd_play(
            &Snds_iface[Main_hall->door_sounds[region][1]],
            Main_hall->door_sound_pan[region]);

        // make sure to set the sound to play from the right spot
        snd_set_pos(
            Main_hall_door_sound_handles[region],
            &Snds_iface[SND_MAIN_HALL_DOOR_CLOSE],
            (float)(Main_hall_door_anim_instance[region]->start_at -
                    Main_hall_door_anim_instance[region]->frame_num) /
                (float)Main_hall_door_anim_instance[region]->parent->total_frames,
            1);
    }
}
...
// if the mouse has moved on this region, handle it accordingly
void
main_hall_mouse_grab_region(int region)
{
    if (Main_hall_frame_skip) {
        return;
    }
    ...
    if (!Main_hall_door_anim_instance[region]) {
        if (Main_hall_door_anim[region]) {
            ...
            anim_play_init(&aps, Main_hall_door_anim[region],
                           Main_hall->door_anim_coords[region][0],
                           Main_hall->door_anim_coords[region][1]);
            aps.screen_id = GS_STATE_MAIN_MENU;
            aps.framerate_independent = 1;
    ...
    else if (Main_hall_door_anim_instance[region]->direction ==
             ANIM_DIRECT_REVERSE) {
        anim_reverse_direction(Main_hall_door_anim_instance[region]);
    }
    ...
    Main_hall_door_sound_handles[region] = snd_play(
        &Snds_iface[Main_hall->door_sounds[region][0]],
        Main_hall->door_sound_pan[region]);
    ...
    if (Main_hall_door_anim_instance[region]->frame_num != -1) {
        snd_set_pos(
            Main_hall_door_sound_handles[region],
            &Snds_iface[SND_MAIN_HALL_DOOR_OPEN],
            (float)Main_hall_door_anim_instance[region]->frame_num /
                (float)Main_hall_door_anim_instance[region]->parent->total_frames,
            1);
    }
}
...
main_hall_handle_right_clicks()
{
    ...
    if (!Main_hall_right_click) {
        if (mouse_down(MOUSE_RIGHT_BUTTON)) {
            ...
            mouse_set_pos(Main_hall->door_anim_coords[new_region][2],
                          Main_hall->door_anim_coords[new_region][3]);
...
main_hall_handle_random_intercom_sounds()
{
    ...
    if ((Main_hall_next_intercom_sound_stamp == -1) &&
        (Main_hall_intercom_sound_handle == -1)) {
        Main_hall_next_intercom_sound_stamp = timestamp((
            int)(((float)rand() / (float)RAND_MAX) *
                 (float)(Main_hall
                             ->intercom_delay[Main_hall_next_intercom_sound][1] -
                         Main_hall->intercom_delay[Main_hall_intercom_sound_handle]
                                                  [0])));
    }
    ...
    if (Main_hall_intercom_sound_handle == -1) {
        ...
        if ((Main_hall_next_intercom_sound_stamp != -1) &&
            ...
            // play the sound
            Main_hall_intercom_sound_handle = snd_play(
                &Snds_iface[Main_hall
    ...
    // if the sound is playing
        ...
        if (!snd_is_playing(Main_hall_intercom_sound_handle)) {
            ...
            Main_hall_next_intercom_sound_stamp = timestamp(
                (int)(((float)rand() / (float)RAND_MAX) *
                      (float)(Main_hall
                                  ->intercom_delay[Main_hall_next_intercom_sound]
                                                  [1] -
                              Main_hall
                                  ->intercom_delay[Main_hall_next_intercom_sound]
                                                  [0])));
...
void
main_hall_set_notify_string(char *str)
{
...
main_hall_notify_do()
{
    ...
    if (Main_hall_notify_stamp != -1) {
        ...
        if (timestamp_elapsed(Main_hall_notify_stamp)) {
            ...
            gr_printf((gr_screen.max_w - w) / 2, gr_screen.max_h - 40,
...
main_hall_start_ambient()
{
    ...
    if (play_ambient_loop) {
        Main_hall_ambient_loop = snd_play_looping(
            &Snds_iface[SND_MAIN_HALL_AMBIENT]);
    }
}
...
main_hall_reset_ambient_vol()
{
    if (Main_hall_ambient_loop >= 0) {
        snd_set_volume(Main_hall_ambient_loop,
                       Snds_iface[SND_MAIN_HALL_AMBIENT].default_volume);
    }
}
...
main_hall_maybe_blit_tooltips()
{
    ...
    if (!help_overlay_active(Main_hall_overlay_id)) {
        int shader_y =
            (Main_hall->region_yval) -
            Main_hall_tooltip_padding[gr_screen.res]; // subtract more to pull higher
        // get the width of the string
        gr_get_string_size(&w, NULL, Main_hall->region_descript[text_index]);

        gr_set_shader(&Main_hall_tooltip_shader);
        gr_shade(0, shader_y, gr_screen.clip_width,
                 (gr_screen.clip_height - shader_y));

        gr_set_color_fast(&Color_bright_white);
        gr_string((gr_screen.max_w - w) / 2, Main_hall->region_yval,
                  Main_hall->region_descript[text_index]);
    }
}
...
main_hall_process_help_stuff()
{
    ...
    if ((y_anim_offset >= (2 * Main_hall_tooltip_padding[gr_screen.res]) + h) ||
    ...
    gr_set_color_fast(&Color_bright_white);
    gr_shade(0, 0, gr_screen.max_w,
             (2 * Main_hall_tooltip_padding[gr_screen.res]) + h - y_anim_offset);
    gr_string((gr_screen.max_w - w) / 2,
              Main_hall_tooltip_padding[gr_screen.res] - y_anim_offset, str);
}
...
main_hall_read_table()
{
    ...
    while (!optional_string("#end")) {
        ...
        for (m_idx = 0; m_idx < GR_NUM_RESOLUTIONS; m_idx++) {
            ...
            for (idx = 0; idx < m->num_misc_animations; idx++) {
                ...
                for (s_idx = 0; s_idx < m->misc_anim_special_sounds[idx][0];
                     s_idx++) {
            ...
            for (idx = 0; idx < m->num_misc_animations; idx++) {
                ...
                for (s_idx = 0; s_idx < m->misc_anim_special_trigger[idx][0];
                     s_idx++) {
    ...
    // are we funny?
    if (Vasudan_funny) {
        Main_hall_defines[GR_640][1].door_sounds[OPTIONS_REGION][0] =
            SND_VASUDAN_BUP;
        Main_hall_defines[GR_640][1].door_sounds[OPTIONS_REGION][1] =
            SND_VASUDAN_BUP;
        Main_hall_defines[GR_1024][1].door_sounds[OPTIONS_REGION][0] =
            SND_VASUDAN_BUP;
        Main_hall_defines[GR_1024][1].door_sounds[OPTIONS_REGION][1] =
            SND_VASUDAN_BUP;

        // set head anim. hehe
        strcpy(Main_hall_defines[GR_640][1].door_anim_name[OPTIONS_REGION],
               "vhallheads");
        strcpy(Main_hall_defines[GR_1024][1].door_anim_name[OPTIONS_REGION],
               "2_vhallheads");
