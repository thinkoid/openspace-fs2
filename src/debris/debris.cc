...
debris_start_death_roll(object *debris_obj, debris *debris_p)
{
    if (debris_p->is_hull) {
        ...
        fireball_create(&debris_obj->pos, fireball_type, OBJ_INDEX(debris_obj),
        ...
        if (Missiontime > debris_p->time_started + 2 * F1_0) {
            snd_play_3d(&Snds[SND_MISSILE_IMPACT1], &debris_obj->pos,
                        &View_position, debris_obj->radius);
        }
    }
    ...
    if (vm_vec_dist_quick(&obj->pos, &Eye_position) < obj->radius * 50.0f) {
        for (i = 0; i < MAX_DEBRIS_ARCS; i++) {
            if (timestamp_valid(db->arc_timestamp[i])) {
                model_add_arc(db->model_num, db->submodel_num, &db->arc_pts[i][0],
...
void
debris_clear_expired_flag(debris *db)
{
...
maybe_delete_debris(debris *db)
{
    if (timestamp_elapsed(db->next_distance_check)) {
        if (vm_vec_dist_quick(&Player_obj->pos, &Objects[db->objnum].pos) >
            MAX_DEBRIS_DIST)
            db->lifeleft = 0.1f;
        else
...
void
debris_process_pre(object *objp, float frame_time)
{ }
...
debris_process_post(object *obj, float frame_time)
{
    ...
    if (!timestamp_elapsed(db->fire_timeout) &&
        ...
        db->next_fireball = timestamp_rand(db->arc_frequency,
                                           db->arc_frequency * 2);
        ...
        if (db->is_hull) {
            ...
            vector v1, v2, v3, v4;
            submodel_get_two_random_points(db->model_num, db->submodel_num, &v1,
                                           &v2);
            submodel_get_two_random_points(db->model_num, db->submodel_num, &v3,
                                           &v4);
    ...
    for (i = 0; i < MAX_DEBRIS_ARCS; i++) {
        if (timestamp_valid(db->arc_timestamp[i])) {
            if (timestamp_elapsed(db->arc_timestamp[i])) {
                ...
                if (mr < RAND_MAX / 5) {
                    vector v1, v2;
                    submodel_get_two_random_points(db->model_num,
                                                   db->submodel_num, &v1, &v2);
...
debris_find_oldest()
{
    ...
    for (db = GET_FIRST(&Hull_debris_list); db != END_OF_LIST(&Hull_debris_list);
         db = GET_NEXT(db)) {
        if ((db->time_started < oldest_time) &&
            !(Objects[db->objnum].flags & OF_SHOULD_BE_DEAD)) {
              ...
              vector *exp_center, int hull_flag, float exp_force)
{
    ...
    if (hull_flag) {
        if (rand() <
            RAND_MAX / 6) // Make some pieces blow up shortly after explosion.
    ...
    if (hull_flag)
        flags |= OF_COLLIDES;
    objnum = obj_create(OBJ_DEBRIS, parent_objnum, n, &source_obj->orient, pos,
                        radius, flags);
    if (objnum == -1) {
    ...
    if (hull_flag) {
        ...
        db->fire_timeout = timestamp(
            fl2i(t)); // fireballs last from 5 - 30 seconds
        ...
        if (Objects[db->source_objnum].radius <
            ...
            nprintf((
                "Alan",
                "A forever chunk of debris was created from ship with radius %f\n",
    ...
    vm_vec_unrotate(&world_rotvel, &source_obj->phys_info.rotvel,
                    &source_obj->orient);
    ...
    if (hull_flag) {
        int ship_info_flag =
            Ship_info[Ships[source_obj->instance].ship_info_index].flags;
        if (ship_info_flag & (SIF_SMALL_SHIP | SIF_NOT_FLYABLE | SIF_HARMLESS)) {
            if (vm_vec_mag_squared(&obj->phys_info.vel) >
                MAX_SPEED_SMALL_DEBRIS * MAX_SPEED_SMALL_DEBRIS) {
                float scale = MAX_SPEED_SMALL_DEBRIS /
                              vm_vec_mag(&obj->phys_info.vel);
            ...
            if (vm_vec_mag_squared(&obj->phys_info.vel) >
                MAX_SPEED_CAPITAL_DEBRIS * MAX_SPEED_CAPITAL_DEBRIS) {
                float scale = MAX_SPEED_CAPITAL_DEBRIS /
                              vm_vec_mag(&obj->phys_info.vel);
                vm_vec_scale(&obj->phys_info.vel, scale);
            }
        }
            ...
            Warning(
                LOCATION,
                "Ship has info flag that is not among the following:  SMALL, NOT_FLYABLE, HARMLESS, BIG, CAPITAL, SUPERCAP");
        }
    }
#endif
...
int
debris_check_collision(object *pdebris, object *other_obj, vector *hitpos,
                       collision_info_struct *debris_hit_info)
{
    ...
    if ((vm_vec_mag_squared(&heavy->phys_info.rotvel) * flFrametime *
        ...
        vm_vec_sub(&debris_hit_info->light_rel_vel, &light->phys_info.vel,
                   &heavy->phys_info.vel);
    }
    ...
    if (debris_hit_info->heavy ==
                ...
                for (int i = 0; i < num_rotating_submodels; i++) {
                    ...
                    if (model_collide(&mc)) {
                        if (mc.hit_dist < debris_hit_info->hit_time) {
                            ...
                            set_hit_struct_info(debris_hit_info, &mc,
                                                SUBMODEL_ROT_HIT);
                            model_find_world_point(&debris_hit_info->hit_pos,
            ...
            *mc.p1 = copy_p1;
            ...
            if (model_collide(&mc)) {
                ...
                if (mc.hit_dist < debris_hit_info->hit_time) {
                    ...
                    if (debris_hit_info->edge_hit == 0) {
                        model_find_obj_dir(&debris_hit_info->collision_normal,
                    ...
                    vector diff;
                    vm_vec_sub(&diff, mc.p1, mc.p0);
                    vm_vec_scale_add(&debris_hit_info->light_collision_cm_pos,
        ...
        if (mc_ret_val) {
            ...
            if (!debris_hit_info->edge_hit) {
                vm_vec_unrotate(&debris_hit_info->collision_normal,
            ...
            vector diff;
            vm_vec_sub(&diff, mc.p1, mc.p0);
            vm_vec_scale_add(&debris_hit_info->light_collision_cm_pos, mc.p0,
                             &diff, mc.hit_dist);
        }
    }
    ...
    if (mc_ret_val) {
        ...
        vm_vec_sub(&debris_hit_info->r_light, &debris_hit_info->hit_pos,
                   &debris_hit_info->light_collision_cm_pos);

        // set normal for edge hit
        if (debris_hit_info->edge_hit) {
            vm_vec_copy_normalize(&debris_hit_info->collision_normal,
                                  &debris_hit_info->r_light);
            vm_vec_negate(&debris_hit_info->collision_normal);
        }

        // get world hitpos
        vm_vec_add(hitpos, &debris_hit_info->heavy->pos,
                   &debris_hit_info->r_heavy);
...
void
calc_debris_physics_properties(physics_info *pi, vector *mins, vector *maxs)
{
    float dx, dy, dz, mass;
    dx = maxs->x - mins->x;
    dy = maxs->y - mins->y;
