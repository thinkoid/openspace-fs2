...
void
pack_file(char *filespec, char *filename, int filesize, time_t time_write)
{
    char path[1024];
...
void
add_directory(const char *dirname)
{
    char path[256];
    char *pathptr = path;
    char *tmpptr;
...
*/
    ...
    for (const auto &entry : std::filesystem::directory_iterator(filespec)) {
        ...
        if (S_ISDIR(sb.st_mode)) {
            ...
            pack_file(filespec, (char *)name.c_str(), (int)sb.st_size,
                      sb.st_mtime);
        }
    }
    add_directory("..");
