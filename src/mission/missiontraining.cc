...
int Training_msg_window_coords[GR_NUM_RESOLUTIONS][2] = { { 174, 40 },
                                                          { 379, 125 } };
...
training_obj_display()
{
    ...
    GR_AABITMAP(Directive_gauge[0].first_frame,
                Directive_coords[gr_screen.res][DIRECTIVE_COORDS_TOP][0] +
                    fl2i(HUD_offset_x),
                Directive_coords[gr_screen.res][DIRECTIVE_COORDS_TOP][1] +
                    fl2i(HUD_offset_y));
    ...
    // print out title
    emp_hud_printf(Directive_coords[gr_screen.res][DIRECTIVE_COORDS_TITLE][0] +
                       fl2i(HUD_offset_x),
                   Directive_coords[gr_screen.res][DIRECTIVE_COORDS_TITLE][1] +
                       fl2i(HUD_offset_y),
                   EG_OBJ_TITLE, XSTR("directives", 422));
    ...
    by = Directive_coords[gr_screen.res][DIRECTIVE_COORDS_MIDDLE][1] +
         fl2i(HUD_offset_y);

    y_count = 0;
    for (i = 0; i < end; i++) {
        x = DIRECTIVE_X + 3 + fl2i(HUD_offset_x);
        y = Training_obj_window_coords[gr_screen.res][1] + fl2i(HUD_offset_y) +
            y_count * height + height / 2 + 1;
        ...
        if (second_line) {
            y = Training_obj_window_coords[gr_screen.res][1] +
                fl2i(HUD_offset_y) + y_count * height + height / 2 + 1;
...
training_mission_init()
{
    ...
    if (!Directive_frames_loaded) {
        for (i = 0; i < NUM_DIRECTIVE_GAUGES; i++) {
            Directive_gauge[i].first_frame = bm_load_animation(
                Directive_fnames[i], &Directive_gauge[i].num_frames);
...
training_mission_page_in()
{
    int i;
    for (i = 0; i < NUM_DIRECTIVE_GAUGES; i++) {
        bm_page_in_aabitmap(Directive_gauge[i].first_frame,
                            Directive_gauge[i].num_frames);
    }
}

int
comp_training_lines_by_born_on_date(const void *m1, const void *m2)
{
    int *e1, *e2;
    e1 = (int *)m1;
    e2 = (int *)m2;
    ...
    return (Mission_events[*e1 & 0xffff].born_on_date -
...
sort_training_objectives()
{
    ...
    qsort(Training_obj_lines, Training_obj_num_lines, sizeof(int),
    ...
    for (i = 0; i < offset; i++) {
        ...
        if (event_status == EVENT_CURRENT) {
            ...
            if (f2i(Missiontime -
                    Mission_events[TRAINING_OBJ_LINES_MASK(i)].satisfied_time) <
                MIN_SATISFIED_TIME) {
                Training_obj_lines[i] |= TRAINING_OBJ_STATUS_UNKNOWN;
                num_offset_events++;
            }
...
training_check_objectives()
{
    ...
    for (event_idx = 0; event_idx < Num_mission_events; event_idx++) {
        ...
        if ((event_status != EVENT_UNBORN) &&
            ...
            (timestamp() > Mission_events[event_idx].born_on_date + 3000)) {
            if (!Training_failure ||
                !strnicmp(Mission_events[event_idx].name,
...
int
message_play_training_voice(int index)
{
    int len;
    CFILE *fp;
        ...
        if (len > 100000) {
            if ((Training_voice < 0) || !Training_voice_type ||
                ...
                if (stricmp(Message_waves[index].name, NOX("none.wav"))) {
                    Training_voice_handle = audiostream_open(
...
// voice file playing
void
message_training_setup(int m, int length)
{
    if ((m < 0) ||
    ...
    if (message_play_training_voice(Messages[m].wave_info.index) < 0) {
        if (length > 0)
            ...
            Training_msg_timestamp = timestamp(
                TRAINING_TIMING_BASE +
                strlen(Messages[m].message) *
...
void
message_training_que(char *text, int timestamp, int length)
{
    int m;
...
message_training_que_check()
{
    ...
    for (i = 0; i < Training_msg_que_count; i++) {
        if (timestamp_elapsed(Training_msg_que[i].timestamp)) {
            message_training_setup(Training_msg_que[i].num,
                                   Training_msg_que[i].length);
...
message_training_display()
{
    ...
    Training_num_lines = split_str(Training_buf, TRAINING_LINE_WIDTH,
                                   Training_line_sizes, Training_lines,
                                   MAX_TRAINING_MSG_LINES);
    Assert(Training_num_lines > 0);
    for (i = 0; i < Training_num_lines; i++) {
        Training_lines[i][Training_line_sizes[i]] = 0;
    ...
    gr_shade(Training_msg_window_coords[gr_screen.res][0],
             Training_msg_window_coords[gr_screen.res][1],
             TRAINING_MSG_WINDOW_WIDTH, Training_num_lines * height + height);
    ...
    for (i = 0; i < Training_num_lines;
        ...
        z = 0;
        x = Training_msg_window_coords[gr_screen.res][0] +
            (TRAINING_MSG_WINDOW_WIDTH - TRAINING_LINE_WIDTH) / 2;
        y = Training_msg_window_coords[gr_screen.res][1] + i * height +
            height / 2 + 1;

        while (*str) { // loop through each character of each line
            if ((count < MAX_TRAINING_MSG_MODS) &&
                (str == Training_msg_mods[count].pos)) {
                buf[z] = 0;
                gr_printf(x, y, buf);
