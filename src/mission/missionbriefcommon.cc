...
int Brief_text_max_lines[GR_NUM_RESOLUTIONS] = { 6, 6 };
...
static float
    Total_move_time; // time in which camera should move from current pos to target pos (seconds)
...
static colored_char Colored_text[MAX_TEXT_STREAMS][MAX_BRIEF_LINES]
                                [MAX_BRIEF_LINE_LEN];
...
color *Brief_text_colors[MAX_BRIEF_TEXT_COLORS] = {
    &Color_white,       &Color_bright_white, &Color_red,
    &Color_green,       &Color_yellow,       &Color_blue,
    &Brief_color_green, &Brief_color_red,    &IFF_colors[IFF_COLOR_NEUTRAL][0],
};
...
void brief_maybe_create_new_grid(grid *gridp, vector *pos, matrix *orient,
                                 int force = 0);
grid *brief_create_grid(grid *gridp, vector *forward, vector *right,
                        vector *center, int nrows, int ncols, float square_size);
...
extern void get_camera_limits(matrix *start_camera, matrix *end_camera,
                              float time, vector *acc_max, vector *w_max);
int brief_text_wipe_finished();

void
brief_set_icon_color(int team)
{
    switch (team) {
    case TEAM_FRIENDLY:
        SET_COLOR_FRIENDLY;
...
int
brief_icon_used_in_briefing(int icon_type)
{
...
void
brief_preload_highlight_anim(brief_icon *bi)
{
    hud_anim *ha;
...
void
brief_preload_fade_anim(brief_icon *bi)
{
    hud_anim *ha;
    int species = ship_get_species_by_type(bi->ship_class);
...
float
brief_icon_get_dist_moved(icon_move_info *mi, float elapsed_time)
{
    float time, dist_moved = 0.0f;

    // first half of movement
    if (elapsed_time < mi->total_move_time / 2.0f) {
        dist_moved = 0.5f * mi->accel * elapsed_time *
                     elapsed_time; // d = 1/2at^2
        return dist_moved;
    }
...
void
brief_render_icon_line(int stage_num, int line_num)
{
    brief_line *bl;
    brief_icon *icon[2];
    ...
    for (i = 0; i < 2; i++) {
        brief_common_get_icon_dimensions(&icon_w, &icon_h, icon[i]->type,
                                         icon[i]->ship_class);
                  ...
                  float w_scale_factor, float h_scale_factor)
{
    ...
    if (!(tv.flags &
        ...
        brief_common_get_icon_dimensions(&icon_w, &icon_h, bi->type,
                                         bi->ship_class);
        ...
        if ((bi->flags & BI_SHOWHIGHLIGHT) && (bi->flags & BI_HIGHLIGHT)) {
            ...
            if (ha->first_frame >= 0) {
                ...
                if (strlen(bi->label) > 0) {
                    ha->sy = bi->hold_y -
                             fl2i(gr_get_font_height() / 2.0f + 0.5) - 2;
                }
                else {
                    ha->sy = bi->hold_y;
                }
...
//
void
brief_render_icons(int stage_num, float frametime)
{
...
void
brief_blit_stage_num(int stage_num, int stage_max)
{
    char buf[64];
    // int w;
    ...
    gr_printf(Brief_stage_text_coords[gr_screen.res][0],
              Brief_stage_text_coords[gr_screen.res][1], buf);
}
...
void
brief_render_line(int line_num, int x, int y, int instance)
{
    ...
    truncate_len = fl2i(Brief_text_wipe_time_elapsed / BRIEF_TEXT_WIPE_TIME *
...
//
void
brief_reset_icons(int stage_num)
{
    brief_stage *bs;
    brief_icon *bi;
    int i;
...
brief_set_camera_target(vector *pos, matrix *orient, int time)
{
    ...
    Start_cam_pos =
        Current_cam_pos; // we need this when checking if camera movement complete
    Start_cam_move = timer_get_milliseconds() *
                     1000.0f; // start time, convert to seconds
    Total_move_time = time_in_seconds;
    Elapsed_time = 0.0f;

    vm_vec_scale_add(&Start_lookat_pos, &Start_cam_pos, &Start_cam_orient.fvec,
                     LOOKAT_DIST);
    vm_vec_scale_add(&Target_lookat_pos, &Target_cam_pos, &Target_cam_orient.fvec,
                     LOOKAT_DIST);
    ...
    get_camera_limits(&Start_cam_orient, &Target_cam_orient, Total_move_time,
                      &Acc_limit, &Vel_limit);
}

ubyte
brief_return_color_index(char c)
{
    switch (c) {
    case 'f':
...
void
brief_set_text_color(int color_index)
{
...
brief_set_move_list(int new_stage, int current_stage, float time)
{
    ...
    for (i = 0; i < cb->num_icons; i++) {
        ...
        for (j = 0; j < newb->num_icons; j++) {
            if ((cb->icons[i].id != 0) &&
                ...
                if (vm_vec_cmp(&cb->icons[i].pos, &newb->icons[j].pos)) {
                    ...
                    imi->peak_speed = imi->total_dist / imi->total_move_time *
                                      2.0f;
        ...
        if (is_gone == 1) {
            ...
            Fading_icons[Num_fade_icons].fade_anim =
                Icon_fade_anims[cb->icons[i].type][species];
    ...
    for (i = 0; i < newb->num_icons; i++) {
        ...
        if (is_new) {
            ...
            newb->icons[i].flags |= BI_FADEIN;
            newb->icons[i].fadein_anim =
                Icon_fade_anims[newb->icons[i].type][species];
...
void
brief_set_new_stage(vector *pos, matrix *orient, int time, int stage_num)
{
    ...
    if ((Last_new_stage != -1) && not_objv) {
        num_movers = brief_set_move_list(stage_num, Last_new_stage,
                                         new_time / 1000.0f);
    }

    if ((Last_new_stage != -1) && (num_movers == 0) && not_objv) {
        if (!vm_vec_cmp(&Briefing->stages[stage_num].camera_pos,
                        &Briefing->stages[Last_new_stage].camera_pos)) {
            if (!vm_vec_cmp(
                    &Briefing->stages[stage_num].camera_orient.fvec,
                    &Briefing->stages[Last_new_stage].camera_orient.fvec)) {
                new_time = 0;
            }
        }
    }
    ...
    if (gr_screen.res == GR_640) {
        // GR_640
        Num_brief_text_lines[0] = brief_color_text_init(msg,
...
//
int
camera_pos_past_target(vector *start, vector *current, vector *dest)
{
    vector num, den;
    float ratio;
...
// and goal.
void
interpolate_matrix(matrix *result, matrix *goal, matrix *start,
                   float elapsed_time, float total_time)
{
    vector fvec, rvec;
    float time0, time1;
...
float
brief_camera_get_dist_moved(float elapsed_time)
{
    float time, dist_moved = 0.0f;

    // first half of movement
    if (elapsed_time < Total_move_time / 2.0f) {
        dist_moved = 0.5f * Cam_accel * elapsed_time *
                     elapsed_time; // d = 1/2at^2
        return dist_moved;
    }
...
void
brief_camera_move(float frametime, int stage_num)
{
    vector dist_moved;
    float dist;
    vector w_out;
...
brief_maybe_create_new_grid(grid *gridp, vector *pos, matrix *orient, int force)
{
    ...
    if (fvi_ray_plane(&gpos, &gridp->center, &gridp->gmatrix.uvec, pos,
                      &orient->fvec, 0.0f) < 0.0f) {
        vector p;
    ...
    if ((square_size != gridp->square_size) || (gpos.x != gridp->center.x) ||
        (gpos.y != gridp->center.y) || (gpos.z != gridp->center.z) || force) {
                  ...
                  int nrows, int ncols, float square_size)
{
    ...
    gridp->planeD = -(center->x * uvec.x + center->y * uvec.y +
                      center->z * uvec.z);
...
brief_create_default_grid(void)
{
    ...
    rgrid = brief_create_grid(&Global_grid, vm_vec_make(&fvec, 0.0f, 0.0f, 1.0f),
                              ...
                              vm_vec_make(&cvec, 0.0f, -10.0f, 0.0f), 100, 100,
                              5.0f);
...
void
brief_modify_grid(grid *gridp)
{
    brief_create_grid(gridp, &gridp->gmatrix.fvec, &gridp->gmatrix.rvec,
                      &gridp->center, gridp->nrows, gridp->ncols,
                      gridp->square_size);
}
...
void
brief_load_voice_file(int voice_num, char *name)
{
    int load_attempts = 0;
    while (1) {
        if (load_attempts++ > 5) {
...
void
brief_voice_play(int stage_num)
{
...
void
brief_voice_stop(int stage_num)
{
    if (Brief_voices[stage_num] == -1)
        return;
...
void
brief_voice_pause(int stage_num)
{
    if (Brief_voices[stage_num] == -1)
        return;
...
void
brief_voice_unpause(int stage_num)
{
    if (Brief_voices[stage_num] == -1)
        return;
...
void
brief_common_get_icon_dimensions(int *w, int *h, int type, int ship_class)
{
...
int
brief_time_to_advance(int stage_num, float frametime)
{
