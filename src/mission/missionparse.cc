...
const char *Ai_behavior_names[MAX_AI_BEHAVIORS] = {
    ...
    { "Be Rearmed" }, { "Safety" },     { "Evade Weapon" }, { "Strafe" },
    { "Play Dead" },  { "Bay Emerge" }, { "Bay Depart" },   { "Sentry Gun" },
    { "Warp Out" },
};
...
int Num_reinforcement_type_names = sizeof(Reinforcement_type_names) /
                                   sizeof(char *);
...
int mission_set_arrival_location(int anchor, int location, int distance,
                                 int objnum, vector *new_pos, matrix *new_orient);
...
parse_mission_info(mission *pm)
{
    ...
    if (pm->version != MISSION_VERSION)
        mprintf(("Older mission, should update it (%.2f<-->%.2f)", pm->version,
                 MISSION_VERSION));
    ...
    if (optional_string("+Player Entry Delay:")) {
        ...
        Entry_delay_time =
            0; // without the reset a mission inherits the previous mission's delay
    }
    ...
    // reassign the player
    if (!Fred_running && (Player != NULL) &&
        (strlen(The_mission.squad_name) > 0) && (Game_mode & GM_CAMPAIGN_MODE)) {
...
parse_player_info2(mission *pm)
{
    ...
    if (nt != Num_teams)
        Error(
            LOCATION,
            "Not enough ship/weapon pools for mission.  There are %d teams and only %d pools.",
            Num_teams, nt);
}

void
parse_plot_info(mission *pm)
{
...
void
parse_briefing_info(mission *pm)
{
    char junk[4096];
...
parse_cmd_brief(mission *pm)
{
    ...
    while (optional_string("$Stage Text:")) {
        ...
        Cur_cmd_brief->stage[stage].text = stuff_and_malloc_string(
            F_MULTITEXT, NULL, CMD_BRIEF_TEXT_MAX);
        ...
        if (optional_string("+Wave Filename:"))
            stuff_string(Cur_cmd_brief->stage[stage].wave_filename, F_FILESPEC,
                         NULL);
        else
...
parse_briefing(mission *pm)
{
        ...
        while (required_string_either("$end_briefing", "$start_stage")) {
            ...
            if (optional_string("$num_lines:")) {
                ...
                if (Fred_running) {
                    ...
                    if (bs->num_lines > 0) {
                        bs->lines = (brief_line *)malloc(sizeof(brief_line) *
                                                         bs->num_lines);
                        Assert(bs->lines != NULL);
                    }
                }
            ...
            if (Fred_running) {
                ...
                if (bs->num_icons > 0) {
                    bs->icons = (brief_icon *)malloc(sizeof(brief_icon) *
                                                     bs->num_icons);
            ...
            while (required_string_either("$end_stage", "$start_icon")) {
                ...
                find_and_stuff("$team:", &team_index, F_NAME, Team_names,
                               Num_team_names, "team name");
                Assert((team_index >= 0) && (team_index < MAX_TEAM_NAMES));
                bi->team = 1 << team_index;

                find_and_stuff("$class:", &bi->ship_class, F_NAME,
                               Ship_class_names, Num_ship_types, "ship class");
    ...
    if (nt != Num_teams)
        Error(
            LOCATION,
            "Not enough briefings in mission file.  There are %d teams and only %d briefings.",
            Num_teams, nt);
}
...
void
parse_debriefing_old(mission *pm)
{
    int junk;
...
// Parse the data required for the mission debriefings
void
parse_debriefing_new(mission *pm)
{
    int stage_num, nt;
    debriefing *db;
    debrief_stage *dbs;
    ...
    for (nt = 0; nt < Num_teams; nt++) {
        ...
        while (required_string_either("#", "$Formula")) {
            ...
            if (Fred_running) {
                stuff_string(dbs->new_recommendation_text, F_MULTITEXT, NULL,
                             MAX_RECOMMENDATION_LEN);
            }
            else {
                dbs->new_recommendation_text = stuff_and_malloc_string(
...
void
position_ship_for_knossos_warpin(p_object *objp, int shipnum, int objnum)
{
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if (Ship_info[Ships[knossos_num].ship_info_index].flags &
            SIF_KNOSSOS_DEVICE) {
            // be close to the right device [allow multiple knossos's
            if (vm_vec_dist_quick(&Objects[knossos_num].pos, &objp->pos) <
                2.0f * (Objects[knossos_num].radius + Objects[objnum].radius)) {
                found = TRUE;
                break;
            }
        }
    ...
    if (found) {
        ...
        vector new_point;
        float dist = fvi_ray_plane(&new_point, &Objects[knossos_num].pos,
                                   &Objects[knossos_num].orient.fvec, &objp->pos,
                                   &objp->orient.fvec, 0.0f);
        polymodel *pm = model_get(
            Ship_info[Ships[shipnum].ship_info_index].modelnum);
        float desired_dist = -pm->mins.z;
        vm_vec_scale_add2(&Objects[objnum].pos, &Objects[objnum].orient.fvec,
                          (dist - desired_dist));
...
parse_create_object(p_object *objp)
{
    ...
    Ships[shipnum].weapons.ai_class =
        objp->ai_class; // Fred uses this instead of above.
    ...
    if (objp->flags & P_SF_USE_UNIQUE_ORDERS) {
        ...
        if (Fred_running) {
            ...
            default_orders = ship_get_default_orders_accepted(
                &Ship_info[Ships[shipnum].ship_info_index]);
    ...
    // MWA -- We shouldn't be setting this flag in single player mode -- it gets
    // set in post process mission.
    if ((objp->flags & P_OF_PLAYER_START) && Fred_running)
    ...
    if ((Ships[shipnum].wingnum != -1) &&
        (Wings[Ships[shipnum].wingnum].flags & WF_NO_ARRIVAL_WARP))
        Ships[shipnum].flags |= SF_NO_ARRIVAL_WARP;

    if ((Ships[shipnum].wingnum != -1) &&
        (Wings[Ships[shipnum].wingnum].flags & WF_NO_DEPARTURE_WARP))
        Ships[shipnum].flags |= SF_NO_DEPARTURE_WARP;
    ...
    // if the wing index and wing pos are set for this parse object, set them for the ship
    ...
    if (objp->ai_goals != -1) {
        ...
        if (objp->wingnum ==
            -1) // free the sexpression nodes only for non-wing ships.  wing code will handle it's own case
            free_sexp2(
                objp->ai_goals); // free up sexp nodes for reused, since they aren't needed anymore.
    }
        ...
        if (!stricmp(sssp->name, NOX("Pilot"))) {
            ...
            for (j = 0; j < wp->num_secondary_banks; j++)
                if (Fred_running) {
                    ...
                    int capacity = fl2i(sssp->secondary_ammo[j] / 100.0f *
                                            sip->secondary_bank_ammo_capacity[j] +
                                        0.5f);
                    wp->secondary_bank_ammo[j] = fl2i(
                        capacity /
        ...
        while (ptr != END_OF_LIST(&Ships[shipnum].subsys_list)) {
            if (!stricmp(ptr->system_info->subobj_name, sssp->name)) {
                if (Fred_running)
                    ...
                    float new_hits;
                    new_hits = ptr->system_info->max_hits *
                               (100.0f - sssp->percent) / 100.f;
                    Ships[shipnum]
                        .subsys_info[ptr->system_info->type]
                        .current_hits -= (ptr->system_info->max_hits - new_hits);
    ...
    if (Fred_running) {
        ...
        Objects[objnum].hull_strength = objp->initial_hull *
                                        sip->initial_hull_strength / 100.0f;
        for (i = 0; i < MAX_SHIELD_SECTIONS; i++)
            Objects[objnum].shields[i] = (float)(objp->initial_shields *
                                                 sip->shields / 100.0f) /
                                         MAX_SHIELD_SECTIONS;
    ...
    if ((Game_mode & GM_IN_MISSION) && (!Fred_running)) {
        ...
        if (!Game_restoring) {
            if (Ships[shipnum].wingnum == -1) {
                ...
                mission_set_arrival_location(objp->arrival_anchor, location,
                                             objp->arrival_distance, objnum, NULL,
                                             NULL);
        ...
        if ((Ships[shipnum].wingnum == -1) && (Ships[shipnum].hotkey != -1))
            mission_hotkey_mf_add(Ships[shipnum].hotkey, Ships[shipnum].objnum,
                                  HOTKEY_MISSION_FILE_ADDED);
        else if ((Ships[shipnum].wingnum != -1) &&
                 (Wings[Ships[shipnum].wingnum].hotkey != -1))
            mission_hotkey_mf_add(Wings[Ships[shipnum].wingnum].hotkey,
                                  Ships[shipnum].objnum,
                                  HOTKEY_MISSION_FILE_ADDED);
...
parse_object(mission *pm, int flag, p_object *objp)
{
    ...
    find_and_stuff("$Class:", &objp->ship_class, F_NAME, Ship_class_names,
                   Num_ship_types, "ship class");
    ...
    int team_index;
    find_and_stuff("$Team:", &team_index, F_NAME, Team_names, Num_team_names,
                   "team name");
    Assert((team_index >= 0) && (team_index < MAX_TEAM_NAMES));
    ...
    find_and_stuff("$AI Behavior:", &objp->behavior, F_NAME, Ai_behavior_names,
                   Num_ai_behaviors, "AI behavior");
    objp->ai_goals = -1;

    if (optional_string("+AI Class:")) {
        objp->ai_class = match_and_stuff(F_NAME, Ai_class_names, Num_ai_classes,
                                         "AI class");
    ...
    objp->cargo1 = -1;
    int temp;
    find_and_stuff_or_add("$Cargo 1:", &temp, F_NAME, Cargo_names, &Num_cargo,
                          MAX_CARGO, "cargo");
    ...
    while (required_string_either("$Arrival Location:", "$Status Description:")) {
        ...
        find_and_stuff("$Status Description:", &objp->status_type[count], F_NAME,
                       Status_desc_names, Num_status_names, "Status Description");
        find_and_stuff("$Status:", &objp->status[count], F_NAME,
                       Status_type_names, Num_status_names, "Status Type");
        find_and_stuff("$Target:", &objp->target[count], F_NAME,
                       Status_target_names, Num_status_names, "Target");
        count++;
    }
    ...
    find_and_stuff("$Arrival Location:", &objp->arrival_location, F_NAME,
    ...
    if (optional_string("+Arrival Delay:")) {
        stuff_int(&delay);
        if (delay < 0)
            Error(LOCATION, "Cannot have arrival delay < 0 (ship %s)",
                  objp->name);
    }
    else
        delay = 0;
    ...
    if (!Fred_running) {
        objp->arrival_delay =
            -delay; // use negative numbers to mean we haven't set up a timer yet
    }
    else {
    ...
    if (!Fred_running && (objp->arrival_cue >= 0)) {
        ...
        if (eval_sexp(
                objp->arrival_cue)) { // evaluate to determine if sexp is always false.
            objp->arrival_delay = timestamp(-objp->arrival_delay * 1000);
        }
    }

    find_and_stuff("$Departure Location:", &objp->departure_location, F_NAME,
                   Departure_location_names, Num_arrival_names,
                   "Departure Location");
    ...
    if (optional_string("+Departure Delay:")) {
        stuff_int(&delay);
        if (delay < 0) {
            Error(LOCATION, "Cannot have departure delay < 0 (ship %s)",
                  objp->name);
        }
    }
    else {
    ...
    if (optional_string("+Docked With:")) {
        ...
        strcpy(Initially_docked[Total_initially_docked].dockee,
               objp->docked_with);
        strcpy(Initially_docked[Total_initially_docked].docker_point,
               objp->docker_point);
        strcpy(Initially_docked[Total_initially_docked].dockee_point,
               objp->dockee_point);
    ...
    // if this if the starting player ship, then copy if to Starting_player_pobject
    ...
    if ((objp->flags & P_SF_INITIALLY_DOCKED) ||
        (!Fred_running && (!eval_sexp(objp->arrival_cue) ||
                           !timestamp_elapsed(objp->arrival_delay) ||
                           (objp->flags & P_SF_REINFORCEMENT)))) {
        Assert(destroy_before_mission_time ==
               -1); // we can't add ships getting destroyed to the arrival list!!!
        ...
        real_objnum = parse_create_object(
            objp); // this object may later get destroyed depending on wing status!!!!

        Subsys_index =
            objp->subsys_index; // free elements that are no longer needed.
        ...
        if (destroy_before_mission_time >= 0) {
            ...
            if (!Fred_running) {
                int i;
                shipfx_blow_up_model(objp, Ships[objp->instance].modelnum, 0, 0,
                                     &objp->pos);
...
void
parse_common_object_data(p_object *objp)
{
    int i;
    ...
    while (optional_string("+Subsystem:")) {
        ...
        if (optional_string("+Cargo Name:")) {
            ...
            int index = string_lookup(cargo_name, Cargo_names, Num_cargo, "cargo",
                                      0);
        ...
        if (optional_string("+Primary Banks:"))
            stuff_int_list(Subsys_status[i].primary_banks, MAX_PRIMARY_BANKS,
                           WEAPON_LIST_TYPE);

        if (optional_string("+Secondary Banks:"))
            stuff_int_list(Subsys_status[i].secondary_banks, MAX_SECONDARY_BANKS,
                           WEAPON_LIST_TYPE);

        if (optional_string("+Sbank Ammo:"))
            stuff_int_list(Subsys_status[i].secondary_ammo, MAX_SECONDARY_BANKS,
                           RAW_INTEGER_TYPE);
    }
}

void
parse_objects(mission *pm, int flag)
{
    p_object temp;
    ...
    while (required_string_either("#Wings", "$Name:")) {
        ...
        parse_object(pm, flag, &temp);
    }
}

int
find_wing_name(char *name)
{
    int i;
...
int
parse_wing_create_ships(wing *wingp, int num_to_create)
{
    p_object *objp;
    ...
    if (!eval_sexp(
            wingp->arrival_cue)) /* || !timestamp_elapsed(wingp->arrival_delay) ) */
        return 0;
    ...
    if (wingp->arrival_location == ARRIVE_FROM_DOCK_BAY) {
        ...
        if (shipnum == -1) {
            ...
            num_remaining = ((wingp->num_waves - wingp->current_wave) *
                             wingp->wave_count);
    ...
    if (!timestamp_valid(wingp->wave_delay_timestamp)) {
        ...
        if ((wingp->wave_delay_min > 0) || (wingp->wave_delay_max > 0)) {
            ...
            time_to_arrive = wingp->wave_delay_min +
                             (int)(frand() * (wingp->wave_delay_max -
        ...
        // if we get here, both min and max values are 0
    ...
    Assert(num_to_create ==
           0); // we should always have enough ships in the list!!!

    // possibly play some event driven music here.  Only do this stuff if actually in the mission.
    if ((objnum != -1) &&
        (Game_mode &
        ...
        mission_log_add_entry(LOG_WING_ARRIVE, wingp->name, NULL,
                              wingp->current_wave);
        ship_num = wingp->ship_index[0];

        if (!(Ships[ship_num].flags & SF_NO_ARRIVAL_MUSIC) &&
            !(wingp->flags & WF_NO_ARRIVAL_MUSIC)) {
            if (timestamp_elapsed(Allow_arrival_music_timestamp)) {
                Allow_arrival_music_timestamp = timestamp(
                    ARRIVAL_MUSIC_MIN_SEPARATION);
...
parse_wing(mission *pm)
{
    ...
    find_and_stuff("$Arrival Location:", &wingp->arrival_location, F_NAME,
                   Arrival_location_names, Num_arrival_names, "Arrival Location");
    if (optional_string("+Arrival Distance:")) {
    ...
    if (optional_string("+Arrival delay:")) {
        stuff_int(&delay);
        if (delay < 0)
            Error(LOCATION, "Cannot have arrival delay < 0 on wing %s",
                  wingp->name);
    }
    else
        delay = 0;
    ...
    if (!Fred_running && (wingp->arrival_cue >= 0)) {
        if (eval_sexp(
                wingp->arrival_cue)) // evaluate to determine if sexp is always false.
            wingp->arrival_delay = timestamp(-wingp->arrival_delay * 1000);
    }

    find_and_stuff("$Departure Location:", &wingp->departure_location, F_NAME,
                   Departure_location_names, Num_arrival_names,
                   "Departure Location");
    ...
    if (optional_string("+Departure delay:")) {
        stuff_int(&delay);
        if (delay < 0)
            Error(LOCATION, "Cannot have departure delay < 0 on wing %s",
                  wingp->name);
    }
    else
        delay = 0;
    ...
    if (!Fred_running)
        wingp->departure_delay =
            -delay; // use negative numbers to mean that delay timer not yet set
    else
    ...
    if (wing_goals != -1) {
        ...
        //if (Fred_running)
        free_sexp2(
            wing_goals); // free up sexp nodes for reused, since they aren't needed anymore.
    }
    ...
    for (i = 0; i < wingp->wave_count; i++) {
        ...
        if (Fred_running) {
            ...
            if (!strnicmp(ship_name, NOX("Player "), 7)) {
                Error(
                    LOCATION,
                    "Old mission file -- please convert by loading/saving in Fred -- see Allender/Hoffoss for help.");
            }
            ...
            if ((num = ship_name_lookup(ship_name)) != -1) {
                ...
                while (objp != END_OF_LIST(&ship_arrival_list)) {
                    if (!strcmp(ship_name, objp->name)) {
                        Assert(
                            objp->wingnum ==
                            -1); // get Allender -- ship appears to be in multiple wings
            ...
            if (!assigned || (assigned > 1))
                Error(
                    LOCATION,
                    "Cannot load mission -- wing %s -- ship %s not present in #Objects section (or specified multiple times in wing.\n",
...
parse_goal(mission *pm)
{
    ...
    find_and_stuff("$Type:", &goalp->type, F_NAME, Goal_type_names,
                   Num_goal_type_names, "goal type");
...
void
parse_waypoint_list(mission *pm)
{
    waypoint_list *wpl;
...
parse_waypoints(mission *pm)
{
    ...
    while (optional_string("$Jump Node:")) {
        ...
        if (Jump_nodes[Num_jump_nodes - 1].name[0] == 0) {
            sprintf(Jump_nodes[Num_jump_nodes - 1].name, "Jump Node %d",
                    Num_jump_nodes);
        }
    }
...
void
parse_reinforcement(mission *pm)
{
    reinforcements *ptr;
    int instance;
    ...
    find_and_stuff("$Type:", &ptr->type, F_NAME, Reinforcement_type_names,
    ...
    if (ship_name_lookup(ptr->name) == -1) {
        if (wing_name_lookup(ptr->name, 1) == -1) {
            ...
            if (p_objp == END_OF_LIST(&ship_arrival_list)) {
                Warning(LOCATION, "Reinforcement %s not found as ship or wing",
                        ptr->name);
                return;
            }
        }
...
void
parse_reinforcements(mission *pm)
{
...
parse_bitmaps(mission *pm)
{
    ...
    if (The_mission.flags & MISSION_FLAG_FULLNEB) {
        ...
        Neb2_poof_flags = ((1 << 0) | (1 << 1) | (1 << 2) | (1 << 3) | (1 << 4) |
                           (1 << 5));
        ...
        if (Nebula_index >= 0) {
            nebula_init(Nebula_filenames[Nebula_index], Nebula_pitch, Nebula_bank,
                        Nebula_heading);
        }
        else {
            nebula_close();
    ...
    while (optional_string("$Starbitmap:")) {
        ...
        if (Num_starfield_bitmaps < MAX_STARFIELD_BITMAPS) {
            ...
            strcpy(Starfield_bitmap_instance[Num_starfield_bitmaps].filename,
                   b.filename);
            Num_starfield_bitmaps++;
        }
    }
...
void
parse_asteroid_fields(mission *pm)
{
...
void
parse_mission(mission *pm, int flag)
{
    int i;

    Player_starts = Num_cargo = Num_waypoint_lists = Num_goals = num_wings =
        num_ship_arrivals = 0;
    ...
    *Player_start_shipname = 0; // make the string 0 length for checking later
    ...
    parse_variables();
    parse_briefing_info(
        pm); // TODO: obsolete code, keeping so we don't obsolete existing mission files
    parse_cmd_briefs(pm);
    parse_briefing(pm);
...
post_process_mission()
{
    ...
    Player_obj->flags |=
        OF_PLAYER_SHIP; // make this object a player controlled ship.
    ...
    if (Player_obj->phys_info.vel.z > 0.0f)
        Player->ci.forward_cruise_percent = Player_obj->phys_info.vel.z /
    ...
    if (Fred_running) {
        ...
        for (i = 0; i < MAX_SHIPS; i++) {
            if ((Ships[i].objnum >= 0) && (Ships[i].arrival_anchor >= 0) &&
                (Ships[i].arrival_anchor < SPECIAL_ARRIVAL_ANCHORS_OFFSET))
        ...
        for (i = 0; i < MAX_WINGS; i++) {
            if (Wings[i].wave_count && (Wings[i].arrival_anchor >= 0) &&
                (Wings[i].arrival_anchor < SPECIAL_ARRIVAL_ANCHORS_OFFSET))
    ...
    for (i = 0; i < MAX_SEXP_NODES; i++) {
        if (is_sexp_top_level(i) && (!Fred_running || (i != Sexp_clipboard))) {
            ...
            Assert(
                op !=
                -1); // need to make sure it is an operator before we treat it like one..
            result = check_sexp_syntax(i, query_operator_return_type(op), 1,
                                       &bindex);
            ...
            if (result) {
                ...
                sprintf(text,
                        ...
                        sexp_error_message(result), sexp_str,
                        Sexp_nodes[bindex].text);
   ...
   */
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if (Ships[shipnum].objnum == -1 ||
            (Objects[Ships[shipnum].objnum].flags & OF_PLAYER_SHIP))
            continue;
        if (Ships[shipnum].flags & SF_IGNORE_COUNT)
            continue;
        if ((Ships[shipnum].wingnum != -1) &&
            (Wings[Ships[shipnum].wingnum].flags & WF_IGNORE_COUNT))
            continue;
    ...
    for (p_objp = GET_FIRST(&ship_arrival_list);
        ...
        if (p_objp->flags & P_SF_IGNORE_COUNT)
            continue;
        if ((p_objp->wingnum != -1) &&
            (Wings[p_objp->wingnum].flags & WF_IGNORE_COUNT))
            continue;
...
int
get_mission_info(char *filename, mission *mission_p)
{
    int rval;
...
int
parse_main(char *mission_name, int flags)
{
    int rval, i;
...
void
mission_set_wing_arrival_location(wing *wingp, int num_to_set)
{
    int index;
    ...
    if ((wingp->arrival_location == ARRIVE_FROM_DOCK_BAY) ||
        ...
        while (index < wingp->current_count) {
            ...
            mission_set_arrival_location(wingp->arrival_anchor,
                                         wingp->arrival_location,
        ...
        if (mission_set_arrival_location(
                wingp->arrival_anchor, wingp->arrival_location,
                wingp->arrival_distance, OBJ_INDEX(leader_objp), &pos, &orient)) {
    ...
    if ((Game_mode & GM_IN_MISSION) &&
        (wingp->arrival_location != ARRIVE_FROM_DOCK_BAY)) {
...
mission_parse_set_arrival_locations()
{
    ...
    for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list);
        ...
        mission_set_arrival_location(shipp->arrival_anchor,
    ...
    while (pobjp != END_OF_LIST(&ship_arrival_list)) {
        ...
        if (pobjp->flags & P_SF_INITIALLY_DOCKED) {
            ...
            if (shipnum != -1) {
                ...
                p1 = model_find_dock_name_index(Ships[shipnum].modelnum,
                                                pobjp->docker_point);
                p2 = model_find_dock_name_index(
                    Ships[Objects[objnum].instance].modelnum,
                    pobjp->dockee_point);
                ...
                if ((p1 >= 0) && (p2 >= 0)) {
                    ...
                    if (ship_docking_valid(
                            shipnum,
                            Objects[objnum]
                                .instance)) // only dock if they are allowed to be docked.
                        ai_dock_with_object(&Objects[Ships[shipnum].objnum],
                                            &Objects[objnum], 89, AIDO_DOCK_NOW,
                                            p1, p2);
                    else
                        ai_dock_with_object(&Objects[objnum],
                                            &Objects[Ships[shipnum].objnum], 89,
                                            AIDO_DOCK_NOW, p2, p1);
                }
                else
                    Int3(); //   Curious.  Two ships told to dock, but one of the dock points is bogus.
...
int
mission_parse_is_multi(char *filename, char *mission_name)
{
    int rval, game_type;
    int filelength;
    CFILE *ftemp;
    ...
    if ((rval = setjmp(parse_abort)) != 0) {
        Error(
            LOCATION,
            "Bogus!  Trying to get multi game type on mission %s returned as a mission from cf_get_filelist\n");
    }
    else {
        read_file_text(filename, CF_TYPE_MISSIONS);
        reset_parse();
...
int
mission_parse_ship_arrived(char *shipname)
{
    p_object *objp;

    for (objp = GET_FIRST(&ship_arrival_list);
         objp != END_OF_LIST(&ship_arrival_list); objp = GET_NEXT(objp)) {
...
p_object *
mission_parse_get_arrival_ship(char *name)
{
    p_object *objp;
...
int
mission_set_arrival_location(int anchor, int location, int dist, int objnum,
                             vector *new_pos, matrix *new_orient)
{
    ...
    if (anchor < SPECIAL_ARRIVAL_ANCHORS_OFFSET) {
        ...
        if (anchor == ANY_FRIENDLY) {
            ...
            shipnum = ship_get_random_team_ship(
                opposing_team_mask(Player_ship->team), SHIP_GET_ANY_SHIP);
        }
        else if (anchor == ANY_FRIENDLY_PLAYER) {
            shipnum = ship_get_random_team_ship(TEAM_FRIENDLY,
                                                SHIP_GET_ONLY_PLAYERS);
        }
        else if (anchor == ANY_HOSTILE_PLAYER) {
            shipnum = ship_get_random_team_ship(
                opposing_team_mask(Player_ship->team), SHIP_GET_ONLY_PLAYERS);
        }
        else
        ...
        if (shipnum == -1) {
            nprintf((
                "Allender",
                "Couldn't find random ship for arrival anchor -- using default location\n"));
            return 0;
        }
    }
    ...
    if (location == ARRIVE_FROM_DOCK_BAY) {
        ...
        if (ai_acquire_emerge_path(&Objects[objnum], anchor_objnum, &pos,
                                   &fvec) == -1) {
        ...
        if (location == ARRIVE_NEAR_SHIP) {
            ...
            vm_vec_copy_scale(&t2, &(Objects[anchor_objnum].orient.rvec),
                              (1.0f - x) * r1);
            vm_vec_copy_scale(&t3, &(Objects[anchor_objnum].orient.uvec),
                              (1.0f - x) * r2);
        ...
        vm_vec_scale_add(&Objects[objnum].pos, &anchor_pos, &rand_vec,
                         (float)dist);
...
void
mission_parse_mark_reinforcement_available(char *name)
{
    int i;
    reinforcements *rp;
...
int
mission_did_ship_arrive(p_object *objp)
{
    int did_arrive;
    ...
    // process
    if (objp->flags & P_SF_REINFORCEMENT) {
        // if this ship did arrive, mark the reinforcement as available
        if (did_arrive) {
...
mission_parse_mark_non_arrivals()
{
    ...
    for (pobjp = GET_FIRST(&ship_arrival_list);
         pobjp != END_OF_LIST(&ship_arrival_list); pobjp = GET_NEXT(pobjp)) {
        if (pobjp->wingnum != -1) {
            if (Sexp_nodes[Wings[pobjp->wingnum].arrival_cue].value ==
                SEXP_KNOWN_FALSE)
...
// ship.
void
mission_parse_support_arrived(int objnum)
{
    int i;
...
mission_eval_arrivals()
{
    ...
    if (timestamp_valid(Arrival_message_delay_timestamp) &&
        ...
        if ((rship == -1) || use_terran) {
            message_send_builtin_to_player(MESSAGE_ARRIVE_ENEMY, NULL,
                                           MESSAGE_PRIORITY_LOW,
                                           MESSAGE_TIME_SOON, 0, 0);
        }
        else if (rship != -1) {
            message_send_builtin_to_player(MESSAGE_ARRIVE_ENEMY, &Ships[rship],
                                           MESSAGE_PRIORITY_LOW,
                                           MESSAGE_TIME_SOON, 0, 0);
        }
    ...
    while (objp != END_OF_LIST(&ship_arrival_list)) {
        p_object *temp = GET_NEXT(objp);
        if (objp->wingnum ==
            -1) { // if this object has a wing -- let code for wings determine if it should be created
    ...
    for (i = 0; i < num_wings; i++) {
        ...
        if ((wingp->current_wave == 0) ||
            ...
            if (created > 0) {
                ...
                if (i ==
                    Starting_wings[STARTING_WING_BETA]) { // this is the beta wing
                    rship = ship_get_random_ship_in_wing(i, SHIP_GET_NO_PLAYERS);
                    if (rship != -1) {
                        message_send_builtin_to_player(MESSAGE_BETA_ARRIVED,
                                                       &Ships[rship],
                                                       MESSAGE_PRIORITY_LOW,
                                                       MESSAGE_TIME_SOON, 0, 0);
                    }
                         ...
                         Starting_wings
                             [STARTING_WING_GAMMA]) { // this is the gamma wing
                    rship = ship_get_random_ship_in_wing(i, SHIP_GET_NO_PLAYERS);
                    if (rship != -1) {
                        message_send_builtin_to_player(MESSAGE_GAMMA_ARRIVED,
                                                       &Ships[rship],
                                                       MESSAGE_PRIORITY_LOW,
                                                       MESSAGE_TIME_SOON, 0, 0);
                    }
                    ...
                    rship = ship_get_random_ship_in_wing(i, SHIP_GET_NO_PLAYERS);
                    if (rship != -1) {
                        message_send_builtin_to_player(MESSAGE_DELTA_ARRIVED,
                                                       &Ships[rship],
                                                       MESSAGE_PRIORITY_LOW,
                                                       MESSAGE_TIME_SOON, 0, 0);
                    }
                    ...
                    rship = ship_get_random_ship_in_wing(i, SHIP_GET_NO_PLAYERS);
                    if (rship != -1) {
                        message_send_builtin_to_player(MESSAGE_EPSILON_ARRIVED,
                                                       &Ships[rship],
                                                       MESSAGE_PRIORITY_LOW,
                                                       MESSAGE_TIME_SOON, 0, 0);
                    }
                    ...
                    if (Ships[rship].team != TEAM_FRIENDLY) {
                        ...
                        if (timestamp_elapsed(Allow_arrival_message_timestamp)) {
                            ...
                            Allow_arrival_message_timestamp = timestamp(
...
mission_eval_departures()
{
    ...
    for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list);
        ...
        if (objp->type == OBJ_SHIP) {
            ...
            if ((shipp->flags & (SF_DEPARTING | SF_DYING | SF_CANNOT_WARP)) ||
            ...
            if (eval_sexp(shipp->departure_cue)) {
                if (shipp->departure_delay <= 0)
                    shipp->departure_delay = timestamp(-shipp->departure_delay *
                                                       1000);
    ...
    for (i = 0; i < num_wings; i++) {
        ...
        if (eval_sexp(wingp->departure_cue)) {
            ...
            if (wingp->departure_delay <= 0)
                wingp->departure_delay = timestamp(-wingp->departure_delay *
                                                   1000);
...
int
get_parse_name_index(char *name)
{
    int i;
...
void
mission_add_to_arriving_support(object *requester_objp)
{
    int i;
    ship *shipp;
    ...
    if (i !=
        Num_arriving_repair_targets) { // found the ship before reaching the end -- ignore it!
        return;
    }

    strcpy(Arriving_repair_targets[Num_arriving_repair_targets],
           Ships[requester_objp->instance].ship_name);
    Num_arriving_repair_targets++;
}

extern int pp_collide_any(vector *curpos, vector *goalpos, float radius,
...
void
mission_warp_in_support_ship(object *requester_objp)
{
   ...
   */
    ...
    if (!get_warp_in_pos(&warp_in_pos, requester_objp, 1.0f, 0.1f, 1.0f))
        if (!get_warp_in_pos(&warp_in_pos, requester_objp, 1.0f, 0.2f, -1.0f))
            if (!get_warp_in_pos(&warp_in_pos, requester_objp, -1.0f, -0.2f,
                                 -1.0f))
                if (!get_warp_in_pos(&warp_in_pos, requester_objp, -1.0f, -0.1f,
                                     1.0f))
                    get_warp_in_pos(&warp_in_pos, requester_objp, 0.1f, 1.0f,
                                    0.2f);
    ...
    for (i = 0; i < Num_ship_types; i++) {
        if ((Ship_info[i].species == requester_species) &&
            (Ship_info[i].flags & SIF_SUPPORT))
            break;
    }
    ...
    pobj->behavior =
        AIM_NONE; // ASSUMPTION:  the mission file has the string "None" which maps to AIM_NONE
    ...
    pobj->arrival_location =
        0; // ASSUMPTION: this is index to arrival_lcation string array for hyperspace!!!!
    ...
    pobj->departure_location =
        0; // ASSUMPTION: this is index to departure_lcation string array for hyperspace!!!!
    pobj->departure_anchor = -1;
    pobj->departure_cue = Locked_sexp_false;
    pobj->departure_delay = 0;

    pobj->determination =
        10; // ASSUMPTION:  mission file always had this number written out
    pobj->wingnum = -1;
    if (Player_obj->flags & P_OF_NO_SHIELDS)
        pobj->flags =
            P_OF_NO_SHIELDS; // support ships have no shields when player has not shields
...
int
mission_is_support_ship_arriving()
{
    if (Arriving_support_ship)
        return 1;
    else
...
int
mission_is_repair_scheduled(object *objp)
{
    char *name;
    int i;
...
int
mission_remove_scheduled_repair(object *objp)
{
    char *name;
    int i, index;
...
int
mission_parse_lookup_alt(char *name)
{
    int idx;
...
void
mission_parse_lookup_alt_index(int index, char *out)
{
    // sanity
    if (out == NULL) {
        return;
...
int
mission_parse_add_alt(char *name)
{
    // sanity
    if (name == NULL) {
        return -1;
