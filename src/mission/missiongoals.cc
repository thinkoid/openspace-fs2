...
static int
    Mission_directive_special_timestamp; // used to specially mark a directive as true even though it's not

char *
Goal_type_text(int n)
{
    switch (n) {
    case 0:
...
void
goal_list::icons_display(int yoff)
{
...
goal_text::display(int n, int y)
{
    ...
    if (*m_lines[n] == '*') { // header line
        ...
        gr_line(Goal_screen_text_x + w + 1, y1,
                Goal_screen_icon_x + Goal_screen_text_w, y1);
    }
    else {
...
mission_init_goals()
{
    ...
    Mission_directive_special_timestamp = timestamp(
        -1); // need to make invalid right away
}
...
mission_show_goals_init()
{
    ...
    for (i = 0; i < NUM_GOAL_SCREEN_BUTTONS; i++) {
        ...
        b->button.create(&Goals_screen_ui_window, "", b->x, b->y, 60, 30, (i < 2),
                         1);
...
void
mission_show_goals_do_frame(float frametime)
{
    int k, i, y, z;
...
int
ML_objectives_init(int x, int y, int w, int h)
{
    int i, type;
...
void
ML_objectives_do_frame(int scroll_offset)
{
    int y, z;
    int font_height = gr_get_font_height();
...
ML_render_objectives_key()
{
    ...
    gr_bitmap(Objective_key_icon_coords[gr_screen.res][0][0],
              Objective_key_icon_coords[gr_screen.res][0][1]);
    gr_set_bitmap(Goal_incomplete_bitmap);
    gr_bitmap(Objective_key_icon_coords[gr_screen.res][1][0],
              Objective_key_icon_coords[gr_screen.res][1][1]);
    gr_set_bitmap(Goal_failed_bitmap);
    gr_bitmap(Objective_key_icon_coords[gr_screen.res][2][0],
              Objective_key_icon_coords[gr_screen.res][2][1]);

    gr_string(Objective_key_text_coords[gr_screen.res][0][0],
              Objective_key_text_coords[gr_screen.res][0][1],
              XSTR("Complete", 1437));
    gr_string(Objective_key_text_coords[gr_screen.res][1][0],
              Objective_key_text_coords[gr_screen.res][1][1],
              XSTR("Incomplete", 1438));
    gr_string(Objective_key_text_coords[gr_screen.res][2][0],
...
void
mission_goal_status_change(int goal_num, int new_status)
{
    int type;
    ...
    if (new_status == GOAL_FAILED) {
        ...
        mission_log_add_entry(LOG_GOAL_FAILED, Mission_goals[goal_num].name, NULL,
                              goal_num);
    }
        ...
        if (Game_mode & GM_NORMAL) {
            ...
            mission_log_add_entry(LOG_GOAL_SATISFIED,
                                  Mission_goals[goal_num].name, NULL, goal_num);
        }

        // deal with the score
        Player->stats.m_score += (int)(Mission_goals[goal_num].score *
...
int
mission_get_event_status(int event)
{
...
void
mission_event_set_directive_special(int event)
{
    // bogus
...
void
mission_event_unset_directive_special(int event)
{
    // bogus
    if ((event < 0) || (event >= Num_mission_events)) {
        return;
...
void
mission_process_event(int event)
{
    int result, sindex;
    ...
    if (sindex >= 0) {
        ...
        else if ((Mission_events[event].flags & MEF_DIRECTIVE_SPECIAL) &&
                 Directive_count > 1) {
            // make it non special
    ...
    if (result || timestamp_valid(Mission_events[event].timestamp)) {
        ...
        if (Mission_events[event].repeat_count <= 0) {
            ...
            Player->stats.m_score += (int)(Mission_events[event].score *
                                           scoring_get_scale_factor());
        }
        else {
...
void
mission_goal_validation_change(int goal_num, int valid)
{
...
// structure
void
mission_goal_mark_invalid(char *name)
{
    int i;
...
void
mission_goal_mark_valid(char *name)
{
    int i;
...
mission_goal_fail_incomplete()
{
    ...
    for (i = 0; i < Num_goals; i++) {
        if (Mission_goals[i].satisfied == GOAL_INCOMPLETE) {
            Mission_goals[i].satisfied = GOAL_FAILED;
            mission_log_add_entry(LOG_GOAL_FAILED, Mission_goals[i].name, NULL,
                                  i);
        }
    }
...
DCF(change_mission_goal, "Change the mission goal")
{
    ...
    if (Dc_command) {
        ...
        if (Dc_arg_type & ARG_TRUE)
            ...
            if (!stricmp(Dc_arg, "satisfied"))
                ...
                dc_printf(
                    "Unknown status %s.  Use 'satisfied', 'failed', or 'unknown'\n",
                    Dc_arg);
        }
    }
    ...
    if (Dc_help) {
        ...
        dc_printf(
            "<status>   --  [bool] where a true value makes the goal satisfied,\n");
        dc_printf("               a false value makes the goal failed.\n");
        dc_printf(
            "The <status> field may also be one of 'satisfied', 'failed', or 'unknown'\n");
        dc_printf(
...
void
mission_goal_mark_all_true(int type)
{
    int i;
    ...
    HUD_sourced_printf(HUD_SOURCE_HIDDEN, NOX("All %s goals marked true"),
                       Goal_type_text(type));
}
...
void
goal_screen_button_pressed(int num)
{
    switch (num) {
...
void
mission_goal_fetch_num_resolved(int desired_type, int *num_resolved, int *total,
                                int team)
{
    int i, type;
...
int
mission_goals_incomplete(int desired_type, int team)
{
    int i, type;
