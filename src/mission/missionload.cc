...
mission_load()
{
    ...
    ml_update_recent_missions(
        Game_current_mission_filename); // update recently played missions list
    write_pilot_file();
    return 0;
}
...
extern int mission_campaign_get_filenames(char *filename,
                                          char dest[][NAME_LENGTH], int *num);
...
mission_load_menu_init()
{
    ...
    mlm_nfiles = cf_get_file_list(MLM_MAX_MISSIONS, mlm_missions,
    ...
    mlm_mission_list.create(&mlm_window, 450, 150, 150, 200, mlm_nfiles,
                            mlm_missions);

    for (i = 0; i < Num_recent_missions; i++) {
        recent_missions[i] = Recent_missions[i];
    }
    recent_mission_list.create(&mlm_window, 250, 150, 150, 200,
                               Num_recent_missions, recent_missions);
    ...
    campaign_filter.create(&mlm_window, 50, 150, 150, 200, Num_campaigns + 2,
                           campaign_names);
...
mission_load_menu_do()
{
    ...
    if (campaign_filter.current() != Campaign_filter_index) {
        ...
        if (Campaign_filter_index > 1) {
            mission_campaign_get_filenames(
                Campaign_file_names[Campaign_filter_index - 2], Campaign_missions,
                &Num_campaign_missions);

            for (i = 0; i < Num_campaign_missions; i++) {
                campaign_missions[i] = Campaign_missions[i];
            }
            mlm_mission_list.set_new_list(Num_campaign_missions,
                                          campaign_missions);
        }
    ...
    if (selected > -1) {
        ...
        if (use_recent_flag) {
            strncpy(mission_name_final, recent_missions[selected],
                    MAX_FILENAME_LEN);
        }
        else {
