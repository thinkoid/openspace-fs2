...
int
message_playing_specific_builtin(int builtin_type)
{
    int i;
...
int
message_get_priority(int which)
{
    int i;
    int priority;
    ...
    for (i = 0; i < Num_messages_playing; i++) {
        ...
        else if ((which == MESSAGE_GET_LOWEST) &&
                 (Playing_messages[i].priority < priority)) {
            priority = Playing_messages[i].priority;
        }
    }
...
void
message_remove_from_queue(message_q *q)
{
    ...
    if (MessageQ_num > 0) {
        qsort(MessageQ, MAX_MESSAGE_Q, sizeof(message_q),
...
message_play_wave(message_q *q)
{
    ...
    if (q->message_num >= Num_builtin_messages) {
        if ((f2fl(Missiontime - Message_times[q->message_num]) < 10) &&
            (f2fl(Missiontime) > 10)) {
            // Int3();  // Get Andsager
        }
    ...
    if (m->wave_info.index != -1) {
        ...
        if (m->wave_info.index >= 0) {
            ...
            Playing_messages[Num_messages_playing].wave = snd_play_raw(
                Message_waves[index].num, 0.0f);
        }
    }
}
...
message_play_anim(message_q *q)
{
    ...
    if ((q->message_num < Num_builtin_messages) ||
        !(_strnicmp(HEAD_PREFIX_STRING, ani_name,
    ...
    if (anim_info->anim_data == NULL) {
        nprintf(("messaging", "Cannot load message avi %s.  Will not play.\n",
                 ani_name));
        m->avi_info.index =
            -1; // if cannot load the avi -- set this index to -1 to avoid trying to load multiple times
    }
    ...
    if (m->avi_info.index >= 0) {
        ...
        if (hud_gauge_active(HUD_TALKING_HEAD)) {
            ...
            anim_start_frame = message_calc_anim_start_frame(
                Message_wave_duration, anim_info->anim_data, is_death_scream);
            anim_play_init(&aps, anim_info->anim_data,
                           Head_coords[gr_screen.res][0],
                           Head_coords[gr_screen.res][1]);
...
message_queue_process()
{
    ...
    if (Num_messages_playing > 0) {
        ...
        while (i < Num_messages_playing) {
            ...
            if (Playing_messages[i].shipnum != -1) {
                if ((Ships[Playing_messages[i].shipnum].flags & SF_DYING) &&
    ...
    if (Num_messages_playing) {
        ...
        else if (message_playing_unique() &&
                 (q->message_num < Num_builtin_messages)) {
                      ...
                      int source, int group, int delay, int builtin_type)
{
    ...
    MessageQ[i].flags = 0;
    if (!stricmp(who_from, TERRAN_COMMAND) && (m_persona != -1) &&
    ...
    if (timing == MESSAGE_TIME_IMMEDIATE)
        ...
        MessageQ[i].window_timestamp = timestamp(
            MESSAGE_ANYTIME_TIMESTAMP); // make invalid
...
int
message_get_persona(ship *shipp)
{
    ...
    if (shipp != NULL) {
        ...
        if (Ship_info[shipp->ship_info_index].species == SPECIES_VASUDAN) {
            ...
            for (i = 0; i < Num_personas; i++) {
                if ((Personas[i].flags & persona_needed) &&
                    (Personas[i].flags & PERSONA_FLAG_VASUDAN)) {
        ...
        for (i = 0; i < Num_personas; i++) {
            ...
            if ((ship_type & SIF_SUPPORT) &&
                !(Personas[i].flags & PERSONA_FLAG_SUPPORT))
                continue;
            else if ((ship_type & (SIF_FIGHTER | SIF_BOMBER)) &&
                     !(Personas[i].flags & PERSONA_FLAG_WINGMAN))
                continue;
            else if (!(ship_type & (SIF_FIGHTER | SIF_BOMBER | SIF_SUPPORT)) &&
                     !(Personas[i].flags & PERSONA_FLAG_LARGE))
                continue;
        ...
        for (i = 0; i < Num_personas; i++) {
            ...
            if ((ship_type & SIF_SUPPORT) &&
                ...
                continue;
                     ...
                     !(Personas[i].flags & PERSONA_FLAG_LARGE))
                continue;
            else if (Personas[i].flags &
                     PERSONA_FLAG_VASUDAN) // don't use any vasudan persona
                continue;
        ...
        nprintf(("messaging",
                 "Couldn't find a new persona for ship %s, reusing persona %s\n",
...
void
message_send_unique_to_player(char *id, void *data, int m_source, int priority,
                              int group, int delay)
{
    int i, source;
    char *who_from;
    ...
    for (i = 0; i < Num_messages; i++) {
        ...
        if (!stricmp(id, Messages[i].name)) {
            ...
            if (m_source == MESSAGE_SOURCE_COMMAND) {
                ...
                if (m_persona == -1) {
                    mprintf(
                        ("Warning:  Message %d has no persona assigned.\n", i));
                }
                ...
                Assert(!(Ship_info[shipp->ship_info_index].flags &
                         SIF_NOT_FLYABLE)); // get allender or alan
            }

            message_queue_message(i, priority, MESSAGE_TIME_ANYTIME, who_from,
                                  source, group, delay);
...
void
message_send_builtin_to_player(int type, ship *shipp, int priority, int timing,
                               int group, int delay)
{
    int i, persona_index;
    int source;
    ...
    do {
        for (i = 0; i < Num_builtin_messages; i++) {
            ...
            if (!stricmp(who_from, TERRAN_COMMAND) &&
                (type == MESSAGE_REARM_ON_WAY)) {
                who_from = SUPPORT_NAME;
            }

            message_queue_message(i, priority, timing, who_from, source, group,
                                  delay, type);
        ...
        if (persona_index >= 0) {
            ...
            persona_index =
                -999; // used here and the next line only -- hard code bad, but I'm lazy
        }
...
int
message_persona_name_lookup(char *name)
{
    int i;
...
message_maybe_distort_text(char *text)
{
    ...
    while (voice_duration > 0) {
        ...
        voice_duration -= fl2i(Distort_patterns[Distort_num][Distort_next] *
                               Message_wave_duration);
        Distort_next++;
        if (Distort_next >= MAX_DISTORT_LEVELS)
            Distort_next = 0;
