                          ...
                          char **desc)
{
    ...
    if ((strlen(fname) < 4) ||
        stricmp(fname + strlen(fname) - 4, FS_CAMPAIGN_FILE_EXT)) {
...
//
int
mission_campaign_get_mission_list(char *filename, char **list, int max)
{
    int rval, i, num = 0;
...
// global variables
void
mission_campaign_build_list(int multiplayer)
{
    Num_campaigns = 0;
    ...
    // via std::filesystem (the builtin campaign above comes from the VPs)
    {
        ...
        for (const auto &entry :
             std::filesystem::directory_iterator("data/missions", ec)) {
            if (entry.is_directory())
                continue;
            const std::string name = entry.path().filename().string();
            size_t extlen = strlen(FS_CAMPAIGN_FILE_EXT);
            if (name.size() <= extlen ||
                stricmp(name.c_str() + name.size() - extlen,
                        FS_CAMPAIGN_FILE_EXT))
                continue;
    ...
    if ((rval = setjmp(parse_abort)) != 0) {
        ...
        while (required_string_either("#End", "$Mission:")) {
            ...
            if (optional_string("+Mission Loop Text:")) {
                cm->mission_loop_desc = stuff_and_malloc_string(
                    F_MULTITEXT, NULL, MISSION_DESC_LENGTH);
            }

            cm->mission_loop_brief_anim = NULL;
            if (optional_string("+Mission Loop Brief Anim:")) {
                cm->mission_loop_brief_anim = stuff_and_malloc_string(
                    F_MULTITEXT, NULL, MAX_FILENAME_LEN);
            }

            cm->mission_loop_brief_sound = NULL;
            if (optional_string("+Mission Loop Brief Sound:")) {
                cm->mission_loop_brief_sound = stuff_and_malloc_string(
                    F_MULTITEXT, NULL, MAX_FILENAME_LEN);
            }
...
int
mission_campaign_load_by_name(char *filename)
{
...
int
mission_campaign_load_by_name_csfe(char *filename, char *callsign)
{
    Game_mode |= GM_NORMAL;
...
void
mission_campaign_savefile_generate_root(char *filename)
{
...
int
campaign_savefile_save(char *pname)
{
...
void
campaign_delete_save(char *cfn, char *pname)
{
    strcpy(Player->callsign, pname);
...
mission_campaign_savefile_load(char *cfilename)
{
    ...
    Assert(
        Game_mode &
        GM_NORMAL); // get allender or DaveB.  trying to save campaign in multiplayer
    ...
    Campaign.type = type_sig == CAMPAIGN_SINGLE_PLAYER_SIG
                        ? CAMPAIGN_TYPE_SINGLE
                        : CAMPAIGN_TYPE_MULTI_COOP;
    ...
    for (i = 0; i < Campaign.num_missions_completed; i++) {
        ...
        Campaign.missions[num].goals = (mgoal *)malloc(
            Campaign.missions[num].num_goals * sizeof(mgoal));
        if (Campaign.missions[num].num_goals > 0) {
            memset(Campaign.missions[num].goals, 0,
                   sizeof(mgoal) * Campaign.missions[num].num_goals);
        ...
        for (j = 0; j < Campaign.missions[num].num_goals; j++) {
            cfread_string_len(Campaign.missions[num].goals[j].name, NAME_LENGTH,
                              fp);
        ...
        Campaign.missions[num].events = (mevent *)malloc(
            Campaign.missions[num].num_events * sizeof(mevent));
        if (Campaign.missions[num].num_events > 0) {
            memset(Campaign.missions[num].events, 0,
                   sizeof(mevent) * Campaign.missions[num].num_events);
        ...
        for (j = 0; j < Campaign.missions[num].num_events; j++) {
            cfread_string_len(Campaign.missions[num].events[j].name, NAME_LENGTH,
                              fp);
...
void
campaign_savefile_load(char *fname, char *pname)
{
    if (Campaign.type == CAMPAIGN_TYPE_SINGLE) {
        Game_mode &= GM_NORMAL;
    }
...
mission_campaign_next_mission()
{
    if ((Campaign.next_mission == -1) ||
        (strlen(Campaign.name) ==
         0)) // will be set to -1 when there is no next mission
        return -1;

    Campaign.current_mission = Campaign.next_mission;
    strncpy(Game_current_mission_filename,
            Campaign.missions[Campaign.current_mission].name, MAX_FILENAME_LEN);
...
mission_campaign_previous_mission()
{
    ...
    memcpy(&Player->stats, &Campaign.missions[Campaign.current_mission].stats,
           sizeof(Player->stats));
...
mission_campaign_eval_next_mission()
{
    ...
    if (Campaign.missions[cur].formula != -1) {
        flush_sexp_tree(
            Campaign.missions[cur].formula); // force formula to be re-evaluated
        eval_sexp(
            Campaign.missions[cur]
                .formula); // this should reset Campaign.next_mission to proper value
    }
    ...
    if (Campaign.missions[cur].has_mission_loop) {
        ...
        if (Campaign.missions[cur].mission_loop_formula != -1) {
            ...
            eval_sexp(
                Campaign.missions[cur]
                    .mission_loop_formula); // this should reset Campaign.next_mission to proper value
        }
...
mission_campaign_store_goals_and_events()
{
    ...
    for (i = 0; i < Num_goals; i++) {
        if (strlen(Mission_goals[i].name) == 0) {
            ...
            strcpy(mission->goals[i].name, Mission_goals[i].name);
        Assert(Mission_goals[i].satisfied !=
...
int
mission_campaign_get_filenames(char *filename, char dest[][NAME_LENGTH], int *num)
{
    int rval;
...
int
mission_campaign_find_mission(char *name)
{
    int i;
...
void
mission_campaign_maybe_play_movie(int type)
{
    int mission;
    char *filename;
...
void
mission_campaign_save_persistent(int type, int sindex)
{
...
void
mission_campaign_skip_to_next(int start_game)
{
    ...
    if (start_game) {
        ...
        if ((Campaign.missions[Campaign.current_mission].has_mission_loop) &&
...
void
mission_campaign_jump_to_mission(char *name)
{
    int i = 0;
    char dest_name[64];
    ...
    for (i = 0; i < Campaign.num_missions; i++) {
        if ((Campaign.missions[i].name != NULL) &&
            !stricmp(Campaign.missions[i].name, name)) {
