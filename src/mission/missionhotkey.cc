...
static int Hotkey_bits
    [MAX_SHIPS]; // bitfield indicating which hotkeys are used by each ship
...
static hotkey_buttons Buttons[GR_NUM_RESOLUTIONS][NUM_BUTTONS] = {
    //XSTR:OFF
    { // GR_640
      hotkey_buttons("HKB_00", 1, 94, 0), hotkey_buttons("HKB_01", 1, 133, 1),
      hotkey_buttons("HKB_02", 15, 342, 2), hotkey_buttons("HKB_03", 84, 342, 3),
      hotkey_buttons("HKB_04", 161, 342, 4), hotkey_buttons("HKB_05", 539, 5, 5),
      hotkey_buttons("HKB_06", 539, 44, 6), hotkey_buttons("HKB_07", 539, 431, 7),
...
int
mission_hotkey_get_set_num(int k)
{
    int i;
...
mission_hotkey_maybe_restore()
{
    ...
    for (i = 0; i < Num_hotkeys_saved; i++) {
        ...
        if (index != -1) {
            hud_target_hotkey_add_remove(Hotkey_saved_info[i].setnum,
                                         &Objects[Ships[index].objnum],
                                         HOTKEY_USER_ADDED);
...
mission_hotkey_set_defaults()
{
    ...
    for (A = GET_FIRST(&obj_used_list); A != END_OF_LIST(&obj_used_list);
        ...
        Assert(sp->objnum >= 0);
        hud_target_hotkey_add_remove(sp->hotkey, &Objects[sp->objnum],
    ...
    for (i = 0; i < num_wings; i++) {
        ...
        for (j = 0; j < wp->current_count; j++) {
            ...
            hud_target_hotkey_add_remove(wp->hotkey, &Objects[sp->objnum],
                                         HOTKEY_MISSION_FILE_ADDED);
        }
    }
}
...
void
mission_hotkey_mf_add(int set, int objnum, int how_to_add)
{
...
int
hotkey_line_add(char *text, int type, int index, int y)
{
...
int
hotkey_line_insert(int n, char *text, int type, int index)
{
    int z;
...
// sorted by name
int
hotkey_line_add_sorted(char *text, int type, int index, int start)
{
    int z;
    ...
    z = Num_lines - 1;
    while ((z >= start) && ((Hotkey_lines[z].type == HOTKEY_LINE_SUBSHIP) ||
                            (stricmp(text, Hotkey_lines[z].label) < 0)))
        z--;
...
int
hotkey_build_team_listing(int team, int y)
{
    ship_obj *so;
    char *str = NULL;
    int i, j, s, z, start;
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if ((Ship_info[Ships[shipnum].ship_info_index].flags & SIF_HARMLESS) &&
            !(Ship_info[Ships[shipnum].ship_info_index].flags & SIF_ESCAPEPOD))
            continue;
        ...
        if (Ships[shipnum].flags &
            (SF_HIDDEN_FROM_SENSORS | SF_DYING | SF_DEPARTING))
            continue;
        ...
        if ((hotkey_get_team(shipnum) == team) && (Ships[shipnum].wingnum < 0)) {
            hotkey_line_add_sorted(Ships[shipnum].ship_name, HOTKEY_LINE_SHIP,
                                   shipnum, start);
        }
    }
    ...
    for (i = 0; i < num_wings; i++) {
        if (Wings[i].current_count &&
            (hotkey_get_team(Wings[i].ship_index[Wings[i].special_ship]) ==
             team)) {
            ...
            if (Wings[i].flags & WF_EXPANDED) {
                for (j = 0; j < Wings[i].current_count; j++) {
                    ...
                    z = hotkey_line_insert(z + 1, Ships[s].ship_name,
...
int
hotkey_line_query_visible(int n)
{
    int y;
    ...
    for (i = 0; i < MAX_KEYED_TARGETS; i++) {
        ...
        for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
            ...
            if (Hotkey_bits[Objects[so->objnum].instance] & (1 << i)) {
                hud_target_hotkey_add_remove(i, &Objects[so->objnum],
                                             HOTKEY_USER_ADDED);
            }
        }
    }
...
//
void
mission_hotkey_do_frame(float frametime)
{
    char buf[256];
    ...
    gr_printf(Hotkey_function_name_coords[gr_screen.res][0] +
    ...
    while (hotkey_line_query_visible(line)) {
        ...
        case HOTKEY_LINE_HEADING:
            ...
            i = y + h / 2 - 1;
            gr_line(Hotkey_list_coords[gr_screen.res][0], i,
                    Hotkey_ship_x[gr_screen.res] - 2, i);
            gr_line(Hotkey_ship_x[gr_screen.res] + w + 1, i,
        ...
        if (hotkeys) {
            for (i = 0; i < MAX_KEYED_TARGETS; i++) {
                if (hotkeys & (1 << i)) {
                    gr_printf(Hotkey_list_coords[gr_screen.res][0] +
                                  Hotkey_function_field_width[gr_screen.res] * i,
                              y, Scan_code_text[Key_sets[i]]);
                }
            }
            /*
         *buf = 0;
