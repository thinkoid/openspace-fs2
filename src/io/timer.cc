...
static longlong Timer_base =
    0; // CLOCK_MONOTONIC reading at timer_init, in microseconds
...
fix
timer_get_fixed_secondsX()
{
    return timer_get_fixed_seconds();
}

fix
timer_get_approx_seconds()
{
    return timer_get_fixed_seconds();
}
...
int
timestamp_has_time_elapsed(int stamp, int time)
{
    int t;
...
int
timing_event_lookup(char *event_name)
{
#ifndef USE_TIMING
    return -1;
#else
...
void
timing_event_start(char *event_name)
{
#ifndef USE_TIMING
    return;
#else
...
void
timing_event_stop(char *event_name)
{
#ifndef USE_TIMING
    return;
#else
    ...
    if (event != -1) {
        ...
        Timing_events[event].microseconds_total += timer_get_microseconds() -
                                                   Timing_events[event].start;
        Timing_events[event].ref_count--;
    }
#endif
}
...
int
timing_event_total(char *event_name)
{
#ifndef USE_TIMING
    return -1;
#else
...
float
timing_event_pct(char *event_name)
{
#ifndef USE_TIMING
    return 0.0f;
#else
    ...
    return (float)((double)Timing_events[event].microseconds_total /
                   (double)Timing_frame.microseconds_total);
#endif
}

// display timing
void
timing_display(int x, int y)
{
#ifndef USE_TIMING
    return;
#else
    ...
    for (idx = 0; idx < Timing_event_count; idx++) {
        gr_printf(x, y, "%s : %f\n", Timing_events[idx].name,
                  timing_event_pct(Timing_events[idx].name));
        y += 10;
    }
#endif
}
