...
void
mouse_mark_button(uint flags, int set)
{
    if (!mouse_inited)
        return;
...
int
mouse_down_count(int n, int reset_count)
{
    int tmp = 0;
    if (!mouse_inited)
        return 0;
...
void
mouse_get_delta(int *dx, int *dy, int *dz)
{
    if (dx)
        *dx = Mouse_dx;
    if (dy)
...
int
mouse_get_pos(int *xpos, int *ypos)
{
    int flags;
...
void
mouse_get_real_pos(int *mx, int *my)
{
    *mx = Mouse_x;
    *my = Mouse_y;
}
...
void
mouse_set_pos(int xpos, int ypos)
{
