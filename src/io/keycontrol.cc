...
extern float do_subobj_hit_stuff(object *ship_obj, object *other_obj,
                                 vector *hitpos, float damage);
...
int Critical_key_set[] = {
    CYCLE_SECONDARY,  CYCLE_NUM_MISSLES, INCREASE_WEAPON, DECREASE_WEAPON,
    INCREASE_SHIELD,  DECREASE_SHIELD,   INCREASE_ENGINE, DECREASE_ENGINE,
    ETS_EQUALIZE,     SHIELD_EQUALIZE,   SHIELD_XFER_TOP, SHIELD_XFER_BOTTOM,
    SHIELD_XFER_LEFT, SHIELD_XFER_RIGHT, XFER_SHIELD,     XFER_LASER,
};

int Non_critical_key_set[] = { CYCLE_NEXT_PRIMARY,
                               CYCLE_PREV_PRIMARY,
                               MATCH_TARGET_SPEED,
                               TOGGLE_AUTO_MATCH_TARGET_SPEED,
                               TARGET_NEXT,
...
void
debug_cycle_player_ship(int delta)
{
    if (Player_obj == NULL)
        return;
    ...
    HUD_sourced_printf(HUD_SOURCE_HIDDEN, XSTR("Player ship changed to %s", 0),
                       Ship_info[si_index].name);
}

// cycle targeted ship to next ship in that species
void
debug_cycle_targeted_ship(int delta)
{
    object *objp;
    ship_info *sip;
    ...
    while (TRUE) {
        ...
        if (sip->species == species &&
            (sip->flags & (SIF_FIGHTER | SIF_BOMBER | SIF_TRANSPORT)))
            break;
    ...
    HUD_sourced_printf(HUD_SOURCE_HIDDEN, XSTR("Changed player target to %s", 1),
                       Ship_info[si_index].name);
}

void
debug_max_secondary_weapons(object *objp)
{
    int index;
...
process_debug_keys(int k)
{
    ...
    case KEY_DEBUGGED + KEY_ALTED + KEY_F:
        Framerate_delay += 10;
        HUD_printf(XSTR("Framerate delay increased to %i milliseconds per frame.",
                        4),
                   Framerate_delay);
        break;
    ...
    case KEY_DEBUGGED + KEY_ALTED + KEY_SHIFTED + KEY_F:
        ...
        HUD_printf(XSTR("Framerate delay decreased to %i milliseconds per frame.",
                        5),
                   Framerate_delay);
        break;
    ...
    case KEY_DEBUGGED1 + KEY_SHIFTED + KEY_C:
        ...
        HUD_printf(XSTR("Countermeasure firing: %s", 6),
                   Countermeasures_enabled ? XSTR("ENABLED", 7)
                                           : XSTR("DISABLED", 8));
        break;
    ...
    case KEY_DEBUGGED1 + KEY_K:
        if (Player_ai->target_objnum != -1) {
            ...
            switch (objp->type) {
            case OBJ_SHIP:
                ship_apply_local_damage(objp, Player_obj, &objp->pos, 100000.0f,
                                        MISS_SHIELDS, CREATE_SPARKS);
                ship_apply_local_damage(objp, Player_obj, &objp->pos, 1.0f,
                                        MISS_SHIELDS, CREATE_SPARKS);
                break;
            case OBJ_WEAPON:
    ...
    case KEY_DEBUGGED + KEY_V:
        ...
        if (Messages[Message_debug_index].avi_info.index == -1) {
            HUD_printf("No anim set for message \"%s\"; None will play!",
                       Messages[Message_debug_index].name);
        }
        break;
    ...
    case KEY_DEBUGGED1 + KEY_SHIFTED + KEY_K:
        if ((Player_ai->target_objnum != -1) &&
            ...
            if (objp->type == OBJ_SHIP) {
                ...
                get_subsystem_world_pos(objp, Player_ai->targeted_subsys,
                                        &g_subobj_pos);

                do_subobj_hit_stuff(objp, Player_obj, &g_subobj_pos,
                                    (float)-Player_ai->targeted_subsys
                                        ->system_info->type); //100.0f);
    ...
    case KEY_DEBUGGED1 + KEY_ALTED + KEY_K: {
        ...
        ship_apply_local_damage(Player_obj, Player_obj, &pos, 25.0f, MISS_SHIELDS,
                                CREATE_SPARKS);
    ...
    case KEY_DEBUGGED1 + KEY_SHIFTED + KEY_I:
        if (Player_ai->target_objnum != -1) {
            ...
            HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                               XSTR("Player's target [%s] is %s", 13),
...
*/
    ...
    case KEY_DEBUGGED + KEY_N:
        AI_watch_object++;
        HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                           XSTR("Spewing debug info about object #%d", 14),
                           AI_watch_object);
        break;
    ...
    case KEY_DEBUGGED1 + KEY_SHIFTED + KEY_W:
        ...
        if (Weapon_energy_cheat) {
            ...
            HUD_sourced_printf(
                HUD_SOURCE_HIDDEN,
                XSTR("Normal weapon energy system / missile count restored", 17));
    ...
    case KEY_DEBUGGED1 + KEY_9:
        ...
        shipp->weapons
            .secondary_bank_weapons[shipp->weapons.current_secondary_bank]++;
        if (shipp->weapons
                .secondary_bank_weapons[shipp->weapons.current_secondary_bank] >=
            Num_weapon_types)
            shipp->weapons
        ...
        HUD_sourced_printf(
            ...
            Weapon_info[shipp->weapons.secondary_bank_weapons
    ...
    case KEY_DEBUGGED1 + KEY_U:
        ...
        object *asteroid_create(asteroid_field * asfieldp, int asteroid_type,
                                int subtype);
    ...
    case KEY_DEBUGGED1 + KEY_0:
        ...
        HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                           XSTR("Primary Weapon forced to %s", 19),
    ...
    case KEY_DEBUGGED + KEY_M: {
        if (Event_music_enabled) {
            event_music_disable();
            HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                               XSTR("Event music disabled", 20));
        }
        else {
            event_music_enable();
            HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                               XSTR("Event music enabled", 21));
        }
    ...
    case KEY_DEBUGGED + KEY_SHIFTED + KEY_T: {
        ...
        Test_begin = 1;
        HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                           XSTR("Frame Rate test started", 23));
    ...
    case KEY_DEBUGGED + KEY_S: {
        ...
        HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                           XSTR("Species of target changed to: %s", 24),
...
ppsk_hotkeys(int k)
{
    ...
    case KEY_F12 + KEY_SHIFTED:
        ...
        if (Player_ai->target_objnum == -1)
            ...
            hud_target_hotkey_add_remove(hotkey_set,
...
void
process_player_ship_keys(int k)
{
    int masked_k;
...
game_do_end_mission_popup()
{
    ...
    choice = popup(pf_flags, 3, POPUP_NO, XSTR("&Yes, Quit", 28),
                   XSTR("Yes, &Restart", 29),
...
#define WITHIN_BBOX()                                                            \
    do {                                                                         \
        ...
        if (pm != NULL) {                                                        \
            ...
            if ((gpos.x >= pm->mins.x * scale) &&                                \
                (gpos.y >= pm->mins.y * scale) &&                                \
                (gpos.z >= pm->mins.z * scale) &&                                \
                (gpos.x <= pm->maxs.x * scale) &&                                \
                (gpos.y <= pm->maxs.y * scale) &&                                \
                (gpos.z <= pm->maxs.z * scale)) {                                \
                collided = 1;                                                    \
            }                                                                    \
        }                                                                        \
    } while (0)
...
game_process_cheats(int k)
{
    ...
    if (!strcmp(Cheat_code_skip, cryptstring) &&
    ...
    if (!strcmp(Cheat_code_pirate, cryptstring) && (Game_mode & GM_IN_MISSION) &&
        (Player_obj != NULL)) {
        ...
        for (int idx = 0; idx < 1; idx++) {
            ...
            int objnum = ship_create(&vmd_identity_matrix, &add,
                                     Num_ship_types - 1);
...
int
button_function_critical(int n)
{
    object *objp;
    player *pl;
    ...
    case CYCLE_NUM_MISSLES:
        ...
        if (objp == Player_obj) {
            if (Player_ship->weapons.num_secondary_banks <= 0) {
                HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                                   XSTR("This ship has no secondary weapons",
                                        33));
        ...
        if (Ships[objp->instance].flags & SF_SECONDARY_DUAL_FIRE) {
            ...
            HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                               XSTR("Secondary weapon set to normal fire mode",
                                    34));
            ...
            Ships[objp->instance].flags |= SF_SECONDARY_DUAL_FIRE;
            HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                               XSTR("Secondary weapon set to dual fire mode",
                                    35));
            snd_play(&Snds[SND_SECONDARY_CYCLE]);
            hud_gauge_popup_start(HUD_WEAPONS_GAUGE);
        }
...
button_function_always(int n)
{
    ...
    case VIEW_EXTERNAL:
        ...
        Viewer_mode ^= VM_EXTERNAL;
        Viewer_mode &=
            ~VM_EXTERNAL_CAMERA_LOCKED; // reset camera lock when leave/entering external view
    ...
    case VIEW_EXTERNAL_TOGGLE_CAMERA_LOCK:
        ...
        if (Viewer_mode & VM_EXTERNAL) {
            ...
            if (Viewer_mode & VM_EXTERNAL_CAMERA_LOCKED) {
                HUD_sourced_printf(
                    HUD_SOURCE_HIDDEN,
                    XSTR("External camera is locked, controls will move ship",
                         36));
            }
            else {
                HUD_sourced_printf(
...
button_function(int n)
{
    ...
    case CYCLE_NEXT_PRIMARY:
        ...
        if (ship_select_next_primary(Player_obj, CYCLE_PRIMARY_NEXT)) {
            ...
            shipp->weapons
                .next_primary_fire_stamp[shipp->weapons.current_primary_bank] =
    ...
    case TOGGLE_AUTO_TARGETING:
        ...
        if (Players[Player_num].flags & PLAYER_FLAGS_AUTO_TARGETING) {
            if (hud_sensors_ok(Player_ship)) {
                hud_target_closest(opposing_team_mask(Player_ship->team), -1,
                                   FALSE, TRUE);
                snd_play(&Snds[SND_SHIELD_XFER_OK], 1.0f);
    ...
    case TARGET_CLOSEST_SHIP_ATTACKING_TARGET:
        ...
        if (hud_sensors_ok(Player_ship)) {
            hud_target_closest(opposing_team_mask(Player_ship->team),
    ...
    case END_MISSION:
        ...
        if (collide_predict_large_ship(Player_obj, 200.0f)) {
            gamesnd_play_iface(SND_GENERAL_FAIL);
            HUD_printf(XSTR(
                "** WARNING ** Collision danger.  Warpout not activated.", 39));
        }
        else if (ship_get_subsystem_strength(Player_ship, SUBSYSTEM_ENGINE) <
                 0.1f) {
            gamesnd_play_iface(SND_GENERAL_FAIL);
...
void
button_info_set(button_info *bi, int n)
{
...
void
button_info_unset(button_info *bi, int n)
{
    int field_num, bit_num;
...
int
button_info_query(button_info *bi, int n)
{
...
void
button_info_clear(button_info *bi)
{
    int i;
...
void
button_strip_noncritical_keys(button_info *bi)
{
    int idx;
