...
void
jumpnode_render(object *jumpnode_objp, vector *pos, vector *view_pos)
{
...
jumpnode_create(vector *pos)
{
    ...
    obj = obj_create(OBJ_JUMP_NODE, -1, Num_jump_nodes, NULL, pos,
                     model_get_radius(Jump_nodes[Num_jump_nodes].modelnum),
                     OF_RENDERS);
    sprintf(Jump_nodes[Num_jump_nodes].name, XSTR("Jump Node %d", 632),
            Num_jump_nodes);
    if (obj >= 0) {
