...
static MENU_REGION Medal_regions
    [NUM_MEDAL_REGIONS]; // a semi-hack for now because we only have 4 medals, but we also include the close button
...
parse_medal_tbl()
{
    ...
    while (required_string_either("#End", "$Name:")) {
        ...
        if (optional_string("+Num Kills:")) {
            ...
            stuff_string(Badge_info[bi].voice_base, F_NAME, NULL,
                         MAX_FILENAME_LEN);
    ...
    for (i = 0; i < MAX_BADGES - 1; i++) {
        if (Medals[Badge_index[i]].kills_needed >=
            Medals[Badge_index[i + 1]].kills_needed) {
            Error(
                LOCATION,
...
void
medal_main_init(player *pl, int mode)
{
    int idx;
    ...
    if (Medals_bitmap < 0) {
        Error(LOCATION, "Error loading medal background bitmap %s",
              Medals_background_filename[gr_screen.res]);
    }
    else {
    ...
    if (Medals_bitmap_mask < 0) {
        Error(LOCATION, "Error loading medal mask file %s",
              Medals_mask_filename[gr_screen.res]);
    }
    else {
...
void
medals_translate_name(char *name, int max_len)
{
...
blit_label(char *label, int *coords, int num)
{
    ...
    x = Medals_label_coords[gr_screen.res][0] +
        (Medals_label_coords[gr_screen.res][2] - sw) / 2;
...
medal_main_do()
{
    ...
    region = snazzy_menu_do((ubyte *)Medals_mask->data, Medals_mask_w,
                            Medals_mask_h, NUM_MEDAL_REGIONS, Medal_regions,
                            &selected);
    switch (region) {
    case ESC_PRESSED:
        if (Medals_mode == MM_NORMAL) {
    ...
    case RANK_MEDAL_REGION:
        blit_label(Ranks[Player_score->rank].name,
                   &Medal_coords[gr_screen.res][region][0], 1);
        break;
    ...
    default:
        if (Player_score->medals[region] > 0) {
            blit_label(Medals[region].name,
                       &Medal_coords[gr_screen.res][region][0],
                       Player_score->medals[region]);
        }
        break;
    } // end switch
...
init_snazzy_regions()
{
    ...
    snazzy_menu_add_region(&Medal_regions[RANK_MEDAL_REGION], "",
                           RANK_MEDAL_REGION, 0);
}
...
blit_medals()
{
    ...
    for (idx = 0; idx < NUM_MEDALS; idx++) {
        if (Player_score->medals[idx] > 0) {
            ...
            gr_bitmap(Medal_coords[gr_screen.res][idx][0],
                      Medal_coords[gr_screen.res][idx][1]);
        }
    }

    // now blit rank, since that "medal" doesnt get loaded (or drawn) the normal way
    gr_set_bitmap(Rank_bm);
    gr_bitmap(Medal_coords[gr_screen.res][RANK_MEDAL_REGION][0],
              Medal_coords[gr_screen.res][RANK_MEDAL_REGION][1]);
}
