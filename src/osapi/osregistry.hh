...
void os_config_write_string(const char *section, const char *name,
                            const char *value);

// same as previous function except we don't use the application name to build up the keyname
void os_config_write_string2(const char *section, const char *name,
                             const char *value);

// Writes an unsigned int to the INI file.
void os_config_write_uint(const char *section, const char *name,
                          unsigned int value);
...
// same buffer.
const char *os_config_read_string(const char *section, const char *name,
                                  const char *default_value = NULL);

// same as previous function except we don't use the application name to build up the keyname
const char *os_config_read_string2(const char *section, const char *name,
                                   const char *default_value = NULL);

// Reads a string from the INI file.  Default_value must
// be passed, and if 'name' isn't found, then returns default_value
unsigned int os_config_read_uint(const char *section, const char *name,
                                 unsigned int default_value);

// uses Ex versions of Windows registry functions
const char *os_config_read_string_ex(const char *keyname, const char *name,
