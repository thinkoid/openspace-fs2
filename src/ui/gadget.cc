...
//
void
UI_GADGET::link_hotspot(int num)
{
    hotspot_num = num;
    linked_to_hotspot = 1;
}
...
//
void
UI_GADGET::update_dimensions(int _x, int _y, int _w, int _h)
{
    if (_x != -1)
        x = _x;
    if (_y != -1)
...
void
UI_GADGET::get_dimensions(int *x_, int *y_, int *w_, int *h_)
{
    *x_ = x;
    *y_ = y;
    *w_ = w;
...
//
int
UI_GADGET::mouse_captured(UI_GADGET *gadget)
{
    if (!gadget)
        gadget = this;
...
//
void
UI_GADGET::base_create(UI_WINDOW *wnd, int _kind, int _x, int _y, int _w, int _h)
{
    int i;
...
//
void
UI_GADGET::set_callback(void (*_user_function)(void))
{
...
//
void
UI_GADGET::get_mouse_pos(int *xx, int *yy)
{
    if (xx)
        *xx = ui_mouse.x - x;
    if (yy)
...
//
int
UI_GADGET::is_mouse_on_children()
{
    UI_GADGET *tmp;
...
void
UI_GADGET::drag_with_children(int dx, int dy)
{
    UI_GADGET *tmp;
...
void
UI_GADGET::start_drag_with_children()
{
    UI_GADGET *tmp;
...
void
UI_GADGET::stop_drag_with_children()
{
    UI_GADGET *tmp;
...
//
void
UI_GADGET::set_parent(UI_GADGET *daddy)
{
    remove_from_family();
    parent = daddy;
