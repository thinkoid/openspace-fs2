...
void
UI_INPUTBOX::init_cursor()
{
    cursor_first_frame = bm_load_animation("cursor1", &cursor_nframes,
                                           &cursor_fps);
...
void
UI_INPUTBOX::set_invalid_chars(char *ichars)
{
    // free up any existing string
...
int
UI_INPUTBOX::validate_input(int chr)
{
...
UI_INPUTBOX::set_text(char *in)
{
    ...
    if (in_length > length)
        Assert(
            0); // tried to force text into an input box that won't fit into allocated memory
