...
UI_WINDOW::set_mask_bmap(int bmap, const char *name)
{
    ...
    if (bmap != mask_bmap_id) {
      ...
      */
        ...
        nprintf(
            ("UI", "Warning: tried to switch bitmap mask to the same bitmap\n"));
    }
}
...
void
UI_WINDOW::create(int _x, int _y, int _w, int _h, int _flags)
{
    x = _x;
    y = _y;
    w = _w;
...
UI_WINDOW::draw()
{
    ...
    if (flags & WIN_BORDER) {
        ui_draw_frame(x - BORDER_WIDTH, y - BORDER_WIDTH,
...
void
UI_WINDOW::draw_XSTR_forced(UI_GADGET *owner, int frame)
{
    int idx;
...
void
UI_WINDOW::render_tooltip(char *str)
{
    int w, h;
...
int
UI_WINDOW::process(int key_in, int process_mouse)
{
    UI_GADGET *tmp;
...
void
UI_WINDOW::check_focus_switch_keys()
{
    return;
}

void
UI_WINDOW::capture_mouse(UI_GADGET *gadget)
{
    mouse_captured_gadget = gadget;
}

void
UI_WINDOW::set_ignore_gadgets(int state)
{
    ignore_gadgets = state;
}

void
UI_WINDOW::add_XSTR(const char *string, int _xstr_id, int _x, int _y, UI_GADGET *_assoc,
...
void
UI_WINDOW::add_XSTR(UI_XSTR *xstr)
{
    int idx;
    int found = -1;
    UI_XSTR *x;
...
void
UI_WINDOW::draw_one_xstr(UI_XSTR *x, int frame)
{
    font *f_backup = NULL;
    char str[255] = "";
