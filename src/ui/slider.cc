                          ...
                          int _dot_width)
{
    ...
    if ((bm_left != NULL) || (left_mask != -1) || (bm_right != NULL) ||
        (right_mask != -1)) {
        Assert((bm_left != NULL) && (left_mask >= 0) && (bm_right != NULL) &&
               (right_mask >= 0));
...
void
UI_DOT_SLIDER_NEW::process(int focus)
{
    if (disabled_flag) {
...
void
UI_DOT_SLIDER::process(int focus)
{
    if (disabled_flag)
        return;
