...
//
void
UI_LISTBOX::link_hotspot(int up_button_num, int down_button_num)
{
...
void
UI_LISTBOX::create(UI_WINDOW *wnd, int _x, int _y, int _w, int _h, int _numitems,
                   char **_list, char *_check_list, int _max_items)
{
    ...
    if (_numitems > nrows) {
        scrollbar.create(wnd, _x + _w + 3, _y, real_h, 0, _numitems - nrows, 0,
                         nrows);
...
UI_LISTBOX::draw()
{
    ...
    if (uses_bmaps) {
        ...
        gr_reset_clip();
        if (has_scrollbar) {
            ui_draw_sunken_border(x - 2, y - 2, x + w + scrollbar.w + 4,
                                  y + h + 1);
        }
        else {
...
void
UI_LISTBOX::set_current(int _index)
{
    current_item = _index;
}

void
UI_LISTBOX::set_first_item(int _index)
{
    if (_index < 0)
        _index = 0;
...
char *
UI_LISTBOX::get_string(int _index)
{
    return list[_index];
}

void
UI_LISTBOX::clear_all_items()
{
    int idx;
...
void
UI_LISTBOX::set_new_list(int _numitems, char **_list)
{
    num_items = _numitems;
    list = _list;
    current_item = 0;
...
int
UI_LISTBOX::add_string(char *str)
{
