...
void
UI_BUTTON::set_hotkey_if_focus(int key)
{
    hotkey_if_focus = key;
}

void
UI_BUTTON::reset_status()
{
...
void
UI_BUTTON::draw_forced(int frame_num)
{
    if (uses_bmaps) {
...
UI_BUTTON::process(int focus)
{
    ...
    if (!(m_flags & BF_DOWN)) {
        next_repeat = 0;
        if ((old_flags & BF_DOWN) &&
            !(old_flags & BF_CLICKED)) // check for release of mouse, not hotkey
            m_flags |= BF_JUST_RELEASED;
...
//
void
UI_BUTTON::set_highlight_action(void (*user_function)(void))
{
    m_just_highlighted_function = user_function;
}

void
UI_BUTTON::set_disabled_action(void (*user_function)(void))
{
...
// Is the mouse over this button?
void
UI_BUTTON::set_button_hilighted()
{
    m_flags |= BF_HIGHLIGHTED;
}
...
void
UI_BUTTON::skip_first_highlight_callback()
{
...
void
UI_BUTTON::maybe_show_custom_cursor()
{
    if (disabled_flag)
        return;
    ...
    if (is_mouse_on()) {
        if ((custom_cursor_bmap >= 0) && (previous_cursor_bmap < 0)) {
            ...
            gr_set_cursor_bitmap(custom_cursor_bmap,
                                 GR_CURSOR_LOCK); // set and lock
        }
    }
}

void
UI_BUTTON::restore_previous_cursor()
{
    if (previous_cursor_bmap >= 0) {
        gr_set_cursor_bitmap(previous_cursor_bmap,
                             GR_CURSOR_UNLOCK); // restore and unlock
        previous_cursor_bmap = -1;
    }
}
