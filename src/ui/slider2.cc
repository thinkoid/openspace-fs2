                   ...
                   void (*_captureCallback)())
{
    ...
    bm_get_info(bmap_ids[S2_NORMAL], &buttonWidth, &buttonHeight, NULL, NULL,
                NULL);
...
UI_SLIDER2::process(int focus)
{
    ...
    if ((OnMe && B1_PRESSED &&
        ...
        if ((timer_get_milliseconds() > last_scrolled + 50) || B1_JUST_PRESSED ||
            mouse_locked) {
...
int
UI_SLIDER2::get_currentPosition()
{
...
void
UI_SLIDER2::set_numberItems(int _numberItems, int reset)
{
    numberItems = _numberItems;
...
void
UI_SLIDER2::set_currentItem(int _currentItem)
{
    ...
    currentPosition = fl2i(((float)currentItem / (float)numberItems) *
                           (float)numberPositions);
}

void
UI_SLIDER2::force_currentItem(int _currentItem)
{
