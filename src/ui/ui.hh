...
public:
    void create(UI_WINDOW *wnd, int _x, int _y, int _h, int _start, int _stop,
    ...
    int set_bmaps(char *up_button_fname, char *down_button_fname,
                  char *line_fname);
};
...
private:
    ...
    int slider_w, slider_h,
        slider_half_h; // this is the width and height and half height of the bitmap used for the slider
    virtual void draw();
    virtual void process(int focus = 0);
...
public:
    ...
    void set_numberItems(
        int _numberItems,
        int reset = 1); // change range. maybe reset back to position 0
    void
    set_currentItem(int _currentItem); // force slider to new position manually
    void force_currentItem(
        int _currentItem); // force slider to new position manually, _not_ calling any callbacks
...
public:
    void create(UI_WINDOW *wnd, int _x, int _y, int _w, int _h, int _numitem,
                char **_list, char *_check_list = NULL, int _max_items = -1);
    int selected(); // selected: Returns >= 0 if an item was selected
    int
    current(); // current:  Returns which item listbox bar is currently on. This could be -1, if none selected!
    int
    ...
    void
    clear_all_items(); // deletes all the items in the list (makes them empty strings)
    ...
    int set_bmaps(char *lbox_fname, char *b_up_fname, char *b_down_fname,
                  char *sb_fname);
    void link_hotspot(int up_button_num, int down_button_num);
};
...
protected:
    ...
    UI_XSTR
        *xstrs[MAX_UI_XSTRS]; // strings for drawing in code instead of in artwork
...
public:
    ...
    void destroy();
    ushort *get_mask_data(int *w, int *h)
    {
        *w = mask_w;
        *h = mask_h;
        return mask_data;
