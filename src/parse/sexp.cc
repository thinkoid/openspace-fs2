...
#include <model/model.hh> // for subsystem types
...
int Num_sexp_ai_goal_links = sizeof(Sexp_ai_goal_links) /
                             sizeof(sexp_ai_goal_link);
int Sexp_build_flag;
...
int
free_one_sexp(int num)
{
    Assert(Sexp_nodes[num].type !=
...
dup_sexp_chain(int node)
{
    ...
    cur = alloc_sexp(Sexp_nodes[node].text, Sexp_nodes[node].type,
                     Sexp_nodes[node].subtype, first, rest);
...
int
cmp_sexp_chains(int node1, int node2)
{
    if ((node1 == -1) && (node2 == -1)) {
        return 1;
    }
...
int
query_node_in_sexp(int node, int sexp)
{
    if (sexp == -1) {
        return 0;
    }
...
int
query_sexp_args_count(int index)
{
    int count = 0;
...
int
check_sexp_syntax(int index, int return_type, int recursive, int *bad_index,
                  int mode)
{
    ...
    op_index =
        index; // save the index of the operator since we need to get to other args.
    if (bad_index)
        *bad_index = op_index;
    ...
    while (index != -1) {
        ...
        if (Sexp_nodes[index].subtype == SEXP_ATOM_LIST) {
            ...
            if (type ==
                OPF_BOOL) // allow numbers to be used where boolean is required.
                type2 = OPR_BOOL;
            ...
            if (ship_name_lookup(CTEXT(index), 1) < 0) {
                if (mission_parse_ship_arrived(
                    ...
                    if (waypoint_lookup(CTEXT(index)) < 0) {
                        if (verify_vector(CTEXT(
                                index))) { // verify return non-zero on invalid point
        ...
        case OPF_SHIP_WING_POINT:
            ...
            if ((ship_name_lookup(CTEXT(index), 1) < 0) &&
                ...
                if (mission_parse_ship_arrived(
                    ...
                    if (waypoint_lookup(CTEXT(index)) < 0) {
                        if (verify_vector(CTEXT(
                                index))) { // non-zero on verify vector mean invalid!
        ...
        case OPF_SUBSYSTEM: {
            ...
            case OP_BEAM_FIRE:
                if (argnum == 1) {
                    ...
                    ship_index =
                        Sexp_nodes[Sexp_nodes[Sexp_nodes[op_index].rest].rest]
                            .rest;
                }
                break;
            ...
            for (i = 0; i < Ship_info[ship_class].n_subsystems; i++) {
                if (!stricmp(Ship_info[ship_class].subsystems[i].subobj_name,
                             CTEXT(index))) {
                    break;
                }
            }
        ...
        case OPF_AI_GOAL:
            ...
            if ((z = check_sexp_syntax(z, OPR_AI_GOAL, recursive, bad_index)) !=
                0) {
                return z;
            }
...
//
void
build_sexp_text_string(char *buffer, int node, int mode)
{
    if (Sexp_nodes[node].type & SEXP_FLAG_VARIABLE) {
        int sexp_variables_index = get_index_sexp_variable_name(
        ...
        if (Sexp_nodes[node].subtype == SEXP_ATOM_NUMBER) {
            Assert(Sexp_variables[sexp_variables_index].type &
...
int
build_sexp_string(int cur_node, int level, int mode)
{
    char pstr[128];
    int len, offset, node;
...
void
build_extended_sexp_string(int cur_node, int level, int mode)
{
    char pstr[128];
    int i, flag = 0, node;
...
void
convert_sexp_to_string(int cur_node, char *outstr, int mode)
{
    Sexp_string = outstr;
    *outstr = 0;
    if (cur_node >= 0)
...
int
sexp_query_has_yet_to_arrive(char *name)
{
    int i;
    ...
    if ((i >= 0) && (Wings[i].num_waves >= 0) &&
        (Wings[i].flags & WF_NEVER_EXISTED)) {
        return 1;
    }
...
int
rand_internal(int low, int high)
{
    int diff;
...
int
rand_sexp(int n, int multiple = 0)
{
    int low = 0;
    int high = 0;
    int rand_num = 0;
...
sexp_or(int n)
{
    ...
    if (n != -1) {
        if (Sexp_nodes[n].first != -1) {
            ...
            if (Sexp_nodes[Sexp_nodes[n].first].value !=
        ...
        while (Sexp_nodes[n].rest != -1) {
            ...
            if (Sexp_nodes[Sexp_nodes[n].rest].value == SEXP_KNOWN_TRUE)
                return SEXP_KNOWN_TRUE; // if one of the OR clauses is TRUE, whole clause is true
            if (Sexp_nodes[Sexp_nodes[n].rest].value !=
                SEXP_KNOWN_FALSE) // if the value is still unknown, they all can't be false
                all_false = 0;
            n = Sexp_nodes[n].rest;
            // don't return on true value -- keep evaluating for mission log purposes
...
int
sexp_and_in_sequence(int n)
{
    int result = -1;
    int all_true;

    all_true =
        1; // represents whether or not all nodes we have seen so far are true
    if (n != -1) {
        if (Sexp_nodes[n].first != -1) {
            result &= eval_sexp(CAR(n));
            if (Sexp_nodes[Sexp_nodes[n].first].value == SEXP_KNOWN_FALSE)
...
sexp_not(int n)
{
    ...
    if (n != -1) {
        if (Sexp_nodes[n].first != -1) {
            ...
            else if (Sexp_nodes[CAR(n)].value ==
                     SEXP_KNOWN_TRUE) // not KNOWN_TRUE == KNOWN_FALSE
                return SEXP_KNOWN_FALSE;
            else if (Sexp_nodes[CAR(n)].value ==
                     SEXP_NAN) // not NAN == TRUE (I think)
                return 1;
            else if (Sexp_nodes[CAR(n)].value == SEXP_NAN_FOREVER)
                return 1;
        }
...
sexp_gt(int n)
{
    ...
    if ((Sexp_nodes[CAR(n)].value == SEXP_NAN) ||
        (Sexp_nodes[CDR(n)].value == SEXP_NAN))
        return 0;
    else if ((Sexp_nodes[CAR(n)].value == SEXP_NAN_FOREVER) ||
...
sexp_is_destroyed(int n, fix *latest_time)
{
    ...
    while (n != -1) {
        ...
        if (mission_log_get_time(LOG_SHIP_DEPART, name, NULL, NULL) ||
            mission_log_get_time(LOG_WING_DEPART, name, NULL, NULL))
            return SEXP_KNOWN_FALSE;
        ...
        if (mission_log_get_time(LOG_SHIP_DESTROYED, name, NULL, &time) ||
            ...
            mission_log_get_time(LOG_SELF_DESTRUCT, name, NULL, &time)) {
            num_destroyed++;
...
int
sexp_is_subsystem_destroyed(int n)
{
    ...
    if (mission_log_get_time(LOG_SHIP_SUBSYS_DESTROYED, ship_name, subsys_name,
                             NULL))
        return SEXP_KNOWN_TRUE;
...
int
sexp_has_docked(int n)
{
    char *docker = CTEXT(n);
    char *dockee = CTEXT(CDR(n));
    int count = atoi(
        CTEXT(CDR(CDR(n)))); // count of times that we should look for
...
sexp_has_undocked(int n)
{
    ...
    Assert(count > 0);
    if (!mission_log_get_time_indexed(LOG_SHIP_UNDOCK, docker, dockee, count,
                                      NULL)) {
...
int
sexp_has_arrived(int n, fix *latest_time)
{
    char *name;
    ...
    while (n != -1) {
        ...
        if (mission_log_get_time(LOG_SHIP_ARRIVE, name, NULL, &time) ||
            mission_log_get_time(LOG_WING_ARRIVE, name, NULL, &time)) {
            num_arrived++;
            if (latest_time && (time > *latest_time))
                *latest_time = time;
        }
...
int
sexp_has_departed(int n, fix *latest_time)
{
    char *name;
    ...
    while (n != -1) {
        ...
        if (mission_log_get_time(LOG_SHIP_DESTROYED, name, NULL, NULL) ||
            ...
            return SEXP_KNOWN_FALSE;
        else if (mission_log_get_time(LOG_SHIP_DEPART, name, NULL, &time) ||
                 mission_log_get_time(LOG_WING_DEPART, name, NULL, &time)) {
            num_departed++;
            if (latest_time && (time > *latest_time))
                *latest_time = time;
        }
...
int
sexp_is_disabled(int n, fix *latest_time)
{
    char *name;
    ...
    while (n != -1) {
        ...
        // the sexpression is not true.
        if (mission_log_get_time(LOG_SHIP_DEPART, name, NULL, &time) ||
...
int
sexp_is_disarmed(int n, fix *latest_time)
{
    char *name;
...
int
sexp_is_subsystem_destroyed_delay(int n)
{
    char *ship_name, *subsys_name;
    fix delay, time;
...
sexp_has_docked_delay(int n)
{
    ...
    if (!mission_log_get_time_indexed(LOG_SHIP_DOCK, docker, dockee, count,
                                      &time))
        return 0;
...
int
sexp_are_waypoints_done_delay(int n)
{
    char *ship_name, *waypoint_name;
    fix time, delay;
    ...
    // now check the log for the waypoints done entry
    if (mission_log_get_time(LOG_WAYPOINTS_DONE, ship_name, waypoint_name,
                             &time)) {
        if ((Missiontime - time) >= delay)
            return SEXP_KNOWN_TRUE;
    }
...
int
sexp_ship_type_destroyed(int n)
{
    int percent, type;
    char *shiptype;
...
sexp_special_warp_dist(int n)
{
    ...
    // false
    if (mission_log_get_time(LOG_SHIP_DESTROYED, ship_name, NULL, NULL) ||
        mission_log_get_time(LOG_SHIP_DEPART, ship_name, NULL, NULL)) {
        return SEXP_NAN_FOREVER;
    }
    ...
    Assert((Ships[shipnum].special_warp_objnum >= 0) &&
           (Ships[shipnum].special_warp_objnum < MAX_OBJECTS));
    if ((Ships[shipnum].special_warp_objnum < 0) &&
        (Ships[shipnum].special_warp_objnum >= MAX_OBJECTS)) {
        return SEXP_NAN;
    }
    ...
    if (warp_objp->type == OBJ_SHIP) {
        if (Ship_info[Ships[warp_objp->instance].ship_info_index].flags &
            SIF_KNOSSOS_DEVICE) {
            valid = TRUE;
        }
    }
    ...
    float dot = fl_abs(
        vm_vec_dotprod(&warp_objp->orient.fvec, &ship_objp->orient.fvec));
    if (dot < 0.707f) {
        return SEXP_NAN;
    }
    ...
    // get distance
    vector hit_pt;
    float dist = fvi_ray_plane(&hit_pt, &warp_objp->pos, &warp_objp->orient.fvec,
                               &ship_objp->pos, &ship_objp->orient.fvec, 0.0f);
...
int
sexp_time_destroyed(int n)
{
    fix time;

    if (!mission_log_get_time(LOG_SHIP_DESTROYED, CTEXT(n), NULL,
...
int
sexp_time_wing_destroyed(int n)
{
    fix time;
...
sexp_shields_left(int n)
{
    ...
    if (mission_log_get_time(LOG_SHIP_DESTROYED, shipname, NULL, NULL) ||
        mission_log_get_time(LOG_SHIP_DEPART, shipname, NULL, NULL)) {
        return SEXP_NAN_FOREVER;
    }
    ...
    percent = (int)(get_shield_strength(&Objects[Ships[shipnum].objnum]) /
                    Ship_info[Ships[shipnum].ship_info_index].shields * 100.0f);
    return percent;
}
...
sexp_is_ship_visible(int n)
{
    ...
    if (Player_ship != NULL && !ship_is_visible) {
        float awacs_level = awacs_get_level(&Objects[Ships[shipnum].objnum],
                                            Player_ship);
        if (awacs_level >= 1.0f) {
            ship_is_visible = 2;
        }
...
int
sexp_hits_left_subsystem(int n)
{
        ...
        if (type == SUBSYSTEM_UNKNOWN) {
            ...
            while (ss != END_OF_LIST(&Ships[shipnum].subsys_list)) {
                if (!stricmp(ss->system_info->subobj_name, subsys_name)) {
                    percent = (int)(ss->current_hits / ss->system_info->max_hits *
                                    100.0f);
                    return percent;
                }
            ...
            percent = (int)(ship_get_subsystem_strength(&Ships[shipnum], type) *
                            100.0f);
            return percent;
        }
    }
...
sexp_distance(int n)
{
    ...
    // check to see if either ship was destroyed or departed.  If so, then make this node known
    // false
    if (mission_log_get_time(LOG_SHIP_DESTROYED, sname1, NULL, NULL) ||
        mission_log_get_time(LOG_SHIP_DEPART, sname1, NULL, NULL) ||
        mission_log_get_time(LOG_SHIP_DESTROYED, sname2, NULL, NULL) ||
        mission_log_get_time(LOG_SHIP_DEPART, sname2, NULL, NULL))
        return SEXP_NAN_FOREVER;

    // one of the names might be the name of a wing.  Check to see if the wing is detroyed or departed
    if (mission_log_get_time(LOG_WING_DESTROYED, sname1, NULL, NULL) ||
        mission_log_get_time(LOG_WING_DEPART, sname1, NULL, NULL) ||
        mission_log_get_time(LOG_WING_DESTROYED, sname2, NULL, NULL) ||
        mission_log_get_time(LOG_WING_DEPART, sname2, NULL, NULL))
        return SEXP_NAN_FOREVER;
...
int
sexp_distance2(int obj1, char *subj)
{
...
int
sexp_distance3(int obj1, int obj2)
{
    if ((obj1 == -1) ||
        (obj2 == -1)) // if either object isn't present in the mission now
...
int
sexp_skill_level_at_least(int n)
{
    int i;
    char *level_name;
...
int
sexp_was_promotion_granted(int n)
{
...
int
sexp_percent_ships_depart_destroy(int n, int what)
{
    int percent;
    int total, count;
    char *name;
...
sexp_depart_node_delay(int n)
{
    ...
    while (n != -1) {
        ...
        if (mission_log_get_time(LOG_SHIP_DESTROYED, name, NULL, NULL))
            return SEXP_KNOWN_FALSE;
        else if (mission_log_get_time(LOG_SHIP_DEPART, name, jump_node_name,
                                      &this_time)) {
            num_departed++;
...
// have departed.
int
sexp_destroyed_departed_delay(int n)
{
    int count, total;
    ...
    while (n != -1) {
        ...
        else if (mission_log_get_time(LOG_SHIP_DESTROYED, name, NULL,
                                      &time_gone)) {
            count++;
        }
...
int
sexp_special_warpout_name(int node)
{
    ...
    if (mission_log_get_time(LOG_SHIP_DESTROYED, ship_name, NULL, NULL) ||
        ...
        mission_log_get_time(LOG_SHIP_DEPART, knossos, NULL, NULL))
        return SEXP_NAN_FOREVER;
...
// of given ships
int
sexp_is_cargo_known(int n, int check_delay)
{
                ...
                if (shipnum != -1) {
                    if (Ships[shipnum].flags & SF_CARGO_REVEALED) {
                        time_known = Missiontime -
                                     Ships[shipnum].time_cargo_revealed;
                        if (f2i(time_known) >= delay)
                            is_known = 1;
                    }
                }
...
int
sexp_has_been_tagged_delay(int n)
{
    int count, shipnum, num_known, delay;
    char *name;
            ...
            if (exited_index != -1) {
                ...
                if (shipnum != -1) {
                    if (Ships[shipnum].time_first_tagged != 0) {
                        time_known = Missiontime -
                                     Ships[shipnum].time_first_tagged;
                        if (f2i(time_known) >= delay)
                            is_known = 1;
                    }
                }
...
int
sexp_cap_subsys_cargo_known_delay(int n)
{
    ...
    while (n != -1) {
        ...
        if (Sexp_nodes[n].value == SEXP_KNOWN_TRUE) {
            ...
            logged = mission_log_get_time(LOG_CAP_SUBSYS_CARGO_REVEALED,
                                          ship_name, subsys_name, &time_known);
            if (logged) {
            ...
            if ((mission_log_get_time(LOG_SHIP_DEPART, ship_name, NULL, NULL) ||
                 mission_log_get_time(LOG_SHIP_DESTROYED, ship_name, NULL,
                                      NULL)) &&
                !logged) {
                return SEXP_KNOWN_FALSE;
            }
...
waypoint_lookup(char *name)
{
    ...
    while (ptr != END_OF_LIST(&obj_used_list)) {
        if (ptr->type == OBJ_WAYPOINT) {
            i = ptr->instance;
            sprintf(buf, "%s:%d", Waypoint_lists[i / 65536].name,
                    (i & 0xffff) + 1);
            if (!stricmp(buf, name))
...
sexp_is_iff(int n)
{
    ...
    if (!stricmp(iff, "friendly"))
        ...
        Int3();
        mprintf((
            "Warning: Team %s no longer supported.  Just Friendly and Hostile.\n",
            iff));
        team = TEAM_HOSTILE;
    }
...
sexp_add_ship_goal(int n)
{
    ...
    sindex = CDR(n);
    ai_add_ship_goal_sexp(sindex, AIG_TYPE_EVENT_SHIP,
                          &(Ai_info[Ships[num].ai_index]));
}

// identical to above, except add a wing
void
sexp_add_wing_goal(int n)
{
    int num, sindex;
    char *wing_name;
...
// and priority.
void
sexp_send_one_message(char *name, char *who_from, char *priority, int group,
                      int delay)
{
    ...
    if (who_from[0] == '#') {
        message_send_unique_to_player(name, &(who_from[1]),
                                      MESSAGE_SOURCE_SPECIAL, ipriority, group,
                                      delay);
        return;
    }
    ...
    else if (mission_log_get_time(LOG_SHIP_DESTROYED, who_from, NULL, NULL) ||
             mission_log_get_time(LOG_SHIP_DEPART, who_from, NULL, NULL) ||
             mission_log_get_time(LOG_WING_DESTROYED, who_from, NULL, NULL) ||
             mission_log_get_time(LOG_WING_DEPART, who_from, NULL, NULL)) {
    ...
    else if (!stricmp(who_from, "<any wingman>") ||
...
void
sexp_send_random_message(int n)
{
    char *name, *who_from, *priority;
...
void
sexp_set_subsystem_strength(int n)
{
    char *shipname, *subsystem;
    int percentage, shipnum, index;
    ship *shipp;
    ...
    if (percentage > 100) {
        nprintf(
            ("Warning",
             "percentage for set_subsystem_strength > 100 -- setting to 100\n"));
        percentage = 100;
    }
    else if (percentage < 0) {
        nprintf(("Werning",
                 "percantage for set_subsystem_strength < 0 -- setting to 0\n"));
        percentage = 0;
    }
...
// as valid or invalid
void
sexp_change_goal_validity(int n, int flag)
{
    char *name;
...
sexp_transfer_cargo(int n)
{
    ...
    if (!stricmp(Cargo_names[Ships[shipnum1].cargo1 & CARGO_INDEX_MASK],
                 "nothing")) {
    ...
    if (!(Ship_info[Ships[shipnum2].ship_info_index].flags &
          (SIF_BIG_SHIP | SIF_HUGE_SHIP))) {
        if (stricmp(Cargo_names[Ships[shipnum2].cargo1 & CARGO_INDEX_MASK],
                    "nothing")) {
            Warning(
                LOCATION,
                "Transfering cargo to %s which already\nhas cargo %s.\nCargo will be replaced",
...
sexp_cargo_no_deplete(int n)
{
    ...
    if (!(Ship_info[Ships[ship_index].ship_info_index].flags &
          (SIF_BIG_SHIP | SIF_HUGE_SHIP))) {
        Warning(
            LOCATION,
...
sexp_good_time_to_rearm(int n)
{
    ...
    team_name = CTEXT(n);
    time = atoi(CTEXT(CDR(
        n))); // this is the time for how long a good rearm is active -- in seconds
    for (i = 0; i < Num_team_names; i++) {
...
sexp_grant_medal(int n)
{
    ...
    Assert(Player->stats.m_medal_earned <
           0); // Mission has problems.  Tried to grant 2 medals in 1 mission.
    medal_name = CTEXT(n);
    for (i = 0; i < NUM_MEDALS; i++) {
        if (!stricmp(medal_name, Medals[i].name))
            break;
...
void
sexp_deal_with_warp(int n, int should_break, int nix)
{
    int index, flag;
    char *name;
    ...
    for (; n != -1; n = CDR(n)) {
        ...
        if (mission_log_get_time(LOG_SHIP_DEPART, name, NULL, NULL) ||
            mission_log_get_time(LOG_SHIP_DESTROYED, name, NULL, NULL))
            continue;
...
void
sexp_good_secondary_time(int n)
{
...
int
sexp_previous_goal_status(int n, int status)
{
    ...
    if (Game_mode & GM_CAMPAIGN_MODE) {
        ...
        if (i == -1) {
            ...
            for (i = 0; i < Campaign.missions[mission_num].num_goals; i++) {
                if (!stricmp(Campaign.missions[mission_num].goals[i].name,
                             goal_name))
                    break;
            }
...
int
sexp_previous_event_status(int n, int status)
{
    char rval = 0, *mission_name;
    char *name;
    int i, mission_num, default_value = 0, use_defaults = 1;
...
int
sexp_event_status(int n, int want_true)
{
    char *name;
    int i, result;
...
int
sexp_event_delay_status(int n, int want_true)
{
    char *name;
    int i, result;
    fix delay;
...
int
sexp_goal_delay_status(int n, int want_true)
{
    char *name;
    fix delay, time;
...
// or cleared (flag==0)
void
sexp_protect_ships(int n, int flag)
{
    char *ship_name;
    int num;
...
// or cleared (flag==0)
void
sexp_beam_protect_ships(int n, int flag)
{
    char *ship_name;
    int num;
...
void
sexp_ships_visible(int n, int visible)
{
    char *ship_name;
    int num;
...
void
sexp_ships_invulnerable(int n, int invulnerable)
{
    char *ship_name;
    object *objp;
    int num;
...
void
sexp_ships_guardian(int n, int guardian)
{
    char *ship_name;
    object *objp;
    int num;
...
sexp_targeted(int node)
{
    ...
    if (CDR(node) >= 0) {
        ...
        if (CDR(CDR(node)) >= 0) {
            ...
            if (!ptr ||
                stricmp(ptr->system_info->subobj_name, CTEXT(CDR(CDR(node))))) {
                return 0;
            }
        }
    }
...
sexp_speed(int node)
{
    if (Training_context & TRAINING_CONTEXT_SPEED) {
        if (Training_context_speed_set) {
            if (timestamp_has_time_elapsed(Training_context_speed_timestamp,
                                           atoi(CTEXT(node)) * 1000)) {
                return SEXP_KNOWN_TRUE;
            }
        }
    }
...
int
sexp_secondaries_depleted(int node)
{
...
{
    if (Training_context & TRAINING_CONTEXT_FLY_PATH) {
        if (Training_context_goal_waypoint ==
            Waypoint_lists[Training_context_path].count) {
            return 1;
        }
    }
...
void
sexp_send_training_message(int node)
{
    int t = -1, delay = 0;
...
int
sexp_shield_recharge_pct(int node)
{
    int sindex;
...
int
sexp_engine_recharge_pct(int node)
{
    int sindex;
...
int
sexp_weapon_recharge_pct(int node)
{
    int sindex;
...
sexp_shield_quad_low(int node)
{
    ...
    if ((Ships[sindex].ship_info_index < 0) ||
        (Ships[sindex].ship_info_index >= Num_ship_types)) {
        return 0;
    }
...
int
sexp_secondary_ammo_pct(int node)
{
    ship *shipp;
    int sindex;
    int check, idx;
    ...
    if (check == 3) {
        for (idx = 0; idx < shipp->weapons.num_secondary_banks; idx++) {
            ret_sum[idx] =
                (int)(((float)shipp->weapons.secondary_bank_ammo[idx] /
                       (float)shipp->weapons.secondary_bank_start_ammo[idx]) *
                      100.0f);
        }
        ...
        ret = (int)(((float)shipp->weapons.secondary_bank_ammo[check] /
                     (float)shipp->weapons.secondary_bank_start_ammo[check]) *
                    100.0f);
    }
...
sexp_beam_fire(int node)
{
    ...
    fire_info.target_subsys = ship_get_subsys(&Ships[sindex],
                                              CTEXT(CDR(CDR(CDR(node)))));
...
sexp_beam_free_all(int node)
{
    ...
    while (subsys != END_OF_LIST(&Ships[sindex].subsys_list)) {
        ...
        if (subsys->system_info->type == SUBSYSTEM_TURRET) {
            ...
            subsys->turret_next_fire_stamp = timestamp(
                (int)frand_range(50.0f, 4000.0f));
        }
...
void
sexp_turret_tagged_only_all(int node)
{
    ship_subsys *subsys;
    int sindex;
...
void
sexp_turret_tagged_clear_all(int node)
{
    ship_subsys *subsys;
    int sindex;
...
void
sexp_add_remove_escort(int node)
{
    int sindex;
    int flag;
    char *whee;
...
void
sexp_awacs_set_radius(int node)
{
    int sindex;
    ship_subsys *awacs;
...
sexp_num_type_kills(int node)
{
    ...
    for (idx = 0; idx < Num_ship_types; idx++) {
        if ((p->stats.m_okKills[idx] > 0) &&
            (Ship_info[idx].flags & Ship_type_flags[st_index])) {
...
void
sexp_subsys_set_random(int node)
{
...
void
sexp_supernova_start(int node)
{
    supernova_start(atoi(CTEXT(node)));
}

int
sexp_is_secondary_selected(int node)
{
    int sindex;
    int bank;
    ship *shipp;
...
int
sexp_is_primary_selected(int node)
{
    int sindex;
    int bank;
    ship *shipp;
...
shield_quad_near_max(int quadnum)
{
    ...
    if ((remaining < 2.0f) ||
        (Player_obj->shields[quadnum] >
         Ship_info[Player_ship->ship_info_index].shields / MAX_SHIELD_SECTIONS -
             5.0f)) {
        return SEXP_TRUE;
    }
    else {
...
process_special_sexps(int index)
{
    ...
    case 1: // Fired Interceptors
        ...
        for (objp = GET_FIRST(&obj_used_list);
            ...
            if (objp->type == OBJ_WEAPON) {
                if (!stricmp(Weapon_info[Weapons[objp->instance].weapon_info_index]
                                 .name,
                             "Interceptor#weak")) {
    ...
    case 2: // Ship "Freighter 1", subsystem "Weapons" is aspect locked by player.
        if (Player_ai->target_objnum != -1) {
            if (!(stricmp(
                    Ships[Objects[Player_ai->target_objnum].instance].ship_name,
                    "Freighter 1"))) {
                if (!(stricmp(Player_ai->targeted_subsys->system_info->name,
                              "Weapons"))) {
                    if (Player_ai->current_target_is_locked) {
                        return SEXP_TRUE;
                    }
    ...
    case 5: // Player's shield is quick repaired
        ...
        if (Player_obj->shields[FRONT_QUAD] >
            Ship_info[Player_ship->ship_info_index].shields / 4.0f)
            Player_obj->shields[FRONT_QUAD] =
                Ship_info[Player_ship->ship_info_index].shields / 4.0f;

        //hud_shield_quadrant_hit(Player_obj, FRONT_QUAD);
        if (Player_obj->shields[FRONT_QUAD] >
            Player_obj->shields[(FRONT_QUAD + 1) % 4] - 2.0f)
            return SEXP_TRUE;
        else
            return SEXP_FALSE;
        break;
...
int
sexp_special_training_check(int node)
{
    int num, rtn;
...
void
sexp_set_training_context_fly_path(int node)
{
    int i;
...
void
sexp_set_training_context_speed(int node)
{
...
eval_sexp(int cur_node)
{
    ...
    if (Sexp_nodes[cur_node].first != -1) {
        ...
        case OP_CARGO_KNOWN_DELAY:
            sexp_val = sexp_is_cargo_known(node,
                                           (op_num == OP_IS_CARGO_KNOWN) ? 0 : 1);
            break;
        ...
        case OP_BEAM_UNPROTECT_SHIP:
            sexp_beam_protect_ships(node,
                                    (op_num == OP_BEAM_PROTECT_SHIP ? 1 : 0));
            sexp_val = 1;
            break;
        ...
        case OP_SHIP_INVULNERABLE:
            sexp_ships_invulnerable(node,
                                    (op_num == OP_SHIP_INVULNERABLE ? 1 : 0));
            sexp_val = 1;
            break;
        ...
        case OP_VALIDATE_GOAL:
            sexp_change_goal_validity(node,
                                      (op_num == OP_INVALIDATE_GOAL ? 0 : 1));
            sexp_val = 1;
            break;
...
int
query_operator_return_type(int op)
{
    if (op < FIRST_OP) {
...
int
query_operator_argument_type(int op, int argnum)
{
    int index = op;
...
void
update_block_names(const char *old_name, const char *new_name)
{
    int i;
...
void
update_sexp_references(char *old_name, char *new_name)
{
    int i;
...
// Strictly used in FRED
void
update_sexp_references(char *old_name, char *new_name, int format)
{
    int i;
...
void
update_sexp_references(char *old_name, char *new_name, int format, int node)
{
    int i, n, op;
    ...
    if ((SEXP_NODE_TYPE(node) == SEXP_LIST) &&
        ...
        if (Sexp_nodes[node].first) {
            update_sexp_references(old_name, new_name, format,
                                   Sexp_nodes[node].first);
        }
        if (Sexp_nodes[node].rest) {
            update_sexp_references(old_name, new_name, format,
                                   Sexp_nodes[node].rest);
        }
...
int
query_sexp_ai_goal_valid(int sexp_ai_goal, int ship)
{
    int i, op;
...
int
extract_sexp_variable_index(int node)
{
...
int
sexp_add_variable(const char *text, const char *var_name, int type, int index)
{
...
void
sexp_modify_variable(char *text, int index)
{
    Assert(index >= 0 && index < MAX_SEXP_VARIABLES);
...
sexp_modify_variable(int n)
{
    ...
    if (n != -1) {
        ...
        if (Sexp_variables[sexp_variable_index].type & SEXP_VARIABLE_NUMBER) {
            ...
            // get new string
            Assert(Sexp_variables[sexp_variable_index].type &
                   SEXP_VARIABLE_STRING);
...
sexp_variable_count()
{
    ...
    for (int i = 0; i < MAX_SEXP_VARIABLES; i++) {
        if ((Sexp_variables[i].type & SEXP_VARIABLE_SET) &&
            !(Sexp_variables[i].type & SEXP_VARIABLE_BLOCK)) {
            count++;
        }
    }
...
int
sexp_var_compare(const void *var1, const void *var2)
{
    int set1, set2;
...
sexp_variable_sort()
{
    qsort((void *)Sexp_variables,
          (size_t)(MAX_SEXP_VARIABLES - sexp_variable_block_count()),
