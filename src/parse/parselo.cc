...
next_tokens()
{
    ...
    while (((ch = *pstr++) != EOLN) && (ch != EOF_CHAR) &&
...
error_display(int error_level, char *format, ...)
{
    ...
    nprintf((error_text, "%s(%i):%s: ", Current_filename, get_line_num(),
             error_text));
    ...
    Warning(LOCATION, "%s(%i):\n%s: %s", Current_filename, get_line_num(),
            error_text, buffer);
}
...
int
skip_to_start_of_strings(char *pstr1, char *pstr2)
{
    int len1, len2;
    ...
    while ((*Mp != EOF_CHAR) && strnicmp(pstr1, Mp, len1) &&
           strnicmp(pstr2, Mp, len2)) {
        advance_to_eoln(NULL);
        ignore_white_space();
    }
...
copy_to_next_white(char *outstr, char *instr, int max)
{
    ...
    while (((ch = *instr++) > 0) && (ch != EOLN) && (ch != EOF_CHAR) &&
        ...
        if (!in_quotes &&
            is_white_space(ch)) // not in quotes, white space terminates string
            break;
        *outstr++ = ch;
        count++;
    }
...
copy_text_until(char *outstr, char *instr, char *endstr, int max_chars)
{
    ...
    if (foundstr - instr + strlen(endstr) < (uint)max_chars) {
        ...
        nprintf(("Error",
                 "Error.  Too much text (%i chars, %i allowed) before %s\n",
...
void
stuff_string_line(char *pstr, int len)
{
    char read_str[2048] = "";
    int read_len = 2048;
    int final_len = len;
...
int
strip_comments_fred(char *readp, int in_comment)
{
    int ch;
    char *writep = readp;
...
// Strip comments from a line of input.
int
strip_comments(char *readp, int in_comment)
{
    int ch;
    char *writep = readp;
...
int
parse_get_line(char *lineout, int max_line_len, char *start, int max_size,
               char *cur)
{
    char *t = lineout;
...
read_file_text(char *filename, int mode)
{
    ...
    while ((num_chars_read = parse_get_line(outbuf, BUF_SIZE, Mission_text_raw,
                                            file_len, mp2)) != 0) {
        mp2 += num_chars_read;
...
int
stuff_string_list(char slp[][NAME_LENGTH], int max_strings)
{
    int count = 0;
    ignore_white_space();
    ...
    while (*Mp != ')') {
        ...
        if (*Mp == '"') {
            ...
            if (ok_flag) {
                ...
                if (num <
                    0) // other negatives used to bypass the above error trap, but should be -1
                    num = -1;
...
void
stuff_parenthesized_vector(vector *vp)
{
    ignore_white_space();

    if (*Mp != '(') {
        error_display(
            1, "Reading parenthesized vector.  Found [%c].  Expecting '('.\n",
            *Mp);
...
// NULL is returned.
char *
split_str_once(char *src, int max_pixel_w)
{
    char *brk = NULL;
