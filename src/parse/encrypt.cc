...
// new encrpytion
void encrypt_new(char *text, int text_len, char *scrambled_text,
                 int *scrambled_len);
void unencrypt_new(char *scrambled_text, int scrambled_len, char *text,
                   int *text_len);

// update cur_seed with the chksum of the new_data of size new_data_size
ushort
chksum_add_short(ushort seed, char *buffer, int size)
{
...
void
encrypt(char *text, int text_len, char *scrambled_text, int *scrambled_len,
        int use_8bit)
{
#ifdef ENCRYPT_NEW
    ...
    for (i = 0; i < text_len; i++) {
        ...
        if (use_8bit) {
            ...
            case 2:
                ...
                scrambled_text[byte_offset] |= ((text[i] >> 1) &
                                                0x3F); // put in top 6 bits
                byte_offset++;
                scrambled_text[byte_offset] = (char)((text[i] << 7) &
                                                     0x80); // put in last bit
                bit_offset = 1;
                break;
            case 3:
                scrambled_text[byte_offset] &= 0xe0; // clear out bottom 5 bits
                scrambled_text[byte_offset] |= ((text[i] >> 2) &
                                                0x1F); // put in top 5 bits
                byte_offset++;
            ...
            case 4:
                ...
                scrambled_text[byte_offset] |= ((text[i] >> 3) &
                                                0x0F); // put in top 4 bits
                byte_offset++;
                scrambled_text[byte_offset] =
            ...
            case 5:
                ...
                scrambled_text[byte_offset] |= ((text[i] >> 4) &
                                                0x07); // put in top 3 bits
                byte_offset++;
                scrambled_text[byte_offset] =
            ...
            case 6:
                ...
                scrambled_text[byte_offset] |= ((text[i] >> 5) &
                                                0x03); // put in top 2 bits
                byte_offset++;
                scrambled_text[byte_offset] =
            ...
            case 7:
                ...
                scrambled_text[byte_offset] |= ((text[i] >> 6) &
                                                0x01); // put in top bit
                byte_offset++;
                scrambled_text[byte_offset] =
...
void
unencrypt(char *scrambled_text, int scrambled_len, char *text, int *text_len)
{
#ifdef ENCRYPT_NEW
...
//          scrambled_len  => size of text after getting scrambled
void
encrypt_new(char *text, int text_len, char *scrambled_text, int *scrambled_len)
{
    ...
    *scrambled_len = 0;
    while (*scrambled_len < text_len) {
        ...
        block_checksum = chksum_add_short(block_checksum, (char *)lvl1_block,
                                          block_size);
...
//          text_len       => actual number of bytes of unscrambled data
void
unencrypt_new(char *scrambled_text, int scrambled_len, char *text, int *text_len)
{
    ushort lvl1_block[NUM_LVL1_KEYS * 2];
    ...
    *text_len = 0;
    ...
    while (*text_len < scrambled_len) {
        ...
        if ((scrambled_len - *text_len) < (NUM_LVL1_KEYS * 2)) {
            memcpy(lvl1_block, scrambled_text + *text_len,
        ...
        // the running checksum
        block_checksum = chksum_add_short(block_checksum, (char *)lvl1_block_copy,
                                          block_size);
...
int
is_encrpyted(char *scrambled_text)
{
    uint encrypt_id;
