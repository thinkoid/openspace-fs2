...
int
must_clip_line(vertex *p0, vertex *p1, ubyte codes_or, uint flags)
{
    int ret = 0;
...
int
g3_draw_line(vertex *p0, vertex *p1)
{
    ubyte codes_or;
...
int
g3_check_normal_facing(vector *v, vector *norm)
{
    vector tempv;
...
int
do_facing_check(vector *norm, vertex **vertlist, vector *p)
{
    Assert(G3_count == 1);
    ...
    if (norm) { //have normal
        ...
        vm_vec_perp(&tempv, (vector *)&vertlist[0]->x, (vector *)&vertlist[1]->x,
                    (vector *)&vertlist[2]->x);
...
int
g3_draw_poly_if_facing(int nv, vertex **pointlist, uint tmap_flags, vector *norm,
                       vector *pnt)
{
    Assert(G3_count == 1);
...
int
g3_draw_poly(int nv, vertex **pointlist, uint tmap_flags)
{
    int i;
    vertex **bufptr;
    ccodes cc;
...
//returns 1 if off screen, 0 if drew
int
g3_draw_poly_constant_sw(int nv, vertex **pointlist, uint tmap_flags,
                         float constant_sw)
{
    int i;
    vertex **bufptr;
...
int
g3_draw_sphere(vertex *pnt, float rad)
{
    Assert(G3_count == 1);
...
int
g3_draw_sphere_ez(vector *pnt, float rad)
{
    vertex pt;
    ubyte flags;
...
// Orient
int
g3_draw_bitmap(vertex *pnt, int orient, float rad, uint tmap_flags)
{
    vertex va, vb;
    float t, w, h;
    float width, height;
...
int
g3_get_bitmap_dims(int bitmap, vertex *pnt, float rad, int *x, int *y, int *w,
                   int *h, int *size)
{
    float t;
    float width, height;
...
//returns 1 if off screen, 0 if drew
int
g3_draw_rotated_bitmap(vertex *pnt, float angle, float rad, uint tmap_flags)
{
    vertex v[4];
...
float
g3_get_poly_area(int nv, vertex **pointlist)
{
    int idx;
...
//returns 1 if off screen, 0 if drew
float
g3_draw_poly_constant_sw_area(int nv, vertex **pointlist, uint tmap_flags,
                              float constant_sw, float area)
{
    int i;
    vertex **bufptr;
    ccodes cc;
...
//returns 1 if off screen, 0 if drew
float
g3_draw_rotated_bitmap_area(vertex *pnt, float angle, float rad, uint tmap_flags,
                            float area)
{
    vertex v[4];
    vertex *vertlist[4] = { &v[3], &v[2], &v[1], &v[0] };
...
g3_draw_horizon_line()
{
    ...
    gr_line(fl2i(horz_pts[0].x), fl2i(horz_pts[0].y), fl2i(horz_pts[1].x),
            fl2i(horz_pts[1].y));
}
...
int
g3_draw_rod(vector *p0, float width1, vector *p1, float width2, vertex *verts,
            uint tmap_flags)
{
...
vector g3_square[4] = { { -1.0f, -1.0f, 20.0f },
                        { -1.0f, 1.0f, 20.0f },
                        { 1.0f, 1.0f, 20.0f },
                        { 1.0f, -1.0f, 20.0f } };
...
float p_theta = 10.0f;
int
g3_draw_perspective_bitmap(angles *a, float scale_x, float scale_y, int div_x,
                           int div_y, uint tmap_flags)
{
    ...
    scale_x *= ((float)gr_screen.max_w / (float)gr_screen.max_h) +
               0.55f; // fudge factor
    ...
    for (idx = 0; idx <= div_x; idx++) {
        for (s_idx = 0; s_idx <= div_y; s_idx++) {
            ...
            stars_project_2d_onto_sphere(&s_points[idx][s_idx], 1000.0f,
                                         s_phi + ((float)idx * d_phi),
...
int
g3_draw_2d_poly_bitmap(int x, int y, int w, int h, uint additional_tmap_flags)
{
    int ret;
    int saved_zbuffer_mode;
    vertex v[4];
    ...
    bm_get_section_size(gr_screen.current_bitmap, gr_screen.current_bitmap_sx,
                        gr_screen.current_bitmap_sy, &bw, &bh);
	...
	*/
    ...
    // set debrief
    ret = g3_draw_poly_constant_sw(
        4, vertlist, TMAP_FLAG_TEXTURED | additional_tmap_flags, 0.1f);
