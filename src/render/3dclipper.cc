...
void
clip_line(vertex **p0, vertex **p1, ubyte codes_or, uint flags)
{
    int plane_flag;
    vertex *old_p1;
    ...
    for (plane_flag = 1; plane_flag <= CC_OFF_USER; plane_flag <<= 1)
        if (codes_or & plane_flag) {
            ...
            *p1 = clip_edge(plane_flag, *p0, *p1, flags);
            codes_or = (unsigned char)((*p0)->codes |
                                       (*p1)->codes); //get new codes
...
int
clip_plane(int plane_flag, vertex **src, vertex **dest, int *nv, ccodes *cc,
           uint flags)
{
    int i;
