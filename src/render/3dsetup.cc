...
void
g3_set_view_matrix(vector *view_pos, matrix *view_matrix, float zoom)
{
    Assert(G3_count == 1);
...
void
g3_set_view_angles(vector *view_pos, angles *view_orient, float zoom)
{
    matrix tmp;
...
ubyte
g3_rotate_vertex_popped(vertex *dest, vector *src)
{
    vector tempv;
...
void
g3_start_instance_matrix(vector *pos, matrix *orient)
{
    vector tempv;
    matrix tempm, tempm2;
...
void
g3_start_instance_angles(vector *pos, angles *orient)
{
    matrix tm;
...
*/
void
g3_start_user_clip_plane(vector *plane_point, vector *plane_normal)
{
...
int
g3_point_behind_user_plane(vector *pnt)
{
    if (G3_user_clip) {
        vector tmp;
