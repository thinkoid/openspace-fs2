...
typedef struct popup_info
{
    ...
    int shortcut_index
        [POPUP_MAX_CHOICES]; // what char should be underlines for shortcut
    ...
    char input_text
        [POPUP_INPUT_MAX_CHARS]; // input box text (if this is an inputbox popup)
...
UI_BUTTON Popup_button_regions
    [POPUP_MAX_CHOICES]; // fake buttons used for mouse detection over text
UI_INPUTBOX Popup_input; // input box for the popup
UI_SLIDER2
    Popup_slider; // if we have more text in the popup than can be displayed at once
...
void
popup_play_default_change_sound(popup_info *pi)
{
    if (pi->nchoices > 1) {
        int i, mouse_over = 0;
        UI_BUTTON *br, *b;
...
popup_split_lines(popup_info *pi, int flags)
{
    ...
    nlines = split_str(pi->raw_text, Popup_text_coords[gr_screen.res][2], n_chars,
                       p_str, POPUP_MAX_LINES);
    Assert(nlines >= 0 && nlines <= POPUP_MAX_LINES);
    ...
    for (i = 0; i < pi->nlines; i++) {
        ...
        strncpy(pi->msg_lines[i], p_str[i + body_offset],
...
popup_get_button_filename(popup_info *pi, int i, int flags)
{
    ...
    case 1:
        if ((flags & PF_USE_AFFIRMATIVE_ICON) &&
            !(flags & PF_NO_SPECIAL_BUTTONS)) {
            fname =
                Popup_button_filenames[gr_screen.res][is_tiny][BUTTON_POSITIVE];
        }
        else if (flags & PF_USE_NEGATIVE_ICON &&
                 !(flags & PF_NO_SPECIAL_BUTTONS)) {
            fname =
                Popup_button_filenames[gr_screen.res][is_tiny][BUTTON_NEGATIVE];
        }
        else {
...
int
popup_init(popup_info *pi, int flags)
{
    int i;
    UI_BUTTON *b;
    ...
    // create base window
    Popup_window.create(pbg->coords[0], pbg->coords[1],
    ...
    for (i = 0; i < pi->nchoices; i++) {
        ...
        if ((pi->nchoices == 1) && (flags & PF_USE_AFFIRMATIVE_ICON)) {
            b->create(&Popup_window, "", Button_coords[gr_screen.res][i + 1][0],
                      Button_coords[gr_screen.res][i + 1][1], 30, 25, 0, 1);
        }
        else {
            b->create(&Popup_window, "", Button_coords[gr_screen.res][i][0],
        ...
        if ((pi->nchoices == 1) && (flags & PF_USE_AFFIRMATIVE_ICON)) {
            ...
            b->create(&Popup_window, "", lx, Button_regions[gr_screen.res][i][1],
    ...
    Popup_slider.create(&Popup_window, Popup_slider_coords[gr_screen.res][0],
...
void
popup_close(popup_info *pi, int screen)
{
    int i;
...
void
popup_set_text_color(int flags)
{
...
// set the popup text color
void
popup_set_title_color(int flags)
{
...
void
popup_draw_title(int sy, char *line, int flags)
{
    int w, h, sx;
...
int
popup_calc_starting_index(popup_info *pi)
{
...
int
popup_calc_starting_y(popup_info *pi, int flags)
{
    int sy, total_h = 0;
    ...
    sy = fl2i((Popup_text_coords[gr_screen.res][1] +
...
void
popup_draw_msg_text(popup_info *pi, int flags)
{
    int sx, sy, i, w, h;
    int line_index;
    int line_count;
    ...
    if (i < pi->nlines) {
        ...
        gr_string(Title_coords[gr_screen.res][4],
...
void
popup_draw_button_text(popup_info *pi, int flags)
{
    int w, h, i, sx, sy;
...
void
popup_force_draw_buttons(popup_info *pi)
{
...
int
popup_do_with_condition(popup_info *pi, int flags, int (*condition)())
{
    int screen_id, choice = -1, done = 0;
    int test;
    screen_id = gr_save_screen();
...
int
popup_till_condition(int (*condition)(), ...)
{
    int choice;
    char *format, *s;
    va_list args;
...
char *
popup_input(int flags, char *caption, int max_output_len)
{
    if (Popup_is_active) {
        Int3(); // should never happen
        return NULL;
...
void
popup_change_text(char *new_text)
{
    // copy the raw text
