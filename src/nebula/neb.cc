...
int Neb2_poof_flags = ((1 << 0) | (1 << 1) | (1 << 2) | (1 << 3) | (1 << 4) |
                       (1 << 5));
...
typedef struct neb2_detail
{
    ...
    float break_x,
        break_y; // x and y alpha fade/break values. adjust alpha on the polys as they move offscreen
...
float neb2_get_alpha_2shell(float inner_radius, float outer_radius,
                            float magic_num, vector *v);
...
// set detail level
void
neb2_set_detail_level(int level)
{
    // sanity
    if (level < 0) {
        Nd = &Neb2_detail[0];
...
void
neb2_render_setup(vector *eye_pos, matrix *eye_orient)
{
    // if the mission is not a fullneb mission, skip
    if (!(The_mission.flags & MISSION_FLAG_FULLNEB)) {
        return;
...
int
neb2_skip_render(object *objp, float z_depth)
{
...
// return the alpha the passed poof should be rendered with, for a 2 shell nebula
float
neb2_get_alpha_2shell(float inner_radius, float outer_radius, float magic_num,
                      vector *v)
{
    float dist;
    float alpha;
...
// return an alpha value for a bitmap offscreen based upon "break" value
float
neb2_get_alpha_offscreen(float sx, float sy, float incoming_alpha)
{
    float alpha = 0.0f;
...
void
neb2_copy(int xyz, int src, int dest)
{
    int idx1, idx2;
...
void
neb2_gen_slice(int xyz, int src, vector *cube_center)
{
    int idx1, idx2;
    ...
    case 0:
        for (idx1 = 0; idx1 < Neb2_slices; idx1++) {
            for (idx2 = 0; idx2 < Neb2_slices; idx2++) {
                ...
                Neb2_cubes[src][idx1][idx2].rot_speed = frand_range(-max_rotation,
                                                                    max_rotation);
    ...
    case 1:
        for (idx1 = 0; idx1 < Neb2_slices; idx1++) {
            for (idx2 = 0; idx2 < Neb2_slices; idx2++) {
                ...
                Neb2_cubes[idx1][src][idx2].rot_speed = frand_range(-max_rotation,
                                                                    max_rotation);
                Neb2_cubes[src][idx1][idx2].flash = 0.0f;
            }
        }
    ...
    case 2:
        for (idx1 = 0; idx1 < Neb2_slices; idx1++) {
            for (idx2 = 0; idx2 < Neb2_slices; idx2++) {
                ...
                Neb2_cubes[idx1][idx2][src].rot_speed = frand_range(-max_rotation,
                                                                    max_rotation);
                Neb2_cubes[src][idx1][idx2].flash = 0.0f;
            }
        }
...
float g3_draw_rotated_bitmap_area(vertex *pnt, float angle, float rad,
                                  uint tmap_flags, float area);
int neb_mode = 1;
int frames_total = 0;
int frame_count = 0;
float frame_avg;
    ...
    for (idx1 = 0; idx1 < Neb2_slices; idx1++) {
        for (idx2 = 0; idx2 < Neb2_slices; idx2++) {
            for (idx3 = 0; idx3 < Neb2_slices; idx3++) {
                ...
                // useless
                if (vm_vec_dot_to_point(&eye_orient.fvec, &eye_pos,
                ...
                if ((ptemp.sx < 0.0f) || (ptemp.sx > (float)gr_screen.max_w) ||
                    (ptemp.sy < 0.0f) || (ptemp.sy > (float)gr_screen.max_h)) {
...
// get near and far fog values based upon object type and rendering mode
void
neb2_get_fog_values(float *fnear, float *ffar, object *objp)
{
    int fog_index;
...
float
neb2_get_fog_intensity(object *obj)
{
...
int tbmap = -1;
void
neb2_pre_render(vector *eye_pos, matrix *eye_orient)
{
...
void
neb2_get_pixel(int x, int y, int *r, int *g, int *b)
{
    int ra, ga, ba;
    ubyte rv, gv, bv;
    int avg_count;
    ...
    avg_count++;
    if (xs > 0) {
        bm_get_components(&tpixels[PIXEL_INDEX_SMALL(xs - 1, ys)], &rv, &gv, &bv,
                          NULL); // left
        ra += rv;
        ba += bv;
        ga += gv;
    ...
    if (ys > 0) {
        bm_get_components(&tpixels[PIXEL_INDEX_SMALL(xs, ys - 1)], &rv, &gv, &bv,
                          NULL); // top
        ra += rv;
        ba += bv;
        ga += gv;
    ...
    if ((xs > 0) && (ys > 0)) {
        bm_get_components(&tpixels[PIXEL_INDEX_SMALL(xs - 1, ys - 1)], &rv, &gv,
                          &bv, NULL); // upper left
        ra += rv;
        ba += bv;
        ga += gv;
...
void
neb2_get_backg_color(int *r, int *g, int *b)
{
    *r = Neb2_background_color[0];
    *g = Neb2_background_color[1];
    *b = Neb2_background_color[2];
...
void
neb2_set_backg_color(int r, int g, int b)
{
...
// fill in the eye orient for this frame
void
neb2_get_eye_orient(matrix *eye)
{
    *eye = Eye_matrix;
}
...
{
    dc_printf(
        "neb2_fog <X> <float> <float>  : set near and far fog planes for ship type X\n");
    ...
    dc_printf(
        "neb2_fog_vals    : display all the current settings for all above values\n");
}
...
DCF(neb2_fog, "")
{
    ...
    if ((index >= 1) && (index <= 11) && (fnear >= 0.0f) && (ffar >= 0.0f) &&
        (ffar > fnear)) {
        if (index == 11) {
...
{
    dc_printf("neb2_fog : \n");
    dc_printf("(1)cargo containers : %f, %f\n", Neb_ship_fog_vals[1][0],
              Neb_ship_fog_vals[1][1]);
    dc_printf("(2)fighters/bombers : %f, %f\n", Neb_ship_fog_vals[2][0],
              Neb_ship_fog_vals[2][1]);
    dc_printf("(3)cruisers : %f, %f\n", Neb_ship_fog_vals[3][0],
              Neb_ship_fog_vals[3][1]);
    dc_printf("(4)freighters : %f, %f\n", Neb_ship_fog_vals[4][0],
              Neb_ship_fog_vals[4][1]);
    dc_printf("(5)cap ships : %f, %f\n", Neb_ship_fog_vals[5][0],
              Neb_ship_fog_vals[5][1]);
    dc_printf("(6)transports : %f, %f\n", Neb_ship_fog_vals[6][0],
              Neb_ship_fog_vals[6][1]);
    dc_printf("(7)support ships : %f, %f\n", Neb_ship_fog_vals[7][0],
              Neb_ship_fog_vals[7][1]);
    dc_printf("(8)navbuoys : %f, %f\n", Neb_ship_fog_vals[8][0],
              Neb_ship_fog_vals[8][1]);
    dc_printf("(9)sentry guns : %f, %f\n", Neb_ship_fog_vals[9][0],
              Neb_ship_fog_vals[9][1]);
    dc_printf("(10)escape pods : %f, %f\n", Neb_ship_fog_vals[10][0],
              Neb_ship_fog_vals[10][1]);
    ...
    dc_printf("neb2_background\t : %d %d %d\n", Neb2_background_color[0],
              Neb2_background_color[1], Neb2_background_color[2]);
}
