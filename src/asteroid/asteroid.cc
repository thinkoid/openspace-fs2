...
debris_struct Field_debris_info[] = { -1,
                                      "None",
                                      ASTEROID_TYPE_SMALL,
                                      "Asteroid Small",
                                      ASTEROID_TYPE_MEDIUM,
                                      "Asteroid Medium",
                                      ASTEROID_TYPE_BIG,
                                      "Asteroid Large",
                                      ...
                                      DEBRIS_TERRAN_SMALL,
                                      "Terran Small",
                                      DEBRIS_TERRAN_MEDIUM,
                                      "Terran Medium",
                                      DEBRIS_TERRAN_LARGE,
                                      "Terran Large",

                                      DEBRIS_VASUDAN_SMALL,
                                      "Vasudan Small",
                                      DEBRIS_VASUDAN_MEDIUM,
                                      "Vasudan Medium",
                                      DEBRIS_VASUDAN_LARGE,
                                      "Vasudan Large",

                                      DEBRIS_SHIVAN_SMALL,
                                      "Shivan Small",
                                      DEBRIS_SHIVAN_MEDIUM,
                                      "Shivan Medium",
                                      DEBRIS_SHIVAN_LARGE,
                                      "Shivan Large" };
...
void
asteroid_update_collide(object *objp)
{
...
int
asteroid_obj_list_add(int objnum)
{
    int index;
    ...
    // retail spelled this !flags & FLAG; exact by accident (USED is bit 0
    // and the only flag)
...
void
asteroid_obj_list_remove(object *obj)
{
...
float
asteroid_cap_speed(int asteroid_info_index, float speed)
{
...
int
asteroid_in_inner_bound_with_axes(asteroid_field *asfieldp, vector *pos,
                                  float delta)
{
    Assert(asfieldp->has_inner_bound);

    int rval = 0;
    if ((pos->x > asfieldp->inner_min_bound.x - delta) &&
        (pos->x < asfieldp->inner_max_bound.x + delta)) {
        rval += 1;
    }

    if ((pos->y > asfieldp->inner_min_bound.y - delta) &&
        (pos->y < asfieldp->inner_max_bound.y + delta)) {
        rval += 2;
    }

    if ((pos->z > asfieldp->inner_min_bound.z - delta) &&
        (pos->z < asfieldp->inner_max_bound.z + delta)) {
        rval += 4;
    }
...
int
asteroid_in_inner_bound(asteroid_field *asfieldp, vector *pos, float delta)
{
...
void
inner_bound_pos_fixup(asteroid_field *asfieldp, vector *pos)
{
...
object *
asteroid_create(asteroid_field *asfieldp, int asteroid_type, int asteroid_subtype)
{
    int n, objnum;
    matrix orient;
    object *objp;
    ...
    objnum = obj_create(OBJ_ASTEROID, -1, n, &orient, &pos, radius,
                        OF_RENDERS | OF_PHYSICS | OF_COLLIDES);
    ...
    if (Game_mode & GM_NORMAL) {
        speed = asteroid_cap_speed(
            asteroid_type,
            asfieldp->speed *
                frand_range(0.5f + (float)Game_skill_level / NUM_SKILL_LEVELS,
                            2.0f + (float)(2 * Game_skill_level) /
                                       NUM_SKILL_LEVELS));
    }
    ...
    objp->phys_info.I_body_inv.rvec.x = 1.0f /
                                        (objp->phys_info.mass *
                                         asip->modelp[asteroid_subtype]->rad);
    ...
    objp->hull_strength = asip->initial_hull_strength *
...
asteroid_sub_create(object *parent_objp, int asteroid_type, vector *relvec)
{
    ...
    vm_vec_scale_add(&new_objp->pos, &parent_objp->pos, relvec,
                     0.5f * parent_objp->radius);
    ...
    if (Game_mode & GM_NORMAL)
        speed = asteroid_cap_speed(asteroid_type,
                                   (frand() + 2.0f) * parent_speed);
    ...
    vm_vec_scale_add(&new_objp->last_pos, &new_objp->pos,
                     &new_objp->phys_info.vel, -flFrametime);
...
void
asteroid_load(int asteroid_info_index, int asteroid_subtype)
{
    asteroid_info *asip;
    //   int            pof_index;
    ...
    asip->model_num[asteroid_subtype] = model_load(
        asip->pof_files[asteroid_subtype], 0, NULL);

    if (asip->model_num[asteroid_subtype] > -1) {
        asip->modelp[asteroid_subtype] = model_get(
            asip->model_num[asteroid_subtype]);

        // Stuff detail level distances.
        for (int i = 0; i < asip->num_detail_levels; i++) {
            asip->modelp[asteroid_subtype]->detail_depth[i] = i2fl(
                asip->detail_distance[i]);
        }
    }
}
...
int
get_debris_from_same_group(int index)
{
...
int
get_debris_weight(int ship_debris_index)
{
...
asteroid_create_all()
{
    ...
    int max_asteroids =
        Asteroid_field
            .num_initial_asteroids; // * (1.0f - 0.1f*(MAX_DETAIL_LEVEL-Detail.asteroid_density)));
    ...
    if (Asteroid_field.debris_genre == DG_SHIP) {
        ...
        for (idx = 0; idx < num_debris_types; idx++) {
            int debris_weight = get_debris_weight(
        ...
        if (Asteroid_field.debris_genre == DG_ASTEROID) {
            ...
            for (idx = 0; idx < 3; idx++) {
                ...
                if (rand_choice < ship_debris_odds_table[idx][0]) {
                    asteroid_create(&Asteroid_field,
                                    ship_debris_odds_table[idx][1], 0);
                    break;
                }
            }
        }
...
int
asteroid_should_wrap(object *objp, asteroid_field *asfieldp)
{
    ...
    if (asfieldp->has_inner_bound) {
        if ((objp->pos.x > asfieldp->inner_min_bound.x) &&
            (objp->pos.x < asfieldp->inner_max_bound.x) &&
            (objp->pos.y > asfieldp->inner_min_bound.y) &&
            (objp->pos.y < asfieldp->inner_max_bound.y) &&
            (objp->pos.z > asfieldp->inner_min_bound.z) &&
            (objp->pos.z < asfieldp->inner_max_bound.z)) {
            return 1;
        }
    }
...
void
asteroid_wrap_pos(object *objp, asteroid_field *asfieldp)
{
    if (objp->pos.x < asfieldp->min_bound.x) {
        objp->pos.x = asfieldp->max_bound.x +
                      (objp->pos.x - asfieldp->min_bound.x);
    }

    if (objp->pos.y < asfieldp->min_bound.y) {
        objp->pos.y = asfieldp->max_bound.y +
                      (objp->pos.y - asfieldp->min_bound.y);
    }

    if (objp->pos.z < asfieldp->min_bound.z) {
        objp->pos.z = asfieldp->max_bound.z +
                      (objp->pos.z - asfieldp->min_bound.z);
    }
...
int
asteroid_is_targeted(object *objp)
{
    ship_obj *so;
    object *ship_objp;
...
void
asteroid_aim_at_target(object *objp, object *asteroid_objp, float delta_time)
{
    vector predicted_center_pos;
    vector rand_vec;
    float speed;

    vm_vec_scale_add(&predicted_center_pos, &objp->pos, &objp->phys_info.vel,
                     delta_time);
    ...
    vm_vec_scale_add(&asteroid_objp->pos, &predicted_center_pos,
                     &asteroid_objp->phys_info.vel, -delta_time);
    vm_vec_scale_add(&asteroid_objp->last_pos, &asteroid_objp->pos,
...
void
maybe_throw_asteroid(int count)
{
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if (so->objnum == Asteroid_throw_objnum) {
            ...
            object *objp = asteroid_create(&Asteroid_field, ASTEROID_TYPE_BIG,
                                           subtype);
            if (objp != NULL) {
...
asteroid_maybe_reposition(object *objp, asteroid_field *asfieldp)
{
    ...
    if (asteroid_should_wrap(objp, asfieldp)) {
        ...
        if (!asteroid_is_targeted(objp)) {
            ...
            dist = vm_vec_normalized_dir(&vec_to_asteroid, &objp->pos,
                                         &Eye_position);
            ...
            if ((dot < 0.7f) || (dist > 3000.0f)) {
                if (Num_asteroids > MAX_ASTEROIDS - 10) {
                    ...
                    vm_vec_normalized_dir(&vec_to_asteroid, &objp->pos,
                                          &Eye_position);
                    dot = vm_vec_dot(&Eye_matrix.fvec, &vec_to_asteroid);
                    ...
                    vm_vec_scale_add(&objp->last_pos, &objp->pos,
                                     &objp->phys_info.vel, -flFrametime);
...
void
lerp(float *goal, float f1, float f2, float scale)
{
    *goal = (f2 - f1) * scale + f1;
}

void
asteroid_process_pre(object *objp, float frame_time)
{
    if (Asteroids_enabled) {
        vector *v, *vv;
        ...
        lerp(&objp->phys_info.vel.x, objp->phys_info.vel.x,
             objp->phys_info.desired_vel.x, flFrametime);
        lerp(&objp->phys_info.vel.y, objp->phys_info.vel.y,
             objp->phys_info.desired_vel.y, flFrametime);
        lerp(&objp->phys_info.vel.z, objp->phys_info.vel.z,
             objp->phys_info.desired_vel.z, flFrametime);
    }
}
                         ...
                         collision_info_struct *asteroid_hit_info)
{
    ...
    if (asteroid_hit_info == NULL) {
        ...
        mc.model_num =
            Asteroid_info[Asteroids[num].type]
    ...
    if ((vm_vec_mag_squared(&heavy->phys_info.rotvel) * flFrametime *
        ...
        if (vm_vec_mag(&asteroid_hit_info->light_rel_vel) > 300) {
            ...
            vm_vec_sub(&asteroid_hit_info->light_rel_vel, &light->phys_info.vel,
                       &heavy->phys_info.vel);
        }
    }
    else {
    ...
    if (asteroid_hit_info->heavy ==
        ...
        mc.model_num =
            Ships[ship_obj->instance].modelnum; // Fill in the model to check
        ...
        if (model_collide(&mc)) {
            ...
            if (asteroid_hit_info->collide_rotate) {
                ...
                model_get_rotating_submodel_list(
                    submodel_list, &num_rotating_submodels, heavy_obj);
                ...
                for (int i = 0; i < num_rotating_submodels; i++) {
                    ...
                    world_find_model_point(&p0, &light_obj->last_pos, pm,
                                           submodel_list[i],
                                           &heavy_obj->last_orient,
                                           &heavy_obj->last_pos);

                    pm->submodel[submodel_list[i]].angs = copy_angles;
                    world_find_model_point(&p1, &light_obj->pos, pm,
                                           submodel_list[i], &heavy_obj->orient,
                                           &heavy_obj->pos);
                    ...
                    if (model_collide(&mc)) {
                        if (mc.hit_dist < asteroid_hit_info->hit_time) {
                            ...
                            set_hit_struct_info(asteroid_hit_info, &mc,
                                                SUBMODEL_ROT_HIT);
            ...
            *mc.p1 = copy_p1;
            ...
            if (model_collide(&mc)) {
                ...
                if (mc.hit_dist < asteroid_hit_info->hit_time) {
                    ...
                    if (asteroid_hit_info->edge_hit == 0) {
                        model_find_obj_dir(&asteroid_hit_info->collision_normal,
                    ...
                    vector diff;
                    vm_vec_sub(&diff, mc.p1, mc.p0);
                    vm_vec_scale_add(&asteroid_hit_info->light_collision_cm_pos,
        ...
        if (mc_ret_val) {
            ...
            if (!asteroid_hit_info->edge_hit) {
                vm_vec_unrotate(&asteroid_hit_info->collision_normal,
                                &mc.hit_normal, &heavy->orient);
            }

            // find position in submodel RF of light object at collison
            vector diff;
            vm_vec_sub(&diff, mc.p1, mc.p0);
            vm_vec_scale_add(&asteroid_hit_info->light_collision_cm_pos, mc.p0,
                             &diff, mc.hit_dist);
        }
    }
    ...
    if (mc_ret_val) {
        ...
        vm_vec_sub(&asteroid_hit_info->r_light, &asteroid_hit_info->hit_pos,
                   &asteroid_hit_info->light_collision_cm_pos);

        // set normal for edge hit
        if (asteroid_hit_info->edge_hit) {
            vm_vec_copy_normalize(&asteroid_hit_info->collision_normal,
                                  &asteroid_hit_info->r_light);
            vm_vec_negate(&asteroid_hit_info->collision_normal);
        }

        // get world hitpos
        vm_vec_add(hitpos, &asteroid_hit_info->heavy->pos,
...
asteroid_render(object *obj)
{
    if (Asteroids_enabled) {
        ...
        model_clear_instance(
            Asteroid_info[asp->type].model_num[asp->asteroid_subtype]);
        model_render(
            Asteroid_info[asp->type].model_num[asp->asteroid_subtype],
            &obj->orient, &obj->pos, MR_NORMAL | MR_IS_ASTEROID,
            OBJ_INDEX(obj)); //  Replace MR_NORMAL with 0x07 for big yellow blobs
    }
}

// Create a normalized vector generally in the direction from *hitpos to other_obj->pos
void
asc_get_relvec(vector *relvec, object *other_obj, vector *hitpos)
{
...
float
asteroid_get_fireball_scale_multiplier(int num)
{
...
float
asteroid_create_explosion(object *objp)
{
    int fireball_objnum;
    ...
    fireball_objnum = fireball_create(&objp->pos, FIREBALL_ASTEROID,
                                      OBJ_INDEX(objp),
                                      objp->radius * fireball_scale_multiplier, 0,
                                      &objp->phys_info.vel);
...
void
asteriod_explode_sound(object *objp, int type, int play_loud)
{
    int sound_index = -1;
    float range_factor =
    ...
    snd_play_3d(&Snds[sound_index], &objp->pos, &Eye_position, objp->radius, NULL,
                0, 1.0f, SND_PRIORITY_MUST_PLAY, NULL, range_factor);
}
...
void
asteroid_do_area_effect(object *asteroid_objp)
{
    object *ship_objp;
    float damage, blast;
    ship_obj *so;
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if (weapon_area_calc_damage(ship_objp, &asteroid_objp->pos,
                                    asip->inner_rad, asip->outer_rad, asip->blast,
                                    asip->damage, &blast, &damage,
                                    asip->outer_rad) == -1)
            continue;

        ship_apply_global_damage(ship_objp, asteroid_objp, &asteroid_objp->pos,
                                 damage);
...
void
asteroid_hit(object *asteroid_obj, object *other_obj, vector *hitpos,
             float damage)
{
    ...
    if (asteroid_obj->hull_strength < 0.0f) {
        ...
        if (other_obj->type == OBJ_WEAPON) {
            ...
            if (wip->impact_weapon_expl_index < 0) {
                particle_create(hitpos, &vmd_zero_vector, 0.0f,
                                Asteroid_impact_explosion_radius, PARTICLE_BITMAP,
...
// to play for a bit.
void
asteroid_maybe_break_up(object *asteroid_obj)
{
    asteroid *asp;
...
int
asteroid_get_random_in_cone(vector *pos, vector *dir, float ang, int danger)
{
    int idx;
    vector asteroid_dir;
...
void
asteroid_test_collide(object *asteroid_obj, object *ship_obj, mc_info *mc)
{
    float asteroid_ray_dist;
    vector asteroid_fvec, terminus;

    // See if ray from asteroid intersects bounding box of escort ship
    asteroid_ray_dist = vm_vec_mag_quick(&asteroid_obj->phys_info.desired_vel) *
    ...
    if (IS_VEC_NULL(&asteroid_fvec)) {
        ...
        vm_vec_scale_add(&terminus, &asteroid_obj->pos, &asteroid_fvec,
                         asteroid_ray_dist);
    }
...
// seconds
int
asteroid_will_collide(object *asteroid_obj, object *escort_objp)
{
    mc_info mc;
...
int
asteroid_valid_ship_to_warn_collide(ship *shipp)
{
...
void
asteroid_update_collide_flag(object *asteroid_objp)
{
    ...
    for (i = 0; i < num_escorts; i++) {
        ...
        if (escort_objnum >= 0) {
            ...
            if (asteroid_valid_ship_to_warn_collide(escort_shipp)) {
                will_collide = asteroid_will_collide(asteroid_objp,
                                                     &Objects[escort_objnum]);
                if (will_collide) {
...
void
asteroid_verify_collide_objnum(asteroid *asp)
{
...
void
asteroid_process_post(object *obj, float frame_time)
{
    if (Asteroids_enabled) {
        int num;
        num = obj->instance;
...
asteroid_parse_section()
{
    ...
    if ((strstr(asip->name, "Asteroid") != NULL) ||
        (strstr(asip->name, "asteroid") != NULL)) {
    ...
    asip->num_detail_levels = stuff_int_list(
        asip->detail_distance, MAX_SHIP_DETAIL_LEVELS, RAW_INTEGER_TYPE);
...
asteroid_parse_tbl()
{
    ...
    if (stricmp(impact_ani_file, NOX("none"))) {
        int num_frames;
        Asteroid_impact_explosion_ani = bm_load_animation(impact_ani_file,
                                                          &num_frames, NULL, 1);
    }
            ...
            if (ship_objp->pos.x + radius > Asteroid_field.min_bound.x)
                if (ship_objp->pos.y + radius > Asteroid_field.min_bound.y)
                    if (ship_objp->pos.z + radius > Asteroid_field.min_bound.z)
                        if (ship_objp->pos.x - radius <
                            Asteroid_field.max_bound.x)
                            if (ship_objp->pos.y - radius <
                                Asteroid_field.max_bound.y)
                                if (ship_objp->pos.z - radius <
                                    Asteroid_field.max_bound.z)
                                    if (!asteroid_in_inner_bound(&Asteroid_field,
                                                                 &ship_objp->pos,
                                                                 radius))
                                        return so->objnum;
        }
    ...
    for (asteroid_objp = GET_FIRST(&obj_used_list);
        ...
        if (hud_gauge_active(HUD_OFFSCREEN_INDICATOR)) {
            if (asteroid_vertex.codes != 0) {
                ...
                gr_set_color_fast(&IFF_colors[IFF_COLOR_SELECTION][1]);
                hud_draw_offscreen_indicator(&asteroid_vertex,
                                             &asteroid_objp->pos, dist);
            }
        }
    }
}
