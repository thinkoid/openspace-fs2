...
aaline_init_tables()
{
    ...
    for (i = 0; i < 512; i++) {
        ...
        for (j = 0; j < F_TABLE_SIZE; j++) {
            new_table[i * F_TABLE_SIZE + j] =
                (ubyte)(((corr_slope * filter_table[j]) & 0xf00000) >> (16 + 4));
...
long int
fix_xy_mult(long int oa, fix_xy ob)
{
    int retval;
...
void
draw_aa_hspan8(fix_xy x, fix_xy y, long int ep_corr, long int slope)
{
    ...
    if (xi < gr_screen.clip_left)
        return;
    if (xi > gr_screen.clip_right)
        return;

    int clipped = 0;

    if (yi < gr_screen.clip_top)
        clipped++;
    if (yi + 3 > gr_screen.clip_bottom)
        clipped++;
...
void
draw_aa_vspan8(fix_xy x, fix_xy y, long int ep_corr, long int slope)
{
#ifndef HARDWARE_ONLY
    long int sample_dist; // Distance from line to sample point
    long int filter_index; // Index into filter table
		...
		*/
        ...
        if (line->negative) {
            ...
            case 3:
                ...
                break;
            case 4:
                ep_corr = ((ef - sf) + 0x80) | 4;
                break;
            case 5:
                ep_corr = ((0x78 - sf) + 0x80) | 4;
                break;
            case 6:
                ep_corr = ef | 4;
                break;
...
void
aaline_setup(aa_vertex *v1, aa_vertex *v2)
{
...
void
gr8_aaline(vertex *v1, vertex *v2)
{
    aa_vertex aa1, aa2;
