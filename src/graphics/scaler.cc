...
typedef void (*scaler_span_fn)(ubyte *sbits, ubyte *dbits, ubyte *lookup,
                               uint *zbuf, uint gz, int w, fix u, fix du);
...
static void
scaler_span8(ubyte *sbits, ubyte *dbits, ubyte *lookup, uint *zbuf, uint gz,
             int w, fix u, fix du)
{
    int x;
...
static void
scaler_span8_stippled(ubyte *sbits, ubyte *dbits, ubyte *lookup, uint *zbuf,
                      uint gz, int w, fix u, fix du)
{
    int x;
...
static void
scaler_span8_alpha(ubyte *sbits, ubyte *dbits, ubyte *lookup, uint *zbuf, uint gz,
                   int w, fix u, fix du)
{
    int x;
    ...
    for (x = 0; x < w; x++) {
        ...
        dbits[x] =
            lookup[eax + dbits[x]]; // mov bl, [ecx+eax+bg] / mov [edi+x], bl
        u += du;
    }
}
...
static void
scaler_span8_alpha_zbuffered(ubyte *sbits, ubyte *dbits, ubyte *lookup,
                             uint *zbuf, uint gz, int w, fix u, fix du)
{
    int x;
...
void
gr8_scaler(vertex *va, vertex *vb)
{
#if 1
    if (Pofview_running) {
        return;
...
// Scales current bitmap, between va and vb
void
gr8_aascaler(vertex *va, vertex *vb)
{
    float x0, y0, x1, y1;
    float u0, v0, u1, v1;
    float clipped_x0, clipped_y0, clipped_x1, clipped_y1;
    ...
    for (y = dy0; y <= dy1; y++) {
        ...
        if (Gr_scaler_zbuffering && gr_zbuffering) {
            ...
            uint *zbuf =
                &gr_zbuffer[dbits - (ubyte *)gr_screen.offscreen_buffer_base];
