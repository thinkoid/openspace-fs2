...
void
calc_alphacolor_hud_type(alphacolor *ac)
{
...
void
calc_alphacolor_blend_type(alphacolor *ac)
{
#ifndef HARDWARE_ONLY
    int i, j;
    int tr, tg, tb, Sr, Sg, Sb;
    ...
    for (j = 1; j < 16; j++) {
        ...
        // PC a gamma of 1.2 makes text look good, but his gamma is
        // really 1.8.   1.8/1.2 = 1.5
        gamma_j1[j] = (int)((pow(i2fl(j) / 15.0f, 1.5f / Gr_gamma) * 16.0f) +
                            0.5);
    }
...
void
grx_init_alphacolor(color *clr, int r, int g, int b, int alpha, int type)
{
    int n;
    alphacolor *ac;

    if (!Alphacolors_intited)
        return;
    ...
    if ((clr->magic == 0xAC01) && (clr->is_alphacolor)) {
        if ((clr->alphacolor >= 0) && (clr->alphacolor < MAX_ALPHACOLORS)) {
            if (Alphacolors[clr->alphacolor].used &&
                (Alphacolors[clr->alphacolor].clr == clr)) {
                n = clr->alphacolor;
                alphacolor *pac = &Alphacolors[n];
                if (pac->r == r && pac->g == g && pac->b == b &&
                    pac->alpha == alpha && pac->type == type &&
    ...
    if (n == -1) {
        ...
        for (int k = 0; k < MAX_ALPHACOLORS; k++) {
            ...
            if (kac->used && kac->r == r && kac->g == g && kac->b == b &&
                kac->alpha == alpha && kac->type == type &&
...
void
grx_get_color(int *r, int *g, int *b)
{
    if (r)
...
void
grx_init_color(color *dst, int r, int g, int b)
{
...
grx_set_color_fast(color *dst)
{
    ...
    if (dst->screen_sig != gr_screen.signature) {
        if (dst->is_alphacolor) {
            grx_init_alphacolor(dst, dst->red, dst->green, dst->blue, dst->alpha,
                                dst->ac_type);
        }
        else {
...
void
grx_set_color(int r, int g, int b)
{
    Assert((r >= 0) && (r < 256));
    Assert((g >= 0) && (g < 256));
    Assert((b >= 0) && (b < 256));
