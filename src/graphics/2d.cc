...
void
gr_set_palette_internal(char *name, ubyte *palette, int restrict_font_to_128)
{
    if (palette == NULL) {
...
void
gr_set_palette(char *name, ubyte *palette, int restrict_font_to_128)
{
    char *p;
    palette_flush();
...
// Returns cpu type.
void
gr_detect_cpu(int *cpu, int *mmx, int *amd3d, int *katmai)
{
    uint RegEDX;
    uint RegEAX;
...
gr_init(int res, int mode, int depth, int fred_x, int fred_y)
{
    ...
    if (Web_cursor_bitmap < 0) {
        ...
        Assert(
            Web_cursor_bitmap >=
            0); // if bitmap didnt load, thats not good (this is protected for in release tho)
    }
...
void
gr_set_cursor_bitmap(int n, int lock)
{
    static int locked = 0;
    Assert(n >= 0);
...
void
gr_bitmap_ex(int x, int y, int w, int h, int sx, int sy)
{
    if (gr_screen.mode == GR_OPENGL) {
...
void
gr_pline_helper(vector *out, vector *in1, vector *in2, int thickness)
{
    vector slope;
...
void
gr_pline_special(vector **pts, int num_pts, int thickness)
{
    ...
    for (idx = 0; idx < num_pts - 1; idx++) {
        ...
        // draw the polys
        g3_draw_poly_constant_sw(4, verts, TMAP_FLAG_GOURAUD | TMAP_FLAG_RGB,
                                 0.1f);
        ...
        if (idx > 0) {
            ...
            g3_draw_poly_constant_sw(3, verts, TMAP_FLAG_GOURAUD | TMAP_FLAG_RGB,
                                     0.1f);
        }
