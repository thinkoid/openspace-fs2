...
char *
gr_force_fit_string(char *str, int max_str, int max_width)
{
    int w;
...
int
get_char_width(ubyte c1, ubyte c2, int *width, int *spacing)
{
    int i, letter;
    ...
    if (letter < 0 ||
        letter >= Current_font->num_chars) { //not in font, draw as space
        *width = 0;
    ...
    *spacing = Current_font->char_data[letter].spacing;
    ...
    if (i > -1) {
        if (!(c2 == 0 || c2 == '\n')) {
            ...
            if ((letter2 >= 0) &&
                (letter2 <
                 Current_font->num_chars)) { //not in font, draw as space
                font_kernpair *k = &Current_font->kern_data[i];
                while ((k->c1 == letter) && (k->c2 < letter2) &&
                       (i < Current_font->num_kern_pairs)) {
                    i++;
                    k++;
                }
                if (k->c2 == letter2) {
...
void
gr_char_centered(int x, int y, char chr)
{
    char str[2];
    int w, sc;
...
gr8_string(int sx, int sy, char *s)
{
    ...
    while (*s) {
        ...
        ubyte *fp = Current_font->pixel_data +
                    Current_font->char_data[letter].offset + xd + yd * width;
...
gr_create_font(char *typeface)
{
    ...
    fnt->bitmap_id = bm_create(8, fnt->bm_w, fnt->bm_h, fnt->bm_data,
                               BMP_AABITMAP);
