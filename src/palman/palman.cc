...
int
palman_is_nondarkening(int r, int g, int b)
{
    int i;

    for (i = 0; i < Palman_num_nondarkening; i++) {
        if ((r == Palman_non_darkening[i][0]) &&
            (g == Palman_non_darkening[i][1]) &&
...
palman_load_pixels()
{
    ...
    while (!optional_string("#END")) {
        ...
        if (required_string("+ND")) {
            stuff_byte(
                &Palman_non_darkening_default[Palman_num_nondarkening_default][0]);
            stuff_byte(
                &Palman_non_darkening_default[Palman_num_nondarkening_default][1]);
            stuff_byte(
                &Palman_non_darkening_default[Palman_num_nondarkening_default++]
                                             [2]);
        }
    }
...
void
palman_set_nondarkening(ubyte colors[MAX_NONDARK_COLORS][3], int size)
{
...
int
palette_cache_find(int r, int g, int b)
{
...
void
palette_cache_add(int r, int g, int b, int index)
{
    int value = ((r / 4) << 12) + ((g / 4) << 6) + (b / 4);
...
void
palette_load_table(char *filename)
{
    int i;
    int w, h;
    int pcx_error;
    ...
    if (pcx_error != PCX_ERROR_NONE) {
        ...
        if (fp == NULL)
            Error(LOCATION, "Can't open palette file <%s>",
...
uint
palette_find(int r, int g, int b)
{
    int i, j;
...
void
palette_write_cached1(char *name)
{
    CFILE *fp;
    char new_name[128];
    ...
    if (palette_blend_table_calculated) {
        ...
        cfwrite_compressed(&palette_blend_table, 256 * 256, NUM_BLEND_TABLES,
                           fp); //64KB*
    }
    else {
        cfwrite_int(0, fp);
...
palette_read_cached(char *name)
{
    ...
    if ((num_blend_tables_saved == NUM_BLEND_TABLES) &&
        (num_blend_tables_saved > 0)) {
        palette_blend_table_calculated = 1;
        cfread_compressed(&palette_blend_table, 256 * 256, NUM_BLEND_TABLES,
                          fp); //64KB*
    }
    else {
...
void
palman_create_blend_table(float factor, ubyte *table)
{
    int i;
        ...
        for (i = 0; i < 256; i++) {
            ...
            if (palman_is_nondarkening(r, g, b)) {
                ...
                for (l = 0; l < 32; l++) {
                    ...
                    gr_fade_table[((l + 1) * 256) + i] = (unsigned char)
                        palette_find(ur, ug, ub);
                }
            }
...
ubyte *
palette_get_blend_table(float alpha)
{
    int i;
...
uint
palette_compute_checksum(ubyte *pal)
{
    int i;
    uint sum1, sum2;
