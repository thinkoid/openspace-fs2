...
//
const char *
openal_error_string(int get_alc)
{
    int i;
...
int
ds_convert_volume(float volume)
{
    int index;
...
float
ds_get_percentage_vol(int ds_vol)
{
    double vol;
              ...
              WAVEFORMATEX **header)
{
    ...
    while (1) {
        ...
        case 0x20746d66: // The 'fmt ' tag
            ...
            if ((*header = (WAVEFORMATEX *)malloc(sizeof(WAVEFORMATEX) +
                                                  cbExtra)) != NULL) {
                ...
                if (cbExtra != 0) {
                    cfread(((ubyte *)(*header) + sizeof(WAVEFORMATEX)), cbExtra,
                           1, fp);
                }
            }
            else {
...
//
int
ds_load_buffer(int *sid, int *hid, int *final_size, void *header, sound_info *si,
               int flags)
{
    ...
    *sid = ds_get_sid();
    ...
    case WAVE_FORMAT_ADPCM:
        ...
        rc = ACM_convert_ADPCM_to_PCM(pwfx, si->data, si->size, &convert_buffer,
                                      0, &convert_len, &src_bytes_used, 16);
        if (rc == -1) {
            return -1;
        }
...
ds_init_channels()
{
    ...
    if (i != MAX_CHANNELS) {
        nprintf(("Warning",
                 "OpenAL: Restricting MAX_CHANNELS to %i (default: %i)\n", i,
                 MAX_CHANNELS));
        MAX_CHANNELS = i;
    }
    ...
    if (Channels == NULL) {
        Error(LOCATION, "Unable to allocate %d bytes for %d audio channels.",
              (int)(sizeof(channel) * MAX_CHANNELS), MAX_CHANNELS);
    }
...
void
ds_get_primary_format(WAVEFORMATEX *wfx)
{
...
//               0           => init success
int
ds_init(int use_a3d, int use_eax)
{
    ...
    OpenAL_C_ErrorCheck(alcMakeContextCurrent(ds_sound_context),
                        goto AL_InitError);
...
//
void
ds_unload_buffer(int sid, int hid)
{
    if (sid != -1) {
...
//
int
ds_get_free_channel(int new_volume, int snd_id, int priority)
{
    ...
    for (i = 0; i < MAX_CHANNELS; i++) {
        ...
        OpenAL_ErrorCheck(alGetSourcei(chp->source_id, AL_SOURCE_STATE, &status),
                          continue);
    ...
    if (instance_count >= limit) {
        ...
        if (lowest_instance_vol_index >= 0 &&
            (Channels[lowest_instance_vol_index].vol <= new_volume)) {
    ...
    if ((first_free_channel >= 0) &&
        ...
        OpenAL_ErrorCheck(
            alGenSources(1, &Channels[first_free_channel].source_id), return -1);
...
int
ds_create_buffer(int frequency, int bits_per_sample, int nchannels, int nseconds)
{
    ALuint i;
    int sid;
    ...
    sound_buffers[sid].nbytes = nseconds * (bits_per_sample / 8) * nchannels *
                                frequency;
...
int
ds_lock_data(int sid, unsigned char *data, int size)
{
    Assert(sid >= 0);
    ...
    OpenAL_ErrorCheck(alBufferData(buf_id, format, data, size,
...
//
int
ds_play(int sid, int hid, int snd_id, int priority, int volume, int pan,
        int looping, bool is_voice_msg)
{
    ...
    if (alpan) {
        OpenAL_ErrorPrint(
            alSource3f(Channels[ch_idx].source_id, AL_POSITION, alpan, 0.0, 1.0));
    }
    else {
        OpenAL_ErrorPrint(
            alSource3f(Channels[ch_idx].source_id, AL_POSITION, 0.0, 0.0, 0.0));
    }

    OpenAL_ErrorPrint(
        alSource3f(Channels[ch_idx].source_id, AL_VELOCITY, 0.0, 0.0, 0.0));

    OpenAL_ErrorPrint(alSourcef(Channels[ch_idx].source_id, AL_PITCH, 1.0));

    OpenAL_ErrorPrint(
        alSourcef(Channels[ch_idx].source_id, AL_GAIN, ds_ds_to_al_gain(volume)));

    ALint status;
    OpenAL_ErrorCheck(alGetSourcei(Channels[ch_idx].source_id, AL_SOURCE_STATE,
                                   &status),
                      return -1);
    ...
    OpenAL_ErrorCheck(alSourcei(Channels[ch_idx].source_id, AL_BUFFER,
                                sound_buffers[sid].buf_id),
                      return -1);

    OpenAL_ErrorPrint(
        alSourcei(Channels[ch_idx].source_id, AL_SOURCE_RELATIVE, AL_FALSE));

    OpenAL_ErrorPrint(alSourcei(Channels[ch_idx].source_id, AL_LOOPING,
...
//
int
ds_is_channel_playing(int channel)
{
...
//
void
ds_set_volume(int channel, int vol)
{
...
ds_set_pan(int channel, int pan)
{
    ...
    OpenAL_ErrorCheck(alGetSourcei(Channels[channel].source_id, AL_SOURCE_STATE,
                                   &state),
                      return);

    if (state == AL_PLAYING) {
        ALfloat alpan = (pan != 0) ? ((float)pan / MAX_PAN) : 0.0f;
        OpenAL_ErrorPrint(alSource3f(Channels[channel].source_id, AL_POSITION,
                                     alpan, 0.0, 1.0));
    }
}
...
//
void
ds_chg_loop_status(int channel, int loop)
{
    ALuint source_id = Channels[channel].source_id;
...
ds3d_play(int sid, int hid, int snd_id, vector *pos, vector *vel, int min,
          int max, int looping, int max_volume, int estimated_vol, int priority)
{
    int ch_idx;
    ALint status;
    ...
    // set volume
    OpenAL_ErrorPrint(alSourcef(Channels[ch_idx].source_id, AL_GAIN,
                                ds_ds_to_al_gain(estimated_vol)));

    // set maximum "inner cone" volume
    OpenAL_ErrorPrint(alSourcef(Channels[ch_idx].source_id, AL_MAX_GAIN,
                                ds_ds_to_al_gain(max_volume)));
...
void
ds_set_position(int channel, DWORD offset)
{
    OpenAL_ErrorPrint(
        alSourcei(Channels[channel].source_id, AL_BYTE_OFFSET, offset));
}

DWORD
ds_get_play_position(int channel)
{
    ALint pos = 0;
    ...
    OpenAL_ErrorCheck(alGetSourcei(Channels[channel].source_id, AL_BYTE_OFFSET,
                                   &pos),
                      return 0);
...
DWORD
ds_get_write_position(int channel)
{
...
int
ds_get_channel_size(int channel)
{
...
//
unsigned int
ds_get_primary_buffer_interface()
{
    return 0;
}
...
//
int
ds_eax_set_volume(float volume)
{
    return -1;
}
...
//
int
ds_eax_set_decay_time(float seconds)
{
    return -1;
}
...
//
int
ds_eax_set_environment(unsigned long envid)
{
    return -1;
}
...
//
int
ds_eax_set_preset(unsigned long envid)
{
    return -1;
}
...
//
int
ds_eax_set_all(unsigned long id, float volume, float damping, float decay)
{
    return -1;
}
...
//
int
ds_eax_get_all(EAX_REVERBPROPERTIES *er)
{
    return -1;
}
