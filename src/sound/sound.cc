...
//
int
snd_init(int use_a3d, int use_eax)
{
    int rval;
    ...
    while (1) {
        ...
        if (rval != 0) {
            nprintf((
                "Sound",
                "SOUND ==> Error initializing DirectSound, trying again in 1 second.\n"));
            os_sleep(1000);
        }
        else {
            break;
...
Failure:
    ...
    nprintf(
        ("Sound",
         "SOUND => Direct Sound init unsuccessful, continuing without sound.\n"));
    return 0;
}
...
int
snd_load(game_snd *gs, int allow_hardware_load)
{
    int n, rc, type;
    sound_info *si;
    sound *snd;
    ...
    si->sample_rate =
        header->nSamplesPerSec; // 32-bit sample rate (nSamplesPerSec)
    si->avg_bytes_per_sec =
        header
            ->nAvgBytesPerSec; // 32-bit average bytes per second (nAvgBytesPerSec)
    si->n_block_align =
        header->nBlockAlign; // 16-bit block alignment (nBlockAlign)
    si->bits = header->wBitsPerSample; // Read 16-bit bits per sample

    snd->duration = fl2i(1000.0f * (si->size / (si->bits / 8.0f)) /
                         si->sample_rate);
    type = 0;
    ...
    rc = ds_load_buffer(&snd->sid, &snd->hid, &snd->uncompressed_size, header, si,
                        type);
...
//
int
snd_play_raw(int soundnum, float pan, float vol_scale, int priority)
{
    game_snd gs;
    int rval;
...
//
int
snd_play(game_snd *gs, float pan, float vol_scale, int priority,
         bool is_voice_msg)
{
    float volume;
    sound *snd;
    ...
    if (volume > MIN_SOUND_VOLUME) {
        handle = ds_play(snd->sid, snd->hid, gs->id_sig, ds_priority(priority),
                         ds_convert_volume(volume), fl2i(pan * MAX_PAN), 0,
                         is_voice_msg);
    }
...
//
int
snd_play_3d(game_snd *gs, vector *source_pos, vector *listen_pos, float radius,
            vector *source_vel, int looping, float vol_scale, int priority,
            vector *sound_fvec, float range_factor, int force)
{
    ...
    distance = vm_vec_normalized_dir_quick(&vector_to_sound, source_pos,
                                           listen_pos);
    ...
    if (play_using_ds3d) {
        ...
        handle = ds3d_play(snd->sid, snd->hid, gs->id_sig, source_pos, source_vel,
                           min_range, max_range, looping,
                           ds_convert_volume(max_volume * Master_sound_volume),
                           ds_convert_volume(volume), ds_priority(priority));
    }
    else {
        ...
        if (looping) {
            handle = snd_play_looping(
                gs, pan, -1, -1, volume / gs->default_volume, priority, force);
        }
        else {
...
void
snd_update_3d_pos(int soundnum, game_snd *gs, vector *new_pos)
{
    float vol, pan;
...
//
int
snd_get_3d_vol_and_pan(game_snd *gs, vector *pos, float *vol, float *pan,
                       float radius)
{
    vector vector_to_sound;
    ...
    *pan = 0.0f;
    ...
    if (distance <= gs->min) {
        ...
        *vol = max_volume -
               (distance - gs->min) * max_volume / (gs->max - gs->min);
    }
...
//
int
snd_play_looping(game_snd *gs, float pan, int start_loop, int stop_loop,
                 float vol_scale, int priority, int force)
{
    float volume;
    int handle = -1;
    sound *snd;
    ...
    if ((volume > MIN_SOUND_VOLUME) || force) {
        handle = ds_play(snd->sid, snd->hid, gs->id_sig, ds_priority(priority),
                         ds_convert_volume(volume), fl2i(pan * MAX_PAN), 1);
    }
...
//
void
snd_set_volume(int sig, float volume)
{
    int channel;
    float new_volume;
...
snd_get_pitch(int sig)
{
    ...
    if (channel == -1) {
        nprintf(
            ("Sound", "WARNING: Trying to get pitch for a non-playing sound.\n"));
        return -1;
    }
...
//
void
snd_set_pitch(int sig, int pitch)
{
    int channel;
    ...
    if (channel == -1) {
        nprintf(
            ("Sound", "WARNING: Trying to set pitch for a non-playing sound.\n"));
        return;
    }
...
//
void
snd_chg_loop_status(int sig, int loop)
{
    int channel;
    ...
    if (channel == -1) {
        nprintf(
            ("Sound",
             "WARNING: Trying to change loop status of a non-playing sound!\n"));
        return;
    }
...
// using
void
snd_update_listener(vector *pos, vector *vel, matrix *orient)
{
...
void
snd_rewind(int snd_handle, game_snd *gs, float seconds)
{
...
// this could probably be optimized a bit
void
snd_ffwd(int snd_handle, game_snd *gs, float seconds)
{
    if (!snd_is_playing(snd_handle))
        return;
...
// this could probably be optimized a bit
void
snd_set_pos(int snd_handle, game_snd *gs, float val, int as_pct)
{
    if (!snd_is_playing(snd_handle))
        return;
    ...
    if (as_pct) {
        ...
        ds_set_position(ds_get_channel(snd_handle),
                        (DWORD)((float)snd->size * val));
    }
...
//          !0 => fail
int
snd_get_data(int handle, char *data)
{
...
void
snd_get_format(int handle, int *bits_per_sample, int *frequency)
{
    Assert(handle >= 0 && handle < MAX_SOUNDS);
...
int
snd_time_remaining(int handle, int bits_per_sample, int frequency)
{
