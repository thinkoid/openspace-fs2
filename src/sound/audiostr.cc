...
#include <cfile/cfile.hh> // needed for cf_find_file_location
#include <io/timer.hh>
#include <sound/sound.hh> /* for Snd_sram */
...
static int
openal_error(const char *where)
{
    ALenum err = alGetError();

    if (err != AL_NO_ERROR) {
        nprintf(("SOUND", "SOUND ==> OpenAL error in %s: %s\n", where,
                 (const char *)alGetString(err)));
        return 1;
    }
...
static int
audiostr_read_word(FILE *fp, WORD *i)
{
...
static int
audiostr_read_dword(FILE *fp, DWORD *i)
{
...
static int
audiostr_read_uint(FILE *fp, uint *i)
{
...
const uint DefBufferServiceInterval =
...
// Create
int
AudioStream::Create(char *pszFilename)
{
    ...
    if (pszFilename) {
        ...
        if (m_pwavefile) {
            ...
            m_pwavefile->Init();
            // Open given file
            m_pwavefile->m_bits_per_sample_uncompressed =
            ...
            if (m_pwavefile->Open(pszFilename)) {
                ...
                m_cbBufSize = (m_pwavefile->GetUncompressedAvgDataRate() *
                               m_nBufLength) /
                              1000;
                ...
                if ((m_cbBufSize == m_pwavefile->m_wfmt.nSamplesPerSec) ||
                    (m_cbBufSize == 11025) || (m_cbBufSize == 22050))
                    m_cbBufSize = (uint)((float)m_cbBufSize * 1.3f);

                // if the requested buffer size is too big then cap it
                m_cbBufSize = (m_cbBufSize > BIGBUF_SIZE) ? BIGBUF_SIZE
                                                          : m_cbBufSize;

                nprintf(("SOUND",
                         "SOUND => Stream buffer created using %d bytes\n",
                         m_cbBufSize));
...
// class.
int
AudioStream::WriteWaveData(uint size, uint *num_bytes_written, int service)
{
    int fRtn = SUCCESS;
    ...
    *num_bytes_written = 0;
    ...
    if (num_bytes_read > 0) {
        ...
        alGetError();
        alBufferData(m_buffer_ids[m_play_buffer_id], m_pwavefile->m_ALformat,
...
uint
AudioStream::GetMaxWriteSize(void)
{
...
// Play
void
AudioStream::Play(long volume, int looping)
{
    if (m_buffer_ids[0] != 0) {
        // If playing, stop
        if (m_fPlaying) {
...
int
AudioStream::TimerCallback(void *dwUser)
{
...
void
AudioStream::Set_Byte_Cutoff(unsigned int byte_cutoff)
{
    if (m_pwavefile == NULL)
        return;
...
unsigned int
AudioStream::Get_Bytes_Committed(void)
{
    if (m_pwavefile == NULL)
        return 0;
...
// Fade_and_Destroy
void
AudioStream::Fade_and_Destroy(void)
{
    m_bFade = TRUE;
...
// Fade_and_Destroy
void
AudioStream::Fade_and_Stop(void)
{
    m_bFade = TRUE;
    m_bDestroy_when_faded = FALSE;
}
...
// Stop_and_Rewind
void
AudioStream::Stop_and_Rewind(void)
{
    if (m_fPlaying) {
        // Stop playback
        alSourceStop(m_source_id);
...
// Set_Volume
void
AudioStream::Set_Volume(long vol)
{
    if (vol < -10000)
        vol = -10000;
    ...
    // OpenAL gain
    ALfloat alvol = (vol != -10000)
                        ? powf(10.0f, (float)vol / (-600.0f / log10f(.5f)))
                        : 0.0f;
...
// Create
int
Timer::Create(uint nPeriod, uint nRes, void *dwUser, TIMERCALLBACK pfnCallback)
{
...
//
Uint32
Timer::TimeProc(Uint32 interval, void *param)
{
...
// Open
int
WaveFile::Open(char *pszFilename)
{
    int done = FALSE;
    WORD cbExtra = 0;
    int fRtn = SUCCESS; // assume success
    ...
    if (!cf_find_file_location(pszFilename, CF_TYPE_ANY, fullpath, &FileSize,
                               &FileOffset)) {
        goto OPEN_ERROR;
    }
    ...
    while (done == FALSE) {
        ...
        case 0x20746d66: // The 'fmt ' tag
            ...
            if ((m_pwfmt_original = (WAVEFORMATEX *)malloc(sizeof(WAVEFORMATEX) +
                                                           cbExtra)) != NULL) {
                ...
                *m_pwfmt_original = pcmwf;
                ...
                if (cbExtra != 0) {
                    fread((ubyte *)(m_pwfmt_original) + WAVEFORMATEX_DISK_SIZE, 1,
                          cbExtra, cfp);
                }
            }
            else {
        ...
        case 0x61746164: // the 'data' tag
            m_nDataSize =
                size; // This is size of data chunk.  Compressed if ADPCM.
    ...
    m_wfmt.cbSize = 0;
    m_wfmt.nBlockAlign =
        (unsigned short)((m_wfmt.nChannels * m_wfmt.wBitsPerSample) / 8);
    ...
    case 8:
        m_ALformat = (m_wfmt.nChannels == 2) ? AL_FORMAT_STEREO8
                                             : AL_FORMAT_MONO8;
        break;

    case 16:
        m_ALformat = (m_wfmt.nChannels == 2) ? AL_FORMAT_STEREO16
                                             : AL_FORMAT_MONO16;
        break;
...
int
WaveFile::Read(ubyte *pbDest, uint cbSize, int service)
{
    ...
    case WAVE_FORMAT_ADPCM:
        if (!m_hStream_open) {
            if (!ACM_stream_open(m_pwfmt_original, &m_wfxDest,
                                 (void **)&m_hStream,
                                 m_bits_per_sample_uncompressed)) {
                m_hStream_open = 1;
            }
    ...
    if (m_wave_format == WAVE_FORMAT_ADPCM) {
        if (num_bytes_read > 0) {
            rc = ACM_convert((void *)m_hStream, dest_buf, num_bytes_read,
                             uncompressed_wave_data, BIGBUF_SIZE, &convert_len,
                             &src_bytes_used);
            if (rc == -1) {
                goto READ_ERROR;
            }
            if (convert_len == 0) {
                if (num_bytes_read < m_nBlockAlign) {
                    nprintf((
                        "SOUND",
                        "SOUND => Warning: short read detected in ACM decode of '%s'\n",
                        m_wFilename));
                }
                else {
                    Int3();
...
audiostream_init()
{
    ...
    if (Compressed_service_buffer == NULL) {
        Compressed_service_buffer = (unsigned char *)malloc(
...
void
audiostream_close_file(int i, int fade)
{
...
void
audiostream_close_all(int fade)
{
    int i;
...
void
audiostream_play(int i, float volume, int looping)
{
    if (!Audiostream_inited)
        return;
...
void
audiostream_stop(int i, int rewind, int paused)
{
    if (!Audiostream_inited)
        return;
...
void
audiostream_set_volume_all(float volume, int type)
{
    int i;
...
void
audiostream_set_volume(int i, float volume)
{
    if (i == -1)
        return;
...
void
audiostream_set_byte_cutoff(int i, unsigned int cutoff)
{
    if (i == -1)
        return;
...
unsigned int
audiostream_get_bytes_committed(int i)
{
    if (i == -1)
        return 0;
...
int
audiostream_done_reading(int i)
{
    if (i == -1)
        return 0;
