...
//
int
ds3d_update_buffer(int channel, float min, float max, vector *pos, vector *vel)
{
...
ds3d_set_sound_cone(int channel, int inner_angle, int outer_angle, int vol)
{
    ...
    OpenAL_ErrorPrint(
        alSourcei(Channels[channel].source_id, AL_CONE_INNER_ANGLE, inner_angle));
    OpenAL_ErrorPrint(
        alSourcei(Channels[channel].source_id, AL_CONE_OUTER_ANGLE, outer_angle));
    OpenAL_ErrorPrint(alSourcef(Channels[channel].source_id, AL_CONE_OUTER_GAIN,
                                ds_get_percentage_vol(vol)));
