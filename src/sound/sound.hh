...
typedef struct game_snd
{
    ...
    int min,
        max; // min: distance at which sound will stop getting louder  max: distance at which sound is inaudible
...
int snd_play(game_snd *gs, float pan = 0.0f, float vol_scale = 1.0f,
...
int snd_play_raw(int soundnum, float pan, float vol_scale = 1.0f,
...
int snd_play_3d(game_snd *gs, vector *source_pos, vector *listen_pos,
                float radius = 0.0f, vector *vel = NULL, int looping = 0,
                float vol_scale = 1.0f,
                int priority = SND_PRIORITY_SINGLE_INSTANCE,
                vector *sound_fvec = NULL, float range_factor = 1.0f,
                int force = 0);
...
int snd_play_looping(game_snd *gs, float pan = 0.0f, int start_loop = -1,
                     int stop_loop = -1, float vol_scale = 1.0f,
...
int snd_get_3d_vol_and_pan(game_snd *gs, vector *pos, float *vol, float *pan,
                           float radius = 0.0f);
...
void snd_set_pos(
    int snd_handle, game_snd *sg, float val,
...
int snd_time_remaining(int handle, int bits_per_sample = 8,
                       int frequency = 11025);
