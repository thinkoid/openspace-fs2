...
typedef struct cscrew_info
{
    ...
    matrix
        real_orient; // the orientation used when calling physics (bashed before rendering)
...
//
void
cscrew_maybe_fire_missile(int shipnum)
{
    ship *sp;
    ship_weapon *swp;
...
void
cscrew_process_pre(object *objp)
{
    cscrew_info *ci;
    ...
    Assert(Corkscrew_missiles[Weapons[objp->instance].cscrew_index].flags &
           CS_FLAG_USED);
...
void
cscrew_process_post(object *objp)
{
    vector cen, neg;
    vector new_pt;
    weapon *wp;
    ...
    vm_rot_point_around_line(&new_pt, &objp->pos, twist_val, &cen,
                             &objp->orient.fvec);
    objp->pos = new_pt;
