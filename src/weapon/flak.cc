...
flak_level_init()
{
    ...
    if (Flak_muzzle_flash_ani == -1) {
        Flak_muzzle_flash_ani = bm_load_animation(MUZZLE_FLASH_FILE, &num_frames,
                                                  &fps, 1);
    }
...
flak_create(weapon *wp)
{
    ...
    obj_set_flags(&Objects[wp->objnum],
                  Objects[wp->objnum].flags & ~(OF_RENDERS));
...
void
flak_pick_range(object *objp, vector *predicted_target_pos,
                float weapon_subsys_strength)
{
    ...
    Assert(Weapon_info[Weapons[objp->instance].weapon_info_index].wi_flags &
           WIF_FLAK);
    ...
    final_range += (Flak_range +
                    (Flak_range * 0.65f * (1.0f - weapon_subsys_strength))) *
...
void
flak_jitter_aim(vector *dir, float dist_to_target, float weapon_subsys_strength)
{
    ...
    // error value
    error_val = Flak_error +
                (Flak_error * 0.65f * (1.0f - weapon_subsys_strength));
    ...
    vm_rot_point_around_line(&rand_twist_post, &rand_twist_pre,
                             fl_radian(359.0f * frand_range(0.0f, 1.0f)),
                             &vmd_zero_vector, dir);
...
void
flak_muzzle_flash(vector *pos, vector *dir, int turret_weapon_class)
{
    // sanity
    Assert((turret_weapon_class >= 0) &&
...
void
flak_maybe_detonate(object *objp)
{
    vector temp;

    // if the shell has gone past its range, blow it up
    vm_vec_sub(&temp, &objp->pos,
               &Flak[Weapons[objp->instance].flak_index].start_pos);
...
// given a just fired flak shell, pick a detonating distance for it
void
flak_set_range(object *objp, vector *start_pos, float range)
{
    Assert(objp->type == OBJ_WEAPON);
    Assert(objp->instance >= 0);
