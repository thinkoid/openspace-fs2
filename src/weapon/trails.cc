...
void
trail_calc_facing_pts(vector *top, vector *bot, vector *fvec, vector *pos,
                      float w)
{
    vector uvec, rvec;
...
// trail is on ship
int
trail_is_on_ship(int trail_index, ship *shipp)
{
    int idx;
...
trail_render(trail *trailp)
{
    ...
    if ((Player_ship != NULL) && trail_is_on_ship(trailp - Trails, Player_ship) &&
    ...
    for (i = 0; i < num_sections; i++) {
        ...
        l = (ubyte)fl2i(
            (trailp->val[n] * (ti->a_end - ti->a_start) + ti->a_start) * 255.0f);
        ...
        if (i > 0) {
            if (i == num_sections - 1) {
                ...
                gr_set_bitmap(ti->bitmap, GR_ALPHABLEND_FILTER,
                              GR_BITBLT_MODE_NORMAL, l / 255.0f);
...
void
trail_add_segment(int trail_num, vector *pos)
{
    if (trail_num < 0)
        return;
    if (trail_num >= MAX_TRAILS)
        return;
...
void
trail_set_segment(int trail_num, vector *pos)
{
    if (trail_num < 0)
        return;
    if (trail_num >= MAX_TRAILS)
...
void
trail_move_all(float frametime)
{
    trail *trailp;
...
void
trail_object_died(int trail_num)
{
    if (trail_num < 0)
        return;
    if (trail_num >= MAX_TRAILS)
...
int
trail_stamp_elapsed(int trail_num)
{
    return timestamp_elapsed(Trails[trail_num].trail_stamp);
}

void
trail_set_stamp(int trail_num)
{
