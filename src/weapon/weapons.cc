...
int
get_weapon_expl_info_index(char *filename)
{
...
int
missile_obj_list_add(int objnum)
{
    int i;
    ...
    for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list);
        ...
        if (objp->type == OBJ_WEAPON &&
            Weapon_info[Weapons[objp->instance].weapon_info_index].subtype ==
                WP_MISSILE) {
            Weapons[objp->instance].missile_list_index = missile_obj_list_add(
                OBJ_INDEX(objp));
        }
    }
}
...
void
weapon_maybe_alert_cmeasure_success(object *objp)
{
...
//
missile_obj *
missile_obj_return_address(int index)
{
    Assert(index >= 0 && index < MAX_MISSILE_OBJS);
...
void
parse_wi_flags(weapon_info *weaponp)
{
    ...
    for (int i = 0; i < num_strings; i++) {
        if (!stricmp(NOX("Electronics"), weapon_strings[i]))
            ...
            if (weaponp->spawn_type == -1) {
                ...
                skip_length = strlen(NOX("Spawn")) +
                              strspn(&temp_string[strlen(NOX("Spawn"))],
                                     NOX(" \t"));
                ...
                strncpy(Spawn_names[Num_spawn_types++],
                        &(weapon_strings[i][skip_length]), name_length);
    ...
    if (weaponp->wi_flags & WIF_FLAK) {
        Assert(!(weaponp->wi_flags & WIF_CORKSCREW) &&
               !(weaponp->wi_flags & WIF_SWARM));
    }
    if (weaponp->wi_flags & WIF_CORKSCREW) {
        Assert(!(weaponp->wi_flags & WIF_FLAK) &&
               !(weaponp->wi_flags & WIF_SWARM));
    }
...
//             the '@' symbol
    ...
    if (is_homing == 1) {
        ...
        if (!stricmp(temp_type, NOX("HEAT"))) {
            ...
            required_string(
                "+Min Lock Time:"); // minimum time (in seconds) to achieve lock
            ...
            required_string(
                "+Catch-up Pixels/Sec:"); // pixels/sec moved while catching-up for a lock
            stuff_int(&wip->catchup_pixels_per_sec);

            required_string(
                "+Catch-up Penalty:"); // number of extra pixels to move while locking as a penalty for catching up for a lock
    ...
    if (optional_string("$Impact Explosion:")) {
        ...
        if (stricmp(impact_ani_file, NOX("none"))) {
            wip->impact_weapon_expl_index = get_weapon_expl_info_index(
                impact_ani_file);
            //int num_frames, fps;
...
weapon_render(object *obj)
{
    ...
    case WRT_LASER: {
        ...
        if (wip->laser_bitmap >= 0) {
            ...
            gr_set_bitmap(wip->laser_bitmap, GR_ALPHABLEND_FILTER,
                          GR_BITBLT_MODE_NORMAL, 0.99999f);

            vector headp;
            vm_vec_scale_add(&headp, &obj->pos, &obj->orient.fvec,
                             wip->laser_length);
            wp->weapon_flags &= ~WF_CONSIDER_FOR_FLYBY_SOUND;
            if (g3_draw_laser(&headp, wip->laser_head_radius, &obj->pos,
                              wip->laser_tail_radius)) {
                wp->weapon_flags |= WF_CONSIDER_FOR_FLYBY_SOUND;
            }
        }
        ...
        if (wip->laser_glow_bitmap >= 0) {
            ...
            vector headp2;
            vm_vec_scale_add(&headp2, &obj->pos, &obj->orient.fvec,
                             wip->laser_length * weapon_glow_scale_l);
            gr_set_bitmap(wip->laser_glow_bitmap, GR_ALPHABLEND_FILTER,
                          GR_BITBLT_MODE_NORMAL, weapon_glow_alpha);
            g3_draw_laser_rgb(&headp2,
                              wip->laser_head_radius * weapon_glow_scale_f,
                              &obj->pos,
                              wip->laser_tail_radius * weapon_glow_scale_r, c.red,
                              c.green, c.blue);
        }
        break;
    }
    ...
    case WRT_POF: {
        ...
        if ((wip->wi_flags & WIF_THRUSTER) && (wp->thruster_bitmap > -1)) {
            ...
            model_set_thrust(wip->model_num, ft, wp->thruster_bitmap,
                             wp->thruster_glow_bitmap, wp->thruster_glow_noise);
...
void
weapon_maybe_play_warning(weapon *wp)
{
    if (wp->homing_object == Player_obj) {
        if (!(wp->weapon_flags & WF_LOCK_WARNING_PLAYED)) {
            ...
            if (Weapon_info[wp->weapon_info_index].wi_flags & WIF_HOMING_HEAT) {
                ...
                Assert(Weapon_info[wp->weapon_info_index].wi_flags &
                       WIF_HOMING_ASPECT);
...
void
detonate_nearby_missiles(cmeasure *cmp)
{
    missile_obj *mop;
    vector cmeasure_pos;
    ...
    while (mop != END_OF_LIST(&Missile_obj_list)) {
        ...
        if (wp->team != cmp->team) {
            if (Missiontime - wp->creation_time > F1_0 / 2) {
                if (vm_vec_dist_quick(&cmeasure_pos, &objp->pos) <
...
void
find_homing_object(object *weapon_objp, int num)
{
    object *objp;
    weapon_info *wip;
    weapon *wp;
    ...
    for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list);
        ...
        if ((objp->type == OBJ_SHIP) || (objp->type == OBJ_CMEASURE)) {
            ...
            if ((homing_object_team != wp->team) ||
                (homing_object_team == TEAM_TRAITOR)) {
                float dist;
                float dot;
                ...
                dist = vm_vec_normalized_dir(&vec_to_object, &objp->pos,
                                             &weapon_objp->pos);
...
void
find_homing_object_cmeasures_1(object *weapon_objp)
{
    object *objp;
    weapon *wp;
    weapon_info *wip;
        ...
        if (objp->type == OBJ_CMEASURE) {
            ...
            if (dist < MAX_CMEASURE_TRACK_DIST) {
                ...
                if (wip->wi_flags & WIF_HOMING_ASPECT) {
                    chance =
                        1.0f /
                        2.0f; // aspect seeker this likely to chase a countermeasure
                }
                else {
                    chance =
                        1.0f /
                        1.5f; // heat seeker this likely to chase a countermeasure
                }
                if ((objp->signature != wp->cmeasure_ignore_objnum) &&
                    (objp->signature != wp->cmeasure_chase_objnum)) {
                    if (frand() < chance) {
                        wp->cmeasure_ignore_objnum =
                            objp->signature; // Don't process this countermeasure again.
                        //nprintf(("Jim", "Frame %i: Weapon #%i ignoring cmeasure #%i\n", Framecount, OBJ_INDEX(weapon_objp), objp->signature));
                    }
                    else {
                        wp->cmeasure_chase_objnum =
                            objp->signature; // Don't process this countermeasure again.
...
find_homing_object_cmeasures()
{
    ...
    for (weapon_objp = GET_FIRST(&obj_used_list);
...
void
find_homing_object_by_sig(object *weapon_objp, int sig)
{
    ship_obj *sop;
    weapon *wp;
...
void
weapon_home(object *obj, int num, float frame_time)
{
    weapon *wp;
    weapon_info *wip;
    object *hobjp;
    ...
    if ((hobjp == END_OF_LIST(&obj_used_list)) ||
        ...
        else if ((obj->phys_info.speed < wip->max_speed / 4) &&
...
*/
    ...
    case OBJ_WEAPON:
        ...
        Assert(Weapon_info[Weapons[hobjp->instance].weapon_info_index].wi_flags &
               WIF_BOMB);
        if (wip->wi_flags & WIF_HOMING_ASPECT)
            find_homing_object_by_sig(obj, wp->target_sig);
        else
    ...
    if (hobjp != END_OF_LIST(&obj_used_list)) {
        ...
        if (wp->homing_subsys != NULL) {
            get_subsystem_world_pos(hobjp, Weapons[num].homing_subsys,
                                    &target_pos);
            wp->homing_pos =
        ...
        dist_to_target = vm_vec_normalized_dir(&vec_to_goal, &target_pos,
                                               &obj->pos);
        ...
        if ((wip->wi_flags & WIF_HOMING_ASPECT) && (old_dot > 0.1f) &&
            (time_to_target > 0.1f))
            vm_vec_scale_add2(&target_pos, &hobjp->phys_info.vel,
        ...
        if (wip->wi_flags & WIF_HOMING_HEAT) {
            if ((old_dot < wip->fov) &&
                ...
                if (wip->fov < 0.95f) {
                 ...
                 WIF_HOMING_ASPECT) { //  subtract life as if max turn is 90 degrees.
            if (wip->fov < 0.95f)
                wp->lifeleft -= flFrametime * (0.95f - old_dot);
        }
        else
        ...
        if (wp->swarm_index < 0) {
            ...
            ai_turn_towards_vector(&target_pos, obj, frame_time, wip->turn_time,
                                   NULL, NULL, 0.0f, 0, NULL);
...
void
weapon_process_pre(object *obj, float frame_time)
{
    ...
    if ((obj->type == OBJ_WEAPON) &&
        (Weapon_info[Weapons[obj->instance].weapon_info_index].wi_flags &
         WIF_FLAK) &&
        (Weapons[obj->instance].flak_index >= 0)) {
...
void
weapon_maybe_play_flyby_sound(object *weapon_objp, weapon *wp)
{
    ...
    if (!(wp->weapon_flags & WF_PLAYED_FLYBY_SOUND) &&
        ...
        if ((dist > radius) && (dist < 55)) {
            ...
            if ((dot < -0.80) && (dot > -0.98)) {
                snd_play_3d(&Snds[SND_WEAPON_FLYBY], &weapon_objp->pos,
                            &Eye_position);
...
void
weapon_process_post(object *obj, float frame_time)
{
    int num;
    weapon_info *wip;
    weapon *wp;
...
void
weapon_set_tracking_info(int weapon_objnum, int parent_objnum, int target_objnum,
...
int
weapon_create(vector *pos, matrix *orient, int weapon_id, int parent_objnum,
              int secondary_flag, int group_id, int is_locked)
{
    int n, objnum;
    int num_deleted;
    ...
    if ((wip->subtype == WP_LASER) ||
        ((wip->subtype == WP_MISSILE) && !(wip->wi_flags & WIF_HOMING))) {
        // set physics flag
    ...
    if (parent_objp != NULL) {
        Assert(parent_objp->type ==
               OBJ_SHIP); //  Get Mike, a non-ship has fired a weapon!
        Assert((parent_objp->instance >= 0) &&
    ...
    if (!(wip->wi_flags & WIF_HOMING)) {
        vm_vec_copy_scale(&objp->phys_info.desired_vel, &objp->orient.fvec,
                          objp->phys_info.max_vel.z);
        ...
        if (parent_objp != NULL) {
            vm_vec_copy_scale(&objp->phys_info.desired_vel, &objp->orient.fvec,
                              vm_vec_dot(&parent_objp->phys_info.vel,
                                         &parent_objp->orient.fvec) +
                                  objp->phys_info.max_vel.z / 4);
        }
        else {
            vm_vec_copy_scale(&objp->phys_info.desired_vel, &objp->orient.fvec,
                              objp->phys_info.max_vel.z / 4);
...
void
spawn_child_weapons(object *objp)
{
    int i;
    int child_id;
    int parent_num;
    ...
    wp = &Weapons[objp->instance];
    Assert((wp->weapon_info_index >= 0) &&
           (wp->weapon_info_index < MAX_WEAPON_TYPES));
    wip = &Weapon_info[wp->weapon_info_index];
    ...
    for (i = 0; i < wip->spawn_count; i++) {
        ...
        weapon_objnum = weapon_create(&pos, &orient, child_id, parent_num, 1, -1,
                                      wp->weapon_flags & WF_LOCKED_WHEN_FIRED);
        ...
        if (weapon_objnum != -1) {
            ...
            Weapons[Objects[weapon_objnum].instance].lifeleft *= rand_val * 0.4f +
                                                                 0.8f;
        }
    }
}
...
//
void
weapon_hit_do_sound(object *hit_obj, weapon_info *wip, vector *hitpos)
{
    int is_hull_hit;
    float shield_str;
    ...
    if (timestamp_elapsed(Weapon_impact_timer)) {
        ...
        if (!is_hull_hit) {
            ...
            if (hit_obj == Player_obj) {
                ...
                if (wip->subtype == WP_MISSILE) {
                    snd_play_3d(&Snds[SND_PLAYER_HIT_MISSILE], hitpos,
                                &Eye_position);
                }
            }
            else {
            ...
            case WP_LASER:
                if (hit_obj == Player_obj)
                    snd_play_3d(&Snds[SND_PLAYER_HIT_LASER], hitpos,
                                &Eye_position);
                else {
                    if (wip->impact_snd != -1) {
                        snd_play_3d(&Snds[wip->impact_snd], hitpos,
                                    &Eye_position);
                    }
                }
                break;
...
void
weapon_do_electronics_affect(object *ship_objp, vector *blast_pos, int wi_index)
{
    weapon_info *wip;
    ship *shipp;
    ship_subsys *ss;
...
//
int
weapon_area_calc_damage(object *objp, vector *pos, float inner_rad,
                        ...
                        float *blast, float *damage, float limit)
{
    ...
    if (dist < (inner_rad + objp->radius)) {
        ...
        *blast = max_blast;
        ...
        dist_to_outer_rad_squared = (outer_rad - min_dist) *
                                    (outer_rad - min_dist);
...
//
void
weapon_area_apply_blast(vector *force_apply_pos, object *ship_obj,
...
#define SHAKE_CONST 3000
    ...
    if (make_shockwave) {
        physics_apply_shock(&force, blast, &ship_obj->phys_info,
                            &ship_obj->orient, &pm->mins, &pm->maxs, pm->rad);
    }
    else {
...
weapon_do_area_effect(object *wobjp, vector *pos, object *other_obj)
{
    ...
    for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list);
        ...
        if (weapon_area_calc_damage(
                objp, pos, wip->inner_radius, wip->outer_radius, wip->blast_force,
                wip->damage, &blast, &damage, wip->outer_radius) == -1) {
            continue;
        }
    ...
    if ((other_obj != NULL) && (wip->wi_flags & WIF_ELECTRONICS)) {
        if (other_obj->type == OBJ_SHIP) {
            weapon_do_electronics_affect(
                other_obj, pos, Weapons[wobjp->instance].weapon_info_index);
        }
    }
}
...
//
void
weapon_hit(object *weapon_obj, object *other_obj, vector *hitpos)
{
    Assert(weapon_obj != NULL);
    if (weapon_obj == NULL) {
        return;
    }
    Assert((weapon_obj->type == OBJ_WEAPON) && (weapon_obj->instance >= 0) &&
    ...
    if (wip->impact_weapon_expl_index > -1) {
        int expl_ani_handle = weapon_get_expl_handle(
            wip->impact_weapon_expl_index, hitpos, wip->impact_explosion_radius);
        particle_create(hitpos, &vmd_zero_vector, 0.0f,
                        wip->impact_explosion_radius, PARTICLE_BITMAP_PERSISTENT,
                        expl_ani_handle);
    }
    ...
    if (wip->wi_flags & WIF_AREA_EFFECT) {
        ...
        if (wip->wi_flags & WIF_SHOCKWAVE) {
            ...
            if (((other_obj) && (other_obj->type == OBJ_WEAPON)) ||
                (Weapons[num].weapon_flags & WF_DESTROYED_BY_WEAPON)) {
                actual_damage /= 4.0f;
    ...
    if (wip->wi_flags & WIF_EMP) {
        emp_apply(&weapon_obj->pos, wip->inner_radius, wip->outer_radius,
                  wip->emp_intensity, wip->emp_time);
    }
...
weapons_page_in()
{
    ...
    for (i = 0; i < Num_weapon_types; i++) {
        ...
        if (wip->wi_flags & WIF_BEAM) {
            ...
            for (idx = 0; idx < wip->b_info.beam_num_sections; idx++) {
                if ((idx < MAX_BEAM_SECTIONS) &&
                    (wip->b_info.sections[idx].texture >= 0)) {
            ...
            if (wip->b_info.beam_particle_ani >= 0) {
                int nframes, fps;
                bm_get_info(wip->b_info.beam_particle_ani, NULL, NULL, NULL,
                            &nframes, &fps);
    ...
    for (i = 0; i < Num_weapon_expl; i++) {
        ...
        for (j = 0; j < Weapon_expl_info[i].lod_count; j++) {
            //load ani
            bitmap_handle = bm_load_animation(Weapon_expl_info[i].lod[j].filename,
                                              &nframes, &fps, 1);
...
void
weapon_get_laser_color(color *c, object *objp)
{
    weapon *wep;
    weapon_info *winfo;
    float pct;
    ...
    if ((winfo->laser_color_2.red == 0) && (winfo->laser_color_2.green == 0) &&
        (winfo->laser_color_2.blue == 0)) {
    ...
    gr_init_color(c,
                  (int)((float)winfo->laser_color_1.red +
                        (((float)winfo->laser_color_2.red -
                          (float)winfo->laser_color_1.red) *
                         pct)),
                  (int)((float)winfo->laser_color_1.green +
                        (((float)winfo->laser_color_2.green -
                          (float)winfo->laser_color_1.green) *
                         pct)),
                  (int)((float)winfo->laser_color_1.blue +
                        (((float)winfo->laser_color_2.blue -
                          (float)winfo->laser_color_1.blue) *
                         pct)));
}
...
void
weapon_maybe_spew_particle(object *obj)
{
    weapon *wp;
    int idx;
    ...
    Assert(Weapon_info[Weapons[obj->instance].weapon_info_index].wi_flags &
           WIF_PARTICLE_SPEW);

    wp = &Weapons[obj->instance];

    // if the weapon's particle timestamp has elapse`d
    if ((wp->particle_spew_time == -1) ||
        timestamp_elapsed(wp->particle_spew_time)) {
        // reset the timestamp
        ...
        for (idx = 0; idx < Weapon_particle_spew_count; idx++) {
            ...
            vm_rot_point_around_line(&direct_temp, &direct, ang, &null_vec,
                                     &obj->orient.fvec);
            direct = direct_temp;
            ...
            particle_create(&particle_pos, &vel, Weapon_particle_spew_lifetime,
                            Weapon_particle_spew_radius, PARTICLE_SMOKE);
        }
    }
}
...
float
weapon_get_damage_scale(weapon_info *wip, object *wep, object *target)
{
    weapon *wp;
    int from_player = 0;
    ...
    if ((wep->parent >= 0) && (wep->parent < MAX_OBJECTS) &&
        (Objects[wep->parent].flags & OF_PLAYER_SHIP)) {
        from_player = 1;
    }

    // if this is a lockarm weapon, and it was fired unlocked
    if ((wip->wi_flags & WIF_LOCKARM) &&
        !(wp->weapon_flags & WF_LOCKED_WHEN_FIRED)) {
        total_scale *= 0.1f;
    }
    ...
    if (target->type == OBJ_SHIP) {
        ...
        if (from_player && is_big_damage_ship &&
            !(wip->wi_flags & (WIF_HURTS_BIG_SHIPS))) {
...
int
weapon_get_expl_handle(int weapon_expl_index, vector *pos, float size)
{
    ...
    if (!g3_get_bitmap_dims(wei->lod[0].bitmap_id, &v, size, &x, &y, &w, &h,
                            &bm_size)) {
