...
swarm_level_init()
{
    ...
    Turret_swarm_validity_next_check_time = timestamp(
        TURRET_SWARM_VALIDITY_CHECKTIME);
}
...
//
void
swarm_maybe_fire_missile(int shipnum)
{
    ship *sp;
    ship_weapon *swp;
...
swarm_update_direction(object *objp, float frametime)
{
    ...
    if (timestamp_elapsed(swarmp->change_timestamp)) {
        if (swarmp->path_num == -1) {
            ...
            vm_vec_scale_add(&swarmp->original_target, &objp->pos,
                             &objp->orient.fvec, SWARM_CONE_LENGTH);
            swarmp->circle_rvec = objp->orient.rvec;
            swarmp->circle_uvec = objp->orient.uvec;

            swarmp->change_count = 1;
            swarmp->change_time = fl2i(SWARM_CHANGE_DIR_TIME +
                                       SWARM_TIME_VARIANCE * (frand() - 0.5f) *
                                           2);
...
swarm_new_target_calced:
    ...
    ai_turn_towards_vector(&swarmp->new_target, objp, frametime, wip->turn_time,
                           NULL, NULL, 0.0f, 0);
...
void
turret_swarm_set_up_info(int parent_objnum, ship_subsys *turret,
                         int turret_weapon_class)
{
    ...
    Assert((turret->turret_enemy_objnum >= 0) &&
...
void
turret_swarm_maybe_fire_missile(int shipnum)
{
    ...
    for (subsys = GET_FIRST(&shipp->subsys_list);
        ...
        if (subsys->turret_swarm_info_index != -1) {
            ...
            // get turret_swarm_info
            Assert((subsys->turret_swarm_info_index >= 0) &&
                   (subsys->turret_swarm_info_index < MAX_TURRET_SWARM_INFO));
            ...
            if ((tsi->parent_sig == parent_obj->signature)) {
                ...
                if (timestamp_elapsed(tsi->time_to_fire)) {
                    ...
                    if (subsys->current_hits > 0) {
                        turret_swarm_fire_from_turret(tsi->turret,
                                                      tsi->parent_objnum,
                                                      target_objnum,
                                                      tsi->target_subsys);
                    }
