...
float Emp_intensity =
    -1.0f; // current intensity of the EMP effect (normalized to EMP_INTENSITY_MAX)
...
void
emp_apply(vector *pos, float inner_radius, float outer_radius,
        ...
        if (Ship_info[Ships[target->instance].ship_info_index].flags &
            ...
            while (moveup != &Ships[target->instance].subsys_list) {
                ...
                if ((moveup->system_info != NULL) &&
                    ...
                    vm_vec_unrotate(&actual_pos, &moveup->system_info->pnt,
                                    &target->orient);
                    ...
                    ship_subsys_set_disrupted(
                        moveup, (int)(MAX_TURRET_DISRUPT_TIME * scale_factor));

                    mprintf(("EMP disrupting subsys %s on ship %s (%f, %f)\n",
                             moveup->system_info->name,
                             Ships[Objects[so->objnum].instance].ship_name,
                             scale_factor,
                             MAX_TURRET_DISRUPT_TIME * scale_factor));
                }
...
void
emp_start_ship(object *ship_obj, float intensity, float time)
{
    ship *shipp;
    ai_info *aip;
...
void
emp_start_local(float intensity, float time)
{
    int idx;
    float start_intensity;
...
// maybe reformat a string
void
emp_maybe_reformat_text(char *text, int max_len, int gauge_id)
{
    wacky_text *wt;
    ...
    if ((wt->stamp == -1) || timestamp_elapsed(wt->stamp)) {
        ...
        case EG_WEAPON_S2:
            ...
            strcpy(
                wt->str,
                Weapon_info[wep_index >= MAX_WEAPON_TYPES ? 0 : wep_index].name);
            break;
