...
typedef struct beam : list_links_t< beam >
{
    ...
    object *
        objp; // the shooting object (who owns the turret that I am being fired from)
...
void beam_get_octant_points(int modelnum, object *objp, int oct_index,
                            int oct_array[BEAM_NUM_GOOD_OCTANTS][4], vector *v1,
                            vector *v2);
...
void beam_calc_facing_pts(vector *top, vector *bot, vector *fvec, vector *pos,
                          float w, float z_add);
...
int
beam_fire(beam_fire_info *fire_info)
{
    beam *new_item;
    weapon_info *wip;
    ship *firing_ship;
    ...
    if ((fire_info->target->type != OBJ_SHIP) &&
    ...
    Assert((fire_info->beam_info_index >= 0) &&
    ...
    if (new_item->type == BEAM_TYPE_D) {
        ...
        if ((fire_info->target != NULL) &&
            ...
            if (Ship_info[target_ship->ship_info_index].flags &
                (SIF_BIG_SHIP | SIF_CAPITAL | SIF_SUPERCAP | SIF_FREIGHTER |
                 SIF_DRYDOCK | SIF_CARGO)) {
                new_item->type = BEAM_TYPE_A;
            }
        }
    }
    ...
    objnum = obj_create(OBJ_BEAM, -1, new_item - Beams, &vmd_identity_matrix,
                        &vmd_zero_vector, 1.0f, OF_COLLIDES);
    if (objnum < 0) {
        Int3();
...
int
beam_fire_targeting(beam_fire_info *fire_info)
{
    beam *new_item;
    weapon_info *wip;
    int objnum;
...
int
beam_get_weapon_info_index(object *bm)
{
    Assert(bm->type == OBJ_BEAM);
    if (bm->type != OBJ_BEAM) {
        return -1;
...
int
beam_get_num_collisions(int objnum)
{
    // sanity checks
...
beam_type_a_move(beam *b)
{
    ...
    ship_get_global_turret_gun_info(b->objp, b->subsys, &b->last_start, &temp, 1,
                                    &temp2);
...
#define BEAM_T(b)                                                                \
    (((b->binfo.delta_ang / b->life_total) * (b->life_total - b->life_left)) /   \
     b->binfo.delta_ang)
void
beam_type_b_move(beam *b)
{
    vector actual_dir;
    vector temp, temp2;
    float dot_save;
    ...
    if (!is_valid) {
        actual_dir = b->binfo.dir_a;
        vm_vec_scale_add(&b->last_shot, &b->last_start, &actual_dir,
                         BEAM_FAR_LENGTH);
    }
}
...
beam_type_c_move(beam *b)
{
    ...
    vm_vec_scale_add(&b->last_shot, &b->last_start, &b->objp->orient.fvec,
                     BEAM_FAR_LENGTH);
}

// type D functions
void
beam_type_d_move(beam *b)
{
...
void
beam_type_d_get_status(beam *b, int *shot_index, int *fire_wait)
{
...
// type e functions
void
beam_type_e_move(beam *b)
{
    vector temp, turret_norm;

    // LEAVE THIS HERE OTHERWISE MUZZLE GLOWS DRAW INCORRECTLY WHEN WARMING UP OR DOWN
    // get the "originating point" of the beam for this frame. essentially bashes last_start
    ship_get_global_turret_gun_info(b->objp, b->subsys, &b->last_start,
                                    &turret_norm, 1, &temp);
    ...
    // put the "last_shot" point arbitrarily far away
    vm_vec_scale_add(&b->last_shot, &b->last_start, &turret_norm,
                     BEAM_FAR_LENGTH);
    Assert(is_valid_vec(&b->last_shot));
}
...
int poly_beam = 0;
void
beam_render(beam_weapon_info *bwi, vector *start, vector *shot, float shrink)
{
    int idx, s_idx;
    ...
    for (s_idx = 0; s_idx < bwi->beam_num_sections; s_idx++) {
        ...
        scale = frand_range(1.0f - bwi->sections[s_idx].flicker,
                            1.0f + bwi->sections[s_idx].flicker);
        beam_calc_facing_pts(&top1, &bottom1, &fvec, start,
                             bwi->sections[s_idx].width * scale * shrink,
                             bwi->sections[s_idx].z_add);
        beam_calc_facing_pts(&top2, &bottom2, &fvec, shot,
                             bwi->sections[s_idx].width * scale * scale * shrink,
                             bwi->sections[s_idx].z_add);
        ...
        gr_set_bitmap(bwi->sections[s_idx].texture, GR_ALPHABLEND_FILTER,
...
void
beam_generate_muzzle_particles(beam *b)
{
    int particle_count;
    int idx;
    weapon_info *wip;
    ...
    particle_count = (int)frand_range(0.0f,
                                      (float)wip->b_info.beam_particle_count);
    ...
    for (idx = 0; idx < particle_count; idx++) {
        ...
        vm_vec_random_cone(&particle_dir, &turret_norm,
                           wip->b_info.beam_particle_angle, &m);
        p_temp = turret_pos;
        vm_vec_scale_add(&p_temp, &turret_pos, &particle_dir,
                         wip->b_info.beam_muzzle_radius *
                             frand_range(0.75f, 0.9f));
        ...
        float p_time_ref = wip->b_info.beam_life +
                           ((float)wip->b_info.beam_warmup / 1000.0f);
...
// render the muzzle glow for a beam weapon
void
beam_render_muzzle_glow(beam *b)
{
    vertex v;
    ...
    gr_set_bitmap(bwi->beam_glow_bitmap, GR_ALPHABLEND_FILTER,
                  GR_BITBLT_MODE_NORMAL, 0.8f * pct);
    // draw 1 bitmap
    g3_draw_bitmap(&v, 0, wip->b_info.beam_muzzle_radius * pct * rand_val,
                   TMAP_FLAG_TEXTURED);

    // maybe draw more
    if (pct > 0.3f) {
        g3_draw_bitmap(&v, 0,
                       wip->b_info.beam_muzzle_radius * pct * 0.75f * rand_val,
                       TMAP_FLAG_TEXTURED);
    }
    if (pct > 0.5f) {
        g3_draw_bitmap(&v, 0,
                       wip->b_info.beam_muzzle_radius * pct * 0.45f * rand_val,
                       TMAP_FLAG_TEXTURED);
    }
    if (pct > 0.7f) {
        g3_draw_bitmap(&v, 0,
                       wip->b_info.beam_muzzle_radius * pct * 0.25f * rand_val,
                       TMAP_FLAG_TEXTURED);
    }
}
...
beam_render_all()
{
    ...
    while (moveup != END_OF_LIST(&Beam_used_list)) {
        ...
        if ((moveup->warmup_stamp == -1) && (moveup->warmdown_stamp == -1) &&
            ...
            beam_render(&Weapon_info[moveup->weapon_info_index].b_info,
                        &moveup->last_start, &moveup->last_shot, moveup->shrink);
        }
...
// w == width of the diff between top and bottom around pos
void
beam_calc_facing_pts(vector *top, vector *bot, vector *fvec, vector *pos, float w,
                     float z_add)
{
    vector uvec, rvec;
    vector temp;
...
void
beam_add_light_small(beam *bm, object *objp, vector *pt_override = NULL)
{
    weapon_info *wip;
    beam_weapon_info *bwi;
    float noise;
    ...
    // some noise
    noise = frand_range(1.0f - bwi->sections[0].flicker,
                        1.0f + bwi->sections[0].flicker);
    ...
    if (pt_override == NULL) {
        float dist;
        vm_vec_dist_to_line(&objp->pos, &bm->last_start, &bm->last_shot, &near_pt,
                            &dist);
        if (dist > light_rad) {
            return;
        }
...
void
beam_add_light_large(beam *bm, object *objp, vector *pt0, vector *pt1)
{
    weapon_info *wip;
    beam_weapon_info *bwi;
    float noise;
    ...
    // add a unique light
    noise *=
        0.1f; // a little less noise here, since we want the beam to generally cast a bright light
...
// mark an object as being lit
void
beam_add_light(beam *b, int objnum, int source, vector *c_point)
{
    beam_light_info *l;
...
beam_apply_lighting()
{
    ...
    for (idx = 0; idx < Beam_light_count; idx++) {
        ...
        case 0:
            ...
            vm_vec_normalize_quick(&dir);
            vm_vec_scale_add(&pt, &l->bm->last_start, &dir,
                             bwi->beam_muzzle_radius * 5.0f);
        ...
        case 1:
            ...
            case OBJ_SHIP:
                ...
                // large ships
                if (Ship_info[Ships[Objects[l->objnum].instance].ship_info_index]
                        .flags &
                    (SIF_BIG_SHIP | SIF_HUGE_SHIP)) {
                    beam_add_light_large(l->bm, &Objects[l->objnum],
                                         &l->bm->last_start, &l->bm->last_shot);
                }
                // small ships
                else {
        ...
        case 2:
            ...
            vm_vec_normalize_quick(&dir);
            vm_vec_scale_add(&pt, &l->c_point, &dir,
                             bwi->beam_muzzle_radius * 5.0f);
...
beam_get_model(object *objp)
{
    ...
    default:
        ...
        mprintf(("Beam couldn't find a good find a good object model/type!! (%d)",
                 objp->type));
        return -1;
    }
...
beam_start_warmup(beam *b)
{
    ...
    b->warmup_stamp = timestamp(
        Weapon_info[b->weapon_info_index].b_info.beam_warmup);

    // start playing warmup sound
    if (Weapon_info[b->weapon_info_index].b_info.beam_warmup_sound >= 0) {
        snd_play_3d(
            &Snds[Weapon_info[b->weapon_info_index].b_info.beam_warmup_sound],
...
beam_start_firing(beam *b)
{
    ...
    if ((b->beam_sound_loop == -1) &&
        (Weapon_info[b->weapon_info_index].b_info.beam_loop_sound >= 0)) {
        b->beam_sound_loop = snd_play_3d(
            &Snds[Weapon_info[b->weapon_info_index].b_info.beam_loop_sound],
...
// start the warmdown phase for the beam
void
beam_start_warmdown(beam *b)
{
    // timestamp
    b->warmdown_stamp = timestamp(
        Weapon_info[b->weapon_info_index].b_info.beam_warmdown);

    // start the warmdown sound
    if (Weapon_info[b->weapon_info_index].b_info.beam_warmdown_sound >= 0) {
        snd_play_3d(
            &Snds[Weapon_info[b->weapon_info_index].b_info.beam_warmdown_sound],
            &b->last_start, &View_position);
    }
...
beam_recalc_sounds(beam *b)
{
    ...
    if (b->beam_sound_loop != -1) {
        ...
        switch (vm_vec_dist_to_line(&View_position, &b->last_start, &b->last_shot,
                                    &pos, NULL)) {
...
// fills in binfo
void
beam_get_binfo(beam *b, float accuracy, int num_shots)
{
    vector p2;
    int model_num, idx;
    vector oct1, oct2;
    ...
    ship_get_global_turret_gun_info(b->objp, b->subsys, &turret_point,
                                    &turret_norm, 1, &p2);
    ...
    case BEAM_TYPE_A:
        ...
        b->binfo.shot_aim[0] = frand_range(
            0.0f, 1.0f + bwi->beam_miss_factor[skill_level] * accuracy);
        b->binfo.shot_count = 1;

        // get random model points, this is useful for big ships, because we never miss when shooting at them
        submodel_get_two_random_points(model_num, 0, &b->binfo.dir_a,
                                       &b->binfo.dir_b);
        break;

    // just 2 points in the "slash"
    case BEAM_TYPE_B:
        beam_get_octant_points(model_num, b->target,
...
beam_aim(beam *b)
{
    ...
    case BEAM_TYPE_A:
        // where the shot is originating from (b->last_start gets filled in)
        ship_get_global_turret_gun_info(b->objp, b->subsys, &b->last_start, &temp,
                                        1, &p2);

        // if we're targeting a subsystem - shoot directly at it
        if (b->target_subsys != NULL) {
            // unrotate the center of the subsystem
            vm_vec_unrotate(&b->last_shot, &b->target_subsys->system_info->pnt,
                            &b->target->orient);
        ...
        if ((b->target->type == OBJ_SHIP) &&
            (Ship_info[Ships[b->target->instance].ship_info_index].flags &
             (SIF_BIG_SHIP | SIF_HUGE_SHIP))) {
    ...
    case BEAM_TYPE_B:
        ...
        // set the shot point
        vm_vec_scale_add(&b->last_shot, &b->last_start, &b->binfo.dir_a,
                         BEAM_FAR_LENGTH);
        Assert(is_valid_vec(&b->last_shot));
        break;
...
// given a model #, and an object, stuff 2 good world coord points
void
beam_get_octant_points(int modelnum, object *objp, int oct_index,
                       int oct_array[BEAM_NUM_GOOD_OCTANTS][4], vector *v1,
                       vector *v2)
{
    vector t1, t2, temp;
...
// throw some jitter into the aim - based upon shot_aim
void
beam_jitter_aim(beam *b, float aim)
{
    ...
    if ((b->objp != NULL) && (b->objp->signature == b->sig) &&
        ...
        subsys_strength = ship_get_subsystem_strength(&Ships[b->objp->instance],
                                                      SUBSYSTEM_WEAPONS);
    ...
    vm_vec_random_in_circle(&circle, &b->last_shot, &m, aim * b->target->radius,
                            0);
...
int
beam_collide_asteroid(obj_pair *pair)
{
    beam *b;
    mc_info test_collide;
    int model_num;
...
int
beam_collide_missile(obj_pair *pair)
{
    beam *b;
    mc_info test_collide;
    int model_num;
...
int
beam_collide_debris(obj_pair *pair)
{
    beam *b;
    mc_info test_collide;
    int model_num;
...
int
beam_collide_early_out(object *a, object *b)
{
    beam *bm;
    weapon_info *bwi;
    ...
    if ((vm_vec_dotprod(&dot_test, &dot_test2) < cull_dot) &&
...
// add a collision to the beam for this frame (to be evaluated later)
void
beam_add_collision(beam *b, object *hit_object, mc_info *cinfo)
{
    beam_collision *bc;
    int idx;
...
// sort collisions for the frame
int
beam_sort_collisions_func(const void *e1, const void *e2)
{
...
beam_handle_collisions(beam *b)
{
    ...
    qsort(b->f_collisions, b->f_collision_count, sizeof(beam_collision),
    ...
    for (idx = 0; idx < b->f_collision_count; idx++) {
        ...
        if (first_hit) {
            snd_play_3d(&Snds[wi->impact_snd],
        ...
        if (do_damage && !physics_paused) {
            ...
            if (wi->impact_weapon_expl_index >= 0) {
                int ani_handle = weapon_get_expl_handle(
                    wi->impact_weapon_expl_index,
                    &b->f_collisions[idx].cinfo.hit_point_world,
                    wi->impact_explosion_radius);
                particle_create(&b->f_collisions[idx].cinfo.hit_point_world,
                                &vmd_zero_vector, 0.0f,
                                wi->impact_explosion_radius,
                                PARTICLE_BITMAP_PERSISTENT, ani_handle);
            }

            switch (Objects[target].type) {
            case OBJ_DEBRIS:
                // hit the debris
                debris_hit(&Objects[target], &Objects[b->objnum],
                           &b->f_collisions[idx].cinfo.hit_point_world,
            ...
            case OBJ_WEAPON:
                ...
                Assert(
                    Weapon_info[Weapons[Objects[target].instance].weapon_info_index]
                        .subtype == WP_MISSILE);
            ...
            case OBJ_ASTEROID:
                // hit the asteroid
                asteroid_hit(&Objects[target], &Objects[b->objnum],
                             &b->f_collisions[idx].cinfo.hit_point_world,
                             Weapon_info[b->weapon_info_index].damage);
                break;
            ...
            case OBJ_SHIP:
                ...
                ship_apply_local_damage(
                    ...
                    &b->f_collisions[idx].cinfo.hit_point_world,
                    beam_get_ship_damage(b, &Objects[target]), -1);

                // if this is the first hit on the player ship. whack him
                if (do_damage) {
                    beam_apply_whack(b, &Objects[target],
                                     &b->f_collisions[idx].cinfo.hit_point_world);
                }
                break;
            }
...
// for a given object, and a firing beam, determine its critical dot product and range
void
beam_get_cull_vals(object *objp, beam *b, float *cull_dot, float *cull_dist)
{
    switch (objp->type) {
...
beam_ok_to_fire(beam *b)
{
    ...
    ship_get_global_turret_gun_info(b->objp, b->subsys, &turret_pos, &turret_dir,
                                    1, &temp);
    if (vm_vec_dotprod(&aim_dir, &turret_dir) <
...
// apply a whack to a ship
void
beam_apply_whack(beam *b, object *objp, vector *hit_point)
{
    weapon_info *wip;
    ship *shipp;
    ...
    Assert(b->weapon_info_index >= 0);
    wip = &Weapon_info[b->weapon_info_index];
    Assert((objp != NULL) && (objp->type == OBJ_SHIP) && (objp->instance >= 0) &&
...
// return the amount of damage which should be applied to a ship. basically, filters friendly fire damage
float
beam_get_ship_damage(beam *b, object *objp)
{
    ...
    Assert((objp->type == OBJ_SHIP) && (objp->instance >= 0) &&
           (objp->instance < MAX_SHIPS));
    ...
    if ((b->team == Ships[objp->instance].team) &&
        (Weapon_info[b->weapon_info_index].damage >
         Beam_friendly_cap[Game_skill_level])) {
...
// if the beam is likely to tool a given target before its lifetime expires
int
beam_will_tool_target(beam *b, object *objp)
{
    weapon_info *wip = &Weapon_info[b->weapon_info_index];
...
beam_test(int whee)
{
    ...
    while (lookup != END_OF_LIST(&Ships[s1].subsys_list)) {
        // turret
        if ((lookup->system_info->type == SUBSYSTEM_TURRET) &&
            !stricmp(lookup->system_info->subobj_name, "turret07")) {
            orion_turret = lookup;
        }
...
beam_test_new(int whee)
{
    ...
    while (lookup != END_OF_LIST(&Ships[s2].subsys_list)) {
        // turret
        if ((lookup->system_info->type == SUBSYSTEM_TURRET) &&
            !stricmp(lookup->system_info->subobj_name, "turret03")) {
            fenris_turret = lookup;
        }
