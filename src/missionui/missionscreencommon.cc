...
#include <math/staticrand.hh> // for rand_alt()
...
void
common_buttons_maybe_reload(UI_WINDOW *ui_window)
{
    UI_BUTTON *b;
    int i;
...
void
common_buttons_init(UI_WINDOW *ui_window)
{
    UI_BUTTON *b;
    int i;
    ...
    for (i = 0; i < NUM_COMMON_BUTTONS; i++) {
        ...
        b->create(ui_window, "",
                  ...
                  Common_buttons[Current_screen - 1][gr_screen.res][i].y, 60, 30,
                  0, 1);
    ...
    ui_window->add_XSTR(
        "Briefing", 1504,
        Common_buttons[Current_screen - 1][gr_screen.res][COMMON_BRIEFING_BUTTON]
            .xt,
        Common_buttons[Current_screen - 1][gr_screen.res][COMMON_BRIEFING_BUTTON]
            .yt,
        &Common_buttons[Current_screen - 1][gr_screen.res][COMMON_BRIEFING_BUTTON]
    ...
    ui_window->add_XSTR(
        "Weapon Loadout", 1068,
        Common_buttons[Current_screen - 1][gr_screen.res][COMMON_WEAPON_BUTTON].xt,
        Common_buttons[Current_screen - 1][gr_screen.res][COMMON_WEAPON_BUTTON].yt,
        &Common_buttons[Current_screen - 1][gr_screen.res][COMMON_WEAPON_BUTTON]
             .button,
        UI_XSTR_COLOR_GREEN);
    ui_window->add_XSTR(
        "Commit", 1062,
        Common_buttons[Current_screen - 1][gr_screen.res][COMMON_COMMIT_BUTTON].xt,
    ...
    ui_window->add_XSTR(
        "Help", 928,
        Common_buttons[Current_screen - 1][gr_screen.res][COMMON_HELP_BUTTON].xt,
...
void
set_active_ui(UI_WINDOW *ui_window)
{
    Active_ui_window = ui_window;
}

void
common_music_init(int score_index)
{
...
common_select_init()
{
    if (Common_select_inited) {
        nprintf(
            ("Alan", "common_select_init() returning without doing anything\n"));
        return;
    }
...
common_reset_buttons()
{
    ...
    case ON_BRIEFING_SELECT:
        Common_buttons[Current_screen - 1][gr_screen.res][COMMON_BRIEFING_REGION]
...
int
common_select_do(float frametime)
{
    int k, new_k;
    ...
    if (help_overlay_active(BR_OVERLAY) || help_overlay_active(SS_OVERLAY) ||
        help_overlay_active(WL_OVERLAY)) {
...
common_select_close()
{
    if (!Common_select_inited) {
        nprintf(
            ("Alan", "common_select_close() returning without doing anything\n"));
        return;
    }
            ...
            for (j = 0; j < MAX_WING_SLOTS; j++) {
                ...
                if (shipp->ship_info_index != slot->ship_class) {
                    ...
                    if (slot->ship_class == -1) {
                        ...
                        hud_set_wingman_status_none(shipp->wing_status_wing_index,
...
int
wss_get_mode(int from_slot, int from_list, int to_slot, int to_list,
             int wl_ship_slot)
{
