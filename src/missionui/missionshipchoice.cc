...
static int Ss_delta_x,
    Ss_delta_y; // used to offset the carried icon to make it smoothly leave static position
...
int Wing_icon_coords[GR_NUM_RESOLUTIONS][MAX_WSS_SLOTS][2] = { { { 124, 345 },
                                                                 { 100, 376 },
                                                                 { 148, 376 },
                                                                 { 124, 407 },
...
struct ss_buttons
{
    ...
    ss_buttons(const char *name, int x1, int y1, int xt1, int yt1, int h, int s)
        : filename(name)
        , x(x1)
        , y(y1)
        , xt(xt1)
...
void
ss_set_carried_icon(int from_slot, int ship_class)
{
...
void
active_list_add(int ship_class)
{
    ss_active_item *sai;
...
void
active_list_remove(int ship_class)
{
...
ship_select_buttons_init()
{
    ...
    for (i = 0; i < NUM_SS_BUTTONS; i++) {
        b = &Ship_select_buttons[gr_screen.res][i];
        b->button.create(&Ship_select_ui_window, "", b->x, b->y, 60, 30,
                         b->scrollable);
...
ship_select_button_do(int i)
{
    ...
    case SHIP_SELECT_SHIP_SCROLL_UP:
        ...
        if (common_scroll_down_pressed(&SS_active_list_start, SS_active_list_size,
                                       MAX_ICONS_ON_SCREEN)) {
    ...
    case SHIP_SELECT_SHIP_SCROLL_DOWN:
        ...
        if (common_scroll_up_pressed(&SS_active_list_start, SS_active_list_size,
                                     MAX_ICONS_ON_SCREEN)) {
            gamesnd_play_iface(SND_SCROLL);
        }
        else {
...
ship_select_init()
{
    ...
    snazzy_menu_add_region(&Region[Num_mask_regions++], "",
                           COMMON_BRIEFING_REGION, 0);
    snazzy_menu_add_region(&Region[Num_mask_regions++], "", COMMON_SS_REGION, 0);
    snazzy_menu_add_region(&Region[Num_mask_regions++], "", COMMON_WEAPON_REGION,
                           0);
    snazzy_menu_add_region(&Region[Num_mask_regions++], "", COMMON_COMMIT_REGION,
                           0);
    snazzy_menu_add_region(&Region[Num_mask_regions++], "", COMMON_HELP_REGION,
                           0);
    snazzy_menu_add_region(&Region[Num_mask_regions++], "", COMMON_OPTIONS_REGION,
                           0);

    snazzy_menu_add_region(&Region[Num_mask_regions++], "",
                           SHIP_SELECT_SHIP_SCROLL_UP, 0);
    snazzy_menu_add_region(&Region[Num_mask_regions++], "",
                           SHIP_SELECT_SHIP_SCROLL_DOWN, 0);

    snazzy_menu_add_region(&Region[Num_mask_regions++], "", SHIP_SELECT_ICON_0,
                           0);
    snazzy_menu_add_region(&Region[Num_mask_regions++], "", SHIP_SELECT_ICON_1,
                           0);
    snazzy_menu_add_region(&Region[Num_mask_regions++], "", SHIP_SELECT_ICON_2,
                           0);
    snazzy_menu_add_region(&Region[Num_mask_regions++], "", SHIP_SELECT_ICON_3,
                           0);
    ...
    Ship_select_background_bitmap = bm_load(
...
//
int
ss_get_ship_class_from_list(int index)
{
    ss_active_item *sai;
    ...
    count = 0;
    list_entry = -1;
    for (sai = GET_FIRST(&SS_active_head); sai != END_OF_LIST(&SS_active_head);
         sai = GET_NEXT(sai)) {
        count++;
...
//
void
maybe_pick_up_list_icon(int offset)
{
    int ship_class;
...
//
void
maybe_change_selected_ship(int offset)
{
    int ship_class;
...
void
maybe_change_selected_wing_ship(int wb_num, int ws_num)
{
...
do_mouse_over_wing_slot(int block, int slot)
{
    ...
    if (!mouse_down(MOUSE_LEFT_BUTTON)) {
        if (ss_icon_being_carried()) {
            ...
            ss_drop(Carried_ss_icon.from_slot, Carried_ss_icon.ship_class,
                    Hot_ss_slot, -1);
            ss_reset_carried_icon();
        }
    }
...
void
do_mouse_over_list_slot(int index)
{
    Hot_ss_icon = index;
    ...
    if (Drop_icon_mflag) {
        if (ss_icon_being_carried()) {
            ...
            if (Carried_ss_icon.from_slot >= 0) {
                // return to list
                ss_drop(Carried_ss_icon.from_slot, -1, -1,
...
ship_select_blit_ship_info()
{
    ...
    gr_string(Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD], y_start,
              XSTR("Class", 739));
    y_start += 10;
    if (strlen(sip->name)) {
        gr_set_color_fast(text);
        gr_string(Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD] + 4,
                  y_start, sip->name);
    }
    y_start += 10;
...
ship_select_do(float frametime)
{
    ...
    ship_select_choice = snazzy_menu_do(ShipSelectMaskData, Shipselect_mask_w,
                                        Shipselect_mask_h, Num_mask_regions,
    ...
    if (Ship_anim_class >= 0) {
        ...
        if (Ss_icons[Selected_ss_class].anim_instance->frame_num ==
            Ss_icons[Selected_ss_class].anim_instance->stop_at) {
            nprintf(("anim", "Frame number = %d, Stop at %d\n",
                     Ss_icons[Selected_ss_class].anim_instance->frame_num,
                     Ss_icons[Selected_ss_class].anim_instance->stop_at));
            anim_play_struct aps;
            anim_release_render_instance(
                Ss_icons[Selected_ss_class].anim_instance);
            anim_play_init(&aps, Ss_icons[Selected_ss_class].anim,
    ...
    if (ss_icon_being_carried()) {
        ...
        gr_set_bitmap(
            Ss_icons[Carried_ss_icon.ship_class].icon_bmaps[ICON_FRAME_SELECTED]);
...
//
void
draw_ship_icon_with_number(int screen_offset, int ship_class)
{
    char buf[32];
    int num_x, num_y;
...
anim *
ss_load_individual_animation(int ship_class)
{
    anim *p_anim;
...
start_ship_animation(int ship_class, int play_sound)
{
    ...
    if (ss_icon->anim_instance == NULL) {
        ...
        anim_play_init(&aps, ss_icon->anim, Ship_anim_coords[gr_screen.res][0],
                       Ship_anim_coords[gr_screen.res][1]);
        aps.screen_id = ON_SHIP_SELECT;
        aps.framerate_independent = 1;
        aps.skip_frames = 0;
...
commit_pressed()
{
    ...
    if (Wss_num_wings > 0) {
        ...
        if (wl_update_ship_weapons(Ships[Player_obj->instance].objnum,
                                   &Wss_slots[0]) == -1) {
...
int
pick_from_ship_list(int screen_offset, int ship_class)
{
    int rval = -1;
    Assert(ship_class >= 0);
...
// mouse in a realistic way
void
pick_from_wing(int wb_num, int ws_num)
{
    int slot_index;
    Assert(wb_num >= 0 && wb_num < MAX_WING_BLOCKS);
    Assert(ws_num >= 0 && ws_num < MAX_WING_SLOTS);
...
draw_wing_block(int wb_num, int hot_slot, int selected_slot, int class_select)
{
    ...
    for (i = 0; i < MAX_WING_SLOTS; i++) {
        ...
        if (bitmap_to_draw != -1) {
            ...
            gr_bitmap(Wing_icon_coords[gr_screen.res][slot_index][0],
    ...
    for (i = 0; i < MAX_WING_BLOCKS; i++) {
        ...
        for (j = 0; j < MAX_WING_SLOTS; j++) {
                    ...
                    if (wb->is_late) {
                        ...
                        if (Ships[wp->ship_index[j]].ship_info_index !=
                            ...
                            change_ship_type(wp->ship_index[j],
                                             Wss_slots[slot_index].ship_class);
                        wl_update_ship_weapons(Ships[wp->ship_index[j]].objnum,
                                               &Wss_slots[i * 4 + j]);
                    }
                }
            ...
            case WING_SLOT_EMPTY | WING_SLOT_IS_PLAYER:
                if (ws->status & WING_SLOT_IS_PLAYER) {
                    popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                          XSTR("Player %s must select a place in player wing",
                               462),
                          Player->callsign);
                    return -1;
                }
...
//
void
update_player_ship(int si_index)
{
    Assert(si_index >= 0);
...
//
int
create_default_player_ship(int use_last_flown)
{
...
int
ss_return_original_ship_class(int slot_num)
{
    int wnum, snum;
...
int
ss_return_saindex(int slot_num)
{
    int wnum, snum;
...
//
int
ss_return_ship(int wing_block, int wing_slot, int *ship_index, p_object **ppobjp)
{
    *ship_index = -1;
    *ppobjp = NULL;
...
void
ss_return_name(int wing_block, int wing_slot, char *name)
{
    ss_slot_info *ws;
    wing *wp;
...
int
ss_fixup_team_data(team_data *tdata)
{
...
ss_load_icons(int ship_class)
{
    ...
    first_frame = bm_load_animation(Ship_info[ship_class].icon_filename,
                                    &num_frames);
    if (first_frame == -1) {
...
int
ss_disabled_slot(int slot_num)
{
    if (Wss_num_wings <= 0) {
        return 0;
    }

    return (Ss_wings[slot_num / 4].ss_slots[slot_num % 4].status &
            WING_SLOT_IGNORE);
}
...
void
ss_init_wing_info(int wing_num, int starting_wing_num)
{
    wing *wp;
...
int
ss_wing_slot_is_console_player(int index)
{
    int wingnum, slotnum;
...
void
ship_select_init_team_data(int team_num)
{
    int idx;
...
// exit: data changed flag
int
ss_grab_from_list(int from_list, int to_slot, int *sound)
{
    wss_unit *slot;
...
// exit: data changed flag
int
ss_swap_list_slot(int from_list, int to_slot, int *sound)
{
    int i, wep[MAX_WL_WEAPONS], wep_count[MAX_WL_WEAPONS];
    wss_unit *slot;
...
void
ss_apply(int mode, int from_slot, int from_list, int to_slot, int to_list,
         int player_index)
{
    int update = 0;
    int sound = -1;
...
void
ss_drop(int from_slot, int from_list, int to_slot, int to_list, int player_index)
{
    int mode;
    common_flash_button_init();
