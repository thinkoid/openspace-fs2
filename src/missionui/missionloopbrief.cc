...
// loop brief anim
int Loop_brief_anim_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                        24, 267 },
                                                      { // GR_640
                                                        167, 491 } };
...
// button press
void
loop_brief_button_pressed(int i)
{
    switch (i) {
    ...
    case LOOP_BRIEF_ACCEPT:
        ...
        Campaign.loop_reentry =
            Campaign.next_mission; // save reentry pt, so we can break out of loop
...
loop_brief_init()
{
    ...
    if (Campaign.missions[Campaign.current_mission].mission_loop_brief_anim !=
        NULL) {
        Loop_anim = anim_load(
            Campaign.missions[Campaign.current_mission].mission_loop_brief_anim);
    }
    else {
    ...
    // open sound
    if (Campaign.missions[Campaign.current_mission].mission_loop_brief_sound !=
        NULL) {
        Loop_sound = audiostream_open(
            Campaign.missions[Campaign.current_mission].mission_loop_brief_sound,
            ASF_VOICE);
...
loop_brief_do()
{
    ...
    brief_render_text(0, Loop_brief_text_coords[gr_screen.res][0],
