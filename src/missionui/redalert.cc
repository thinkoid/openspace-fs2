...
static int
    Red_alert_new_mission_timestamp; // timestamp used to give user a little warning for red alerts
...
int Ra_brief_text_wnd_coords[GR_NUM_RESOLUTIONS][4] = { { 14, 151, 522, 289 },
                                                        { 52, 241, 785, 463 } };
...
red_alert_voice_load()
{
    ...
    if (strnicmp(Briefing->stages[0].voice, NOX("none"), 4) &&
        (strlen(Briefing->stages[0].voice) > 0)) {
...
void
red_alert_button_pressed(int n)
{
    switch (n) {
    case RA_CONTINUE:
...
red_alert_blit_title()
{
    ...
    color flash_color;
    if (Ra_flash_up) {
        gr_init_alphacolor(&flash_color,
                           (int)(255.0f * (Ra_flash_time / RA_FLASH_CYCLE)), 0, 0,
                           255);
    }
    else {
        gr_init_alphacolor(
            &flash_color,
            (int)(255.0f * (1.0f - (Ra_flash_time / RA_FLASH_CYCLE))), 0, 0, 255);
    }

    // draw
    gr_set_color_fast(&flash_color);
    gr_string(Ra_brief_text_wnd_coords[gr_screen.res][0] +
...
red_alert_init()
{
    ...
    if (Briefing->num_stages > 0) {
        ...
        brief_color_text_init(Briefing->stages[0].new_text,
...
void
red_alert_do_frame(float frametime)
{
    int i, k;
...
void
red_alert_set_status(int status)
{
...
void
red_alert_store_weapons(red_alert_ship_status *ras, ship_weapon *swp)
{
    int i, sidx;
...
void
red_alert_bash_weapons(red_alert_ship_status *ras, ship_weapon *swp)
{
    int i, j, sidx;
...
void
red_alert_bash_subsys_status(red_alert_ship_status *ras, ship *shipp)
{
    ship_subsys *ss;
    int count = 0;
...
void
red_alert_store_subsys_status(red_alert_ship_status *ras, ship *shipp)
{
    ship_subsys *ss;
    int count = 0;
...
red_alert_store_wingman_status()
{
    ...
    for (int idx = 0; idx < MAX_EXITED_SHIPS; idx++) {
        ...
        if (Ships_exited[idx].flags & SEF_RED_ALERT_CARRY) {
            ...
            ras->ship_class =
                RED_ALERT_EXITED_SHIP_CLASS; //shipp->ship_info_index;
...
void
red_alert_delete_ship(ship *shipp)
{
...
void
red_alert_write_wingman_status(CFILE *fp)
{
    int i, j;
    red_alert_ship_status *ras;
...
void
red_alert_read_wingman_status(CFILE *fp, int version)
{
    int i, j;
    red_alert_ship_status *ras;
