...
UI_XSTR Cmd_brief_text[GR_NUM_RESOLUTIONS][CMD_BRIEF_NUM_TEXT] = {
    {
        ...
        { "Help", 928, 500, 440, UI_XSTR_COLOR_GREEN, -1,
          &Cmd_brief_buttons[0][HELP_BUTTON].button },
        { "Options", 1036, 479, 464, UI_XSTR_COLOR_GREEN, -1,
          &Cmd_brief_buttons[0][OPTIONS_BUTTON].button },
        { "Continue", 1069, 564, 413, UI_XSTR_COLOR_PINK, -1,
          &Cmd_brief_buttons[0][ACCEPT_BUTTON].button },
    },
    {
        // GR_1024
        { "Help", 928, 800, 704, UI_XSTR_COLOR_GREEN, -1,
          &Cmd_brief_buttons[1][HELP_BUTTON].button },
        { "Options", 1036, 797, 743, UI_XSTR_COLOR_GREEN, -1,
          &Cmd_brief_buttons[1][OPTIONS_BUTTON].button },
        { "Continue", 1069, 917, 661, UI_XSTR_COLOR_PINK, -1,
          &Cmd_brief_buttons[1][ACCEPT_BUTTON].button },
    }
};
...
cmd_brief_check_stage_done()
{
    ...
    if (!Voice_ended_time)
        Voice_ended_time = Voice_started_time +
                           max(5000, Num_brief_text_lines[0] * 3500);
...
void
cmd_brief_voice_play(int stage_num)
{
    int voice = -1;
...
cmd_brief_new_stage(int stage)
{
    ...
    Cur_stage = stage;
    brief_color_text_init(Cur_cmd_brief->stage[stage].text,
                          Cmd_text_wnd_coords[gr_screen.res][CMD_W_COORD]);
    ...
    if (i != Anim_playing_id) {
        if (Cur_cmd_brief->stage[i].anim) {
            anim_play_init(&aps, Cur_cmd_brief->stage[i].anim,
                           Cmd_image_wnd_coords[gr_screen.res][CMD_X_COORD],
                           Cmd_image_wnd_coords[gr_screen.res][CMD_Y_COORD]);
            aps.looped = 1;
...
void
cmd_brief_button_pressed(int n)
{
    switch (n) {
    case HELP_BUTTON:
        launch_context_help();
...
cmd_brief_ani_wave_init(int index)
{
    ...
    for (i = 0; i < index; i++) {
        if (!stricmp(Cur_cmd_brief->stage[i].ani_filename,
                     Cur_cmd_brief->stage[index].ani_filename)) {
            if (Cur_cmd_brief->stage[i].anim_ref >= 0)
                Cur_cmd_brief->stage[index].anim_ref =
                    Cur_cmd_brief->stage[i].anim_ref;
            else
...
cmd_brief_init(int team)
{
   ...
   */
    ...
    Cmd_brief_buttons[gr_screen.res][FIRST_STAGE_BUTTON].button.set_hotkey(
        KEY_SHIFTED | KEY_LEFT);
    Cmd_brief_buttons[gr_screen.res][LAST_STAGE_BUTTON].button.set_hotkey(
        KEY_SHIFTED | KEY_RIGHT);
    Cmd_brief_buttons[gr_screen.res][PREV_STAGE_BUTTON].button.set_hotkey(
        KEY_LEFT);
    Cmd_brief_buttons[gr_screen.res][NEXT_STAGE_BUTTON].button.set_hotkey(
        KEY_RIGHT);
    Cmd_brief_buttons[gr_screen.res][ACCEPT_BUTTON].button.set_hotkey(KEY_CTRLED |
                                                                      KEY_ENTER);
...
void
cmd_brief_do_frame(float frametime)
{
    char buf[40];
    int i, k, w;
    ...
    sprintf(buf, XSTR("Stage %d of %d", 464), Cur_stage + 1,
            Cur_cmd_brief->num_stages);
    gr_get_string_size(&w, NULL, buf);
    gr_string(Cmd_text_wnd_coords[gr_screen.res][CMD_X_COORD] +
                  Cmd_text_wnd_coords[gr_screen.res][CMD_W_COORD] - w,
              Cmd_stage_y[gr_screen.res], buf);

    if (brief_render_text(
            Scroll_offset, Cmd_text_wnd_coords[gr_screen.res][CMD_X_COORD],
            Cmd_text_wnd_coords[gr_screen.res][CMD_Y_COORD],
            Cmd_text_wnd_coords[gr_screen.res][CMD_H_COORD], frametime, 0, 1)) {
        Voice_good_to_go = 1;
    }
