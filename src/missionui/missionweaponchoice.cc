...
static int Wl_weapon_icon_coords[GR_NUM_RESOLUTIONS][MAX_WEAPON_ICONS_ON_SCREEN]
                                [2] = { { { 27, 152 },
                                          { 27, 182 },
                                          { 27, 212 },
                                          { 27, 242 },
...
static char Weapon_desc_lines
    ...
    [WEAPON_DESC_MAX_LENGTH]; // 1st 2 lines are title, rest are desc
...
// ship select text
#define WEAPON_SELECT_NUM_TEXT 1
...
int
wl_fury_missile_offset_hack(int weapon_class, int num_missiles)
{
    if (weapon_class < 0) {
        return 0;
    }
...
void
wl_set_carried_icon(int from_bank, int from_slot, int weapon_class)
{
    int mx, my;
...
int
wl_get_pilot_subsys_index(p_object *pobjp)
{
...
weapon_buttons_init()
{
    ...
    for (i = 0; i < NUM_WEAPON_BUTTONS; i++) {
        b = &Buttons[gr_screen.res][i];
        b->button.create(&Weapon_ui_window, "", Buttons[gr_screen.res][i].x,
...
//
void
wl_render_overhead_view(float frametime)
{
    ...
    if (wl_ship->anim_instance == NULL) {
        if (wl_ship->overhead_bitmap < 0) {
            ...
            if (gr_screen.res == GR_640) {
                // lo-res
                wl_ship->overhead_bitmap = bm_load(
                    Ship_info[ship_class].overhead_filename);
            }
            else {
                // high-res
        ...
        gr_bitmap(Wl_overhead_coords[gr_screen.res][0],
                  Wl_overhead_coords[gr_screen.res][1]);
    }

    ss_return_name(Selected_wl_slot / 4, Selected_wl_slot % 4, name);
    gr_set_color_fast(&Color_normal);
    gr_string(Wl_ship_name_coords[gr_screen.res][0],
              Wl_ship_name_coords[gr_screen.res][1], name);
}
...
void
wl_set_disabled_weapons(int ship_class)
{
    int i;
    ship_info *sip;
...
void
maybe_select_wl_slot(int block, int slot)
{
    int sidx;
...
void
wl_load_icons(int weapon_class)
{
    wl_icon_info *icon;
    ...
    first_frame = bm_load_animation(Weapon_info[weapon_class].icon_filename,
                                    &num_frames);
    if (first_frame == -1) {
...
wl_load_anim(int weapon_class)
{
    ...
    if (gr_screen.res == GR_1024) {
        ...
        if (icon->anim == NULL) {
            mprintf(
                ("Weapon ANI: Can not find %s, using lowres version instead.\n",
                 animation_filename));
...
void
wl_set_selected_slot(int slot_num)
{
...
int
wl_calc_missile_fit(int wi_index, int capacity)
{
    if (wi_index < 0) {
        return 0;
    }
...
void
wl_get_ship_class_weapons(int ship_class, int *wep, int *wep_count)
{
    ship_info *sip;
    int i;
    ...
    for (i = 0; i < MAX_WL_WEAPONS; i++) {
        wep[i] = -1;
        wep_count[i] =
            -1; // -1 means weapon bank doesn't exist.. 0 just means it is empty
    }
    ...
    for (i = 0; i < sip->num_secondary_banks; i++) {
        ...
        wep_count[i + MAX_WL_PRIMARY] = wl_calc_missile_fit(
            sip->secondary_bank_weapons[i], sip->secondary_bank_ammo_capacity[i]);
    }
}

// fill out the wep[] and wep_count[] arrays for a ship
void
wl_get_ship_weapons(int ship_index, int *wep, int *wep_count)
{
    int i;
    wing *wp;
    ship_weapon *swp;
...
void
wl_get_parseobj_weapons(int sa_index, int ship_class, int *wep, int *wep_count)
{
    int i, pilot_index;
    subsys_status *ss;
    ship_info *sip;
...
void
wl_cull_illegal_weapons(int ship_class, int *wep, int *wep_count)
{
    int i;
    for (i = 0; i < MAX_WL_WEAPONS; i++) {
        if (wep[i] < 0) {
...
void
wl_get_default_weapons(int ship_class, int slot_num, int *wep, int *wep_count)
{
...
void
wl_add_index_to_list(int wi_index)
{
    int i;
...
void
wl_remove_weps_from_pool(int *wep, int *wep_count, int ship_class)
{
    int i, wi_index;
                    ...
                    if ((Wl_pool[wi_index] <= 0) || (wep_count[i] == 0)) {
                        ...
                        for (wep_pool_index = 0;
                            ...
                            for (wep_precedence_index = 0;
                                 ...
                                 wep_precedence_index++) {
                                if (wep_pool_index ==
                                    Player_weapon_precedence[wep_precedence_index]) {
...
void
weapon_select_init_team(int team_num)
{
...
weapon_select_init()
{
    ...
    if (Weapon_select_open) {
        ...
        nprintf(
            ("Alan", "weapon_select_init() returning without doing anything\n"));
    ...
    bm_get_info(WeaponSelectMaskBitmap, &Weaponselect_mask_w,
                &Weaponselect_mask_h);
    ...
    snazzy_menu_add_region(&Weapon_select_region[Num_weapon_select_regions++], "",
    ...
    Weapon_select_background_bitmap = bm_load(
        Weapon_select_background_fname[gr_screen.res]);
...
wl_dump_carried_icon()
{
    if (wl_icon_being_carried()) {
        ...
        if (Carried_wl_icon.from_bank >= 0) {
            // return to list
            wl_drop(Carried_wl_icon.from_bank, -1, -1,
...
//
int
drop_icon_on_slot(int bank_num)
{
    if (Selected_wl_slot == -1) {
        return 0;
    }
    ...
    wl_drop(Carried_wl_icon.from_bank, Carried_wl_icon.weapon_class, bank_num, -1,
            Selected_wl_slot);
    return 1;
}
...
//
void
maybe_drop_icon_on_slot(int bank_num)
{
    int dropped = 0;
...
//
void
do_mouse_over_list_weapon(int index)
{
...
void
weapon_select_render(float frametime)
{
...
// this wipes in
void
wl_render_weapon_desc(float frametime)
{
    ...
    if (!Weapon_desc_wipe_done) {
        ...
        int bright_char_index = (int)(Weapon_desc_wipe_time_elapsed *
                                      WEAPON_DESC_MAX_LENGTH /
        ...
        for (i = 0; i < 2; i++) {
            ...
            if (bright_char_index < curr_len) {
                ...
                gr_string(weapon_title_coords[0],
                          weapon_title_coords[1] + (10 * i),
                          Weapon_desc_lines[i]);

                // draw the bright letters
                gr_set_color_fast(&Color_bright_white);
                gr_get_string_size(&w, &h, Weapon_desc_lines[i], curr_len);
                gr_printf(weapon_title_coords[0] + w,
                          weapon_title_coords[1] + (10 * i), "%c",
                          bright_char[i]);
        ...
        for (i = 2; i < WEAPON_DESC_MAX_LINES; i++) {
            ...
            if (bright_char_index < curr_len) {
                ...
                gr_string(weapon_desc_coords[0],
                          weapon_desc_coords[1] + (10 * (i - 2)),
                          Weapon_desc_lines[i]);

                // draw the bright letters
                gr_set_color_fast(&Color_bright_white);
                gr_get_string_size(&w, &h, Weapon_desc_lines[i], curr_len);
                gr_printf(weapon_desc_coords[0] + w,
                          weapon_desc_coords[1] + (10 * (i - 2)), "%c",
                          bright_char[i]);
        ...
        gr_set_color_fast(&Color_white);
        gr_string(weapon_title_coords[0], weapon_title_coords[1],
                  Weapon_desc_lines[0]);
        gr_string(weapon_title_coords[0], weapon_title_coords[1] + 10,
                  Weapon_desc_lines[1]);
        gr_string(weapon_desc_coords[0], weapon_desc_coords[1],
                  Weapon_desc_lines[2]);
        gr_string(weapon_desc_coords[0], weapon_desc_coords[1] + 10,
                  Weapon_desc_lines[3]);
        gr_string(weapon_desc_coords[0], weapon_desc_coords[1] + 20,
                  Weapon_desc_lines[4]);
        gr_string(weapon_desc_coords[0], weapon_desc_coords[1] + 30,
                  Weapon_desc_lines[5]);
    }
}
...
wl_weapon_desc_start_wipe()
{
    ...
    if (w > Weapon_title_max_width[gr_screen.res]) {
        ...
        currchar_src = (int)(((float)title_len / (float)w) *
                             Weapon_title_max_width
                                 [gr_screen.res]); // char to start space search at
        ...
        strcpy(Weapon_desc_lines[1],
               &(Weapon_desc_lines[0][currchar_src +
                                      1])); // copy remainder into line 1
    }
    else {
...
//
void
weapon_select_do(float frametime)
{
    ...
    wl_choice = snazzy_menu_do(WeaponSelectMaskData, Weaponselect_mask_w,
                               Weaponselect_mask_h, Num_weapon_select_regions,
                               Weapon_select_region, &snazzy_action, 0);
    ...
    if (Weapon_anim_class != -1 && (Selected_wl_class == Weapon_anim_class)) {
        ...
        if (icon->anim_instance) {
            if (icon->anim_instance->frame_num == icon->anim_instance->stop_at) {
                ...
                anim_play_init(&aps, icon->anim, weapon_ani_coords[0],
                               weapon_ani_coords[1]);
    ...
    if (wl_icon_being_carried()) {
        ...
        if (Wl_icons[Carried_wl_icon.weapon_class].can_use > 0) {
            ...
            gr_set_bitmap(Wl_icons[Carried_wl_icon.weapon_class]
                              .icon_bmaps[WEAPON_ICON_FRAME_SELECTED]);
            gr_bitmap(sx, sy);
        }
        ...
        if (Carried_wl_icon.from_bank >= MAX_WL_PRIMARY) {
            if (mx == Carried_wl_icon.from_x && my == Carried_wl_icon.from_y) {
                ...
                wl_render_icon_count(
                    num_missiles,
                    Wl_bank_coords[gr_screen.res][Carried_wl_icon.from_bank][0],
            ...
            if ((diffx > 2) || (diffy > 2)) {
                ...
                if (Lcl_gr) {
                    ...
                    strncpy(display_name,
                            Weapon_info[Carried_wl_icon.weapon_class].name, 128);
                    ...
                    popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                          ...
                          Weapon_info[Carried_wl_icon.weapon_class].name);
                }
                wl_unpause_anim();
                wl_dump_carried_icon();
            }
...
weapon_select_close()
{
    if (!Weapon_select_open) {
        nprintf(
            ("Alan", "weapon_select_close() returning without doing anything\n"));
        return;
    }
               ...
               int hot_bank_mask, int select_mask)
{
    ...
    if (Selected_wl_class > -1) {
        if (Selected_wl_class == select_mask)
            bitmap_id =
                icon->icon_bmaps[WEAPON_ICON_FRAME_SELECTED]; // selected icon
    }
...
wl_draw_ship_weapons(int index)
{
    ...
    for (i = 0; i < MAX_WL_WEAPONS; i++) {
        ...
        if ((wep[i] != -1) && (wep_count[i] > 0)) {
            ...
            x_offset = 0;
            wl_render_icon(wep[i], Wl_bank_coords[gr_screen.res][i][0] + x_offset,
...
draw_wl_icon_with_number(int list_count, int weapon_class)
{
    ...
    wl_render_icon(weapon_class,
                   Wl_weapon_icon_coords[gr_screen.res][list_count][0],
...
// input: index (0..7)
void
wl_pick_icon_from_list(int index)
{
...
int
wl_slots_all_empty(wss_unit *slot)
{
    int i;
...
wl_update_parse_object_weapons(p_object *pobjp, wss_unit *slot)
{
    ...
    for (i = 0; i < MAX_WL_SECONDARY; i++) {
        ...
        if ((slot->wep_count[sidx] > 0) && (slot->wep[sidx] >= 0)) {
            ...
            ss->secondary_ammo[j] = fl2i(
                i2fl(slot->wep_count[sidx]) / max_count * 100.0f + 0.5f);
            j++;
        }
    }
}
...
//
void
start_weapon_animation(int weapon_class)
{
    wl_icon_info *icon;
    int *weapon_ani_coords;
...
void
wl_bash_ship_weapons(ship_weapon *swp, wss_unit *slot)
{
    int i, j, sidx;
...
void
wl_swap_weapons(int ship_slot, int from_bank, int to_bank)
{
    wss_unit *slot;
    int tmp;
...
void
wl_saturate_bank(int ship_slot, int bank)
{
    wss_unit *slot;
...
wl_swap_slot_slot(int from_bank, int to_bank, int ship_slot, int *sound)
{
    ...
    if ((IS_BANK_PRIMARY(from_bank) && IS_BANK_SECONDARY(to_bank)) ||
        (IS_BANK_SECONDARY(from_bank) && IS_BANK_PRIMARY(to_bank))) {
        // put from_bank back into list
        Wl_pool[slot->wep[from_bank]] += slot->wep_count[from_bank];
        ; // return to list
...
wl_grab_from_list(int from_list, int to_bank, int ship_slot, int *sound)
{
    ...
    if (to_bank < MAX_WL_PRIMARY) {
        ...
        max_fit = wl_calc_missile_fit(
            from_list,
            Ship_info[slot->ship_class]
                .secondary_bank_ammo_capacity[to_bank - MAX_WL_PRIMARY]);
    }
...
void
wl_apply(int mode, int from_bank, int from_list, int to_bank, int to_list,
         int ship_slot, int player_index)
{
    int update = 0;
    int sound = -1;
...
void
wl_drop(int from_bank, int from_list, int to_bank, int to_list, int ship_slot,
        int player_index)
{
    int mode;
    ...
    if (mode >= 0) {
        wl_apply(mode, from_bank, from_list, to_bank, to_list, ship_slot,
                 player_index);
    }
}
