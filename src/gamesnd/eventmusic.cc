...
#include <mission/missionload.hh> /* for Builtin_mission_names[] */
...
float Master_event_music_volume =
    DEFAULT_MASTER_EVENT_MUSIC_VOLUME; // range is 0->1
...
event_music_force_switch()
{
    ...
    if ((new_pattern == SONG_BTTL_3) && (Patterns[SONG_BTTL_3].handle == -1)) {
        ...
        if (Current_pattern == SONG_BTTL_3 && new_pattern == SONG_BTTL_1) {
            ...
            if (Player_obj != NULL && Player_ship != NULL) {
                ...
                Assert(Ship_info[Player_ship->ship_info_index]
                           .initial_hull_strength != 0);
                float integrity =
    ...
    audiostream_play(Patterns[new_pattern].handle, Master_event_music_volume,
                     0); // no looping
    audiostream_set_byte_cutoff(Patterns[new_pattern].handle,
                                fl2i(Patterns[new_pattern].num_measures *
                                     Patterns[new_pattern].bytes_per_measure));
...
event_music_do_frame()
{
    ...
    if (timestamp_elapsed(Pattern_timer_id)) {
        ...
        if (Current_pattern != -1) {
            ...
            audiostream_play(Patterns[Current_pattern].handle,
                             Master_event_music_volume, 0); // no looping
            audiostream_set_byte_cutoff(
                Patterns[Current_pattern].handle,
                fl2i(Patterns[Current_pattern].num_measures *
                     Patterns[Current_pattern].bytes_per_measure));
        }
    }
    ...
    if (Current_pattern != -1) {
        ...
        else if (!audiostream_is_playing(pat->handle) &&
                 !audiostream_is_paused(pat->handle)) {
            audiostream_stop(pat->handle); // stop current and rewind
            pat->loop_for--;
            if (pat->loop_for > 0) {
                audiostream_play(pat->handle, Master_event_music_volume,
                                 0); // no looping
                audiostream_set_byte_cutoff(
                    Patterns[Current_pattern].handle,
                    fl2i(Patterns[Current_pattern].num_measures *
        ...
        // Third case: switching to a different track by interruption
            ...
            if (measures_played < pat->num_measures) {
                audiostream_set_byte_cutoff(
                    pat->handle, pat->bytes_per_measure * (measures_played + 1));
        ...
        if (Current_pattern == SONG_BTTL_1 || Current_pattern == SONG_BTTL_2 ||
...
*/
    ...
    Assert(Current_soundtrack_num >= 0 &&
    ...
    for (i = 0; i < strack->num_patterns; i++) {
        ...
        Patterns[i].handle = audiostream_open(strack->pattern_fnames[i],
                                              ASF_EVENTMUSIC);
        ...
        Patterns[i].bytes_per_measure =
            Pattern_bytes_per_measure[Current_soundtrack_num][i];
...
event_music_enemy_arrival()
{
    ...
    if (Player_obj != NULL && Player_ship != NULL) {
        Assert(Player_ship->ship_info_index >= 0);
        Assert(Ship_info[Player_ship->ship_info_index].initial_hull_strength !=
               0);
        float integrity =
            Player_obj->hull_strength /
            Ship_info[Player_ship->ship_info_index].initial_hull_strength;
    ...
    if (Current_pattern != -1) {
      ...
      */
        ...
        audiostream_set_byte_cutoff(
            ...
            fl2i(Patterns[SONG_AARV_1].num_measures *
                 Patterns[SONG_AARV_1].bytes_per_measure));
        //}
    }
        ...
        while (required_string_either("#Menu Music Start", "#SoundTrack Start")) {
            ...
            while (required_string_either("#SoundTrack End", "$Name:")) {
                ...
                while (token != NULL) {
                    ...
                    if (count == 0) {
                        Pattern_num_measures[Num_soundtracks][num_patterns] =
                            (float)atof(token);
                    }
                    else {
                        Pattern_bytes_per_measure[Num_soundtracks][num_patterns] =
                            atoi(token);
                    }
                ...
                strcpy(Soundtracks[Num_soundtracks].pattern_fnames[num_patterns],
                       fname);
                num_patterns++;
            }
...
//
void
event_music_change_pattern(int new_pattern)
{
    if (Event_music_enabled == FALSE) {
        nprintf((
            "EVENTMUSIC",
...
//
void
event_music_set_volume_all(float volume)
{
...
hostile_ships_present()
{
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if ((shipp->flags & (SF_DISABLED | SF_DYING | SF_DEPARTING)) ||
...
hostile_ships_to_arrive()
{
    ...
    while (p_objp != END_OF_LIST(&ship_arrival_list)) {
        if ((p_objp->team != Player_ship->team) &&
...
void
event_music_get_info(char *outbuf)
{
    if (Event_music_enabled == FALSE || Event_music_level_inited == FALSE ||
        ...
        sprintf(outbuf, XSTR("soundtrack: %s [%s]", 214),
                Soundtracks[Current_soundtrack_num].name,
                Pattern_description[Current_pattern]);
    }
}
...
//
int
event_music_next_soundtrack(int delta)
{
    int new_soundtrack;
...
void
event_music_get_soundtrack_name(char *outbuf)
{
    if (Event_music_enabled == FALSE || Event_music_level_inited == FALSE) {
...
void
event_music_set_soundtrack(char *name)
{
    int i;
