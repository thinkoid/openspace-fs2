...
void
calculate_ship_ship_collision_physics(collision_info_struct *ship_ship_hit_info);

int ship_hit_shield(object *obj, mc_info *mc, collision_info_struct *sshs);
void collect_ship_ship_physics_info(object *heavy, object *light,
                                    mc_info *mc_info,
...
int
ships_are_docking(object *objp1, object *objp2)
{
    ai_info *aip1, *aip2;
    ship *shipp1, *shipp2;
...
int
bay_emerge_or_depart(object *heavy_objp, object *light_objp)
{
...
int
ship_ship_check_collision(collision_info_struct *ship_ship_hit_info,
                          vector *hitpos)
{
    ...
    if ((heavy_obj->flags & OF_SHOULD_BE_DEAD) ||
        (light_obj->flags & OF_SHOULD_BE_DEAD)) {
        return 0;
    }
    ...
    if (!Collide_friendly) { // Collide_friendly is a global value changed via debug console
        if ((!player_involved) && (Ships[heavy_obj->instance].team ==
                                   Ships[light_obj->instance].team)) {
            return 0;
        }
    }
#endif
    ...
    if (Ships[heavy_obj->instance].team == Ships[light_obj->instance].team) {
        if ((!(heavy_obj->flags & OF_WAS_RENDERED) &&
             !(light_obj->flags & OF_WAS_RENDERED))) {
            return 0;
        }
    }
    ...
    if (!(Ship_info[Ships[light_obj->instance].ship_info_index].flags &
    ...
    vector zero, p0, p1;
    vm_vec_zero(
        &zero); // we need the physical vector and can not set its value to zero
    ...
    if ((vm_vec_mag_squared(&heavy_obj->phys_info.max_rotvel) * .04) <
        ...
        vm_vec_sub(&ship_ship_hit_info->light_rel_vel, &light_obj->phys_info.vel,
    ...
    if (!(Ship_info[Ships[heavy_obj->instance].ship_info_index].flags &
          SIF_DONT_COLLIDE_INVIS)) {
        if ((heavy_obj->flags & OF_PLAYER_SHIP) ||
            (light_obj->flags & OF_PLAYER_SHIP) ||
            (Ships[heavy_obj->instance].team !=
             Ships[light_obj->instance].team)) {
    ...
    // copy important data
    int copy_flags =
        mc.flags; // make a copy of start end positions of sphere in  big ship RF
    ...
    if (model_collide(&mc)) {
        ...
        if (ship_ship_hit_info->collide_rotate) {
            model_get_rotating_submodel_list(submodel_list,
                                             &num_rotating_submodels, heavy_obj);
            ...
            for (int i = 0; i < num_rotating_submodels; i++) {
                ...
                pm->submodel[submodel_list[i]].angs =
                    pm->submodel[submodel_list[i]].sii->prev_angs;
                world_find_model_point(&p0, &light_obj->last_pos, pm,
                                       submodel_list[i], &heavy_obj->last_orient,
                                       &heavy_obj->last_pos);

                pm->submodel[submodel_list[i]].angs = copy_angles;
                world_find_model_point(&p1, &light_obj->pos, pm, submodel_list[i],
                ...
                if (model_collide(&mc)) {
                    if (mc.hit_dist < ship_ship_hit_info->hit_time) {
                        ...
                        set_hit_struct_info(ship_ship_hit_info, &mc,
                                            SUBMODEL_ROT_HIT);
                        model_find_world_point(&ship_ship_hit_info->hit_pos,
                                               &mc.hit_point, mc.model_num,
                                               mc.hit_submodel,
        ...
        *mc.p1 = copy_p1;
        ...
        if (model_collide(&mc)) {
            ...
            if (mc.hit_dist < ship_ship_hit_info->hit_time) {
                ...
                if (ship_ship_hit_info->edge_hit == 0) {
                    model_find_obj_dir(&ship_ship_hit_info->collision_normal,
                                       &mc.hit_normal, heavy_obj,
                                       mc.hit_submodel);
                }
                ...
                vector diff;
                vm_vec_sub(&diff, mc.p1, mc.p0);
                vm_vec_scale_add(&ship_ship_hit_info->light_collision_cm_pos,
                                 mc.p0, &diff, mc.hit_dist);
    ...
    if (valid_hit_occured) {
        ...
        if ((collide_obj != NULL) &&
            (Ship_info[Ships[collide_obj->instance].ship_info_index].flags &
        ...
        if (heavy_obj - Objects ==
            Ai_info[Ships[light_obj->instance].ai_index].target_objnum) {
            Ai_info[Ships[light_obj->instance].ai_index].ai_flags |=
                AIF_TARGET_COLLISION;
        }
        if (light_obj - Objects ==
            Ai_info[Ships[heavy_obj->instance].ai_index].target_objnum) {
            Ai_info[Ships[heavy_obj->instance].ai_index].ai_flags |=
                AIF_TARGET_COLLISION;
        }
        ...
        vm_vec_sub(&ship_ship_hit_info->r_light, &ship_ship_hit_info->hit_pos,
                   &ship_ship_hit_info->light_collision_cm_pos);

        // set normal for edge hit
        if (ship_ship_hit_info->edge_hit) {
            vm_vec_copy_normalize(&ship_ship_hit_info->collision_normal,
                                  &ship_ship_hit_info->r_light);
            vm_vec_negate(&ship_ship_hit_info->collision_normal);
        }

        // get world hitpos
        vm_vec_add(hitpos, &ship_ship_hit_info->heavy->pos,
      ...
      if (vm_vec_mag(&diff) > 0.1) {
        ...
        if (Ships[heavy_obj->instance].team == Ships[light_obj->instance].team) {
            ...
            if ((Ship_info[heavy_shipp->ship_info_index].flags &
                 SIF_SMALL_SHIP) &&
                (Ship_info[light_shipp->ship_info_index].flags &
                 SIF_SMALL_SHIP)) {
                if ((heavy_obj->flags & OF_PLAYER_SHIP) ||
                    (light_obj->flags & OF_PLAYER_SHIP)) {
                    /*
               ...
               if (vm_vec_dot(&heavy_obj->phys_info.vel, &h2l_vec) < 0.0f) {
                  ...
                  if (vm_vec_dot(&heavy_obj->orient.rvec, &h2l_vec) < 0.0f) {
                    ...
                    vm_vec_sub(&rel_vel_h, &heavy_obj->phys_info.vel,
                               &light_obj->phys_info.vel);
                    float mass_sum = light_obj->phys_info.mass +
                                     heavy_obj->phys_info.mass;

                    // get comp of rel_vel perp to h_to_l_vec;
                    float mag = vm_vec_dotprod(&h_to_l_vec, &rel_vel_h) /
                                vm_vec_mag_squared(&h_to_l_vec);
                    vm_vec_scale_add(&perp_rel_vel, &rel_vel_h, &h_to_l_vec,
                                     -mag);
                    vm_vec_normalize(&perp_rel_vel);

                    vm_vec_scale_add2(&heavy_obj->phys_info.vel, &perp_rel_vel,
                                      SEP_VEL * light_obj->phys_info.mass /
                                          mass_sum);
                    vm_vec_scale_add2(&light_obj->phys_info.vel, &perp_rel_vel,
                                      -SEP_VEL * heavy_obj->phys_info.mass /
                                          mass_sum);

                    vm_vec_rotate(&heavy_obj->phys_info.prev_ramp_vel,
                                  &heavy_obj->phys_info.vel, &heavy_obj->orient);
                    vm_vec_rotate(&light_obj->phys_info.prev_ramp_vel,
                ...
                float rel_vel = vm_vec_mag_quick(
                    &ship_ship_hit_info->light_rel_vel);
                if (rel_vel < 1) {
                    rel_vel = 1.0f;
                }
                float mass_sum = heavy_obj->phys_info.mass +
                                 light_obj->phys_info.mass;
                vm_vec_scale_add2(
                    &heavy_obj->phys_info.vel, &ship_ship_hit_info->light_rel_vel,
                    SEP_VEL * light_obj->phys_info.mass / (mass_sum * rel_vel));
                vm_vec_rotate(&heavy_obj->phys_info.prev_ramp_vel,
                              &heavy_obj->phys_info.vel, &heavy_obj->orient);
                vm_vec_scale_add2(
                    &light_obj->phys_info.vel, &ship_ship_hit_info->light_rel_vel,
                    -SEP_VEL * heavy_obj->phys_info.mass / (mass_sum * rel_vel));
                vm_vec_rotate(&light_obj->phys_info.prev_ramp_vel,
                              &light_obj->phys_info.vel, &light_obj->orient);
            }
        }
                                         ...
                                         float *cruiser_mass, int *cruiser_light)
{
    ...
    if (heavy->type == OBJ_ASTEROID) {
        ...
        if (Ship_info[Ships[light->instance].ship_info_index].flags &
        ...
        if (Ship_info[Ships[heavy->instance].ship_info_index].flags &
            SIF_BIG_SHIP) {
...
calculate_ship_ship_collision_physics(collision_info_struct *ship_ship_hit_info)
{
    ...
    special_cruiser_asteroid_collision = check_special_cruiser_asteroid_collision(
        heavy, light, &cruiser_mass, &cruiser_light);
    ...
    vector
        v_rel_parallel_m; // normalized v_rel (Va-Vb) projected onto collision surface
    vector world_rotvel_heavy_m, world_rotvel_light_m, vel_from_rotvel_heavy_m,
        vel_from_rotvel_light_m, v_rel_m, vel_heavy_m, vel_light_m;

    coeff_restitution =
        0.1f; // relative velocity wrt normal is zero after the collision ( range 0-1 )
    ...
    if (ship_ship_hit_info->submodel_rot_hit == 1) {
        ...
        if (!pm->submodel[ship_ship_hit_info->submodel_num].sii->axis_set) {
            model_init_submodel_axis_pt(
                pm->submodel[ship_ship_hit_info->submodel_num].sii,
                Ships[heavy->instance].modelnum,
        ...
        else if (pm->submodel[ship_ship_hit_info->submodel_num].movement_axis ==
                 MOVEMENT_AXIS_Y) {
            axis = vmd_y_vector;
        }
        else if (pm->submodel[ship_ship_hit_info->submodel_num].movement_axis ==
        ...
        model_find_obj_dir(&omega, &axis, heavy,
                           ship_ship_hit_info->submodel_num);
        vm_vec_scale(
            &omega,
            pm->submodel[ship_ship_hit_info->submodel_num].sii->cur_turn_rate);

        // world coords for r_rot
        vector temp;
        vm_vec_unrotate(
            &temp,
            &pm->submodel[ship_ship_hit_info->submodel_num].sii->pt_on_axis,
            &heavy->orient);
    ...
    vm_vec_add(&vel_light_m, &vel_from_rotvel_light_m,
               &ship_ship_hit_info->light_rel_vel);
    ...
    v_rel_normal_m = vm_vec_dotprod(
        &v_rel_m,
        &ship_ship_hit_info
             ->collision_normal); // if less than zero, colliding contact taking place
    ...
    vector rotational_impulse_heavy, rotational_impulse_light, delta_rotvel_heavy,
        delta_rotvel_light;
    vector delta_vel_from_delta_rotvel_heavy, delta_vel_from_delta_rotvel_light,
        impulse;
    ...
    vm_vec_projection_onto_plane(&v_rel_parallel_m, &v_rel_m,
                                 &ship_ship_hit_info->collision_normal);
    collision_speed_parallel = vm_vec_normalize_safe(&v_rel_parallel_m);
    parallel_mag = float(-COLLISION_FRICTION_FACTOR) * collision_speed_parallel /
                   vm_vec_mag(&v_rel_m);
    ...
    if (heavy == Player_obj) {
        ...
        vm_vec_crossprod(&rotational_impulse_heavy, &ship_ship_hit_info->r_heavy,
                         &impulse);
        get_I_inv(&heavy_I_inv, &heavy->phys_info.I_body_inv, &heavy->orient);
        vm_vec_rotate(&delta_rotvel_heavy, &rotational_impulse_heavy,
                      &heavy_I_inv);
        vm_vec_scale(
            &delta_rotvel_heavy,
            float(
                COLLISION_ROTATION_FACTOR)); // hack decrease rotation (delta_rotvel)
        vm_vec_crossprod(&delta_vel_from_delta_rotvel_heavy, &delta_rotvel_heavy,
                         &ship_ship_hit_info->r_heavy);
        heavy_denom = vm_vec_dotprod(&delta_vel_from_delta_rotvel_heavy,
                                     &ship_ship_hit_info->collision_normal);
        if (heavy_denom < 0) {
            // sanity check
            heavy_denom = 0.0f;
    ...
    if (light == Player_obj) {
        ...
        vm_vec_crossprod(&rotational_impulse_light, &ship_ship_hit_info->r_light,
                         &impulse);
        get_I_inv(&light_I_inv, &light->phys_info.I_body_inv, &light->orient);
        vm_vec_rotate(&delta_rotvel_light, &rotational_impulse_light,
                      &light_I_inv);
        vm_vec_scale(
            &delta_rotvel_light,
            float(
                COLLISION_ROTATION_FACTOR)); // hack decrease rotation (delta_rotvel)
        vm_vec_crossprod(&delta_vel_from_delta_rotvel_light, &delta_rotvel_light,
                         &ship_ship_hit_info->r_light);
        light_denom = vm_vec_dotprod(&delta_vel_from_delta_rotvel_light,
                                     &ship_ship_hit_info->collision_normal);
        if (light_denom < 0) {
            // sanity check
            light_denom = 0.0f;
    ...
    physics_collide_whack(&impulse, &delta_rotvel_light, &light->phys_info,
                          &light->orient);
    vm_vec_negate(&impulse);
    vm_vec_scale(&delta_rotvel_heavy, -impulse_mag);
    physics_collide_whack(&impulse, &delta_rotvel_heavy, &heavy->phys_info,
                          &heavy->orient);
    ...
    vm_vec_add(&light->pos, &heavy->pos,
               &ship_ship_hit_info->light_collision_cm_pos);
    ...
    vm_vec_sub(&direction_light, &ship_ship_hit_info->light_rel_vel,
    ...
    vm_vec_scale_add2(&heavy->pos, &direction_light,
                      0.2f * light->phys_info.mass /
                          (heavy->phys_info.mass + light->phys_info.mass));
    vm_vec_scale_add2(&light->pos, &direction_light,
                      -0.2f * heavy->phys_info.mass /
                          (heavy->phys_info.mass + light->phys_info.mass));
    vm_vec_scale_add2(&heavy->pos, &ship_ship_hit_info->collision_normal,
                      -0.1f * light->phys_info.mass /
                          (heavy->phys_info.mass + light->phys_info.mass));
...
//
void
get_I_inv(matrix *I_inv, matrix *I_inv_body, matrix *orient)
{
...
void
mcp_1(object *player_objp, object *planet_objp)
{
    float planet_radius;
    float dist;
...
// Else return false.
int
maybe_collide_planet(object *obj1, object *obj2)
{
    ship_info *sip1, *sip2;
...
int
get_ship_quadrant_from_global(vector *global_pos, object *objp)
{
    vector tpos;
    vector rotpos;
...
void
collide_ship_ship_do_sound(vector *world_hit_pos, object *A, object *B,
                           int player_involved)
{
    vector rel_vel;
    float rel_speed;
    ...
    if (rel_speed > MIN_REL_SPEED_FOR_LOUD_COLLISION) {
        ...
        if (player_involved) {
            if (!snd_is_playing(Player_collide_sound)) {
                Player_collide_sound = snd_play_3d(&Snds[SND_SHIP_SHIP_LIGHT],
                                                   world_hit_pos, &View_position);
            }
        }
        else {
            if (!snd_is_playing(AI_collide_sound)) {
                AI_collide_sound = snd_play_3d(&Snds[SND_SHIP_SHIP_LIGHT],
                                               world_hit_pos, &View_position);
            }
        }
    }
        ...
        if (player_involved) {
            if (!snd_is_playing(Player_collide_sound)) {
                Player_collide_shield_sound = snd_play_3d(
                    &Snds[SND_SHIP_SHIP_SHIELD], world_hit_pos, &View_position);
            }
        }
        else {
            if (!snd_is_playing(Player_collide_sound)) {
...
void
do_kamikaze_crash(object *obj1, object *obj2)
{
    ai_info *aip1, *aip2;
    ship *ship1, *ship2;
    ...
    if (ship1->team != ship2->team) {
        if (aip1->ai_flags & AIF_KAMIKAZE) {
            if (Ship_info[ship2->ship_info_index].flags &
                (SIF_BIG_SHIP | SIF_HUGE_SHIP)) {
        ...
        if (aip2->ai_flags & AIF_KAMIKAZE) {
            if (Ship_info[ship1->ship_info_index].flags &
                (SIF_BIG_SHIP | SIF_HUGE_SHIP)) {
...
void
maybe_push_little_ship_from_fast_big_ship(object *big, object *small,
                                          float impulse, vector *normal)
{
    // Move player out of the way of a BIG|HUGE ship warping in or out
    if (Ship_info[Ships[big->instance].ship_info_index].flags &
        (SIF_CAPITAL | SIF_SUPERCAP)) {
        if (Ship_info[Ships[small->instance].ship_info_index].flags &
            (SIF_SMALL_SHIP)) {
            ...
            if (big_speed > 3 * big->phys_info.max_vel.z) {
                ...
                vm_vec_scale_add(&perp, &temp, &big->orient.fvec,
                                 -vm_vec_dotprod(&temp, &big->orient.fvec));
                ...
                vm_vec_scale_add2(&small->phys_info.vel, &perp,
                                  added_perp_vel_mag);
                vm_vec_rotate(&small->phys_info.prev_ramp_vel,
                              &small->phys_info.vel, &small->orient);
            }
        }
    }
}
...
int
collide_ship_ship(obj_pair *pair)
{
    int player_involved;
    float dist;
    object *A = pair->a;
...
//mprintf(("Ship:Ship damage = %7.3f\n", speed));
            ...
            if (!Collide_friendly) {
                if (Ships[A->instance].team == Ships[B->instance].team) {
                    ...
                    vm_vec_normalized_dir(&collision_vec,
                                          &ship_ship_hit_info.hit_pos, &A->pos);
                    if (vm_vec_dot(&collision_vec, &A->orient.fvec) > 0.999f) {
                        right_angle_vec = A->orient.rvec;
                    }
                    else {
                        vm_vec_cross(&right_angle_vec, &A->orient.uvec,
                                     &collision_vec);
                    }
            ...
            if ((LightOne->flags & OF_PLAYER_SHIP) ||
                (HeavyOne->flags & OF_PLAYER_SHIP)) {
                damage *= (float)(Game_skill_level * Game_skill_level + 1) /
                          (NUM_SKILL_LEVELS + 1);
            }
            else if (Ships[LightOne->instance].team ==
            ...
            if ((ship_ship_hit_info.heavy->flags & OF_NO_SHIELDS) ||
                !ship_is_shield_up(ship_ship_hit_info.heavy, quadrant_num)) {
                quadrant_num = -1;
            }

            ship_apply_local_damage(ship_ship_hit_info.heavy,
                                    ship_ship_hit_info.light, &world_hit_pos,
            ...
            ship_apply_local_damage(ship_ship_hit_info.light,
                                    ship_ship_hit_info.heavy, &world_hit_pos,
                                    dam2, MISS_SHIELDS, NO_SPARKS, -1,
                                    &ship_ship_hit_info.collision_normal);
        ...
        if (ship_is_beginning_warpout_speedup(A)) {
            shipA_max_speed = max(ship_get_max_speed(&Ships[A->instance]),
                                  ship_get_warp_speed(A));
        }
        else {
        ...
        if (ship_is_beginning_warpout_speedup(B)) {
            shipB_max_speed = max(ship_get_max_speed(&Ships[B->instance]),
                                  ship_get_warp_speed(B));
        }
        else {
...
void
collect_ship_ship_physics_info(object *heavy, object *light, mc_info *mc_info,
                               collision_info_struct *ship_ship_hit_info)
{
    ...
    if (!ship_ship_hit_info->submodel_rot_hit) {
        ...
        *light_collide_cm_pos = *mc_info->p0;
        vm_vec_scale_add2(light_collide_cm_pos, &displacement,
                          ship_ship_hit_info->hit_time);
    }
...
// sphere_sphere_case_handled separately
    ...
    nprintf((
        ...
        Framecount, Ships[heavy->instance].ship_name, heavy->last_pos.x,
        heavy->last_pos.y, heavy->last_pos.z, heavy_collide_cm_pos.x,
        heavy_collide_cm_pos.y, heavy_collide_cm_pos.z, heavy->phys_info.vel.x,
        heavy->phys_info.vel.y, heavy->phys_info.vel.z));

    nprintf((
        "Physics",
        "Frame: %i %s info: last_pos: [%4.1f, %4.1f, %4.1f], collide_pos: [%4.1f, %4.1f, %4.1f] vel: [%4.1f, %4.1f, %4.1f]\n",
        Framecount, Ships[light->instance].ship_name, light->last_pos.x,
        light->last_pos.y, light->last_pos.z, light_collide_cm_pos.x,
        light_collide_cm_pos.y, light_collide_cm_pos.z, light->phys_info.vel.x,
        light->phys_info.vel.y, light->phys_info.vel.z));
#endif
}
