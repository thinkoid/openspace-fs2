...
int check_inside_radius_for_big_ships(object *ship, object *weapon,
                                      obj_pair *pair);
...
update_danger_weapon(object *ship_obj, object *weapon_obj)
{
    ...
    else if (aip->danger_weapon_signature ==
             Objects[aip->danger_weapon_objnum].signature) {
        float danger_old_time, danger_new_time;

        danger_old_time = ai_endangered_time(ship_obj,
                                             &Objects[aip->danger_weapon_objnum]);
...
void
ship_weapon_do_hit_stuff(object *ship_obj, object *weapon_obj,
                         vector *world_hitpos, vector *hitpos, int quadrant_num,
                         int submodel_num)
{
    ...
    ship_apply_local_damage(ship_obj, weapon_obj, world_hitpos, damage,
                            quadrant_num, CREATE_SPARKS, submodel_num);
    ...
    if ((Ships[ship_obj->instance].wing_status_wing_index >= 0) &&
        ((Ships[ship_obj->instance].wing_status_wing_pos >= 0))) {
        hud_wingman_status_start_flash(shipp->wing_status_wing_index,
...
int
ship_weapon_check_collision(object *ship_obj, object *weapon_obj,
                            float time_limit = 0.0f, int *next_hit = NULL)
{
    mc_info mc;
    int num;
    ship *shipp;
...
#ifndef NDEBUG
    if (!(ship_obj->flags & OF_NO_SHIELDS) && New_shield_system &&
        (pm->shield.ntris > 0)) {
#else
...
*/
                valid_hit_occured = 1;
                // shield effect
                add_shield_point(ship_obj - Objects, mc.shield_hit_tri,
                                 &mc.hit_point);
                do_model_check =
                    0; // since we hit the shield, no need to check the model
            }
            else {
                quadrant_num = -1; // ignore shield hit
            }
    ...
    if (next_hit && valid_hit_occured) {
        // find hit time
        *next_hit = (int)(1000.0f * (mc.hit_dist * (flFrametime + time_limit) -
                                     flFrametime));
        if (*next_hit > 0)
    ...
    if (valid_hit_occured) {
        ship_weapon_do_hit_stuff(ship_obj, weapon_obj, &mc.hit_point_world,
                                 &mc.hit_point, quadrant_num, mc.hit_submodel);
    }
    else if ((Missiontime - wp->creation_time > F1_0 / 2) &&
             (wip->wi_flags & WIF_HOMING) && (wp->homing_object == ship_obj)) {
            ...
            if (vm_vec_dot(&vec_to_ship, &weapon_obj->orient.fvec) < 0.0f) {
                ...
                if (!(Ship_info[shipp->ship_info_index].flags &
                    ...
                    if (ship_obj == Player_obj)
                        nprintf((
                            "Jim",
                            "Frame %i: Weapon %i set to detonate, dist = %7.3f.\n",
...
int
collide_ship_weapon(obj_pair *pair)
{
    int did_hit;
    object *ship = pair->a;
    ...
    if ((Ship_info[Ships[ship->instance].ship_info_index].flags &
        ...
        if (vm_vec_dist_squared(&ship->pos, &weapon->pos) <
...
float
estimate_ship_speed_upper_limit(object *ship, float time)
{
    float exponent;
    float delta_v;
    float factor;

    delta_v = Ship_info[Ships[ship->instance].ship_info_index].max_vel.z -
...
int
check_inside_radius_for_big_ships(object *ship, object *weapon, obj_pair *pair)
{
    ...
    time_to_exit_sphere = (ship->radius + vm_vec_dist(&ship->pos, &weapon->pos)) /
                          (weapon->phys_info.max_vel.z -
                           ship->phys_info.max_vel.z);
    ship_speed_at_exit_sphere = estimate_ship_speed_upper_limit(
        ship, time_to_exit_sphere);
    // update estimated time to exit sphere
    time_to_exit_sphere = (ship->radius + vm_vec_dist(&ship->pos, &weapon->pos)) /
                          (weapon->phys_info.max_vel.z -
                           ship_speed_at_exit_sphere);
    vm_vec_scale_add(&error_vel, &ship->phys_info.vel, &weapon->orient.fvec,
                     -vm_vec_dotprod(&ship->phys_info.vel, &weapon->orient.fvec));
    error_vel_mag = vm_vec_mag_quick(&error_vel);
    error_vel_mag += 0.5f * (ship->phys_info.max_vel.z - error_vel_mag) *
                     (time_to_exit_sphere /
                      ship->phys_info.forward_accel_time_const);
    ...
    if (ship_weapon_check_collision(ship, weapon, limit_time, &hit_time)) {
        ...
        if (limit_time > time_to_max_error) {
            ...
            if (1000 * time_to_max_error > 200) {
                pair->next_check_time = timestamp(
                    (int)(1000 * time_to_max_error) - 200);
            }
            else {
                pair->next_check_time = 1;
            }
