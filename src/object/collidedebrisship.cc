...
int
collide_debris_ship(obj_pair *pair)
{
    float dist;
...
*/
    ...
    if (dist < pdebris->radius + pship->radius) {
        ...
        if (hit) {
            ...
            if (Ship_info[Ships[pship->instance].ship_info_index].flags &
                SIF_SUPERCAP) {
                float cap_percent_damage = frand_range(0.1f, 0.2f);
                ship_damage = min(
                    ship_damage,
                    cap_percent_damage *
                        Ship_info[Ships[pship->instance].ship_info_index]
            ...
            if (debris_hit_info.heavy == pship) {
                ...
                if ((pship->flags & OF_NO_SHIELDS) ||
                    !ship_is_shield_up(pship, quadrant_num)) {
                    quadrant_num = -1;
                }
                if (apply_ship_damage) {
                    ship_apply_local_damage(debris_hit_info.heavy,
                                            debris_hit_info.light, &hitpos,
                                            ship_damage, quadrant_num,
                                            CREATE_SPARKS,
                ...
                if (apply_ship_damage) {
                    ship_apply_local_damage(debris_hit_info.light,
                                            debris_hit_info.heavy, &hitpos,
            ...
            collide_ship_ship_do_sound(&hitpos, pship, pdebris,
                                       pship == Player_obj);
        ...
        if (ship_is_beginning_warpout_speedup(pship)) {
            ship_max_speed = max(ship_get_max_speed(shipp),
                                 ship_get_warp_speed(pship));
        }
        else {
...
collide_asteroid_ship(obj_pair *pair)
{
    ...
    if (dist < pasteroid->radius + pship->radius) {
        ...
        hit = asteroid_check_collision(pasteroid, pship, &hitpos,
                                       &asteroid_hit_info);
        if (hit) {
            float ship_damage;
            ...
            float max_ship_impulse =
                (2.0f * pship->phys_info.max_vel.z +
            ...
            if ((ship_damage > 500.0f) &&
                (ship_damage > Ship_info[Ships[pship->instance].ship_info_index]
                                       .initial_hull_strength /
                                   8.0f)) {
                ship_damage = Ship_info[Ships[pship->instance].ship_info_index]
                                  .initial_hull_strength /
                ...
                nprintf((
                    "AI",
                    "Pinning damage to %s from asteroid at %7.3f (%7.3f percent)\n",
            ...
            asteroid_hit(pasteroid, pship, &hitpos,
            ...
            if (asteroid_hit_info.heavy == pship) {
                ...
                ship_apply_local_damage(asteroid_hit_info.heavy,
                                        asteroid_hit_info.light, &hitpos,
                ...
                ship_apply_local_damage(asteroid_hit_info.light,
            ...
            collide_ship_ship_do_sound(&hitpos, pship, pasteroid,
                                       pship == Player_obj);
        ...
        asteroid_max_speed = vm_vec_mag(
            &pasteroid->phys_info.vel); // Asteroid... vel gets reset, not max vel.z
