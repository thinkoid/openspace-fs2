...
int
reject_obj_pair_on_parent(object *A, object *B)
{
    if (A->type == OBJ_SHIP) {
...
void
obj_add_pair(object *A, object *B, int check_time, int add_to_end)
{
    uint ctype;
...
*/
    ...
    if (check_collision == collide_debris_weapon) {
        // weapon is B
        if (!(Weapon_info[Weapons[B->instance].weapon_info_index].wi_flags &
              WIF_TURNS)) {
            ...
            if (Weapon_info[Weapons[B->instance].weapon_info_index].subtype ==
                ...
                vm_vec_sub(&velocity_rel_weapon, &B->phys_info.vel,
                           &A->phys_info.vel);
            ...
            if (vm_vec_dist_squared(&A->pos, &B->pos) >
                (vm_vec_mag_squared(&delta_v) * Weapons[B->instance].lifeleft *
    ...
    if (check_collision == collide_ship_weapon) {
        ...
        if (!(Objects[B->parent].flags & OF_PLAYER_SHIP) &&
            ...
            (Ship_info[Ships[A->instance].ship_info_index].flags &
             SIF_SMALL_SHIP) &&
...
obj_check_all_collisions()
{
    ...
    while (tmp != NULL) {
        int removed = 0;
        if ((tmp->a != NULL) && (tmp->b != NULL) &&
            timestamp_elapsed(tmp->next_check_time)) {
            Num_pairs_checked++;
                        ...
                        float radius_scale)
{
    ...
    r = find_nearest_point_on_line(&nearest_point, start_pos, end_pos,
                                   &objp->pos);
    if ((r >= 0.0f) && (r <= 1.0f)) {
...
int
weapon_will_never_hit(object *weapon, object *other, obj_pair *current_pair)
{
    ...
    if (!(Weapon_info[Weapons[weapon->instance].weapon_info_index].wi_flags &
        ...
        float vdot;
        if (Weapon_info[Weapons[weapon->instance].weapon_info_index].subtype ==
            WP_LASER) {
            vector velocity_rel_weapon;
            vm_vec_sub(&velocity_rel_weapon, &weapon->phys_info.vel,
                       &other->phys_info.vel);
    ...
    if (current_pair) {
        ...
        if (!(Weapon_info[Weapons[weapon->instance].weapon_info_index].wi_flags &
            ...
            delta_t =
                (other->radius + 10.0f) /
                max_vel_other; // time to get from center to radius of other obj
            ...
            dist = vm_vec_dist(&other->pos, &weapon->pos) -
                   (other->radius + 10.0f);
            if (dist > 0.0f) {
                ...
                time_ms -=
                    200; // Allow at least one worst case frametime to elapse (~5 fps)

                if (time_ms >
                    100) { // If it takes longer than 1/10th of a second, then delay it
...
pp_collide(vector *curpos, vector *goalpos, object *goalobjp, float radius)
{
    ...
    mc.model_num =
        Ships[goalobjp->instance].modelnum; // Fill in the model to check
...
collide_predict_large_ship(object *objp, float distance)
{
    ...
    for (objp2 = GET_FIRST(&obj_used_list); objp2 != END_OF_LIST(&obj_used_list);
         objp2 = GET_NEXT(objp2)) {
        if ((objp != objp2) && (objp2->type == OBJ_SHIP)) {
            if (Ship_info[Ships[objp2->instance].ship_info_index].flags &
        ...
        else if (!(sip->flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP)) &&
                 (objp2->type == OBJ_ASTEROID)) {
...
void
crw_check_weapon(int weapon_num, int collide_next_check)
{
...
void
set_hit_struct_info(collision_info_struct *hit, mc_info *mc, int submodel_rot_hit)
{
