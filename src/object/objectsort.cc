...
vector check_offsets[8] = { { -1.0f, -1.0f, -1.0f }, { -1.0f, -1.0f, 1.0f },
...
void
obj_render_all(void (*render_function)(object *objp))
{
    object *objp;
    int i, j, incr;
...
*/
        ...
        while (incr > 0) {
            for (i = incr; i < Num_sorted_objects; i++) {
                ...
                while (j >= 0) {
                    ...
                    if ((Sorted_objects[Object_sort_order[j]].max_z <
                         Sorted_objects[Object_sort_order[j + incr]].max_z)) {
    ...
    for (i = 0; i < Num_sorted_objects; i++) {
        ...
        if ((The_mission.flags & MISSION_FLAG_FULLNEB) &&
            ...
            // a bit of time
            if ((fog_near != gr_screen.fog_near) ||
                (fog_far != gr_screen.fog_far)) {
                gr_fog_set(GR_FOGMODE_FOG, gr_screen.current_fog_color.red,
                           gr_screen.current_fog_color.green,
