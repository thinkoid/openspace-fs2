...
int Radar_radius[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                              120, 100 },
                                            { // GR_1024
                                              192, 160 } };

float Radar_center[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                322.0f, 422.0f },
                                              { // GR_1024
                                                515.0f, 675.0f } };

int Radar_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                              257, 369 },
                                            { // GR_1024
                                              411, 590 } };
char Radar_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = { "radar1", "2_radar1" };

int Radar_blip_radius_normal[GR_NUM_RESOLUTIONS] = { 2, 4 };
int Radar_blip_radius_target[GR_NUM_RESOLUTIONS] = { 5, 8 };
...
static int
    Radar_calc_dim_dist_timer; // timestamp at which we recalc Radar_dim_range

#define NUM_FLICKER_TIMERS 2
static int Radar_flicker_timer
    [NUM_FLICKER_TIMERS]; // timestamp used to flicker blips on and off
...
radar_init()
{
    ...
    Radar_gauge.first_frame = bm_load_animation(Radar_fname[gr_screen.res],
                                                &Radar_gauge.num_frames);
    ...
    for (i = 0; i < MAX_RADAR_LEVELS; i++) {
        for (j = 0; j < MAX_RADAR_COLORS; j++) {
            gr_init_alphacolor(&Radar_colors[i][j], Radar_color_rgb[i][j].r,
                               Radar_color_rgb[i][j].g, Radar_color_rgb[i][j].b,
                               255);
        }
    }
...
radar_blip_color(object *objp)
{
    ...
    case OBJ_SHIP:
        ...
        else if (Ship_info[shipp->ship_info_index].flags &
                 (SIF_NAVBUOY | SIF_CARGO)) {
            color = RCOL_NAVBUOYS;
        }
        else {
...
radar_plot_object(object *objp)
{
    ...
    case OBJ_WEAPON: {
        ...
        if ((obj_team(objp) == Player_ship->team) &&
            (Player_ship->team != TEAM_TRAITOR)) {
            return;
        }
        break;
...
void
radar_draw_circle(int x, int y, int rad)
{
    if (rad == Radar_blip_radius_target[gr_screen.res]) {
        gr_string(Large_blip_offset_x + x, Large_blip_offset_y + y,
                  Large_blip_string);
    }
    else {
        // rad = RADAR_BLIP_RADIUS_NORMAL;
        gr_string(Small_blip_offset_x + x, Small_blip_offset_y + y,
                  Small_blip_string);
    }
}
...
void
radar_blip_draw_distorted(blip *b)
{
    int xdiff, ydiff;
    float scale;
    xdiff = -10 + rand() % 20;
...
void
radar_blip_draw_flicker(blip *b)
{
...
radar_draw_range()
{
    ...
    case RR_SHORT:
        gr_printf(Radar_dist_coords[gr_screen.res][RR_SHORT][0],
                  Radar_dist_coords[gr_screen.res][RR_SHORT][1], XSTR("2k", 467));
        break;

    case RR_LONG:
        gr_printf(Radar_dist_coords[gr_screen.res][RR_LONG][0],
                  Radar_dist_coords[gr_screen.res][RR_LONG][1], XSTR("10k", 468));
        break;

    case RR_INFINITY:
        sprintf(buf, NOX("%c"), Lcl_special_chars);
        gr_printf(Radar_dist_coords[gr_screen.res][RR_INFINITY][0],
                  Radar_dist_coords[gr_screen.res][RR_INFINITY][1], buf);
        break;
...
void
radar_frame_render(float frametime)
{
    float sensors_str;
