...
//
float
vm_vec_projection_parallel(vector *component, vector *src, vector *unit_vec)
{
    float mag;
...
//
void
vm_vec_projection_onto_plane(vector *projection, vector *src, vector *unit_normal)
{
    float mag;
...
//
void
vm_project_point_onto_plane(vector *new_point, vector *point,
...
void
vm_vec_add(vector *dest, vector *src0, vector *src1)
{
...
#ifndef _INLINE_VECMAT
void
vm_vec_sub(vector *dest, vector *src0, vector *src1)
{
    dest->x = src0->x - src1->x;
    dest->y = src0->y - src1->y;
    dest->z = src0->z - src1->z;
...
#ifndef _INLINE_VECMAT
void
vm_vec_add2(vector *dest, vector *src)
{
    dest->x += src->x;
    dest->y += src->y;
    dest->z += src->z;
...
#ifndef _INLINE_VECMAT
void
vm_vec_sub2(vector *dest, vector *src)
{
    dest->x -= src->x;
    dest->y -= src->y;
    dest->z -= src->z;
...
vector *
vm_vec_avg(vector *dest, vector *src0, vector *src1)
{
...
vector *
vm_vec_avg4(vector *dest, vector *src0, vector *src1, vector *src2, vector *src3)
{
...
#ifndef _INLINE_VECMAT
void
vm_vec_scale(vector *dest, float s)
{
    dest->x = dest->x * s;
    dest->y = dest->y * s;
    dest->z = dest->z * s;
...
#ifndef _INLINE_VECMAT
void
vm_vec_copy_scale(vector *dest, vector *src, float s)
{
    dest->x = src->x * s;
    dest->y = src->y * s;
    dest->z = src->z * s;
...
#ifndef _INLINE_VECMAT
void
vm_vec_scale_add(vector *dest, vector *src1, vector *src2, float k)
{
...
#ifndef _INLINE_VECMAT
void
vm_vec_scale_add2(vector *dest, vector *src, float k)
{
    dest->x += src->x * k;
    dest->y += src->y * k;
    dest->z += src->z * k;
...
#ifndef _INLINE_VECMAT
void
vm_vec_scale_sub2(vector *dest, vector *src, float k)
{
    dest->x -= src->x * k;
    dest->y -= src->y * k;
    dest->z -= src->z * k;
...
#ifndef _INLINE_VECMAT
void
vm_vec_scale2(vector *dest, float n, float d)
{
    d = 1.0f / d;
...
#ifndef _INLINE_VECMAT
float
vm_vec_dotprod(vector *v0, vector *v1)
{
...
#ifndef _INLINE_VECMAT
float
vm_vec_dot3(float x, float y, float z, vector *v)
{
...
float
vm_vec_dist_squared(vector *v0, vector *v1)
{
    float dx, dy, dz;
...
float
vm_vec_dist(vector *v0, vector *v1)
{
    float t1;
    vector t;
...
//uses dist = largest + next_largest*3/8 + smallest*3/16
float
vm_vec_dist_quick(vector *v0, vector *v1)
{
    vector t;
...
float
vm_vec_copy_normalize(vector *dest, vector *src)
{
    float m;
...
float
vm_vec_normalized_dir(vector *dest, vector *end, vector *start)
{
    float t;
...
//NOTE: the order of the parameters matches the vector subtraction
float
vm_vec_normalized_dir_quick(vector *dest, vector *end, vector *start)
{
    vm_vec_sub(dest, end, start);
...
//NOTE: the order of the parameters matches the vector subtraction
float
vm_vec_normalized_dir_quick_mag(vector *dest, vector *end, vector *start)
{
    float t;
    vm_vec_sub(dest, end, start);
...
vector *
vm_vec_normal(vector *dest, vector *p0, vector *p1, vector *p2)
{
...
//your inputs are ok.
vector *
vm_vec_crossprod(vector *dest, vector *src0, vector *src1)
{
...
int
vm_test_parallel(vector *src0, vector *src1)
{
    if ((fl_abs(src0->x - src1->x) < 1e-4) &&
        (fl_abs(src0->y - src1->y) < 1e-4) &&
        (fl_abs(src0->z - src1->z) < 1e-4)) {
        return 1;
    }
    else {
        return 0;
...
//dest CANNOT equal either source
vector *
vm_vec_perp(vector *dest, vector *p0, vector *p1, vector *p2)
{
    vector t0, t1;
...
//returned.
float
vm_vec_delta_ang(vector *v0, vector *v1, vector *fvec)
{
    float t;
    vector t0, t1, t2;
...
float
vm_vec_delta_ang_norm(vector *v0, vector *v1, vector *fvec)
{
    float a;
    vector t;
...
matrix *
sincos_2_matrix(matrix *m, float sinp, float cosp, float sinb, float cosb,
                float sinh, float cosh)
{
...
matrix *
vm_angles_2_matrix(matrix *m, angles *a)
{
    matrix *t;
...
matrix *
vm_vec_ang_2_matrix(matrix *m, vector *v, float a)
{
    matrix *t;
...
matrix *
vm_vector_2_matrix(matrix *m, vector *fvec, vector *uvec, vector *rvec)
{
    ...
    if (uvec == NULL) {
        ...
        bad_vector2:;
            ...
            if ((zvec->x == 0.0) &&
                (zvec->z == 0.0)) { //forward vec is straight up or down
...
matrix *
vm_vector_2_matrix_norm(matrix *m, vector *fvec, vector *uvec, vector *rvec)
{
    vector *xvec = &m->rvec, *yvec = &m->uvec, *zvec = &m->fvec;
...
vector *
vm_vec_rotate(vector *dest, vector *src, matrix *m)
{
...
vector *
vm_vec_unrotate(vector *dest, vector *src, matrix *m)
{
...
matrix *
vm_copy_transpose_matrix(matrix *dest, matrix *src)
{
    Assert(dest != src);
...
//dest CANNOT equal either source
matrix *
vm_matrix_x_matrix(matrix *dest, matrix *src0, matrix *src1)
{
    Assert(dest != src0 && dest != src1);

    dest->rvec.x = vm_vec_dot3(src0->rvec.x, src0->uvec.x, src0->fvec.x,
                               &src1->rvec);
    dest->uvec.x = vm_vec_dot3(src0->rvec.x, src0->uvec.x, src0->fvec.x,
                               &src1->uvec);
    dest->fvec.x = vm_vec_dot3(src0->rvec.x, src0->uvec.x, src0->fvec.x,
                               &src1->fvec);

    dest->rvec.y = vm_vec_dot3(src0->rvec.y, src0->uvec.y, src0->fvec.y,
                               &src1->rvec);
    dest->uvec.y = vm_vec_dot3(src0->rvec.y, src0->uvec.y, src0->fvec.y,
                               &src1->uvec);
    dest->fvec.y = vm_vec_dot3(src0->rvec.y, src0->uvec.y, src0->fvec.y,
                               &src1->fvec);

    dest->rvec.z = vm_vec_dot3(src0->rvec.z, src0->uvec.z, src0->fvec.z,
                               &src1->rvec);
    dest->uvec.z = vm_vec_dot3(src0->rvec.z, src0->uvec.z, src0->fvec.z,
                               &src1->uvec);
    dest->fvec.z = vm_vec_dot3(src0->rvec.z, src0->uvec.z, src0->fvec.z,
                               &src1->fvec);
...
angles *
vm_extract_angles_matrix(angles *a, matrix *m)
{
    ...
    if (cosp ==
        0.0) //the cosine of pitch is zero.  we're pitched straight up. say no bank
...
angles *
vm_extract_angles_vector_normalized(angles *a, vector *v)
{
...
//extract heading and pitch from a vector, assuming bank==0
angles *
vm_extract_angles_vector(angles *a, vector *v)
{
    vector t;
...
float
vm_dist_to_plane(vector *checkp, vector *norm, vector *planep)
{
    float t1;
    vector t;
...
void
vm_trackball(int idx, int idy, matrix *RotMat)
{
...
//returns ptr to matrix
void
vm_orthogonalize_matrix(matrix *m_src)
{
    float umag, rmag;
    matrix tempm;
    matrix *m = &tempm;
...
vm_fix_matrix(matrix *m)
{
    ...
    if (fmag <= 0.0f) {
        if ((umag > 0.0f) && (rmag > 0.0f) &&
            !vm_test_parallel(&m->uvec, &m->rvec)) {
    ...
    if ((umag <= 0.0f) ||
        vm_test_parallel(&m->fvec, &m->uvec)) { // no up vector to use..
        if ((rmag <= 0.0f) ||
            vm_test_parallel(
                &m->fvec,
                &m->rvec)) { // no right vector either, so make something up
...
void
vm_rotate_matrix_by_angles(matrix *orient, angles *tangles)
{
...
void
vm_rot_point_around_line(vector *out, vector *in, float angle, vector *line_point,
                         vector *line_dir)
{
    vector tmp, tmp1;
    matrix m, r, im;
...
void
vm_interp_angle(float *h, float desired_angle, float step_size)
{
    float delta;
...
int
vm_check_matrix_for_zeros(matrix *m)
{
...
int
vm_vec_same(vector *v1, vector *v2)
{
...
void
vm_quaternion_rotate(matrix *M, float theta, vector *u)
...
void
vm_matrix_to_rot_axis_and_angle(matrix *m, float *theta, vector *rot_axis)
{
    ...
    if (cos_theta >
        ...
        *theta = 0.0f;
        ...
        vm_vec_make(rot_axis, 1.0f, 0.0f, 0.0f);
             ...
             -0.999999875f) { // angle is within limits between 0 and PI
...
float
approach(float w_in, float w_max, float theta_goal, float aa, float delta_t,
         float *w_out, int no_overshoot)
{
    ...
    if (w_in * w_in > 2.0f * 1.05f * aa * theta_goal) { // overshoot condition
        ...
        if (delta_theta > theta_goal) { // pass goal during this frame
            float t_goal = (-w_in + fl_sqrt(w_in * w_in +
                                            2.0f * effective_aa * theta_goal)) /
                           effective_aa;
            ...
            delta_theta -= away(-w_in, w_max, 0.0f, aa, delta_t - t_goal, w_out,
                                no_overshoot);
            *w_out = -*w_out;
            return delta_theta;
        }
            ...
            if (delta_theta < 0) {
             ...
             2.0f * 0.95f * aa * theta_goal) { // undershoot condition
        ...
        if (wp_sqr > w_max * w_max) {
            ...
            if (wp_sqr > (w_in + aa * delta_t) * (w_in + aa * delta_t)) {
                ...
                *w_out = w_in + aa * delta_t;
                ...
                // nothing, decel clamps, theta_goal is returned. Retail's
                // shipping build sailed through with asserts compiled out
                Assert(time_to_wp >= 0);
...

{
    ...
    t_excess = delta_t - t0;
    delta_theta += approach(0.0f, w_max, theta_goal, aa, t_excess, w_out,
                            no_overshoot);
    return delta_theta;
}
...
void
vm_matrix_interpolate(matrix *goal_orient, matrix *curr_orient, vector *w_in,
                      float delta_t, matrix *next_orient, vector *w_out,
                      vector *vel_limit, vector *acc_limit, int no_overshoot)
{
    ...
    if (theta_goal.x > 0) {
        if (w_in->x >= 0) {
            delta_theta = approach(w_in->x, vel_limit->x, theta_goal.x,
                                   acc_limit->x, delta_t, &w_out->x,
                                   no_overshoot);
            theta_end.x = delta_theta;
        }
        else { // w_in->x < 0
            delta_theta = away(w_in->x, vel_limit->x, theta_goal.x, acc_limit->x,
                               delta_t, &w_out->x, no_overshoot);
            theta_end.x = delta_theta;
        }
    }
        ...
        if (w_in->x <= 0) {
            ...
            delta_theta = away(-w_in->x, vel_limit->x, -theta_goal.x,
                               acc_limit->x, delta_t, &w_out->x, no_overshoot);
            theta_end.x = -delta_theta;
            w_out->x = -w_out->x;
        }
    }
    ...
    if (theta_goal.y > 0) {
        if (w_in->y >= 0) {
            delta_theta = approach(w_in->y, vel_limit->y, theta_goal.y,
                                   acc_limit->y, delta_t, &w_out->y,
                                   no_overshoot);
            theta_end.y = delta_theta;
        }
        else { // w_in->y < 0
            delta_theta = away(w_in->y, vel_limit->y, theta_goal.y, acc_limit->y,
                               delta_t, &w_out->y, no_overshoot);
            theta_end.y = delta_theta;
        }
    }
        ...
        if (w_in->y <= 0) {
            ...
            delta_theta = away(-w_in->y, vel_limit->y, -theta_goal.y,
                               acc_limit->y, delta_t, &w_out->y, no_overshoot);
            theta_end.y = -delta_theta;
            w_out->y = -w_out->y;
        }
    }
    ...
    if (theta_goal.z > 0) {
        if (w_in->z >= 0) {
            delta_theta = approach(w_in->z, vel_limit->z, theta_goal.z,
                                   acc_limit->z, delta_t, &w_out->z,
                                   no_overshoot);
            theta_end.z = delta_theta;
        }
        else { // w_in->z < 0
            delta_theta = away(w_in->z, vel_limit->z, theta_goal.z, acc_limit->z,
                               delta_t, &w_out->z, no_overshoot);
            theta_end.z = delta_theta;
        }
    }
        ...
        if (w_in->z <= 0) {
            ...
            delta_theta = away(-w_in->z, vel_limit->z, -theta_goal.z,
                               acc_limit->z, delta_t, &w_out->z, no_overshoot);
            theta_end.z = -delta_theta;
            w_out->z = -w_out->z;
        }
    }
    ...
    // arrived at goal?
    if (theta_end.x == theta_goal.x && theta_end.y == theta_goal.y &&
        theta_end.z == theta_goal.z) {
        *next_orient = *goal_orient;
    }
    else {
...
void
get_camera_limits(matrix *start_camera, matrix *end_camera, float time,
                  vector *acc_max, vector *w_max)
{
                           ...
                           vector *vel_limit, vector *acc_limit, int no_overshoot)
{
    ...
    vector
        local_rot_axis; // vector indicating direction of rotation axis (local coords)
    ...
    if (no_bank) { // no rotation on z, so we're done (no rotation of w)
        *next_orient = M_intermed;
        ...
        if (delta_bank > 0) {
            if (w_in->z >= 0) {
                delta_theta = approach(w_in->z, vel_limit->z, delta_bank,
                                       acc_limit->z, delta_t, &w_out->z,
                                       no_overshoot);
                bank = delta_theta;
            }
            else { // w_in->z < 0
                delta_theta = away(w_in->z, vel_limit->z, delta_bank,
                                   acc_limit->z, delta_t, &w_out->z,
                                   no_overshoot);
                bank = delta_theta;
            }
...
vm_forward_interpolate(vector *goal_f, matrix *orient, vector *w_in,
                       ...
                       vector *w_out, vector *vel_limit, vector *acc_limit,
                       int no_overshoot)
{
    matrix Mtemp1; // temporary matrix
    vector
    ...
    // no rotation if delta_bank and w_in both 0 or rotational acc in forward is 0
    no_bank = (delta_bank == 0.0f && vel_limit->z == 0.0f &&
               acc_limit->z == 0.0f);
...
void
vm_find_bounding_sphere(vector *pnts, int num_pnts, vector *center, float *radius)
{
    int i;
...
{
    return is_valid_vec(&m->fvec) && is_valid_vec(&m->uvec) &&
           is_valid_vec(&m->rvec);
}

// interpolate between 2 vectors. t goes from 0.0 to 1.0. at
void
vm_vec_interp_constant(vector *out, vector *v0, vector *v1, float t)
{
    vector cross;
    float total_ang;
...
void
vm_vec_random_cone(vector *out, vector *in, float max_angle, matrix *orient)
{
    vector t1, t2;
    matrix *rot;
    matrix m;
    ...
    vm_rot_point_around_line(&t1, in,
                             fl_radian(frand_range(-max_angle, max_angle)),
                             &vmd_zero_vector, &rot->fvec);

    // axis 2
    vm_rot_point_around_line(&t2, &t1,
                             fl_radian(frand_range(-max_angle, max_angle)),
                             &vmd_zero_vector, &rot->rvec);

    // axis 3
    vm_rot_point_around_line(out, &t2,
                             fl_radian(frand_range(-max_angle, max_angle)),
                             &vmd_zero_vector, &rot->uvec);
}
...
void
vm_vec_random_in_circle(vector *out, vector *in, matrix *orient, float radius,
                        int on_edge)
{
    vector temp;

    // point somewhere in the plane
    vm_vec_scale_add(&temp, in, &orient->rvec,
                     on_edge ? radius : frand_range(0.0f, radius));

    // rotate to a random point on the circle
    vm_rot_point_around_line(out, &temp, fl_radian(frand_range(0.0f, 359.0f)), in,
                             &orient->fvec);
}
...
int
vm_vec_dist_to_line(vector *p, vector *l0, vector *l1, vector *nearest,
                    float *dist)
{
    vector a, b, c;
    float b_mag, comp;
