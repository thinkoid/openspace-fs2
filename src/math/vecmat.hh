...
#define IS_VEC_NULL(v)                                                           \
    (((v)->x == (float)0.0) && ((v)->y == (float)0.0) && ((v)->z == (float)0.0))
...
#define IDENTITY_MATRIX                                                          \
    { (float)1.0, (float)0.0, (float)0.0, (float)0.0, (float)1.0,                \
...
#define vm_vec_add(dst, src0, src1)                                              \
    do {                                                                         \
        (dst)->x = (src0)->x + (src1)->x;                                        \
        (dst)->y = (src0)->y + (src1)->y;                                        \
        (dst)->z = (src0)->z + (src1)->z;                                        \
...
//dest -= k * src
#ifdef _INLINE_VECMAT
#define vm_vec_scale_sub2(dst, src, k)                                           \
    do {                                                                         \
        float tmp_k = (k);                                                       \
        (dst)->x -= (src)->x * tmp_k;                                            \
        (dst)->y -= (src)->y * tmp_k;                                            \
...
#ifdef _INLINE_VECMAT
#define vm_vec_sub(dst, src0, src1)                                              \
    do {                                                                         \
        (dst)->x = (src0)->x - (src1)->x;                                        \
        (dst)->y = (src0)->y - (src1)->y;                                        \
        (dst)->z = (src0)->z - (src1)->z;                                        \
...
vector *vm_vec_avg4(vector *dest, vector *src0, vector *src1, vector *src2,
                    vector *src3);

//scales a vector in place.  returns ptr to vector
#ifdef _INLINE_VECMAT
#define vm_vec_scale(dst, k)                                                     \
    do {                                                                         \
        float tmp_k = (k);                                                       \
        (dst)->x *= tmp_k;                                                       \
        (dst)->y *= tmp_k;                                                       \
...
#ifdef _INLINE_VECMAT
#define vm_vec_copy_scale(dst, src, k)                                           \
    do {                                                                         \
        float tmp_k = (k);                                                       \
        (dst)->x = (src)->x * tmp_k;                                             \
        (dst)->y = (src)->y * tmp_k;                                             \
...
//dest = src1 + k * src2
#ifdef _INLINE_VECMAT
#define vm_vec_scale_add(dst, src1, src2, k)                                     \
    do {                                                                         \
        float tmp_k = (k);                                                       \
...
//dest += k * src
#ifdef _INLINE_VECMAT
#define vm_vec_scale_add2(dst, src, k)                                           \
    do {                                                                         \
        float tmp_k = (k);                                                       \
        (dst)->x += (src)->x * tmp_k;                                            \
        (dst)->y += (src)->y * tmp_k;                                            \
...
float vm_vec_projection_parallel(vector *component, vector *src,
                                 vector *unit_vector);

// finds the projection of source vector onto a surface given by surface normal
void vm_vec_projection_onto_plane(vector *projection, vector *src,
                                  vector *normal);
...
#ifdef _INLINE_VECMAT
#define vm_vec_dotprod(v0, v1)                                                   \
    (((v1)->x * (v0)->x) + ((v1)->y * (v0)->y) + ((v1)->z * (v0)->z))
#define vm_vec_dot(v0, v1)                                                       \
    (((v1)->x * (v0)->x) + ((v1)->y * (v0)->y) + ((v1)->z * (v0)->z))
#else
...
matrix *vm_vector_2_matrix_norm(matrix *m, vector *fvec, vector *uvec,
                                vector *rvec);
...
void vm_rot_point_around_line(vector *out, vector *in, float angle,
                              vector *line_point, vector *line_dir);
...
void vm_matrix_interpolate(matrix *goal_orient, matrix *start_orient,
                           vector *rotvel_in, float delta_t, matrix *next_orient,
...
void vm_forward_interpolate(vector *goal_fvec, matrix *orient, vector *rotvel_in,
                            float delta_t, float delta_bank, matrix *next_orient,
                            vector *rotvel_out, vector *vel_limit,
                            vector *acc_limit, int no_overshoot = 0);
...
vector *vm_rotate_vec_to_world(vector *world_vec, vector *body_vec,
                               matrix *orient);
...
void vm_vec_random_cone(vector *out, vector *in, float max_angle,
                        matrix *orient = NULL);
...
void vm_vec_random_in_circle(vector *out, vector *in, matrix *orient,
...
int vm_vec_dist_to_line(vector *p, vector *l0, vector *l1, vector *nearest,
                        float *dist);
