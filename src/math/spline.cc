...
// set control points
void
bez_spline::bez_set_points(int _num_pts, vector *_pts[MAX_BEZ_PTS])
{
    int idx;
...
// blend function
#define COMB(_n, _k)                                                             \
    ((float)bez_fact(_n) / (float)(bez_fact(_k) * bez_fact(_n - _k)))
float
bez_spline::BEZ(int k, int n, float u)
{
...
void
bez_spline::bez_get_point(vector *out, float u)
{
    int idx;
    float bez_val;
...
// render a bezier
void
bez_spline::bez_render(int divs, color *c)
{
    float inc;
    int idx;
    vertex a, b;
...
// constructor
herm_spline::herm_spline(int _num_pts, vector *_pts[MAX_HERM_PTS],
...
// set the points
void
herm_spline::herm_set_points(int _num_pts, vector *_pts[MAX_HERM_PTS],
                             vector *_d_pts[MAX_HERM_PTS])
{
    int idx;
...
void
herm_spline::herm_get_point(vector *out, float u, int k)
{
...
void
herm_spline::herm_get_deriv(vector *deriv, float u, int k)
{
...
// render a bezier
void
herm_spline::herm_render(int divs, color *clc)
{
    int idx;
    int s_idx;
