...
void accurate_square_root(float A, float B, float C, float discriminant,
                          float *root1, float *root2);

float
matrix_determinant_from_vectors(vector *v1, vector *v2, vector *v3)
{
    float ans;
    ans = v1->x * v2->y * v3->z;
    ans += v2->x * v3->y * v1->z;
...
//
void
fvi_two_lines_in_3space(vector *p1, vector *v1, vector *p2, vector *v2, float *s,
                        float *t)
{
    vector cross, delta;
...
int
fvi_segment_plane(vector *new_pnt, vector *plane_pnt, vector *plane_norm,
                  vector *p0, vector *p1, float rad)
{
    ...
    t = fvi_ray_plane(new_pnt, plane_pnt,
...
//else returns 0
int
fvi_segment_sphere(vector *intp, vector *p0, vector *p1, vector *sphere_pos,
                   float sphere_rad)
{
...
//else returns 0
int
fvi_ray_sphere(vector *intp, vector *p0, vector *p1, vector *sphere_pos,
               float sphere_rad)
{
    vector d, dn, w, closest_point;
    float mag_d, dist, w_dist, int_dist;
...
int
fvi_ray_boundingbox(vector *min, vector *max, vector *p0, vector *pdir,
                    vector *hitpt)
{
...
int
fvi_point_face(vector *checkp, int nv, vector **verts, vector *norm1,
...
int check_sphere_point(vector *point, vector *sphere_start, vector *sphere_vel,
...
int
fvi_sphere_plane(vector *intersect_point, vector *sphere_center_start,
                 ...
                 float *crossing_time)
{
    ...
    *hit_time = t1;
    ...
    if (t1 > 0 && t1 < 1) {
        ...
        vm_project_point_onto_plane(intersect_point, &v_temp, plane_normal,
                                    plane_point);
    }
...
fvi_sphere_perp_edge(vector *intersect_point, vector *sphere_center_start,
                     vector *sphere_velocity, float sphere_radius,
                     vector *edge_point1, vector *edge_point2,
                     float *collide_time)
{
    ...
    return (check_sphere_point(intersect_point, sphere_center_start,
                               sphere_velocity, sphere_radius, collide_time));
}
...
int
fvi_polyedge_sphereline(vector *hit_point, vector *xs0, vector *vs, float Rs,
                        int nv, vector **verts, float *hit_time)
{
    ...
    for (i = 0; i < nv; i++) {
        ...
        C = delta_x_dot_ve * delta_x_dot_ve + Rs * Rs * ve_sqr -
            delta_x_sqr * ve_sqr;
        ...
        C = 2 * delta_x_dot_ve * delta_x_dot_vs * ve_dot_vs +
            Rs * Rs * ve_dot_vs * ve_dot_vs -
            delta_x_sqr * ve_dot_vs * ve_dot_vs -
        ...
        if ((root1 >= 0) && (root1 <= 1)) {
            ...
            if (fl_abs(q - Rs * Rs) <
                0.2 * Rs) { // error less than 0.1m absolute (2*delta*Radius)
                goto Hit;
            }
        }
...
fvi_closest_point_on_line_segment(vector *closest_point, vector *fixed_point,
                                  vector *line_point1, vector *line_point2)
{
    ...
    t = -vm_vec_dotprod(&delta_x, &line_velocity) /
                         ...
                         vector *sphere_end, float sphere_r)
{
    ...
    fvi_closest_point_on_line_segment(&closest_point, poly_center, sphere_start,
                                      sphere_end);
...
//
void
fvi_closest_line_line(vector *x0, vector *vx, vector *y0, vector *vy,
...
// --------------------------------------------------------------------------------------------------------------------
void
accurate_square_root(float A, float B, float C, float discriminant, float *root1,
                     float *root2)
{
...
int
project_point_onto_bbox(vector *mins, vector *maxs, vector *start, vector *box_pt)
{
    int inside = TRUE;
