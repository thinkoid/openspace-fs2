...
void
anim_check_for_palette_change(anim_instance *instance)
{
...
anim_instance *
init_anim_instance(anim *ptr, int bpp)
{
    anim_instance *inst;
...
void
free_anim_instance(anim_instance *inst)
{
    Assert(inst->frame);
    free(inst->frame);
    inst->frame = NULL;
...
int
anim_get_next_frame(anim_instance *inst)
{
    int bm, bitmap_flags;
    int aabitmap = 0;
    int bpp = 16;
    ...
    if (anim_instance_is_streamed(inst)) {
        ...
        inst->data = unpack_frame(inst, inst->data, inst->frame,
                                  inst->parent->width * inst->parent->height,
                                  inst->parent->palette_translation, aabitmap,
                                  bpp);
    }

    bm = bm_create(bpp, inst->parent->width, inst->parent->height, inst->frame,
                   bitmap_flags);
    bm_unload(bm);
    return bm;
}
...
ubyte *
anim_get_next_raw_buffer(anim_instance *inst, int xlate_pal, int aabitmap,
                         int bpp)
{
    if (anim_instance_is_streamed(inst)) {
        if (inst->file_offset < 0) {
...
//
int
anim_get_frame(anim_instance *inst, int frame_num, int xlate_pal)
{
    /*
...
int
pack_key_frame(ubyte *frame, ubyte *save, long size, long max, int compress_type)
{
...
// returns: actual number of bytes data packed to or -1 if error
int
pack_frame(ubyte *frame, ubyte *frame2, ubyte *save, long size, long max,
           int compress_type)
{
...
int
unpack_pixel(anim_instance *ai, ubyte *data, ubyte pix, int aabitmap, int bpp)
{
    int bit_24;
    ushort bit_16 = 0;
    ubyte bit_8 = 0;
...
// unpack a pixel given the passed index and the anim_instance's palette, return bytes stuffed
int
unpack_pixel_count(anim_instance *ai, ubyte *data, ubyte pix, int count,
...
ubyte *
unpack_frame(anim_instance *ai, ubyte *ptr, ubyte *frame, int size,
    ...
    if (*ptr == PACKING_METHOD_RLE_KEY) { // key frame, Hoffoss's RLE format
        ...
        while (size > 0) {
            ...
            if (value != packer_code) {
                if (xlate_pal) {
                    stuffed = unpack_pixel(ai, frame, pal_translate[value],
                                           aabitmap, bpp);
                }
                else {
                    stuffed = unpack_pixel(ai, frame, (ubyte)value, aabitmap,
                                           bpp);
                }
                frame += stuffed;
                size--;
                ...
                if (xlate_pal) {
                    stuffed = unpack_pixel_count(ai, frame, pal_translate[value],
                                                 count, aabitmap, bpp);
                }
                else {
                    stuffed = unpack_pixel_count(ai, frame, (ubyte)value, count,
                                                 aabitmap, bpp);
                }
        ...
        while (size > 0) {
            ...
            if (value != packer_code) {
                ...
                if (value != transparent_code) {
                    if (xlate_pal) {
                        stuffed = unpack_pixel_count(ai, frame,
                                                     pal_translate[value], count,
                                                     aabitmap, bpp);
                    }
                    else {
                        stuffed = unpack_pixel_count(ai, frame, (ubyte)value,
                                                     count, aabitmap, bpp);
                    }
                }
                else {
             ...
             PACKING_METHOD_STD_RLE) { // normal frame, with high bit as count
        ptr++;
        while (size > 0) {
            value = *ptr++;
            if (!(value & STD_RLE_CODE)) {
...
// pal_translate = color translation lookup table (NULL if no palette translation desired)
int
unpack_frame_from_file(anim_instance *ai, ubyte *frame, int size,
                       ubyte *pal_translate, int aabitmap, int bpp)
{
    int xlate_pal, value, count = 0;
    int offset = 0;
...
anim_set_palette(anim *ptr)
{
    ...
    for (i = 0; i < 256; i++) {
        int is_xparent = (ptr->palette[i * 3] == ptr->xparent_r) &&
                         (ptr->palette[i * 3 + 1] == ptr->xparent_g) &&
                         (ptr->palette[i * 3 + 2] == ptr->xparent_b);
        ...
        if (gr_screen.bits_per_pixel == 8) {
            if (is_xparent) {
                ...
                ptr->palette_translation[i] = (ubyte)palette_find(
                    ptr->palette[i * 3], ptr->palette[i * 3 + 1],
                    ptr->palette[i * 3 + 2]);
            }
        }
        else {
