...
//
void
anim_render_all(int screen_id, float frametime)
{
    anim_instance *A;
    anim_instance *temp;
...
anim_play(anim_play_struct *aps)
{
    ...
    instance->frame = (ubyte *)malloc(instance->parent->width *
                                      instance->parent->height * 2);
...
anim_show_next_frame(anim_instance *instance, float frametime)
{
    ...
    if (instance->direction == ANIM_DIRECT_FORWARD)
        new_frame_num = instance->start_at - 1 +
                        fl2i(percent_through * n_frames + 0.5f);
    else
        new_frame_num = instance->start_at - 1 -
                        fl2i(percent_through * n_frames + 0.5f);
    ...
    bpp = (gr_screen.mode == GR_SOFTWARE)
              ? 8
              : 16; // unpack must match the bm_create below
        ...
        for (i = 0; i < frame_diff; i++) {
            ...
            if (anim_instance_is_streamed(instance)) {
                if (instance->xlate_pal) {
                    ...
                    temp_file_offset = unpack_frame_from_file(
                        instance, instance->frame,
                        instance->parent->width * instance->parent->height, NULL,
                        aabitmap, bpp);
                }
            }
            else {
                if (instance->xlate_pal) {
                    ...
                    temp = unpack_frame(instance, instance->data, instance->frame,
                                        instance->parent->width *
    ...
    if (frame_diff == 0 && instance->last_bitmap != -1) {
        ...
        bitmap_id = bm_create((gr_screen.mode == GR_SOFTWARE) ? 8 : 16,
                              instance->parent->width, instance->parent->height,
...
//
void
anim_release_render_instance(anim_instance *instance)
{
    Assert(instance != NULL);
...
anim_read_header(anim *ptr, CFILE *fp)
{
    ...
    float waste = 100.0f * float((floor_size - diff)) /
                  (2.0f * (float)floor_size);

    if (diff != 0) {
        if (ptr->height > 16) {
            mprintf(("ANI %s with size %dx%d (%.1f%% wasted)\n", ptr->name,
                     ptr->height, ptr->height, waste));
        }
    }
#endif
...
anim_write_frames_out(char *filename)
{
    ...
    for (i = 0; i < source_anim->total_frames; i++) {
        ...
        pcx_write_bitmap(pcxname, source_anim->width, source_anim->height,
                         row_data, source_anim->palette);
...
void
anim_reverse_direction(anim_instance *ai)
{
    int temp;
    ...
    if (ai->direction == ANIM_DIRECT_FORWARD) {
        ai->time_elapsed = ((float)ai->frame_num - (float)ai->start_at - 1.0f) /
                           (float)ai->parent->fps;
    }
    else if (ai->direction == ANIM_DIRECT_REVERSE) {
        ai->time_elapsed = ((float)ai->start_at - (float)ai->frame_num - 1.0f) /
                           (float)ai->parent->fps;
    }
}

void
anim_pause(anim_instance *ai)
{
    ai->paused = 1;
}

void
anim_unpause(anim_instance *ai)
{
    ai->paused = 0;
}
...
int
anim_instance_is_streamed(anim_instance *ai)
{
    Assert(ai);
...
unsigned char
anim_instance_get_byte(anim_instance *ai, int offset)
{
    int absolute_offset;
    anim *parent;
