...
physics_info
    Descent_physics; // used when we want to control the player like the descent ship

////////////////////////////////////////////////////////////
// Module data
////////////////////////////////////////////////////////////
static int Player_all_alone_msg_inited =
    0; // flag used for initalizing a player-specific voice msg
...
void
view_modify(angles *ma, angles *da, float minv, float maxv, int slew,
            float frame_time)
{
    int axis[JOY_NUM_AXES];
    float t;

    if ((!slew) && (Viewer_mode & VM_EXTERNAL) &&
    ...
    t = (check_control_timef(PITCH_FORWARD) - check_control_timef(PITCH_BACK)) /
        16.0f;
    if (t != 0.0f)
        da->p += t;
    else
...
{
    view_modify(&Viewer_slew_angles, &Viewer_slew_angles_delta, -PI / 3, PI / 3,
                1, frame_time);

    if ((Viewer_slew_angles.p == 0.0f) && (Viewer_slew_angles.b == 0.0f) &&
...
void
do_view_chase(float frame_time)
{
    float t;
...
void
do_view_external(float frame_time)
{
    float t;
...
void
do_thrust_keys(control_info *ci)
{
...
void
player_control_reset_ci(control_info *ci)
{
...
void
playercontrol_read_stick(int *axis, float frame_time)
{
    int i;

#ifndef NDEBUG
    // Make sure things get reset properly between missions.
    if ((Joystick_last_reading != -1) &&
        (timestamp_until(Joystick_last_reading) > 1000)) {
    ...
    if ((Joystick_last_reading == -1) ||
        timestamp_elapsed(Joystick_last_reading)) {
        // Read the stick
        control_get_axes_readings(&Joystick_saved_reading[0],
                                  &Joystick_saved_reading[1],
                                  &Joystick_saved_reading[2],
                                  &Joystick_saved_reading[3],
                                  &Joystick_saved_reading[4]);
        Joystick_last_reading = timestamp(
            1000 / 10); // Read 10x per second, like we did in Descent.
    }
...
void
read_keyboard_controls(control_info *ci, float frame_time, physics_info *pi)
{
    ...
    if (ok_to_read_ci_pitch_yaw) {
        ...
        ci->vertical = (key_down_timef(KEY_PADPLUS) -
                        key_down_timef(KEY_PADENTER));
        ...
        if (Player->flags & PLAYER_FLAGS_MATCH_TARGET) {
            if ((Player_ai->last_target == Player_ai->target_objnum) &&
                ...
                if (tspeed < MATCH_SPEED_THRESHOLD) {
                    ...
                    aip =
                        &Ai_info[Ships[Objects[Player_ai->target_objnum].instance]
                                     .ai_index];
...
*/
        ...
        if (Axis_map_to[JOY_REL_THROTTLE_AXIS] >= 0)
            ci->forward_cruise_percent += f2fl(axis[JOY_REL_THROTTLE_AXIS]) *
                                          100.0f * frame_time;
        ...
        if (check_control(AFTERBURNER)) {
            if (!afterburner_last) {
                Assert(Player_ship);
                if (!(Ship_info[Player_ship->ship_info_index].flags &
                      SIF_AFTERBURNER)) {
...
read_player_controls(object *objp, float frametime)
{
...

    {
        ...
        case PCM_WARPOUT_STAGE3: // Go 40 km/s steady through the effect
            ...
            memset(&(Player->ci), 0,
                   sizeof(control_info)); // set the controls to 0
            ...
            if ((objp->type == OBJ_SHIP) && (!(Game_mode & GM_DEAD))) {
                ...
                if (Warpout_forced) {
                    ...
                    if (Ships[objp->instance].flags &
                        (SF_WARP_BROKEN | SF_WARP_NEVER)) {
                        HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                                           XSTR("Cannot warp out at this time.",
                                                81));
                        warp_failed = 1;
                    }
                    else {
                        ...
                        if (can_warp) {
                            float diff = TARGET_WARPOUT_SPEED -
        ...
        if (!(Ships[Player_obj->instance].flags & SF_DYING)) {
            ...
            if ((Ships[objp->instance].wash_intensity > 0) &&
                ...
                float intensity = 0.3f *
                                  min(Ships[objp->instance].wash_intensity, 1.0f);
                vm_vec_copy_scale(&wash_rot, &Ships[objp->instance].wash_rot_axis,
                                  intensity);
                          ...
                          char *match_on_text)
{
    ...
    if (Player->flags & PLAYER_FLAGS_MATCH_TARGET) {
        ...
        if (Objects[Player_ai->target_objnum].phys_info.speed >
            ...
            ai_info *aip;
            aip = &Ai_info[Ships[Objects[Player_ai->target_objnum].instance]
                               .ai_index];
            if (aip->ai_flags & AIF_DOCKED) {
                Assert(aip->dock_objnum != -1);
                if (Objects[aip->dock_objnum].phys_info.fspeed >
                    MATCH_SPEED_THRESHOLD) {
                    can_match = 1;
                }
            }
...
toggle_player_object()
{
    ...
    HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                       NOX("Using %s style physics for player ship."),
...
void
player_set_pilot_defaults(player *p)
{
...
player_level_init()
{
    ...
    Player->allow_warn_timestamp =
        1; // init timestamp that is used for managing attack warnings sent to player
    ...
    Player->request_repair_timestamp =
        1; // timestamp until next 'requesting repair sir' message can be played
...
player_process_pending_praise()
{
    if (timestamp_elapsed(Player->praise_delay_timestamp)) {
        ...
        if (ship_index >= 0) {
            ...
            if (Objects[Ships[ship_index].objnum].hull_strength /
                    Ship_info[Ships[ship_index].ship_info_index]
                        .initial_hull_strength >
                0.5f) {
                message_send_builtin_to_player(MESSAGE_PRAISE, &Ships[ship_index],
                                               MESSAGE_PRIORITY_HIGH,
                                               MESSAGE_TIME_SOON, 0, 0);
                Player->allow_praise_timestamp = timestamp(
                    PLAYER_ALLOW_PRAISE_INTERVAL * (Game_skill_level + 1));
                Player->allow_scream_timestamp = timestamp(
                    20000); // prevent death scream following praise
                Player->praise_count++;
                return 1;
            }
        }
...
player_inspect_cargo(float frametime, char *outstr)
{
    ...
    if (Player_ai->current_target_distance <
        max(CARGO_REVEAL_MIN_DIST, (cargo_objp->radius + CARGO_RADIUS_DELTA))) {
...
player_inspect_cap_subsys_cargo(float frametime, char *outstr)
{
    ...
    if (Player_ai->current_target_distance <
        max(CAP_CARGO_REVEAL_MIN_DIST,
            (subsys_rad + CAPITAL_CARGO_RADIUS_DELTA))) {
        // check if player is facing cargo, do not proceed with inspection if not
        vm_vec_normalized_dir(&vec_to_cargo, &subsys_pos, &Player_obj->pos);
        dot = vm_vec_dot(&vec_to_cargo, &Player_obj->orient.fvec);
        int hud_targetbox_subsystem_in_view(object * target_objp, int *sx,
                                            int *sy);
...
char *
player_generate_death_text(player *player_p, char *death_text)
{
    char weapon_name[NAME_LENGTH];
    weapon_name[0] = 0;

    player_generate_killer_weapon_name(player_p->killer_weapon_index,
                                       player_p->killer_species, weapon_name);
    ...
    case OBJ_WEAPON:
        ...
        if ((ship_index >= 0) && (Player_ship != NULL) &&
            ...
            sprintf(death_text,
                    XSTR("%s was killed by friendly fire from %s", 1338),
                    player_p->callsign, player_p->killer_parent_name);
        }
        else {
            sprintf(death_text, XSTR("%s was killed by %s", 94),
                    player_p->callsign, player_p->killer_parent_name);
        }
        break;
    case OBJ_SHIP:
        if (player_p->flags & PLAYER_FLAGS_KILLED_BY_EXPLOSION) {
            sprintf(death_text,
                    XSTR("%s was killed by a blast from %s exploding", 95),
                    player_p->callsign, player_p->killer_parent_name);
        }
        else if (player_p->flags & PLAYER_FLAGS_KILLED_BY_ENGINE_WASH) {
            sprintf(death_text,
                    XSTR("%s was killed by engine wash from %s", 1494),
                    player_p->callsign, player_p->killer_parent_name);
        }
        else {
            sprintf(death_text, XSTR("%s was killed by a collision with %s", 96),
                    player_p->callsign, player_p->killer_parent_name);
        }
        break;
    case OBJ_DEBRIS:
        sprintf(death_text, XSTR("%s was killed by a collision with debris", 97),
                player_p->callsign);
        break;
    case OBJ_ASTEROID:
        sprintf(death_text,
                XSTR("%s was killed by a collision with an asteroid", 98),
                player_p->callsign);
        break;
    case OBJ_BEAM:
        if (strlen(player_p->killer_parent_name) <= 0) {
            ...
            if ((ship_index >= 0) && (Player_ship != NULL) &&
                (Player_ship->team == Ships[ship_index].team)) {
                sprintf(death_text,
                        XSTR("%s was destroyed by friendly beam fire from %s",
                             1339),
                        player_p->callsign, player_p->killer_parent_name);
            }
            else {
                sprintf(death_text,
                        XSTR("%s was destroyed by a beam from %s", 1082),
                        player_p->callsign, player_p->killer_parent_name);
            }
        }
        break;
...
player_show_death_message()
{
    ...
    if (Player->flags & PLAYER_KILLED_SELF) {
        ...
        if (Player->flags & PLAYER_FLAGS_KILLED_SELF_SHOCKWAVE) {
            ...
            sprintf(death_text,
                    XSTR("You have killed yourself with your own missiles",
                         1422));
        }
        else {
        ...
        Player->flags &= ~(PLAYER_FLAGS_KILLED_SELF_MISSILES |
                           PLAYER_FLAGS_KILLED_SELF_SHOCKWAVE);
    }
    else {
...
void
player_maybe_fire_turret(object *objp)
{
    ...
    if (aip->ai_flags & (AIF_AWAITING_REPAIR | AIF_BEING_REPAIRED)) {
        if (aip->dock_objnum > -1) {
            if (vm_vec_dist_quick(&objp->pos, &Objects[aip->dock_objnum].pos) <
                (objp->radius + Objects[aip->dock_objnum].radius) * 1.25f)
                return;
        }
    }
...
player_maybe_play_all_alone_msg()
{
    ...
    if (rand() & 1) {
        message_send_builtin_to_player(MESSAGE_ALL_ALONE, NULL,
                                       MESSAGE_PRIORITY_HIGH,
...
void
player_get_padlock_orient(matrix *eye_orient)
{
...
player_display_packlock_view()
{
    ...
    if (!(Viewer_mode & (VM_CHASE | VM_EXTERNAL | VM_SLEWED))) {
        ...
        case 0:
            strcpy(str, XSTR("top view", 101));
            break;
        case 1:
            strcpy(str, XSTR("rear view", 102));
            break;
        case 2:
            strcpy(str, XSTR("left view", 103));
            break;
        case 3:
            strcpy(str, XSTR("right view", 104));
            break;
        }
...
void
player_get_eye(vector *eye_pos, matrix *eye_orient)
{
    object *viewer_obj;
    vector eye_dir;
