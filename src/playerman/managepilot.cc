...
void read_stats_block(CFILE *file, int Player_file_version,
...
void
delete_pilot_file(char *pilot_name, int single)
{
...
// used in barracks and player_select
int
verify_pilot_file(char *filename, int single, int *rank)
{
    CFILE *file;
...
void
pilot_write_techroom_data(CFILE *file)
{
    int idx;
    ubyte out;
    ...
    for (idx = 0; idx < Num_weapon_types; idx++) {
        out = (Weapon_info[idx].wi_flags & WIF_IN_TECH_DATABASE) ? (ubyte)1
                                                                 : (ubyte)0;
        cfwrite_ubyte(out, file);
    }
...
void
pilot_read_techroom_data(CFILE *file)
{
    int idx;
...
void
pilot_write_loadout(CFILE *file)
{
    int i, j;
    wss_unit *slot;
...
int
read_pilot_file(char *callsign, int single, player *p)
{
    ubyte num_ctrls;
    ubyte is_multi = 0;
    ...
    Assert(strlen(callsign) <
           MAX_FILENAME_LEN - 4); // ensure we won't overrun the buffer
    ...
    if (is_multi) {
        ...
        p->flags &=
            ~PLAYER_FLAGS_IS_MULTI; // this takes care of unsetting any leftover bits from a (possibly) previously selected pilot
    }
    ...
    if (p->last_ship_flown_si_index < 0) {
        nprintf((
            "Warning",
            "WARNING => Ship class %s not located in Ship_info[] in player file\n",
            ship_name));
    ...
    if (Player_file_version < 137) {
        ...
        memcpy(&HUD_config.clr[NUM_HUD_GAUGES - 2], &HUD_config.clr[HUD_RADAR],
               sizeof(color));
        memcpy(&HUD_config.clr[NUM_HUD_GAUGES - 1], &HUD_config.clr[HUD_RADAR],
               sizeof(color));
    }
    else {
...
void
read_stats_block(CFILE *file, int Player_file_version, scoring_struct *stats)
{
    int i, total;
    ...
    if (total > MAX_SHIP_TYPES) {
        Warning(
            LOCATION,
            "Some ship kill information will be lost due to MAX_SHIP_TYPES decrease");
    }
...
int
write_pilot_file_core(player *p)
{
...
int
write_pilot_file(player *the_player)
{
    ...
    do {
        ...
        if (pilot_write_rval) {
            int popup_rval = popup(
                PF_TITLE_RED | PF_TITLE_BIG, 3, XSTR("&Retry", 41),
                XSTR("&Ignore", 42), XSTR("&Quit Game", 43),
                XSTR(
...
void
write_stats_block(CFILE *file, scoring_struct *stats)
{
    int i;
    int total;
...
void
write_multiplayer_options(player *p, CFILE *file)
{
...
void
read_multiplayer_options(player *p, CFILE *file)
{
...
void
init_new_pilot(player *p, int reset)
{
    int cur_speed;
...
void
pilot_set_short_callsign(player *p, int max_width)
{
...
// pick a random image for the passed player
void
pilot_set_random_squad_pic(player *p)
{
    // if there are no available pilot pics, set the image filename to null
...
void
pilot_format_callsign_personal(char *in_callsign, char *out_callsign)
{
...
int
pilot_verify_overwrite()
{
    return popup(PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON | PF_TITLE_RED |
                     PF_TITLE_BIG,
...
pilot_load_pic_list()
{
    ...
    Num_pilot_images = cf_get_file_list_preallocated(
        ...
        CF_TYPE_PLAYER_IMAGES_MAIN, NOX("*.pcx"));
...
pilot_load_squad_pic_list()
{
    ...
    Num_pilot_squad_images = cf_get_file_list_preallocated(
        ...
        CF_TYPE_SQUAD_IMAGES_MAIN, NOX("*.pcx"));

    // sort all filenames
    cf_sort_filenames(Num_pilot_squad_images, Pilot_squad_image_names,
                      CF_SORT_NAME);
}
...
// set squadron
void
player_set_squad(player *p, char *squad_name)
{
    // sanity check
    if (p == NULL) {
        return;
