        ...
        while (!check_for_string("#")) {
            ...
            if (i >= 0) {
                ...
                if (buf[i] != '"') {
                    ...
                    buf[i + 1] =
                        0; // drop down a null terminator (prolly unnecessary)
                    while (!is_white_space(buf[i]))
                        i--; // back up over the potential offset
                    while (is_white_space(buf[i]))
                        i--; // now back up over intervening spaces
                    num_offsets_on_this_line = 1;
                    if (buf[i] != '"') {
                        // could have a 2nd offset value (one for 640, one for 1024)
                        // so back up again
                        while (!is_white_space(buf[i]))
                            i--; // back up over the 2nd offset
                        while (is_white_space(buf[i]))
                            i--; // now back up over intervening spaces
                    ...
                    p_offset =
                        &buf[i + 1]; // get ptr to string section with offset in it
                    if (buf[i] != '"') {
                        Error(LOCATION,
                              NOX("Strings.tbl is corrupt")); // now its an error
                    }
                }
...
void
lcl_add_dir_to_path_with_filename(char *current_path)
{
...
void
lcl_ext_localize(char *in, char *out, int max_len, int *id)
{
    char first_four[5];
    char text_str[2048] = "";
    ...
    if ((Lcl_ext_file == NULL) || (str_id < 0) ||
        (Lcl_current_lang == LCL_DEFAULT_LANGUAGE)) {
        strcpy(out, text_str);
        if (id != NULL) {
            *id = str_id;
        }
    ...
    if (lcl_ext_lookup(lookup_str, str_id)) {
        ...
        if (strlen(lookup_str) > (unsigned int)(max_len - 1)) {
            ...
            out[max_len - 1] =
                '\0'; // ensure null terminator, since strncpy(...) doesnt.
        }
        else {
...
int
lcl_get_xstr_offset(int index, int res)
{
    if (res == GR_640) {
...
// given a valid XSTR() tag piece of text, extract the string portion, return it in out, nonzero on success
int
lcl_ext_get_text(char *xstr, char *out)
{
...
// given a valid XSTR() tag piece of text, extract the id# portion, return the value in out, nonzero on success
int
lcl_ext_get_id(char *xstr, int *out)
{
    char *p, *pnext;
    int str_len;
...
// given a valid XSTR() id#, lookup the string in tstrings.tbl, filling in out if found, nonzero on success
int
lcl_ext_lookup(char *out, int id)
{
    char text[1024];
    int ret;
    int pointer;
...
int
lcl_is_valid_numeric_char(char c)
{
    return ((c == '-') || (c == '0') || (c == '1') || (c == '2') || (c == '3') ||
            (c == '4') || (c == '5') || (c == '6') || (c == '7') || (c == '8') ||
            (c == '9'))
               ? 1
               : 0;
...
lcl_ext_setup_pointers()
{
    ...
    if (Lcl_ext_file == NULL) {
        error_display(
            0,
            "Error opening externalization file! File likely does not exist or could not be found");
        return;
    }
    ...
    while (cfgets(line, 1024, Lcl_ext_file)) {
        ...
        case 3:
            ...
            Lcl_pointers[Lcl_pointer_count++] = cftell(Lcl_ext_file) -
                                                strlen(line) - 1;
            lcl_ext_close();
            return;
        }
...
void
lcl_get_language_name(char *lang_name)
{
...
char *
lcl_fix_umlauts(char *str, int which_way)
{
    int i = 0;
...
//
void
lcl_translate_wep_name(char *name)
{
...
//
void
lcl_translate_brief_icon_name(char *name)
{
    char *pos;
    char buf[128];
...
//
void
lcl_translate_ship_name(char *name)
{
...
//
void
lcl_translate_targetbox_name(char *name)
{
    char *pos;
    char buf[128];
