...
typedef struct fhash_node
{
    ...
    int id; // can be -1
    fhash_node *next,
        *prev; // for chaining in an individual hash table entry (non-circular, doubly linked list)
} fhash_node;
...
// insert a string into hash table index N, will take care of allocating/chaining everything
void
fhash_insert(char *str, int id, int n)
{
    fhash_node *new_node;
    fhash_node *moveup;
