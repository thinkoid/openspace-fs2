...
void
gameplay_help_blit_control_line(int x, int y, int id)
{
    int has_key = 0, has_joy = 0;
    char buf[256];
    config_item *ci;
...
void
gameplay_help_blit_control_line_raw(int x, int y, char *control_text,
...
void
gameplay_help_set_title(char *title)
{
    int sy = TITLE_Y;
    char buf[128];
    ...
    sprintf(buf, XSTR("Page %d of %d", 132), Current_help_page + 1,
            Gp_last_screen + 1);
...
gameplay_help_init()
{
    ...
    Buttons[gr_screen.res][CONTINUE_BUTTON].button.set_hotkey(KEY_CTRLED |
                                                              KEY_ENTER);
...
void
gameplay_help_process_key(int k)
{
    switch (k) {
    case KEY_ESC:
        gameplay_help_leave();
...
void
gameplay_help_button_pressed(int n)
{
    switch (n) {
...
gameplay_help_draw_text()
{
    ...
    case GP_HELP_ADVANCED_TARGET_KEYS:
        ...
        y_offset += separation;
        gameplay_help_blit_control_line_raw(x_offset, y_offset,
...
*/
    ...
    case GP_HELP_MISC_KEYS:
        ...
        y_offset += separation;
        gameplay_help_blit_control_line_raw(x_offset, y_offset, XSTR("ESC", 160),
                                            XSTR("invoke abort mission popup",
                                                 161));
    ...
    case GP_HELP_MULTI_KEYS:
        ...
        // ingame messaging
        gr_set_color_fast(&Color_bright);
        gr_printf(0x8000, y_offset,
                  XSTR("Ingame messaging keys (tap for text, hold for voice)",
                       168));
        gr_set_color_fast(&Color_normal);
...
void
gameplay_help_do_frame(float frametime)
{
    int i, k;
