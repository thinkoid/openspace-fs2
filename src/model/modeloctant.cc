...
int
point_in_octant(polymodel *pm, model_octant *oct, vector *vert)
{
    if (vert->x < oct->min.x)
        return 0;
    if (vert->x > oct->max.x)
...
void
model_octant_find_shields(polymodel *pm, model_octant *oct)
{
    int i, j, n;
    shield_tri *tri;
    ...
    oct->nshield_tris = n;
    oct->shield_tris = (shield_tri **)malloc(sizeof(shield_tri *) *
                                             oct->nshield_tris);
...
void
moff_tmappoly(ubyte *p, polymodel *pm, model_octant *oct, int just_count)
{
    int i, nv;
    ...
    if (pm->version < 2003) {
        ...
        *vp(p + 20) = center_point;
        ...
        for (i = 0; i < nv; i++) {
            float dist = vm_vec_dist(&center_point,
                                     Interp_verts[verts[i].vertnum]);
            if (dist > rad) {
                rad = dist;
            }
        }
...
int
model_octant_find_faces_sub(polymodel *pm, model_octant *oct, void *model_ptr,
                            int just_count)
{
...
void
model_octant_find_faces(polymodel *pm, model_octant *oct)
{
    ubyte *p;
...
void
model_octant_create(polymodel *pm)
{
...
void
model_octant_free(polymodel *pm)
{
    int i;
...
// data.
int
model_which_octant_distant_many(vector *pnt, int model_num, matrix *model_orient,
                                vector *model_pos, polymodel **pm, int *octs)
{
    vector tempv, rotpnt;
    ...
    *pm = model_get(model_num);
    ...
    if (rotpnt.x > center.x)
        x = 1;
    else
        x = 0;
    if (rotpnt.y > center.y)
        y = 1;
    else
        y = 0;
    if (rotpnt.z > center.z)
        z = 1;
    else
        z = 0;
...
// data.
int
model_which_octant_distant(vector *pnt, int model_num, matrix *model_orient,
...
int
model_which_octant(vector *pnt, int model_num, matrix *model_orient,
                   vector *model_pos, model_octant **oct)
{
    polymodel *pm;
    vector tempv, rotpnt;
