...
static vector
    Mc_p0; // The ray origin rotated into the current submodel's frame of reference
static vector
    Mc_p1; // The ray end rotated into the current submodel's frame of reference
static float Mc_mag; // The length of the ray
static vector
    Mc_direction; // A vector from the ray's origin to it's end, in the current submodel's frame of reference

static vector *Mc_point_list
    [MAX_POLYGON_VECS]; // A pointer to the current submodel's vertex list
...
int
mc_ray_boundingbox(vector *min, vector *max, vector *p0, vector *pdir,
                   vector *hitpos)
{
    vector tmp_hitpos;
    if (hitpos == NULL) {
    ...
    if (Mc->flags & MC_CHECK_SPHERELINE) {
        ...
        return fvi_ray_boundingbox(&sphere_mod_min, &sphere_mod_max, p0, pdir,
                                   hitpos);
    }
    else {
...
static void
mc_check_face(int nv, vector **verts, vector *plane_pnt, float face_rad,
              vector *plane_norm, uv_pair *uvl_list, int ntmap, ubyte *poly)
{
    vector hit_point;
    float dist;
    float u, v;
    ...
    dist = fvi_ray_plane(NULL, plane_pnt, plane_norm, &Mc_p0, &Mc_direction,
                         0.0f);
                         ...
                         int ntmap, ubyte *poly)
{
    ...
    if (!fvi_sphere_plane(&hit_point, &Mc_p0, &Mc_direction, Mc->radius,
                          plane_norm, plane_pnt, &face_t, &delta_t)) {
        return;
    }
    ...
    if (check_face) {
        ...
        if (fvi_point_face(&hit_point, nv, verts, plane_norm, &u, &v, uvl_list)) {
            ...
            if ((temp_dist - DIST_TOL > Mc->radius) ||
                (temp_dist + DIST_TOL < Mc->radius)) {
                // get Andsager
                mprintf((
                    "Estimated radius error: Estimate %f, actual %f Mc->radius\n",
                    temp_dist, Mc->radius));
            }
    ...
    if (check_edges) {
        ...
        float sphere_time;
        if (fvi_polyedge_sphereline(&hit_point, &Mc_p0, &Mc_direction, Mc->radius,
                                    nv, verts, &sphere_time)) {
...
void
model_collide_defpoints(ubyte *p)
{
    int n;
    int nverts = w(p + 8);
    int offset = w(p + 16);
...
void
model_collide_flatpoly(ubyte *p)
{
    int i;
    int nv;
    ...
    if (Mc->flags & MC_CHECK_SPHERELINE) {
        mc_check_sphereline_face(nv, points, vp(p + 20), fl(p + 32), vp(p + 8),
                                 NULL, -1, p);
    }
    else {
...
void
model_collide_tmappoly(ubyte *p)
{
    int i;
    int nv;
    ...
    if (Mc->flags & MC_CHECK_SPHERELINE) {
        ...
        mc_check_face(nv, points, vp(p + 20), fl(p + 32), vp(p + 8), uvlist,
                      tmap_num, p);
    }
}
...
void
model_collide_sortnorm(ubyte *p)
{
    ...
    if (Mc_pm->version >= 2000) {
        if (!mc_ray_boundingbox(vp(p + 56), vp(p + 68), &Mc_p0, &Mc_direction,
                                NULL)) {
            return;
        }
    }
...
int
model_collide_sub(void *model_ptr)
{
    ...
    while (chunk_type != OP_EOF) {
        ...
        case OP_SORTNORM:
            ...
            break;
        case OP_BOUNDBOX:
            if (!mc_ray_boundingbox(vp(p + 8), vp(p + 20), &Mc_p0, &Mc_direction,
                                    NULL)) {
                return 1;
            }
            break;
...
mc_check_shield()
{
    ...
    for (o = 0; o < 8; o++) {
        ...
        if (!mc_ray_boundingbox(&poct1->min, &poct1->max, &Mc_p0, &Mc_direction,
                                NULL)) {
            continue;
        }
        ...
        for (i = 0; i < poct1->nshield_tris; i++) {
            ...
            if (!(Mc->flags &
                  MC_CHECK_SPHERELINE)) { // Don't do this test for sphere colliding against shields
                // Find the intersection of this ray with the plane that the Mc_pmly
                // lies in
                dist = fvi_ray_plane(NULL, points[0], &tri->norm, &Mc_p0,
                                     &Mc_direction, 0.0f);

                if (dist < 0.0f)
                    continue; // If the ray is behind the plane there is no collision
                if (!(Mc->flags & MC_CHECK_RAY) && (dist > 1.0f))
                    continue; // The ray isn't long enough to intersect the plane
                ...
                // also finds the uv's where the ray hit.
                if (fvi_point_face(&hitpoint, 3, points, &tri->norm, NULL, NULL,
                                   NULL)) {
                    Mc->hit_dist = dist;
                ...
                mc_check_sphereline_face(3, points, points[0], 10000.0f,
                                         &tri->norm, NULL, 0, NULL);
                if (Mc->num_hits &&
                    Mc->hit_dist < sphere_check_closest_shield_dist) {
...
mc_check_subobj(int mn)
{
    ...
    if (Mc_pm->detail[0] == mn) {
        ...
        if (!mc_ray_boundingbox(&Mc_pm->mins, &Mc_pm->maxs, &Mc_p0, &Mc_direction,
                                NULL)) {
            return;
        }
...
model_collide(mc_info *mc_info)
{
    ...
    if (Mc->flags & MC_CHECK_SPHERELINE) {
        ...
        if (fvi_segment_sphere(&Mc->hit_point_world, Mc->p0, Mc->p1, Mc->pos,
        ...
        if (Mc->flags & MC_CHECK_RAY) {
            r = fvi_ray_sphere(&Mc->hit_point_world, Mc->p0, Mc->p1, Mc->pos,
                               model_radius);
        }
        else {
            r = fvi_segment_sphere(&Mc->hit_point_world, Mc->p0, Mc->p1, Mc->pos,
                                   model_radius);
        }
        if (r) {
            if (Mc->flags & MC_ONLY_SPHERE) {
    ...
    if (Mc->num_hits) {
        if (Mc->flags & MC_SUBMODEL) {
            ...
            model_find_world_point(&Mc->hit_point_world, &Mc->hit_point,
                                   Mc->model_num, Mc->hit_submodel, Mc->orient,
                                   Mc->pos);
        }
    }
