...
{ /* contains rotation rate info */
    uint flags; // See MSS_FLAG_* defines above
    char name[MAX_NAME_LEN]; // name of the subsystem.  Probably displayed on HUD
    char subobj_name
        [MAX_NAME_LEN]; // Temporary (hopefully) parameter used to match stuff in ships.tbl
...
typedef struct bsp_info
{
    ...
    vector
        geometric_center; // geometric center of this subobject.  In the same Frame Of
    ...
    int details
        [MAX_MODEL_DETAIL_LEVELS]; // A list of all the lower detail "mirrors" of this submodel
...
typedef struct model_path
{
    char name[MAX_NAME_LEN]; // name of the subsystem.  Probably displayed on HUD
    char parent_name
        [MAX_NAME_LEN]; // parent name of submodel that path is linked to in POF
    int parent_submodel;
    int nverts;
    mp_vert *verts;
...
typedef struct shield_tri
{
    ...
    int neighbors
        [3]; // indices into array of triangles. neighbor = shares edge.  list found in shield_info struct
...
typedef struct model_octant
{
    ...
    vector **
        verts; // The vertices in this octant in the high-res hull.  A vertex can only be in one octant.
    int nshield_tris; // how many shield triangles are in the octant
    shield_tri **
        shield_tris; // the shield triangles that make up this octant. A tri could be in multiple octants.
} model_octant;
...
typedef struct eye
{
    ...
    vector
        norm; // direction the eye faces.  Not used with first eye since player orient is used
} eye;
...
typedef struct polymodel
{
    ...
    eye view_positions
        [MAX_EYES]; //viewing positions.  Default to {0,0,0}. in location 0
    ...
    int textures
        [MAX_MODEL_TEXTURES]; // what textures you draw with.  reset to original_textures by model_set_instance
    char texture_file
        [MAX_MODEL_TEXTURES]
        [FILENAME_LEN]; // POF texture filenames; bound to handles by model_load_textures()
    ...
    thruster_bank
        *thrusters; // array of thruster objects -- likely to change in the future
    ...
    float split_plane
        [MAX_SPLIT_PLANE]; // actual split plane z coords (for big ship explosions)
...
void model_copy_subsystems(int n_subsystems, model_subsystem *d_sp,
...
void model_render(int model_num, matrix *orient, vector *pos,
                  uint flags = MR_NORMAL, int objnum = -1,
                  int lighting_skip = -1);

// Renders just one particular submodel on a model.
// See MR_? defines for values for flags
void submodel_render(int model_num, int submodel_num, matrix *orient, vector *pos,
                     uint flags = MR_NORMAL, int light_ignore_id = -1);
...
extern int model_find_2d_bound(int model_num, matrix *orient, vector *pos,
...
// rect.
extern int model_find_2d_bound_min(int model_num, matrix *orient, vector *pos,
                                   int *x1, int *y1, int *x2, int *y2);
...
// so the bounds will change as the object rotates, to give the minimum bouding
// rect.
int submodel_find_2d_bound_min(int model_num, int submodel, matrix *orient,
...
// This function just looks at the radius, and not the orientation, so the
// bounding box won't change depending on the obj's orient.
int subobj_find_2d_bound(float radius, matrix *orient, vector *pos, int *x1,
...
extern int model_rotate_gun(int model_num, model_subsystem *turret,
                            matrix *orient, angles *turret1, angles *turret2,
                            vector *pos, vector *dst);
...
extern void model_find_world_point(vector *outpnt, vector *mpnt, int model_num,
                                   int sub_model_num, matrix *objorient,
                                   vector *objpos);
...
void world_find_model_point(vector *out, vector *world_pt, polymodel *pm,
...
void model_get_rotating_submodel_list(int *submodel_list,
                                      int *num_rotating_submodesl, object *objp);

// For a rotating submodel, find a point on the axis
void model_init_submodel_axis_pt(submodel_instance_info *sii, int model_num,
                                 int submodel_num);

// Given a direction (pnt) that is in sub_model_num's frame of
// reference, and given the object's orient and position,
// return the point in 3-space in outpnt.
extern void model_find_world_dir(vector *out_dir, vector *in_dir, int model_num,
                                 int sub_model_num, matrix *objorient,
                                 vector *objpos);
...
void model_set_instance_info(submodel_instance_info *sii, float turn_rate,
                             float turn_accel);
...
extern void model_set_instance(int model_num, int sub_model_num,
                               submodel_instance_info *sii);

// Adds an electrical arcing effect to a submodel
void model_add_arc(int model_num, int sub_model_num, vector *v1, vector *v2,
                   int arc_type);

// Fills in an array with points from a model.  Only gets up to max_num verts.
// Returns number of verts found
extern int submodel_get_points(int model_num, int submodel_num, int max_num,
                               vector **nts);

// Gets two random points on the surface of a submodel
extern void submodel_get_two_random_points(int model_num, int submodel_num,
                                           vector *v1, vector *v2,
                                           vector *n1 = NULL, vector *n2 = NULL);
...
void model_find_obj_dir(vector *w_vec, vector *m_vec, object *ship_obj,
                        int sub_model_num);
...
typedef struct mc_info
{
    ...
    vector
        hit_point; // Where the collision occurred at in hit_submodel's coordinate system
    ...
    vector
        hit_normal; //  Vector normal of polygon of collision.  (This is in submodel RF)
...
// data.
int model_which_octant_distant(vector *pnt, int model_num, matrix *model_orient,
                               vector *model_pos, model_octant **oct);
...
int model_which_octant(vector *pnt, int model_num, matrix *model_orient,
                       vector *model_pos, model_octant **oct);
...
float model_find_closest_point(vector *outpnt, int model_num, int submodel_num,
...
float get_world_closest_box_point_with_delta(vector *closest_box_point,
                                             object *box_obj, vector *start_point,
