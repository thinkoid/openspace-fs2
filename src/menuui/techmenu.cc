...
int Tech_ani_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                 196, 115 },
                                               { // GR_1024
                                                 449, 245 } };
...
static int
    Cur_entry_index; // this is the current entry selected, using master list indexing
...
void
techroom_init_desc(char *src, int w)
{
...
void
techroom_render_desc(int xo, int yo, int h)
{
    ...
    if (z < Text_size) {
        ...
        int more_txt_x =
            ...
            10; // FIXME should move these to constants since they dont move
        int more_txt_y = Tech_desc_coords[gr_screen.res][1] +
                         Tech_desc_coords[gr_screen.res]
                                         [3]; // located below brief text, centered
        int w, h;
        gr_get_string_size(&w, &h, XSTR("more", 1469),
                           strlen(XSTR("more", 1469)));
        gr_set_color_fast(&Color_black);
        gr_rect(more_txt_x - 2, more_txt_y, w + 3, h);
        gr_set_color_fast(&Color_red);
        gr_string(more_txt_x, more_txt_y,
                  XSTR("more", 1469)); // base location on the input x and y?
    }
}

// new version of weapons
void
techroom_weapons_render2(float frametime)
{
    // render common stuff
    tech_common_render();
    ...
    if ((Cur_entry > 0) && (Current_list[Cur_entry].animation == NULL) &&
        (Current_list[Cur_entry].bitmap >= 0)) {
        gr_set_bitmap(Current_list[Cur_entry].bitmap);
        gr_bitmap(Tech_ani_coords[gr_screen.res][0],
                  Tech_ani_coords[gr_screen.res][1]);
    }
}
...
tech_common_render()
{
    ...
    techroom_render_desc(Tech_desc_coords[gr_screen.res][SHIP_X_COORD],
    ...
    while (y + font_height <= Tech_list_coords[gr_screen.res][SHIP_H_COORD]) {
        ...
        gr_string(Tech_list_coords[gr_screen.res][SHIP_X_COORD],
...
void
techroom_ships_render(float frametime)
{
    ...
    model_clear_instance(Techroom_ship_modelnum);
    model_set_detail_level(0);
    model_render(Techroom_ship_modelnum, &Techroom_ship_orient, &vmd_zero_vector,
...
{
    if (List_offset +
            Tech_list_coords[gr_screen.res][SHIP_H_COORD] / gr_get_font_height() <
        Current_list_size) {
        List_offset++;
        gamesnd_play_iface(SND_SCROLL);
    }
...
void
techroom_weapons_render(float frametime)
{
    gr_set_color_fast(&Color_text_normal);
    techroom_render_desc(Tech_desc_coords[gr_screen.res][0],
                         Tech_desc_coords[gr_screen.res][1],
                         Tech_desc_coords[gr_screen.res][3]);
...
void
techroom_intel_render(float frametime)
{
    tech_common_render();
...
techroom_start_anim()
{
    ...
    if (Current_list[Cur_entry].animation != NULL) {
        ...
        if (id != Anim_playing_id) {
            Anim_playing_id = -1;
            if (animp) {
                anim_play_init(&aps, animp, Tech_ani_coords[gr_screen.res][0],
                               Tech_ani_coords[gr_screen.res][1]);
                aps.looped = 1;
...
techroom_change_tab(int num)
{
    ...
    case SHIPS_DATA_TAB:
        ...
        if (Ships_loaded == 0) {
            ...
            for (i = 0; i < Num_ship_types; i++) {
                if (Ship_info[i].flags & mask) {
                    ...
                    Ship_list[Ship_list_size].animation =
                        NULL; // no anim for ships
        ...
        font_height = gr_get_font_height();
        max_num_entries_viewable = Tech_list_coords[gr_screen.res][SHIP_H_COORD] /
                                   font_height;
        Tech_slider.set_numberItems(Current_list_size > max_num_entries_viewable
                                        ? Current_list_size -
            ...
            for (i = 0; i < MAX_WEAPON_TYPES; i++) {
                if (Weapon_info[i].wi_flags & mask) {
                    ...
                    if ((Weapon_list_size > 0) &&
                        (!strcmp(Weapon_info[i].tech_desc,
                                 Weapon_list[Weapon_list_size - 1].desc))) {
                        continue;
                    }
                    ...
                    if (!techroom_load_ani(
                            &Weapon_list[Weapon_list_size].animation,
                        ...
                        Weapon_list[Weapon_list_size].bitmap = bm_load(
                            Weapon_info[i].tech_anim_filename);
                    }
            ...
            for (i = 0; i < Intel_info_size; i++) {
                if (Intel_info[i].in_tech_db != 0) {
                    ...
                    if (!strcmp(Intel_info[i].anim_filename, "none")) {
                        ...
                        if (techroom_load_ani(
                                &Intel_list[Intel_list_size].animation,
                                Intel_info[i].anim_filename)) {
                            Intel_list[Intel_list_size].has_anim = 1;
                        }
                        else {
                            Intel_list[Intel_list_size].bitmap = bm_load(
                                Intel_info[i].anim_filename);
                        }
                    }
...
int
techroom_button_pressed(int num)
{
    switch (num) {
    case SHIPS_DATA_TAB:
    case WEAPONS_DATA_TAB:
...
int
techroom_load_ani(anim **animpp, char *name)
{
    int load_attempts = 0;
    ...
    if (!inited) {
        if ((rval = setjmp(parse_abort)) != 0) {
            ...
            while (optional_string("$Entry:")) {
                ...
                stuff_string(Intel_info[Intel_info_size].anim_filename, F_NAME,
                             NULL, 32);
                required_string("$AlwaysInTechRoom:");
                stuff_int(&Intel_info[Intel_info_size].in_tech_db);
                required_string("$Description:");
                stuff_string(Intel_info[Intel_info_size].desc, F_MULTITEXT, NULL,
                             TECH_INTEL_DESC_LEN);
...
techroom_init()
{
    ...
    Ui_window.add_XSTR("Ships", 293, Buttons[gr_screen.res][SHIPS_DATA_TAB].xt,
                       ...
                       &Buttons[gr_screen.res][SHIPS_DATA_TAB].button,
                       UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR("Weapons", 1553,
                       Buttons[gr_screen.res][WEAPONS_DATA_TAB].xt,
                       Buttons[gr_screen.res][WEAPONS_DATA_TAB].yt,
                       &Buttons[gr_screen.res][WEAPONS_DATA_TAB].button,
                       UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR("Intelligence", 1066,
                       Buttons[gr_screen.res][INTEL_DATA_TAB].xt,
                       Buttons[gr_screen.res][INTEL_DATA_TAB].yt,
                       &Buttons[gr_screen.res][INTEL_DATA_TAB].button,
                       UI_XSTR_COLOR_GREEN);
    ...
    View_window.create(&Ui_window, "",
                       Tech_ship_display_coords[gr_screen.res][SHIP_X_COORD],
                       Tech_ship_display_coords[gr_screen.res][SHIP_Y_COORD],
                       Tech_ship_display_coords[gr_screen.res][SHIP_W_COORD],
                       Tech_ship_display_coords[gr_screen.res][SHIP_H_COORD], 1,
                       1);
    View_window.hide();
    ...
    // setup slider
    Tech_slider.create(&Ui_window,
                       Tech_slider_coords[gr_screen.res][SHIP_X_COORD],
                       ...
                       Num_ship_types, Tech_slider_filename[gr_screen.res],
...
void
techroom_do_frame(float frametime)
{
    int i, k;
