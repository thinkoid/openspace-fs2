...
int
sim_room_line_add(int type, char *name, char *filename, int x, int y, int flags)
{
...
int
campaign_room_campaign_filter(char *filename)
{
...
int
sim_room_campaign_mission_filter(char *filename)
{
    int num;
...
int
sim_room_standalone_mission_filter(char *filename)
{
    int type;
...
//
int
build_standalone_mission_list_do_frame()
{
...
//
int
build_campaign_mission_list_do_frame()
{
    int font_height = gr_get_font_height();
    char str[256];
    ...
    if (Campaign.missions[Num_campaign_missions_with_info].completed) {
        if (!get_mission_info(
                Campaign.missions[Num_campaign_missions_with_info].name)) {
            // add to list
            Campaign_mission_names[Num_campaign_missions_with_info] = strdup(
                The_mission.name);
    ...
    if (Player->readyroom_listing_mode == MODE_MISSIONS) {
        if (Hash_table_inited) {
            if (!Standalone_mission_names_inited) { // Is this the first time through
                ...
                popup_till_condition(build_standalone_mission_list_do_frame,
                                     POPUP_CANCEL, XSTR("Loading missions", 991));
            }
            else {
                for (i = 0; i < Num_standalone_missions_with_info; i++) {
                    if (Standalone_mission_names[i]) {
                        ...
                        strcpy(full_filename, cf_add_ext(Mission_filenames[i],
                                                         FS_MISSION_FILE_EXT));
                        fs_builtin_mission *fb = game_find_builtin_mission(
                            full_filename);
                        if ((fb != NULL) && (fb->flags & FSB_FROM_VOLITION)) {
                        ...
                        sim_room_line_add(READYROOM_LINE_MISSION,
                                          Standalone_mission_names[i],
                                          Mission_filenames[i],
                                          list_x1 + M_TEXT_X, y, flags);
                        y += font_height + 2;
                    }
        ...
        if (!Campaign_mission_names_inited) { // Is this the first time through
            popup_till_condition(build_campaign_mission_list_do_frame,
                                 POPUP_CANCEL,
            ...
            for (i = 0; i < Num_campaign_missions_with_info; i++) {
                if (Campaign_mission_names[i]) {
                    ...
                    int flags = 0;
                    memset(full_filename, 0, 256);
                    strcpy(full_filename, cf_add_ext(Campaign.missions[i].name,
                                                     FS_MISSION_FILE_EXT));
                    fs_builtin_mission *fb = game_find_builtin_mission(
                        full_filename);
                    if ((fb != NULL) && (fb->flags & FSB_FROM_VOLITION)) {
                    ...
                    sim_room_line_add(READYROOM_LINE_CMISSION,
                                      Campaign_mission_names[i],
...
int
sim_room_line_query_visible(int n)
{
    int y;
...
int
sim_room_can_resume_savegame(char *savegame_filename)
{
...
sim_room_commit()
{
    ...
    strncpy(Game_current_mission_filename, sim_room_lines[Selected_line].filename,
            MAX_FILENAME_LEN);
...
sim_room_init()
{
    ...
    *Game_current_mission_filename = 0;
    ...
    Ui_window.add_XSTR("Technical Database", 1055,
                       ...
                       UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR("Mission Simulator", 1056,
                       Buttons[gr_screen.res][SIMULATOR_BUTTON].xt,
                       Buttons[gr_screen.res][SIMULATOR_BUTTON].yt,
                       &Buttons[gr_screen.res][SIMULATOR_BUTTON].button,
                       UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR("Cutscenes", 1057,
                       Buttons[gr_screen.res][CUTSCENES_BUTTON].xt,
                       Buttons[gr_screen.res][CUTSCENES_BUTTON].yt,
                       &Buttons[gr_screen.res][CUTSCENES_BUTTON].button,
                       UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR("Credits", 1058, Buttons[gr_screen.res][CREDITS_BUTTON].xt,
                       Buttons[gr_screen.res][CREDITS_BUTTON].yt,
                       &Buttons[gr_screen.res][CREDITS_BUTTON].button,
                       UI_XSTR_COLOR_GREEN);
    ...
    Buttons[gr_screen.res][COMMIT_BUTTON].button.set_hotkey(KEY_CTRLED |
                                                            KEY_ENTER);
    ...
    Num_standalone_missions = cf_get_file_list(MAX_MISSIONS, Mission_filenames,
                                               CF_TYPE_MISSIONS, wild_card,
                                               CF_SORT_NAME);

    Num_campaign_missions_with_info = Num_standalone_missions_with_info =
        Standalone_mission_names_inited = Campaign_names_inited =
...
//
void
sim_room_do_frame(float frametime)
{
    char buf[256];
    int i, k, y, z, line;
    int font_height = gr_get_font_height();
...
void
sim_room_blit_icons(int line_index, int y_start, fs_builtin_mission *fb,
                    int is_md)
{
    ...
    if (fb == NULL) {
        is_from_volition = (sim_room_lines[line_index].flags &
                            READYROOM_FLAG_FROM_VOLITION)
                               ? 1
                               : 0;
    }
    else {
    ...
    if (is_from_volition && (Mission_icon_bitmaps[MISSION_ICON_VOLITION] >= 0)) {
        ...
        gr_bitmap(Sim_volition_icon_x[gr_screen.res],
                  y_start + MISSION_ICON_VOLITION_Y_OFFSET);
    }
}
...
campaign_room_build_listing()
{
    ...
    for (c = 0; c < Num_campaigns; c++) {
        if (Campaign_names[c]) {
            ...
            sim_room_line_add(READYROOM_LINE_CAMPAIGN, Campaign_names[c],
                              Campaign_file_names[c],
...
set_new_campaign_line(int n)
{
    ...
    Num_info_lines = 0;
    if (str) {
        Num_info_lines = split_str(str, Cr_info_coords[gr_screen.res][2],
                                   Info_text_line_size, Info_text_ptrs,
                                   MAX_INFO_LINES);
...
// returns: 0 = success, !0 = aborted or failed
int
campaign_room_reset_campaign(int n)
{
    char *filename;
    int z;
...
campaign_room_commit()
{
    ...
    if (stricmp(Campaign_file_names[Selected_campaign_index],
                Campaign.filename)) { // new campaign selected
        ...
        strcpy(Player->current_campaign,
...
int
campaign_room_button_pressed(int n)
{
    switch (n) {
...
campaign_room_init()
{
    ...
    Cr_buttons[gr_screen.res][CR_COMMIT_BUTTON].button.set_hotkey(KEY_CTRLED |
                                                                  KEY_ENTER);
    ...
    for (i = 0; i < Num_campaigns; i++) {
        for (j = 0; j < Num_campaigns; j++) {
            if (Campaign_file_names_temp[j]) {
                if (!strncmp(Campaign_file_names[i], Campaign_file_names_temp[j],
                             strlen(Campaign_file_names_temp[j]) - 4)) {
...
void
campaign_room_do_frame(float frametime)
{
    char buf[256];
    ...
    while (sim_room_line_query_visible(line)) {
        ...
        List_buttons[line - Scroll_offset].update_dimensions(
            Cr_list_coords[gr_screen.res][0], y, Cr_list_coords[gr_screen.res][2],
            font_height);
        List_buttons[line - Scroll_offset].enable();
        ...
        if (!stricmp(sim_room_lines[line].filename, Campaign.filename)) {
            ...
            gr_line(Cr_list_coords[gr_screen.res][0] - 10, i,
                    Cr_list_coords[gr_screen.res][0] - 8, i);
            gr_line(Cr_list_coords[gr_screen.res][0] - 6, i - 4,
                    Cr_list_coords[gr_screen.res][0] - 6, i - 2);
            gr_line(Cr_list_coords[gr_screen.res][0] - 4, i,
                    Cr_list_coords[gr_screen.res][0] - 2, i);
            gr_line(Cr_list_coords[gr_screen.res][0] - 6, i + 2,
        ...
        strcpy(buf, sim_room_lines[line].name);
        gr_force_fit_string(buf, 255,
                            Cr_list_coords[gr_screen.res][0] +
                                Cr_list_coords[gr_screen.res][2] -
    ...
    while (y + font_height <= Cr_info_coords[gr_screen.res][3]) {
        ...
        gr_string(Cr_info_coords[gr_screen.res][0],
                  Cr_info_coords[gr_screen.res][1] + y, line_text);
        y += font_height;
        i++;
    }
