...
int
credits_screen_button_pressed(int n)
{
    switch (n) {
...
credits_init()
{
    ...
    int credits_spooled_music_index = event_music_get_spooled_music_index(
        "Cinema");
    ...
    if (fp != NULL) {
        ...
        while (!check_for_string_raw("#end")) {
            ...
            do {
                linep2 = split_str_once(linep1,
                                        Credits_text_coords[gr_screen.res][2]);
...
void
credits_do_frame(float frametime)
{
    ...
    percent = (int)(100.0f - (CREDITS_ARTWORK_DISPLAY_TIME - Credits_counter) *
                                 100.0f / CREDITS_ARTWORK_FADE_TIME);
    if (percent < 0) {
        percent = 0;
    }
    ...
    if ((bm1 != -1) && (bm2 != -1)) {
        ...
        bx1 = Credits_image_coords[gr_screen.res][CREDITS_X_COORD] +
              ((Credits_image_coords[gr_screen.res][CREDITS_W_COORD] - bw1) / 2);
        by1 = Credits_image_coords[gr_screen.res][CREDITS_Y_COORD] +
              ((Credits_image_coords[gr_screen.res][CREDITS_H_COORD] - bh1) / 2);
        bx2 = Credits_image_coords[gr_screen.res][CREDITS_X_COORD] +
              ((Credits_image_coords[gr_screen.res][CREDITS_W_COORD] - bw2) / 2);
        by2 = Credits_image_coords[gr_screen.res][CREDITS_Y_COORD] +
