...
awacs_update_all_levels()
{
    ...
    for (moveup = GET_FIRST(&Ship_obj_list);
        ...
        if ((shipp->flags & SF_DYING) || (shipp->flags & SF_DEPARTING) ||
            (shipp->flags & SF_ARRIVING)) {
            continue;
        }
        ...
        for (ship_system = GET_FIRST(&shipp->subsys_list);
            ...
            if ((ship_system->system_info != NULL) &&
                ...
                Awacs_team[shipp->team] += ship_system->awacs_intensity *
                                           (ship_system->current_hits /
                                            ship_system->system_info->max_hits);
...
awacs_get_level(object *target, ship *viewer, int use_awacs)
{
    ...
    if ((target->type == OBJ_SHIP) &&
        (Ships[target->instance].team == viewer->team)) {
        return 1.5f;
    }

    int stealth_ship = (target->type == OBJ_SHIP) && (target->instance >= 0) &&
                       (Ship_info[Ships[target->instance].ship_info_index].flags &
                        SIF_STEALTH);
    int nebula_mission = (The_mission.flags & MISSION_FLAG_FULLNEB);
    int check_huge_ship =
        (target->type == OBJ_SHIP) && (target->instance >= 0) &&
        (Ship_info[Ships[target->instance].ship_info_index].flags &
         SIF_HUGE_SHIP);
    ...
    if ((stealth_ship || nebula_mission) && use_awacs) {
        for (idx = 0; idx < Awacs_count; idx++) {
            ...
            if (!get_subsystem_pos(&subsys_pos, Awacs[idx].objp,
                                   Awacs[idx].subsys)) {
                continue;
            }
            ...
            if (check_huge_ship) {
                ...
                if (check_world_pt_in_expanded_ship_bbox(
                        &subsys_pos, target, Awacs[idx].subsys->awacs_radius)) {
                    closest_index = idx;
                    break;
                }
    ...
    // all other ships
        ...
        if (check_huge_ship) {
            if (check_world_pt_in_expanded_ship_bbox(&Objects[viewer->objnum].pos,
                                                     target, scan_nebula_range)) {
                if (check_world_pt_in_expanded_ship_bbox(
...
team_visibility_update()
{
    int friendly_count, hostile_count, neutral_count, unknown_count,
        traitor_count, num_stealth;
    ...
    friendly_count = hostile_count = neutral_count = unknown_count =
        traitor_count = num_stealth = 0;
    ...
    for (int cur_team = 0; cur_team < 3; cur_team++) {
        ...
        for (int en_team_inc = 1; en_team_inc < NUM_TEAMS; en_team_inc++) {
            // set up enemy team
            en_team =
                (cur_team + en_team_inc) %
                NUM_TEAMS; // enemy_team (index) is cur_team + (1-4), wrapped back to range 0-4
            ...
            for (en_idx = 0; en_idx < en_count; en_idx++) {
                ...
                for (idx = 0; idx < cur_count; idx++) {
                    ...
                    if (awacs_get_level(
                            ...
                            &Ships[cur_team_ships[idx]], idx == 0) > 1) {
...
int
ship_is_visible_by_team(int ship_num, int team)
{
    Assert((ship_num >= 0) && (ship_num < MAX_SHIPS));
