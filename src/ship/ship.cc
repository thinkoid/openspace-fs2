...
int Player_ship_precedence
    [MAX_PLAYER_SHIP_CHOICES]; // Array of ship types, precedence list for player ship/wing selection

static int
    Laser_energy_out_snd_timer; // timer so we play out of laser sound effect periodically
static int
    Missile_out_snd_timer; // timer so we play out of laser sound effect periodically
...
char *Ship_type_names[MAX_SHIP_TYPE_COUNTS] = {
    ...
    "transport",  "support",       "navbuoy",
    "sentry gun", "escape pod",    "super cap",
    "stealth",    "fighter",       "bomber",
    "drydock",    "awacs",         "gas miner",
    "corvette",   "knossos device"
    //XSTR:ON
};
...
ship_iff_init_colors()
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        gr_init_alphacolor(&IFF_colors[IFF_COLOR_HOSTILE][i], 0xff, 0x00, 0x00,
                           alpha);
        gr_init_alphacolor(&IFF_colors[IFF_COLOR_FRIENDLY][i], 0x00, 0xff, 0x00,
                           alpha);
        gr_init_alphacolor(&IFF_colors[IFF_COLOR_NEUTRAL][i], 0xff, 0x00, 0x00,
                           alpha);
        gr_init_alphacolor(&IFF_colors[IFF_COLOR_UNKNOWN][i], 0xff, 0x00, 0xff,
                           alpha);
        gr_init_alphacolor(&IFF_colors[IFF_COLOR_SELECTION][i], 0xff, 0xff, 0xff,
                           alpha);
        gr_init_alphacolor(&IFF_colors[IFF_COLOR_MESSAGE][i], 0x7f, 0x7f, 0x7f,
                           alpha);
        gr_init_alphacolor(&IFF_colors[IFF_COLOR_TAGGED][i], 0xff, 0xff, 0x00,
                           alpha);
    }
}
...
void
ship_obj_list_reset_slot(int index)
{
...
int
ship_in_abgz(ship *shipp)
{
    if (!strcmp(shipp->ship_name, "Alpha 1"))
        return 1;
    if (!strcmp(shipp->ship_name, "Alpha 2"))
        return 1;
    if (!strcmp(shipp->ship_name, "Alpha 3"))
        return 1;
    if (!strcmp(shipp->ship_name, "Alpha 4"))
        return 1;

    if (!strcmp(shipp->ship_name, "Beta 1"))
        return 1;
    if (!strcmp(shipp->ship_name, "Beta 2"))
        return 1;
    if (!strcmp(shipp->ship_name, "Beta 3"))
        return 1;
    if (!strcmp(shipp->ship_name, "Beta 4"))
        return 1;

    if (!strcmp(shipp->ship_name, "Gamma 1"))
        return 1;
    if (!strcmp(shipp->ship_name, "Gamma 2"))
        return 1;
    if (!strcmp(shipp->ship_name, "Gamma 3"))
        return 1;
    if (!strcmp(shipp->ship_name, "Gamma 4"))
        return 1;

    if (!strcmp(shipp->ship_name, "Zeta 1"))
        return 1;
    if (!strcmp(shipp->ship_name, "Zeta 2"))
        return 1;
    if (!strcmp(shipp->ship_name, "Zeta 3"))
        return 1;
    if (!strcmp(shipp->ship_name, "Zeta 4"))
        return 1;
...
ship_obj_list_rebuild()
{
    ...
    for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list);
        ...
        if (objp->type == OBJ_SHIP) {
            Ships[objp->instance].ship_list_index = ship_obj_list_add(
                OBJ_INDEX(objp));
        }
    }
}
...
ship_obj *
get_ship_obj_ptr_from_index(int index)
{
    Assert(index >= 0 && index < MAX_SHIP_OBJS);
...
parse_ship()
{
    ...
    int n_subsystems = 0;
    model_subsystem
        subsystems[MAX_MODEL_SUBSYSTEMS]; // see model.h for max_model_subsystems
    ...
    if (ship_info_lookup(sip->name) != -1) {
        Error(
            LOCATION,
            "Error:  Ship name %s already exists in ships.tbl.  All ship class names must be unique.",
            sip->name);
    }
    ...
    find_and_stuff("$Species:", &sip->species, F_NAME, Species_names,
                   MAX_SPECIES_NAMES, "species names");
    diag_printf("Ship species -- %s\n", Species_names[sip->species]);

    sip->type_str = sip->maneuverability_str = sip->armor_str =
    ...
    if (optional_string("$Allowed PBanks:")) {
        num_allowed = stuff_int_list(allowed_weapons, MAX_WEAPON_TYPES,
                                     WEAPON_LIST_TYPE);
    ...
    sip->num_primary_banks = stuff_int_list(sip->primary_bank_weapons,
                                            MAX_PRIMARY_BANKS, WEAPON_LIST_TYPE);
    ...
    sip->num_secondary_banks = stuff_int_list(
        sip->secondary_bank_weapons, MAX_SECONDARY_BANKS, WEAPON_LIST_TYPE);
    ...
    int capacity_count;
    capacity_count = stuff_int_list(sip->secondary_bank_ammo_capacity,
                                    MAX_SECONDARY_BANKS, RAW_INTEGER_TYPE);
    if (capacity_count != sip->num_secondary_banks) {
        Warning(
            LOCATION,
            "Secondary bank capacities have not been specified for ship class %s... fix this!!",
            sip->name);
    }
    ...
    find_and_stuff("$AI Class:", &sip->ai_class, F_NAME, Ai_class_names,
                   Num_ai_classes, "AI class names");
    ...
    while (cont_flag) {
        ...
        case 1: {
            ...
            sp = &subsystems
                     [n_subsystems++]; // subsystems a local -- when done, we will malloc and copy
            ...
            sp->max_hits = sip->initial_hull_strength *
                           (percentage_of_hits / 100.0f);
            ...
            if (optional_string("$Default PBanks:")) {
                stuff_int_list(sp->primary_banks, MAX_PRIMARY_BANKS,
                               WEAPON_LIST_TYPE);
            }

            // Get default secondary bank weapons
            if (optional_string("$Default SBanks:")) {
                stuff_int_list(sp->secondary_banks, MAX_SECONDARY_BANKS,
                               WEAPON_LIST_TYPE);
            }

            // Get the capacity of each secondary bank
            if (optional_string("$Sbank Capacity:")) {
                stuff_int_list(sp->secondary_bank_capacity, MAX_SECONDARY_BANKS,
                               RAW_INTEGER_TYPE);
            }
            ...
            sp->turret_weapon_type =
                sp->primary_banks[0]; // temporary, will be obsolete later.
    ...
    if (n_subsystems > 0) {
        sip->subsystems = (model_subsystem *)malloc(sizeof(model_subsystem) *
                                                    n_subsystems);
    ...
    if (sip->flags & SIF_SHIP_COPY) {
        ...
        index = ship_info_base_lookup(
            Num_ship_types); // Num_ship_types is our current entry into the array
        if (index == -1) {
            ...
            if (p)
                *p = '\0';
            Error(LOCATION,
                  "Ship %s is a copy, but base ship %s couldn't be found.",
                  sip->name, name);
        }
    }
...
char
get_engine_wash_index(char *engine_wash_name)
{
    int i;
...
parse_shiptbl()
{
    ...
    Num_player_ship_precedence = stuff_int_list(
        Player_ship_precedence, MAX_PLAYER_SHIP_CHOICES, SHIP_INFO_TYPE);
...
void
ship_add_exited_ship(ship *sp, int reason)
{
    int i, entry;
...
int
ship_find_exited_ship_by_name(char *name)
{
    int i;
...
// function which attempts to find information about an exited ship based on shipname
int
ship_find_exited_ship_by_signature(int signature)
{
    int i;
...
int
ship_get_default_orders_accepted(ship_info *sip)
{
    int ship_info_flag;
    ...
    else if (ship_info_flag &
             (SIF_CRUISER | SIF_GAS_MINER | SIF_AWACS | SIF_CORVETTE))
...
void
ship_set(int ship_index, int objnum, int ship_type)
{
    int i;
    ...
    for (i = 0; i < MAX_SECONDARY_BANKS; i++) {
        ...
        swp->secondary_bank_rearm_time[i] = timestamp(
            0); // will be able to rearm this bank immediately
    }
...
// be kept up to date.
void
ship_recalc_subsys_strength(ship *shipp)
{
    int i;
    ...
    if ((shipp->subsys_info[SUBSYSTEM_ENGINE].num > 0) &&
        (shipp->subsys_info[SUBSYSTEM_ENGINE].current_hits == 0.0f)) {
        shipp->flags |= SF_DISABLED;
    }
...
void
ship_copy_subsystem_fixup(ship_info *sip)
{
    int i, model_num;
...
void
subsys_set(int objnum, int ignore_subsys_info)
{
    ...
    for (i = 0; i < sinfo->n_subsystems; i++) {
        ...
        for (k = 0; k < MAX_SECONDARY_BANKS; k++) {
            if (sp->secondary_banks[k] != -1) {
                ship_system->weapons.secondary_bank_weapons[j] =
                    sp->secondary_banks[k];
                ship_system->weapons.secondary_bank_capacity[j] =
                    sp->secondary_bank_capacity[k];
                ship_system->weapons.next_secondary_fire_stamp[j++] = timestamp(
                    0);
            }
        }
        ...
        model_set_instance_info(&ship_system->submodel_info_1, sp->turn_rate,
                                turn_accel);
...
ship_render(object *obj)
{
   ...
   if ( (aip->target_objnum >= 0)  && (Ship_info[Ships[Objects[aip->target_objnum].instance].ship_info_index].flags & (SIF_SUPERCAP|SIF_CAPITAL|SIF_CRUISER)) ) {
    ...
    if (vm_vec_dist_quick(&obj->pos, &Eye_position) < obj->radius * 50.0f) {
        ...
        for (i = 0; i < MAX_SHIP_ARCS; i++) {
            if (timestamp_valid(shipp->arc_timestamp[i])) {
                render_flags |=
                    MR_ALWAYS_REDRAW; // Turn off model caching if arcing.
                model_add_arc(shipp->modelnum, -1, &shipp->arc_pts[i][0],
    ...
    if (shipp->large_ship_blowup_index > -1) {
        ...
        if ((shipp->thruster_bitmap > -1) && (!(shipp->flags & SF_DISABLED)) &&
            ...
            model_set_thrust(shipp->modelnum, ft, shipp->thruster_bitmap,
                             shipp->thruster_glow_bitmap,
                             shipp->thruster_glow_noise);
        ...
        if (warp_shipp->flags & (SF_ARRIVING | SF_DEPART_WARP)) {
            clip_started = 1;
            g3_start_user_clip_plane(&warp_shipp->warp_effect_pos,
                                     &warp_shipp->warp_effect_fvec);
        ...
        // small ships
        if ((The_mission.flags & MISSION_FLAG_FULLNEB) &&
            (si->flags & SIF_SMALL_SHIP)) {
            // force detail levels
            float fog_val = neb2_get_fog_intensity(obj);
            if (fog_val >= 0.6f) {
                model_set_detail_level(2);
                model_render(shipp->modelnum, &obj->orient, &obj->pos,
                             render_flags | MR_LOCK_DETAIL, OBJ_INDEX(obj));
            }
            else {
                model_render(shipp->modelnum, &obj->orient, &obj->pos,
                             render_flags, OBJ_INDEX(obj));
            }
        }
        else {
            model_render(shipp->modelnum, &obj->orient, &obj->pos, render_flags,
                         OBJ_INDEX(obj));
        }
...
*/
    ...
    if (Show_tnorms) {
        ...
        while (systemp != END_OF_LIST(&shipp->subsys_list)) {
            ship_get_global_turret_gun_info(obj, systemp, &tpos, &tnorm, 1,
                                            &temp);
...
void
ship_subsystem_delete(ship *shipp)
{
...
void
ship_wing_cleanup(int shipnum, wing *wingp)
{
    int i, index = -1, team;
        ...
        if ((wingp->current_wave == wingp->num_waves) &&
            ...
            for (so = GET_FIRST(&Ship_obj_list);
                ...
                if ((Ships[Objects[so->objnum].instance].wingnum ==
                     WING_INDEX(wingp)) &&
                    !(Ships[Objects[so->objnum].instance].flags &
                      (SF_DEPARTING | SF_DYING)))
                    Int3();
            }
#endif
...
ship_destroyed(int num)
{
    ...
    if (!(shipp->flags & SF_IGNORE_COUNT) ||
        ((shipp->wingnum != -1) &&
         !(Wings[shipp->wingnum].flags & WF_IGNORE_COUNT)))
    ...
    if (shipp->ai_index != -1) {
        ai_ship_destroy(num,
                        SEF_DESTROYED); //   Do AI stuff for destruction of ship.
    }
    ...
    if ((shipp->wing_status_wing_index >= 0) &&
        ...
        hud_set_wingman_status_dead(shipp->wing_status_wing_index,
...
ship_vanished(int num)
{
    ...
    if ((sp->wing_status_wing_index >= 0) && (sp->wing_status_wing_pos >= 0)) {
        hud_set_wingman_status_departed(sp->wing_status_wing_index,
                                        sp->wing_status_wing_pos);
    }

    ai_ship_destroy(
        num, SEF_DEPARTED); // should still do AI cleanup after ship has departed
}

void
ship_departed(int num)
{
    ship *sp;
    int i;
    ...
    for (i = 0; i < Num_jump_nodes; i++) {
        ...
        if (dist <= radius) {
            mission_log_add_entry(LOG_SHIP_DEPART, sp->ship_name,
                                  Jump_nodes[i].name, sp->wingnum);
            break;
        }
        dist = 1.0f;
    }
...
//
int
ship_explode_area_calc_damage(vector *pos1, vector *pos2, float inner_rad,
                              float outer_rad, float max_damage, float max_blast,
...
void
ship_blow_up_area_apply_blast(object *exp_objp)
{
    ship_info *sip;
    ...
    if ((exp_objp->hull_strength <= KAMIKAZE_HULL_ON_DEATH) &&
        (Ai_info[Ships[exp_objp->instance].ai_index].ai_flags & AIF_KAMIKAZE) &&
        (Ships[exp_objp->instance].special_exp_index < 0)) {
        float override =
            Ai_info[Ships[exp_objp->instance].ai_index].kamikaze_damage;
        ...
        if (Ships[exp_objp->instance].special_exp_index != -1) {
            ...
            if (propagates) {
                shockwave_speed = (float)atoi(
                    Sexp_variables[start + SHOCK_SPEED].text);
            }
            else {
    ...
    if (shockwave_speed > 0) {
        ...
        for (objp = GET_FIRST(&obj_used_list);
            ...
            if (ship_explode_area_calc_damage(&exp_objp->pos, &objp->pos,
                                              inner_rad, outer_rad, max_damage,
...
void
do_dying_undock_physics(object *objp, ship *sp)
{
    ...
    float impulse_mag = 50.f * dock_obj->phys_info.mass * objp->phys_info.mass /
                        (dock_obj->phys_info.mass + objp->phys_info.mass);
    ...
    physics_apply_whack(&impulse_vec, &pos, &dock_obj->phys_info,
                        &dock_obj->orient, dock_obj->phys_info.mass);
    ...
    physics_apply_whack(&impulse_vec, &pos, &objp->phys_info, &objp->orient,
                        objp->phys_info.mass);
...
void
ship_dying_frame(object *objp, int ship_num)
{
    ship *sp;
    sp = &Ships[ship_num];
    ...
    if (sp->flags & SF_DYING) {
        knossos_ship = (Ship_info[sp->ship_info_index].flags &
                        SIF_KNOSSOS_DEVICE);
        ...
        if (objp->hull_strength > 0) {
            ...
            objp->hull_strength -= hits_left * (1000.0f * flFrametime) /
                                   time_left;
        }
        ...
        if (sp->flags & SF_VAPORIZE) {
            ...
            if (timestamp_elapsed(sp->final_death_time)) {
                // play death sound
                snd_play_3d(&Snds[SND_VAPORIZED], &objp->pos, &View_position,
                            objp->radius, NULL, 0, 1.0f, SND_PRIORITY_MUST_PLAY);
        ...
        if (!(Ai_info[sp->ai_index].ai_flags & AIF_KAMIKAZE) &&
            ship_get_exp_propagates(sp)) {
            if (timestamp_elapsed(Ships[ship_num].next_fireball)) {
                vector outpnt, pnt1, pnt2;
                polymodel *pm = model_get(sp->modelnum);

                // Gets two random points on the surface of a submodel
                submodel_get_two_random_points(sp->modelnum, pm->detail[0], &pnt1,
                                               &pnt2);

                //   vm_vec_avg( &tmp, &pnt1, &pnt2 ); [KNOSSOS get random in plane 1/1.414 in rad
                model_find_world_point(&outpnt, &pnt1, sp->modelnum,
                                       pm->detail[0], &objp->orient, &objp->pos);

                float rad = objp->radius * 0.1f;
                int fireball_type = FIREBALL_EXPLOSION_LARGE1 +
                                    rand() % FIREBALL_NUM_LARGE_EXPLOSIONS;
                fireball_create(&outpnt, fireball_type, OBJ_INDEX(objp), rad, 0,
                                &objp->phys_info.vel);
                ...
                do_sub_expl_sound(objp->radius, &outpnt,
                                  sp->sub_expl_sound_handle);
            }
        }
        ...
        if (knossos_ship) {
            if (timestamp_elapsed(Ships[ship_num].next_fireball)) {
                ...
                float scale = -vm_vec_dotprod(&objp->orient.fvec, &rand_vec) *
                              (0.9f + 0.2f * frand());
                ...
                pe.vel =
                    objp->phys_info.vel; // Initial velocity of all the particles
                pe.min_life = 2.0f; // How long the particles live
                pe.max_life = 12.0f; // How long the particles live
                pe.normal =
                    objp->orient.uvec; // What normal the particle emit around
                pe.normal_variance =
        ...
        if ((time_until_minor_explosions < 500) &&
            ...
            for (int zz = 0; zz < 6; zz++) {
                ...
                model_find_world_point(&outpnt, &tmp, sp->modelnum, pm->detail[0],
                                       &objp->orient, &objp->pos);

                float rad = frand() * 0.30f;
                rad += objp->radius * 0.40f;
                fireball_create(&outpnt, FIREBALL_EXPLOSION_MEDIUM,
        ...
        if (timestamp_elapsed(sp->final_death_time)) {
            ...
            int sound_index;
            if (Ship_info[sp->ship_info_index].flags &
                (SIF_CAPITAL | SIF_KNOSSOS_DEVICE)) {
            ...
            if (ship_get_exp_propagates(sp)) {
                ...
                fireball_objnum = fireball_create(&objp->pos, fireball_type,
                                                  OBJ_INDEX(objp), big_rad, 0,
                                                  &objp->phys_info.vel);
                ...
                sp->really_final_death_time = timestamp(
        ...
        if (timestamp_elapsed(sp->really_final_death_time)) {
            ...
            if (sp->large_ship_blowup_index > -1) {
                if (shipfx_large_blowup_do_frame(sp, flFrametime)) {
                    ...
                    ship_destroyed(
                        ship_num); // call ship function to clean up after the ship is destroyed.
                }
                return;
            }
            ...
            shipfx_blow_up_model(objp, Ships[ship_num].modelnum, 0, 20,
                                 &objp->pos);
...
char Thrust_anim_names[NUM_THRUST_ANIMS][MAX_FILENAME_LEN] = {
    //XSTR:OFF
    "thruster01", "thruster01a", "thruster02", "thruster02a",
...
char Thrust_glow_anim_names[NUM_THRUST_GLOW_ANIMS][MAX_FILENAME_LEN] = {
    //XSTR:OFF
    "thrusterglow01", "thrusterglow01a", "thrusterglow02", "thrusterglow02a",
...
ship_init_thrusters()
{
    ...
    for (i = 0; i < num_thrust_anims; i++) {
        ta = &Thrust_anims[i];
        ta->first_frame = bm_load_animation(Thrust_anim_names[i], &ta->num_frames,
                                            &fps, 1);
        if (ta->first_frame == -1) {
...
ship_do_thruster_frame(ship *shipp, object *objp, float frametime)
{
    ...
    if (shipp->thruster_frame < 0.0f)
        shipp->thruster_frame = 0.0f;
    if (shipp->thruster_frame > 100.0f)
        shipp->thruster_frame = 0.0f;

    while (shipp->thruster_frame > the_anim->time) {
        shipp->thruster_frame -= the_anim->time;
    }
    framenum = fl2i((shipp->thruster_frame * the_anim->num_frames) /
                    the_anim->time);
    if (framenum < 0)
        framenum = 0;
    if (framenum >= the_anim->num_frames)
        framenum = the_anim->num_frames - 1;
    ...
    if (shipp->thruster_glow_frame < 0.0f)
        shipp->thruster_glow_frame = 0.0f;
    if (shipp->thruster_glow_frame > 100.0f)
        shipp->thruster_glow_frame = 0.0f;

    while (shipp->thruster_glow_frame > the_anim->time) {
        shipp->thruster_glow_frame -= the_anim->time;
    }
    framenum = fl2i((shipp->thruster_glow_frame * the_anim->num_frames) /
                    the_anim->time);
    if (framenum < 0)
        framenum = 0;
    if (framenum >= the_anim->num_frames)
...
void
ship_do_weapon_thruster_frame(weapon *weaponp, object *objp, float frametime)
{
    float rate;
    int framenum;
    int anim_index;
    ...
    framenum = fl2i((weaponp->thruster_frame * the_anim->num_frames) /
                    the_anim->time);
    if (framenum < 0)
        framenum = 0;
    if (framenum >= the_anim->num_frames)
    ...
    framenum = fl2i((weaponp->thruster_glow_frame * the_anim->num_frames) /
                    the_anim->time);
    if (framenum < 0)
        framenum = 0;
    if (framenum >= the_anim->num_frames)
...
ship_auto_repair_frame(int shipnum, float frametime)
{
    ...
    for (ssp = GET_FIRST(&sp->subsys_list); ssp != END_OF_LIST(&sp->subsys_list);
         ssp = GET_NEXT(ssp)) {
        Assert(ssp->system_info->type >= 0 &&
        ...
        if (ssp->current_hits != ssp->system_info->max_hits) {
            ...
            ssp->current_hits += ssp->system_info->max_hits *
                                 SHIP_REPAIR_SUBSYSTEM_RATE * frametime;
            ssip->current_hits += ssip->total_hits * SHIP_REPAIR_SUBSYSTEM_RATE *
                                  frametime;
...
void
ship_check_player_distance_sub(player *p)
{
    ...
    if (give_warning_to_player) {
        ...
        if (p->distance_warning_count <= PLAYER_DISTANCE_MAX_WARNINGS) {
            message_send_builtin_to_player(MESSAGE_STRAY_WARNING, NULL,
    ...
    if (!(p->flags & PLAYER_FLAGS_FORCE_MISSION_OVER) &&
        ...
        if (!(p->flags & PLAYER_FLAGS_DIST_TO_BE_KILLED)) {
            message_send_builtin_to_player(MESSAGE_STRAY_WARNING_FINAL, NULL,
                                           MESSAGE_PRIORITY_HIGH,
        ...
        if ((p->flags & PLAYER_FLAGS_DIST_TO_BE_KILLED) &&
            ...
            ship_apply_global_damage(&Objects[p->objnum], &Objects[p->objnum],
                                     NULL, damage);
        }
    }
...
void
ship_reset_disabled_physics(object *objp, int ship_class)
{
...
void
ship_subsys_disrupted_check(ship *sp)
{
    ship_subsys *ss;
    ...
    if (engines_disabled) {
        if (!(sp->subsys_disrupted_flags & (1 << SUBSYSTEM_ENGINE))) {
            if (!(sp->flags & SF_DISABLED)) {
                ship_reset_disabled_physics(&Objects[sp->objnum],
                                            sp->ship_info_index);
            }
        }
    }
...
void
ship_subsys_disrupted_maybe_check(ship *shipp)
{
...
int
ship_subsys_disrupted(ship_subsys *ss)
{
    if (!ss) {
...
void
ship_subsys_set_disrupted(ship_subsys *ss, int time)
{
    int time_left = 0;
...
//
int
ship_subsys_disrupted(ship *sp, int type)
{
        ...
        if (Framecount % 10 == 0) {
            ...
            if ((aip->target_objnum != -1) &&
                ...
                int num_turrets_attacking(object * turret_parent,
                                          int target_objnum);
                num_turrets = num_turrets_attacking(&Objects[aip->target_objnum],
                                                    Ships[aip->shipnum].objnum);
            }
            nprintf(("lethality",
...
void
ship_process_pre(object *objp, float frametime)
{ }
...
void
ship_process_post(object *obj, float frametime)
{
    int num;
    ship *shipp;
    ...
    if (!(shipp->flags & SF_AMMO_COUNT_RECORDED)) {
        for (int i = 0; i < MAX_SECONDARY_BANKS; i++) {
            if (red_alert_mission()) {
                int max_missiles = get_max_ammo_count_for_bank(
    ...
    // Plot ship on the radar.
    if (obj != Player_obj) // don't plot myself.
        radar_plot_object(obj);

    //   Do ship sparks.  Don't do sparks on my ship (since I cannot see it).
    ...
    if (shipp->flags & SF_ARRIVING &&
        ...
        // maybe fire swarm missiles
        // this code might call ship_fire_secondary
        ...
        // check player object is not too far from starting location
        if (obj == Player_obj) {
            ship_check_player_distance();
        }
        ...
        // if the ship is a player ship don't need to do AI
        if (obj->flags & OF_PLAYER_SHIP) {
            return;
        }
...
void
ship_set_default_weapons(ship *shipp, ship_info *sip)
{
    int i;
    polymodel *po;
    ship_weapon *swp = &shipp->weapons;
    ...
    if (po->n_guns > sip->num_primary_banks) {
        ...
        Warning(
            LOCATION,
            "There are %d primary banks in ships.tbl for %s\nbut only %d primary banks in the model\n",
    ...
    if (po->n_missiles > sip->num_secondary_banks) {
        ...
        for (i = sip->num_secondary_banks; i < po->n_missiles; i++) {
            ...
            swp->secondary_bank_weapons[i] = weapon_info_lookup(
                NOX("Rockeye Missile"));
        ...
        Warning(
            LOCATION,
            "There are %d secondary banks in ships.tbl for %s,\n but only %d secondary banks in the model.\n",
...
int
ship_check_collision_fast(object *obj, object *other_obj, vector *hitpos)
{
    int num;
    mc_info mc;
...
void
ship_maybe_fixup_subsys_path(polymodel *pm, int path_num)
{
    vector *v1, *v2, dir;
    float dist;
    int index_1, index_2;
...
void
ship_set_subsys_path_nums(ship_info *sip, polymodel *pm)
{
    int i, j, found_path;
    ...
    for (i = 0; i < sip->n_subsystems; i++) {
        ...
        for (j = 0; j < pm->n_paths; j++) {
            if ((sip->subsystems[i].subobj_num != -1) &&
                (sip->subsystems[i].subobj_num == pm->paths[j].parent_submodel)) {
                found_path = 1;
            }
            else if (!stricmp(sip->subsystems[i].subobj_name,
                              pm->paths[j].parent_name)) {
                found_path = 1;
            }
...
void
ship_set_bay_path_nums(ship_info *sip, polymodel *pm)
{
    int bay_num, i;
    char bay_num_str[3];
    ...
    pm->ship_bay->arrive_flags =
        0; // bitfield, set to 1 when that path number is reserved for an arrival
    pm->ship_bay->depart_flags =
        0; // bitfield, set to 1 when that path number is reserved for a departure
...
void
ship_make_create_time_unique(ship *shipp)
{
...
// -1 means failed.
int
ship_create(matrix *orient, vector *pos, int ship_type)
{
    ...
    Assert(sip->flags & (SIF_SMALL_SHIP | SIF_BIG_SHIP | SIF_CAPITAL |
                         SIF_NO_SHIP_TYPE | SIF_NOT_FLYABLE | SIF_ESCAPEPOD |
                         SIF_SUPERCAP | SIF_DRYDOCK | SIF_KNOSSOS_DEVICE));

    sip->modelnum = model_load(
    ...
    if (strlen(sip->pof_file_hud)) {
        ...
        for (idx = 0; idx < Num_ship_types; idx++) {
            if (!stricmp(Ship_info[idx].pof_file, sip->pof_file_hud)) {
                Ship_info[idx].modelnum = model_load(
                    Ship_info[idx].pof_file, Ship_info[idx].n_subsystems,
                    &Ship_info[idx].subsystems[0]);
            }
        }
    ...
    if (sip->flags & SIF_NAVBUOY) {
        ...
        objnum = obj_create(OBJ_SHIP, -1, n, orient, pos,
                            model_get_radius(shipp->modelnum),
                            OF_RENDERS | OF_PHYSICS);
    }
    else {
        objnum = obj_create(OBJ_SHIP, -1, n, orient, pos,
                            model_get_radius(shipp->modelnum),
    ...
    if (pm->shield.ntris) {
        shipp->shield_integrity = (float *)malloc(sizeof(float) *
                                                  pm->shield.ntris);
                ...
                for (k = 0; k < pm->paths[i].verts[j].nturrets; k++) {
                    int ptindex =
                        pm->paths[i].verts[j].turret_ids
                            [k]; // this index is a submodel number (ala bspgen)
                    int index;
                    ship_subsys *ss;
                    ...
                    while (ss != END_OF_LIST(&Ships[n].subsys_list)) {
                        if (ss->system_info->subobj_num ==
                            ...
                            pm->paths[i].verts[j].turret_ids[k] =
                                index; // in path structure to index a ship_subsys
                            break;
                        }
                        index++;
                        ss = GET_NEXT(ss);
    ...
    shipp->wing_status_wing_pos =
        -1; // wing position (0-5) in wingman status gauge
...
//
void
ship_model_change(int n, int ship_type)
{
    int model_num, i;
    ship_info *sip;
    ship *sp;
    ...
    model_num = model_load(sip->pof_file, sip->n_subsystems,
                           &sip->subsystems[0]); // use the highest detail level
...
//
void
change_ship_type(int n, int ship_type)
{
    ship_info *sip;
    ship *sp;
    object *objp;
...
int
ship_fire_primary_debug(object *objp)
{
    int i;
    ...
    if (i != MAX_WEAPONS) {
        ...
        weapon_objnum = weapon_create(&wpos, &objp->orient, i, OBJ_INDEX(objp),
                                      0);
        weapon_set_tracking_info(weapon_objnum, OBJ_INDEX(objp),
...
int
ship_launch_countermeasure(object *objp, int rand_val)
{
    int fired;
    vector pos;
    ship *shipp;
    ...
    shipp->cmeasure_fire_stamp = timestamp(
        CMEASURE_WAIT); // Can launch every half second.
#ifndef NDEBUG
    ...
    // check the count of countermeasures left
    fired = -1;

    if (shipp->cmeasure_count <= 0) {
        if (objp == Player_obj) {
    ...
    if (Cmeasure_info[shipp->current_cmeasure].launch_sound != -1) {
        snd_play_3d(&Snds[Cmeasure_info[shipp->current_cmeasure].launch_sound],
                    &pos, &View_position);
    }
...
// internal function.. see if enough time has elapsed to play fail sound again
int
ship_maybe_play_secondary_fail_sound(weapon_info *wip)
{
    hud_start_flash_weapon(Player_ship->weapons.num_primary_banks +
...
//             0  =>    weapon can fire
int
ship_weapon_maybe_fail(ship *sp)
{
    int rval;
...
void
ship_fire_tracer(int weapon_objnum)
{
    particle_info pinfo;
    object *objp = &Objects[weapon_objnum];
    weapon_info *wip =
        &Weapon_info[Weapons[Objects[weapon_objnum].instance].weapon_info_index];
...
float Ship_fire_delay_scale_hostile[NUM_SKILL_LEVELS] = { 4.0f, 2.5f, 1.75f,
                                                          1.25f, 1.0f };
float Ship_fire_delay_scale_friendly[NUM_SKILL_LEVELS] = { 2.0f, 1.4f, 1.25f,
                                                           1.1f, 1.0f };

int tracers[MAX_SHIPS][4][4];

// fires a primary weapon for the given object.
int
ship_fire_primary(object *obj, int stream_weapons, int force)
{
    ...
    for (i = 0; i < num_primary_banks; i++) {
        ...
        if (!timestamp_elapsed(swp->next_primary_fire_stamp[bank_to_fire])) {
            if (timestamp_until(swp->next_primary_fire_stamp[bank_to_fire]) >
                5000) {
        ...
        if (!(obj->flags & OF_PLAYER_SHIP)) {
            if (shipp->team == Ships[Player_obj->instance].team) {
                next_fire_delay *=
                    Ship_fire_delay_scale_friendly[Game_skill_level];
            }
            else {
        ...
        next_fire_delay *= 1.0f + (num_primary_banks - 1) *
        ...
        if (po->n_guns > 0) {
            ...
            if ((shipp->weapon_energy < num_slots * winfo_p->energy_consumed) &&
                !force) {
                if (obj == Player_obj) {
                    swp->next_primary_fire_stamp[bank_to_fire] = timestamp(
                        swp->next_primary_fire_stamp[bank_to_fire]);
            ...
            for (j = 0; j < num_slots; j++) {
                ...
                // create the weapon
                weapon_objnum = weapon_create(&firing_pos, &obj->orient, weapon,
                                              OBJ_INDEX(obj), 0, new_group_id);
                weapon_set_tracking_info(weapon_objnum, OBJ_INDEX(obj),
                                         aip->target_objnum,
                                         aip->current_target_is_locked,
                                         aip->targeted_subsys);

                // create the muzzle flash effect
                shipfx_flash_create(obj, shipp, &pnt, &obj->orient.fvec, 1,
                                    weapon);

                // maybe shudder the ship - if its me
                if ((winfo_p->wi_flags & WIF_SHUDDER) && (obj == Player_obj)) {
                    // calculate some arbitrary value between 100
                    // (mass * velocity) / 10
                    game_shudder_apply(500, (winfo_p->mass * winfo_p->max_speed) /
                                                10.0f);
                }
            ...
            if (obj == Player_obj) {
                if (winfo_p->launch_snd != -1) {
                    ...
                    if (winfo_p->launch_snd == SND_AUTOCANNON_SHOT) {
                        snd_play(&Snds[winfo_p->launch_snd], 0.0f, 1.0f,
                ...
                if (winfo_p->launch_snd != -1) {
                    snd_play_3d(&Snds[winfo_p->launch_snd], &obj->pos,
                                &View_position);
                }
            }
        }
...
void
ship_start_targeting_laser(ship *shipp)
{
    int bank0_laser = 0;
    int bank1_laser = 0;
    ...
    if ((shipp->weapons.primary_bank_weapons[0] >= 0) &&
        (Weapon_info[shipp->weapons.primary_bank_weapons[0]].wi_flags &
         WIF_BEAM) &&
        (Weapon_info[shipp->weapons.primary_bank_weapons[0]].b_info.beam_type ==
         BEAM_TYPE_C)) {
        bank0_laser = 1;
    }
    if ((shipp->weapons.primary_bank_weapons[1] >= 0) &&
        (Weapon_info[shipp->weapons.primary_bank_weapons[1]].wi_flags &
         WIF_BEAM) &&
        (Weapon_info[shipp->weapons.primary_bank_weapons[1]].b_info.beam_type ==
         BEAM_TYPE_C)) {
        bank1_laser = 1;
    }
...
void
ship_stop_targeting_laser(ship *shipp)
{
    shipp->targeting_laser_bank = -1;
    shipp->targeting_laser_objnum = -1;
}
...
int
maybe_detonate_weapon(ship_weapon *swp, object *src)
{
...
int
ship_fire_secondary_detonate(object *obj, ship_weapon *swp)
{
...
int
ship_select_next_valid_secondary_bank(ship_weapon *swp)
{
    int cycled = 0;
...
extern void ai_maybe_announce_shockwave_weapon(object *firing_objp,
                                               int weapon_index);
...
int
ship_fire_secondary(object *obj, int allow_swarm)
{
    ...
    // in the case where the object is a ghost (the player right after death, for instance)
    ...
    Assert((swp->secondary_bank_weapons[bank] >= 0) &&
    ...
    if (ship_fire_secondary_detonate(obj, swp)) {
        ...
        if (swp->current_secondary_bank >= 0) {
            if (timestamp_elapsed(swp->next_secondary_fire_stamp[bank])) {
                swp->next_secondary_fire_stamp[bank] = timestamp(
                    max((int)flFrametime * 3000, 250));
            }
        }
        return 0;
    }
    ...
    if (wip->wi_flags & WIF_NO_DUMBFIRE) {
        if (aip->current_target_is_locked <= 0) {
            if (obj == Player_obj) {
                if (!Weapon_energy_cheat) {
                    if ((aip->target_objnum != -1) &&
                        (vm_vec_dist_quick(&obj->pos,
                                           &Objects[aip->target_objnum].pos) >
                         wip->lifetime * wip->max_speed)) {
                        HUD_sourced_printf(
                            HUD_SOURCE_HIDDEN,
                        ...
                        hud_end_string_at_first_hash_symbol(missile_name);
                        HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                                           XSTR("Cannot fire %s without a lock",
                                                488),
                                           missile_name);
                    }
                    ...
                    snd_play(&Snds[SND_OUT_OF_MISSLES]);
                    swp->next_secondary_fire_stamp[bank] = timestamp(
                        800); // to avoid repeating messages
                    return 0;
                }
            }
            else {
    ...
    if ((wip->wi_flags & WIF_CORKSCREW) && !allow_swarm) {
        shipp->num_corkscrew_to_fire =
            (ubyte)(shipp->num_corkscrew_to_fire +
                    (ubyte)Corkscrew_num_missiles_fired);
        return 1; // Note: Missiles didn't get fired, but the frame interval code will fire them.
    }

    swp->next_secondary_fire_stamp[bank] = timestamp(
        (int)(Weapon_info[weapon].fire_wait *
              1000.0f)); // They can fire 5 times a second
    ...
    if (ship_weapon_maybe_fail(shipp)) {
        if (obj == Player_obj)
            if (ship_maybe_play_secondary_fail_sound(wip)) {
                ...
                HUD_sourced_printf(
                    HUD_SOURCE_HIDDEN,
                    XSTR("Cannot fire %s due to weapons system damage", 489),
                    missile_name);
            }
        goto done_secondary;
    }
    ...
    if (po->n_missiles > 0) {
        ...
        if (shipp->flags & SF_SECONDARY_DUAL_FIRE) {
            ...
            if (swp->secondary_bank_ammo[bank] < 2) {
                end_slot = start_slot;
            }
            else {
        ...
        for (j = start_slot; j <= end_slot; j++) {
            ...
            // create the weapon
            weapon_num = weapon_create(&firing_pos, &obj->orient, weapon,
                                       OBJ_INDEX(obj), 0, -1,
                                       aip->current_target_is_locked);
            weapon_set_tracking_info(weapon_num, OBJ_INDEX(obj),
                                     aip->target_objnum,
                                     aip->current_target_is_locked,
                                     aip->targeted_subsys);
            ...
            swp->detonate_weapon_time = timestamp(
                500); //   Can detonate 1/2 second later.
            if (weapon_num != -1) {
    ...
    if (obj == Player_obj) {
        if (Weapon_info[weapon].launch_snd != -1) {
            snd_play(&Snds[Weapon_info[weapon].launch_snd], 0.0f, 1.0f,
                     SND_PRIORITY_MUST_PLAY);
        }
    }
    else {
        if (Weapon_info[weapon].launch_snd != -1) {
            snd_play_3d(&Snds[Weapon_info[weapon].launch_snd], &obj->pos,
                        &View_position);
        }
    }
    ...
    if ((obj->flags & OF_PLAYER_SHIP) && (swp->secondary_bank_ammo[bank] <= 0)) {
        if (ship_select_next_valid_secondary_bank(swp)) {
            ...
            if (timestamp_elapsed(
                    swp->next_secondary_fire_stamp[swp->current_secondary_bank])) {
                swp->next_secondary_fire_stamp[swp->current_secondary_bank] =
...
//       for the player ship.
int
ship_select_next_primary(object *objp, int direction)
{
    ship *shipp;
    ship_weapon *swp;
...
//       for the player ship.
int
ship_select_next_secondary(object *objp)
{
    Assert(objp != NULL);
    Assert(objp->type == OBJ_SHIP);
    Assert(objp->instance >= 0 && objp->instance < MAX_SHIPS);
    ...
    case 3:
        ...
        for (i = 1; i < swp->num_secondary_banks; i++) {
            new_bank = (swp->current_secondary_bank + i) %
...
int
ship_info_base_lookup(int si_index)
{
    int i;
...
int
get_subsystem_pos(vector *pos, object *objp, ship_subsys *subsysp)
{
    matrix m;
...
ship_model_start(object *objp)
{
    ...
    for (pss = GET_FIRST(&shipp->subsys_list);
        ...
        case SUBSYSTEM_TURRET:
            Assert(!(psub->flags &
                     MSS_FLAG_ROTATES)); // Turrets can't rotate!!! See John!
            break;
        default:
        ...
        if (psub->subobj_num > -1) {
            model_set_instance(shipp->modelnum, psub->subobj_num,
                               &pss->submodel_info_1);
        }

        if ((psub->subobj_num != psub->turret_gun_sobj) &&
            (psub->turret_gun_sobj > -1)) {
            model_set_instance(shipp->modelnum, psub->turret_gun_sobj,
                               &pss->submodel_info_2);
        }
    }
}
...
void
compute_slew_matrix(matrix *orient, angles *a)
{
    matrix tmp, tmp2;
    angles t1, t2;
...
void
ship_get_eye(vector *eye_pos, matrix *eye_orient, object *obj)
{
    ship *shipp;
    polymodel *pm;
    eye *ep;
    ...
    // element.
    model_find_world_point(eye_pos, &ep->pnt, shipp->modelnum, ep->parent,
                           &obj->orient, &obj->pos);
...
ship_subsys *
ship_get_best_subsys_to_attack(ship *sp, int subsys_type, vector *attacker_pos)
{
    ship_subsys *ss;
    ...
    for (ss = GET_FIRST(&sp->subsys_list); ss != END_OF_LIST(&sp->subsys_list);
        ...
        if ((ss->system_info->type == subsys_type) && (ss->current_hits > 0)) {
            ...
            if (ship_subsystem_in_sight(&Objects[sp->objnum], ss, attacker_pos,
                                        &gsubpos)) {
...
ship_subsys *
ship_get_indexed_subsys(ship *sp, int index, vector *attacker_pos)
{
    int count;
    ship_subsys *ss;
    ...
    if (index < 0) {
        ...
        subsys_type = -index;
        if (sp->subsys_info[subsys_type].current_hits ==
...
int
ship_get_index_from_subsys(ship_subsys *ssp, int objnum, int error_bypass)
{
    if (ssp == NULL)
        return -1;
    else {
...
int
ship_get_subsys_index(ship *sp, char *ss_name, int error_bypass)
{
    int count;
    ship_subsys *ss;
...
float
ship_get_subsystem_strength(ship *shipp, int type)
{
    float strength;
    ship_subsys *ssp;
    ...
    strength = shipp->subsys_info[type].current_hits /
               shipp->subsys_info[type].total_hits;
...
*/
    ...
    while (ssp != END_OF_LIST(&shipp->subsys_list)) {
        if (ssp->current_hits < ssp->system_info->max_hits &&
            ...
            if (shipp->subsys_info[subsys_type].current_hits >
                shipp->subsys_info[subsys_type].total_hits)
                shipp->subsys_info[subsys_type].current_hits =
                    shipp->subsys_info[subsys_type].total_hits;
...
object *
ship_find_repair_ship(object *requester_obj)
{
    object *objp;
    ship *requester_ship;
    ...
    Assert((requester_obj->instance >= 0) &&
           (requester_obj->instance < MAX_OBJECTS));
    ...
    for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list);
        ...
        if ((objp->type == OBJ_SHIP) && !(objp->flags & OF_SHOULD_BE_DEAD)) {
            ...
            if (num_support_ships >= MAX_SUPPORT_SHIPS_PER_TEAM) {
                mprintf(
                    ("Why is there more than %d support ships in this mission?\n",
...
ship_assign_sound(ship *sp)
{
    ...
    while (moveup != END_OF_LIST(&sp->subsys_list)) {
        ...
        if (strstr(moveup->system_info->name, "enginelarge")) {
            obj_snd_assign(sp->objnum, SND_ENGINE_LOOP_LARGE,
                           &moveup->system_info->pnt, 0);
        }
        else if (strstr(moveup->system_info->name, "enginehuge")) {
            obj_snd_assign(sp->objnum, SND_ENGINE_LOOP_HUGE,
                           &moveup->system_info->pnt, 0);
        }
    ...
    if (Dc_command) {
        ...
        if (!stricmp(Dc_arg, "weapons")) {
            ...
            if ((Dc_arg_float < 0.0f) || (Dc_arg_float > 1.0f)) {
                ...
                ship_set_subsystem_strength(Player_ship, SUBSYSTEM_WEAPONS,
                                            Dc_arg_float);
            }
        }
            ...
            if ((Dc_arg_float < 0.0f) || (Dc_arg_float > 1.0f)) {
                ...
                ship_set_subsystem_strength(Player_ship, SUBSYSTEM_ENGINE,
                                            Dc_arg_float);
            ...
            if ((Dc_arg_float < 0.0f) || (Dc_arg_float > 1.0f)) {
                ...
                ship_set_subsystem_strength(Player_ship, SUBSYSTEM_SENSORS,
                                            Dc_arg_float);
            }
        }
            ...
            if ((Dc_arg_float < 0.0f) || (Dc_arg_float > 1.0f)) {
                ...
                ship_set_subsystem_strength(Player_ship, SUBSYSTEM_COMMUNICATION,
                                            Dc_arg_float);
            }
        }
            ...
            if ((Dc_arg_float < 0.0f) || (Dc_arg_float > 1.0f)) {
                ...
                ship_set_subsystem_strength(Player_ship, SUBSYSTEM_NAVIGATION,
                                            Dc_arg_float);
            }
        }
            ...
            if ((Dc_arg_float < 0.0f) || (Dc_arg_float > 1.0f)) {
                ...
                ship_set_subsystem_strength(Player_ship, SUBSYSTEM_RADAR,
                                            Dc_arg_float);
            }
        }
        else {
...
// what is happening.
void
ship_add_ship_type_count(int ship_info_flag, int num)
{
...
void
ship_add_ship_type_kill_count(int ship_info_flag)
{
    if (ship_info_flag & SIF_CARGO)
...
int
ship_query_general_type(int ship)
{
    return ship_query_general_type(&Ships[ship]);
}

int
ship_query_general_type(ship *shipp)
{
    int flags;
...
int
ship_docking_valid(int docker, int dockee)
{
    ...
    if (docker_type == SHIP_TYPE_ESCAPEPOD) {
        if ((dockee_type == SHIP_TYPE_TRANSPORT) ||
            (dockee_type == SHIP_TYPE_CRUISER) ||
            (dockee_type == SHIP_TYPE_FREIGHTER) ||
            (dockee_type == SHIP_TYPE_DRYDOCK) ||
            (dockee_type == SHIP_TYPE_CORVETTE) ||
            (dockee_type == SHIP_TYPE_GAS_MINER) ||
    ...
    if (docker_type == SHIP_TYPE_FREIGHTER) {
        if ((dockee_type == SHIP_TYPE_CARGO) ||
            (dockee_type == SHIP_TYPE_CRUISER) ||
            (dockee_type == SHIP_TYPE_CAPITAL) ||
            (dockee_type == SHIP_TYPE_SUPERCAP) ||
            (dockee_type == SHIP_TYPE_DRYDOCK) ||
            (dockee_type == SHIP_TYPE_CORVETTE) ||
            (dockee_type == SHIP_TYPE_GAS_MINER) ||
            (dockee_type == SHIP_TYPE_AWACS)) {
    ...
    // docker == cruiser
    if ((docker_type == SHIP_TYPE_CRUISER) ||
        (docker_type == SHIP_TYPE_CORVETTE) ||
        (docker_type == SHIP_TYPE_GAS_MINER) ||
        (docker_type == SHIP_TYPE_AWACS)) {
        if ((dockee_type == SHIP_TYPE_CARGO) ||
            (dockee_type == SHIP_TYPE_CRUISER) ||
            (dockee_type == SHIP_TYPE_CAPITAL) ||
            (dockee_type == SHIP_TYPE_SUPERCAP) ||
    ...
    if (docker_type == SHIP_TYPE_TRANSPORT) {
        if ((dockee_type == SHIP_TYPE_CARGO) ||
            (dockee_type == SHIP_TYPE_CRUISER) ||
            (dockee_type == SHIP_TYPE_FREIGHTER) ||
            (dockee_type == SHIP_TYPE_TRANSPORT) ||
            (dockee_type == SHIP_TYPE_CAPITAL) ||
            (dockee_type == SHIP_TYPE_ESCAPEPOD) ||
            (dockee_type == SHIP_TYPE_SUPERCAP) ||
            (dockee_type == SHIP_TYPE_DRYDOCK) ||
            (dockee_type == SHIP_TYPE_CORVETTE) ||
            (dockee_type == SHIP_TYPE_GAS_MINER) ||
    ...
    if (docker_type == SHIP_TYPE_REPAIR_REARM) {
        if ((dockee_type == SHIP_TYPE_FIGHTER_BOMBER) ||
...
int
ship_get_random_player_wing_ship(int flags, float max_dist, int persona_index,
                                 int get_first)
{
    ...
    for (i = 0; i < num_wings; i++) {
        ...
        if (wingnum == -1) {
            if (!stricmp(Wings[i].name, NOX("delta")) ||
                !stricmp(Wings[i].name, NOX("epsilon"))) {
                wingnum = i;
            }
        }
        ...
        for (j = 0; j < Wings[wingnum].current_count; j++) {
            ...
            if ((flags == SHIP_GET_NO_PLAYERS) &&
                (Objects[Ships[ship_index].objnum].flags & OF_PLAYER_SHIP)) {
                continue;
            }
            ...
            if (max_dist > 0) {
                float dist;
                dist = vm_vec_dist_quick(&Objects[Ships[ship_index].objnum].pos,
                                         &Player_obj->pos);
                if (dist > max_dist) {
                    continue;
                }
...
// input:   max_dist => OPTIONAL PARAMETER (default value 0.0f) max range ship can be from player
int
ship_get_random_ship_in_wing(int wingnum, int flags, float max_dist,
                             int get_first)
{
...
// input:   max_dist => OPTIONAL PARAMETER (default value 0.0f) max range ship can be from player
int
ship_get_random_team_ship(int team, int flags, float max_dist)
{
    int num, which_one;
    ...
    for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list);
        ...
        if (Ships[objp->instance].team != team)
            continue;
        else if (Ship_info[Ships[objp->instance].ship_info_index].flags &
                 SIF_NOT_FLYABLE)
            continue;
        else if ((flags == SHIP_GET_NO_PLAYERS) && (objp->flags & OF_PLAYER_SHIP))
            continue;
        else if ((flags == SHIP_GET_ONLY_PLAYERS) &&
...
//
int
ship_secondary_bank_has_ammo(int shipnum)
{
    ship_weapon *swp;
    ...
    Assert(swp->current_secondary_bank >= 0 &&
           swp->current_secondary_bank < MAX_SECONDARY_BANKS);
...
//
int
ship_return_subsys_path_normal(ship *sp, ship_subsys *ss, vector *gsubpos,
                               vector *norm)
{
    if (ss->system_info->path_num >= 0) {
        ...
        if (mp->nverts >= 2) {
            ...
            vm_vec_unrotate(&gpath_point, path_point,
                            &Objects[sp->objnum].orient);
...
int
ship_subsystem_in_sight(object *objp, ship_subsys *subsys, vector *eye_pos,
                        vector *subsys_pos, int do_facing_check, float *dot_out,
                        vector *vec_out)
{
    float dist, dot;
    mc_info mc;
    ...
    if (do_facing_check) {
        if (ship_return_subsys_path_normal(&Ships[objp->instance], subsys,
                                           subsys_pos, &subsys_fvec)) {
...
ship_subsys *
ship_return_next_subsys(ship *shipp, int type, vector *attacker_pos)
{
    ship_subsys *ssp;
...
float
ship_quadrant_shield_strength(object *hit_objp, vector *hitpos)
{
    int quadrant_num, i;
    float max_quadrant;
    vector tmpv1, tmpv2;
    ...
    max_quadrant = Ship_info[Ships[hit_objp->instance].ship_info_index].shields /
                   4.0f;
    if (max_quadrant <= 0) {
        return 0.0f;
    }
...
int
ship_has_homing_missile_locked(ship *shipp)
{
...
int
ship_is_getting_locked(ship *shipp)
{
    ship_obj *so;
    object *objp;
    ai_info *aip;
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if (aip->target_objnum == shipp->objnum) {
            if (aip->aspect_locked_time > 0.1f) {
                ...
                wep_range = ship_get_secondary_weapon_range(
                    &Ships[objp->instance]);
                if (wep_range > dist) {
...
//
char *
ship_return_orders(char *outbuf, ship *sp)
{
    ai_info *aip;
    ai_goal *aigp;
    char *order_text;
    ...
    case AI_GOAL_DESTROY_SUBSYSTEM: {
        ...
        if (aip->targeted_subsys != NULL) {
            sprintf(outbuf, XSTR("atk %s %s", 496), aigp->ship_name,
                    hud_targetbox_truncate_subsys_name(
...
// it suits your needs for something.
char *
ship_return_time_to_goal(char *outbuf, ship *sp)
{
    ai_info *aip;
    ...
    if (aip->mode == AIM_WAYPOINTS) {
        ...
        if (aip->wp_list >= 0) {
            ...
            for (int i = aip->wp_index; i < wpl->count - 1; i++) {
                dist += vm_vec_dist_quick(&wpl->waypoints[i],
                                          &wpl->waypoints[i + 1]);
            }
        }
...
ship_check_cargo_all()
{
    ...
    while (cargo_so != END_OF_LIST(&Ship_obj_list)) {
        ...
        if ((Ship_info[cargo_sp->ship_info_index].flags & SIF_CARGO) &&
            !(cargo_sp->team & TEAM_FRIENDLY)) {
            ...
            for (ship_so = GET_FIRST(&Ship_obj_list);
                 ship_so != END_OF_LIST(&Ship_obj_list);
                ...
                if (Ship_info[ship_sp->ship_info_index].flags &
                    ...
                    dist_squared = vm_vec_dist_squared(
                        &cargo_objp->pos, &Objects[ship_sp->objnum].pos);
                    limit_squared = (cargo_objp->radius + CARGO_RADIUS_DELTA) *
                                    (cargo_objp->radius + CARGO_RADIUS_DELTA);
                    if (dist_squared <=
                        max(limit_squared,
                            CARGO_REVEAL_MIN_DIST * CARGO_REVEAL_MIN_DIST)) {
...
ship_maybe_warn_player(ship *enemy_sp, float dist)
{
    ...
    vm_vec_normalized_dir(&vec_to_target, &Objects[enemy_sp->objnum].pos,
                          &Eye_position);
...
*/
    ...
    if (msg_type != -1) {
        ...
        if (ship_index >= 0) {
            message_send_builtin_to_player(msg_type, &Ships[ship_index],
                                           MESSAGE_PRIORITY_HIGH,
                                           MESSAGE_TIME_IMMEDIATE, 0, 0);
...
ship_maybe_praise_player(ship *deader_sp)
{
    ...
    if (deader_sp->team ==
        Player_ship->team) { // only praise if killing an enemy!
        return;
    }
...
// -----------------------------------------------------------------------------
void
awacs_maybe_ask_for_help(ship *sp)
{
    object *objp;
    int message = -1;
    objp = &Objects[sp->objnum];
    ...
    if (objp->hull_strength <
        ((AWACS_HELP_HULL_LOW + 0.01f * (static_rand(objp - Objects) & 5)) *
         Ship_info[sp->ship_info_index].initial_hull_strength)) {
        ...
        if (!(sp->awacs_warning_flag & AWACS_WARN_25)) {
            ...
            sp->awacs_warning_flag |= AWACS_WARN_25;
             ...
             ((AWACS_HELP_HULL_HI + 0.01f * (static_rand(objp - Objects) & 5)) *
              Ship_info[sp->ship_info_index].initial_hull_strength)) {
    ...
    if (message >= 0) {
        message_send_builtin_to_player(message, sp, MESSAGE_PRIORITY_HIGH,
                                       MESSAGE_TIME_IMMEDIATE, 0, 0);
...
// -----------------------------------------------------------------------------
void
ship_maybe_ask_for_help(ship *sp)
{
    object *objp;
    ...
    if (get_shield_strength(objp) >
        (ASK_HELP_SHIELD_PERCENT * Ship_info[sp->ship_info_index].shields)) {
        return;
    }

play_ask_help:

    Assert(Ship_info[sp->ship_info_index].flags &
           (SIF_FIGHTER | SIF_BOMBER)); // get Alan
    message_send_builtin_to_player(MESSAGE_HELP, sp, MESSAGE_PRIORITY_HIGH,
                                   MESSAGE_TIME_IMMEDIATE, 0, 0);
    Player->allow_ask_help_timestamp = timestamp(PLAYER_ASK_HELP_INTERVAL);

    if (timestamp_until(Player->allow_scream_timestamp) < 15000) {
        Player->allow_scream_timestamp = timestamp(
            15000); // prevent overlap with death message
    }
...
ship_maybe_lament()
{
    ...
    if (rand() % 4 == 0) {
        ship_index = ship_get_random_player_wing_ship(SHIP_GET_NO_PLAYERS);
        if (ship_index >= 0) {
            message_send_builtin_to_player(MESSAGE_PLAYED_DIED,
                                           &Ships[ship_index],
                                           MESSAGE_PRIORITY_HIGH,
                                           MESSAGE_TIME_IMMEDIATE, 0, 0);
        }
...
ship_scream(ship *sp)
{
    ...
    message_send_builtin_to_player(MESSAGE_WINGMAN_SCREAM, sp,
                                   MESSAGE_PRIORITY_HIGH, MESSAGE_TIME_IMMEDIATE,
                                   0, 0);
    ...
    if (timestamp_until(Player->allow_ask_help_timestamp) < 15000) {
        Player->allow_ask_help_timestamp = timestamp(
            15000); // prevent overlap with death message
    }
}
...
ship_maybe_scream(ship *sp)
{
    ...
    // if on different teams, no scream
...
void
ship_maybe_tell_about_rearm(ship *sp)
{
    ...
    if (Objects[sp->objnum].hull_strength /
            Ship_info[sp->ship_info_index].initial_hull_strength <
        0.4) {
        heavily_damaged = 1;
    }
    ...
    if (message_type >= 0) {
        if (rand() & 1) {
            message_send_builtin_to_player(message_type, sp,
                                           MESSAGE_PRIORITY_NORMAL,
                                           MESSAGE_TIME_SOON, 0, 0);
        }
        Player->request_repair_timestamp = timestamp(
            PLAYER_REQUEST_REPAIR_MSG_INTERVAL);
    }
}

int
ship_get_SIF(ship *shipp)
{
...
int
ship_get_by_signature(int signature)
{
    ship_obj *so;
...
// (at least when this function was written), one for the player, and one for AI ships.
void
ship_do_cargo_revealed(ship *shipp)
{
    ...
    if (stricmp(Cargo_names[shipp->cargo1 & CARGO_INDEX_MASK], NOX("nothing"))) {
        mission_log_add_entry(LOG_CARGO_REVEALED, shipp->ship_name, NULL,
                              (shipp->cargo1 & CARGO_INDEX_MASK));
    }
}

void
ship_do_cap_subsys_cargo_revealed(ship *shipp, ship_subsys *subsys)
{
    ...
    if ((subsys->subsys_cargo_name > 0) &&
        stricmp(Cargo_names[subsys->subsys_cargo_name], NOX("nothing"))) {
        mission_log_add_entry(LOG_CAP_SUBSYS_CARGO_REVEALED, shipp->ship_name,
                              subsys->system_info->name,
...
float
ship_get_secondary_weapon_range(ship *shipp)
{
    float srange = 0.0f;
...
//
int
get_max_ammo_count_for_bank(int ship_class, int bank, int ammo_type)
{
    float capacity, size;
...
ship_page_in()
{
    ...
    for (p_objp = GET_FIRST(&ship_arrival_list);
         p_objp != END_OF_LIST(&ship_arrival_list); p_objp = GET_NEXT(p_objp)) {
    ...
    for (i = 0; i < MAX_SHIP_TYPES; i++) {
        if (ship_class_used[i]) {
            ...
            for (j = 0; j < MAX_SHIP_TYPES; j++) {
                if ((Ship_info[j].modelnum > -1) &&
                    !stricmp(si->pof_file, Ship_info[j].pof_file)) {
            ...
            if (model_previously_loaded > -1) {
                ...
                si->modelnum = model_load(si->pof_file, si->n_subsystems,
                                          &si->subsystems[0]);
...
// Verify that all the subsystem model numbers are updated
    ...
    nprintf(("Paging", "There are %d ship classes used in this mission.\n",
             num_ship_types_used));
    mprintf((
        "This mission requires %d Ship_subsystems. See #define MAX_SHIP_SUBOBJECTS.\n",
        num_subsystems_needed));
...
// Must be friendly and not Shivan.
int
is_support_allowed(object *objp)
{
    ...
    if (model_find_dock_index(Ships[objp->instance].modelnum, DOCK_TYPE_REARM) <
        0) {
        mprintf((
            "support not allowed for %s because its model lacks a rearming dockpoint\n",
...
void
ship_jettison_cargo(ship *shipp)
{
    object *objp;
    object *cargo_objp;
    vector impulse, pos;
    ...
    if (Ai_info[Ships[Objects[Ai_info[shipp->ai_index].dock_objnum].instance]
                    .ai_index]
            .dock_objnum != OBJ_INDEX(objp)) {
        return;
    }
    ...
    // whack the ship
    physics_apply_whack(&impulse, &pos, &cargo_objp->phys_info,
                        &cargo_objp->orient, cargo_objp->phys_info.mass);
}

float
ship_get_exp_damage(object *objp)
{
    Assert(objp->type == OBJ_SHIP);
    float damage;
    ...
    if (shipp->special_exp_index != -1) {
        damage = (float)atoi(
            Sexp_variables[shipp->special_exp_index + DAMAGE].text);
    }
    else {
...
int
ship_get_exp_propagates(ship *sp)
{
    return Ship_info[sp->ship_info_index].explosion_propagates;
}

float
ship_get_exp_outer_rad(object *ship_objp)
{
    float outer_rad;
    ...
    if (Ships[ship_objp->instance].special_exp_index == -1) {
        outer_rad =
            Ship_info[Ships[ship_objp->instance].ship_info_index].outer_rad;
    }
    else {
        outer_rad = (float)atoi(
            Sexp_variables[Ships[ship_objp->instance].special_exp_index +
                           OUTER_RAD]
                .text);
    }
...
int
valid_cap_subsys_cargo_list(char *subsys)
{
    if (strstr(subsys, "nav") || strstr(subsys, "comm") ||
        strstr(subsys, "engines") || strstr(subsys, "fighter") // fighter bays
        || strstr(subsys, "sensors") || strstr(subsys, "weapons")) {
        return 1;
    }
...
int
ship_get_turret_type(ship_subsys *subsys)
{
    // not a turret at all
...
ship_subsys *
ship_get_subsys(ship *shipp, char *subsys_name)
{
    ship_subsys *lookup;
...
int
wing_has_conflicting_teams(int wing_index)
{
    int first_team, idx;

    // sanity checks
    Assert((wing_index >= 0) && (wing_index < num_wings) &&
           (Wings[wing_index].current_count > 0));
...
int
ship_get_reinforcement_team(int r_index)
{
    int wing_index;
    p_object *objp;
...
int
ship_get_texture(int bitmap)
{
    int idx;

    // check all ship types
    for (idx = 0; idx < Num_ship_types; idx++) {
        if ((Ship_info[idx].modelnum >= 0) &&
            model_find_texture(Ship_info[idx].modelnum, bitmap) == 1) {
            return idx;
        }
    }
...
ship_update_artillery_lock()
{
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        Assert((shipp->weapons.current_primary_bank >= 0) &&
               (shipp->weapons.current_primary_bank < 2));
        if ((shipp->weapons.current_primary_bank < 0) ||
            (shipp->weapons.current_primary_bank >= 2)) {
            continue;
        }
        Assert(shipp->weapons
                   .primary_bank_weapons[shipp->weapons.current_primary_bank] >=
               0);
        if (shipp->weapons
                .primary_bank_weapons[shipp->weapons.current_primary_bank] < 0) {
            continue;
        }
        Assert((Weapon_info[shipp->weapons.primary_bank_weapons
        ...
        if (aip->artillery_lock_time >= 2.0f) {
            ...
            vector temp;
            vm_vec_unrotate(&temp, &aip->artillery_lock_pos,
                            &Objects[aip->artillery_objnum].orient);
            vm_vec_add2(&temp, &Objects[aip->artillery_objnum].pos);
            ssm_create(&temp, &Objects[so->objnum].pos, 0);
...
int
check_world_pt_in_expanded_ship_bbox(vector *world_pt, object *objp,
                                     float delta_box)
{
    Assert(objp->type == OBJ_SHIP);
    ...
    return ((ship_pt.x > pm->mins.x - delta_box) &&
            (ship_pt.x < pm->maxs.x + delta_box) &&
            (ship_pt.y > pm->mins.y - delta_box) &&
            (ship_pt.y < pm->maxs.y + delta_box) &&
            (ship_pt.z > pm->mins.z - delta_box) &&
            (ship_pt.z < pm->maxs.z + delta_box));
}

// returns true when objp is ship and is tagged
int
ship_is_tagged(object *objp)
{
    ship *shipp;
    if (objp->type == OBJ_SHIP) {
        shipp = &Ships[objp->instance];
...
float
ship_get_warp_speed(object *objp)
{
    Assert(objp->type == OBJ_SHIP);
...
int
ship_is_beginning_warpout_speedup(object *objp)
{
    Assert(objp->type == OBJ_SHIP);
...
int
ship_get_species_by_type(int ship_info_index)
{
    // sanity
