...
typedef struct
{
    ...
    int flags;
    char no_messages
        [MAX_REINFORCEMENT_MESSAGES]
        [NAME_LENGTH]; // list of messages to possibly send when calling for reinforcement not available
    char yes_messages
        [MAX_REINFORCEMENT_MESSAGES]
        [NAME_LENGTH]; // list of messages to acknowledge reinforcement on the way
} reinforcements;
...
typedef struct
{
    ...
    int primary_bank_weapons
        [MAX_PRIMARY_BANKS]; // Weapon_info[] index for the weapon in the bank
    int secondary_bank_weapons
        [MAX_SECONDARY_BANKS]; // Weapon_info[] index for the weapon in the bank
    ...
    int next_primary_fire_stamp
        [MAX_PRIMARY_BANKS]; // next time this primary bank can fire
    int next_secondary_fire_stamp
        [MAX_SECONDARY_BANKS]; // next time this secondary bank can fire

    int secondary_bank_ammo
        [MAX_SECONDARY_BANKS]; // Number of missiles left in secondary bank
    int secondary_bank_start_ammo
        [MAX_SECONDARY_BANKS]; // Number of missiles starting in secondary bank
    ...
    int secondary_bank_rearm_time
        [MAX_SECONDARY_BANKS]; // timestamp which indicates when bank can get new missile
...
typedef struct ship_subsys
{
    ...
    // turret info
    vector
        turret_last_fire_direction; // direction pointing last time this turret fired
    ...
    vector
        turret_big_attack_point; // local coordinate of point for this turret to attack on enemy
...
#include <ship/ai.hh> // ship_subsys must be declared before we include this.
#include <weapon/weapon.hh> // ship_subsys must be declared before we include this.
...
typedef struct ship
{
    ...
    ship_subsys_info
        subsys_info[SUBSYSTEM_MAX]; // info on particular generic types of subsystems
    ...
    int damage_ship_id
        [MAX_DAMAGE_SLOTS]; // signature of the damager (corresponds to each entry in damage_ship)
    ...
    int arc_timestamp
        [MAX_SHIP_ARCS]; // When this times out, the spark goes away.  -1 is not used
...
#define SIF_ALL_SHIP_TYPES                                                       \
    (SIF_CARGO | SIF_FIGHTER | SIF_BOMBER | SIF_CRUISER | SIF_FREIGHTER |        \
     SIF_CAPITAL | SIF_TRANSPORT | SIF_SUPPORT | SIF_NO_SHIP_TYPE |              \
     SIF_NAVBUOY | SIF_SENTRYGUN | SIF_ESCAPEPOD | SIF_SUPERCAP | SIF_CORVETTE | \
     SIF_GAS_MINER | SIF_AWACS | SIF_KNOSSOS_DEVICE)
#define SIF_SMALL_SHIP (SIF_FIGHTER | SIF_BOMBER | SIF_SUPPORT | SIF_ESCAPEPOD)
#define SIF_BIG_SHIP                                                             \
    (SIF_CRUISER | SIF_FREIGHTER | SIF_TRANSPORT | SIF_CORVETTE |                \
     SIF_GAS_MINER | SIF_AWACS)
#define SIF_HUGE_SHIP                                                            \
    (SIF_CAPITAL | SIF_SUPERCAP | SIF_DRYDOCK | SIF_KNOSSOS_DEVICE)
#define SIF_NOT_FLYABLE                                                          \
    (SIF_CARGO | SIF_NAVBUOY |                                                   \
     SIF_SENTRYGUN) // AL 11-24-97: this useful to know for targeting reasons
#define SIF_HARMLESS                                                             \
    (SIF_CARGO | SIF_NAVBUOY |                                                   \
     SIF_ESCAPEPOD) // AL 12-3-97: ships that are not a threat
// for ships of this type, we make beam weapons miss a little bit otherwise they'd be way too powerful
#define SIF_BEAM_JITTER                                                          \
    (SIF_CARGO | SIF_FIGHTER | SIF_BOMBER | SIF_FREIGHTER | SIF_TRANSPORT |      \
     SIF_SENTRYGUN | SIF_NAVBUOY | SIF_ESCAPEPOD)
...
typedef struct ship_info
{
    ...
    vector
        max_vel; //  max velocity of the ship in the linear directions -- read from ships.tbl
    vector
        afterburner_max_vel; //  max velocity of the ship in the linear directions when afterburners are engaged -- read from ships.tbl
    vector max_rotvel; // maximum rotational velocity
    vector
        rotation_time; // time to rotate in x/y/z dimension traveling at max rotvel
    ...
    int secondary_bank_ammo_capacity
        [MAX_SECONDARY_BANKS]; // Capacity of bank (not number of missiles)
    ...
    vector
        closeup_pos; // position for camera when using ship in closeup view (eg briefing and hud target monitor)
    float closeup_zoom; // zoom when using ship in closeup view (eg briefing and hud target monitor)

    int allowed_weapons
        [MAX_WEAPON_TYPES]; // array which specifies which weapons can be loaded out by the
    ...
    char icon_filename
        [NAME_LENGTH]; // filename for icon that is displayed in ship selection
    char anim_filename
        [NAME_LENGTH]; // filename for animation that plays in ship selection
    char overhead_filename
        [NAME_LENGTH]; // filename for animation that plays weapons loadout
...
typedef struct wing
{
    ...
    int ship_index
        [MAX_SHIPS_PER_WING]; // index into ships array of all ships currently in the wing
    ...
    ai_goal
        ai_goals[MAX_AI_GOALS]; // goals for the wing -- converted to ai_goal struct
...
extern int ship_check_collision_fast(object *obj, object *other_obj,
                                     vector *hitpos);
...
extern int get_available_secondary_weapons(object *objp, int *outlist,
                                           int *outbanklist);
...
extern int
    Num_player_ship_precedence; // Number of ship types in Player_ship_precedence
extern int Player_ship_precedence
...
extern void create_shield_explosion(int objnum, int model_num, matrix *orient,
...
extern void ship_get_eye(
    ...
    object
        *obj); // returns in eye the correct viewing position for the given object
extern ship_subsys *ship_get_indexed_subsys(
    ship *sp, int index,
    vector *attacker_pos = NULL); // returns index'th subsystem of this ship
extern int ship_get_index_from_subsys(ship_subsys *ssp, int objnum,
                                      int error_bypass = 0);
extern int ship_get_subsys_index(
    ship *sp, char *ss_name,
...
extern int
get_quadrant(vector *hit_pnt); //   Return quadrant num of last hit ponit.
...
int ship_return_subsys_path_normal(ship *sp, ship_subsys *ss, vector *gsubpos,
                                   vector *norm);
int ship_subsystem_in_sight(object *objp, ship_subsys *subsys, vector *eye_pos,
                            vector *subsys_pos, int do_facing_check = 1,
                            float *dot_out = NULL, vector *vec_out = NULL);
...
extern int ship_get_random_team_ship(int team, int flags = SHIP_GET_ANY_SHIP,
                                     float max_dist = 0.0f);
extern int ship_get_random_player_wing_ship(int flags = SHIP_GET_ANY_SHIP,
                                            float max_dist = 0.0f,
                                            int persona_index = -1,
                                            int get_first = 0);
extern int ship_get_random_ship_in_wing(int wingnum,
                                        int flags = SHIP_GET_ANY_SHIP,
                                        float max_dist = 0.0f, int get_first = 0);
...
void ship_get_global_turret_gun_info(object *objp, ship_subsys *ssp, vector *gpos,
                                     vector *gvec, int use_angles,
                                     vector *targetp);
...
// in global space.
void ship_get_global_turret_info(object *objp, model_subsystem *tp, vector *gpos,
                                 vector *gvec);
...
int object_in_turret_fov(object *objp, model_subsystem *tp, vector *tvec,
...
int check_world_pt_in_expanded_ship_bbox(vector *world_pt, object *objp,
                                         float delta_box);
