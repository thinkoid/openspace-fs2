...
// ----------------------------------------------------------
static int
    Player_afterburner_loop_id; // identifies the looping afterburner sound of the player ship
static int
    Player_afterburner_loop_delay; // timestamp used to time the start of the looping afterburner sound
...
//
void
afterburners_start(object *objp)
{
    ship_info *sip;
    ship *shipp;
    float percent_left;
    ...
    if (objp == Player_obj) {
        ...
        snd_play_3d(&Snds[SND_ABURN_ENGAGE], &objp->pos, &View_position,
                    objp->radius);
    }
...
afterburners_update(object *objp, float fl_frametime)
{
    ...
    if (!(objp->phys_info.flags & PF_AFTERBURNER_ON)) {
        ...
        if (shipp->afterburner_fuel < sip->afterburner_fuel_capacity) {
            ...
            recharge_scale =
                ...
                Skill_level_afterburner_recharge_scale[Game_skill_level];
            shipp->afterburner_fuel += (sip->afterburner_recover_rate *
                                        fl_frametime * recharge_scale);
    ...
    if (objp == Player_obj) {
        ...
        if (timestamp_elapsed(volume_chg_timer)) {
            ...
            percent_afterburner_left = shipp->afterburner_fuel /
            ...
            if (percent_afterburner_left < AFTERBURNER_PERCENT_VOL_ATTENUATE) {
                Player_afterburner_vol = percent_afterburner_left *
