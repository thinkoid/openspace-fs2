...
#define DELAY_TARGET_TIME                                                        \
    (12 *                                                                        \
     1000) //  time in milliseconds until a ship can target a new enemy after an order.

// Make enemy ships turn more slowly at lower skill levels.
float Turn_time_skill_level_scale[NUM_SKILL_LEVELS] = { 3.0f, 2.2f, 1.6f, 1.3f,
                                                        1.0f };
...
fix Skill_level_delay[NUM_SKILL_LEVELS] = { 2 * F1_0, 3 * F1_0 / 2, 4 * F1_0 / 3,
                                            F1_0 / 2, 0 };

// AI ships link primary weapons if energy levels greater than the following amounts:
float Link_energy_levels_always[NUM_SKILL_LEVELS] = { 100.0f, 80.0f, 60.0f, 40.0f,
                                                      20.0f }; // always link
float Link_energy_levels_maybe[NUM_SKILL_LEVELS] = {
    90.0f, 60.0f, 40.0f, 20.0f, 10.0f
}; // link if hull strength low
...
float Cmeasure_fire_chance[NUM_SKILL_LEVELS] = {
    0.2f, 0.3f, 0.5f, 0.9f, 1.1f
}; // Note, this gets scaled by ai_class
...
int Ai_friendly_rearm_timestamp, Ai_hostile_rearm_timestamp,
    Ai_neutral_rearm_timestamp, Ai_traitor_rearm_timestamp,
    Ai_unknown_rearm_timestamp;
...
void create_model_exit_path(object *pl_objp, object *mobjp, int path_num,
                            int count = 1);
void copy_xlate_model_path_points(object *objp, model_path *mp, int dir,
                                  int count, int path_num, pnode *pnp,
                                  int randomize_pnt = -1);
...
void
ai_set_rearm_status(int team, int time)
{
    Assert(time >= 0);
...
void
ai_good_secondary_time(int team, int weapon_index, int max_fire_count,
                       char *shipname)
{
    int i, index;
...
int
is_preferred_weapon(int weapon_num, object *firer_objp, object *target_objp)
{
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if (shipp->ai_index != -1) {
            ...
            if ((aip->path_length > 0) && (aip->path_start > -1)) {
                for (int i = aip->path_start;
                     i < aip->path_start + aip->path_length; i++) {
                    Assert(
                        pp_xlate[i] ==
                        0); //   If this is not 0, then two paths use this point!
                    pp_xlate[i] = 1;
                }
            }
        }
...
int
hash(unsigned int curval, int newval)
{
    int addval = curval & 1;
...
int
create_object_hash(object *objp)
{
    int *ip;
...
int
is_object_stealth_ship(object *objp)
{
...
void
init_ai_stealth_info(ai_info *aip, object *stealth_objp)
{
...
int
ai_is_stealth_visible(object *viewer_objp, object *stealth_objp)
{
    ship *shipp;
    ...
    if (!ship_is_visible_by_team(stealth_objp->instance,
                                 Ships[viewer_objp->instance].team)) {
        vm_vec_sub(&vec_to_stealth, &stealth_objp->pos, &viewer_objp->pos);
        dist_to_stealth = vm_vec_mag_quick(&vec_to_stealth);
        dot_to_stealth = vm_vec_dotprod(&viewer_objp->orient.fvec,
                                        &vec_to_stealth) /
                         dist_to_stealth;

        // get max dist at which stealth is visible
        max_stealth_dist = get_skill_stealth_dist_scaler() *
...
float
compute_dots(object *objp, object *other_objp, float *to_dot, float *from_dot)
{
    vector v2o;
    float dist;
...
void
ai_update_danger_weapon(int attacked_objnum, int weapon_objnum)
{
...
void
ai_turn_towards_vector(vector *dest, object *objp, float frametime,
                       float turn_time, vector *slide_vec, vector *rel_pos,
                       float bank_override, int flags, vector *rvec)
{
    //matrix   goal_orient;
    matrix curr_orient;
    ...
    if (objp->type == OBJ_WEAPON)
        delta_bank = 0.0f;
    else if ((bank_override) &&
             (Ships[objp->instance].team &
              opposing_team_mask(
                  Player_ship
                      ->team))) { //   Theoretically, this will only happen for Shivans.
...
int
set_target_objnum(ai_info *aip, int objnum)
{
    /*
...
set_targeted_subsys(ai_info *aip, ship_subsys *new_subsys, int parent_objnum)
{
    ...
    if (new_subsys) {
        ...
        if (new_subsys->system_info->type == SUBSYSTEM_ENGINE) {
            if (aip != Player_ai) {
                ai_select_primary_weapon(&Objects[Ships[aip->shipnum].objnum],
                                         &Objects[parent_objnum], WIF_PUNCTURE);
            }
        }
...
void
adjust_accel_for_docking(ai_info *aip)
{
    if (aip->dock_objnum != -1) {
        ...
        if (obj2p->signature == aip->dock_signature) {
            ...
            ratio = obj1p->phys_info.mass /
                    (obj1p->phys_info.mass + obj2p->phys_info.mass);
...
// -------------------------------------------------------------------
void
accelerate_ship(ai_info *aip, float accel)
{
...
// --------------------------------------------------------------------------
void
change_acceleration(ai_info *aip, float delta_accel)
{
    float new_accel;
...
void
set_accel_for_target_speed(object *objp, float tspeed)
{
    float max_speed;
    ai_info *aip;
...
void
project_point_to_perimeter(vector *perim_point, vector *pos, float radius,
                           vector *vp)
{
    vector v1;
    float mag;
...
void
get_tangent_point(vector *tan1, vector *p0, vector *centerp, vector *p1,
                  float radius)
{
...
// approach behavior.
void
turn_towards_point(object *objp, vector *point, vector *slide_vec,
                   float bank_override)
{
    ai_info *aip;
...
void
turn_away_from_point(object *objp, vector *point, float bank_override)
{
    ...
    ai_turn_towards_vector(
        &opposite_point, objp, flFrametime,
        Ship_info[Ships[objp->instance].ship_info_index].srotation_time, NULL,
        NULL, bank_override, AITTV_FAST);
}
...
float
turn_towards_tangent(object *objp, vector *point, float radius)
{
    vector vec_to_point;
    vector goal_point;
...
float
turn_toward_tangent_with_axis(object *objp, object *center_objp, float radius)
{
    ...
    vm_vec_scale_add(&center_vec, &center_objp->pos, &center_objp->orient.fvec,
                     center_obj_z);
...
void
get_tangent_point(vector *goal_point, object *objp, vector *point, float radius)
{
    vector vec_to_point;
    vector perp_point; //  point radius away from *point on vector to objp->pos
    vector up_vec, perp_vec;
...
int
object_is_targetable(object *target, ship *viewer)
{
    int stealth_ship = 0;

    // if target is ship, check if visible by team
    if (target->type == OBJ_SHIP) {
        stealth_ship = (Ship_info[Ships[target->instance].ship_info_index].flags &
                        SIF_STEALTH);
...
int
num_enemies_attacking(int objnum)
{
    object *objp;
    ship *sp;
    ship_subsys *ssp;
...
int
get_enemy_team_mask(int objnum)
{
    ...
    if (Mission_all_attack) {
        ...
        case TEAM_UNKNOWN:
            ...
            break;
        case TEAM_TRAITOR:
            enemy_team_mask = TEAM_FRIENDLY | TEAM_NEUTRAL | TEAM_HOSTILE |
                              TEAM_TRAITOR;
            break;
        default:
            enemy_team_mask = TEAM_HOSTILE;
...
float
get_wing_lowest_max_speed(object *objp)
{
    ship *shipp;
    ai_info *aip;
            ...
            if (oaip->ai_flags & AIF_DOCKED) {
                if (oaip->dock_objnum > -1)
                    if (Objects[oaip->dock_objnum].type == OBJ_SHIP)
                        cur_max *= o->phys_info.mass /
                                   (o->phys_info.mass +
                                    Objects[oaip->dock_objnum].phys_info.mass);
            }
...
//    FALSE otherwise
int
is_ignore_object(ai_info *aip, int objnum)
{
...
*/
    ...
    else if (aip->ignore_objnum >=
             0) { // This means it's ignoring an object, not a wing.
...
int
get_nearest_bbox_point(object *ship_obj, vector *start, vector *box_pt)
{
...
void
evaluate_object_as_nearest_objnum(eval_nearest_objnum *eno)
{
    ai_info *aip;
    ...
    if ((attacking_subsystem != NULL) ||
        !(eno->trial_objp->flags & OF_PROTECTED)) {
        if (OBJ_INDEX(eno->trial_objp) != eno->objnum) {
            ...
            if ((eno->enemy_wing != -1) &&
                (Ships[eno->trial_objp->instance].wingnum != eno->enemy_wing))
                return;
            ...
            ship_info *sip =
                &Ship_info[Ships[eno->trial_objp->instance].ship_info_index];
            ...
            if (Ships[eno->trial_objp->instance].team & eno->enemy_team_mask) {
                ...
                if (!eno->check_danger_weapon_objnum) {
                    ...
                    if (!object_is_targetable(
                        ...
                        if (!((sip->flags & SIF_STEALTH) &&
                              ai_is_stealth_visible(&Objects[eno->objnum],
                                                    eno->trial_objp))) {
                            return;
                        }
                    }
                ...
                if (sip->flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP)) {
                    ...
                    if (inside) {
                        ...
                        dist = vm_vec_dist_quick(&Objects[eno->objnum].pos,
                                                 &box_pt);
                    }
                }
                else {
                ...
                if ((Ship_info[Ships[eno->trial_objp->instance].ship_info_index]
                         .flags &
                ...
                if ((sip->flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP)) ||
                    ...
                    if (eno->trial_objp->flags & OF_PLAYER_SHIP) {
                        dist *=
                            1.0f +
                            (NUM_SKILL_LEVELS - Game_skill_level - 1) /
...
int
get_nearest_objnum(int objnum, int enemy_team_mask, int enemy_wing, float range,
                   int max_attackers)
{
    ...
    if ((eno.nearest_objnum == -1) && (enemy_wing != -1)) {
        return get_nearest_objnum(objnum, enemy_team_mask, -1, range,
                                  max_attackers);
    }
...
int
find_nearby_hostile(int objnum, int enemy_team_mask, float range, int *count)
{
    int nearest_objnum;
    float nearest_dist;
    object *objp;
...
int
valid_turret_enemy(object *objp, object *turret_parent)
{
    ...
    if (objp->type == OBJ_WEAPON) {
        if (Weapon_info[Weapons[objp->instance].weapon_info_index].wi_flags &
            WIF_BOMB) {
...
int
object_in_turret_fov(object *objp, model_subsystem *tp, vector *tvec,
                     vector *tpos, float dist)
{
    vector v2e;
    float dot;
...
int
bomb_headed_towards_ship(object *bomb_objp, object *ship_objp)
{
    float dot;
...
int
num_turrets_attacking(object *turret_parent, int target_objnum)
{
    ship_subsys *ss;
    ship *shipp;
    int count = 0;
...
void
evaluate_obj_as_target(object *objp, eval_enemy_obj_struct *eeo)
{
    ...
    if (objp->type == OBJ_SHIP) {
        ...
        if (eeo->big_only_flag) {
            if (!(Ship_info[shipp->ship_info_index].flags &
        ...
        if ((eeo->turret_subsys->weapons.flags & SW_FLAG_TAGGED_ONLY) &&
            !ship_is_tagged(objp)) {
            return;
        }
        ...
        if (!object_is_targetable(
            ...
            if (is_object_stealth_ship(objp)) {
                ...
                float speed_mod = -0.1f +
                                  vm_vec_mag_quick(&objp->phys_info.vel) / 70.0f;
    ...
    if (objp->type == OBJ_WEAPON) {
        if (Weapons[objp->instance].homing_object ==
            ...
            if (dist < eeo->nearest_homing_bomb_dist) {
                if ((eeo->current_enemy == -1) ||
                    object_in_turret_fov(objp, tp, eeo->tvec, eeo->tpos,
                                         dist + objp->radius)) {
        ...
        else if (bomb_headed_towards_ship(objp,
                                          &Objects[eeo->turret_parent_objnum])) {
    ...
    if ((shipp) && (dist < eeo->weapon_travel_dist)) {
        ...
        int num_att_turrets = num_turrets_attacking(turret_parent_obj,
                                                    OBJ_INDEX(objp));
...
int
is_target_beam_valid(ship_subsys *turret_subsys, int objnum)
{
    ...
    if (Weapon_info[tp->turret_weapon_type].wi_flags & WIF_BEAM) {
        ...
        if (Weapon_info[tp->turret_weapon_type].wi_flags & WIF_HUGE) {
            if (Objects[objnum].type == OBJ_SHIP &&
                !(Ship_info[Ships[Objects[objnum].instance].ship_info_index].flags &
                  (SIF_BIG_SHIP | SIF_HUGE_SHIP))) {
                return 0;
            }
        }
...
int
get_nearest_turret_objnum(int turret_parent_objnum, ship_subsys *turret_subsys,
                          int enemy_team_mask, vector *tpos, vector *tvec,
                          int current_enemy, int big_only_flag)
{
    ...
    weapon_travel_dist = min(Weapon_info[tp->turret_weapon_type].lifetime *
    ...
    if (ship_get_subsystem_strength(
            &Ships[Objects[turret_parent_objnum].instance], SUBSYSTEM_WEAPONS) >
        0) {
        weapon_system_ok = 1;
    }
    ...
    Assert(eeo.nearest_attacker_objnum < 0 ||
           is_target_beam_valid(turret_subsys, eeo.nearest_attacker_objnum));
...
// Don't attack a ship that already has at least max_attackers attacking it.
int
find_enemy(int objnum, float range, int max_attackers)
{
    int enemy_team_mask;
    ...
    if (timestamp_elapsed(aip->choose_enemy_timestamp)) {
        ...
        return get_nearest_objnum(objnum, enemy_team_mask, aip->enemy_wing, range,
                                  max_attackers);
    }
    else {
        aip->target_objnum = -1;
...
//          current_enemy  => objnum of current turret target
int
find_turret_enemy(ship_subsys *turret_subsys, int objnum, vector *tpos,
                  vector *tvec, int current_enemy, float fov,
                  int big_only_flag = 0)
{
    ...
    if ((sip->flags & SIF_SMALL_SHIP) && (aip->target_objnum != -1)) {
        ...
        if (Objects[target_objnum].signature == aip->target_signature) {
            if (Ships[Objects[target_objnum].instance].team & enemy_team_mask) {
                if (!(Objects[target_objnum].flags &
                      OF_PROTECTED)) { // check this flag as well.
                    // nprintf(("AI", "Frame %i: Object %i resuming goal of object %i\n", AI_FrameCount, objnum, target_objnum));
                    return target_objnum;
                }
            }
        ...
        if ((frand() < 0.8f) && (aip->target_objnum != -1) && Use_parent_target) {
            ...
            if (!((target_flags & OF_BEAM_PROTECTED) &&
                  (Weapon_info[tp->turret_weapon_type].wi_flags & WIF_BEAM))) {
                if (Objects[aip->target_objnum].type == OBJ_SHIP) {
                    ...
                    if (!big_only_flag ||
                        ...
                        if ((turret_subsys->weapons.flags &
                             SW_FLAG_TAGGED_ONLY) &&
    ...
    enemy_objnum = get_nearest_turret_objnum(objnum, turret_subsys,
                                             enemy_team_mask, tpos, tvec,
                                             current_enemy, big_only_flag);
    if (enemy_objnum >= 0) {
        Assert(!((Objects[enemy_objnum].flags & OF_BEAM_PROTECTED) &&
                 (Weapon_info[tp->turret_weapon_type].wi_flags & WIF_BEAM)));
...
void
ai_set_goal_maybe_abort_dock(object *objp, ai_info *aip)
{
...
void
force_avoid_player_check(object *objp, ai_info *aip)
{
    if (Ships[objp->instance].team == Player_ship->team) {
        aip->avoid_check_timestamp = timestamp(
            0); //   Force a check for collision next frame.
    }
}
...
void
ai_attack_object(object *attacker, object *attacked, int priority,
                 ship_subsys *ssp)
{
    ai_info *aip;
    ...
    if (!(Ship_info[Ships[attacker->instance].ship_info_index].flags &
          SIF_SMALL_SHIP)) {
    ...
    aip->mode = AIM_CHASE;
    aip->submode =
        SM_ATTACK; // AL 12-15-97: need to set submode?  I got an assert() where submode was bogus
    ...
    if (ssp == NULL) {
        ...
        if (aip->target_objnum != -1) {
            ...
            Objects[aip->target_objnum].flags &=
                ~OF_PROTECTED; //   If ship had been protected, unprotect it.
        }
    }
    else {
...
// Attack point *rel_pos on object.  This is for supporting attacking subsystems.
void
ai_attack_wing(object *attacker, int wingnum, int priority)
{
    ai_info *aip;
...
void
ai_evade_object(object *evader, object *evaded, int priority)
{
    ai_info *aip;
...
void
ai_ignore_object(object *ignorer, object *ignored, int priority)
{
    ai_info *aip;
...
// Ignore some object without changing mode.
void
ai_ignore_wing(object *ignorer, int wingnum, int priority)
{
    ai_info *aip;
...
void
add_path_point(vector *pos, int path_num, int path_index, int modify_index)
{
    pnode *pnp;
...
void
bisect_chord(vector *p0, vector *p1, vector *centerp, float radius)
{
    vector tvec;
    vector new_pnt;
...
void
create_path_to_point(vector *curpos, vector *goalpos, object *curobjp,
                     object *goalobjp, int subsys_path)
{
...
void
copy_xlate_model_path_points(object *objp, model_path *mp, int dir, int count,
                             int path_num, pnode *pnp, int randomize_pnt)
{
    matrix m;
    int i;
    vector v1;
    ...
    if (pnp == NULL)
        ...
        pp_index =
            0; // pp_index will get assigned to index in Path_points to reuse.
...
void
create_model_path(object *pl_objp, object *mobjp, int path_num, int subsys_path)
{
    ...
    if (pp_collide(&pl_objp->pos, &gp0, mobjp, pl_objp->radius)) {
        ...
        if (vm_vec_dist_quick(&pl_objp->pos, &mobjp->pos) < mobjp->radius) {
            project_point_to_perimeter(&perim_point2, &mobjp->pos, mobjp->radius,
                                       &pl_objp->pos);
        ...
        if (vm_vec_dist_quick(&mobjp->pos, &gp0) < mobjp->radius) {
            project_point_to_perimeter(&perim_point1, &mobjp->pos, mobjp->radius,
                                       &gp0);
            create_path_to_point(&perim_point2, &perim_point1, pl_objp, mobjp,
                                 subsys_path);
            add_path_point(&perim_point1, path_num, -1, -1);
        }
        else { // The predefined path extends outside the sphere.  Create path to that point.
            create_path_to_point(&perim_point2, &gp0, pl_objp, mobjp,
                                 subsys_path);
        }
    }
    ...
    aip->path_next_create_time = timestamp(
        1000); // OK to try to create one second later
...
// relative to the start of the path.
void
create_model_exit_path(object *pl_objp, object *mobjp, int path_num, int count)
{
    ship *shipp = &Ships[pl_objp->instance];
    ai_info *aip = &Ai_info[shipp->ai_index];
...
// Calls pp_collide
int
pp_collide_any(vector *curpos, vector *goalpos, float radius,
               object *ignore_objp1, object *ignore_objp2, int big_only_flag)
{
    ship_obj *so;
...
void
ai_find_path(object *pl_objp, int objnum, int path_num, int exit_flag,
             int subsys_path)
{
...
extern int vector_object_collision(vector *start_pos, vector *end_pos,
                                   object *objp, float radius_scale);
...
int
maybe_avoid_player(object *objp, vector *goal_pos)
{
    ai_info *aip;
    ...
    if (vector_object_collision(&objp->pos, &new_goal_pos, player_objp, 1.5f)) {
        ...
        vm_vec_scale_add(&aip->avoid_goal_point, &player_objp->pos, &avoid_vec,
                         400.0f);
        ...
        aip->ai_flags &= ~AIF_AVOIDING_SMALL_SHIP;
        aip->avoid_check_timestamp = timestamp((int)(obj_obj_dist / 200.0f) +
                                               500);
...
void
ai_stay_still(object *still_objp, vector *view_pos)
{
    ship *shipp;
    ai_info *aip;
    ...
    if (view_pos != NULL)
        ...
        vm_vec_scale_add(&aip->goal_point, &still_objp->pos,
                         &still_objp->orient.fvec, 100.0f);
}
...
void
ai_do_objects_docked_stuff(object *docker, object *dockee)
{
...
// has blown up!
void
ai_do_objects_undocked_stuff(object *docker, object *dockee)
{
    ai_info *aip, *other_aip;
...
void
ai_dock_with_object(object *docker, object *dockee, int priority, int dock_type,
                    int docker_index, int dockee_index)
{
    ...
    if ((aip->ai_flags & AIF_DOCKED) && (dock_type == AIDO_DOCK)) {
        ...
        nprintf(
            ("AI",
             "Ship %s told to dock with %s, but it was already docked with %s.\n",
             Ships[docker->instance].ship_name, Ships[dockee->instance].ship_name,
             Ships[Objects[aip->dock_objnum].instance].ship_name));
...
void
ai_start_waypoints(object *objp, int waypoint_list_index, int wp_flags)
{
    ai_info *aip;
...
void
ai_do_stay_near(object *objp, object *other_objp, float dist)
{
    ai_info *aip;
...
void
ai_form_on_wing(object *objp, object *goal_objp)
{
    ai_info *aip;
    ship *shipp;
    ship_info *sip;
...
int
ai_formation_object_get_slotnum(int objnum, object *objp)
{
    int slotnum =
        1; //  Note: Slot #0 means leader, which isn't someone who was told to form-on-wing.
    ...
    for (o = GET_FIRST(&obj_used_list); o != END_OF_LIST(&obj_used_list);
        ...
        if (objp == o)
            ...
            if (Ai_info[Ships[o->instance].ai_index].ai_flags &
                AIF_FORMATION_OBJECT)
    ...
    Assert(
        o !=
        END_OF_LIST(
            &obj_used_list)); // Didn't find objp in list of used ships.  Impossible!
...
float
compute_collision_time(vector *targpos, vector *targvel, vector *attackpos,
                       float weapon_speed)
{
...
float
compute_time_to_enemy(float dist_to_enemy, object *pobjp, object *eobjp)
{
    float time_to_enemy;
    ...
    if (dist_to_enemy > 1.5 * max_laser_distance) {
        ...
        if (pl_speed > 0.1f) {
            ...
            scale = (float)((dist_to_enemy - max_laser_distance) /
                            max_laser_distance);

            time_to_enemy = (float)(dist_to_enemy /
                                    (pl_speed * scale +
                                     max_laser_speed * (1.0f - scale)));
        }
        else
            time_to_enemy = 2.0f;
    }
...
void
fds_aux(float *dot, float *tts, vector *pos, float dtime, object *objp)
{
    vector v2s;
...
// --------------------------------------------------------------------------
void
ai_set_positions(object *pl_objp, object *en_objp, ai_info *aip,
                 vector *player_pos, vector *enemy_pos)
{
    *player_pos = pl_objp->pos;
    ...
    if (aip->next_predict_pos_time > Missiontime) {
        ...
        *enemy_pos = en_objp->pos;
        ...
        aip->next_predict_pos_time = Missiontime +
                                     Skill_level_delay[Game_skill_level];
...
// --------------------------------------------------------------------------
int
find_nearest_waypoint(object *objp)
{
    int i;
    ...
    for (i = 0; i < wpl->count; i++) {
        ...
        dot = vm_vec_dot_to_point(&objp->orient.fvec, &objp->pos,
                                  &wpl->waypoints[i]);
...
int
get_base_path_info(int path_cur, int goal_objnum, model_path **pmp,
                   mp_vert **pmpv)
{
...
void
modify_model_path_points(object *objp)
{
    ...
    copy_xlate_model_path_points(mobjp, &pm->paths[path_num], dir,
                                 pm->paths[path_num].nverts, path_num, pnp);
}
...
float
ai_matrix_dist(matrix *mat1, matrix *mat2)
{
    float t;
...
float
maybe_recreate_path(object *objp, ai_info *aip, int force_recreate_flag)
{
    int hashval;
    ...
    if (force_recreate_flag || timestamp_elapsed(aip->path_next_create_time)) {
        ...
        if ((hashval = create_object_hash(path_objp)) !=
            aip->path_goal_obj_hash) {
            float dist;

            dist = vm_vec_dist_quick(&path_objp->pos, &aip->path_create_pos);
            dist += ai_matrix_dist(&path_objp->orient, &aip->path_create_orient) *
                    25.0f;
...
void
set_accel_for_docking(object *objp, ai_info *aip, float dot, float dot_to_next,
    ...
    if (objp->phys_info.speed < 0.0f) {
        ...
        if ((aip->mode == AIM_DOCK) && (dist_to_next < 150.0f) &&
            ...
            set_accel_for_target_speed(
                objp, sip->max_speed * max(dist_to_next / 500.0f, 1.0f));
            ...
            if (dist_to_goal > 200.0f)
                set_accel_for_target_speed(objp,
                                           sip->max_speed * (dot + 1.0f) / 2.0f);
            else {
                float xdot;
                ...
                if ((aip->mode != AIM_DOCK) && (dot > 0.9f)) {
                    ...
                    if ((aip->path_cur - aip->path_start <
                        ...
                        set_accel_for_target_speed(objp,
                                                   dist_to_goal / 8.0f + 2.0f);
                    }
                    else {
                        set_accel_for_target_speed(
                            objp, sip->max_speed * (2 * xdot + 0.25f) / 4.0f);
                    }
                }
            }
        }
...
ai_path()
{
    ...
    if (aip->path_start == -1) {
        int path_num;
        path_num = ai_return_path_num_from_dockbay(&Objects[aip->goal_objnum],
                                                   aip->dockee_index);
    ...
    if ((aip->path_cur + aip->path_dir - aip->path_start < num_points) ||
        ...
        Assert((aip->path_cur - aip->path_dir >= aip->path_start) &&
...
*/
    ...
    if ((aip->mode != AIM_DOCK) ||
        ...
        if ((aip->path_cur + aip->path_dir > aip->path_start) &&
            ...
            if (timestamp_elapsed(aip->path_next_check_time)) {
                ...
                if (!pp_collide(&Pl_objp->pos, nvp, gobjp,
                                1.1f * Pl_objp->radius)) {
                    cvp = nvp;
    ...
    if (dist_to_goal > 0.1f)
        ai_turn_towards_vector(&gcvp, Pl_objp, flFrametime, sip->srotation_time,
                               slop_vec, NULL, 0.0f, 0);
    ...
    set_accel_for_docking(Pl_objp, aip, dot, dot_to_next, dist_to_next,
                          dist_to_goal, sip);
    aip->prev_dot_to_goal = dot;
    ...
    if ((dist_to_goal < MIN_DIST_TO_WAYPOINT_GOAL) ||
        ...
        r = find_nearest_point_on_line(&nearest_point, &Pl_objp->last_pos,
                                       &Pl_objp->pos, &gcvp);
...
void
update_min_max(float val, float *min, float *max)
{
    if (val < *min)
        *min = val;
    else if (val > *max)
...
int
get_enemy_team_range(object *my_objp, float range, int enemy_team_mask,
                     vector *min_vec, vector *max_vec)
{
    object *objp;
    ship_obj *so;
    int count = 0;
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if (Ships[objp->instance].team & enemy_team_mask) {
            if (Ship_info[Ships[objp->instance].ship_info_index].flags &
                (SIF_FIGHTER | SIF_BOMBER | SIF_CRUISER | SIF_CAPITAL |
                 SIF_SUPERCAP | SIF_DRYDOCK | SIF_CORVETTE | SIF_AWACS |
                 SIF_GAS_MINER))
...
void
ai_safety_pick_spot(object *objp)
{
    int objnum;
    int enemy_team_mask;
    ...
    if (get_enemy_team_range(objp, 1000.0f, enemy_team_mask, &min_vec,
                             &max_vec)) {
...
float
ai_safety_goto_spot(object *objp)
{
    float dot, dist;
    ai_info *aip;
    vector vec_to_goal;
...
void
ai_safety_circle_spot(object *objp)
{
    vector goal_point;
    ship_info *sip;
    float dot;
...
ai_waypoints()
{
    ...
    if (Ai_info[Ships[Pl_objp->instance].ai_index].wing >= 0) {
        scale =
            Wings[Ai_info[Ships[Pl_objp->instance].ai_index].wing].current_count;
        scale = (int)((scale + 1) / 2);
    }
    else {
        scale = 1;
    ...
    if (dist_to_goal > 0.1f) {
        ai_turn_towards_vector(wp_cur, Pl_objp, flFrametime,
                               sip->srotation_time * 3.0f * scale, slop_vec, NULL,
                               0.0f, 0);
    }
            ...
            if (wp_index >= wpl->count)
                if (aip->wp_flags & WPF_REPEAT) {
                    ...
                    if (treat_as_ship) {
                        ...
                        mission_log_add_entry(LOG_WAYPOINTS_DONE,
...
avoid_ship()
{
    ...
    if (away_dot < 0.0f) {
        ...
        vm_vec_scale_add(&goal_pos, &En_objp->pos, &En_objp->orient.fvec,
                         -100.0f);
...
int
maybe_resume_previous_mode(object *objp, ai_info *aip)
{
...
int
static_rand_timed(int num, int modulus)
{
    if (modulus < 2)
        return 1;
    else {
...
int
ai_maybe_fire_afterburner(object *objp, ai_info *aip)
{
    if (aip->ai_class == 0)
        ...
        if (aip->ai_class >= Num_ai_classes - 2)
            ...
            return static_rand_timed(objp - Objects,
                                     Num_ai_classes - aip->ai_class);
        }
    }
}
...
void
maybe_afterburner_after_ship_hit(object *objp, ai_info *aip, object *en_objp)
{
...
int
is_instructor(object *objp)
{
    return !strnicmp(Ships[objp->instance].ship_name, INSTRUCTOR_SHIP_NAME,
    ...
    if (locked_weapon_objp != NULL) {
        if (unlocked_weapon_objp != NULL) {
            if (vm_vec_dist_quick(&locked_weapon_objp->pos, &Pl_objp->pos) <
                1.5f *
                    vm_vec_dist_quick(&unlocked_weapon_objp->pos, &Pl_objp->pos))
    ...
    if (dot_from_enemy < 0.3f) {
        ...
        if ((dot_to_enemy < -0.5f) || (dot_to_enemy > 0.5f)) {
            ...
            if ((rdot < -0.5f) || (rdot > 0.5f))
                vm_vec_scale_add(&goal_point, &Pl_objp->pos,
                                 &Pl_objp->orient.rvec, -200.0f);
            else
                vm_vec_scale_add(&goal_point, &Pl_objp->pos,
                                 &Pl_objp->orient.rvec, 200.0f);
...
slide_face_ship()
{
    ...
    ai_turn_towards_vector(&En_objp->pos, Pl_objp, flFrametime,
    ...
    vm_vec_scale_add(&goal_pos, &En_objp->pos, &En_objp->orient.rvec,
                     right * 200.0f);
    vm_vec_scale_add(&goal_pos, &En_objp->pos, &En_objp->orient.uvec,
                     up * 200.0f);
...
evade_ship()
{
    ...
    if (Game_skill_level == NUM_SKILL_LEVELS - 1) {
        ...
        accel_val = (float)(((Missiontime ^ rand_int) >> 14) & 0x0f) / 32.0f +
                    0.5f;
        ...
        accelerate_ship(aip,
                        (float)(Game_skill_level + 2) / (NUM_SKILL_LEVELS + 1));
    ...
    if (dist > 250.0f) {
        ...
        vm_vec_scale_add(&goal_point, &enemy_pos, &En_objp->orient.fvec,
                         -1000.0f);
    }
        ...
        if (dot_to_enemy > 0.75f) {
            ...
            vm_vec_scale_add(&goal_point, &enemy_pos, &Pl_objp->orient.rvec,
                             1000.0f);
            if (dist < 100.0f)
            ...
            bank_override =
                Pl_objp->phys_info
                    .speed; //   In enemy's sights, not pointing at him, twirl away.
    ...
    evade_ship_l1:;
        if (aip->ai_evasion > myrand() * 100.0f / 32767.0f) {
            ...
            vm_vec_scale_add(&goal_point, &enemy_pos, &En_objp->orient.rvec,
                             scale);
            ...
            if ((aip->prev_goal_point.x != 0.0f) ||
                (aip->prev_goal_point.y != 0.0f) ||
                (aip->prev_goal_point.z != 0.0f))
...
int
ai_select_primary_weapon(object *objp, object *other_objp, int flags)
{
    ship *shipp = &Ships[objp->instance];
    ...
    if (flags & WIF_PUNCTURE) {
        if (swp->current_primary_bank >= 0) {
            ...
            if (Weapon_info[swp->primary_bank_weapons[bank_index]].wi_flags &
                WIF_PUNCTURE) {
        ...
        for (int i = 0; i < swp->num_primary_banks; i++) {
            if (swp->primary_bank_weapons[i] > -1) {
                if (!(Weapon_info[swp->primary_bank_weapons[i]].wi_flags &
                      WIF_PUNCTURE)) {
                    swp->current_primary_bank = i;
                    nprintf(("AI", "%i: Ship %s selecting weapon %s\n",
...
void
set_primary_weapon_linkage(object *objp)
{
    ship *shipp;
    ai_info *aip;
    ...
    if (Objects[aip->target_objnum].type == OBJ_SHIP) {
        ...
        if (ship_get_SIF(&Ships[Objects[aip->target_objnum].instance]) &
            SIF_BIG_SHIP) {
    ...
    if ((aip->active_goal != AI_GOAL_NONE) &&
        ...
        if (aip->goals[aip->active_goal].ai_mode &
            ...
            if (swp->num_primary_banks == 2) {
                if (!(Weapon_info[swp->primary_bank_weapons[0]].wi_flags &
                      WIF_PUNCTURE))
                    return;
                if (!(Weapon_info[swp->primary_bank_weapons[1]].wi_flags &
                      WIF_PUNCTURE))
                    return;
            }
        }
    ...
    if (shipp->weapon_energy > Link_energy_levels_always[Game_skill_level]) {
        ...
        if (objp->hull_strength <
            Ship_info[shipp->ship_info_index].initial_hull_strength / 3.0f)
            shipp->flags |= SF_PRIMARY_LINKED;
    }
}
...
void
ai_fire_primary_weapon(object *objp)
{
    ship *shipp = &Ships[objp->instance];
    ship_weapon *swp = &shipp->weapons;
    ship_info *sip;
    ...
    if (!(vm_vec_mag_quick(&G_predicted_pos) < AICODE_SMALL_MAGNITUDE)) {
        if (!vm_vec_cmp(&G_predicted_pos, &G_fire_pos)) {
            ...
            if (dot >
                .998629534f) { //   if within 3.0 degrees of desired heading, bash
    ...
    if (aip->target_objnum != -1) {
        ...
        if (tobjp->flags & OF_PROTECTED) {
            if (aip->targeted_subsys != NULL) {
                ...
                if (ship_get_subsystem_strength(&Ships[tobjp->instance], type) ==
                    0.0f) {
                    aip->target_objnum = -1;
                    return;
                }
...
int
num_nearby_fighters(int enemy_team_mask, vector *pos, float threshold)
{
    ship_obj *so;
    object *ship_objp;
    int count = 0;
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if (Ships[ship_objp->instance].team & enemy_team_mask) {
            if (Ship_info[Ships[ship_objp->instance].ship_info_index].flags &
                (SIF_FIGHTER | SIF_BOMBER)) {
...
void
ai_select_secondary_weapon(object *objp, ship_weapon *swp, int priority1 = -1,
                           int priority2 = -1)
{
    ...
    num_weapon_types = get_available_secondary_weapons(objp, weapon_id_list,
                                                       weapon_bank_list);
    ...
    for (i = 0; i < num_weapon_types; i++) {
        ...
        wi_flags = Weapon_info[swp->secondary_bank_weapons[weapon_bank_list[i]]]
                       .wi_flags;
        if (!(wi_flags & ignore_mask)) { //  Maybe bombs are illegal.
            if (wi_flags & priority1) {
                ...
                priority2_index = weapon_bank_list
                    [i]; //   Found second priority, but might still find first priority.
        }
    }
...
int
compute_num_homing_objects(object *target_objp)
{
    object *objp;
    int count = 0;
...
ai_maybe_announce_shockwave_weapon(object *firing_objp, int weapon_index)
{
    if ((firing_objp->type == OBJ_SHIP) &&
        (Weapon_info[weapon_index].shockwave_speed > 0.0f)) {
        ship_obj *so;
...
float
compute_incoming_payload(object *target_objp)
{
    missile_obj *mo;
    float payload = 0.0f;
...
int
check_ok_to_fire(int objnum, int target_objnum, weapon_info *wip)
{
    int num_homers = 0;
    ...
    if (target_objnum > -1) {
        ...
        if (tobjp->type == OBJ_SHIP) {
            ...
            if ((wip->wi_flags & WIF_BEAM) &&
                (tobjp->flags & OF_BEAM_PROTECTED)) {
                Int3();
                return 0;
            }
            if (Ship_info[Ships[tobjp->instance].ship_info_index].flags &
                SIF_SMALL_SHIP) {
        ...
        if (wip->wi_flags & WIF_HOMING) {
            if ((target_objnum > -1) && (tobjp->flags & OF_PLAYER_SHIP)) {
                if (Ai_info[Ships[tobjp->instance].ai_index].target_objnum !=
                    ...
                    int t = ((Missiontime / (65536 * 10)) ^ target_objnum ^
                             0x01) %
                            (NUM_SKILL_LEVELS + 2);
                ...
                if (wip->wi_flags & WIF_SWARM)
                    swarmers =
                        2; // Note, always want to be able to fire swarmers if no currently incident homers.
                if (Max_allowed_player_homers[Game_skill_level] <
                    num_homers + swarmers) {
                    return 0;
                }
            }
                ...
                incoming_payload = compute_incoming_payload(
...
int
ai_fire_secondary_weapon(object *objp, int priority1, int priority2)
{
    ship_weapon *swp;
    ship *shipp;
    ship_info *sip;
    ...
    current_bank =
        swp->current_secondary_bank; //ai_select_secondary_weapon(objp, swp, priority1, priority2);
...
// collide with obj2.
int
might_collide_with_ship(object *obj1, object *obj2, float dot_to_enemy,
                        float dist_to_enemy, float duration)
{
...
int
might_hit_teammate(object *firing_objp)
{
    int team;
    object *objp;
    ship_obj *so;
...
void
render_all_ship_bay_paths(object *objp)
{
    int i, j, color;
...
void
render_all_subsys_paths(object *objp)
{
    int i, j, color;
    ship *sp = &Ships[objp->instance];
    polymodel *pm;
...
void
render_path_points(object *objp)
{
    ship *shipp = &Ships[objp->instance];
    ai_info *aip = &Ai_info[shipp->ai_index];
    object *dobjp;
    ...
    if (pm->n_docks) {
        ...
        model_find_world_point(&global_dock_point, &dock_point,
                               Ships[dobjp->instance].modelnum, 0, &dobjp->orient,
                               &dobjp->pos);
...
float
ai_get_weapon_dist(ship_weapon *swp)
{
    int bank_num, weapon_num;
...
float
ai_get_weapon_speed(ship_weapon *swp)
{
    int bank_num, weapon_num;
...
void
set_predicted_enemy_pos_turret(vector *predicted_enemy_pos, vector *gun_pos,
                               object *pobjp, vector *enemy_pos,
                               vector *enemy_vel, float weapon_speed,
                               float time_enemy_in_range)
{
    ...
    if (time_enemy_in_range < range_time) {
        ...
        vm_vec_scale_add(predicted_enemy_pos, enemy_pos, enemy_vel,
        ...
        collision_time = compute_collision_time(enemy_pos, enemy_vel, gun_pos,
                                                weapon_speed);
...
// Also, stuff globals G_predicted_pos, G_collision_time and G_fire_pos.
void
set_predicted_enemy_pos(vector *predicted_enemy_pos, object *pobjp, object *eobjp,
                        ai_info *aip)
{
    ...
    if (aip->time_enemy_in_range < range_time) {
        ...
        vm_vec_scale_add(predicted_enemy_pos, &eobjp->pos, &eobjp->phys_info.vel,
        ...
        collision_time = compute_collision_time(
            &eobjp->pos, &eobjp->phys_info.vel, &gun_pos, weapon_speed);
...
ai_chase_ct()
{
    ...
    ai_turn_towards_vector(&tvec, Pl_objp, flFrametime, sip->srotation_time, NULL,
                           NULL, 0.0f, 0);
    accelerate_ship(aip, 1.0f);
}
...
void
ai_chase_eb(ai_info *aip, ship_info *sip, vector *predicted_enemy_pos,
            float dist_to_enemy)
{
    vector _pep;
    ...
    ai_turn_towards_vector(&_pep, Pl_objp, flFrametime, sip->srotation_time, NULL,
                           NULL, 0.0f, 0);
...
float
ai_endangered_time(object *ship_objp, object *weapon_objp)
{
    ...
    if (weapon_objp->phys_info.speed < 1.0f)
        return dist + 1.0f;
    else if ((from_dot > 0.1f) &&
             (dist / (from_dot * from_dot) <
              48 *
                  ship_objp
                      ->radius)) //: don't require them to see it, they have instruments!: && (fl_abs(to_dot) > 0.5f))
...
float
ai_endangered_by_weapon(ai_info *aip)
{
    object *weapon_objp;
...
int
ai_near_full_strength(object *objp, ship_info *sip)
{
    return (objp->hull_strength / sip->initial_hull_strength > 0.9f) ||
           (get_shield_strength(objp) / sip->shields > 0.8f);
}

// Set acceleration while in attack mode.
void
attack_set_accel(ai_info *aip, float dist_to_enemy, float dot_to_enemy,
                 float dot_from_enemy)
{
    float speed_ratio;
    ...
    if (aip->ai_flags & AIF_ATTACK_SLOWLY) {
        if ((dist_to_enemy > 200.0f) && (dist_to_enemy < 800.0f)) {
            if ((dot_from_enemy < 0.9f) ||
                ai_near_full_strength(
                    Pl_objp,
                    &Ship_info[Ships[Pl_objp->instance].ship_info_index])) {
                //nprintf(("AI", " slowly "));
                accelerate_ship(aip, max(1.0f - (dist_to_enemy - 200.0f) / 600.0f,
                                         0.1f));
                return;
            }
        }
    ...
    if (dist_to_enemy >
        ...
        accelerate_ship(aip,
                        0.0f); //   No one attacking us, so don't need to move.
    }
        ...
        if (dot_from_enemy > 0.75f)
            ...
            set_accel_for_target_speed(Pl_objp,
                                       En_objp->phys_info.speed * 0.75f + 3.0f);
    }
    else {
...
void
get_behind_ship(ai_info *aip, ship_info *sip, float dist_to_enemy)
{
    vector new_pos;
    float dot;
    vector vec_from_enemy;
    ...
    ai_turn_towards_vector(&new_pos, Pl_objp, flFrametime, sip->srotation_time,
                           NULL, NULL, 0.0f, 0);
...
int
avoid_player(object *objp, vector *goal_pos)
{
    ...
    if (aip->ai_flags & AIF_AVOIDING_SMALL_SHIP) {
        ...
        if (aip->ai_flags & AIF_AVOIDING_SMALL_SHIP) {
            ai_turn_towards_vector(&aip->avoid_goal_point, objp, flFrametime,
                                   sip->srotation_time, NULL, NULL, 0.0f, 0);
...
int
will_collide_pp(vector *p0, vector *p1, float radius, object *big_objp,
                vector *collision_point)
{
    mc_info mc;

    mc.model_num =
        Ships[big_objp->instance].modelnum; // Fill in the model to check
...
int
will_collide_with_big_ship(object *objp, vector *goal_point, object *big_objp,
                           vector *collision_point, float delta_time)
{
    float radius;
    vector end_pos;
    ...
    return will_collide_pp(&objp->pos, &end_pos, objp->radius, big_objp,
                           collision_point);
}
...
int
will_collide_with_big_ship_all(object *objp, object *ignore_objp,
                               vector *goal_point, vector *collision_point,
                               float *distance, float delta_time)
{
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if (Ship_info[Ships[big_objp->instance].ship_info_index].flags &
            (SIF_BIG_SHIP | SIF_HUGE_SHIP)) {
            vector cur_collision_point;
            float cur_dist;

            if (will_collide_with_big_ship(objp, goal_point, big_objp,
                                           &cur_collision_point, delta_time)) {
...
void
mabs_pick_goal_point(object *objp, object *big_objp, vector *collision_point,
                     vector *avoid_pos)
{
    matrix mat1;
    sgoal goals[4];
    ...
    for (s = 0.5f; s < 1.3f; s += 0.25f) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            goals[i].collide = will_collide_pp(&objp->pos, &p, objp->radius,
                                               big_objp, collision_point);
            if (!goals[i].collide)
                found = 1;
        }
...
int
maybe_avoid_big_ship(object *objp, object *ignore_objp, ai_info *aip,
                     vector *goal_point, float delta_time)
{
    if (timestamp_elapsed(aip->avoid_check_timestamp)) {
        float distance;
        vector collision_point;
        ...
        if ((ship_num = will_collide_with_big_ship_all(
            ...
            mabs_pick_goal_point(objp, &Objects[ship_num], &collision_point,
    ...
    if (aip->ai_flags & AIF_AVOIDING_BIG_SHIP) {
        ...
        ai_turn_towards_vector(&aip->avoid_goal_point, Pl_objp, flFrametime,
                               sip->srotation_time, NULL, NULL, 0.0f, 0);
...
void
compute_desired_rvec(vector *rvec, vector *goal_pos, vector *cur_pos)
{
    vector v2e;
...
ai_stealth_find()
{
    ...
    vm_vec_scale_add(&new_pos, &aip->stealth_last_pos, &aip->stealth_velocity,
                     (delta_time * 0.001f));
    ...
    if ((delta_time > 800) && (aip->submode_parm0 == SM_SF_AHEAD) &&
        ...
        // do turn around)
        vm_vec_scale_add(&aip->goal_point, &Pl_objp->pos, &Pl_objp->orient.fvec,
                         -300.0f);
    ...
    if ((dist_to_enemy >
        ...
        ai_turn_towards_vector(&new_pos, Pl_objp, flFrametime,
                               sip->srotation_time, NULL, NULL, 0.0f, 0);
        return;
    }
    ...
    if (dist_to_enemy > 500.0f) {
        ...
        ai_turn_towards_vector(&new_pos, Pl_objp, flFrametime,
                               sip->srotation_time, NULL, NULL, 0.0f, 0, &rvec);
    }
    else {
        ai_turn_towards_vector(&new_pos, Pl_objp, flFrametime,
                               sip->srotation_time, NULL, NULL, 0.0f, 0);
...
ai_stealth_sweep()
{
    ...
    if (aip->submode_parm0 == SM_SS_SET_GOAL) {
        ...
        vm_vec_scale_add(&goal_pt, &aip->stealth_last_pos, &aip->stealth_velocity,
                         (0.001f * lost_time));

        // make box size based on speed of stealth and expected time to intercept (keep box in range 200-500)
        float box_size = vm_vec_mag_quick(&aip->stealth_velocity) *
                         (0.001f * lost_time);
    ...
    // pt1 -U +R
    case SM_SS_LR:
        vm_vec_scale_add(&goal_pt, &aip->goal_point, &up,
                         -aip->stealth_sweep_box_size);
    ...
    ai_turn_towards_vector(&goal_pt, Pl_objp, flFrametime, sip->srotation_time,
                           NULL, NULL, 0.0f, 0);
...
// ATTACK submode handler for chase mode.
void
ai_chase_attack(ai_info *aip, ship_info *sip, vector *predicted_enemy_pos,
                float dist_to_enemy)
{
    int start_bank;
    ...
    if (dist_to_enemy > 500.0f) {
        ...
        ai_turn_towards_vector(&new_pos, Pl_objp, flFrametime,
                               sip->srotation_time, NULL, rel_pos, bank_override,
                               0, &rvec);
    }
    else {
        ai_turn_towards_vector(&new_pos, Pl_objp, flFrametime,
...
void
ai_chase_es(ai_info *aip, ship_info *sip)
{
    vector tvec;
    fix timeslice;
    fix scale;
    ...
    ai_turn_towards_vector(&tvec, Pl_objp, flFrametime / 2, sip->srotation_time,
                           NULL, NULL, bank_override, 0);
    accelerate_ship(aip, 1.0f);
}

// Trying to get away from opponent.
void
ai_chase_ga(ai_info *aip, ship_info *sip)
{
...
int
ai_set_attack_subsystem(object *objp, int subnum)
{
    ...
    shipp = &Ships[attacked_objp
                       ->instance]; //  need to get our target's ship pointer!!!
...
void
ai_set_guard_vec(object *objp, object *guard_objp)
{
    ai_info *aip;
    float radius;
...
void
ai_set_guard_wing(object *objp, int wingnum)
{
    ship *shipp;
    ai_info *aip;
...
// To be called from the goals code.
void
ai_set_evade_object(object *objp, object *other_objp)
{
    ship *shipp;
    ai_info *aip;
    int other_objnum;
...
void
ai_set_guard_object(object *objp, object *other_objp)
{
    ship *shipp;
    ai_info *aip;
    int other_objnum;
    ...
    if ((other_aip->wing != -1) && (other_aip->wing != aip->wing)) {
        ai_set_guard_wing(objp,
                          Ai_info[Ships[other_objp->instance].ai_index].wing);
    }
    else {
...
void
update_aspect_lock_information(ai_info *aip, vector *vec_to_enemy,
                               float dist_to_enemy, float enemy_radius)
{
    float dot_to_enemy;
    int num_weapon_types;
    int weapon_id_list[MAX_WEAPON_TYPES], weapon_bank_list[MAX_WEAPON_TYPES];
    ...
    num_weapon_types = get_available_secondary_weapons(Pl_objp, weapon_id_list,
                                                       weapon_bank_list);
...
// Fly away from it!
void
ai_chase_fly_away(object *objp, ai_info *aip)
{
    int abort_flag = 0;

    if (aip->ai_flags & AIF_TARGET_COLLISION) {
        aip->ai_flags &=
            ~AIF_TARGET_COLLISION; //  Don't process this hit again next frame.
...
int
has_preferred_secondary(object *objp, object *en_objp, ship_weapon *swp)
{
    ...
    for (i = 0; i < swp->num_secondary_banks; i++) {
        if (swp->secondary_bank_capacity[i] > 0) {
            if (swp->secondary_bank_ammo[i] > 0) {
                if (is_preferred_weapon(swp->secondary_bank_weapons[i], objp,
                                        en_objp) != -1) {
                    return i;
                }
            }
...
float
set_secondary_fire_delay(ai_info *aip, ship *shipp, weapon_info *swip)
{
...
void
ai_chase_big_approach_set_goal(vector *goal_pos, object *attack_objp,
...
void
ai_chase_big_circle_set_goal(vector *goal_pos, object *attack_objp,
                             object *target_objp, float *accel)
{
    get_tangent_point(goal_pos, attack_objp, &target_objp->pos,
...
void
ai_chase_big_get_separations(object *attack_objp, object *target_objp,
                             ...
                             float *desired_separation, float *cur_separation)
{
    ...
    vm_vec_scale_add(horz_vec_to_target, &vec_to_target,
                     &target_objp->orient.fvec, -perp_dist);
...
void
ai_chase_big_parallel_set_goal(vector *goal_pos, object *attack_objp,
                               object *target_objp, float *accel)
{
    int opposing;
    float temp, r_target, r_attacker, h_attacker, h_target;
    ...
    opposing = (vm_vec_dotprod(&attack_objp->orient.fvec,
                               &target_objp->orient.fvec) < 0);

    ai_chase_big_get_separations(attack_objp, target_objp, &horz_vec_to_target,
                                 &optimal_separation, &separation);
    ...
    vm_vec_scale_add(goal_pos, &target_objp->pos, &target_objp->orient.fvec,
                     dist);
    // then add horizontal separation
    vm_vec_scale_add2(goal_pos, &horz_vec_to_target,
                      optimal_separation / separation);
    ...
    float match_accel =
        target_objp->phys_info.vel.z /
        Ship_info[Ships[attack_objp->instance].ship_info_index].max_vel.z;
...
void
ai_cruiser_chase_set_goal_pos(vector *goal_pos, object *pl_objp, object *en_objp)
{
    ai_info *aip;
...
ai_cruiser_chase()
{
    ...
    if (aip->ai_flags & AIF_KAMIKAZE) {
        ai_turn_towards_vector(&En_objp->pos, Pl_objp, flFrametime, turn_time,
                               NULL, NULL, 0.0f, 0);
        accelerate_ship(aip, 1.0f);
    }
    ...
    // really track down and chase
        ...
        // check valid submode
        Assert((aip->submode == SM_ATTACK) || (aip->submode == SM_BIG_APPROACH) ||
        ...
        // now move as desired
        ai_turn_towards_vector(&goal_pos, Pl_objp, flFrametime, turn_time, NULL,
                               NULL, 0.0f, 0, rvecp);
        ...
        case SM_BIG_APPROACH:
            if (dist_to_enemy <
                ...
                if (moving) {
                    ...
                    if (vm_vec_dotprod(&En_objp->orient.fvec,
                                       &Pl_objp->orient.fvec) > 0) {
                        aip->submode = SM_BIG_PARALLEL;
                    }
                }
            ...
            if (moving) {
                ...
                if (vm_vec_dotprod(&vec_to_enemy, &En_objp->orient.fvec) > 0) {
                    ...
                    if (vm_vec_dotprod(&En_objp->orient.fvec,
                                       &Pl_objp->orient.fvec) > 0) {
                        // get separation
                        ai_chase_big_get_separations(Pl_objp, En_objp, &temp,
                                                     &desired_sep, &cur_sep);
        ...
        case SM_BIG_PARALLEL:
            // we're opposing
            if (vm_vec_dotprod(&Pl_objp->orient.fvec, &En_objp->orient.fvec) <
                0) {
...
ai_chase()
{
    ...
    if (!(sip->flags & (SIF_FIGHTER | SIF_BOMBER | SIF_ESCAPEPOD))) {
        Warning(
            LOCATION,
            "Ship %s is not 'small', but is in chase mode.\nSwitching to AI=none.\n",
            shipp->ship_name);
        aip->mode = AIM_NONE;
        return;
    }
    ...
    if (!(Ships[En_objp->instance].flags & SF_HIDDEN_FROM_SENSORS) &&
        !is_stealthy_ship) {
        update_aspect_lock_information(aip, &real_vec_to_enemy, dist_to_enemy,
                                       En_objp->radius);
    }
    else {
        aip->current_target_is_locked = 0;
    ...
    if ((real_dot_to_enemy < 0.25f) || (aip->target_time < 1.0f) ||
        ...
        if (aip->targeted_subsys != NULL) {
            ...
            if (get_shield_strength(En_objp) / esip->shields <
                HULL_DAMAGE_THRESHOLD_PERCENT) {
                //int   rval;
                ...
                if (aip->targeted_subsys != NULL) {
                    ...
                    set_predicted_enemy_pos(&predicted_enemy_pos, Pl_objp,
                                            En_objp, aip);
                    set_target_objnum(aip, -1);
                }
                ...
                set_predicted_enemy_pos(&predicted_enemy_pos, Pl_objp, En_objp,
                                        aip);
    ...
    case SM_ATTACK_FOREVER:
        if (vm_vec_dist_quick(&Pl_objp->pos, &predicted_enemy_pos) >
            ...
            if (maybe_avoid_big_ship(Pl_objp, En_objp, aip, &predicted_enemy_pos,
                                     10.0f))
                return;
        }
        ...
        if ((dot_from_enemy > 0.5f) || (En_objp->phys_info.speed < 10.0f)) {
            if (might_collide_with_ship(Pl_objp, En_objp, dot_to_enemy,
                ...
                if ((Missiontime - aip->last_hit_time > F1_0 * 4) &&
                    (dist_to_enemy < Pl_objp->radius * 2 + En_objp->radius * 2)) {
                    accelerate_ship(aip, -1.0f);
                }
                else {
    ...
    case SM_ATTACK:
        ...
        if ((aip->ai_flags & AIF_STEALTH_PURSIUT) &&
        ...
        else if (ai_near_full_strength(Pl_objp, sip) &&
        ...
        else if ((dot_to_enemy > 0.2f) && (dot_from_enemy > -0.2f) &&
                 (dot_from_enemy < 0.1f)) {
            ...
            aip->submode_start_time = Missiontime;
                 ...
                 (dot_from_enemy >
                  dot_to_enemy + 0.5f + aip->ai_courage * .002)) {
            if ((Missiontime - aip->last_hit_target_time > i2f(5)) &&
                (frand() < (float)(aip->ai_class + Game_skill_level) /
    ...
    case SM_SUPER_ATTACK:
        ...
        if ((aip->ai_flags & AIF_STEALTH_PURSIUT) &&
        ...
        else if ((dist_to_enemy < 100.0f) && (dot_to_enemy < 0.8f) &&
                 (enemy_sip_flags & SIF_SMALL_SHIP) &&
            ...
            case 2:
            case 3:
                if (frand() < (float)0.5f * (aip->ai_class + Game_skill_level) /
                                  (Num_ai_classes + NUM_SKILL_LEVELS)) {
                    aip->submode = SM_GET_AWAY;
                    aip->submode_start_time = Missiontime;
                }
    ...
    case SM_GET_AWAY:
        if (Missiontime - aip->submode_start_time > i2f(2)) {
            ...
            if ((Missiontime - aip->submode_start_time > i2f(5)) ||
                ...
                aip->submode = SM_ATTACK;
                aip->time_enemy_in_range =
                    2.0f; //  Cheat.  Presumably if they were running away from you, they were monitoring you!
                aip->submode_start_time = Missiontime;
                aip->last_attack_time = Missiontime;
            }
        }
    ...
    case SM_STEALTH_FIND:
        ...
        else if (((Missiontime - aip->submode_start_time) > i2f(5)) ||
                 (aip->submode_parm0 == SM_SF_BAIL)) {
            // begin sweep mode
    ...
    case SM_STEALTH_SWEEP:
        ...
        else if (/*(Missiontime - aip->submode_start_time) > i2f(30) || */ (
            aip->submode_parm0 == SM_SS_DONE)) {
            // set target objnum = -1
            set_target_objnum(aip, -1);
    ...
    if (aip->mode != AIM_EVADE) {
        if (dot_to_enemy >
            ...
            if (dot_to_enemy >
                max(0.5f, 0.90f + aip->ai_accuracy / 10.0f -
                    ...
                    if (!(En_objp->flags & OF_PROTECTED)) {
                        ...
                        weapon_info *swip =
                            &Weapon_info[tswp->secondary_bank_weapons
                                             [tswp->current_secondary_bank]];

                        if (current_bank > -1) {
                            if (aip->ai_flags & AIF_UNLOAD_SECONDARIES) {
                                if (timestamp_until(swp->next_secondary_fire_stamp
                                                        [current_bank]) >
                                    swip->fire_wait * 1000.0f) {
                                    swp->next_secondary_fire_stamp[current_bank] =
                                        timestamp(
                            ...
                            if (timestamp_elapsed(
                                ...
                                if (tswp->current_secondary_bank >= 0) {
                                    weapon_info *swip =
                                        &Weapon_info
                                            [tswp->secondary_bank_weapons
                                                 [tswp->current_secondary_bank]];
                                    float firing_range;

                                    if (swip->wi_flags & WIF_BOMB)
                                        firing_range = swip->max_speed *
                                                       swip->lifetime * 0.75f;
                                    else
                                        firing_range = swip->max_speed *
                                                       swip->lifetime *
...
// Return distance.
void
dock_move_towards_point(object *objp, vector *dp, vector *vp, float speed_scale,
...
void
set_goal_dock_orient(matrix *dom, dock_bay *db_dest, dock_bay *db_src,
                     matrix *dorient, matrix *sorient)
{
    vector fvec, uvec;
    matrix m1, m2, m3;
...
float
dock_orient_and_approach(object *objp, object *dobjp, int dock_mode)
{
    ship_info *sip0, *sip1;
    polymodel *pm0, *pm1;
    ai_info *aip;
    ...
    int docker_index,
        dockee_index; // index into docking_bays[] array for objects docking
    ...
    if ((dock_mode != DOA_DOCK_STAY) && (dock_mode != DOA_DOCK)) {
        if (maybe_recreate_path(objp, &Ai_info[Ships[objp->instance].ai_index],
                                0) > 5.0f) {
...
*/    }
    ...
    case DOA_APPROACH: {
        ...
        set_goal_dock_orient(&dom, &pm1->docking_bays[dockee_index],
                             &pm0->docking_bays[docker_index], &dobjp->orient,
                             &objp->orient);
        ...
        vm_matrix_interpolate(&dom, &objp->orient, &w_in, flFrametime, &nm,
                              &w_out, &vel_limit, &acc_limit, 1);
    ...
    case DOA_DOCK_STAY:
        ...
        vm_vec_unrotate(&db0, &pm0->docking_bays[docker_index].pnt[0],
                        &objp->orient);
        vm_vec_add2(&db0, &objp->pos);

        vm_vec_unrotate(&db1, &pm0->docking_bays[docker_index].pnt[1],
                        &objp->orient);
        vm_vec_add2(&db1, &objp->pos);

        vm_vec_unrotate(&db2, &pm1->docking_bays[dockee_index].pnt[0],
                        &dobjp->orient);
        vm_vec_add2(&db2, &dobjp->pos);

        vm_vec_unrotate(&db3, &pm1->docking_bays[dockee_index].pnt[1],
                        &dobjp->orient);
        ...
        if (dock_mode == DOA_DOCK) {
            ...
            fdist = vm_vec_dist_quick(vm_vec_avg(&t1, &db0, &db1),
                                      vm_vec_avg(&t2, &db2, &db3));
    ...
    case DOA_UNDOCK_3: {
        ...
        if (vm_vec_dist_quick(&goal_point, &dobjp->pos) <
            vm_vec_dist_quick(&objp->pos, &dobjp->pos))
            fdist = 0.0f;
        else {
            float dot, accel;
            float turn_time =
                Ship_info[Ships[objp->instance].ship_info_index].srotation_time;
            ai_turn_towards_vector(&goal_point, objp, flFrametime, turn_time,
                                   NULL, NULL, 0.0f, 0);
    ...
    vm_vec_unrotate(&d1, &pm1->docking_bays[dockee_index].norm[0],
                    &dobjp->orient);
...
int
num_ships_attacking(int objnum)
{
    object *objp;
    ship_obj *so;
    int count = 0;
...
void
remove_farthest_attacker(int objnum)
{
    ...
    if (farthest_objp != NULL) {
        ...
        Assert((farthest_objp->instance > -1) &&
...
//             -1 => nothing was done
int
ai_maybe_limit_attackers(int attacked_objnum)
{
    int rval = -1;
...
void
guard_object_was_hit(object *guard_objp, object *hitter_objp)
{
    int hitter_objnum;
    ai_info *aip;
    ...
    if (aip->ai_flags &
        AIF_NO_DYNAMIC) // Not allowed to pursue dynamic goals.  So, why are we guarding?
        return;

    Assert((hitter_objp->type == OBJ_SHIP) ||
    ...
    if ((aip->submode == AIS_GUARD_PATROL) ||
        (aip->submode == AIS_GUARD_STATIC)) {
        if (hitter_objp->type == OBJ_SHIP) {
            if (!(Ship_info[Ships[guard_objp->instance].ship_info_index].flags &
                  SIF_SMALL_SHIP)) {
                return;
            }
...
void
maybe_update_guard_object(object *hit_objp, object *hitter_objp)
{
    object *objp;
    ship_obj *so;
...
int
ai_guard_find_nearby_bomb(object *guarding_objp, object *guarded_objp)
{
    missile_obj *mo;
    ...
    for (mo = GET_FIRST(&Missile_obj_list); mo != END_OF_LIST(&Missile_obj_list);
        ...
        if (dist < (MAX_GUARD_DIST + guarded_objp->radius) * 3) {
            dist_to_guarding_obj = vm_vec_dist_quick(&bomb_objp->pos,
                                                     &guarding_objp->pos);
...
void
ai_guard_find_nearby_ship(object *guarding_objp, object *guarded_objp)
{
...
void
ai_guard_find_nearby_asteroid(object *guarding_objp, object *guarded_objp)
{
    float dist;
    ...
    for (asteroid_objp = GET_FIRST(&obj_used_list);
        ...
        if (asteroid_objp->type == OBJ_ASTEROID) {
            ...
            if (dist < (MAX_GUARD_DIST + guarded_objp->radius) * 2) {
                dist_to_self = vm_vec_dist_quick(&asteroid_objp->pos,
                                                 &guarding_objp->pos);
                ...
                if (dist_to_self < closest_asteroid_dist) {
                    ...
                    if (vm_vec_mag_quick(&asteroid_objp->phys_info.vel) <
                        guarding_objp->phys_info.max_vel.z) {
...
float
get_cylinder_points(object *other_objp, object *cyl_objp, vector *axis_pt,
                    vector *r_vec, float *radius)
{
    ...
    *radius = max(tempx, tempy);
    ...
    vm_vec_scale_add(axis_pt, &cyl_objp->pos, &cyl_objp->orient.fvec,
                     extended_cylinder_z);
...
ai_big_guard()
{
    ...
    case AIS_GUARD_PATROL: {
        ...
        extended_z = get_cylinder_points(Pl_objp, guard_objp, &axis_pt, &r_vec,
                                         &radius);
        ...
        float desired_guard_dist = min_guard_dist +
                                   0.5f * ((1.0f + objval) * MAX_GUARD_DIST);
        float max_guard_dist = min_guard_dist +
                               1.0f * ((1.0f + objval) * MAX_GUARD_DIST);
        ...
        float desired_z = min_z + length * static_randf(Pl_objp - Objects ^
                                                        (Missiontime >>
                                                         (22 + time_choose)));
        ...
        if (cur_guard_rad < max_guard_dist) {
            if (cur_guard_rad > min_guard_dist) {
                if (inside) {
                    // orbit
                    vm_vec_scale_add(&goal_pt, &axis_pt, &r_vec,
                                     desired_guard_dist);
                    ...
                    if (extended_z < min_z) {
                        vm_vec_scale_add(&goal_pt, &guard_objp->pos,
                                         &guard_objp->orient.fvec, min_z);
                    }
                    else {
                        vm_vec_scale_add(&goal_pt, &guard_objp->pos,
                                         &guard_objp->orient.fvec, max_z);
                    }
            ...
            float delta_z = desired_z - extended_z;
            vm_vec_scale_add(&goal_pt, &guard_objp->pos, &guard_objp->orient.fvec,
                             delta_z);
        }
...
*/
    ...
    case AIS_GUARD_PATROL:
        ...
        if (dist_to_goal_point >
            ...
            if (maybe_avoid_big_ship(Pl_objp, guard_objp, aip, &goal_point,
                                     5.0f)) {
                return;
            }
            ...
            ai_turn_towards_vector(
                &goal_point, Pl_objp, flFrametime,
                Ship_info[shipp->ship_info_index].srotation_time, NULL, NULL,
                0.0f, 0, &rvec);

            accelerate_ship(aip,
                            accel_scale * (0.25f + dist_to_goal_point / 700.0f));
        }
        else {
            if (maybe_avoid_big_ship(Pl_objp, guard_objp, aip, &goal_point,
                                     2.0f)) {
            ...
            if (guard_objp->type == OBJ_SHIP) {
                ai_info *guard_aip =
                    &Ai_info[Ships[guard_objp->instance].ai_index];

                if (guard_aip->dock_objnum != -1) {
                    speed = max(speed,
                                Objects[guard_aip->dock_objnum].phys_info.speed);
                }
            }
            ...
            if (speed > 10.0f) {
                ...
                if ((dist_to_guardobj >
                    ...
                    accelerate_ship(aip,
                                    0.5f * (1.0f + dot) * (0.3f + objval / 3.0f));
            }
        }
...
object *
ai_find_docked_object(object *docker)
{
    ai_info *aip;
...
void
ai_do_objects_repairing_stuff(object *repaired_objp, object *repair_objp, int how)
{
    ...
    case REPAIR_INFO_BEGIN:
        ...
        if (repaired_objp->flags & OF_PLAYER_SHIP) {
            Player->stats.m_score -=
                (int)(REPAIR_PENALTY *
                      scoring_get_scale_factor()); // subtract the penalty
        }
        break;
    ...
    case REPAIR_INFO_KILLED:
        ...
        if (p_index >= 0) {
            ...
            if (how == REPAIR_INFO_KILLED) {
                message_send_builtin_to_player(MESSAGE_SUPPORT_KILLED, NULL,
                                               MESSAGE_PRIORITY_HIGH,
                                               MESSAGE_TIME_SOON, 0, 0);
            }
            else {
                if (repair_objp) {
                    message_send_builtin_to_player(MESSAGE_REPAIR_ABORTED,
        ...
        if (repaired_objp->flags & OF_PLAYER_SHIP) {
            mission_log_add_entry(LOG_PLAYER_REARM_ABORT,
    ...
    case REPAIR_INFO_COMPLETE:
        ...
        if (p_index >= 0) {
            ...
            message_send_builtin_to_player(MESSAGE_REPAIR_DONE,
                                           &Ships[repair_objp->instance],
...
void
ai_cleanup_dock_mode(ai_info *aip, ship *shipp)
{
    object *objp;
    ...
    if (aip->ai_flags & AIF_REPAIRING) {
        Assert(aip->goal_objnum != -1);
        ai_do_objects_repairing_stuff(&Objects[aip->goal_objnum],
        ...
        Assert(aip->dock_objnum != -1);
        ai_do_objects_repairing_stuff(&Objects[shipp->objnum],
                                      &Objects[aip->dock_objnum],
                                      REPAIR_INFO_KILLED);
    }
        ...
        if (mission_is_repair_scheduled(objp)) {
            ...
            if (aip->dock_objnum != -1)
                ai_do_objects_repairing_stuff(objp, &Objects[aip->dock_objnum],
                                              REPAIR_INFO_ABORT);
            else
...
int
maybe_dock_obstructed(object *cur_objp, object *goal_objp, int big_only_flag)
{
    ...
    Ai_info[Ships[goal_objp->instance].ai_index].ai_flags &=
        ~AIF_REPAIR_OBSTRUCTED;
    ...
    collide_objnum = pp_collide_any(curpos, goalpos, radius, cur_objp, goal_objp,
                                    big_only_flag);
...
ai_dock()
{
    ...
    case AIS_DOCK_0:
        ...
        if (!Dock_path_warning_given && (aip->path_length < 4)) {
            Warning(
                LOCATION,
                "Ship '%s' has only %i points on dock path.  Docking will look strange.  Contact Adam.",
                shipp->ship_name, aip->path_length);
            Dock_path_warning_given =
                1; //   This is on a mission-wide basis, but it's just a hack for now...
        }
    ...
    case AIS_DOCK_1: {
        ...
        if ((r = maybe_dock_obstructed(Pl_objp, goal_objp, 1)) != -1) {
            int r1;
            if ((r1 = maybe_avoid_big_ship(Pl_objp, goal_objp, aip,
                                           &goal_objp->pos, 7.0f)) != 0) {
        ...
        if ((r = maybe_dock_obstructed(Pl_objp, goal_objp, 0)) != -1) {
            ...
            if (dist == DOCK_BACKUP_RETURN_VAL) {
                ...
                path_num = ai_return_path_num_from_dockbay(
                    &Objects[aip->goal_objnum], aip->dockee_index);
                Assert(aip->goal_objnum >= 0 && aip->goal_objnum < MAX_OBJECTS);
                ai_find_path(Pl_objp, aip->goal_objnum, path_num, 0);
                break;
            }
    ...
    case AIS_DOCK_3A: {
        ...
        if ((r = maybe_dock_obstructed(Pl_objp, goal_objp, 0)) != -1) {
            ...
            float dist = dock_orient_and_approach(
                Pl_objp, &Objects[aip->goal_objnum], DOA_DOCK);
            Assert(dist != UNINITIALIZED_VALUE);
            ...
            if (dist <
                2 * flFrametime * (1.0f + fl_sqrt(goal_objp->phys_info.speed))) {
                ...
                if (aip->submode == AIS_DOCK_3) {
                    snd_play_3d(&Snds[SND_DOCK_ATTACH], &Pl_objp->pos,
                                &View_position);
    ...
    case AIS_DOCK_4: {
        ...
        //nprintf(("AI", "Time = %7.3f, submode = %i\n", f2fl(Missiontime), aip->submode));
        float dist = dock_orient_and_approach(Pl_objp, &Objects[aip->goal_objnum],
                                              DOA_DOCK);
        Assert(dist != UNINITIALIZED_VALUE);
        ...
        if (dist > 5.0f) { // Oops, too far away!
            if (goal_aip->ai_flags & AIF_BEING_REPAIRED)
                ai_do_objects_repairing_stuff(goal_objp, Pl_objp,
                                              REPAIR_INFO_BROKEN);
    ...
    case AIS_UNDOCK_0: {
        ...
        if (aip->dock_objnum == -1) {
            ...
            path_num = ai_return_path_num_from_dockbay(goal_objp,
                                                       aip->dock_path_index);
    ...
    case AIS_UNDOCK_1: {
        ...
        if (Missiontime - aip->submode_start_time < REARM_BREAKOFF_DELAY) {
            ...
            if (aip->submode_start_time != 0)
                snd_play_3d(&Snds[SND_DOCK_DEPART], &Pl_objp->pos,
                            &View_position);
        ...
        float dist_to_dock_obj = vm_vec_dist_quick(
            &Pl_objp->pos, &Objects[aip->goal_objnum].pos);
        ...
        if ((dist < 2 * flFrametime) ||
            (dist_to_dock_obj > 2 * Pl_objp->radius)) {
    ...
    case AIS_UNDOCK_2: {
        ...
        if ((dist < 2.0f) ||
            ...
            (goal_objp->phys_info.speed > MAX_UNDOCK_ABORT_SPEED)) {
            // reset the dock flags.  If rearm/repair, reset rearm repair flags for those ships as well.
            if (sip->flags & SIF_SUPPORT) {
                ai_do_objects_repairing_stuff(&Objects[aip->dock_objnum], Pl_objp,
                                              REPAIR_INFO_END);
            }
            ...
            physics_ship_init(Pl_objp);
            aip->submode =
                AIS_UNDOCK_3; // The do-nothing mode, until another order is issued
            ...
            if (!(sip->flags & SIF_SUPPORT))
                mission_log_add_entry(LOG_SHIP_UNDOCK,
                                      Ships[Pl_objp->instance].ship_name,
                                      Ships[goal_objp->instance].ship_name);
        }
        break;
    ...
    case AIS_UNDOCK_3: {
        ...
        if ((sip->flags & SIF_SUPPORT) && (aip->ai_flags & AIF_REPAIRING)) {
            ai_do_objects_repairing_stuff(&Objects[aip->goal_objnum], Pl_objp,
                                          REPAIR_INFO_ABORT);
        }
...
// in global space.
void
ship_get_global_turret_info(object *objp, model_subsystem *tp, vector *gpos,
                            vector *gvec)
{
    matrix m;
    vm_copy_transpose_matrix(&m, &objp->orient);
...
void
ship_get_global_turret_gun_info(object *objp, ship_subsys *ssp, vector *gpos,
                                vector *gvec, int use_angles, vector *targetp)
{
    vector *gun_pos;
    ...
    gun_pos = &tp->turret_firing_point[ssp->turret_next_fire_pos %
                                       tp->turret_num_firing_points];

    model_find_world_point(gpos, gun_pos, tp->model_num, tp->turret_gun_sobj,
                           &objp->orient, &objp->pos);

    if (use_angles)
        model_find_world_dir(gvec, &tp->turret_norm, tp->model_num,
...
int
aifft_rotate_turret(ship *shipp, int parent_objnum, ship_subsys *ss, object *objp,
                    object *lep, vector *predicted_enemy_pos, vector *gvec)
{
    if (ss->turret_enemy_objnum != -1) {
        ...
        weapon_system_strength = ship_get_subsystem_strength(shipp,
                                                             SUBSYSTEM_WEAPONS);

        ship_get_global_turret_info(&Objects[parent_objnum], tp, &gun_pos,
                                    &gun_vec);

        weapon_speed = Weapon_info[tp->turret_weapon_type].max_speed;
        float weapon_travel_dist = weapon_speed *
                                   Weapon_info[tp->turret_weapon_type].lifetime;

        vector enemy_point;
        if (ss->targeted_subsys != NULL) {
            if (ss->turret_enemy_objnum != -1) {
                vm_vec_unrotate(&enemy_point,
                                &ss->targeted_subsys->system_info->pnt,
        ...
        set_predicted_enemy_pos_turret(
            ...
            &lep->phys_info.vel, weapon_speed,
        ...
        if (weapon_system_strength < 0.7f) {
            ...
            vm_vec_scale_add2(predicted_enemy_pos, &rand_vec,
                              (1.0f - weapon_system_strength) * 1.5f *
                                  lep->radius);
        }
...
float
aifft_compute_turret_dot(object *objp, object *enemy_objp, vector *abs_gunposp,
                         ship_subsys *turret_subsysp, ship_subsys *enemy_subsysp)
{
    float dot_out;
    vector subobj_pos, vector_out;

    vm_vec_unrotate(&subobj_pos, &enemy_subsysp->system_info->pnt,
                    &enemy_objp->orient);
    vm_vec_add2(&subobj_pos, &enemy_objp->pos);

    if (ship_subsystem_in_sight(enemy_objp, enemy_subsysp, abs_gunposp,
                                &subobj_pos, 1, &dot_out, &vector_out)) {
        vector turret_norm;

        vm_vec_rotate(&turret_norm, &turret_subsysp->system_info->turret_norm,
                      &objp->orient);
                         ...
                         float *dot_out)
{
    ...
    *dot_out = best_dot;
    ...
    for (idx = offset; idx < aifft_list_size; idx += stride) {
        dot = aifft_compute_turret_dot(objp, enemy_objp, &abs_gun_pos, ssp,
                                       aifft_list[idx]);
...
void
ai_turret_select_default_weapon(ship_subsys *turret)
{
    ship_weapon *twp;
...
int
turret_should_pick_new_target(ship_subsys *turret)
{
    //   int target_type;
...
void
turret_set_next_fire_timestamp(ship_subsys *turret, ai_info *aip)
{
    float wait;
    int weapon_id;
...
int
turret_should_fire_aspect(ship_subsys *turret, float dot, int weapon_class)
{
    weapon_info *wip;
...
void
turret_update_enemy_in_range(ship_subsys *turret, float seconds)
{
...
void
turret_fire_weapon(ship_subsys *turret, int parent_objnum, vector *turret_pos,
                   vector *turret_fvec, vector *predicted_pos = NULL)
{
    ...
    if (check_ok_to_fire(parent_objnum, turret->turret_enemy_objnum,
                         &Weapon_info[turret_weapon_class])) {
        ...
        if (Weapon_info[turret_weapon_class].wi_flags & WIF_BEAM) {
            ...
            if (Weapon_info[turret_weapon_class].wi_flags & WIF_SWARM) {
                turret_swarm_set_up_info(parent_objnum, turret,
                                         turret_weapon_class);
                return;
            }
            else {
                weapon_objnum = weapon_create(turret_pos, &turret_orient,
                                              turret_weapon_class, parent_objnum,
                                              0, -1, 1);
                weapon_set_tracking_info(weapon_objnum, parent_objnum,
            ...
            if (weapon_objnum != -1) {
                ...
                if (Weapon_info[turret_weapon_class].launch_snd != -1) {
                    ...
                    if (parent_objnum != OBJ_INDEX(Player_obj)) {
                        snd_play_3d(
                            &Snds[Weapon_info[turret_weapon_class].launch_snd],
...
void
turret_swarm_fire_from_turret(ship_subsys *turret, int parent_objnum,
                              int target_objnum, ship_subsys *target_subsys)
{
    ...
    //   change firing point
    ship_get_global_turret_gun_info(&Objects[parent_objnum], turret, &turret_pos,
                                    &turret_fvec, 1, NULL);
    ...
    weapon_objnum = weapon_create(&turret_pos, &turret_orient,
                                  turret_weapon_class, parent_objnum, 0, -1, 1);
    weapon_set_tracking_info(weapon_objnum, parent_objnum, target_objnum, 1,
                             target_subsys);
    ...
    if (weapon_objnum > -1) {
        ...
        if (Weapon_info[turret_weapon_class].launch_snd != -1) {
            // Don't play turret firing sound if turret sits on player ship... it gets annoying.
            if (parent_objnum != OBJ_INDEX(Player_obj)) {
                snd_play_3d(&Snds[Weapon_info[turret_weapon_class].launch_snd],
                            &turret_pos, &View_position);
            }
        }
    }
...
void
ai_fire_from_turret(ship *shipp, ship_subsys *ss, int parent_objnum)
{
    ...
    if (ship_subsys_disrupted(
            ss)) { // AL 1/19/98: Make sure turret isn't suffering disruption effects
        return;
    }
    ...
    if ((ss->turret_enemy_objnum >= 0 && ss->turret_enemy_objnum < MAX_OBJECTS) &&
        ...
        if (Weapon_info[turret_weapon_class].wi_flags & WIF_HUGE) {
            ...
            if (!(Ship_info[Ships[lep->instance].ship_info_index].flags &
                  (SIF_BIG_SHIP | SIF_HUGE_SHIP))) {
                return;
            }
        }
        ...
        if (lep->type == OBJ_SHIP) {
            ...
            if ((lep->flags & OF_BEAM_PROTECTED) &&
                (Weapon_info[turret_weapon_class].wi_flags & WIF_BEAM)) {
                ss->turret_enemy_objnum = -1;
                ss->turret_time_enemy_in_range = 0.0f;
                return;
            }
    ...
    use_angles = aifft_rotate_turret(shipp, parent_objnum, ss, objp, lep,
                                     &predicted_enemy_pos, &gvec);
    ...
    if (ss->turret_enemy_objnum != -1) {
        float dist_to_enemy = vm_vec_normalized_dir(&v2e, &predicted_enemy_pos,
                                                    &gpos) -
                              lep->radius;
    ...
    if (Weapon_info[turret_weapon_class].wi_flags & WIF_SPAWN) {
        ...
        num_ships_nearby = num_nearby_fighters(get_enemy_team_mask(parent_objnum),
                                               &gpos, 1500.0f);
        if ((num_ships_nearby >= 3) ||
            ((num_ships_nearby >= 2) && (frand() < 0.1f))) {
            turret_fire_weapon(ss, parent_objnum, &gpos,
                               &ss->turret_last_fire_direction);
        }
        else {
            ss->turret_next_fire_stamp = timestamp(
    ...
    if (turret_should_pick_new_target(ss)) {
        ...
        int objnum = find_turret_enemy(
            ...
            tp->turret_fov, Weapon_info[turret_weapon_class].wi_flags & WIF_HUGE);
        ...
        if (ss->turret_enemy_objnum != -1) {
            ...
            lep = &Objects[ss->turret_enemy_objnum];
            if (lep->type == OBJ_SHIP) {
                ss->targeted_subsys = aifft_find_turret_subsys(objp, ss, lep,
                                                               &dot);
            }
            ss->turret_next_enemy_check_stamp = timestamp(
                (int)(max(dot, 0.5f) * 2000.0f) + 1000);
        }
        else {
            ss->turret_next_enemy_check_stamp = timestamp(
                (int)(2000.0f *
                      frand_range(0.9f, 1.1f))); //   Check every two seconds
        }
    }
    ...
    if (dot > tp->turret_fov) {
        ...
        ship_get_global_turret_gun_info(&Objects[parent_objnum], ss, &gpos, &gvec,
        ...
        if (Weapon_info[turret_weapon_class].wi_flags & WIF_FLAK) {
            flak_jitter_aim(&v2e, dist_to_enemy,
                            ship_get_subsystem_strength(shipp,
                                                        SUBSYSTEM_WEAPONS));
        }
        ...
        if (!(Weapon_info[turret_weapon_class].wi_flags & WIF_HOMING)) {
            if ((dist_to_enemy < 75.0f) || (dot > AICODE_TURRET_DUMBFIRE_ANGLE)) {
                turret_update_enemy_in_range(
                    ss, 2 * Weapon_info[turret_weapon_class].fire_wait);
                ok_to_fire = 1;
            }
        }
        else if (Weapon_info[turret_weapon_class].wi_flags &
                 WIF_HOMING_HEAT) { // if heat seekers
            if ((dist_to_enemy < 50.0f) || (dot > AICODE_TURRET_HEATSEEK_ANGLE)) {
                ...
                ok_to_fire = 1;
                 ...
                 WIF_HOMING_ASPECT) { // if aspect seeker
        ...
        if (ok_to_fire) {
            ...
            turret_fire_weapon(ss, parent_objnum, &gpos, &v2e,
                               &predicted_enemy_pos);
        }
        else {
            turret_update_enemy_in_range(
                ss, -4 * Weapon_info[tp->turret_weapon_type].fire_wait);
            ss->turret_next_fire_stamp = timestamp(500);
        }
    }
    else {
    ...
    if (ship_get_subsystem_strength(shipp, SUBSYSTEM_ENGINE) == 0.0f) {
        if ((sip->flags & (SIF_FIGHTER | SIF_BOMBER)) &&
            ...
            if (aip->mode != AIM_BAY_DEPART) {
                ai_attack_object(
                    objp, NULL, 99,
                    NULL); // Regardless of current mode, enter attack mode.
                aip->submode =
                    SM_ATTACK_FOREVER; // Never leave attack submode, don't avoid, evade, etc.
            }
        }
    }
}
...
int
get_wing_index(object *objp, int wingnum)
{
    wing *wingp;
    int i;
...
#define DEFAULT_WING_MAG                                                         \
    (fl_sqrt(DEFAULT_WING_X_DELTA * DEFAULT_WING_X_DELTA +                       \
...
void
get_wing_delta(vector *_delta_vec, int wing_index)
{
    int wi0;
    ...
    _delta_vec->x = ((float)column - (float)row / 2.0f) * DEFAULT_WING_X_DELTA /
                    DEFAULT_WING_MAG;
    _delta_vec->y = ((float)row + (float)bank * 2.25f) * DEFAULT_WING_Y_DELTA /
                    DEFAULT_WING_MAG;
    _delta_vec->z = -((float)row + 0.5f * (float)bank) * DEFAULT_WING_Z_DELTA /
                    DEFAULT_WING_MAG;
}

// Compute the largest radius of a ship in a *objp's wing.
float
gwlr_1(object *objp, ai_info *aip)
{
...
float
gwlr_object_1(object *objp, ai_info *aip)
{
    float max_radius;
    object *o;
    ship_obj *so;
...
float
get_wing_largest_radius(object *objp, int formation_object_flag)
{
    ship *shipp;
    ai_info *aip;
...
void
get_absolute_wing_pos(vector *result_pos, object *leader_objp, int wing_index,
                      int formation_object_flag)
{
    vector wing_delta, rotated_wing_delta;
    float wing_spread_size;

    get_wing_delta(&wing_delta,
                   wing_index); //  Desired location in leader's reference frame

    wing_spread_size = max(50.0f, 3.0f * get_wing_largest_radius(
                                             leader_objp, formation_object_flag) +
                                      15.0f);
    ...
    vm_vec_scale(&wing_delta, wing_spread_size *
                                  (1.0f + leader_objp->phys_info.speed / 70.0f));

    vm_vec_unrotate(&rotated_wing_delta, &wing_delta,
                    &leader_objp->orient); //   Rotate into leader's reference.

    vm_vec_add(result_pos, &leader_objp->pos,
...
void
render_wing_phantoms(object *objp)
{
    int i;
    ship *shipp;
    ai_info *aip;
...
void
ai_fly_in_formation(int wingnum)
{
    object *objp;
    ship *shipp;
    ship_obj *so;
...
// wingnum     Wing to force to fly in formation
void
ai_disband_formation(int wingnum)
{
    object *objp;
    ship *shipp;
    ship_obj *so;
...
int
formation_is_leader_chaotic(object *objp)
{
    if (objp != Player_obj)
        return 0;
    ...
    if (Framecount != Chaos_frame) {
        ...
        fdot = 5.0f *
               (1.0f - vm_vec_dot(&objp->orient.fvec, &objp->last_orient.fvec)) *
               flFrametime;
        udot = 8.0f *
               (1.0f - vm_vec_dot(&objp->orient.uvec, &objp->last_orient.uvec)) *
               flFrametime;
...
ai_formation()
{
    ...
    vector goal_point, future_goal_point_5, future_goal_point_2,
    ...
    get_absolute_wing_pos(&goal_point, leader_objp, wing_index,
                          aip->ai_flags & AIF_FORMATION_OBJECT);
    ...
    vm_vec_scale_add(&future_goal_point_x, &goal_point, &leader_objp->orient.fvec,
                     10.0f); //  used when very close to destination
    vm_vec_scale_add(&future_goal_point_1000x, &goal_point,
                     &leader_objp->orient.fvec,
    ...
    if (dist_to_goal > 500.0f) {
        ...
        if (dot_to_goal > -0.5f) {
            ...
            if (range_speed > 0.0f)
                set_accel_for_target_speed(
                    Pl_objp, leader_speed +
                                 range_speed * (dist_to_goal + 100.0f) / 500.0f);
            else
        ...
        if (chaotic_leader) {
            ...
            set_accel_for_target_speed(Pl_objp,
                                       scale * (leader_speed + delta_speed));
        }
        else {
            //nprintf(("AI", "Dot = %7.3f\n", dot_to_goal));
            ...
            if (leader_speed < 5.0f) {
                ...
                if ((dist_to_goal > 10.0f) ||
                    ...
                    if (Pl_objp->phys_info.speed < 0.5f) {
                        //nprintf(("MK", "(2) "));
                        turn_towards_point(Pl_objp, &future_goal_point_1000x,
                                           NULL, 0.0f);
                    }
                    else {
                        //nprintf(("MK", "(3) "));
                ...
                if (Pl_objp->phys_info.speed < 0.1f)
                    turn_towards_point(Pl_objp, &future_goal_point_1000x, NULL,
                                       0.0f);
                else
    ...
    if (up_dot < 0.996f) {
        ...
        vm_matrix_interpolate(&leader_objp->orient, &Pl_objp->orient,
                              &Pl_objp->phys_info.rotvel, flFrametime,
                              &new_orient, &w_out, &Pl_objp->phys_info.max_rotvel,
                              &angular_accel, 1);
...
int
find_repairing_objnum(int objnum)
{
    object *objp;
    ship *shipp;
    ship_info *sip;
...
void
ai_do_repair_frame(object *objp, ai_info *aip, float frametime)
{
        ...
        if (aip->ai_flags & AIF_BEING_REPAIRED) {
            ...
            if (Missiontime - repair_aip->submode_start_time >
                ...
                if (repaired && (repair_aip->submode == AIS_DOCK_4)) {
                    ...
                    ai_do_objects_repairing_stuff(objp, &Objects[dock_objnum],
...
void
call_doa(object *obj1, object *obj2, ship_info *sip1)
{
    if (sip1->flags & SIF_SUPPORT) {
        ...
        if (Ship_info[Ships[obj1->instance].ship_info_index].flags & SIF_CARGO)
            dock_orient_and_approach(obj1, obj2, DOA_DOCK_STAY);
        else if (Ship_info[Ships[obj2->instance].ship_info_index].flags &
                 SIF_CARGO)
...
void
ai_maybe_launch_cmeasure(object *objp, ai_info *aip)
{
    float dist;
    ship_info *sip;
    ship *shipp;
...
// --------------------------------------------------------------------------
void
ai_preprocess_ignore_objnum(object *objp, ai_info *aip)
{
    //   if (aip->ignore_objnum == UNUSED_OBJNUM)
    //      return;
...
ai_chase_circle(object *objp)
{
    ...
    Assert(vm_vec_mag(&aip->goal_point) >=
    ...
    if (aip->ignore_objnum == UNUSED_OBJNUM) {
        ...
        if (dist_to_goal > 2 * CHASE_CIRCLE_DIST) {
            ...
            vm_vec_scale_add(&aip->goal_point, &objp->pos, &vec_to_goal,
                             CHASE_CIRCLE_DIST);
        }
        ...
        if (dist < ignore_objp->radius * 2 + 1500.0f) {
            vm_vec_scale_add(&goal_point, &Pl_objp->pos, &tvec1,
                             ignore_objp->radius * 2 + 1400.0f);
            if (dist < ignore_objp->radius * 2 + 1300.0f)
                target_speed = sip->max_speed *
                               (1.25f -
                                dist / (ignore_objp->radius * 2 + 1500.0f));
        }
    }
...
void
ai_transfer_shield(object *objp, int quadrant_num)
{
    int i;
    ...
    if (objp->shields[quadrant_num] + (MAX_SHIELD_SECTIONS - 1) * transfer_delta >
        max_quadrant_strength)
        transfer_delta = (max_quadrant_strength - objp->shields[quadrant_num]) /
                         (MAX_SHIELD_SECTIONS - 1);
...
void
ai_manage_shield(object *objp, ai_info *aip)
{
    ship_info *sip;
    ...
    if (timestamp_elapsed(aip->shield_manage_timestamp)) {
        ...
        delay = delay + delay * (float)(3 * (Num_ai_classes - aip->ai_class - 1) /
                                        (Num_ai_classes - 1));
            ...
            if (dist < dist2) {
                ...
                case AIM_CHASE:
                    ...
                    if (((((Missiontime >> 18) ^ OBJ_INDEX(objp)) & 3) == 0) ||
                        ...
                        if (aip->submode !=
                            SM_ATTACK_FOREVER) { //   SM_ATTACK_FOREVER means engines blown.
                ...
                case AIM_BAY_EMERGE:
                    ...
                    aip->mode_time = timestamp(
                        MAX_EVADE_TIME); //  Max time to evade.
...
void
maybe_evade_dumbfire_weapon(ai_info *aip)
{
    //   Only small ships evade an incoming missile.  Why would a capital ship try to swerve?
    if (!(Ship_info[Ships[Pl_objp->instance].ship_info_index].flags &
          SIF_SMALL_SHIP)) {
        return;
    }
    ...
    case AIM_GUARD:
        ...
        if ((aip->guard_objnum != -1) &&
            (aip->guard_signature == Objects[aip->guard_objnum].signature)) {
            if (vm_vec_dist_quick(&Objects[Ships[aip->shipnum].objnum].pos,
    ...
    if ((t > 0.0f) && (t < 1.0f)) {
        ...
        case AIM_SAFETY:
            if (!(aip->ai_flags & (AIF_NO_DYNAMIC | AIF_KAMIKAZE)) &&
                (Ship_info[Ships[aip->shipnum].ship_info_index].flags &
                 SIF_SMALL_SHIP)) {
                aip->active_goal = AI_ACTIVE_GOAL_DYNAMIC;
                aip->previous_mode = aip->mode;
                aip->previous_submode = aip->submode;
...
//           0    => success
int
ai_acquire_emerge_path(object *pl_objp, int parent_objnum, vector *pos,
                       vector *fvec)
{
    ...
    create_model_exit_path(pl_objp, &Objects[parent_objnum], path_index,
                           pm->paths[path_index].nverts);
...
void
ai_emerge_bay_path_cleanup(ai_info *aip)
{
    aip->path_start = -1;
    aip->path_cur = -1;
    aip->path_length = 0;
...
int
ai_find_closest_depart_path(ai_info *aip, polymodel *pm)
{
...
//          0  => found depart path
int
ai_acquire_depart_path(object *pl_objp, int parent_objnum)
{
    ...
    if (parent_objnum == -1) {
        ...
        while (so != END_OF_LIST(&Ship_obj_list)) {
            ...
            if ((Ship_info[sp->ship_info_index].flags & (SIF_HUGE_SHIP)) &&
                (sp->team == Ships[pl_objp->instance].team)) {
                objnum = so->objnum;
                break;
            }
            so = GET_NEXT(so);
...
void
ai_execute_behavior(ai_info *aip)
{
    switch (aip->mode) {
    case AIM_CHASE:
        if (En_objp) {
            ...
            if (strnicmp(shipp->ship_name, INSTRUCTOR_SHIP_NAME,
                         strlen(INSTRUCTOR_SHIP_NAME))) {
                if (sip->flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP)) {
                    aip->mode = AIM_NONE;
                }
...
int
mrs_subsystem(ship *shipp, int type)
{
    float t;
...
int
num_allies_rearming(object *objp)
{
    ship_obj *so;
    int team;
    int count = 0;
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if (Ships[A->instance].team == team) {
            if (Ai_info[Ships[A->instance].ai_index].ai_flags &
                (AIF_REPAIRING | AIF_AWAITING_REPAIR)) {
                count++;
            }
        }
    }
...
int
maybe_request_support(object *objp)
{
    ship_info *sip;
    ship *shipp;
    ai_info *aip;
    ...
    for (int i = 0; i < swp->num_secondary_banks; i++) {
        if (swp->secondary_bank_start_ammo[i] > 0) {
            ...
            float r = (float)swp->secondary_bank_ammo[i] /
    ...
    if (ai_good_time_to_rearm(objp)) {
        ...
        if (desire >= 8) { // guarantees disabled will cause repair request
            ai_issue_rearm_request(objp);
                 ...
                 3) { //   >= 3 means having a single subsystem fully blown will cause repair.
            int count;
            int objnum = find_nearby_hostile(OBJ_INDEX(objp),
                                             get_enemy_team_mask(OBJ_INDEX(objp)),
                                             2000.0f, &count);
...
void
ai_set_mode_warp_out(object *objp, ai_info *aip)
{
    ai_abort_rearm_request(objp);
...
ai_maybe_warp_out(object *objp)
{
    ...
    if (sip->flags & SIF_SUPPORT) {
        if (timestamp_elapsed(aip->warp_out_timestamp)) {
            ...
            if ((aip->dock_objnum == -1) &&
                (objp->hull_strength / sip->initial_hull_strength < 0.25f)) {
...
ai_warp_out(object *objp)
{
    ...
    case AIS_WARP_2: // Make sure won't collide with any object.
        if (timestamp_elapsed(aip->force_warp_time) ||
            ...
            if (ship_get_warp_speed(objp) >
                ship_get_max_speed(&Ships[objp->instance])) {
...
// Return nearest one.
int
ai_find_shockwave_weapon(object *objp, ai_info *aip)
{
    missile_obj *mo;
...
void
ai_announce_ship_dying(object *dying_objp)
{
    ...
    if (damage >= EVADE_SHOCKWAVE_DAMAGE_THRESHOLD) {
        ...
        for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
             so = GET_NEXT(so)) {
            if (Ship_info[Ships[Objects[so->objnum].instance].ship_info_index]
                    .flags &
                (SIF_SMALL_SHIP | SIF_FREIGHTER)) {
                ai_info *aip;
...
// Return nearest one.
int
ai_find_shockwave_ship(object *objp, ai_info *aip)
{
    ship_obj *so;
    float nearest_dist = 999999.9f;
    int nearest_index = -1;
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if (shipp->flags & SF_DYING) {
            ...
            if (damage >=
                EVADE_SHOCKWAVE_DAMAGE_THRESHOLD) { //   Only evade quite large blasts
                float dist;
...
int
aas_1(object *objp, ai_info *aip, vector *safe_pos)
{
    ...
    if (aip->ai_flags & AIF_AVOID_SHOCKWAVE_WEAPON) {
        ...
        if (!pos_set) {
            ...
            vm_vec_scale_add(&expected_pos, &weapon_objp->pos,
                             &weapon_objp->orient.fvec, time_scale);
        }
        ...
        if (vm_vec_dist_quick(&objp->pos, &expected_pos) > danger_dist * 2.0f) {
            ...
            vm_vec_scale_add(safe_pos, &weapon_objp->pos,
                             &weapon_objp->orient.fvec, -50000.0f * dir);
        ...
        vm_vec_scale_add(safe_pos, &ship_objp->pos, &safe_vec,
...
int
ai_avoid_shockwave(object *objp, ai_info *aip)
{
    vector safe_pos;
    ...
    if (!(aip->ai_flags & AIF_AVOID_SHOCKWAVE_STARTED))
        if (!rand_chance(
                flFrametime,
                (float)aip->ai_class / 4.0f +
                    0.25f)) //   Chance to avoid in 1 second is 0.25 + ai_class/4
            return 0;
...
int
ai_await_repair_frame(object *objp, ai_info *aip)
{
    if (!(aip->ai_flags & (AIF_AWAITING_REPAIR | AIF_BEING_REPAIRED)))
        return 0;
    ...
    vm_vec_scale_add(&goal_point, &repair_objp->pos, &repair_objp->orient.uvec,
...
void
ai_maybe_self_destruct(object *objp, ai_info *aip)
{
    ...
    if ((Ship_info[Ships[objp->instance].ship_info_index].flags &
         SIF_SMALL_SHIP) &&
        (Ships[objp->instance].wingnum != -1)) {
        if ((ship_get_subsystem_strength(&Ships[objp->instance],
                                         SUBSYSTEM_ENGINE) <= 0.0f) ||
            (ship_get_subsystem_strength(&Ships[objp->instance],
                                         SUBSYSTEM_WEAPONS) <= 0.0f)) {
            if (aip->self_destruct_timestamp < 0)
                aip->self_destruct_timestamp = timestamp(
                    90 * 1000); //  seconds until self-destruct
        }
        else {
        ...
        if (timestamp_elapsed(aip->self_destruct_timestamp)) {
            ship_apply_local_damage(objp, objp, &objp->pos,
                                    objp->hull_strength * flFrametime + 1.0f,
                                    MISS_SHIELDS);
        }
    }
}
...
int
ai_need_new_target(object *pl_objp, int target_objnum)
{
    object *objp;
...
int
maybe_big_ship_collide_recover_frame(object *objp, ai_info *aip)
{
    float dot, dist;
    vector v2g;
    ...
    if (aip->ai_flags & AIF_BIG_SHIP_COLLIDE_RECOVER_1) {
        ai_turn_towards_vector(
            &aip->big_recover_pos_1, objp, flFrametime,
            Ship_info[Ships[objp->instance].ship_info_index].srotation_time, NULL,
            NULL, 0.0f, 0, NULL);
        ...
        ai_turn_towards_vector(
            &aip->big_recover_pos_2, objp, flFrametime,
            Ship_info[Ships[objp->instance].ship_info_index].srotation_time, NULL,
            NULL, 0.0f, 0, NULL);
...
void
validate_mode_submode(ai_info *aip)
{
    switch (aip->mode) {
    case AIM_CHASE:
        // check valid submode
...
*/
    ...
    if (aip->ai_flags & AIF_AVOID_SHOCKWAVE) {
        if (ai_avoid_shockwave(Pl_objp, aip)) {
            aip->ai_flags &= ~(AIF_BIG_SHIP_COLLIDE_RECOVER_1 |
                               AIF_BIG_SHIP_COLLIDE_RECOVER_2);
            if (aip->ai_flags & (AIF_AWAITING_REPAIR | AIF_BEING_REPAIRED))
        ...
        if ((aip->mode != AIM_EVADE_WEAPON) &&
            ...
            if (!(Ship_info[shipp->ship_info_index].flags & SIF_HARMLESS)) {
                target_objnum = find_enemy(
                    ...
                    Skill_level_max_attackers
                        [Game_skill_level]); // Attack up to 25K units away.
                if (target_objnum != -1) {
...
*/
    ...
    if ((En_objp != NULL) && (En_objp->pos.x == Pl_objp->pos.x) &&
        (En_objp->pos.y == Pl_objp->pos.y) &&
        (En_objp->pos.z == Pl_objp->pos.z)) {
        mprintf(
            ("Warning: Object and its enemy have same position.  Object #%i\n",
             Pl_objp - Objects));
        En_objp = NULL;
    }
    ...
    if ((aip->resume_goal_time > 0) && (aip->resume_goal_time < Missiontime)) {
        aip->active_goal = AI_GOAL_NONE;
        aip->resume_goal_time = -1;
        target_objnum = find_enemy(objnum, 2000.0f,
                                   Skill_level_max_attackers[Game_skill_level]);
        if (target_objnum != -1) {
            if (aip->target_objnum != target_objnum) {
                aip->aspect_locked_time = 0.0f;
            }
    ...
    if ((En_objp != NULL) && (aip->targeted_subsys != NULL)) {
        ...
        if (aip->targeted_subsys->current_hits <= 0.0f) {
            ...
            if (subsys_type != -1) {
                ...
                new_subsys = ship_return_next_subsys(&Ships[En_objp->instance],
                                                     subsys_type, &Pl_objp->pos);
                if (new_subsys != NULL) {
...
void
ai_process(object *obj, int ai_index, float frametime)
{
    ...
    if ((Ships[obj->instance].flags & SF_DYING) &&
        !(Ship_info[Ships[obj->instance].ship_info_index].flags &
          (SIF_HUGE_SHIP | SIF_BIG_SHIP))) {
        return;
    }
    ...
    if (rfc == 1) {
        ...
        physics_read_flying_controls(&obj->orient, &obj->phys_info, &AI_ci,
                                     frametime);
...
init_ai_object(int objnum)
{
    ...
    aip->ai_courage =
        Ai_classes[Ship_info[ship_type].ai_class].ai_courage[Game_skill_level];
    aip->ai_patience =
        Ai_classes[Ship_info[ship_type].ai_class].ai_patience[Game_skill_level];
    aip->ai_evasion =
        Ai_classes[Ship_info[ship_type].ai_class].ai_evasion[Game_skill_level];
    aip->ai_accuracy =
        Ai_classes[Ship_info[ship_type].ai_class].ai_accuracy[Game_skill_level];
    ...
    aip->last_predicted_enemy_pos.x =
    ...
    aip->rearm_first_missile =
        TRUE; //  flag to indicate that next missile to load is the first missile
    ...
    aip->choose_enemy_timestamp = timestamp(
        3 * (NUM_SKILL_LEVELS - Game_skill_level) * ((rand_alt() % 500) + 500));
...
void
ai_set_default_behavior(object *obj, int classnum)
{
    ai_info *aip;
...
void
ai_do_default_behavior(object *obj)
{
    ai_info *aip;
    int ship_flags;
    ...
    if (!is_instructor(obj) && (ship_flags & (SIF_FIGHTER | SIF_BOMBER))) {
        int enemy_objnum = find_enemy(
            OBJ_INDEX(obj), 1000.0f, Skill_level_max_attackers[Game_skill_level]);
...
void
process_friendly_hit_message(int message, object *objp)
{
    int index;
    ...
    if (index >= 0) {
        message_send_builtin_to_player(message, &Ships[index],
                                       MESSAGE_PRIORITY_HIGH,
                                       MESSAGE_TIME_ANYTIME, 0, 0);
    }
    else {
        message_send_builtin_to_player(message, NULL, MESSAGE_PRIORITY_HIGH,
                                       MESSAGE_TIME_ANYTIME, 0, 0);
    }
}
...
void
maybe_process_friendly_hit(object *objp_hitter, object *objp_hit,
                           object *objp_weapon)
{
    ...
    if ((objp_hitter == Player_obj) && (Player_ship->team == TEAM_FRIENDLY)) {
        ...
        if (pp->friendly_hits != 0) {
            float time_since_last_hit = f2fl(Missiontime -
                                             pp->friendly_last_hit_time);
        ...
        damage =
            Weapon_info[Weapons[objp_weapon->instance].weapon_info_index].damage;
        ...
        float accredited_damage = min(MAX_BURST_DAMAGE,
                                      pp->damage_this_burst + damage) -
        ...
        nprintf((
            "AI",
            "Friendly damage: %.1f, threshold: %.1f, inc damage: %.1f, max burst: %d\n",
            pp->friendly_damage,
            FRIENDLY_DAMAGE_THRESHOLD *
                (1.0f + (float)(NUM_SKILL_LEVELS + 1 - Game_skill_level) / 3.0f),
        ...
        if (is_instructor(objp_hit)) {
            ...
            if (pp->friendly_damage > FRIENDLY_DAMAGE_THRESHOLD) {
                message_send_builtin_to_player(MESSAGE_INSTRUCTOR_ATTACK, NULL,
                                               MESSAGE_PRIORITY_HIGH,
                                               MESSAGE_TIME_IMMEDIATE, 0, 0);
                ...
                ai_set_mode_warp_out(
                    objp_hit, &Ai_info[Ships[objp_hit->instance].ai_index]);
                gameseq_post_event(
                    GS_EVENT_PLAYER_WARPOUT_START_FORCED); //  Force player to warp out.
                ...
                message_send_builtin_to_player(MESSAGE_INSTRUCTOR_HIT, NULL,
                                               MESSAGE_PRIORITY_HIGH,
                                               MESSAGE_TIME_IMMEDIATE, 0, 0);
                pp->last_warning_message_time = Missiontime;
            }
...
void
maybe_set_dynamic_chase(ai_info *aip, int hitter_objnum)
{
    Assert(Ship_info[Ships[aip->shipnum].ship_info_index].flags &
           (SIF_FIGHTER | SIF_BOMBER));
    ...
    if (aip->target_objnum != hitter_objnum)
        aip->aspect_locked_time = 0.0f;
    set_target_objnum(aip, hitter_objnum);
    aip->resume_goal_time = Missiontime +
                            i2f(20); //   Only chase up to 20 seconds.
    aip->active_goal = AI_ACTIVE_GOAL_DYNAMIC;

    set_targeted_subsys(aip, NULL,
                        -1); //  Say not attacking any particular subsystem.
...
int
firing_aspect_seeking_bomb(object *objp)
{
    ship *shipp;
    int bank_index;
    ship_weapon *swp;
    ...
    if (bank_index != -1)
        if (swp->secondary_bank_ammo[bank_index] > 0) {
            if (Weapon_info[swp->secondary_bank_weapons[bank_index]].wi_flags &
                WIF_BOMB) {
                if (Weapon_info[swp->secondary_bank_weapons[bank_index]].wi_flags &
                    WIF_HOMING_ASPECT) {
                    return 1;
...
void
big_ship_collide_recover_start(object *objp, object *big_objp,
                               vector *collide_pos, vector *collision_normal)
{
    ai_info *aip;
    ...
    get_world_closest_box_point_with_delta(&aip->big_recover_pos_2, big_objp,
                                           &aip->big_recover_pos_1, NULL, 300.0f);
...
void
ai_update_lethality(object *ship_obj, object *other_obj, float damage)
{
    ...
    if (Objects[parent].type == OBJ_SHIP) {
        if (Objects[parent].signature == other_obj->parent_sig) {
            ...
            if (Ships[ship_obj->instance].team !=
                ...
                if (other_obj->type == OBJ_WEAPON) {
                    ...
                    if (Ship_info[Ships[Objects[parent].instance].ship_info_index]
                            .flags &
                        (SIF_BIG_SHIP | SIF_HUGE_SHIP)) {
...
void
ai_ship_hit(object *objp_ship, object *hit_objp, vector *hitpos,
            int shield_quadrant, vector *hit_normal)
{
    int hitter_objnum = -2;
    ...
    if (hit_objp->type == OBJ_SHIP) {
        ...
        if (Ship_info[Ships[hit_objp->instance].ship_info_index].flags &
            (SIF_BIG_SHIP | SIF_HUGE_SHIP)) {
            // And the current object is _not_ a big ship
            if (!(Ship_info[Ships[objp_ship->instance].ship_info_index].flags &
                  (SIF_BIG_SHIP | SIF_HUGE_SHIP))) {
                ...
                big_ship_collide_recover_start(objp_ship, hit_objp, hitpos,
                                               hit_normal);
            }
        }
    }
    ...
    if (hit_objp->type == OBJ_WEAPON) {
        ...
        if (shipp->team ==
            Ships[hit_objp->instance]
                .team) //  Don't have AI react to collisions between teammates.
            return;
    ...
    if (aip->ai_flags & (AIF_AWAITING_REPAIR | AIF_BEING_REPAIRED)) {
        ...
        if (objp_ship->hull_strength / sip->initial_hull_strength < 0.3f) {
            ...
            if (!(objp_ship->flags &
                  OF_PLAYER_SHIP)) // mwa -- don't abort rearm for a player
    ...
    //   If the hitter object is the ignore object, don't attack it.
    ship_info *sip = &Ship_info[shipp->ship_info_index];
    if ((is_ignore_object(aip, objp_hitter - Objects)) &&
        (sip->flags & (SIF_BOMBER | SIF_FIGHTER))) {
        if (aip->mode == AIM_NONE) {
            aip->mode =
                AIM_CHASE; // This will cause the ship to move, if not attack.
            aip->submode = SM_EVADE;
        }
        return;
    }
    ...
    case AIM_GUARD:
        ...
        if ((aip->guard_objnum != -1) &&
            (aip->guard_signature == Objects[aip->guard_objnum].signature)) {
            if (vm_vec_dist_quick(&objp_ship->pos,
                                  &Objects[aip->guard_objnum].pos) > 500.0f) {
                return;
            }
        }
    ...
    if (shipp->team != Ships[objp_hitter->instance].team) {
        ...
        if ((hitter_objnum != aip->target_objnum) &&
            (sip->flags & (SIF_FIGHTER | SIF_BOMBER))) {
            maybe_set_dynamic_chase(aip, hitter_objnum);
            maybe_afterburner_after_ship_hit(objp_ship, aip,
                                             &Objects[hitter_objnum]);
        }
        else {
            if ((aip->mode == AIM_CHASE) &&
                ((objp_ship->hull_strength / sip->initial_hull_strength > 0.9f) ||
                 (get_shield_strength(objp_ship) / sip->shields > 0.8f))) {
                switch (aip->submode) {
                case SM_ATTACK:
                case SM_SUPER_ATTACK:
                case SM_GET_AWAY:
...
void
ai_ship_destroy(int shipnum, int method)
{
    int objnum;
    object *other_objp;
    ship *shipp;
    ...
    if (dead_aip->ai_flags & (AIF_AWAITING_REPAIR | AIF_BEING_REPAIRED)) {
        if (dead_aip->dock_objnum >= 0)
            ai_do_objects_repairing_stuff(&Objects[objnum],
                                          &Objects[dead_aip->dock_objnum],
                                          REPAIR_INFO_END);
        else
            ai_do_objects_repairing_stuff(&Objects[objnum], NULL,
                                          REPAIR_INFO_END);
    }
...
void
ai_deathroll_start(object *ship_obj)
{
    ai_info *aip;
...
int
ai_abort_rearm_request(object *requester_objp)
{
    ...
    Assert((requester_objp->instance >= 0) &&
    ...
    Assert((requester_shipp->ai_index >= 0) &&
        ...
        if (requester_aip->dock_objnum != -1) {
            ...
            if (repair_objp->signature == requester_aip->dock_signature) {
                ...
                if (repair_aip->submode < AIS_UNDOCK_0) {
                    ai_do_objects_repairing_stuff(requester_objp, repair_objp,
                                                  REPAIR_INFO_ABORT);
            ...
            ai_do_objects_repairing_stuff(requester_objp, NULL,
                                          REPAIR_INFO_ABORT);
...
void
ai_add_rearm_goal(object *requester_objp, object *support_objp)
{
    ...
    requester_aip->ai_flags |=
        AIF_AWAITING_REPAIR; //  Tell that I'm awaiting repair.
    if (requester_objp->flags & OF_PLAYER_SHIP)
        ai_add_ship_goal_player(AIG_TYPE_PLAYER_SHIP, AI_GOAL_REARM_REPAIR, -1,
                                requester_shipp->ship_name, support_aip);
    else
        ai_add_goal_ship_internal(support_aip, AI_GOAL_REARM_REPAIR,
                                  requester_shipp->ship_name, -1, -1);
}
...
int
ai_issue_rearm_request(object *requester_objp)
{
    object *objp;
    ship *requester_shipp;
    ai_info *requester_aip;
    ...
    if (requester_aip->ai_flags & AIF_AWAITING_REPAIR) {
        nprintf(("AI", "Ship %s already awaiting rearm by ship %s.\n",
                 ...
                 &Ships[Objects[requester_aip->dock_objnum].instance].ship_name));
        return -1;
    }
...
void
ai_rearm_repair(object *objp, object *goal_objp, int priority, int docker_index,
                int dockee_index)
{
    ...
    ai_dock_with_object(objp, goal_objp, priority, AIDO_DOCK, docker_index,
                        dockee_index);
    aip->ai_flags |=
        AIF_REPAIRING; //  Tell that repair guy is busy trying to repair someone.

    goal_aip = &Ai_info[Ships[goal_objp->instance].ai_index];
    goal_aip->dock_objnum = objp -
                            Objects; //   Tell which object is coming to repair.
...
int
ai_return_path_num_from_dockbay(object *dockee_objp, int dockbay_index)
{
...
void
cheat_fire_synaptic(object *objp, ship *shipp, ai_info *aip)
{
    ship_weapon *swp;
    swp = &shipp->weapons;
...
void
maybe_cheat_fire_synaptic(object *objp, ai_info *aip)
{
                ...
                if (time > 0) {
                    ...
                    if ((time % modulus) < 2) {
                        int count = num_nearby_fighters(
                            get_enemy_team_mask(OBJ_INDEX(objp)), &objp->pos,
                            1500.0f);
