...
void
shipfx_subsystem_mabye_create_live_debris(object *ship_obj, ship *ship_p,
                                          ...
                                          float exp_mag)
{
    ...
    void model_get_rotating_submodel_axis(vector * model_axis,
                                          vector * world_axis, int modelnum,
                                          int submodel_num, object *obj);
    model_get_rotating_submodel_axis(&model_axis, &world_axis, ship_p->modelnum,
                                     submodel_num, ship_obj);
    vm_vec_copy_scale(&rotvel, &world_axis, sii->cur_turn_rate);

    model_find_world_point(&world_axis_pt, &sii->pt_on_axis, ship_p->modelnum,
    ...
    for (i = 0; i < num_live_debris; i++) {
        ...
        model_find_world_point(&start_world_pos,
                               &pm->submodel[live_debris_submodel].offset,
                               ship_p->modelnum, live_debris_submodel,
        ...
        model_find_world_point(&end_world_pos, &start_model_pos, ship_p->modelnum,
                               submodel_num, &ship_obj->orient, &ship_obj->pos);

        // create fireball here.
        fireball_create(&end_world_pos, FIREBALL_EXPLOSION_MEDIUM,
                        OBJ_INDEX(ship_obj),
                        pm->submodel[live_debris_submodel].rad);

        // create debris
        live_debris_obj = debris_create(ship_obj, ship_p->modelnum,
                                        live_debris_submodel, &end_world_pos,
        ...
        if (live_debris_obj) {
            ...
            if (Ship_info[ship_p->ship_info_index].flags & SIF_KNOSSOS_DEVICE) {
                ...
                float vel_mag = vm_vec_mag_quick(&radial_vel) * 1.3f *
                                (0.9f + 0.2f * frand());
                vm_vec_normalized_dir(&vec_to_center, &world_axis_pt,
                                      &end_world_pos);
                vm_vec_rand_vec_quick(&rand_vec);
                vm_vec_scale_add2(&vec_to_center, &rand_vec, 0.2f);
                vm_vec_scale_add2(&live_debris_obj->phys_info.vel, &vec_to_center,
                                  vel_mag);
            }
            else {
                ...
                vector
                    temp_vel; // explosion velocity with ship_obj velocity removed
                vm_vec_sub(&temp_vel, &live_debris_obj->phys_info.vel,
                           &ship_obj->phys_info.vel);
                ...
                if (rotvel_mag > 0.1) {
                    ...
                    vm_vec_scale_add2(&live_debris_obj->phys_info.vel,
                                      &radial_vel, scale);
                }

                // scale up speed of debris if ship_obj > 125, but not for knossos
                if (ship_obj->radius > 250 &&
                    !(Ship_info[ship_p->ship_info_index].flags &
                      SIF_KNOSSOS_DEVICE)) {
                    vm_vec_scale(&live_debris_obj->phys_info.vel,
                                 ship_obj->radius / 250.0f);
                }
            }
        }
    }
...
void
set_ship_submodel_as_blown_off(ship *shipp, char *name)
{
    int found = FALSE;
...
void
shipfx_maybe_create_live_debris_at_ship_death(object *ship_obj)
{
    ...
    for (int idx = 0; idx < pm->num_debris_objects; idx++) {
        if (pm->submodel[pm->debris_objects[idx]].is_live_debris) {
            ...
            int parent = model_get_parent_submodel_for_live_debris(
                shipp->modelnum, live_debris_submodel);
            Assert(parent != -1);
            ...
            if (!pm->submodel[parent].blown_off) {
                ...
                if (pss != NULL) {
                    ...
                    model_find_world_point(&exp_center, &tmp, shipp->modelnum,
                                           parent, &ship_obj->orient,
                                           &ship_obj->pos);

                    // if not blown off, blow it off
                    shipfx_subsystem_mabye_create_live_debris(
                        ship_obj, shipp, pss, &exp_center, 3.0f);

                    // now set subsystem as blown off, so we only get one copy
                    pm->submodel[parent].blown_off = 1;
                    set_ship_submodel_as_blown_off(&Ships[ship_obj->instance],
...
void
shipfx_blow_off_subsystem(object *ship_obj, ship *ship_p, ship_subsys *subsys,
                          vector *exp_center)
{
    vector subobj_pos;
...
*/
    ...
    shipfx_blow_up_model(ship_obj, ship_p->modelnum, psub->subobj_num, 50,
                         &subobj_pos);

    // create live debris objects, if any
    // TODO:  some MULITPLAYER implcations here!!
    shipfx_subsystem_mabye_create_live_debris(ship_obj, ship_p, subsys,
                                              exp_center, 1.0f);

    // create first fireball
    fireball_create(&subobj_pos, FIREBALL_EXPLOSION_MEDIUM, OBJ_INDEX(ship_obj),
                    psub->radius);
}

void
shipfx_blow_up_hull(object *obj, int model, vector *exp_center)
{
    int i;
    polymodel *pm;
    ...
    for (i = 0; i < pm->num_debris_objects; i++) {
        if (!pm->submodel[pm->debris_objects[i]].is_live_debris) {
            ...
            model_find_world_point(&tmp,
                                   &pm->submodel[pm->debris_objects[i]].offset,
                                   model, 0, &obj->orient, &obj->pos);
            debris_create(obj, model, pm->debris_objects[i], &tmp, exp_center, 1,
                          3.0f);
        }
        else {
            if (try_live_debris) {
...
// ship's model.
void
shipfx_blow_up_model(object *obj, int model, int submodel, int ndebris,
                     vector *exp_center)
{
    int i;
    ...
    for (i = 0; i < ndebris; i++) {
        ...
        model_find_world_point(&outpnt, &tmp, model, submodel, &obj->orient,
                               &obj->pos);
...
static float
shipfx_calculate_effect_radius(object *objp)
{
    float w, h, rad;
...
static float
shipfx_calculate_warp_time(object *objp)
{
    // Find rad_percent from 0 to 1, 0 being smallest ship, 1 being largest
    float rad_percent = (objp->radius - SMALLEST_RAD) /
                        (LARGEST_RAD - SMALLEST_RAD);
    if (rad_percent < 0.0f) {
        rad_percent = 0.0f;
    }
...
float
shipfx_calculate_warp_speed(object *objp)
{
    float length;
...
void
shipfx_actually_warpin(ship *shipp, object *objp)
{
...
// time.
void
shipfx_warpin_start(object *objp)
{
    ship *shipp;
    ...
    if (shipp->special_warp_objnum >= 0) {
        ...
        if ((ref_objnum >= 0) && (ref_objnum < MAX_OBJECTS)) {
            ...
            if (sp_objp->type == OBJ_SHIP) {
                if (Ship_info[Ships[sp_objp->instance].ship_info_index].flags &
                    SIF_KNOSSOS_DEVICE) {
    ...
    if (shipp->special_warp_objnum >= 0) {
        ...
        vm_vec_scale_add(&shipp->warp_effect_pos, &objp->pos, &objp->orient.fvec,
                         -pm->mins.z);
    }
    else {
        vm_vec_scale_add(&shipp->warp_effect_pos, &objp->pos, &objp->orient.fvec,
    ...
    if (!(shipp->flags & SF_INITIALLY_DOCKED)) {
        ...
        effect_time = shipfx_calculate_warp_time(objp) + SHIPFX_WARP_DELAY +
                      SHIPFX_WARP_DELAY;
        ...
        if (shipp->special_warp_objnum >= 0) {
            ...
            effect_radius = min(
                effect_radius, 0.8f * Objects[shipp->special_warp_objnum].radius);
            warp_objnum = fireball_create(&shipp->warp_effect_pos,
                                          FIREBALL_KNOSSOS_EFFECT,
                                          shipp->special_warp_objnum,
                                          effect_radius, 0, NULL, effect_time,
                                          shipp->ship_info_index);
        }
        else {
            warp_objnum = fireball_create(&shipp->warp_effect_pos,
                                          FIREBALL_WARP_EFFECT, OBJ_INDEX(objp),
                                          effect_radius, 0, NULL, effect_time,
                                          shipp->ship_info_index);
        }
        if (warp_objnum <
            0) { // JAS: This must always be created, if not, just warp the ship in
            shipfx_actually_warpin(shipp, objp);
            return;
        }
        ...
        if (shipp->special_warp_objnum >= 0) {
            if (vm_vec_dotprod(&shipp->warp_effect_fvec, &objp->orient.fvec) <
                0) {
...
void
shipfx_warpin_frame(object *objp, float frametime)
{
    ship *shipp;
    ...
    if (shipp->flags & SF_ARRIVING_STAGE_1) {
        if (timestamp_elapsed(shipp->final_warp_time)) {
            ...
            float speed = shipfx_calculate_warp_speed(
                objp); // How long it takes to move through warp effect
...
// fx.
void
shipfx_actually_warpout(ship *shipp, object *objp)
{
...
int
compute_special_warpout_stuff(object *objp, float *speed, float *warp_time,
                              vector *warp_pos)
{
    object *sp_objp = NULL;
    ship *shipp;
    ...
    dist_to_plane = fvi_ray_plane(warp_pos, &sp_objp->pos, &facing_normal,
                                  &objp->pos, &objp->orient.fvec, 0.0f);
    ...
    if (-vm_vec_dotprod(&objp->orient.fvec, &facing_normal) <
...
void
compute_warpout_stuff(object *objp, float *speed, float *warp_time,
                      vector *warp_pos)
{
    ...
    *speed = shipfx_calculate_warp_speed(objp);
    ...
    ship_move_dist = (*speed * SHIPFX_WARP_DELAY) +
...
void
shipfx_warpout_start(object *objp)
{
    float warp_time;
    ship *shipp;
    shipp = &Ships[objp->instance];
    ...
    if (shipp->special_warp_objnum >= 0) {
        // cap radius to size of knossos
        effect_radius = min(effect_radius,
                            0.8f * Objects[shipp->special_warp_objnum].radius);
        warp_objnum = fireball_create(&shipp->warp_effect_pos,
                                      FIREBALL_KNOSSOS_EFFECT,
                                      shipp->special_warp_objnum, effect_radius,
                                      1, NULL, effect_time,
                                      shipp->ship_info_index);
    }
    else {
        warp_objnum = fireball_create(&shipp->warp_effect_pos,
                                      FIREBALL_WARP_EFFECT, OBJ_INDEX(objp),
                                      effect_radius, 1, NULL, effect_time,
                                      shipp->ship_info_index);
    }
    if (warp_objnum <
        0) { // JAS: This must always be created, if not, just warp the ship in
...
void
shipfx_warpout_frame(object *objp, float frametime)
{
    ship *shipp;
    shipp = &Ships[objp->instance];
    ...
    if (objp == Player_obj) {
        ...
        if ((warp_pos > objp->radius) || (shipp->flags & SF_NO_DEPARTURE_WARP) ||
            timed_out) {
            shipfx_actually_warpout(shipp, objp);
        }
    }
...
// it can see the sun.
int
shipfx_point_in_shadow(vector *p0, matrix *src_orient, vector *src_pos,
                       float radius)
{
    mc_info mc;
    object *objp;
...
int
shipfx_in_shadow(object *src_obj)
{
    mc_info mc;
    object *objp;
    ship_obj *so;
...
int
shipfx_eye_in_shadow(vector *eye_pos, object *src_obj, int sun_n)
{
    mc_info mc;
    object *objp;
    ship_obj *so;
...
// accordingly.
void
shipfx_flash_create(object *objp, ship *shipp, vector *gun_pos, vector *gun_dir,
                    int is_primary, int weapon_info_index)
{
    ...
    if ((Weapon_info[weapon_info_index].wi_flags & WIF_MFLASH) &&
        !(Weapon_info[weapon_info_index].wi_flags & WIF_FLAK)) {
        // spiffy new flash stuff
        mflash_create(gun_pos, gun_dir,
                      Weapon_info[weapon_info_index].muzzle_flash);
    }
...
void
shipfx_flash_light_model(object *objp, ship *shipp)
{
...
void
shipfx_flash_do_frame(float frametime)
{
    ship_flash *sf;
    int kill_it = 0;
    int i;
...
DCF(particle_width, "Multiplier for angular width of the particle spew")
{
    if (Dc_command) {
        ...
        if ((Dc_arg_float >= 0) && (Dc_arg_float <= 5)) {
            ...
            dc_printf(
                "Illegal value for particle width. (Must be from 0-5) \n\n");
        }
    }
}
...
void
shipfx_emit_spark(int n, int sn)
{
    int create_spark = 1;
    object *obj;
    vector outpnt;
    ...
    if (shipp->sparks[spark_num].submodel_num != -1) {
        ...
        model_find_world_point(&outpnt, &shipp->sparks[spark_num].pos,
                               shipp->modelnum,
        ...
        if (sn > -1) {
            ...
            if (si->flags & SIF_FIGHTER) {
                if (hull_percent > 0.6f) {
                    ...
                    shipp->sparks[spark_num].end_time = timestamp(
                        (int)(1000.0f * spark_duration));
                }
                else {
            ...
            pe.min_life = 0.35f * 2.0f *
                          spark_time_scale; // How long the particles live
            pe.max_life = 0.75f * 2.0f *
                          spark_time_scale; // How long the particles live
    ...
    "Multiplier time between fireball in big ship explosion")
{
    if (Dc_command) {
        ...
        if ((Dc_arg_float >= 0.1) && (Dc_arg_float <= 5)) {
            ...
            dc_printf(
                "Illegal value for bs_exp_fire_time_mult. (Must be from 0.1-5) \n\n");
        }
    }
}
...
typedef struct clip_ship
{
    ...
    vector
        model_center_disp_to_orig_center; // displacement from half ship center to original model center
...
static void
split_ship_init(ship *shipp, split_ship *split_ship)
{
    ...
    snd_play_3d(&Snds[SND_SHOCKWAVE_EXPLODE], &parent_ship_obj->pos,
                &View_position, 0.0f, NULL, 0, 1.0f, SND_PRIORITY_SINGLE_INSTANCE,
                NULL, 3.0f);

    // initialize both ships
    split_ship->front_ship.parent_obj = parent_ship_obj;
    split_ship->back_ship.parent_obj = parent_ship_obj;
    split_ship->explosion_flash_timestamp = timestamp(
        (int)(0.00075f * parent_ship_obj->radius));
    ...
    vm_vec_copy_scale(&split_ship->back_ship.clip_plane_norm, &vmd_z_vector,
                      -1.0f);
    ...
    vm_vec_make(&split_ship->front_ship.model_center_disp_to_orig_center, 0.0f,
                0.0f, -dist);
    ...
    vm_vec_make(&split_ship->back_ship.model_center_disp_to_orig_center, 0.0f,
                0.0f, -dist);
    ...
    for (int i = 0; i < pm->num_debris_objects; i++) {
        ...
        model_find_world_point(&tmp, &tmp1, shipp->modelnum, -1,
                               &vmd_identity_matrix, &temp_pos);
   ...
   for (i=pm->num_debris_objects; i<MAX_DEBRIS_OBJECTS; i++) {
    ...
    vm_vec_copy_scale(&split_ship->back_ship.phys_info.rotvel, &rotvel,
                      -(front_length * front_length) /
                          (back_length * back_length));
    split_ship->front_ship.phys_info.rotvel.z =
        parent_ship_obj->phys_info.rotvel.z;
    ...
    maybe_fireball_wipe(&split_ship->front_ship,
                        (int *)&split_ship->sound_handle);
    maybe_fireball_wipe(&split_ship->back_ship, (int *)&split_ship->sound_handle);
    vm_vec_scale_add2(&split_ship->front_ship.phys_info.vel, &orient->fvec,
                      front_vel);
    vm_vec_scale_add2(&split_ship->back_ship.phys_info.vel, &orient->fvec,
                      back_vel);
...
static void
half_ship_render_ship_and_debris(clip_ship *half_ship, ship *shipp)
{
    ...
    vector orig_ship_world_center, clip_plane_norm, model_clip_plane_pt,
        debris_clip_plane_pt;
    vm_vec_unrotate(&clip_plane_norm, &half_ship->clip_plane_norm,
                    &half_ship->orient);
    vm_vec_unrotate(&orig_ship_world_center,
    ...
    vm_vec_unrotate(&debris_clip_plane_pt,
                    &half_ship->model_center_disp_to_orig_center,
                    &half_ship->orient);
        ...
        if (half_ship->draw_debris[i] == DEBRIS_DRAW) {
            ...
            if (half_ship->explosion_vel > 0) {
                if (half_ship->cur_clip_plane_pt >
                    tmp1.z + pm->submodel[pm->debris_objects[i]].max.z -
                ...
                if (half_ship->cur_clip_plane_pt <
                    tmp1.z + pm->submodel[pm->debris_objects[i]].min.z -
                        0.1f * half_ship->explosion_vel) {
                    create_debris = 1;
                }
                // is the debris visible
            ...
            if (!is_live_debris) {
                model_find_world_point(&tmp, &tmp1, shipp->modelnum, -1,
                                       &half_ship->orient, &temp_pos);
                submodel_render(shipp->modelnum, pm->debris_objects[i],
                                &half_ship->orient, &tmp, render_flags);
            }
            ...
            if (create_debris) {
                ...
                if (!is_live_debris) {
                    object *debris_obj;
                    debris_obj = debris_create(half_ship->parent_obj,
                                               shipp->modelnum,
                                               pm->debris_objects[i], &tmp,
                                               &half_ship->local_pivot, 1, 1.0f);
                    ...
                    if (debris_obj) {
                        ...
                        vm_vec_sub(&center_to_debris, &tmp,
                                   &half_ship->local_pivot);
                        vm_vec_crossprod(&debris_vel, &center_to_debris,
    ...
    model_render(shipp->modelnum, &half_ship->orient, &orig_ship_world_center,
                 render_flags);
}
...
int
shipfx_large_blowup_init(ship *shipp)
{
...
float
get_model_cross_section_at_z(float z, polymodel *pm)
{
    if (pm->num_xc < 2) {
        return 0.0f;
    }
    ...
    increment = (pm->xc[pm->num_xc - 1].z - pm->xc[0].z) /
                (float)(pm->num_xc - 1);
...
int
get_sound_time_played(int snd_id, int handle)
{
    if (handle == -1) {
        return 100000;
    }
...
void
do_sub_expl_sound(float radius, vector *sound_pos, int *sound_handle)
{
    ...
    if (handle == -1) {
        ...
        sound_handle[handle_index] = snd_play_3d(&Snds[sound_index], sound_pos,
                                                 &View_position, 0.0f, NULL, 0,
                                                 0.6f, SND_PRIORITY_MUST_PLAY,
                                                 NULL, sound_range);
    }
        ...
        if (get_sound_time_played(Snds[sound_index].id, handle) > 400) {
            ...
            sound_handle[handle_index] = snd_play_3d(
                &Snds[sound_index], sound_pos, &View_position, 0.0f, NULL, 0,
...
static void
maybe_fireball_wipe(clip_ship *half_ship, int *sound_handle)
{
    ...
    if (timestamp_elapsed(half_ship->next_fireball)) {
        if (half_ship->length_left > 0.2f * fl_abs(half_ship->explosion_vel)) {
            polymodel *pm = model_get(
                Ships[half_ship->parent_obj->instance].modelnum);
            ...
            float rad = get_model_cross_section_at_z(half_ship->cur_clip_plane_pt,
                                                     pm);
            if (rad < 1) {
            ...
            fireball_create(&model_clip_plane_pt, fireball_type,
            ...
            // do sound
            do_sub_expl_sound(half_ship->parent_obj->radius, &model_clip_plane_pt,
                              sound_handle);
...
int
shipfx_large_blowup_do_frame(ship *shipp, float frametime)
{
    ...
    if (timestamp_elapsed(the_split_ship->explosion_flash_timestamp)) {
        if (!the_split_ship->explosion_flash_started) {
            ...
            if (objp->flags & OF_WAS_RENDERED) {
                float excess_dist = vm_vec_dist(&Player_obj->pos, &objp->pos) -
    ...
    physics_sim(&the_split_ship->front_ship.local_pivot,
                &the_split_ship->front_ship.orient,
                &the_split_ship->front_ship.phys_info, frametime);
    physics_sim(&the_split_ship->back_ship.local_pivot,
    ...
    maybe_fireball_wipe(&the_split_ship->front_ship,
                        (int *)&the_split_ship->sound_handle);
    maybe_fireball_wipe(&the_split_ship->back_ship,
                        (int *)&the_split_ship->sound_handle);
    return 0;
}
...
void
shipfx_do_damaged_arcs_frame(ship *shipp)
{
    int i;
    int should_arc;
    ...
    if (timestamp_elapsed(shipp->arc_next_time)) {
        shipp->arc_next_time = timestamp(
            -1); // invalid, so it gets restarted next frame
        ...
        for (i = 0; i < MAX_SHIP_ARCS; i++) {
            ...
            if (lifetime > 750) {
                // 1.00 second effect
                snd_play_3d(&Snds[SND_DEBRIS_ARC_05], &snd_pos, &View_position,
                            obj->radius);
            }
            else if (lifetime > 500) {
                // 0.75 second effect
                snd_play_3d(&Snds[SND_DEBRIS_ARC_04], &snd_pos, &View_position,
                            obj->radius);
            }
            else if (lifetime > 250) {
                // 0.50 second effect
                snd_play_3d(&Snds[SND_DEBRIS_ARC_03], &snd_pos, &View_position,
                            obj->radius);
            }
            else if (lifetime > 100) {
                // 0.25 second effect
                snd_play_3d(&Snds[SND_DEBRIS_ARC_02], &snd_pos, &View_position,
                            obj->radius);
            }
            else {
                // 0.10 second effect
                snd_play_3d(&Snds[SND_DEBRIS_ARC_01], &snd_pos, &View_position,
                            obj->radius);
            }
        }
    }
            ...
            if (!timestamp_elapsed(shipp->arc_timestamp[i])) {
                ...
                if (mr < RAND_MAX / 5) {
                    ...
                    if (obj->radius > 200.0f) {
                        ...
                        d = vm_vec_mag_quick(&tmp);
                        if (d > max_dist) {
                            vm_vec_scale_add(&v1, &static_one, &tmp,
                                             max_dist / d);
                        }
                    }
...
void
shipfx_do_shockwave_stuff(ship *shipp, shockwave_create_info *sci)
{
    ship_info *sip;
    object *objp;
    polymodel *pm;
    ...
    for (idx = 0; idx < sip->shockwave_count; idx++) {
        ...
        shockwave_create(shipp->objnum, &shockwave_pos, &sci2, SW_SHIP_DEATH,
...
void
engine_wash_ship_process(ship *shipp)
{
    int idx, j;
    ...
    vector world_thruster_pos, world_thruster_norm, apex, thruster_to_ship,
        apex_to_ship, temp;
    ...
    if ((objp->type != OBJ_SHIP) ||
        !(Ship_info[shipp->ship_info_index].flags & (SIF_FIGHTER | SIF_BOMBER))) {
        return;
    }
    ...
    if (time_to_next_hit < 0) {
        ...
        shipp->wash_timestamp = timestamp(ENGINE_WASH_CHECK_INTERVAL +
                                          time_to_next_hit);
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if (ship_get_subsystem_strength(&Ships[ship_objp->instance],
                                        SUBSYSTEM_ENGINE) < 0.01) {
            continue;
        }
        ...
        for (idx = 0; idx < pm->n_thrusters; idx++) {
            ...
            if (bank->wash_info_index < 0) {
                ...
                if (Ship_info[Ships[ship_objp->instance].ship_info_index].flags &
                    ...
                    nprintf(("wash", "Adding default engine wash to ship %s",
                             Ship_info[Ships[ship_objp->instance].ship_info_index]
                                 .name));
                }
                else {
                    continue;
            ...
            for (j = 0; j < bank->num_slots; j++) {
                ...
                vm_vec_unrotate(&world_thruster_pos, &bank->pnt[j],
                                &ship_objp->orient);
                vm_vec_add2(&world_thruster_pos, &ship_objp->pos);

                // get world norm of thruster;
                vm_vec_unrotate(&world_thruster_norm, &bank->norm[j],
                                &ship_objp->orient);
                ...
                dot_to_ship = vm_vec_dotprod(&thruster_to_ship,
                                             &world_thruster_norm);
                if (dot_to_ship > 0) {
                    // get max wash distance
                    max_wash_dist = max(ewp->length,
                                        bank->radius[j] * ewp->radius_mult);
                    ...
                    if (dist_sqr < max_wash_dist * max_wash_dist) {
                        ...
                        if (dist_sqr < radius_mult * radius_mult *
                                           bank->radius[j] * bank->radius[j]) {
                            vm_vec_crossprod(&temp, &world_thruster_norm,
                                             &thruster_to_ship);
                            ...
                            inset_depth = float(bank->radius[j] /
                                                tan(half_angle));
                            vm_vec_scale_add(&apex, &world_thruster_pos,
                            ...
                            if (vm_vec_dotprod(&apex_to_ship,
                                               &world_thruster_norm) >
                                cos(half_angle)) {
                                vm_vec_crossprod(&temp, &world_thruster_norm,
                                                 &thruster_to_ship);
    ...
    if (shipp->wash_intensity > 0) {
        ...
        if (!do_damage) {
            ...
            damage = (0.001f * 0.003f * ENGINE_WASH_CHECK_INTERVAL *
                      Ship_info[shipp->ship_info_index].initial_hull_strength *
                      shipp->wash_intensity);
        }

        ship_apply_wash_damage(&Objects[shipp->objnum], max_ship_intensity_objp,
                               damage);

        // if we had no wash before now, add the wash object sound
        if (started_with_no_wash) {
            if (shipp != Player_ship) {
                obj_snd_assign(shipp->objnum, SND_ENGINE_WASH, &vmd_zero_vector,
                               1);
            }
            else {
                Player_engine_wash_loop = snd_play_looping(&Snds[SND_ENGINE_WASH],
                                                           0.0f, -1, -1, 1.0f);
            }
        }
    }
