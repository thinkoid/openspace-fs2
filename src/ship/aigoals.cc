...
// player wing arrives.
void
ai_maybe_add_form_goal(wing *wingp)
{
    int j;
    ...
    for (j = 0; j < wingp->current_count; j++) {
        ...
        if (aip->goals[0].ai_mode == AI_GOAL_NONE) {
            ...
            ai_add_ship_goal_player(AIG_TYPE_PLAYER_SHIP, AI_GOAL_FORM_ON_WING,
                                    -1, Player_ship->ship_name, aip);
        }
    }
}
...
int
ai_query_goal_valid(int ship, int ai_goal)
{
    int accepted;
...
void
ai_remove_ship_goal(ai_info *aip, int index)
{
    ...
    aip->goals[index].flags =
        0; // must reset the flags since not doing so will screw up goal sorting.
...
void
ai_clear_ship_goals(ai_info *aip)
{
    int i;
...
void
ai_clear_wing_goals(int wingnum)
{
    int i;
...
void
ai_mission_wing_goal_complete(int wingnum, ai_goal *remove_goalp)
{
    ...
    for (i = 0; i < wingp->current_count; i++) {
        ...
        for (j = 0; j < MAX_AI_GOALS; j++) {
            ...
            if ((aigp->ai_mode == mode) && (aigp->ai_submode == submode) &&
                (aigp->priority == priority) && !stricmp(name, aigp->ship_name)) {
...
void
ai_mission_goal_complete(ai_info *aip)
{
...
int
ai_get_subsystem_type(char *subsystem)
{
...
void
ai_goal_purge_invalid_goals(ai_goal *aigp, ai_goal *goal_list)
{
    int i;
    ai_goal *purge_goal;
    char *name;
        ...
        case AI_GOAL_IGNORE:
            if (purge_ai_mode &
                (AI_GOAL_DISABLE_SHIP | AI_GOAL_DISARM_SHIP | AI_GOAL_CHASE |
                 AI_GOAL_CHASE_WING | AI_GOAL_DESTROY_SUBSYSTEM))
...
void
ai_goal_purge_all_invalid_goals(ai_goal *aigp)
{
    int mode, i;
    ship_obj *sop;
...
void
ai_goal_fixup_dockpoints(ai_info *aip, ai_goal *aigp)
{
    ...
    if (Ship_info[Ships[shipnum].ship_info_index].flags & SIF_CARGO) {
        docker_index = model_find_dock_index(Ships[aip->shipnum].modelnum,
                                             DOCK_TYPE_CARGO);
        dockee_index = model_find_dock_index(Ships[shipnum].modelnum,
                                             DOCK_TYPE_CARGO);
    }
...
void
ai_add_goal_sub_player(int type, int mode, int submode, char *shipname,
                       ai_goal *aigp)
{
    ...
    if ((mode == AI_GOAL_STAY_NEAR_SHIP) || (mode == AI_GOAL_KEEP_SAFE_DISTANCE))
        ...
        aigp->priority =
            PLAYER_PRIORITY_WING; // player wing goals not as high as ship goals
    else
        aigp->priority = PLAYER_PRIORITY_SHIP;
}

int
ai_goal_find_empty_slot(ai_goal *goals)
{
    ...
    for (gindex = 0; gindex < MAX_AI_GOALS; gindex++) {
        ...
        if ((empty_index == -1) && (goals[gindex].ai_mode ==
                                    AI_GOAL_NONE)) // get the index for this goal
            empty_index = gindex;
    }
...
void
ai_add_ship_goal_player(int type, int mode, int submode, char *shipname,
                        ai_info *aip)
{
    int empty_index;
    ai_goal *aigp;
...
// ships in the wing
void
ai_add_wing_goal_player(int type, int mode, int submode, char *shipname,
                        int wingnum)
{
    int i, empty_index;
    wing *wingp = &Wings[wingnum];
    ...
    ai_add_goal_sub_player(type, mode, submode, shipname,
                           &wingp->ai_goals[empty_index]);
}

// common routine to add a sexpression mission goal to the appropriate goal structure.
void
ai_add_goal_sub_sexp(int sexp, int type, ai_goal *aigp)
{
    int node, dummy, op;
    char *text;
...
void
ai_add_ship_goal_sexp(int sexp, int type, ai_info *aip)
{
    int gindex;
...
void
ai_add_wing_goal_sexp(int sexp, int type, int wingnum)
{
    int i;
    wing *wingp = &Wings[wingnum];
...
void
ai_add_goal_ship_internal(ai_info *aip, int goal_type, char *name,
                          int docker_point, int dockee_point, int immediate)
{
    int gindex;
    ai_goal *aigp;
    ...
    case AI_GOAL_REARM_REPAIR:
        ...
        aigp->flags &=
            ~AIGF_GOAL_OVERRIDE; // don't override this goal.  rearm repair requests should happen in order
        ai_goal_fixup_dockpoints(aip, aigp);
        break;
...
void
ai_add_goal_wing_internal(wing *wingp, int goal_type, char *name, int immediate)
{
    int i;
    ...
    for (i = 0; i < wingp->current_count; i++) {
        ...
        if (num == -1) // ship must have been destroyed or departed
            continue;
        ai_add_goal_ship_internal(&Ai_info[Ships[num].ai_index], goal_type, name,
                                  -1, -1, immediate);
    }
}
...
void
ai_copy_mission_wing_goal(ai_goal *aigp, ai_info *aip)
{
    int j;
...
ai_mission_goal_achievable(int objnum, ai_goal *aigp)
{
    ...
    if ((aigp->ai_mode == AI_GOAL_KEEP_SAFE_DISTANCE) ||
        (aigp->ai_mode == AI_GOAL_WARP) || (aigp->ai_mode == AI_GOAL_CHASE_ANY) ||
        (aigp->ai_mode == AI_GOAL_STAY_STILL) ||
        (aigp->ai_mode == AI_GOAL_PLAY_DEAD))
    ...
    if ((aigp->ai_mode == AI_GOAL_WAYPOINTS_ONCE) ||
        ...
        if (aigp->wp_index == -1) {
            ...
            if (i == Num_waypoint_lists) {
                Warning(
                    LOCATION,
                    "Unknown waypoint %s.  not found in mission file.  Killing ai goal",
                    aigp->ship_name);
    ...
    case AI_GOAL_DESTROY_SUBSYSTEM: {
        ...
        status = mission_log_get_time(LOG_SHIP_SUBSYS_DESTROYED, aigp->ship_name,
                                      ssp->system_info->name, NULL);

        break;
    }
    case AI_GOAL_DISABLE_SHIP:
        status = mission_log_get_time(LOG_SHIP_DISABLED, aigp->ship_name, NULL,
                                      NULL);
        break;
    case AI_GOAL_DISARM_SHIP:
        status = mission_log_get_time(LOG_SHIP_DISARMED, aigp->ship_name, NULL,
                                      NULL);
        break;
    ...
    case AI_GOAL_REARM_REPAIR: {
        ...
        shipnum = ship_name_lookup(aigp->ship_name);
        if (shipnum == -1) {
            status = mission_log_get_time(LOG_SHIP_DEPART, aigp->ship_name, NULL,
                                          NULL);
            if (!status) {
                status = mission_log_get_time(LOG_SHIP_DESTROYED, aigp->ship_name,
                                              NULL, NULL);
                if (status)
    ...
    case AI_GOAL_GUARD_WING:
        status = mission_log_get_time(LOG_WING_DEPART, aigp->ship_name, NULL,
                                      NULL);
        if (!status) {
            status = mission_log_get_time(LOG_WING_DESTROYED, aigp->ship_name,
                                          NULL, NULL);
            if (status)
                return_val = AI_GOAL_NOT_ACHIEVABLE;
        }
    ...
    case AI_GOAL_CHASE_WEAPON:
        ...
        if (Objects[Weapons[aigp->wp_index].objnum].signature !=
            aigp->weapon_signature)
            return AI_GOAL_NOT_ACHIEVABLE;
    ...
    if ((status == SHIP_STATUS_ARRIVED) && (aigp->ai_mode & PURGE_GOALS) &&
    ...
    if ((aigp->ai_mode == AI_GOAL_DOCK) && (status == SHIP_STATUS_ARRIVED)) {
        ...
        if (aigp->flags & AIGF_DOCKEE_NAME_VALID) {
            ...
            if (shipnum != -1) {
                ...
                aigp->dockee.index =
                    -1; // this will force code into if statement below making goal not achievable.
        }
        ...
        if (aip->ai_flags & AIF_DOCKED) {
            ...
            if (aip->dock_objnum != Ships[shipnum].objnum) {
                ...
                if (!(aigp->flags & AIGF_GOAL_ON_HOLD))
                    ai_add_goal_ship_internal(aip, AI_GOAL_UNDOCK, NULL, -1, -1,
                                              0);
    ...
    else if ((aigp->ai_mode == AI_GOAL_DESTROY_SUBSYSTEM) &&
             (status == SHIP_STATUS_ARRIVED)) {
        ...
        if (aigp->flags & AIGF_SUBSYS_NAME_VALID) {
            ...
            shipnum = ship_name_lookup(aigp->ship_name);
            if (shipnum != -1) {
                aigp->ai_submode = ship_get_subsys_index(&Ships[shipnum],
                                                         aigp->docker.name);
                aigp->flags &= ~AIGF_SUBSYS_NAME_VALID;
            }
            else {
    ...
    else if ((aigp->ai_mode == AI_GOAL_IGNORE) &&
             (status == SHIP_STATUS_ARRIVED)) {
        int shipnum;
        object *ignored;
...
ai_goal_priority_compare(const void *a, const void *b)
{
    ...
    else if ((ga->flags & AIGF_GOAL_OVERRIDE) &&
             !(gb->flags & AIGF_GOAL_OVERRIDE))
        return -1;
    else if (!(ga->flags & AIGF_GOAL_OVERRIDE) &&
             (gb->flags & AIGF_GOAL_OVERRIDE))
        return 1;
...
validate_mission_goals(int objnum, ai_info *aip)
{
    ...
    for (i = 0; i < MAX_AI_GOALS; i++) {
        ...
        if (state == AI_GOAL_NOT_KNOWN)
            aigp->flags |=
                AIGF_GOAL_ON_HOLD; // put this goal on hold until it becomes true
    }
    ...
    if ((aip->goals[0].ai_mode != AI_GOAL_NONE) &&
        (aip->goals[0].flags & AIGF_GOAL_ON_HOLD))
        aip->mode = AIM_NONE;
...
void
ai_process_mission_orders(int objnum, ai_info *aip)
{
...
*/
    ...
    case AI_GOAL_DOCK: {
        ...
        ai_dock_with_object(objp, other_obj, current_goal->priority, AIDO_DOCK,
                            current_goal->docker.index,
    ...
    case AI_GOAL_UNDOCK:
        ...
        ai_dock_with_object(objp, other_obj, current_goal->priority, AIDO_UNDOCK,
                            0, 0);
        aip->submode_start_time = Missiontime;
        break;
    ...
    case AI_GOAL_REARM_REPAIR:
        shipnum = ship_name_lookup(current_goal->ship_name);
        other_obj = &Objects[Ships[shipnum].objnum];
        ai_rearm_repair(objp, other_obj, current_goal->priority,
                        current_goal->docker.index, current_goal->dockee.index);
        break;
...
void
ai_update_goal_references(ai_goal *goals, int type, char *old_name,
                          char *new_name)
{
...
int
query_referenced_in_ai_goals(ai_goal *goals, int type, char *name)
{
    int i, mode, flag;
