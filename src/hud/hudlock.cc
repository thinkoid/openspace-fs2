...
static int Rotate_time_id =
    1; // timer id for controlling how often to rotate triangles around lock indicator
...
int Lock_gauge_half_w[GR_NUM_RESOLUTIONS] = { 17, 28 };
...
char Lock_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = { "lock1", "2_lock1" };

char Lockspin_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = { "lockspin",
                                                              "2_lockspin" };
...
void
hud_draw_diamond(int x, int y, int width, int height)
{
    Assert(height > 0);
    Assert(width > 0);
...
void
hud_show_lock_indicator(float frametime)
{
...
void
hud_lock_reset(float lock_time_scale)
{
    weapon_info *wip;
    ship_weapon *swp;
...
int
hud_lock_world_pos_in_range(vector *target_world_pos, vector *vec_to_target)
{
...
hud_lock_target_in_range()
{
    ...
    if (Player_ai->targeted_subsys != NULL) {
        vm_vec_unrotate(&target_world_pos,
                        &Player_ai->targeted_subsys->system_info->pnt,
                        &targetp->orient);
        ...
        if (Player->locking_subsys) {
            vm_vec_unrotate(&target_world_pos,
                            &Player->locking_subsys->system_info->pnt,
                            &targetp->orient);
            vm_vec_add2(&target_world_pos, &targetp->pos);
        }
        else {
...
hud_lock_on_subsys_ok()
{
    ...
    if (Player->subsys_in_view < 0) {
        in_sight = ship_subsystem_in_sight(target_objp, subsys, &View_position,
                                           &subobj_pos);
    }
    else {
...
int
hud_lock_secondary_weapon_changed(ship_weapon *swp)
{
...
// missle locking
void
hud_update_lock_indicator(float frametime)
{
    ship_weapon *swp;
    weapon_info *wip;
...
void
hud_draw_lock_triangles_old(int center_x, int center_y, int radius)
{
    ...
    for (; ang <= end_ang; ang += PI / 2.0f) {
        ...
        xpos = center_x + (float)cos(ang) *
                              (radius + Lock_triangle_height[gr_screen.res] + 2);
        ypos = center_y - (float)sin(ang) *
                              (radius + Lock_triangle_height[gr_screen.res] + 2);
...
void
hud_draw_lock_triangles(int center_x, int center_y, float frametime)
{
    if (Lock_anim.first_frame == -1) {
        hud_draw_lock_triangles_old(center_x, center_y,
                                    Lock_target_box_width[gr_screen.res] / 2);
    }
    else {
        // render the anim
        ...
        if (Lock_anim.time_elapsed < Lock_anim.total_time) {
            ...
            if ((Lock_gauge_draw_stamp < 0) ||
                ...
                // reset timestamp
                Lock_gauge_draw_stamp = timestamp(1000 /
                                                  (2 * LOCK_GAUGE_BLINK_RATE));
...
void
hud_calculate_lock_position(float frametime)
{
    ship_weapon *swp;
    weapon_info *wip;
    ...
    if (Player->target_in_lock_cone) {
        ...
        if (catching_up) {
            ...
            if ((Players[Player_num].lock_dist_to_target - last_dist_to_target) >
                2.0f) {
                catching_up = 1;
                catch_up_distance = last_dist_to_target +
                                    wip->catchup_pixel_penalty;
            }
        }
        ...
        if (delta_x != 0) {
            accumulated_x_pixels += pixels_moved_while_locking * delta_x /
                                    hypotenuse;
        }

        if (delta_y != 0) {
            accumulated_y_pixels += pixels_moved_while_locking * delta_y /
                                    hypotenuse;
        }
        ...
        if (fl_abs(accumulated_x_pixels) > 1.0f) {
            ...
            if (fl_abs(Players[Player_num].lock_indicator_x -
                       Player->current_target_sx) < fl_abs(int_portion))
                Players[Player_num].lock_indicator_x = Player->current_target_sx;
        ...
        if (fl_abs(accumulated_y_pixels) > 1.0f) {
            ...
            if (fl_abs(Players[Player_num].lock_indicator_y -
                       Player->current_target_sy) < fl_abs(int_portion))
                Players[Player_num].lock_indicator_y = Player->current_target_sy;
        ...
        if (Missile_track_loop == -1) {
            Missile_track_loop = snd_play_looping(&Snds[SND_MISSILE_TRACKING],
                                                  0.0f, -1, -1);
        }
        ...
        pixels_moved_while_degrading = 2.0f * wip->lock_pixels_per_sec *
                                       frametime;

        if (delta_x != 0)
            accumulated_x_pixels += pixels_moved_while_degrading * delta_x /
                                    hypotenuse;

        if (delta_y != 0)
            accumulated_y_pixels += pixels_moved_while_degrading * delta_y /
                                    hypotenuse;
...
void
hud_lock_update_lock_pos(object *target_objp, vector *lock_world_pos)
{
    if (Player_ai->targeted_subsys) {
        get_subsystem_world_pos(target_objp, Player_ai->targeted_subsys,
                                lock_world_pos);
        return;
    }
    ...
    if (Player->locking_on_center) {
        *lock_world_pos = target_objp->pos;
        ...
        get_subsystem_world_pos(target_objp, Player->locking_subsys,
                                lock_world_pos);
    }
}
...
void
hud_lock_get_new_lock_pos(object *target_objp, vector *lock_world_pos)
{
    ...
    if ((target_shipp) && (Ship_info[target_shipp->ship_info_index].flags &
...
void
hud_lock_determine_lock_point(vector *lock_world_pos_out)
{
    vector lock_world_pos;
    vertex lock_point;
    object *target_objp;
