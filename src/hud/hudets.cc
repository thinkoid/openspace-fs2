...
// shields
float Skill_level_weapon_energy_scale[NUM_SKILL_LEVELS] = { 10.0f, 4.0f, 2.5f,
                                                            2.0f, 1.5f };
float Skill_level_shield_energy_scale[NUM_SKILL_LEVELS] = { 4.0f, 2.0f, 1.5f,
                                                            1.25f, 1.0f };
...
int ETS_bar_h[GR_NUM_RESOLUTIONS] = { 41, 41 };
...
ets_gauge_info Ets_gauge_info_german[GR_NUM_RESOLUTIONS][3] = { { // GR_640
                                                                  'G',
                                                                  { 525, 422 },
                                                                  { 523, 380 },
                                                                  { 523, 430 },
...
ets_gauge_info Ets_gauge_info_english[GR_NUM_RESOLUTIONS][3] = { { // GR_640
                                                                   'G',
                                                                   { 525, 422 },
                                                                   { 523, 380 },
                                                                   { 523, 430 },
...
char Ets_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = { "energy1", "energy1" };
...
hud_init_ets()
{
    ...
    Ets_gauge.first_frame = bm_load_animation(Ets_fname[gr_screen.res],
                                              &Ets_gauge.num_frames);
...
//
void
update_ets(object *objp, float fl_frametime)
{
    ...
    float shield_delta;
    max_new_shield_energy = fl_frametime * MAX_SHIELD_REGEN_PER_SECOND *
                            sinfo_p->shields;
    if (objp->flags & OF_PLAYER_SHIP) {
        shield_delta = Energy_levels[ship_p->shield_recharge_index] *
                       max_new_shield_energy *
                       Skill_level_shield_energy_scale[Game_skill_level];
    }
    else {
        shield_delta = Energy_levels[ship_p->shield_recharge_index] *
                       max_new_shield_energy;
    ...
    if (y == Energy_levels[INTIAL_ENGINE_RECHARGE_INDEX]) {
        ...
        if (y < Energy_levels[INTIAL_ENGINE_RECHARGE_INDEX]) {
            ship_p->current_max_speed =
                0.5f * sinfo_p->max_speed +
                (y * (0.5f * sinfo_p->max_speed)) /
            ...
            ship_p->current_max_speed =
                sinfo_p->max_speed +
                (y - Energy_levels[INTIAL_ENGINE_RECHARGE_INDEX]) *
                    (sinfo_p->max_overclocked_speed - sinfo_p->max_speed) /
...
hud_show_ets()
{
    ...
    for (j = 0; j < 3; j++) {
        ...
        gr_printf(Ets_gauge_info[i].letter_coords[0],
                  Ets_gauge_info[i].letter_coords[1], NOX("%c"),
    ...
    for (j = 0; j < 3; j++) {
        ...
        if (index < NUM_ENERGY_LEVELS - 1) {
            ...
            GR_AABITMAP_EX(Ets_gauge.first_frame, x, y_start, w, y_end - y_start,
                           0, ETS_bar_h[gr_screen.res] - clip_h);
        }
        ...
        if (index > 0) {
            ...
            GR_AABITMAP_EX(Ets_gauge.first_frame + 1, x, y_start, w,
                           y_end - y_start, 0, clip_h);
            ...
            GR_AABITMAP_EX(Ets_gauge.first_frame + 2, x, y, w,
                           ETS_bar_h[gr_screen.res] - clip_h, 0, 0);
        }
        i++;
    }
...
void
set_default_recharge_rates(object *obj)
{
    int ship_properties;
...
void
increase_recharge_rate(object *obj, SYSTEM_TYPE ship_system)
{
...
// the other two systems.
void
decrease_recharge_rate(object *obj, SYSTEM_TYPE ship_system)
{
...
void
transfer_energy_weapon_common(object *objp, float from_field, float to_field,
                              float *from_delta, float *to_delta, float max,
                              float scale)
{
    float delta;
...
// to shield energy.
void
transfer_energy_to_shields(object *obj)
{
    ship *ship_p = &Ships[obj->instance];
    ship_info *sinfo_p = &Ship_info[ship_p->ship_info_index];
    ...
    transfer_energy_weapon_common(obj, ship_p->weapon_energy,
                                  ...
                                  &ship_p->target_shields_delta, sinfo_p->shields,
                                  0.5f);
}
...
// to weapon energy.
void
transfer_energy_to_weapons(object *obj)
{
    ship *ship_p = &Ships[obj->instance];
    ship_info *sinfo_p = &Ship_info[ship_p->ship_info_index];
    ...
    transfer_energy_weapon_common(obj, get_shield_strength(obj),
