...
int Escort_gauge_text_coords[GR_NUM_RESOLUTIONS][MAX_ESCORT_SHIPS][4][2] = {
    {
        // GR_640
        { { 489, 219 }, { 599, 212 }, { 604, 219 }, { 474, 219 } },
        { { 489, 230 }, { 599, 223 }, { 604, 230 }, { 474, 230 } },
        { { 489, 241 }, { 599, 234 }, { 604, 241 }, { 474, 241 } },
    },
    {
        // GR_1024
        { { 869, 343 }, { 973, 338 }, { 981, 343 }, { 854, 343 } },
        { { 869, 354 }, { 973, 349 }, { 981, 354 }, { 854, 354 } },
        { { 869, 365 }, { 973, 360 }, { 981, 365 }, { 854, 365 } },
    }
};

// escort gauge coords
int Escort_coords[GR_NUM_RESOLUTIONS][4][2] = { { // GR_640
                                                  { 486, 206 },
                                                  { 486, 219 },
                                                  { 486, 230 },
                                                  { 486, 241 } },
...
int Monitoring_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                   489, 208 },
                                                 { // GR_1024
                                                   869, 331 } };
...
static int
    Last_target_index; // index into Escort_gauges for last targeted via 'Next Escort Target' key
...
hud_escort_init()
{
    ...
    if (!Escort_gauges_loaded) {
        for (i = 0; i < MAX_ESCORT_SHIPS; i++) {
            Escort_gauges[i].first_frame = bm_load_animation(
                Escort_gauge_filenames[gr_screen.res][i],
                &Escort_gauges[i].num_frames);
...
int
escort_compare_func(const void *e1, const void *e2)
{
...
hud_create_complete_escort_list(escort_info *escorts, int *num_escorts)
{
    ...
    *num_escorts = 0;
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if ((Ships[objp->instance].flags & (SF_DYING | SF_DEPARTING)) ||
...
//
void
hud_setup_escort_list(int level)
{
    ...
    qsort(complete_escorts, num_complete_escorts, sizeof(escort_info),
          escort_compare_func);
...
void
merge_escort_lists(escort_info *complete_escorts, int num_complete_escorts)
{
...
void
hud_remove_ship_from_escort_index(int dead_index, int objnum)
{
    int i, count, num_complete_escorts;
    escort_info bakup_arr[MAX_ESCORT_SHIPS],
...
hud_escort_cull_list()
{
    ...
    for (i = 0; i < Num_escort_ships; i++) {
        ...
        if (Objects[objnum].flags & OF_SHOULD_BE_DEAD ||
            Ships[Objects[objnum].instance].flags & SF_HIDDEN_FROM_SENSORS) {
...
int
hud_escort_set_gauge_color(int index, int friendly)
{
    int is_flashing = 0;
    shield_hit_info *shi;
    ...
    if (!timestamp_elapsed(shi->shield_hit_timers[HULL_HIT_OFFSET])) {
        if (timestamp_elapsed(shi->shield_hit_next_flash[HULL_HIT_OFFSET])) {
            shi->shield_hit_next_flash[HULL_HIT_OFFSET] = timestamp(
                SHIELD_FLASH_INTERVAL);
            shi->shield_show_bright ^=
                (1
                 << HULL_HIT_OFFSET); // toggle between default and bright frames
        }
...
void
hud_escort_show_icon(int index, object *objp)
{
    ...
    if (Player_ship != NULL) {
        hud_escort_set_gauge_color(index,
                                   (sp->team == Player_ship->team) ? 1 : 0);
    }
    else {
    ...
    if ((sp->flags & SF_DISABLED) ||
        ...
        emp_hud_string(Escort_gauge_text_coords[gr_screen.res][index][3][0],
                       Escort_gauge_text_coords[gr_screen.res][index][3][1],
                       EG_NULL, XSTR("D", 284));
    }
    ...
    emp_hud_string(Escort_gauge_text_coords[gr_screen.res][index][0][0],
                   Escort_gauge_text_coords[gr_screen.res][index][0][1],
    ...
    emp_hud_printf(Escort_gauge_text_coords[gr_screen.res][index][2][0] + offset,
                   Escort_gauge_text_coords[gr_screen.res][index][2][1], EG_NULL,
                   "%d", screen_integrity);
}
...
hud_display_escort()
{
    ...
    GR_AABITMAP(Escort_gauges[0].first_frame, Escort_coords[gr_screen.res][0][0],
                Escort_coords[gr_screen.res][0][1]);
    gr_string(Monitoring_coords[gr_screen.res][0],
              Monitoring_coords[gr_screen.res][1], XSTR("monitoring", 285));

    if (Num_escort_ships >= 2) {
        GR_AABITMAP(Escort_gauges[1].first_frame,
                    Escort_coords[gr_screen.res][1][0],
                    Escort_coords[gr_screen.res][1][1]);
    }

    if (Num_escort_ships >= 3) {
        GR_AABITMAP(Escort_gauges[1].first_frame,
                    Escort_coords[gr_screen.res][2][0],
                    Escort_coords[gr_screen.res][2][1]);
    }

    // draw bottom of box
    GR_AABITMAP(Escort_gauges[2].first_frame, Escort_coords[gr_screen.res][3][0],
                Escort_coords[gr_screen.res][Num_escort_ships][1]);
...
void
hud_add_ship_to_escort(int objnum, int supress_feedback)
{
    escort_info complete_escorts[MAX_COMPLETE_ESCORT_LIST];
...
//
void
hud_add_remove_ship_escort(int objnum, int supress_feedback)
{
    int in_escort, i;
...
void
hud_remove_ship_from_escort(int objnum)
{
    int in_escort, i;
...
void
hud_escort_ship_hit(object *objp, int quadrant)
{
    int num, i;
    shield_hit_info *shi;
...
int
hud_escort_return_objnum(int index)
{
...
hudescort_page_in()
{
    ...
    for (i = 0; i < MAX_ESCORT_SHIPS; i++) {
        bm_page_in_aabitmap(Escort_gauges[i].first_frame,
                            Escort_gauges[i].num_frames);
    }
}
