...
#include <playerman/player.hh> // for MAX_PLAYERS
...
int Wenergy_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                416, 265, 60, 60 },
                                              { // GR_1024
                                                666, 424, 86, 96 } };
...
char Energy_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = { "energy2",
                                                            "2_energy2" };
int Weapon_energy_text_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                           439, 318 },
                                                         { // GR_1024
                                                           708, 509 } };
...
int Toggle_target_gauge_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                          ...
                                                          { // GR_1024
                                                            960, 648 } };
int Toggle_speed_gauge_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                           577, 404 },
                                                         { // GR_1024
                                                           960, 672 } };
char Toggle_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = { "toggle1",
                                                            "toggle1" };
...
static int Hud_toggle_coords[GR_NUM_RESOLUTIONS][4][2] = { { // GR_640
                                                             { 590, 382 },
                                                             { 584, 390 },
                                                             { 590, 406 },
                                                             { 587, 414 } },
...
int Weapon_gauge_secondary_coords[GR_NUM_RESOLUTIONS][5][2] = {
    {
        ...
        { 497,
          317 }, // middle of gauge, 2 secondaries AND middle of gauge, 3 secondaries
        { 497,
          326 }, // bottom of gauge, 2 secondaries AND middle of gauge, 3 secondaries
    ...
    },
    {
        ...
        { 880,
          569 }, // middle of gauge, 2 secondaries AND middle of gauge, 3 secondaries
        { 880,
          578 }, // bottom of gauge, 2 secondaries AND middle of gauge, 3 secondaries
        { 880, 587 } // bottom of gauge, 3 secondaries
    }
};
int Weapon_title_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                     518, 275 },
                                                   { // GR_1024
                                                     901, 527 } };
...
int
hud_target_invalid_awacs(object *objp)
{
...
ship_subsys *
advance_subsys(ship_subsys *cur, int next_flag)
{
    if (next_flag) {
        return GET_NEXT(cur);
    }
...
void
hud_maybe_set_sorted_turret_subsys(ship *shipp)
{
...
void
hud_stuff_reticle_list(reticle_list *rl, object *objp, float measure,
                       int dot_flag)
{
    if (dot_flag) {
        rl->dot = measure;
...
hud_reticle_list_update(object *objp, float measure, int dot_flag)
{
    ...
    for (rl = GET_FIRST(&Reticle_cur_list); rl != END_OF_LIST(&Reticle_cur_list);
         rl = GET_NEXT(rl)) {
        if (rl->objp == objp)
            return;
    }
...
hud_reticle_pick_target()
{
    ...
    for (cur_rl = GET_FIRST(&Reticle_cur_list);
         cur_rl != END_OF_LIST(&Reticle_cur_list); cur_rl = GET_NEXT(cur_rl)) {
...
void
hud_make_shader(shader *sh, int r, int g, int b, float dimmer = 1000.0f)
{
    float rf, gf, bf, cf;
...
hud_weapons_init()
{
    ...
    if (!Weapon_gauges_loaded) {
        for (i = 0; i < NUM_WEAPON_GAUGES; i++) {
            Weapon_gauges[i].first_frame = bm_load_animation(
                Weapon_gauge_fnames[gr_screen.res][i],
                &Weapon_gauges[i].num_frames);
...
hud_init_targeting()
{
    ...
    if (!Lead_indicator_gauge_loaded) {
        Lead_indicator_gauge.first_frame = bm_load_animation(
            Lead_fname[gr_screen.res], &Lead_indicator_gauge.num_frames);
    ...
    if (!Energy_bar_gauges_loaded) {
        Energy_bar_gauges.first_frame = bm_load_animation(
            Energy_fname[gr_screen.res], &Energy_bar_gauges.num_frames);
    ...
    if (!Toggle_gauge_loaded) {
        Toggle_gauge.first_frame = bm_load_animation(Toggle_fname[gr_screen.res],
                                                     &Toggle_gauge.num_frames);
    ...
    if (!Cmeasure_gauge_loaded) {
        Cmeasure_gauge.first_frame = bm_load_animation(
            Cm_fname[gr_screen.res], &Cmeasure_gauge.num_frames);
...
hud_target_subobject_common(int next_flag)
{
    ...
    if (subsys_to_target == NULL) {
        ...
        set_targeted_subsys(Player_ai, subsys_to_target,
...
object *
advance_fb(object *objp, int next_flag)
{
    if (next_flag)
...
void
hud_target_common(int team, int next_flag)
{
    ...
    for (A = start2; A != start; A = advance_fb(A, next_flag)) {
        ...
        if (A == Player_obj || (A->type != OBJ_SHIP && A->type != OBJ_WEAPON &&
        ...
        if (A->type == OBJ_WEAPON) {
            if (!(Weapon_info[Weapons[A->instance].weapon_info_index].wi_flags &
                  WIF_BOMB)) {
                continue;
            }
        }
...
void
hud_target_next(int team)
{
    hud_target_common(team, 1);
}

void
hud_target_prev(int team)
{
...
//
missile_obj *
advance_missile_obj(missile_obj *mo, int next_flag)
{
    if (next_flag) {
        return GET_NEXT(mo);
    }
...
ship_obj *
advance_ship(ship_obj *so, int next_flag)
{
    if (next_flag) {
        return GET_NEXT(so);
    }
...
void
hud_target_missile(object *source_obj, int next_flag)
{
    ...
    for (mo = start; mo != end; mo = advance_missile_obj(mo, next_flag)) {
        ...
        if ((obj_team(A) == Player_ship->team) &&
    ...
    if (!target_found) {
        ...
        for (so = advance_ship(start, next_flag); so != start;
            ...
            // only allow targeting of hostile bombs
            if ((obj_team(ship_obj) == Player_ship->team) &&
                (Player_ship->team != TEAM_TRAITOR)) {
                continue;
            }
...
int
hud_target_ship_can_be_scanned(ship *shipp)
{
    ship_info *sip;
...
void
hud_target_uninspected_cargo(int next_flag)
{
    object *A, *start, *start2;
    ship *shipp;
    int target_found = 0;
...
int
sort_turret_func(const void *e1, const void *e2)
{
...
void
hud_target_live_turret(int next_flag, int auto_advance, int only_player_target)
{
    ship_subsys *A;
    ...
    if (Player_ai->targeted_subsys != NULL) {
        if (Player_ai->targeted_subsys->system_info->type == SUBSYSTEM_TURRET) {
            if (Player_ai->targeted_subsys->system_info->turret_weapon_type >=
                0) {
    ...
    int get_closest_turret = (auto_advance || !last_subsys_turret ||
    ...
    for (A = GET_FIRST(&target_shipp->subsys_list);
        ...
        if (A->system_info->type == SUBSYSTEM_TURRET) {
            ...
            if ((A->current_hits > 0) &&
                (A->system_info->turret_weapon_type >= 0)) {
                if (!only_player_target ||
                    ...
                    distance = vm_vec_normalized_dir(&vec_to_subsys, &gsubpos,
                                                     &View_position);

                    // check if facing and in view
                    int facing = ship_subsystem_in_sight(objp, A, &View_position,
                                                         &gsubpos, 0);

                    if (!auto_advance && get_closest_turret &&
                        !only_player_target) {
                        // if within 3 degrees and not previous subsys, use subsys in front
                        dot = vm_vec_dotprod(&vec_to_subsys,
                                             &Player_obj->orient.fvec);
                    ...
                    if (Weapon_info[A->system_info->turret_weapon_type].wi_flags &
                        WIF_BEAM) {
                        ent[num_live_turrets].type = TYPE_FACING_BEAM;
                    }
                    else if (Weapon_info[A->system_info->turret_weapon_type]
                                 .wi_flags &
                             WIF_FLAK) {
                        ent[num_live_turrets].type = TYPE_FACING_FLAK;
                    }
                    else {
                        if (Weapon_info[A->system_info->turret_weapon_type]
                                .subtype == WP_MISSILE) {
...
{
    return ((TEAM_FRIENDLY | TEAM_NEUTRAL | TEAM_HOSTILE) & ~team_mask) |
           TEAM_TRAITOR;
}
...
float
hud_find_target_distance(object *targetee, object *targeter)
{
    vector tmp_pnt;
    ...
    if (model_num > -1) {
        dist = model_find_closest_point(&tmp_pnt, model_num, -1,
                                        &targetee->orient, &targetee->pos,
                                        &targeter->pos);
    }
    else {
...
void
evaluate_ship_as_closest_target(esct *esct)
{
    ...
    if ((esct->shipp->objnum == OBJ_INDEX(Player_obj)) ||
        (esct->shipp->flags & TARGET_SHIP_IGNORE_FLAGS)) {
        return;
    }
    ...
    if (esct->filter) {
        if (!(Ship_info[esct->shipp->ship_info_index].flags &
    ...
    if (Ship_info[esct->shipp->ship_info_index].flags &
        (SIF_BIG_SHIP | SIF_HUGE_SHIP)) {
        for (ss = GET_FIRST(&esct->shipp->subsys_list);
             ss != END_OF_LIST(&esct->shipp->subsys_list); ss = GET_NEXT(ss)) {
            if ((ss->system_info->type == SUBSYSTEM_TURRET) &&
                (ss->current_hits > 0)) {
                if (esct->check_all_turrets ||
                    (ss->turret_enemy_objnum == esct->attacked_objnum)) {
                    turret_is_attacking = 1;
                    esct->check_nearest_turret = TRUE;

                    if (!esct->turret_attacking_target ||
                        ...
                        new_distance = vm_vec_dist_quick(&gsubpos,
                                                         &Player_obj->pos);
...
int
hud_target_closest(int team, int attacked_objnum, int play_fail_snd, int filter,
                   int get_closest_turret_attacking_player)
{
    ...
    if ((attacked_objnum >= 0) && (attacked_objnum != player_obj_index)) {
        ...
        if (Player_ai->target_objnum == -1) {
            ...
            if (!(Ships[Objects[attacked_objnum].instance].flags &
                  TARGET_SHIP_IGNORE_FLAGS)) {
                goto Target_closest_done;
            }
        }
    }
    ...
    if (target_found) {
        ...
        if (check_nearest_turret) {
            ...
            if (ss == NULL || get_closest_turret_attacking_player) {
                ...
                Ships[nearest_obj->instance].last_targeted_subobject[Player_num] =
                    Player_ai->targeted_subsys;
            }
        }
        else {
...
int
object_targetable_in_reticle(object *target_objp)
{
    int obj_type;
...
hud_target_subsystem_in_reticle()
{
    ...
    if (nearest_subsys != NULL) {
        ...
        HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                           XSTR("Targeting subsystem %s.", 323),
                           ...
                           matrix *from_orientp)
{
    ...
    y1 = (float)sin(dot_product) *
         (Outer_circle_radius[gr_screen.res] - T_OFFSET_FROM_CIRCLE);
    x1 = (float)cos(dot_product) *
         (Outer_circle_radius[gr_screen.res] - T_OFFSET_FROM_CIRCLE);
    ...
    y2 = (float)sin(dot_product) *
         (Outer_circle_radius[gr_screen.res] - T_OFFSET_FROM_CIRCLE - T_LENGTH);
    x2 = (float)cos(dot_product) *
         (Outer_circle_radius[gr_screen.res] - T_OFFSET_FROM_CIRCLE - T_LENGTH);
    ...
    gr_line(fl2i(x1), fl2i(y1), fl2i(x2),
            fl2i(y2)); // part of T pointing towards center
}

void
hud_tri(float x1, float y1, float x2, float y2, float x3, float y3)
{
    int i;
...
void
hud_tri_empty(float x1, float y1, float x2, float y2, float x3, float y3)
{
...
void
hud_render_tail_missile_triangle(float ang, float xpos, float ypos,
                                 float cur_dist, int draw_solid, int draw_inside)
{
    float x1 = 0.0f;
    float x2 = 0.0f;
    float y1 = 0.0f;
...
void
hud_render_split_missile_triangle(float ang, float xpos, float ypos,
                                  float cur_dist, int draw_solid, int draw_inside)
{
    ...
    if (cur_dist < Min_warning_missile_dist) {
        ...
        triangle_sep = Max_offscreen_tri_seperation[gr_screen.res] +
                       Max_front_seperation[gr_screen.res];
    }
    else {
        triangle_sep = (cur_dist / Max_warning_missile_dist) *
    ...
    if (triangle_sep == 0) {
        ...
        gr_line(fl2i(x2 + 0.5f), fl2i(y2 + 0.5f), fl2i(x5 + 0.5f),
                fl2i(y5 + 0.5f));
    }
}
                    ...
                    int split_tri)
{
    ...
    if (hostile_vertex.codes == 0) { // on screen
        ...
        if (!(hostile_vertex.flags & PF_OVERFLOW)) { // make sure point projected
            ...
            mag_squared =
                (projected_x - Hud_reticle_center[gr_screen.res][0]) *
                    (projected_x - Hud_reticle_center[gr_screen.res][0]) +
                (projected_y - Hud_reticle_center[gr_screen.res][1]) *
                    (projected_y - Hud_reticle_center[gr_screen.res][1]);

            if (mag_squared < Outer_circle_radius[gr_screen.res] *
                                  Outer_circle_radius[gr_screen.res]) {
                if (!show_interior) {
                    return;
                }
                else {
    ...
    if (draw_inside) {
        xpos = Hud_reticle_center[gr_screen.res][0] +
               cos_ang * (Outer_circle_radius[gr_screen.res] - 7);
        ypos = Hud_reticle_center[gr_screen.res][1] -
               sin_ang * (Outer_circle_radius[gr_screen.res] - 7);
    }
    else {
        xpos = Hud_reticle_center[gr_screen.res][0] +
               cos_ang * (Outer_circle_radius[gr_screen.res] + 4);
        ypos = Hud_reticle_center[gr_screen.res][1] -
               sin_ang * (Outer_circle_radius[gr_screen.res] + 4);
    }
...
hud_show_homing_missiles()
{
    ...
    for (mo = GET_NEXT(&Missile_obj_list); mo != END_OF_LIST(&Missile_obj_list);
        ...
        if (wp->homing_object == Player_obj) {
            hud_render_triangle(&A->pos,
                                Weapon_info[wp->weapon_info_index].wi_flags &
                                    WIF_HOMING_ASPECT,
                                1, 1);
            ...
            if (dist < nearest_dist) {
                nearest_dist = dist;
                if (Weapon_info[wp->weapon_info_index].wi_flags &
                    WIF_HOMING_ASPECT) {
                    closest_is_aspect = 1;
                }
                else {
    ...
    if (nearest_dist < Homing_beep.max_cycle_dist) {
        ...
        if ((delta_time * 1000) > cycle_time) {
            ...
            if (closest_is_aspect) {
                Homing_beep.snd_handle = snd_play(
                    &Snds[SND_PROXIMITY_ASPECT_WARNING]);
            }
            else {
...
hud_show_remote_detonate_missile()
{
    ...
    for (mo = GET_FIRST(&Missile_obj_list); mo != END_OF_LIST(&Missile_obj_list);
        ...
        if ((Player_obj != NULL) &&
            ...
            if (Weapon_info[Weapons[mobjp->instance].weapon_info_index].wi_flags &
                WIF_REMOTE) {
                // get distance
                ...
                if (!(target_point.flags &
                    ...
                    case OBJ_WEAPON:
                        modelnum =
                            Weapon_info[Weapons[mobjp->instance].weapon_info_index]
                                .model_num;
                        bound_rval = model_find_2d_bound_min(modelnum,
                                                             &mobjp->orient,
                                                             &mobjp->pos, &x1,
                                                             &y1, &x2, &y2);
                        break;
                    ...
                    if (bound_rval == 0) {
                        ...
                        draw_bounding_brackets(x1 - 5, y1 - 5, x2 + 5, y2 + 5, 0,
                                               0, distance, OBJ_INDEX(mobjp));
                    }
...
hud_show_message_sender()
{
    ...
    if (!(target_point.flags & PF_OVERFLOW)) { // make sure point projected
        ...
        case OBJ_SHIP:
            ...
            bound_rval = model_find_2d_bound_min(
        ...
        if (target_point.codes != 0) { // target center is not on screen
            ...
            if ((OBJ_INDEX(targetp) != Player_ai->target_objnum) ||
                ...
                if (hud_sensors_ok(Player_ship, 0)) {
                    ...
                    hud_draw_offscreen_indicator(&target_point, &targetp->pos,
                                                 dist);
                }
            }
        }
...
hud_prune_hotkeys()
{
    ...
    for (i = 0; i < MAX_KEYED_TARGETS; i++) {
        ...
        while (hitem != END_OF_LIST(plist)) {
            ...
            if (remove_item || (objp->flags & OF_SHOULD_BE_DEAD) ||
                (sp->flags & (SF_DEPARTING | SF_DYING))) {
                if (sp != NULL) {
...
hud_show_selection_set()
{
    ...
    for (hitem = GET_FIRST(plist); hitem != END_OF_LIST(plist);
        ...
        if (!(target_point.flags & PF_OVERFLOW)) { // make sure point projected
            ...
            case OBJ_SHIP:
                modelnum = target_shipp->modelnum;
                bound_rval = model_find_2d_bound_min(modelnum, &targetp->orient,
                                                     &targetp->pos, &x1, &y1, &x2,
                                                     &y2);
                break;
...
void
hud_show_brackets(object *targetp, vertex *projected_v)
{
    int x1, x2, y1, y2;
    int draw_box = TRUE;
    int team, bound_rc;
    ...
    if (Player->target_is_dying <= 0) {
        ...
        case OBJ_WEAPON:
            Assert(Weapon_info[Weapons[targetp->instance].weapon_info_index]
                       .subtype == WP_MISSILE);
        ...
        case OBJ_ASTEROID: {
            ...
            modelnum = Asteroid_info[Asteroids[targetp->instance].type]
                           .model_num[subtype];
            bound_rc = model_find_2d_bound_min(modelnum, &targetp->orient,
                                               &targetp->pos, &x1, &y1, &x2, &y2);
        } break;
        ...
        if (draw_box == TRUE) {
            ...
            gr_set_color_fast(&IFF_colors[color][1]);
            draw_bounding_brackets(x1 - 5, y1 - 5, x2 + 5, y2 + 5, 0, 0, distance,
                                   OBJ_INDEX(targetp));
        }
...
void
hud_update_target_in_reticle(vertex *projected_v)
{
    float mag_squared;
    mag_squared = (projected_v->sx - Hud_reticle_center[gr_screen.res][0]) *
                      (projected_v->sx - Hud_reticle_center[gr_screen.res][0]) +
                  (projected_v->sy - Hud_reticle_center[gr_screen.res][1]) *
                      (projected_v->sy - Hud_reticle_center[gr_screen.res][1]);

    if (mag_squared <
        Outer_circle_radius[gr_screen.res] * Outer_circle_radius[gr_screen.res]) {
...
hud_show_targeting_gauges(float frametime, int in_cockpit)
{
    ...
    if (Player_ai->targeted_subsys != NULL) {
        ...
        Player_ai->current_target_distance = vm_vec_dist_quick(&target_pos,
                                                               &Player_obj->pos);
    }
    else {
        target_pos = targetp->pos;

        Player_ai->current_target_distance = hud_find_target_distance(targetp,
                                                                      Player_obj);
    }
    ...
    if (in_cockpit) {
        ...
        if ((hud_gauge_active(HUD_ORIENTATION_TEE)) &&
        ...
        if (target_point.codes != 0) { // target center is not on screen
            ...
            if (Message_shipnum != Objects[Player_ai->target_objnum].instance) {
                if (hud_gauge_maybe_flash(HUD_OFFSCREEN_INDICATOR) != 1) {
                    ...
                    dist = hud_find_target_distance(targetp, Player_obj);
                    hud_draw_offscreen_indicator(&target_point, &target_pos,
                                                 dist);
                }
            }
        }
...
//
void
polish_predicted_target_pos(vector *enemy_pos, vector *predicted_enemy_pos,
                            float dist_to_enemy, vector *last_delta_vec,
                            int num_polish_steps)
{
    ...
    Assert(shipp->weapons.current_primary_bank <
           shipp->weapons.num_primary_banks);
    weapon_info *wip = &Weapon_info[shipp->weapons.primary_bank_weapons
    ...
    for (iteration = 0; iteration < num_polish_steps; iteration++) {
        ...
        vm_vec_scale_add(predicted_enemy_pos, enemy_pos,
                         ...
                         time_to_enemy);
        vm_vec_sub(last_delta_vec, predicted_enemy_pos,
...
void
hud_show_lead_indicator(vector *target_world_pos)
{
    ...
    if (targetp->type == OBJ_WEAPON) {
        if (!(Weapon_info[Weapons[targetp->instance].weapon_info_index].wi_flags &
              WIF_BOMB)) {
            return;
        }
    }
    ...
    frame_offset = hudtarget_lead_indicator_pick_frame(prange, srange,
                                                       dist_to_target);
    if (frame_offset < 0) {
        return;
    }
    ...
    if (vm_vec_mag_quick(&targetp->phys_info.vel) < 0.1f) // Find distance!
        ...
        vm_vec_add(&Players[Player_num].lead_target_pos, target_world_pos,
                   &target_moving_direction);
        polish_predicted_target_pos(target_world_pos,
                                    &Players[Player_num].lead_target_pos,
    ...
    if (lead_target_vertex.codes == 0) { // on screen
        ...
        if (!(lead_target_vertex.flags & PF_OVERFLOW)) {
            ...
            if (indicator_frame >= 0) {
                GR_AABITMAP(indicator_frame,
                            fl2i(lead_target_vertex.sx -
                                 Lead_indicator_half[gr_screen.res][0]),
                            fl2i(lead_target_vertex.sy -
                                 Lead_indicator_half[gr_screen.res][1]));
            }
        }
    }
}
...
//
void
hud_cease_subsystem_targeting(int print_message)
{
    int ship_index;
...
hud_cease_targeting()
{
    ...
    HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                       XSTR("Deactivating targeting system", 325));
    hud_lock_reset();
}
...
//
void
hud_restore_subsystem_target(ship *shipp)
{
...
//
vector *
get_subsystem_world_pos(object *parent_obj, ship_subsys *subsys,
                        vector *world_pos)
{
    if (subsys == NULL) {
...
// text on the HUD.
void
hud_maybe_flash_docking_text(object *objp)
{
    ai_info *aip;
    ...
    if ((Player_ai->target_objnum == docker_objnum) ||
        (Player_ai->target_objnum == dockee_objnum)) {
...
hud_target_change_check()
{
    ...
    if (Player_ai->last_target != Player_ai->target_objnum) {
        ...
        else if (Player_ai->current_target_distance >
                 Player_ai->last_dist + 0.01) {
...
//
void
hud_draw_offscreen_indicator(vertex *target_point, vector *tpos, float distance)
{
    char buf[32];
    int w = 0, h = 0;
    ...
    if (triangle_sep > Max_offscreen_tri_seperation[gr_screen.res] +
                           Max_front_seperation[gr_screen.res]) {
        triangle_sep = Max_offscreen_tri_seperation[gr_screen.res] +
                       Max_front_seperation[gr_screen.res];
    }
...
hud_show_afterburner_gauge()
{
    ...
    if (clip_h > 0) {
        GR_AABITMAP_EX(Energy_bar_gauges.first_frame,
                       Aburn_coords[gr_screen.res][0],
                       Aburn_coords[gr_screen.res][1], w, clip_h, 0, 0);
    }

    if (clip_h <= Aburn_coords[gr_screen.res][3]) {
        GR_AABITMAP_EX(Energy_bar_gauges.first_frame + 1,
                       Aburn_coords[gr_screen.res][0],
                       Aburn_coords[gr_screen.res][1] + clip_h, w, h - clip_h, 0,
                       clip_h);
    }
}
...
hud_show_weapon_energy_gauge()
{
    ...
    percent_left = Player_ship->weapon_energy /
    ...
    if (percent_left <= 0.3) {
        ...
        hud_num_make_mono(buf);
        gr_string(Weapon_energy_text_coords[gr_screen.res][0],
                  Weapon_energy_text_coords[gr_screen.res][1], buf);
    }
    ...
    clip_h = fl2i((1.0f - percent_left) * Wenergy_coords[gr_screen.res][3] +
                  0.5f);
    ...
    if (clip_h > 0) {
        GR_AABITMAP_EX(Energy_bar_gauges.first_frame + 2,
                       Wenergy_coords[gr_screen.res][0],
                       Wenergy_coords[gr_screen.res][1], w, clip_h, 0, 0);
    }

    if (clip_h <= Wenergy_coords[gr_screen.res][3]) {
        GR_AABITMAP_EX(Energy_bar_gauges.first_frame + 3,
                       Wenergy_coords[gr_screen.res][0],
                       Wenergy_coords[gr_screen.res][1] + clip_h, w, h - clip_h,
                       0, clip_h);
    }
...
hud_show_target_triangle_indicator(vertex *projected_v)
{
    ...
    if ((hud_gauge_active(HUD_TARGET_TRIANGLE)) && !Player->target_is_dying &&
        !Target_in_reticle) {
        ...
        xpos = Hud_reticle_center[gr_screen.res][0] +
               (float)cos(ang) * (Outer_circle_radius[gr_screen.res] + 4);
        ypos = Hud_reticle_center[gr_screen.res][1] -
               (float)sin(ang) * (Outer_circle_radius[gr_screen.res] + 4);
...
void
hud_show_secondary_weapon(int count, ship_weapon *sw, int dual_fire)
{
    char ammo_str[32];
    ...
    for (i = 0; i < count; i++) {
        ...
        if (sw->current_secondary_bank == i) {
            emp_hud_printf(Weapon_sunlinked_x[gr_screen.res],
                           Weapon_secondary_y[gr_screen.res][i] - np * 12,
                           EG_NULL, "%c", Lcl_special_chars + 2);

            if (dual_fire) {
                emp_hud_printf(Weapon_slinked_x[gr_screen.res],
                               Weapon_secondary_y[gr_screen.res][i] - np * 12,
                               EG_NULL, "%c", Lcl_special_chars + 2);
            }

            emp_hud_string(Weapon_secondary_name_x[gr_screen.res],
                           Weapon_secondary_y[gr_screen.res][i] - np * 12,
            ...
            if ((sw->secondary_bank_ammo[i] > 0) &&
                ...
                if ((ms_till_fire >= 500) &&
                    ...
                    emp_hud_printf(Weapon_secondary_reload_x[gr_screen.res],
                                   Weapon_secondary_y[gr_screen.res][i] - np * 12,
                                   EG_NULL, "%d",
        ...
        emp_hud_string(Weapon_secondary_ammo_x[gr_screen.res] - w,
                       Weapon_secondary_y[gr_screen.res][i] - np * 12, EG_NULL,
                       ammo_str);
...
void
hud_start_flash_weapon(int index)
{
...
void
hud_maybe_flash_weapon(int index)
{
    if (index >= MAX_WEAPON_FLASH_LINES) {
        Int3(); // Get Alan
        return;
...
hud_show_cmeasure_gague()
{
    ...
    // blit the background
    GR_AABITMAP(Cmeasure_gauge.first_frame, Cm_coords[gr_screen.res][0],
                Cm_coords[gr_screen.res][1]);

    // blit text
    gr_string(Cm_text_coords[gr_screen.res][0], Cm_text_coords[gr_screen.res][1],
              XSTR("cm.", 327));
    if (!Player_ship) {
        Int3(); // player ship doesn't exist?
        return;
    }
    gr_printf(Cm_text_val_coords[gr_screen.res][0],
              Cm_text_val_coords[gr_screen.res][1], NOX("%02d"),
...
hud_show_weapons()
{
    ...
    GR_AABITMAP(Weapon_gauges[0].first_frame,
                Weapon_gauge_primary_coords[gr_screen.res][0][0],
                Weapon_gauge_primary_coords[gr_screen.res][0][1]);

    emp_hud_string(Weapon_title_coords[gr_screen.res][0],
                   Weapon_title_coords[gr_screen.res][1], EG_WEAPON_TITLE,
                   XSTR("weapons", 328));

    switch (np) {
    case 0:
        // draw bottom of border
        GR_AABITMAP(Weapon_gauges[2].first_frame,
                    Weapon_gauge_primary_coords[gr_screen.res][1][0],
                    Weapon_gauge_primary_coords[gr_screen.res][1][1]);

        emp_hud_string(Weapon_pname_coords[gr_screen.res][0][0],
                       Weapon_pname_coords[gr_screen.res][0][1], EG_WEAPON_P1,
                       XSTR("<none>", 329));
    ...
    case 1:
        ...
        emp_hud_printf(Weapon_plink_coords[gr_screen.res][0][0],
                       Weapon_plink_coords[gr_screen.res][0][1], EG_NULL, "%c",
                       Lcl_special_chars + 2);
        emp_hud_printf(Weapon_pname_coords[gr_screen.res][0][0],
                       Weapon_pname_coords[gr_screen.res][0][1], EG_WEAPON_P2,
                       "%s", name);
        break;

    case 2:
        // draw border to accomodate second primary weapon
        GR_AABITMAP(Weapon_gauges[1].first_frame,
                    Weapon_gauge_primary_coords[gr_screen.res][1][0],
                    Weapon_gauge_primary_coords[gr_screen.res][1][1]);

        // draw bottom of border
        GR_AABITMAP(Weapon_gauges[2].first_frame,
                    Weapon_gauge_primary_coords[gr_screen.res][2][0],
                    Weapon_gauge_primary_coords[gr_screen.res][2][1]);
        ...
        if ((sw->current_primary_bank == 0) ||
            (Player_ship->flags & SF_PRIMARY_LINKED)) {
            emp_hud_printf(Weapon_plink_coords[gr_screen.res][0][0],
                           Weapon_plink_coords[gr_screen.res][0][1], EG_NULL,
                           "%c", Lcl_special_chars + 2);
        }
        emp_hud_printf(Weapon_pname_coords[gr_screen.res][0][0],
                       Weapon_pname_coords[gr_screen.res][0][1], EG_WEAPON_P1,
                       "%s", name);
        ...
        if (sw->current_primary_bank == 1 ||
            (Player_ship->flags & SF_PRIMARY_LINKED)) {
            emp_hud_printf(Weapon_plink_coords[gr_screen.res][1][0],
                           Weapon_plink_coords[gr_screen.res][1][1], EG_NULL,
                           "%c", Lcl_special_chars + 2);
        }
        emp_hud_printf(Weapon_pname_coords[gr_screen.res][1][0],
                       Weapon_pname_coords[gr_screen.res][1][1], EG_WEAPON_P2,
                       "%s", name);
        np = 0;
        break;
    ...
    case 0:
        ...
        GR_AABITMAP(Weapon_gauges[4].first_frame,
                    Weapon_gauge_secondary_coords[gr_screen.res][0][0],
                    Weapon_gauge_secondary_coords[gr_screen.res][0][1] - 12 * np -
                        1);

        emp_hud_string(Weapon_pname_coords[gr_screen.res][0][0],
                       Weapon_secondary_y[gr_screen.res][0] - np * 12,
    ...
    case 1:
        // draw the bottom of the secondary weapons
        GR_AABITMAP(Weapon_gauges[4].first_frame,
                    Weapon_gauge_secondary_coords[gr_screen.res][1][0],
                    Weapon_gauge_secondary_coords[gr_screen.res][1][1] - 12 * np -
                        1);

        hud_show_secondary_weapon(1, sw,
                                  Player_ship->flags & SF_SECONDARY_DUAL_FIRE);
        break;

    case 2:
        // draw the middle border, only present when there are 2 or more secondaries
        GR_AABITMAP(Weapon_gauges[3].first_frame,
                    Weapon_gauge_secondary_coords[gr_screen.res][2][0],
                    Weapon_gauge_secondary_coords[gr_screen.res][2][1] - np * 12);

        // draw the bottom of the secondary weapons
        GR_AABITMAP(Weapon_gauges[4].first_frame,
                    Weapon_gauge_secondary_coords[gr_screen.res][3][0],
                    Weapon_gauge_secondary_coords[gr_screen.res][3][1] - 12 * np);

        hud_show_secondary_weapon(2, sw,
                                  Player_ship->flags & SF_SECONDARY_DUAL_FIRE);
        break;
    ...
    case 3:
        ...
        GR_AABITMAP(Weapon_gauges[3].first_frame,
                    Weapon_gauge_secondary_coords[gr_screen.res][3][0],
                    Weapon_gauge_secondary_coords[gr_screen.res][3][1] - np * 12);

        // draw the bottom of the secondary weapons
        GR_AABITMAP(Weapon_gauges[4].first_frame,
                    Weapon_gauge_secondary_coords[gr_screen.res][4][0],
                    Weapon_gauge_secondary_coords[gr_screen.res][4][1] - 12 * np);

        hud_show_secondary_weapon(3, sw,
                                  Player_ship->flags & SF_SECONDARY_DUAL_FIRE);
        break;
...
hud_sensors_ok(ship *sp, int show_msg)
{
    ...
    if ((sensors_str < MIN_SENSOR_STR_TO_TARGET) ||
        ...
        if (show_msg) {
            HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                               XSTR("Targeting is disabled due to sensors damage",
                                    330));
            snd_play(&Snds[SND_TARGET_FAIL]);
        }
        return 0;
...
int
hud_communications_state(ship *sp, int show_msg)
{
    float str;
    ...
    if ((str <= 0.01) || ship_subsys_disrupted(sp, SUBSYSTEM_COMMUNICATION)) {
        if (show_msg) {
            HUD_sourced_printf(
                HUD_SOURCE_HIDDEN,
                XSTR("Messaging is restricted due to communications damage",
                     331));
        }
...
void
hud_target_next_list(int hostile, int next_flag)
{
    ...
    cur_dist = hud_find_target_distance(&Objects[Player_ai->target_objnum],
                                        Player_obj);
...
hud_auto_target_icon()
{
    ...
    GR_AABITMAP(Toggle_gauge.first_frame + frame_offset,
                Toggle_target_gauge_coords[gr_screen.res][0],
    ...
    gr_string(Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_AUTOT][0],
              Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_AUTOT][1],
              XSTR("auto", 1463));
    gr_string(Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_TARGET][0],
              Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_TARGET][1],
              XSTR("target", 1465));
}
...
hud_auto_speed_match_icon()
{
    ...
    GR_AABITMAP(Toggle_gauge.first_frame + frame_offset,
                Toggle_speed_gauge_coords[gr_screen.res][0],
    ...
    gr_string(Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_AUTOS][0],
              Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_AUTOS][1],
              XSTR("auto", 1463));
    gr_string(Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_SPEED][0],
              Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_SPEED][1],
              XSTR("speed", 1464));
}
...
hud_target_closest_repair_ship(int goal_objnum)
{
    ...
    if (nearest_obj != &obj_used_list) {
        ...
        if (goal_objnum == -1) {
            HUD_sourced_printf(
                HUD_SOURCE_HIDDEN,
                XSTR("No support ships in area.  Use messaging to call one in.",
                     332));
        }
        rval = 0;
    }
...
hud_target_toggle_hidden_from_sensors()
{
    if (TARGET_SHIP_IGNORE_FLAGS & SF_HIDDEN_FROM_SENSORS) {
        ...
        HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                           NOX("Target hiding from sensors disabled"));
    }
    else {
        TARGET_SHIP_IGNORE_FLAGS |= SF_HIDDEN_FROM_SENSORS;
        HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                           NOX("Target hiding from sensors enabled"));
    }
}
...
void
hud_target_uninspected_object(int next_flag)
{
    object *A, *min_obj, *max_obj, *nearest_obj;
    ship *shipp;
    ship_obj *so;
...
int
hud_target_last_transmit_newest()
{
    int latest_slot;
...
hud_target_last_transmit()
{
            ...
            if (Player_ai->target_objnum == transmit_objnum) {
                ...
                if (Transmit_target_list[transmit_index].objsig ==
                    Objects[Transmit_target_list[transmit_index].objnum]
                        .signature) {
                    if (!(Ships[Objects[transmit_objnum].instance].flags &
                          TARGET_SHIP_IGNORE_FLAGS)) {
                        Transmit_target_current_slot = transmit_index - 1;
                        if (Transmit_target_current_slot < 0) {
                            Transmit_target_current_slot = MAX_TRANSMIT_TARGETS -
                                                           1;
                        }
                        break;
                    }
    ...
    if ((targeted_objnum >= 0) && (targeted_objnum < MAX_OBJECTS)) {
        ...
        hud_restore_subsystem_target(
            &Ships[Objects[Transmit_target_list[transmit_index].objnum].instance]);
    }
}

// called externally to add a message sender to the list
void
hud_target_last_transmit_add(int ship_num)
{
    object *ship_objp;
    int ship_objnum;
...
hudtarget_page_in()
{
    ...
    for (i = 0; i < NUM_WEAPON_GAUGES; i++) {
        bm_page_in_aabitmap(Weapon_gauges[i].first_frame,
                            Weapon_gauges[i].num_frames);
    }
    bm_page_in_aabitmap(Lead_indicator_gauge.first_frame,
                        Lead_indicator_gauge.num_frames);
    bm_page_in_aabitmap(Energy_bar_gauges.first_frame,
                        Energy_bar_gauges.num_frames);
