...
char Target_view_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = { "targetview1",
                                                                 "targetview1" };
char Target_integ_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = { "targetview2",
                                                                  "targetview2" };
char Target_extra_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = { "targetview3",
                                                                  "targetview3" };
...
int Targetbox_coords[GR_NUM_RESOLUTIONS][NUM_TBOX_COORDS][2] = { { // GR_640
                                                                   { 5, 319 },
                                                                   { 13, 316 },
                                                                   { 13, 325 },
                                                                   { 13, 337 },
...
int Integrity_bar_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                      138, 371, 4, 88 },
                                                    { // GR_1024
                                                      138, 642, 4, 88 } };
int Integrity_string_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                         112, 372 },
                                                       { // GR_1024
                                                         112, 643 } };
...
char *
hud_targetbox_truncate_subsys_name(char *outstr)
{
    if (Lcl_gr) {
...
void
hud_targetbox_init_flash_timer(int index)
{
...
hud_targetbox_maybe_flash(int index, int flash_fast)
{
    ...
    if (!timestamp_elapsed(Targetbox_flash_timers[index][0])) {
        if (timestamp_elapsed(Targetbox_flash_timers[index][1])) {
            if (flash_fast) {
                Targetbox_flash_timers[index][1] = timestamp(
                    fl2i(TBOX_FLASH_INTERVAL / 2.0f));
            }
            else {
                Targetbox_flash_timers[index][1] = timestamp(TBOX_FLASH_INTERVAL);
            }
            Targetbox_flash_flags ^=
...
hud_targetbox_init()
{
    if (!Target_view_gauge_loaded) {
        Target_view_gauge.first_frame = bm_load_animation(
            Target_view_fname[gr_screen.res], &Target_view_gauge.num_frames);
    ...
    if (!Target_view_integrity_gauge_loaded) {
        Target_view_integrity_gauge.first_frame = bm_load_animation(
            Target_integ_fname[gr_screen.res],
            &Target_view_integrity_gauge.num_frames);
    ...
    if (!Target_view_extra_loaded) {
        Target_view_extra.first_frame = bm_load_animation(
            Target_extra_fname[gr_screen.res], &Target_view_extra.num_frames);
...
hud_render_target_background()
{
    ...
    GR_AABITMAP(Target_view_gauge.first_frame,
                Targetbox_coords[gr_screen.res][TBOX_BACKGROUND][0],
...
//
void
hud_render_target_setup(vector *camera_eye, matrix *camera_orient, float zoom)
{
    ...
    HUD_set_clip(Target_window_coords[gr_screen.res][0],
...
hud_blit_target_foreground()
{
    ...
    GR_AABITMAP(Target_view_gauge.first_frame + 1,
                Targetbox_coords[gr_screen.res][TBOX_BACKGROUND][0],
                Targetbox_coords[gr_screen.res][TBOX_BACKGROUND][1]);
}
...
void
hud_targetbox_show_extra_ship_info(ship *target_shipp, ai_info *target_aip)
{
    ...
    if (not_training && (hud_gauge_active(HUD_TARGET_MONITOR_EXTRA_DATA)) &&
        ...
        if ((Player_ship->team == target_shipp->team) &&
            !(ship_get_SIF(target_shipp) & SIF_NOT_FLYABLE)) {
            extra_data_shown = 1;
            ...
            emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_EXTRA_ORDERS][0],
                           Targetbox_coords[gr_screen.res][TBOX_EXTRA_ORDERS][1],
                           EG_TBOX_EXTRA1, outstr);
        }
        ...
        if (has_orders) {
            ...
            if (ship_return_time_to_goal(tmpbuf, target_shipp)) {
                ...
                emp_hud_string(
                    Targetbox_coords[gr_screen.res][TBOX_EXTRA_TIME][0],
                    Targetbox_coords[gr_screen.res][TBOX_EXTRA_TIME][1],
                    EG_TBOX_EXTRA2, outstr);
            }
        }
    }
    ...
    if (target_aip->ai_flags & AIF_DOCKED) {
        if (target_aip->dock_objnum >= 0) {
            sprintf(outstr, XSTR("Docked: %s", 339),
                    Ships[Objects[target_aip->dock_objnum].instance].ship_name);
            ...
            emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_EXTRA_DOCK][0],
                           Targetbox_coords[gr_screen.res][TBOX_EXTRA_DOCK][1],
    ...
    if (extra_data_shown) {
        ...
        GR_AABITMAP(Target_view_extra.first_frame,
                    Targetbox_coords[gr_screen.res][TBOX_EXTRA][0],
...
void
hud_render_target_jump_node(object *target_objp)
{
    char outstr[256];
    ...
    emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_NAME][0],
                   Targetbox_coords[gr_screen.res][TBOX_NAME][1], EG_TBOX_NAME,
                   Jump_nodes[target_objp->instance].name);
    ...
    emp_hud_printf(Targetbox_coords[gr_screen.res][TBOX_DIST][0] + hx,
                   Targetbox_coords[gr_screen.res][TBOX_DIST][1] + hy,
                   EG_TBOX_DIST, outstr);
}
...
hud_render_target_asteroid(object *target_objp)
{
    ...
    if (Detail.targetview_model) {
        ...
        model_render(Asteroid_info[asteroidp->type].model_num[subtype],
                     &target_objp->orient, &obj_pos,
                     MR_NO_LIGHTING | MR_LOCK_DETAIL);
        hud_render_target_close();
    }
    ...
    emp_hud_printf(Targetbox_coords[gr_screen.res][TBOX_NAME][0],
                   Targetbox_coords[gr_screen.res][TBOX_NAME][1], EG_TBOX_NAME,
                   hud_name);

    time_to_impact = asteroid_time_to_impact(target_objp);
    if (time_to_impact >= 0) {
        emp_hud_printf(Targetbox_coords[gr_screen.res][TBOX_CLASS][0],
...
get_turret_subsys_name(model_subsystem *system_info, char *outstr)
{
    ...
    if (system_info->turret_weapon_type >= 0) {
        ...
        else if (Weapon_info[system_info->turret_weapon_type].wi_flags &
                 WIF_BEAM) {
            sprintf(outstr, "%s", XSTR("Beam turret", 1567));
        }
        else {
            if (Weapon_info[system_info->turret_weapon_type].subtype ==
                WP_LASER) {
                sprintf(outstr, "%s", XSTR("Laser turret", 1568));
            }
            else if (Weapon_info[system_info->turret_weapon_type].subtype ==
                     WP_MISSILE) {
...
//
void
hud_render_target_ship_info(object *target_objp)
{
    ship *target_shipp;
    ...
    emp_hud_printf(Targetbox_coords[gr_screen.res][TBOX_CLASS][0],
                   Targetbox_coords[gr_screen.res][TBOX_CLASS][1], EG_TBOX_CLASS,
                   temp_name);
    ...
    emp_hud_printf(Targetbox_coords[gr_screen.res][TBOX_HULL][0] - w,
                   Targetbox_coords[gr_screen.res][TBOX_HULL][1], EG_TBOX_HULL,
                   "%s", outstr);
    hud_set_gauge_color(HUD_TARGET_MONITOR);
    ...
    if (Player_ai->targeted_subsys != NULL) {
        shield_strength = Player_ai->targeted_subsys->current_hits /
                          Player_ai->targeted_subsys->system_info->max_hits *
                          100.0f;
        ...
        if (Player_ai->targeted_subsys->system_info->type == SUBSYSTEM_TURRET) {
            get_turret_subsys_name(Player_ai->targeted_subsys->system_info,
                                   outstr);
        }
        else {
        ...
        gr_printf(Target_window_coords[gr_screen.res][0] + 2,
                  Target_window_coords[gr_screen.res][1] +
        ...
        if (strnicmp(NOX("fighter"),
            ...
            sprintf(outstr, XSTR("%d%%", 341), screen_integrity);
            gr_get_string_size(&w, &h, outstr);
            gr_printf(Target_window_coords[gr_screen.res][0] +
                          Target_window_coords[gr_screen.res][2] - w - 1,
                      Target_window_coords[gr_screen.res][1] +
                          Target_window_coords[gr_screen.res][3] - h,
                      "%s", outstr);
        }
    ...
    if ((target_shipp->flags & SF_DISABLED) ||
        ...
        gr_get_string_size(&w, &h, outstr);
        gr_printf(Target_window_coords[gr_screen.res][0] +
                      Target_window_coords[gr_screen.res][2] / 2 - w / 2 - 1,
                  Target_window_coords[gr_screen.res][1] +
                      Target_window_coords[gr_screen.res][3] - 2 * h,
                  "%s", outstr);
    }
...
void
hud_blit_target_integrity(int disabled, int force_obj_num)
{
    object *objp;
    int clip_h, w, h;
    char buf[16];
    ...
    if (disabled) {
        GR_AABITMAP(Target_view_integrity_gauge.first_frame,
                    Integrity_bar_coords[gr_screen.res][0],
    ...
    clip_h = fl2i((1 - Pl_target_integrity) *
                  Integrity_bar_coords[gr_screen.res][3]);

    // print out status of ship
    if ((Ships[objp->instance].flags & SF_DISABLED) ||
        (ship_subsys_disrupted(&Ships[objp->instance], SUBSYSTEM_ENGINE))) {
    ...
    emp_hud_string(Integrity_string_coords[gr_screen.res][0],
                   Integrity_string_coords[gr_screen.res][1], EG_TBOX_INTEG, buf);
    ...
    if (clip_h > 0) {
        ...
        GR_AABITMAP_EX(Target_view_integrity_gauge.first_frame,
                       Integrity_bar_coords[gr_screen.res][0],
    ...
    if (clip_h <= Integrity_bar_coords[gr_screen.res][3]) {
        ...
        GR_AABITMAP_EX(Target_view_integrity_gauge.first_frame + 1,
                       Integrity_bar_coords[gr_screen.res][0],
                       Integrity_bar_coords[gr_screen.res][1] + clip_h, w,
                       h - clip_h, 0, clip_h);
    }
}
...
int
hud_targetbox_subsystem_in_view(object *target_objp, int *sx, int *sy)
{
    ship_subsys *subsys;
    vector subobj_pos;
    ...
    if (subsys != NULL) {
        vm_vec_unrotate(&subobj_pos, &subsys->system_info->pnt,
                        &target_objp->orient);
        vm_vec_add2(&subobj_pos, &target_objp->pos);

        // is it subsystem in view
        if (Player->subsys_in_view == -1) {
            rval = ship_subsystem_in_sight(target_objp, subsys, &View_position,
                                           &subobj_pos, 0);
        }
        else {
...
void
hud_maybe_render_cargo_scan(ship_info *target_sip)
{
    int x1, y1, x2, y2;
    ...
    y1 = fl2i(0.5f + Cargo_scan_coords[gr_screen.res][1] +
              ((i2fl(Player->cargo_inspect_time) / scan_time) *
    ...
    x1 = fl2i(0.5f + Cargo_scan_coords[gr_screen.res][0] +
              ((i2fl(Player->cargo_inspect_time) / scan_time) *
...
hud_targetbox_get_eye(vector *eye_pos, matrix *orient, int ship_num)
{
    ...
    model_find_world_point(eye_pos, &ep->pnt, shipp->modelnum, ep->parent, orient,
                           &origin);
}
...
//
void
hud_render_target_ship(object *target_objp)
{
    vector obj_pos = { 0.0f, 0.0f, 0.0f };
    vector camera_eye = { 0.0f, 0.0f, 0.0f };
    matrix camera_orient = IDENTITY_MATRIX;
    ...
    if (Detail.targetview_model) {
        ...
        if (target_sip->modelnum_hud >= 0) {
            model_render(target_sip->modelnum_hud, &target_objp->orient, &obj_pos,
                         MR_NO_LIGHTING | MR_LOCK_DETAIL | MR_AUTOCENTER);
        }
        else {
            model_render(target_sip->modelnum, &target_objp->orient, &obj_pos,
                         MR_NO_LIGHTING | MR_LOCK_DETAIL | MR_AUTOCENTER);
        }
        ...
        if (Player_ai->targeted_subsys == Player_ai->last_subsys_target) {
            ...
            subsys_in_view = hud_targetbox_subsystem_in_view(target_objp, &sx,
                                                             &sy);
            ...
            if (subsys_in_view != -1) {
                ...
                if (subsys_in_view) {
                    draw_brackets_square_quick(sx - 10, sy - 10, sx + 10,
                                               sy + 10);
                }
                else {
                    draw_brackets_diamond_quick(sx - 10, sy - 10, sx + 10,
                                                sy + 10);
                }
            }
        }
...
//
void
hud_render_target_debris(object *target_objp)
{
    vector obj_pos = { 0.0f, 0.0f, 0.0f };
    vector camera_eye = { 0.0f, 0.0f, 0.0f };
    matrix camera_orient = IDENTITY_MATRIX;
    ...
    if (Detail.targetview_model) {
        ...
        submodel_render(debrisp->model_num, debrisp->submodel_num,
                        &target_objp->orient, &obj_pos,
                        MR_NO_LIGHTING | MR_LOCK_DETAIL);
        hud_render_target_close();
    }
    ...
    emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_CLASS][0],
                   Targetbox_coords[gr_screen.res][TBOX_CLASS][1], EG_TBOX_CLASS,
...
//
void
hud_render_target_weapon(object *target_objp)
{
    vector obj_pos = { 0.0f, 0.0f, 0.0f };
    vector camera_eye = { 0.0f, 0.0f, 0.0f };
    matrix camera_orient = IDENTITY_MATRIX;
    ...
    if (Detail.targetview_model) {
        ...
        model_render(viewed_model_num, &viewed_obj->orient, &obj_pos,
                     MR_NO_LIGHTING | MR_LOCK_DETAIL);
        hud_render_target_close();
    }
...
void
hud_cargo_scan_update(object *targetp, float frametime)
{
    ...
    if (targetp->type == OBJ_SHIP) {
        if (player_inspect_cargo(frametime, outstr)) {
            if (hud_gauge_active(HUD_TARGET_MONITOR)) {
                ...
                emp_hud_string(
                    Targetbox_coords[gr_screen.res][TBOX_CARGO][0] + hx,
                    Targetbox_coords[gr_screen.res][TBOX_CARGO][1] + hy,
                    EG_TBOX_CARGO, outstr);
                hud_set_gauge_color(HUD_TARGET_MONITOR);
            }
        }
    } // end if (is_ship)
...
//
void
hud_show_target_data(float frametime)
{
    char outstr[256]; // temp buffer for sprintf'ing hud output
    ...
    sprintf(outstr, XSTR("d: %.0f%s", 350), Player_ai->current_target_distance,
            modifiers[Player_ai->current_target_dist_trend]);

    hud_num_make_mono(outstr);
    gr_get_string_size(&w, &h, outstr);

    emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_DIST][0] + hx,
                   Targetbox_coords[gr_screen.res][TBOX_DIST][1] + hy,
                   EG_TBOX_DIST, outstr);
    ...
    sprintf(outstr, XSTR("s: %.0f%s", 351), spd,
            (spd > 1) ? modifiers[Player_ai->current_target_speed_trend] : "");
    hud_num_make_mono(outstr);

    emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_SPEED][0] + hx,
                   Targetbox_coords[gr_screen.res][TBOX_SPEED][1] + hy,
                   EG_TBOX_SPEED, outstr);
    ...
    if (Show_target_debug_info && (is_ship == 1)) {
        ...
        if (shipp->ai_index >= 0) {
            ...
            gr_printf(sx, sy, "Max speed = %d, (%d%%)",
                      (int)shipp->current_max_speed,
                      (int)(100.0f * vm_vec_mag(&target_objp->phys_info.vel) /
            ...
            if (aip->target_objnum != -1) {
                ...
                dist = vm_vec_dist_quick(&Objects[Player_ai->target_objnum].pos,
                                         &Objects[aip->target_objnum].pos);
                vm_vec_normalized_dir(&v2t, &Objects[aip->target_objnum].pos,
                                      &Objects[Player_ai->target_objnum].pos);

                dot = vm_vec_dot(&v2t,
                                 &Objects[Player_ai->target_objnum].orient.fvec);
            ...
            sprintf(outstr, "MAX G/E: %.0f/%.0f", shipp->weapon_energy,
                    shipp->current_max_speed);
            gr_printf(sx, sy, outstr);
            sy += dy;

            sprintf(outstr, "G/S/E: %.2f/%.2f/%.2f",
                ...
                if (Enemy_attacker != NULL)
                    if (Enemy_attacker->type == OBJ_SHIP) {
                        ...
                        if (eaip->target_objnum == Player_obj - Objects) {
                            found = 1;
                            dist = vm_vec_dist_quick(&Enemy_attacker->pos,
                                                     &Player_obj->pos);
                    ...
                    for (i = 0; i < MAX_OBJECTS; i++)
                        if (Objects[i].type == OBJ_SHIP) {
                            ...
                            if (i != Player_ai->target_objnum) {
                                enemy =
                                    Ai_info[Ships[Objects[i].instance].ai_index]
                                        .target_objnum;
    ...
    if (Show_target_weapons && (is_ship == 1)) {
        ...
        for (i = 0; i < swp->num_secondary_banks; i++) {
            sprintf(outstr, "%d. %s", i + 1,
                    Weapon_info[swp->secondary_bank_weapons[i]].name);
            gr_printf(sx, sy, outstr);
            sy += dy;
        }
    }
...
int
hud_targetbox_static_maybe_blit(float frametime)
{
    float sensors_str;
...
void
hud_targetbox_end_flash(int index)
{
    Targetbox_flash_timers[index][0] = 1;
}
...
int
hud_targetbox_is_bright(int index)
{
...
int
hud_targetbox_flash_expired(int index)
{
    if (timestamp_elapsed(Targetbox_flash_timers[index][0])) {
        return 1;
    }
...
{
    bm_page_in_aabitmap(Target_view_gauge.first_frame,
                        Target_view_gauge.num_frames);

    bm_page_in_aabitmap(Target_view_integrity_gauge.first_frame,
                        Target_view_integrity_gauge.num_frames);

    bm_page_in_aabitmap(Target_view_extra.first_frame,
                        Target_view_extra.num_frames);
}
