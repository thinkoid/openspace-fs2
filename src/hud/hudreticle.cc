...
int Hud_throttle_frame_bottom_y[GR_NUM_RESOLUTIONS] = { 325, 520 };
int Hud_throttle_h[GR_NUM_RESOLUTIONS] = { 50, 80 };
int Hud_throttle_bottom_y[GR_NUM_RESOLUTIONS] = { 307, 491 };
...
static int
    Threat_dumbfire_timer; // timestamp for when to show next flashing frame for dumbfire threat
static int
    Threat_lock_timer; // timestamp for when to show next flashing frame for lock threat

static int
    Threat_dumbfire_frame; // frame offset of current dumbfire flashing warning
static int Threat_lock_frame; // frame offset of current lock flashing warning

// coordinates
static int Max_speed_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                         236, 254 },
                                                       { // GR_1024
                                                         377, 406 } };
static int Zero_speed_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                          252, 303 },
                                                        { // GR_1024
                                                          403, 485 } };
...
hud_init_reticle()
{
    ...
    for (i = 0; i < NUM_RETICLE_ANIS; i++) {
        ...
        hfp->first_frame = bm_load_animation(
            Reticle_frame_names[gr_screen.res][i], &hfp->num_frames);
...
void
hud_render_throttle_background(int y_end)
{
    int x, y, w, h;
    ...
    if (y_end > y) {
        GR_AABITMAP_EX(Reticle_gauges[RETICLE_LEFT_ARC].first_frame + 1, x, y, w,
                       y_end - y + 1, 0, 0);
    }
}
...
void
hud_render_throttle_foreground(int y_end)
{
    int x, y, w, h;
    ...
    if (y_end < Hud_throttle_frame_bottom_y[gr_screen.res]) {
        GR_AABITMAP_EX(Reticle_gauges[RETICLE_LEFT_ARC].first_frame + 2, x, y_end,
                       w, h - (y_end - y), 0, y_end - y);
    }
}

// Draw the throttle speed number
void
hud_render_throttle_speed(float current_speed, int y_end)
{
    char buf[32];
    ...
    x_pos = (int)sqrt(double(Outer_circle_radius[gr_screen.res] *
...
void
hud_render_throttle_line(int y)
{
    // hud_set_bright_color();
    hud_set_gauge_color(HUD_THROTTLE_GAUGE, HUD_C_BRIGHT);

    GR_AABITMAP_EX(Reticle_gauges[RETICLE_LEFT_ARC].first_frame + 3,
                   Reticle_frame_coords[gr_screen.res][RETICLE_LEFT_ARC][0], y,
                   Hud_throttle_frame_w[gr_screen.res], 1, 0,
                   y - Reticle_frame_coords[gr_screen.res][RETICLE_LEFT_ARC][1]);
}
...
hud_show_throttle()
{
    ...
    if (desired_speed <
        0.0f) { // so ships that go backwards don't force the indicators below where they can go
        desired_speed = 0.0f;
    }

    desired_y_pos =
        Hud_throttle_bottom_y[gr_screen.res] -
        fl2i(Hud_throttle_h[gr_screen.res] * desired_speed / max_speed + 0.5f) -
        1;
    ...
    if (percent_max > 1) {
        ...
        percent_aburn_max = (current_speed - max_speed) /
                            (sip->afterburner_max_vel.z - max_speed);
    ...
    y_end = Hud_throttle_bottom_y[gr_screen.res] -
            fl2i(Hud_throttle_h[gr_screen.res] * percent_max + 0.5f);
    if (percent_aburn_max > 0) {
        y_end -= fl2i(percent_aburn_max * Hud_throttle_aburn_h[gr_screen.res] +
                      0.5f);
    }
    ...
    gr_printf(Max_speed_coords[gr_screen.res][0],
              Max_speed_coords[gr_screen.res][1], "%d", fl2i(max_speed));
    gr_printf(Zero_speed_coords[gr_screen.res][0],
              Zero_speed_coords[gr_screen.res][1], XSTR("0", 292));
}
...
hud_show_lock_threat()
{
    ...
    GR_AABITMAP(Reticle_gauges[RETICLE_LOCK_WARN].first_frame + frame_offset,
                Reticle_frame_coords[gr_screen.res][RETICLE_LOCK_WARN][0],
    ...
    if ((frame_offset > 0) && (Player->threat_flags & THREAT_LOCK)) {
        if (hud_targetbox_flash_expired(TBOX_FLASH_CMEASURE)) {
            ...
            hud_show_text_flash_icon(XSTR("Launch", 1507),
                                     Reticle_launch_coords[gr_screen.res][1],
                                     bright);
        }
    }
}
...
hud_show_dumbfire_threat()
{
    ...
    GR_AABITMAP(Reticle_gauges[RETICLE_LASER_WARN].first_frame + frame_offset,
                Reticle_frame_coords[gr_screen.res][RETICLE_LASER_WARN][0],
...
hud_show_center_reticle()
{
    ...
    GR_AABITMAP(Reticle_gauges[RETICLE_CENTER].first_frame,
                Reticle_frame_coords[gr_screen.res][RETICLE_CENTER][0],
...
hud_show_top_arc()
{
    ...
    if (hud_gauge_active(HUD_THREAT_GAUGE)) {
        ...
        GR_AABITMAP(Reticle_gauges[RETICLE_TOP_ARC].first_frame + 1,
                    Reticle_frame_coords[gr_screen.res][RETICLE_TOP_ARC][0],
...
hud_show_right_arc()
{
    ...
    GR_AABITMAP(Reticle_gauges[RETICLE_RIGHT_ARC].first_frame + 1,
                Reticle_frame_coords[gr_screen.res][RETICLE_RIGHT_ARC][0],
...
hud_show_left_arc()
{
    ...
    GR_AABITMAP(Reticle_gauges[RETICLE_LEFT_ARC].first_frame,
                Reticle_frame_coords[gr_screen.res][RETICLE_LEFT_ARC][0],
