...
LOCAL int
    Msg_instance; // variable which holds ship/wing instance to send the message to
...
//#ifndef NDEBUG
LOCAL int
    Msg_enemies; // tells us whether or not to message enemy ships or friendlies
//#endif
...
comm_order Comm_orders[MAX_SHIP_ORDERS] = {
    ATTACK_TARGET_ITEM,   DISABLE_TARGET_ITEM,
    DISARM_TARGET_ITEM,   DISABLE_SUBSYSTEM_ITEM,
    PROTECT_TARGET_ITEM,  IGNORE_TARGET_ITEM,
    FORMATION_ITEM,       COVER_ME_ITEM,
    ENGAGE_ENEMY_ITEM,    CAPTURE_TARGET_ITEM,
    REARM_REPAIR_ME_ITEM, ABORT_REARM_REPAIR_ITEM,
    DEPART_ITEM,
};
...
char *
comm_order_menu_text(int index)
{
    switch (index) {
    case 0:
        return XSTR("Destroy my target", 299);
        break;
    case 1:
        return XSTR("Disable my target", 300);
        break;
    case 2:
        return XSTR("Disarm my target", 301);
        break;
    case 3:
        return XSTR("Destroy subsystem", 302);
        break;
    case 4:
        return XSTR("Protect my target", 303);
        break;
    case 5:
        return XSTR("Ignore my target", 304);
        break;
    case 6:
        return XSTR("Form on my wing", 305);
        break;
    case 7:
        return XSTR("Cover me", 306);
        break;
    case 8:
        return XSTR("Engage enemy", 307);
        break;
    case 9:
        return XSTR("Capture my target", 308);
        break;
    case 10:
        return XSTR("Rearm me", 309);
        break;
    case 11:
        return XSTR("Abort rearm", 310);
        break;
    case 12:
        return XSTR("Depart", 311);
        break;
    default:
        Assert(0);
    }
...
char *
comm_order_hotkey_text(int index)
{
    int i;
...
#define CAN_MESSAGE                                                              \
    (SIF_FIGHTER | SIF_BOMBER | SIF_CRUISER | SIF_FREIGHTER | SIF_TRANSPORT |    \
     SIF_CAPITAL | SIF_SUPPORT | SIF_SUPERCAP | SIF_DRYDOCK | SIF_GAS_MINER |    \
     SIF_AWACS | SIF_CORVETTE)
...
static int Mbox_title_coord[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                         447, 6 },
                                                       { // GR_1024
                                                         829, 6 } };
static int Mbox_item_coord[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                        449, 18 },
                                                      { // GR_1024
                                                        831, 18 } };
...
int hud_squadmsg_is_target_order_valid(int order, int find_order,
                                       ai_info *aip = NULL);
...
*/
    ...
    Msg_mode_timestamp = timestamp(
        DEFAULT_MSG_TIMEOUT); // initialize our timer to bogus value
    Msg_shortcut_command = -1; // assume no shortcut key being used
    Msg_target_objnum =
        Player_ai->target_objnum; // save the players target object number
    Msg_targeted_subsys =
        Player_ai->targeted_subsys; // save the players currently targted subsystem
#ifndef NDEBUG
...
hud_squadmsg_count_fighters()
{
    ...
    for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list);
        ...
        if (shipp->orders_accepted != 0) {
            ...
            if ((shipp->team == team) && (objp != Player_obj) &&
                (Ship_info[shipp->ship_info_index].flags &
...
int
hud_squadmsg_count_ships(int add_to_menu)
{
    int count;
    int team;
    ship *shipp;
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
        ...
        if ((&Objects[so->objnum] == Player_obj) ||
            is_instructor(&Objects[so->objnum]))
            continue;
        ...
        // cannot message player ships
        if (Objects[so->objnum].flags & OF_PLAYER_SHIP)
            continue;
        ...
        if (Msg_shortcut_command != -1) {
            if (!(shipp->orders_accepted & Msg_shortcut_command))
                continue;
            else if (!hud_squadmsg_ship_order_valid(Objects[so->objnum].instance,
...
int
hud_squadmsg_wing_valid(wing *wingp, int team)
{
    ...
    if ((Ships[Player_obj->instance].wingnum == WING_INDEX(wingp)) &&
        (wingp->current_count == 1))
        return 0;
...
int
hud_squadmsg_count_wings(int add_to_menu)
{
    int count, i, j;
    int team;
...
hud_squadmsg_get_key()
{
    ...
    for (i = 0; i < num_keys_used; i++) {
        if (k == keys_used[i]) {
            ...
            if (k ==
                KEY_PAGEDOWN) { // pageup and pagedown scroll the menu -- deal with these seperately!!
                hud_squadmsg_page_down();
                return -1;
            }
                ...
                return -1;
                     ...
                     (Squad_msg_mode ==
                      SM_MODE_REINFORCEMENTS)) // return any key if selecting reinforcement
                return i;
            ...
            // play general fail sound if inactive item hit.
                ...
                gamesnd_play_iface(SND_GENERAL_FAIL);
                     ...
                     (MsgItems[i]
                          .active)) // only return keys that are associated with menu items
                return i;

            else {
                Msg_key_used =
                    0; // if no #-key pressed for visible item, break and allow timer to
...
void
hud_squadmsg_display_menu(char *title)
{
    int bx, by, sx, sy, i, nitems, none_valid;

    // hud_set_bright_color();
    hud_set_gauge_color(HUD_MESSAGE_BOX, HUD_C_BRIGHT);
    if (title) {
        gr_string(Mbox_title_coord[gr_screen.res][0],
                  Mbox_title_coord[gr_screen.res][1], title);
    }
    ...
    if (Num_menu_items < MAX_MENU_DISPLAY)
        ...
        else if ((Num_menu_items - First_menu_item) <=
    ...
    bx = Mbox_bmap_coords[gr_screen.res]
                         [0]; // global x-offset where bitmap gets drawn
    by = Mbox_bmap_coords[gr_screen.res]
                         [1]; // global y-offset where bitmap gets drawn

    none_valid =
        1; // variable to tell us whether all items in the menu are valid or not
    ...
    for (i = 0; i < nitems; i++) {
        ...
        // then the text
        emp_hud_string(sx + Mbox_item_xoffset[gr_screen.res], sy, EG_SQ1 + i,
                       text);
    ...
    // display
    if (none_valid || ((First_menu_item + nitems) < Num_menu_items) ||
    ...
    if (First_menu_item > 0) {
        gr_printf(Menu_pgup_coords[gr_screen.res][0],
                  Menu_pgup_coords[gr_screen.res][1], XSTR("[pgup]", 312));
    }

    if ((First_menu_item + nitems) < Num_menu_items) {
        gr_printf(Menu_pgdn_coords[gr_screen.res][0],
                  Menu_pgdn_coords[gr_screen.res][1], XSTR("[pgdn]", 313));
    }

    if (none_valid) {
        gr_printf(sx, by - Mbox_item_h[gr_screen.res] + 2,
                  XSTR("No valid items", 314));
    }
    else if (!none_valid && (Msg_shortcut_command != -1)) {
        gr_printf(sx, by - Mbox_item_h[gr_screen.res] + 2, "%s",
                  comm_order_hotkey_text(Msg_shortcut_command));
    }
}

// function to return true or false if the given ship can rearm, or be repaired
int
hud_squadmsg_can_rearm(ship *shipp)
{
...
void
hud_squadmsg_repair_rearm(int toggle_state)
{
    int robjnum;
    object *robjp;
    object *tobj;
    ...
    if (is_support_allowed(tobj)) {
        if (mission_is_repair_scheduled(tobj)) {
            message_send_builtin_to_player(MESSAGE_REARM_ON_WAY, NULL,
            ...
            if (robjnum != -1) {
                message_send_builtin_to_player(MESSAGE_REARM_ON_WAY,
                                               &Ships[Objects[robjnum].instance],
                                               MESSAGE_PRIORITY_NORMAL,
                                               MESSAGE_TIME_SOON, 0, 0);
            }
            else {
                ...
                if (robjnum != -1) {
                    ...
                    message_send_builtin_to_player(MESSAGE_ON_WAY,
                                                   &Ships[robjp->instance],
                                                   MESSAGE_PRIORITY_NORMAL,
                                                   MESSAGE_TIME_SOON, 0, 0);
                }
                else {
                    ...
                    message_send_builtin_to_player(MESSAGE_REARM_WARP, NULL,
                                                   MESSAGE_PRIORITY_NORMAL,
                                                   MESSAGE_TIME_SOON, 0, 0);
                }

                mission_log_add_entry(LOG_PLAYER_REARM,
...
int
hud_squadmsg_is_target_order_valid(int order, int find_order, ai_info *aip)
{
    int target_objnum, i;
    ...
    if (objp->type == OBJ_WEAPON) {
        if ((Comm_orders[order].value == ATTACK_TARGET_ITEM) &&
            (Weapon_info[Weapons[objp->instance].weapon_info_index].wi_flags &
             WIF_BOMB) &&
    ...
    if ((Squad_msg_mode == SM_MODE_SHIP_COMMAND) &&
        (target_objnum == Msg_instance)) {
        return 0;
    }

    // if the order is a disable order or depart, and the ship is disabled, order isn't active
    if ((Comm_orders[order].value == DISABLE_TARGET_ITEM) &&
        (shipp->flags & SF_DISABLED)) {
        return 0;
    }

    // same as above except for disabled.
    if ((Comm_orders[order].value == DISARM_TARGET_ITEM) &&
        ((shipp->subsys_info[SUBSYSTEM_TURRET].num > 0) &&
         (shipp->subsys_info[SUBSYSTEM_TURRET].current_hits == 0.0f))) {
        return 0;
    }

    // if order is disable subsystem, and no subsystem targeted or no hits, then order not valid
    if ((Comm_orders[order].value == DISABLE_SUBSYSTEM_ITEM) &&
    ...
    if ((shipp->team == ordering_shipp->team) &&
        (Comm_orders[order].value & FRIENDLY_TARGET_MESSAGES)) {
        return 1;
    }
    else if ((shipp->team != ordering_shipp->team) &&
             (Comm_orders[order].value & ENEMY_TARGET_MESSAGES)) {
        return 1;
    }
    else {
        return 0;
...
hud_squadmsg_send_to_all_fighters(int command)
{
    ...
    if (command == IGNORE_TARGET_ITEM) {
        ...
        if ((Msg_instance != MESSAGE_ALL_FIGHTERS) &&
            (Squad_msg_mode == SM_MODE_SHIP_COMMAND)) {
            do_ship = 1;
            send_message = 0;
        }
    }
    ...
    for (i = 0; i < num_wings; i++) {
        ...
        if (!to_everyone && !(Ship_info[shipp->ship_info_index].flags &
                              (SIF_FIGHTER | SIF_BOMBER)))
            continue;
        ...
        if (Wings[i].current_count > 1) {
            if (hud_squadmsg_send_wing_command(i, command, send_message)) {
                send_message = 0;
            }
        }
    }
    ...
    for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list);
        ...
        if (hud_squadmsg_send_ship_command(objp->instance, command,
                                           send_message)) {
            send_message = 0;
        }
    }
...
#define OVERRIDE_PROTECT_SHIP_TYPE                                               \
    (SIF_FIGHTER | SIF_BOMBER | SIF_FREIGHTER | SIF_TRANSPORT)
...
int
hud_squadmsg_send_ship_command(int shipnum, int command, int send_message)
{
    ai_info *ainfo;
    ...
    int message;
    int target_team,
        ship_team; // team id's for the ship getting message and any target the player has
    ship *ordering_shipp;
    ...
    if (hud_squadmsg_is_target_order_valid(command, 1, ainfo)) {
        ...
        case ATTACK_TARGET_ITEM:
            if (Objects[ainfo->target_objnum].type == OBJ_SHIP) {
                ...
                if (Ship_info[Ships[Objects[ainfo->target_objnum].instance]
                                  .ship_info_index]
                        .flags &
                    OVERRIDE_PROTECT_SHIP_TYPE) {
                ...
                ai_submode =
                    Objects[ainfo->target_objnum]
                        .instance; // store the instance of the weapon -- ai goals code will deal with it
            }
            else
                Int3();
    ...
    if (send_message) {
        message_send_builtin_to_player(message, &Ships[shipnum],
                                       MESSAGE_PRIORITY_NORMAL,
...
int
hud_squadmsg_send_wing_command(int wingnum, int command, int send_message)
{
    ai_info *ainfo;
    int ai_mode, ai_submode; // ai mode and submode needed for ship commands
    char *target_shipname; // ship number of possible targets
    ...
    if (hud_squadmsg_is_target_order_valid(command, 1, ainfo)) {
        ...
        if (ai_mode != AI_GOAL_NONE) {
            Assert(ai_submode != -1234567);
            ai_add_wing_goal_player(AIG_TYPE_PLAYER_WING, ai_mode, ai_submode,
                                    target_shipname, wingnum);
        }
    }
    ...
    if (send_message) {
        ...
        // its possible that all ships in a wing are players. so we'll just send from a random ship
        if (ship_num == -1) {
        ...
        if (ship_num != -1) {
            message_send_builtin_to_player(message, &Ships[ship_num],
                                           MESSAGE_PRIORITY_NORMAL,
                                           MESSAGE_TIME_ANYTIME, 0, 0);
            message_sent = 1;
        }
...
hud_squadmsg_type_select()
{
    ...
    if (Ai_info[Ships[Player_obj->instance].ai_index].ai_flags &
        (AIF_AWAITING_REPAIR | AIF_BEING_REPAIRED)) {
...
hud_squadmsg_ship_select()
{
    ...
    if (k != -1) { // if true, we have selected a ship.
        if (Msg_shortcut_command == -1) {
            ...
            hud_squadmsg_send_ship_command(MsgItems[First_menu_item + k].instance,
                                           Msg_shortcut_command, 1);
            hud_squadmsg_toggle();
        }
    }
...
hud_squadmsg_wing_select()
{
    ...
    if (k != -1) { // if true, we have selected a ship.
        if (Msg_shortcut_command ==
            ...
            Assert(Msg_shortcut_command != IGNORE_TARGET_ITEM);
            hud_squadmsg_send_wing_command(MsgItems[First_menu_item + k].instance,
                                           Msg_shortcut_command, 1);
            hud_squadmsg_toggle();
        }
    }
...
// called to actually bring in a reinforcement.
void
hud_squadmsg_call_reinforcement(int reinforcement_num)
{
    ...
    // increment the number of times this is used.
    rp->num_uses++;
...
hud_squadmsg_ship_command()
{
    ...
    for (i = 0; i < MAX_SHIP_ORDERS; i++) {
        ...
        if (default_orders & Comm_orders[i].value) {
            ...
            if ((Msg_instance != MESSAGE_ALL_FIGHTERS) &&
                !hud_squadmsg_ship_order_valid(Msg_instance,
                                               Comm_orders[i].value))
                MsgItems[Num_menu_items].active = 0;
            ...
            if (Msg_instance == MESSAGE_ALL_FIGHTERS) {
                ...
                ship *shipp;
                int partial_accept,
                    all_accept; // value which tells us what to do with menu item
                ...
                for (so = GET_FIRST(&Ship_obj_list);
                    ...
                    all_accept &=
                        shipp->orders_accepted; // 'and'ing will either keep this bit set or zero it properly
                    partial_accept |=
                        (shipp->orders_accepted &
                         Comm_orders[i]
                             .value); // 'or'ing will tell us if at least one accepts
                }
    ...
    if (k != -1) {
        ...
        if ((Msg_instance != MESSAGE_ALL_FIGHTERS) &&
            (MsgItems[k].instance != IGNORE_TARGET_ITEM))
...
hud_squadmsg_wing_command()
{
    ...
    Num_menu_items = 0;
    orders =
        Ships[wingp->ship_index[0]]
            .orders_accepted; // get the orders that the first ship in the wing will accept
    for (i = 0; i < MAX_SHIP_ORDERS; i++) {
...
void
hud_init_squadmsg(void)
{
    int i;

    if (!Mbox_frames_loaded) {
        for (i = 0; i < NUM_MBOX_FRAMES; i++) {
            Mbox_gauge[i].first_frame = bm_load_animation(
                Mbox_fnames[gr_screen.res][i], &Mbox_gauge[i].num_frames);
...
void
hud_squadmsg_shortcut(int command)
{
    // check if the communications system is capable of sending a message
    if ((hud_communications_state(Player_ship, 1) != COMM_OK) &&
    ...
    if (Player->flags &
        PLAYER_FLAGS_MSG_MODE) // we are already in messaging mode -- maybe do sometime more interesting?
        return;
    hud_squadmsg_toggle();
    ...
    else if (Msg_shortcut_command ==
             IGNORE_TARGET_ITEM) { //  ignoring target applied to all ships
...
hud_squadmsg_do_frame()
{
    ...
    if (Mbox_gauge[0].first_frame >= 0) {
        GR_AABITMAP(Mbox_gauge[0].first_frame, Mbox_top_coords[gr_screen.res][0],
                    Mbox_top_coords[gr_screen.res][1]);
    }
...
void
hud_add_issued_order(char *name, int order, char *target)
{
    ...
    if (target)
        Squadmsg_history[squadmsg_history_index].target = get_parse_name_index(
            target);
    else
...
int
hud_query_order_issued(char *name, char *order, char *target)
{
    int i, o = -1, ship, t;
