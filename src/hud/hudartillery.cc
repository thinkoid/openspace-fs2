...
void
ssm_get_random_start_pos(vector *out, vector *start, matrix *orient,
                         int ssm_index)
{
    vector temp;
...
void
ssm_create(vector *target, vector *start, int ssm_index)
{
    ssm_strike *ssm;
    matrix dir;
    int idx;
    ...
    for (idx = 0; idx < Ssm_info[ssm_index].count; idx++) {
        ssm->sinfo.delay_stamp[idx] = timestamp(
            200 + (int)frand_range(-199.0f, 1000.0f));
        ssm_get_random_start_pos(&ssm->sinfo.start_pos[idx], start, &dir,
                                 ssm_index);
    }
            ...
            if (!moveup->done_flags[idx]) {
                ...
                if (moveup->fireballs[idx] >= 0) {
                    ...
                    if ((1.0f - fireball_lifeleft_percent(
                                    &Objects[moveup->fireballs[idx]])) >= 0.5f) {
                        // get an orientation
                        vector temp;
                        matrix orient;

                        vm_vec_sub(&temp, &moveup->sinfo.target,
                                   &moveup->sinfo.start_pos[idx]);
                        vm_vec_normalize(&temp);
                        vm_vector_2_matrix(&orient, &temp, NULL, NULL);

                        // fire the missile and flash the screen
                        weapon_create(&moveup->sinfo.start_pos[idx], &orient,
                                      si->weapon_info_index, -1, 1, -1, 1);
                ...
                // maybe create his warpin effect
                else if ((moveup->sinfo.delay_stamp[idx] >= 0) &&
                    ...
                    moveup->fireballs[idx] = fireball_create(
                        &moveup->sinfo.start_pos[idx], FIREBALL_WARP_EFFECT, -1,
                        si->warp_radius, 0, &vmd_zero_vector, si->warp_time, 0,
                        &orient);
                }
            }
        }
