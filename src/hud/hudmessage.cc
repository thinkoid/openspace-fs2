...
int ACTIVE_BUFFER_LINES =
    4; // number of HUD messages that can be shown at one time + 1
...
hud_show_msg_window()
{
    ...
    HUD_set_clip(MSG_WINDOW_X_START, MSG_WINDOW_Y_START, MSG_WINDOW_WIDTH,
                 MSG_WINDOW_HEIGHT + 2);
    ...
    if (Hud_list_start != Hud_list_end && !Scroll_needed) {
        ...
        HUD_active_msgs_list[Active_index].total_life = timestamp(
            HUD_MESSAGE_TOTAL_LIFE);
        ...
        if (Scroll_needed) {
            HUD_active_msgs_list[Active_index].y = (ACTIVE_BUFFER_LINES - 1) *
                                                   MSG_WINDOW_FONT_HEIGHT;
            HUD_active_msgs_list[Active_index].target_y =
                HUD_active_msgs_list[Active_index].y - MSG_WINDOW_FONT_HEIGHT;
        }
        else {
            HUD_active_msgs_list[Active_index].y = (ACTIVE_BUFFER_LINES - 2) *
                                                   MSG_WINDOW_FONT_HEIGHT;
            HUD_active_msgs_list[Active_index].target_y =
                HUD_active_msgs_list[Active_index].y;
        }
        ...
        if (!timestamp_elapsed(HUD_active_msgs_list[i].total_life)) {
            if (HUD_active_msgs_list[i].y > HUD_active_msgs_list[i].target_y) {
                ...
                if (timestamp_elapsed(Scroll_time_id)) {
                    ...
                    if (HUD_active_msgs_list[i].y <
                        HUD_active_msgs_list[i].target_y)
                        HUD_active_msgs_list[i].y =
                            HUD_active_msgs_list[i].target_y;
            ...
            if (hud_gauge_active(HUD_MESSAGE_LINES)) {
                if (!(Player->flags & PLAYER_FLAGS_MSG_MODE)) {
                    ...
                    gr_printf(MSG_WINDOW_X_START + HUD_active_msgs_list[i].msg.x -
                                  2,
                              HUD_active_msgs_list[i].y, "%s",
...
//
void
HUD_printf_line(char *text, int source, int time = 0, int x = 0)
{
    Assert(text != NULL);
    ...
    if (Hud_list_end == Hud_list_start) {
        nprintf(("Warning",
                 "HUD ==> Exceeded msg scroll buffer, discarding message %s\n",
                 text));
        Hud_list_end--;
        if (Hud_list_end == 0)
    ...
    if (strlen(text) > MAX_HUD_LINE_LEN - 1) {
        nprintf(
            ("Warning",
             "HUD_printf_line() ==> Following string truncated to %d chars: %s\n",
...
void
HUD_ship_sent_printf(int sh, char *format, ...)
{
    va_list args;
    char tmp[HUD_MSG_LENGTH_MAX];
    int len;
...
void
hud_sourced_print(int source, char *msg)
{
    char *ptr, *str;
    ...
    if (ptr) {
        gr_get_string_size(&sw, NULL, msg,
                           ptr - msg); // get width of the speaker field
    ...
    t = timestamp();
    str = msg;
    while ((ptr = split_str_once(str, MSG_WINDOW_WIDTH - x - 7)) !=
...
void
HUD_add_to_scrollback(char *text, int source)
{
    if (!strlen(text)) {
        nprintf(("Warning",
                 "HUD ==> attempt to print a 0 length string in msg window\n"));
...
//
void
hud_add_line_to_scrollback(char *text, int source, int t, int x, int y,
                           int underline_width)
{
    line_node *new_line;
...
void
hud_add_msg_to_scrollback(char *text, int source, int t)
{
...
void
hud_scrollback_button_pressed(int n)
{
    switch (n) {
...
hud_scrollback_init()
{
    ...
    Ui_window.add_XSTR("Continue", 1069, Buttons[gr_screen.res][ACCEPT_BUTTON].xt,
                       ...
                       &Buttons[gr_screen.res][ACCEPT_BUTTON].button,
                       UI_XSTR_COLOR_PINK);
    Ui_window.add_XSTR("Events", 1070,
                       Buttons[gr_screen.res][SHOW_EVENTS_BUTTON].xt,
                       Buttons[gr_screen.res][SHOW_EVENTS_BUTTON].yt,
                       &Buttons[gr_screen.res][SHOW_EVENTS_BUTTON].button,
                       UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR("Objectives", 1071,
                       Buttons[gr_screen.res][SHOW_OBJS_BUTTON].xt,
                       Buttons[gr_screen.res][SHOW_OBJS_BUTTON].yt,
                       &Buttons[gr_screen.res][SHOW_OBJS_BUTTON].button,
                       UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR("Messages", 1072,
                       Buttons[gr_screen.res][SHOW_MSGS_BUTTON].xt,
                       Buttons[gr_screen.res][SHOW_MSGS_BUTTON].yt,
                       &Buttons[gr_screen.res][SHOW_MSGS_BUTTON].button,
                       UI_XSTR_COLOR_GREEN);
...
void
hud_scrollback_do_frame(float frametime)
{
    int i, k, x, y;
    int font_height = gr_get_font_height();
   ...
   */
    ...
    if (Scrollback_mode == SCROLLBACK_MODE_EVENT_LOG) {
        ...
        mission_log_scrollback(Scroll_offset,
                               Hud_mission_log_list_coords[gr_screen.res][0],
                               Hud_mission_log_list_coords[gr_screen.res][1],
                               Hud_mission_log_list_coords[gr_screen.res][2],
                ...
                if ((node_ptr->source == HUD_SOURCE_HIDDEN) ||
                    ...
                    if ((node_ptr->source == HUD_SOURCE_FAILED) ||
                        ...
                        i = Hud_mission_log_list_coords[gr_screen.res][1] + y +
                            font_height / 2 - 1;
                        gr_circle(Hud_mission_log_list2_coords[gr_screen.res][0] -
                                      6,
                                  i, 5);
                        ...
                        gr_line(
                            ...
                            i, Hud_mission_log_list2_coords[gr_screen.res][0] - 8,
                            i);
                        gr_line(
                            ...
                            i - 2);
                        gr_line(
                            Hud_mission_log_list2_coords[gr_screen.res][0] - 4, i,
    ...
    gr_print_timestamp(Hud_mission_log_time_coords[gr_screen.res][0],
                       Hud_mission_log_time_coords[gr_screen.res][1] -
                           font_height,
                       (int)(f2fl(Missiontime) * 1000));
    gr_string(Hud_mission_log_time2_coords[gr_screen.res][0],
              Hud_mission_log_time_coords[gr_screen.res][1] - font_height,
              XSTR("Current time", 289));
    gr_flip();
}
