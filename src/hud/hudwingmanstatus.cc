...
static int HUD_wingman_flash_duration[HUD_WINGMAN_MAX_WINGS]
                                     [HUD_WINGMAN_MAX_SHIPS_PER_WINGS];
static int HUD_wingman_flash_next[HUD_WINGMAN_MAX_WINGS]
                                 [HUD_WINGMAN_MAX_SHIPS_PER_WINGS];
...
int HUD_wingman_status_single_coords[GR_NUM_RESOLUTIONS][4][2] = {
    {
        // GR_640
        { 589,
          159 }, // where to draw dots 1 - 4 if we have only one wing present (special case)
        { 582, 167 },
        { 596, 167 },
        { 589, 175 },
    },
...
int HUD_wingman_status_name_coords[GR_NUM_RESOLUTIONS][HUD_WINGMAN_MAX_WINGS][2] = {
    {
        // GR_640
        { 459,
          185 }, // duplicated the first item because we only ever display 5 items
        { 459, 185 },
        { 494, 185 },
        { 529, 185 },
        { 564, 185 },
...
int HUD_wingman_status_coords
    [GR_NUM_RESOLUTIONS][HUD_WINGMAN_MAX_WINGS][HUD_WINGMAN_MAX_SHIPS_PER_WINGS]
    [2] = {
...
int
hud_wingman_status_wing_index(char *wing_name)
{
    //XSTR:OFF
...
void
hud_set_wingman_status_dead(int wing_index, int wing_pos)
{
...
void
hud_set_wingman_status_departed(int wing_index, int wing_pos)
{
    Assert(wing_index >= 0 && wing_index < HUD_WINGMAN_MAX_WINGS);
    Assert(wing_pos >= 0 && wing_index < HUD_WINGMAN_MAX_SHIPS_PER_WINGS);
...
void
hud_set_wingman_status_none(int wing_index, int wing_pos)
{
    int i;
...
float
hud_get_ship_hull_percent(int ship_index)
{
    ship_info *sip;
    object *ship_objp;
...
hud_init_wingman_status_gauge()
{
    ...
    if (!Wingman_status_gauge_loaded) {
        for (i = 0; i < HUD_WINGMAN_STATUS_NUM_FRAMES; i++) {
            Wingman_status_frames[i].first_frame = bm_load_animation(
                Wingman_status_filenames[gr_screen.res][i],
                &Wingman_status_frames[i].num_frames);
            if (Wingman_status_frames[i].first_frame == -1) {
                Warning(
                    LOCATION,
                    NOX("Error loading Wingman_status_filenames[gr_screen.res][i]'\n"));
                return;
            }
        }
        ...
        for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
            ...
            if ((wing_index >= 0) && (wing_pos >= 0)) {
                ...
                HUD_wingman_status[wing_index].hull[wing_pos] =
                    hud_get_ship_hull_percent(ship_objp->instance);
...
void
hud_wingman_status_blit_left_frame(int num_wings_to_draw)
{
    int sx, sy, bitmap;
...
void
hud_wingman_status_blit_middle_frame(int num_wings_to_draw)
{
    int sx, sy, bitmap;
    int idx;
...
void
hud_wingman_status_blit_right_frame(int num_wings_to_draw)
{
    int sx, sy, bitmap;
...
void
hud_wingman_status_blit_dots(int wing_index, int screen_index,
        ...
        case HUD_WINGMAN_STATUS_ALIVE:
            ...
            if (HUD_wingman_status[wing_index].hull[i] > 0.5f) {
                ...
                hud_set_gauge_color(HUD_WINGMEN_STATUS,
                                    is_bright ? HUD_C_BRIGHT : HUD_C_NORMAL);
            }
            else {
...
int
hud_wingman_status_wingmen_exist(int num_wings_to_draw)
{
    int i, j, count = 0;
...
void
hud_wingman_status_start_flash(int wing_index, int wing_pos)
{
    HUD_wingman_flash_duration[wing_index][wing_pos] = timestamp(
        TBOX_FLASH_DURATION);
}
...
hud_wingman_status_maybe_flash(int wing_index, int wing_pos)
{
    ...
    if (!timestamp_elapsed(HUD_wingman_flash_duration[wing_index][wing_pos])) {
        if (timestamp_elapsed(HUD_wingman_flash_next[wing_index][wing_pos])) {
            HUD_wingman_flash_next[wing_index][wing_pos] = timestamp(
                TBOX_FLASH_INTERVAL);
...
int
hud_wingman_status_wing_pos(int shipnum, int wing_status_index, wing *wingp)
{
    int i, wing_pos = -1;
...
void
hud_wingman_status_set_index(int shipnum)
{
    int wing_index, wing_pos;
    ship *shipp;
    wing *wingp;
...
hudwingmanstatus_page_in()
{
    int i;
    for (i = 0; i < HUD_WINGMAN_STATUS_NUM_FRAMES; i++) {
        bm_page_in_aabitmap(Wingman_status_frames[i].first_frame,
                            Wingman_status_frames[i].num_frames);
    }
}
